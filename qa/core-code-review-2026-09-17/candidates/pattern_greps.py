...
T1 = ["app.py","config.py","Constants.py","model_capabilities.py","Logging_Config.py","emergency_stop.py","runtime_policy","Chat","Agents","Tools","MCP","DB","LLM_Calls","Event_Handlers","Utils","RAG_Search","UI/Screens/chat_screen.py","UI/Screens/library_screen.py","UI/Screens/settings_screen.py","UI/Screens/personas_screen.py","UI/Console_Modules","UI/Library_Modules","UI/Navigation","UI/MCP_Modules","UI/Persona_Modules","Widgets"]
files = []
for t in T1:
    p = pkg / t
    files += [p] if p.is_file() else [f for f in p.rglob("*.py") if "Splash_Screens" not in f.parts]
