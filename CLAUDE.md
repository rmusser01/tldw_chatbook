...
4. Pick a `[tools] <name>_enabled` gate key -- it defaults to OFF, and the Settings ▸ Tools screen derives its switches from that same table. Caveat: a config-gated `LocalToolSpec` tool that isn't a `_GATEABLE_BUILTINS` entry (e.g. `web_deep_search`, `Agents/local_tool_provider.py`) doesn't get a switch for free -- it needs a manual row hand-added to `UI/Tools_Settings_Window.py`'s `_compose_tool_settings`/save/reset (see `WEB_DEEP_SEARCH_GATE_KEY`). Also: that whole Tool Settings surface is currently nav-unreachable in the running app (`ToolsSettingsWindow` carries a `DEPRECATED (TASK-1346)` banner and its route resolves to a different screen) -- tracked as task-3240.
