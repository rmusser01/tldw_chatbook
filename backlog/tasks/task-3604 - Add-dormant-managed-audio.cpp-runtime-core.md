...
<!-- SECTION:NOTES:BEGIN -->
Implemented the dormant Managed audio.cpp runtime behind the native adapter: strict active-mode launch validation, one app-scoped exact-child supervisor, generation-bound HTTP and health evidence, latest-wins staged configuration, deliberate lazy start and lifecycle APIs, shared-deadline shutdown, bounded private diagnostics, and complete-WAV delivery with no Slice 5 UI. Hardened concurrency and privacy during review, including save/start/restart races, exit and output-failure publication, in-flight catalog and health fencing, retained cleanup, shutdown deadline adoption, stable error normalization, and immutable running launch snapshots. Core files are TTS_Generation.py, adapters/audio_cpp.py, audio_cpp_supervisor.py, with focused adapter, supervisor, managed-integration, and request-admission regressions; the module guide and live-verification lesson were updated. Verification: affected surface 455 passed; full Tests/TTS 2492 passed, 16 skipped, 13 warnings; focused cross-surface suite 1018 passed with one exact unchanged origin/dev baseline failure in test_first_time_audio_cpp_setup_lab_generation_and_console_handoff caused by its fake service omitting response_format, previously reproduced on origin/dev 6b4ccf475. Ruff check and format passed for 28 files, compileall passed, mypy passed for 11 source files, stable operation-code and boundary checks passed, cumulative origin/dev diff checks passed, and independent review found no Critical or Important production issues. ADR check: implementation conforms to ADR-023 and ADR-039 plus the approved managed-lifecycle spec; no new ADR was required because no additional architectural boundary was introduced.
<!-- SECTION:NOTES:END -->
