...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Sources provides multiline/bulk entry with persistent labels, row-level validation/duplicate feedback, draft preservation, and an explicit partial-result decision.
- [x] #2 Users can keyboard-select multiple sources, understand filtered/select-all/range semantics, and create a Watchlist from the selected set without repeated membership dialogs.
- [x] #3 Implemented shortcuts are discoverable in valid footer/command-palette hints; primary meanings never depend on tooltips or color alone.
- [x] #4 Artifacts foregrounds Generate/Schedule when empty and moves downstream actions into selected-briefing context or a labeled disclosure.
- [x] #5 The collection automation receipt shows interval, app-open limitation, next eligibility, last attempt/success, reload state, and attention/recovery state.
- [x] #6 Briefing refresh/generation retains the last good table/body, shows inline progress, preserves content on failure with Retry, and provides a recoverable reload diagnostic when durable storage and the pane disagree.
- [x] #7 Production-shaped Textual tests cover first-time and power-user paths at the supported 160x42 pressure point and a normal size, including focus order, Escape, draft preservation, stale/error states, and receipt deep-links.
<!-- AC:END -->
...
Three independent review rounds closed scoped-selection pruning, partial-result
Escape, exact storage-mismatch retry, server-only action honesty, focus-aware
shortcut help, and backend-switch identity/state regressions. Backend changes now
patch mounted controls in place: table/form identity, focus, cursor, selection, and
draft destination survive local/server transitions while stale local entities stay
cleared and server submissions omit local Watchlist membership. Final controller
evidence: 94 core workflow tests, 11 review mutations, 4 production identity/
destination tests, and 4 exact mounted receipt deep-links passed; Ruff, generated
CSS sync, and diff checks passed. Implementation commits: `0624b4207`, `243bc7698`,
and `249918b62`.
<!-- SECTION:NOTES:END -->
