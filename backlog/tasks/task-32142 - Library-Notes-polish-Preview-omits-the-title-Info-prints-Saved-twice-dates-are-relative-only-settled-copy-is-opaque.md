...
Fix round 1 (Important 3): format_console_relative_age's own literal "now" (under a minute) concatenated into "now ago" -- live-caught: "Modified 2026-09-08 23:51 · now ago". Added a small `_relative_age_with_ago()` wrapper in library_notes_state.py, scoped to THIS new concatenation only (the shared formatter itself is used by many unrelated callers across the codebase that never append "ago" and stay untouched): "now" -> "just now" (no "ago"), every other label gets " ago" appended as before. New test (`test_editor_state_renders_just_now_not_the_grammatically_broken_now_ago`, Tests/Library/test_library_notes_state.py) modifies a note with `last_modified` equal to `now` and asserts "just now" appears, "now ago" does not. Escalated minor (cheap): the Preview title's `.destination-section` class (AC#1) carried no rule in scope in either size (only `#library-note-context-region .destination-section` was reached, a DIFFERENT parent id) -- grouped `#library-note-preview-region .destination-section` into that same compact-mode rule in tldw_chatbook/css/components/_agentic_terminal.tcss (rebuilt the bundle), matching its Info siblings rather than inventing new styling nothing else in this canvas has; pinned at the source level (a live geometry assertion couldn't discriminate a 1-line title, which measures height==1 either way). Files this round: tldw_chatbook/Library/library_notes_state.py, tldw_chatbook/css/components/_agentic_terminal.tcss (+ rebuilt tldw_cli_modular.tcss). Deferred (no action, reviewer-accepted): `#library-note-context-status` (AC#2) stays composed then forced hidden rather than removed from compose -- two existing row-height geometry pins (Tests/UI/test_library_shell.py's editor-state-allocation and same-side-resize tables) still list it by id, and those tables are already pre-existing-baseline-broken (documented `#library-notes-row-0` staleness) so removing the Static now would be an unverifiable, speculative extra edit; `_parse_browser_timestamp` (AC#3) is a private, underscore-prefixed cross-package import from tldw_chatbook.Workspaces.conversation_browser_state -- reused rather than re-implementing ISO-timestamp parsing, accepted as a known wart.
<!-- SECTION:NOTES:END -->
