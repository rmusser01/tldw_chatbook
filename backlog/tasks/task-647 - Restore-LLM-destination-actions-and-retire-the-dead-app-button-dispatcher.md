...
- [x] #3 Models navigation is owned by the production Lab `LLMScreen` rail and drives `LLMManagementWindow.active_view`, while `TldwCli.llm_active_view`, its watcher, `llm_nav_events` root routing, `button_handler_map`, and `_build_handler_map` are removed.
...
3. Register every supported action on `LLMManagementWindow` with the normalized `(window, app, event)` contract while the Lab `LLMScreen` owns its lifted navigation rail.
...
Verification: the reconciled destination-action suite passes through normal production `TldwCli` and the registered Lab `LLMScreen`; final integrated counts and static checks are recorded in the PR.
<!-- SECTION:NOTES:END -->
