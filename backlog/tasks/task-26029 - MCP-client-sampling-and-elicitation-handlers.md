...
<!-- SECTION:NOTES:BEGIN -->
Protocol-correct, fail-closed handler + connection dispatch shipped and tested. Production injection of the LIVE chat provider + approval surface is split to TASK-26042 (app-context, not headless-verifiable), so this stays In Progress with ACs #1/#4 pending that wiring.
...
Remaining (TASK-26042): construct the dispatcher with chat_api_call (complete_fn) and the approval-store round-trip (elicit_fn), source SamplingPolicy from config, and set client._server_request_dispatcher at MCP/local_control_service.py:778.
