...
That yields 151 candidates, then each is classified by hand. Two traps the candidate list is full of: **`BaseException.add_note()` (PEP 678)** accounts for dozens of hits in TTS/, Model_Artifacts/, Personal_Context/, app.py and DB/private_sqlite* and is not a note at all; and MOST `INSERT INTO notes...` outside `ChaChaNotes_DB` target `notes_sync_*` / `notes_object_mirror` / `notes_organization_*` rather than the `notes` table. **One exception, and it is a live seam on this very list:** `Notes/note_import_executor.py` writes the real `notes` table directly (`INSERT` in `_create_note`, `UPDATE` in `_update_note`), deliberately bypassing `CharactersRAGDB.add_note` for its own version and client-id semantics — its own comment says so. It is the only such site tree-wide. Stated as an absolute this trap would instruct the next re-deriver to discard the one SQL-pass hit that matters (re-review round 3).
