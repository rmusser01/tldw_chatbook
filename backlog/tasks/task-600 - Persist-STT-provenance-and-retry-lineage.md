...
ADR check: existing [ADR-025](../decisions/025-shared-stt-artifacts-and-runtime-routing.md) applies; no new ADR was required. Verification: 849 affected tests passed; Ruff, focused mypy, compileall, and `git diff --check` passed. Repository-wide collection remains blocked on current `dev` by two unrelated existing imports: removed `StreamDone` in `Tests/Event_Handlers/test_worker_events_contract.py` and removed `TabState` in `Tests/UI/test_chat_shell_bar.py` (21,960 tests otherwise collected). Task status remains In Progress until that mandatory repository-wide gate is green.
<!-- SECTION:NOTES:END -->
