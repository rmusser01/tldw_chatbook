...
**Observed, not fixed (out of AC scope):** `search_notes_fts5` still has the three-dot relative import (`from ...Notes.DB...`) that task-260 fixed for the conversations seam -- the notes FTS leg ImportErrors at call time and is swallowed by gather; follow-up candidate.
