...
<!-- SECTION:NOTES:BEGIN -->
Completed all live UAT journeys against the user-owned external audio.cpp listener without adding lifecycle ownership. Closed UAT-03 with live two-model generation and playback-state evidence for pocket-tts-en and supertonic-3. Fixed F-1700-11 by clearing a pending exact voice and reprojecting the audio.cpp catalog when Server default is selected, with a keyboard-driven regression test. Updated the privacy-reviewed live and release evidence records and retained ADR-039 and ADR-040 as the governing decisions. Verification: 141 focused UI/catalog tests and 2 release-evidence tests passed; Ruff, compileall, and git diff checks passed.
<!-- SECTION:NOTES:END -->
