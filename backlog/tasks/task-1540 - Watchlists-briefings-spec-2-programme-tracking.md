...
- [ ] #2 Phase 2: N-speaker presets, script casting, and audio synthesis
  - [ ] Selection-mode picker: `briefing_selection_mode` shipped in phase 1 with no writer (default `auto_featured` only, `auto`/`curated` unreachable) -- add the picker. Deferred from phase 1, confirmed by the project owner 2026-07-30.
  - [ ] Citations: phase 1 renders `[item N]` markers as plain text (`hyperlinks=False` by constraint) -- add links-into-reader and pruned-item degradation, incl. the named `citation-to-pruned-item-degrades` invariant test. Deferred from phase 1, confirmed by the project owner 2026-07-30.
