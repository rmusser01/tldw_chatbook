...
Repository owner instruction: ignore all CI checks. Repository-full and CI were waived and are not green evidence. The earlier full run reached about 83% and stopped at the shared Library navigation hang; the exact node timed out at 300 seconds on both branch and clean dev. TASK-14913 updated the stale stub to NoteFlushOutcome(PERMITTED), after which the exact node passed in 1.08 seconds and its adjacent group passed in 2.18 seconds. No later repository-full or CI run is represented as successful.
<!-- SECTION:NOTES:END -->
