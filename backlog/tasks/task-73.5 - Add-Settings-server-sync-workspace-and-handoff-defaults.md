...
Settings now exposes server profile/authority, sync safety/recovery, workspace default/Library visibility, handoff policy, and ACP handoff readiness as read-only status/default rows. Verification: `python -m pytest -q Tests/UI/test_settings_configuration_hub.py Tests/UI/test_home_screen.py Tests/UI/test_console_persistent_rails.py Tests/UI/test_destination_shells.py --tb=short` reported 287 passed with one existing dependency warning; `git diff --check` passed; approved screenshot is `Docs/superpowers/qa/product-maturity/screen-qa/settings/settings-server-sync-defaults-2026-05-31-large.png`.
