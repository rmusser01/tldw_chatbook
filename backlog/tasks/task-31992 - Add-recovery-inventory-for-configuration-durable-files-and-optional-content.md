...
Actual briefing test after7d702TTSfix now reaches native Completecapture/archive/isolatedplan then refuses owner_relocation_unverified:subscriptions.assets (1fail13.56s, /private/tmp/briefing-audio-restore-lock-corrected.log). Release exact original restore correction: staging._config_targets exact _BriefingAudio root branch; existing _SubscriptionsAdapter relocate_restore/validate_restore_dependencies hooks binding original authenticated same-profile asset IDs to explicit selected direct-child destination and remapped config root. Private candidate DB file_path only; preserve every other completed-row field and all incomplete rows, no sourceopen or generic raw exemption. A owns2production files +existing expected-success test+new focused dependencytest; use existing dispatch/context only, stop if insufficient. Native source-absent readback and finite missing/foreign/ambiguous/wrongroot/unsafe/changedcandidate negatives required. /private/tmp/briefing-audio-relocation-preflight.md reviewed root; capture validation unchanged, no schema/connector/model/activation changes.
Briefing reference validation uses existing topology/context only; no per-file expected digest invented in ownerhook. Exact same-profile dependency/reference/root/regularpeer checks are adapter responsibility; existing sealed archive/staging/publication fingerprints enforce WAV bytes, tested through that actual boundary. No new unchecked proof forwarded.
First briefing correctedpositive fails23.15s beforepublication because ordinary connector write inside stage preview scope correctly refuses discovery_read_only_required; no bypass or speculative admission changes. Reviewed exact existing _RecoveredAdapter.relocate_restore precedent: use open_recovery_validation on disposable candidate with existing _Restrictions and narrowly fixed authorizer permitting only main.briefing_audio.file_path UPDATE and transactioncontrol, source=None, delegatingallotherSQL. This is the existing candidate-write API, not ordinarysourceauthority. Released within existing2productionfiles; add negativeSQL tests, keep rollback/resource/extension/trusted-schema limits and fixed parameterized updates. No private_sqlite/connector/policy edit.
2026-09-11 briefing frozen4 independently reviewed by P and root. Verified four exact source hashes, frozen patch and all23 evidence hashes; root source Bandit0, Ruff only unchanged UP035. All16 new cases plus prior dependency and census11 passed across documented bounded cohorts (15 final combined passes + corrected13); sole interim expected-error fixture assertion retained honestly. Finite completed-row locator update reuses existing private-candidate validation API with exact column authorizer; capture methods unchanged, native isolated fresh readers work with original WAV absent, staged byte drift refused before publication. Reports /private/tmp/chatbook-briefing-audio-relocation-report.md and /private/tmp/briefing-audio-relocation-independent-review.md. Ready scoped commit, owner total task remains InProgress.
