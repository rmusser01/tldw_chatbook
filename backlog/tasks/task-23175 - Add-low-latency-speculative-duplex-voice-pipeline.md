...
28. Task 4 PR portability follow-up: preserve all 315 pristine hashes while applying a second pinned missing-header patch through the existing offline helpers; enforce native-scoped LF checkout including patch/legal bytes; correct both repository-root cibuildwheel script paths. Verify only the selected file-level regressions and static guards. Strict inventory scope includes `Packaging/speculative_voice_source_paths.txt` and `Packaging/speculative_voice_python_paths.txt`: add only the new patch and correct the three stale schema-69 references to the existing schema-70 test/migration paths after the narrow inventory guards reproduce the omission. No compiler/native execution, release evidence regeneration, qualification, push or merge.

Task 4 ADR required: no new ADR.
Controller scope correction: both strict inventories also include Task 3's existing `Tests/Chat/test_console_runtime_lazy_voice.py`, with its source-presence requirement in the existing source inventory guard. Preserve every other entry.
ADR path: `backlog/decisions/098-low-latency-speculative-duplex-voice-pipeline.md`.
Reason: declared missing-header portability repair, checkout/path corrections and existing schema-70 inventory alignment preserve all architecture and integrity boundaries.
<!-- SECTION:PLAN:END -->

## Implementation Notes

<!-- SECTION:NOTES:BEGIN -->
Task 4 PR portability repair (2026-09-08): added pinned patch 0002 for the clock-drift header's direct `<stddef.h>` include using a verified offline pristine reconstruction; only the header, patch-series metadata and two current manifest entries changed. Preserved patch 0001, all 315 pristine hashes, upstream/dependency/legal pins, package versions and packaged qualification/build identity bytes. Native-scoped LF attributes cover vendor, patches and external legal metadata; a materialized `core.autocrlf=true` temporary checkout passes the full real verifier using its own tool/patch files while an unprotected control becomes CRLF. Corrected both repository-root cibuildwheel test paths. Strict inventories now include the new patch and existing Task 3 lazy-owner regression and use the existing schema-70 migration/test paths; corrected the matching path references in the consumed legacy plan. File-only RED: four portability failures, then two inventory failures. Final evidence: all 26 selected cases passed across the 24-pass/2-failure run and the two corrected guard reruns; Ruff lint/format, exact version sync and the full offline verifier passed. Existing requests and webrtcvad deprecation warnings only. ADR-098 applies with a documentation-only patch-series clarification; no new ADR or verification relaxation. No local compiler/build/native binding execution, live app/audio/provider/model, qualification, push or merge. Task remains In Progress pending its existing release qualification requirements.
