...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The Console-only schedule command supports `every_12_hours`, `every_24_hours`, `every_7_days`, `off`, and advanced 3,600–2,678,400-second intervals; “Every 24 hours” stores exactly 86,400 seconds.
- [x] #2 A never-attempted enabled schedule is immediately eligible; later eligibility is the latest attempt plus interval, including after failure, and overdue work runs when the app/scheduler resumes.
- [x] #3 `off` clears only cadence and preserves prior briefings, preset, and selection mode; optional supplied preset/selection changes validate and commit with cadence atomically, while omitted values reuse the collection's stored briefing preset and existing app/provider defaults rather than the current Console conversation model.
- [x] #4 A successful write requests immediate scheduler reload and returns stored cadence, gate/running state, next eligibility, last attempt/success, and separate reload-requested versus reload-acknowledged values.
- [x] #5 A new bounded acknowledgement token completes only after the running loop reloads its queue; stopped, timed-out, disabled, or failed callbacks never report acknowledgement.
- [x] #6 Artifacts and Settings show stored-but-inactive schedules honestly and use “Every 24 hours,” UTC storage/comparison, local display time, and app-open limitation copy.
- [x] #7 Projection, loop, command, persistence-failure, timezone, gate, restart, and UI receipt tests pin the interval contract.
<!-- AC:END -->
...
Independent review required four rounds and closed compact compositor clipping, missing canonical Settings ownership, disabled-gate reload suppression, malformed selection-mode leakage, partial config-commit honesty, and contradictory restart-required actions. Final controller verification passed 68 projection/persistence/SchedulerLoop tests, 26 command/provider/Console tests, 4 mounted Settings cases, 6 Artifacts compositor cases, 3 exact Console-only/external-MCP boundary tests, and 14 CSS integrity tests; Ruff and diff checks passed. The existing Requests dependency warning remains. Three loopback feed-server tests stayed sandbox-blocked and are unrelated to schedule behavior; no full suite or live database was used. ADR-019 remains the sole scheduler authority and ADR-032 remains the Console-only mutation boundary. Added the structured config partial-commit lesson to backlog/docs/lessons-testing-evidence.md.
<!-- SECTION:NOTES:END -->
