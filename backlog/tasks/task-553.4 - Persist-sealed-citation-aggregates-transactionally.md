...
<!-- SECTION:NOTES:BEGIN -->
Implemented the v24→v25 citation-provenance migration, exact 12-table schema, stable local identity context, default-off runtime policy, atomic CitationTraceRepository, authorized summary/hydration reads, and explicit ChatPersistenceService sealed-write seam. The standalone SQL is the sole DDL source and is executed statement-by-statement through the active transaction cursor; identity creation and version update are rollback-safe and the version changes last. Exact schema tests cover columns/types/nullability/PKs, every unique/composite/partial index and predicate, grouped FKs/delete behavior, bounds/CHECKs, fresh/upgrade/partial-schema paths, and FTS/sync/Library/RAG exclusion.
