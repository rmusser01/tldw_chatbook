...
<!-- SECTION:NOTES:BEGIN -->
Implemented the sensitive one-shot Prompt improvement boundary and user-controlled Auto, Review, and Recipe flows across Tasks 10-13, including the normalized source-identity and canonical Additional-context correction subtasks. Client SHA: 12acb277751ebd3985b768ff8a66605da3ae3818. Server compatibility SHA: a6e289031ddbe531fab4983d7e5671a6a85292ed. Fresh final verification: exact client matrix 939 passed with 2 environment warnings; exact server matrix 160 passed with 2 test-environment warnings; CSS build, app import, compileall, both diff checks, and recursive Bandit passed with zero findings. The isolated real-app QA run produced and visually inspected 18 SVG captures at 140x40, 100x30, and 80x24. Its single canary provider call produced zero log occurrences for System, User, block, inline-file body, inline-file label, opaque placeholder, and response content. Prompts remains exclusively the first normal composer-hamburger item; idle composer geometry is unchanged. External Server Browse was unavailable, so its real unavailable/Retry state is paired with automated old/modern-server compatibility evidence. The server worktree's two unrelated untracked watchlist templates were preserved. Configured Ruff/format/mypy checks reproduce the documented repository baseline and were not mechanically rewritten. ADR required: yes for the stage; existing ADR-040 remains applicable and unchanged because no new storage, authority, provider-boundary, security, dependency, or cross-module decision was introduced. Full evidence and design sections 1-15 traceability: Docs/superpowers/qa/console-prompt-improvement-2026-08/README.md.
<!-- SECTION:NOTES:END -->
