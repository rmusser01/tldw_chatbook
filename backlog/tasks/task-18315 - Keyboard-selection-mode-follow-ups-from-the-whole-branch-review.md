...
- [ ] Row-destruction cleanup is eager: `_cancel_selection_if_row_removed` (or its successor) also clears `_kb_selection_row`/`_kb_anchor`/`_kb_end` and hides the hint, so a replaced row never leaves the docked hint lingering until the next keypress
- [ ] `h`'s motion candidate carries the same len(text) upper clamp as every other motion, so a stale `_kb_end` after a streaming shrink self-heals on `h` too
