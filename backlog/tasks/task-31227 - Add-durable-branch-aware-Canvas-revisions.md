...
1. Allocate Canvas revision creation at schema 66→67 after the verified current-dev head 66 and implement the immutable Canvas repository with migration, concurrency, rollback, lifecycle, and query-plan tests. The original pre-integration branch allocated 65→66 before dev claimed that edge.
...
Relevant files: tldw_chatbook/Canvas/{repository,service,staging}.py, tldw_chatbook/DB/migrations/chachanotes_v66_to_v67_canvas_revisions.sql, tldw_chatbook/Chat/{chat_persistence_service,console_chat_store,console_transaction_contribution}.py, focused Tests/Canvas, Tests/ChaChaNotesDB, and Tests/Chat coverage. ADR: backlog/decisions/121-local-versioned-canvas-artifacts-and-browser-sandbox.md.
<!-- SECTION:NOTES:END -->
