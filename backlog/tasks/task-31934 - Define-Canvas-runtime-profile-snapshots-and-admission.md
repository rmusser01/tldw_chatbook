...
Review correction: each catalog entry now loads an exact safe per-profile manifest and retains one immutable runtime-owned closure containing its verified manifest, engine, worker, renderer and library bytes. The frozen snapshot directly owns those closures and provides pure exact-profile lookup, avoiding both mutable rereads and a global registry. The real non-executable V2 descriptor remains assigned to Task 2 after candidate assets exist; no placeholder identity was fabricated.
<!-- SECTION:NOTES:END -->
