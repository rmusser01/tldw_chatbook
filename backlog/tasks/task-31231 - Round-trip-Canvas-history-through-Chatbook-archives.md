...
3. Migrate integrated Canvas revision runtime-profile storage from schema 67 to 68 so well-formed bounded unknown profiles can be retained inert, while execution remains restricted to explicitly supported profiles; verify genuine-v67 migration, rollback, and fresh-schema parity. The original pre-integration delivery used 66→67.
...
- Added runtime-profile widening, originally at schema 66→67 and now integrated at 67→68, so bounded, well-formed future runtime-profile identifiers can be stored inert without weakening the compiler/renderer support allowlist. Genuine-v67 migration, rollback, fresh-schema parity, repository constraints, and synchronization exclusion are tested; the original delivery evidence retains its historical 66→67 labels.
