...
2. `resolve_adaptive_reader_layout` drops an `items` priority when `preferences.items_open and not reader_has_item and profile.list_first_when_empty and width < LIBRARY_EMERGENCY_WIDTH`, so the existing rule owns the case for every caller rather than being restated in a second branch. With something open, `reader_has_item` is True and nothing changes. TWO profiles set the flag: Notes (new) and Media (pre-existing, task-32065), so the drop moves Media's geometry too. That is CLAIMED, not accidental -- Media Trash resolves with `priority="items"` and no item open, which produced the same 32/18 shape at 60 columns the rule exists to kill -- and both profiles are pinned at 48/56/60/63 columns in `Tests/Library/test_library_adaptive_reader_state.py` (review finding F2; the earlier claim that no other destination moved was wrong). `preferences.items_open` is in the condition because the priority is the pane's only reopen lever below the floor and task-32065's rescue branch needs the preference (review finding F3, pinned in the same file). Collections, Conversations, Skills, Prompts and File-notes do not set the flag and are unchanged.
