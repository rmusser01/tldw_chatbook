...
Post-rebase verification passed 1,530 tests across every changed test module plus the persistent-diagnostic, profile-owned-path, CSS-build, backlog-ID, OpenAI-sampling, and Console seam gates. After dev advanced again, the final high-risk gate passed the first 300 Chat/TTS cases, then the full affected Settings module and all remaining Speech/UAT/architecture targets passed 246 tests after stabilizing one deferred-focus assertion with the test's existing settle helper. The broader suite was exercised in ordered segments through the remaining collection; every branch-specific regression found during that run was fixed. Remaining stops reproduce on current dev or are environment/load-sensitive upstream baselines: the destination and Library worker censuses, Textual Select mount-order failures in Library and Watchlists, and the mosaic colour assertion under `NO_COLOR=1` (the exact mosaic test passes with that variable unset). Ruff, compileall, inventory, and diff checks are part of the final branch gate.
<!-- SECTION:NOTES:END -->
