...
- **Gate reconciliation (post-PR):** the Derived-artifacts CI gate failed because the new modules' diagnostic statements and the handlers' removals drifted the production diagnostic inventory -- re-pinned via the sanctioned review-then-write (`0472e17cfa`), with the summarization manifest boundary re-pinned alongside; the ui-ready module census ratchet bumped 1022 -> 1023 (`f2b10d88aa`) since the split adds module objects, not mount work.
