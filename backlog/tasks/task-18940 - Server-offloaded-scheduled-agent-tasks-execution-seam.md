...
- **Client slice 5 (this PR):** AC#7 client visibility — a **Model** column on the Automations definitions table showing each definition's pinned execution target (`automation_execution_target_label`: `provider/model`, either part, or "server default" when the definition rides the fallback chain). The payload protocol already carries per-task selection (server `resolve_execution_target` honors `input.provider`/`input.model`/`input.max_tokens` with definition→config→default precedence); the server-side authoring bound (validate the override against configured/BYOK/admin-override providers at preview/create/update) is filed as tldw_server TASK-12126.
