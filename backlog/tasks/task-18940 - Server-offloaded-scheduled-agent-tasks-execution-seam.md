...
**Remaining slices:** `agent_task` authoring + execution certification (the rest of AC#2), the server-side provider bound (tldw_server TASK-12126, completes AC#7), live end-to-end verification with both server env gates enabled (AC#8), schedules.md full write-up (Automations/Run history/Model sections added in slices 2/4/5; transfer machine section from this PR still needed).
