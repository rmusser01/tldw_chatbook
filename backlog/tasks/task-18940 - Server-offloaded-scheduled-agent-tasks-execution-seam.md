...
**Remaining slices:** definition authoring from the client (create/preview/submit — the rest of AC#2), per-task model selection on the definition payload (AC#7), live end-to-end verification with both server env gates enabled (AC#8), schedules.md full write-up (Automations + Run history sections added in slices 2/4).
