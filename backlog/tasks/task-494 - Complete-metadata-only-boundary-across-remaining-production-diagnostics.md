...
Complete ADR-029 across every production diagnostic domain not covered by the provider/tool remediation so persistent application logs cannot retain user, model, credential, file-content, or response-body values.
...
Reason: TASK-494 completes the accepted metadata-only persistence boundary without changing ADR-029.
