...
- Added content-free stream signals and presentation metadata plus an eight-entry current-session original-attempt preview. Preview state does not alter message content, provider history, persistence, export, or TTS. Request-scoped repair sessions now clear their evidence-bearing contract and provider resolution in the shared cleanup path.
- Privacy coverage uses distinct initial-body, repaired-body, evidence, source-identity, locator, exact constructed repair-prompt, and provider-exception sentinels. The final matrix covers request-fit failure, provider raise, empty/oversized output, invalid markers, changed claims, user cancellation (direct and agent), late chunks, session close, and fallback bypass; it captures stdlib and Loguru diagnostics and inspects signals, presentation, cleaned session fields, run state/history, cleanup state, and persistence.
...
- Scoped verification with `PYTHONPATH=/tmp/codex_task55315_no_mlx`: privacy TDD proof RED `1 failed, 366 deselected`, final privacy matrix `18 passed, 365 deselected`; complete `Tests/Chat/test_citation_repair.py` `117 passed`; focused groups `18 passed, 91 deselected`, `17 passed, 152 deselected`, `105 passed, 250 deselected`, and `20 passed, 318 deselected`. The three groups that import `requests` emitted one existing dependency-version warning each.
- Exact 22-file Ruff check passed. The exact 11-file format check reported inherited whole-file drift in `tldw_chatbook/Chat/console_agent_bridge.py`, `tldw_chatbook/Chat/console_chat_controller.py`, `tldw_chatbook/UI/Screens/chat_screen.py`, and `tldw_chatbook/Widgets/Console/console_transcript.py`; the other seven files passed, and changed-range review found the new controller/test hunks formatted, so unrelated baseline files were not rewritten.
