...
Restore the local-marks v16 migration test's historical fixture contract after
citation-provenance tables were introduced, without weakening the production
migration's fail-closed handling of pre-existing provenance tables.
...
2. Correct only the legacy test fixture so it removes provenance objects before recording v16.
...
Reason: ADR-024 already defines the citation schema and fail-closed migration contract; this task only repairs a historical test fixture to represent a true pre-provenance database.
<!-- SECTION:PLAN:END -->

## Implementation Notes

<!-- SECTION:NOTES:BEGIN -->
Implemented a test-fixture-only correction for the local-marks v16 migration replay. The fixture now removes all 12 citation-provenance tables in their established foreign-key-safe order before recording schema version 16; reopening the database therefore exercises the real v16-to-current migration path. Production migration behavior and schema code were not changed.

ADR decision: no new ADR required. Existing ADR-024 (`backlog/decisions/024-rag-citation-provenance-and-source-resolution.md`) defines the provenance schema and fail-closed/atomic migration contract; this change restores the fixture to that contract.
