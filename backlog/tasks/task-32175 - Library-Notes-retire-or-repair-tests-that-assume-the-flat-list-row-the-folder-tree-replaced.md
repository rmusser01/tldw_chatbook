...
- test_library_notes_reader.py (whole file): 13 failed/21 passed -> 3 failed/31 passed. The 3 residual reds are proven unrelated (different assertion/location, no prod change made). Note the honest wording for one of them: test_database_notes_capability_inventory_and_modes was RE-TARGETED, not repaired -- it is red on the base too, and now fails later, on #library-notes-filter-clear never mounting (task-32185 AC#1).
