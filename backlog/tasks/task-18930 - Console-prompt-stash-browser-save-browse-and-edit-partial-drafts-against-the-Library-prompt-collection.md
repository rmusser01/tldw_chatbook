...
- The protected Derived Artifacts gate exposed two missing release artifacts after the feature checks passed: the new ordering index lacked a no-statistics query-plan decision, and the three new persistent diagnostics were absent from the reviewed inventory. Added a real 100-row `EXPLAIN QUERY PLAN` regression that pins `idx_local_prompt_drafts_updated`, sanitized the new diagnostics to omit database paths, raw exceptions, and tracebacks, and regenerated the reviewed production diagnostic inventory. Post-fix verification passed all 18 Draft Shelf service tests, both artifact guards, scoped F821, and `git diff --check`.
<!-- SECTION:NOTES:END -->
