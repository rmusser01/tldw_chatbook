...
**Latent production bug fixed (surfaced by the empty-honesty contract test):** `ChromaVectorStore.get_collection_stats` raised numpy "truth value ambiguous" on `peek()["embeddings"]` (numpy arrays in 1.5.8) — chiefly on *populated* collections (peek returns a multi-row array) — swallowed → masked the real count with `{count:0,error:...}` (degrading health_check→DEGRADED and get_chunk_count→0). Fixed with explicit `len()` checks; a positive-int `count` now flows to all consumers.

**PR #771 review fixes:** hnsw:space kept last in the collection-metadata merge so a stray key can't override the real metric (Qodo #3); the no-profiles ultimate fallback in `create_rag_service` also runs the adoption migration. Declined Qodo #1 (persist_directory path-validation) for SP1 — pre-existing store-wide pattern; fixing only the new module risks a path-string divergence from the store → the SharedSystemClient collision SP1 avoids — filed as follow-up task-482 (uniform validation across both Chroma client sites).
