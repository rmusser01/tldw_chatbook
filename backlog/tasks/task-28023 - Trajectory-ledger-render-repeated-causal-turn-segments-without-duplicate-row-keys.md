...
4. Run the focused regression and the full Trajectory screen test module, then run scoped static checks.
...
<!-- SECTION:NOTES:BEGIN -->
Implemented segment-aware Trajectory ledger row identity without altering causal projection output. The first occurrence retains its existing turn:<id> key for live cursor stability; later occurrences use turn-segment:<occurrence>:<id> and map back to the logical turn for inspection and collapse. Logical turn numbering now follows first occurrence, and inspector counts aggregate records across repeated segments.

Added mounted Textual regression coverage for repeated t1 -> t2 -> t1 rendering, unique headers, complete record rendering, logical-turn actions, and live first-header stability.
...
Verification: Ruff format/check passed; 41 Trajectory screen/timeline tests passed; 8 unaffected live Trajectory tests passed; git diff --check passed. One pre-existing test_trajectory_launch_action_presents_screen failure was excluded because user-owned dirty chat_screen.py changes require _retrieval during harness setup before the Trajectory launch path is reached.
<!-- SECTION:NOTES:END -->
