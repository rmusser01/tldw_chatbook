...
Reason: this task implements ADR-121's accepted same-origin served topology, authenticated child-control boundary, origin-wide web authentication, and browser-session isolation; the ADR will be amended with the concrete textual-serve extension seam and protocol version rather than duplicated.
...
6. Request independent security and UX review, update ADR-121 and this task with the concrete contracts and evidence, and mark the task Done only after every acceptance criterion is verified.
<!-- SECTION:PLAN:END -->
...
- ADR check: ADR-121 was updated with the actual textual-serve seam, protocol and limits, credential/authentication flow, proxy policy, settlement contract, and live isolation evidence. No new ADR was needed.
<!-- SECTION:NOTES:END -->
