...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Strict JSON decoding rejects duplicate keys, non-finite values, invalid UTF-8/surrogates, over-limit trees/strings, and non-NFC identities with stable error categories.
- [ ] #2 Canonical JSON produces deterministic LF-terminated UTF-8 bytes with normalized strings, sorted object keys, and no non-finite values.
- [ ] #3 Immutable manifest/profile/rule contracts reject missing, unknown, malformed, colliding, or out-of-grammar fields and states.
- [ ] #4 Portable contract fingerprints include normalized tool identity, description, input schema, and deduplicated sorted policy-risk tags while differing from runtime definition hashes.
- [ ] #5 Focused contract tests and scoped static checks pass.
<!-- AC:END -->
