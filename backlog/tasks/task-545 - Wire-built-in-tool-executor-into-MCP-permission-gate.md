...
**Current ownership reconciliation (2026-08-12):** The P2 exclusions above are historical only; they do not define current or pending work. `web_search` is now owned by `LocalToolProvider` as `local:web_search` under ADR-032. Direct note retrieval is owned by `LibraryToolProvider.library_search_notes`, while fallback RAG retrieval is owned by `LibraryRagToolProvider.search_library_rag`; ADR-030 governs both Library paths. `code_audit` never became a System B capability and has no live owner. TASK-743 owns the complete audit-subsystem keep/redesign/delete decision across every live Console file-mutation seam: built-in `write_file` and local `fs_write`, `fs_edit`, and `fs_patch`. TASK-545 remains closed.
...
- **System A's fate.** `6825ba6ab` confirmed via grep that `ToolExecutor`/`ToolResultCache`/`get_tool_executor` had zero production callers anywhere (the Console never reached System A — established back in TASK-545's original rescoping) and removed their implementations (653 lines) from `Tools/tool_executor.py`, along with the file's only `install_claude_code_hooks()` call site (see TASK-743, now filed, for what that leaves unreferenced). The `Tool` ABC and its two concrete built-ins stayed, because System B's tool classes still derive from that ABC — the file could be gutted but not deleted.
- **Config bug-class sweep.** Per the P3 spec, doing this work surfaced the same `get_cli_setting` bare-section-silent-default defect four more times in unrelated subsystems (splash screen ×2, web server, TTS OpenAI backend ×3). None were fixed inline; each is filed as its own task (TASK-740/741/742) plus a task for the bug class itself (TASK-744), rather than papered over here.
