...
- Review hardening added strict nested Persona voice/setup validation at the pure contract boundary, bounded ZIP-directory and decoded-image admission, cleanup for every publication/cancellation outcome, owned sequential Persona portrait anchors hidden from normal Character lists, real Console/Buddy invalidation, isolated post-commit refresh failures, and a fresh review loop after stale activation.
- Verification: after rebasing onto current `dev`, `Tests/Actor_Packs` passed 197 tests and the scoped architecture/UI workflow set passed 70 tests; the persistent diagnostic inventory pin passed, and scoped Ruff, format, compileall, and `git diff --check` passed. The rebase retained `dev`'s deferred Actor Pack recovery lifecycle and audited three inherited stale diagnostic rows: proposition logging contains only fixed text, counts, configuration values, and exception type names; RAG preserves its pre-existing exception interpolation while changing logger wiring; app lifecycle logging contains fixed text, counts/phases, internal worker state, and exception text. The reviewed inventory pin was regenerated. Repository-wide collection reached 56,385 tests and is blocked only by the pre-existing parametrization error in `Tests/UI/test_library_file_notes_workspace.py` (`push_phase` is declared but unused by the test function).
