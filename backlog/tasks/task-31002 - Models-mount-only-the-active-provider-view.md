...
- Verification: 146 targeted tests passed across deferred Models views, GGUF modes, Labs adoption, Installed models, and the legacy downloader architecture guard. Ruff, `py_compile`, and scoped `git diff --check` passed. Per repository policy, the full test suite was not run without explicit opt-in.
