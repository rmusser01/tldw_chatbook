...
required fast lane while preserving comprehensive coverage on main, nightly, and
manual runs. The new lane must become enforceable without introducing a required
...
- [ ] Pushes to `main`, scheduled nightly runs against `dev`, and manual dispatch retain their documented comprehensive test coverage
...
4. Remove pull-request admission and obsolete PR-only summary permissions from the heavyweight `Tests` workflow while preserving main, nightly, and manual coverage.
...
6. Rebase on latest `dev`, open the PR, verify its live required gate and runner fan-out, address all review feedback, then merge the exact reviewed head.
