...
TASK-32102 ITEMS, LANDED HERE (handed over by the session that owns task-32102 so `library_file_notes_workspace.py` lands once): the structural-wait surface got its three fixes in this branch. Cancel is now revealed at the patience boundary rather than at t=0, so it arrives with the line that first admits the wait is slow (`_update_root_surface` gates `#library-structural-wait-cancel` on the same `slow` the other two decisions use; the two crit8 tests that pressed it at t=0 now age the wait first, through the existing wait clock rather than a new one). An exit-triggered abandon (`_flush_active_file_notes` -> `cancel_structural_wait(leaving=True)`) announces the outcome instead of writing `ROOT_CHANGE_CANCELLED_COPY` into the root row of the canvas being torn down. And the wait branch now clears `-warning` and `-offline` as well as `-empty-root`, since it returns before the resets the other branches perform and a stale tint sat under the wait line. Tests: `test_cancel_appears_only_once_the_wait_admits_it_is_slow`, `test_leaving_during_a_folder_change_announces_the_cancellation`, `test_the_wait_line_carries_no_tint_from_the_state_it_replaced` -- all three RED on the pre-change source.
