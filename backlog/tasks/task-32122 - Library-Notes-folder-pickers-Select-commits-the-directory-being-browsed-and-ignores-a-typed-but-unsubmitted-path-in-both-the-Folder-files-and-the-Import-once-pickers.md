...
- [x] #2 In folder mode the field is labelled 'Folder path', never 'File name'. Accepted AS INTERPRETED (controller ruling, review round 2) for the files-or-folder dialog (Import once, Keep-synced, Import media): the field starts labelled "File name:" and swaps live to "Folder path:" once the typed value resolves to a directory -- "folder mode" there is a live state of one dual-purpose field, not a fixed dialog mode, since the same field also accepts a file. The directory-only pickers (`EnhancedSelectDirectory`, vendored `SelectDirectory`) show "Folder path:" unconditionally, matching the literal AC text.
- [x] #3 The selection confirmation shows the absolute path, not the basename
- [~] #4 Both pickers share one component, one start location rule (last used, else home) and one hint line that is visible at 235x52 and 100x30. Honest partial (controller ruling, review round 2): "one component" is now met -- after round 2's consolidation, all three dialogs (`EnhancedSelectDirectory`, vendored `SelectDirectory`, `FileOpen(offer_select_folder=True)`) route their typed-path validation through the one shared `resolve_typed_directory()` in `base_dialog.py`. The hint line is met and live-verified for both vendored pickers. "Else home" is met (`resolve_default_location()`, same file). "Last used" is NOT implemented for these three flows (Import once, Keep-synced, Folder files) -- but `library_screen.py`'s `_library_ingest_browse_location` (~25642-25679) already does last-used-else-home caller-side for the fourth `offer_select_folder=True` flow, Import media (`get_cli_setting("library.ingest", "last_directory", ...)`, persisted via `_persist_library_ingest_location`), so the follow-up for the other three is a small caller-side change matching that existing pattern, not a new persistence scheme.
