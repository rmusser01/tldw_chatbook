...
- `chat_screen.py` compose: the left rail's Context `ConsoleRailSectionHeader` + `#console-rail-section-body-context` block is gone; `ConsoleStagedContextTray` (`#console-staged-context-tray`, unchanged id so `_sync_console_staged_context_tray` and all live-update paths keep working) composes as the FIRST child of `#console-inspector-rail-body` - visible without scrolling, above the run inspector's "Source Readiness" section (user-review placement fix: the initial bottom-of-rail slot above the "Live work sources" card was invisible in a fresh session without scrolling). Splitting the monolithic `ConsoleRunInspector` at its Run/Source-Readiness boundary was rejected: the TASK-259 in-place-update fingerprinting pins its exact child sequence. Same pure display-state seam (`_build_console_staged_context_state`) - no DB reads added to compose/recompose.
- `_apply_console_live_work_card_swap` keeps its PRE-MOVE anchor: cards mount after `#console-run-inspector` at the rail-body bottom; the tray stays pinned on top through swaps in both directions (test-pinned). Width-gate reality (verified in-app): the Inspector force-collapses under 150 cols EXCEPT the 118-128 standard-width contract, which auto-opens a fresh session (measured open at 125 cols, collapsed at 140, default-closed preference at >=150). First-start visibility of Context now depends on the Inspector being open; once opened it is the first block visible.
...
- PR review follow-through: the staged-count badge moved off the LEFT handle with the section -- `build_console_context_rail_badge` is workspace/session-only and `build_console_inspector_rail_badge` gained the "N staged"/"staged" case (below action-required failed/setup/blocked/approvals/tools, above the informational artifact/source fallbacks). A new swap-path test also pins that `_frame_console_region` styles the tray in place (no wrapper), so the tray is a valid direct-sibling mount anchor for the live-work card swap in both directions.
- Follow-up candidate (out of scope): the collapsed left-rail handle label still reads "Context (glyph)", mirroring the rail's established "Console context" title (task-186 chrome, renamed from "Session & Context"). With the staged badge relocated the pairing is no longer misleading, but renaming the rail identity itself is a visual-naming decision for a dedicated pass.
<!-- SECTION:NOTES:END -->
