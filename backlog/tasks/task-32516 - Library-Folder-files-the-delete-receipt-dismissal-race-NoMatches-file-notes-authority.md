...
fails when run in isolation on dev at roughly 2 runs in 5 (the wave-3 docs
sweep saw it red on the merged tree and green on the next run without a
code change). The failure is a Textual `NoMatches` for
...
Evidence: wave-3 docs sweep, 2026-09-12, on dev 7159fc0b99 (see the
task-32271 Implementation Notes for the run log).
