...
- Restored the separate default config exactly after its post-run byte fingerprint differed from the validated pre-run snapshot. The restore was the correct containment precaution; concurrent unrelated activity meant the UAT did not identify the writer.
...
- The post-session comparison found that the unrelated default config's byte fingerprint differed from its validated snapshot. The exact snapshot was restored before cleanup. Because concurrent unrelated activity existed, the UAT could not attribute the mutation to the isolated app lifecycle; TASK-15674 later added a controlled regression that left a distinct decoy default byte-identical while approved-quit persistence selected the effective scratch profile.
