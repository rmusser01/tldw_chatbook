...
**Interaction with task-32250 (surfaced on dev at 87f6edb4c1, after #2612 and #2618 both merged).** The precedence in `_library_notes_work_first_preferences` was composed as described -- the review rule is checked first and wins its state -- but AC#2's "every phase" contradicted task-32250's negative pin that the select phase leaves the list open (`Tests/UI/test_library_notes_wave_import_ux.py::test_the_review_takes_the_pane_while_it_is_the_task_in_hand`, second assertion). Ruling: AC#2 stands; that pin was flipped to `items_open is False`. Neither group's landing pass ran the other's pin file, which is why the contradiction reached dev.
<!-- SECTION:NOTES:END -->
