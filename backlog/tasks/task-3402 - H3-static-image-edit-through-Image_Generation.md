...
- [ ] #2 Exactly one staged image and one instruction produce exactly one PNG at the source dimensions; seed, steps, and sampler are applied, while explicit batch, size, negative-prompt, CFG, model, alternate-format, and other unsupported overrides are rejected before image upload or prompt submission.
