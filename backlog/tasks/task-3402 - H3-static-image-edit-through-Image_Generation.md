...
- [ ] #3 Successful results use the existing Image_Generation attachment storage and generation-metadata contract, with no Video_Generation storage path; Stop/unmount cancellation cannot orphan a success or update stale UI, only the exact initiating attachment is consumed after durable persistence, and H3 Regenerate requires restaging the source.
