...
Item 5 (PR-T3 fix round F) addendum: the original evidence chain's third bullet cited
`normalize_runtime_source_state()` as the function `PolicyEngine.evaluate()` calls
directly ("confirmed by reading its body") -- one hop off. `evaluate()`
(`runtime_policy/engine.py:33`) calls `state.normalized_for_policy(...)`
(`RuntimeSourceState.normalized_for_policy()`, `runtime_policy/types.py:72`), which in
turn imports and calls `normalize_runtime_source_state()`
(`runtime_policy/source_state.py:45`, `types.py:75-77`). The underlying claim -- that
normalization never changes `active_source`, so the `wrong_source` deny branch is
unreachable for `LOCAL_ONLY_SOURCES` -- still holds; only the call site named as the
direct caller was wrong. Corrected in the evidence-chain paragraph above. A closed
"cannot happen" task is only as good as its chain, one hop at a time.
<!-- SECTION:NOTES:END -->
