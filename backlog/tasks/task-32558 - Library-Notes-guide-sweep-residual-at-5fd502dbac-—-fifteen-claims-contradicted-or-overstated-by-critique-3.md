...
**The gate's reach, measured rather than assumed.** `scripts/check_guide_claim_strings.py` did NOT catch the Export… claim — it flags neither :23 nor :324 on the pre-sweep page. `emitted_somewhere('Export…')` returns True because the checker greps raw source text and the string lives in a dozen comments and docstrings under UI/Library_Modules/, one of them (library_notes_controller.py:5406) stale in the same way the guide was. So the checker catches a string no source file contains anywhere, and is blind to one that exists only in prose about the code — the more dangerous half, since stale comments are where an outdated label survives. Recorded as a `ponytail:` comment on the function (with the incident and the ast/ast.Constant upgrade path) and as task-32589 AC#7, written as a proven hole.

Re-verified: preflight exit 0; Tests/Docs 22 passed / 2 failed (the same two README.md names on detached origin/dev); id-uniqueness 3 passed; script self-test OK.
<!-- SECTION:NOTES:END -->
