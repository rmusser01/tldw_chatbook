...
<!-- SECTION:PLAN:BEGIN -->
1. Read task-27019 (now TASK-31201), the size-ratchet model test (test_budget_is_not_left_slack_after_a_wave) and the recompose census file fully.
...
5. Mark this task Done with Implementation Notes.
...
This task was filed as TASK-27019 on the wave-2 branch and renumbered to TASK-31201 when merging origin/dev surfaced the same id collision the foundation branch had already resolved for this file (see "Renumbering provenance" below, carried over from dev). The status/AC/plan/notes above are wave-2's own completion record, merged onto the surviving TASK-31201 file rather than left orphaned on the deleted task-27019 path.
<!-- SECTION:NOTES:END -->
