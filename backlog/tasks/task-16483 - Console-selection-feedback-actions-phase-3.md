...
- **Assistant prose is agent output (product decision 2026-08-16).** The user widened `_row_supports_selection_feedback`: ASSISTANT-role rows (markdown or plain — the agent's own prose replies are the most natural review target) now qualify alongside TOOL-role rows and diff rows; USER-role rows still never offer feedback. One condition change (`message.role in (ASSISTANT, TOOL)`), same run gating; pinned by `test_drag_on_assistant_markdown_row_shows_feedback_entries` + `test_drag_on_assistant_markdown_row_run_gating_without_status_seam` in `Tests/UI/test_console_selection_end_to_end.py` (the pre-existing USER-row exclusion test stays green).
