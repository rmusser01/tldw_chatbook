...
PR verification caught one Linux UI-ready census regression: constructing the Hub-test preview registry and execution coordinator eagerly imported `MCP.hub_test_execution` before first paint. The control plane now creates that synchronized state on the first Test Tool interaction, preserving injected state independently, and the boot census explicitly keeps the module deferred. The exact performance-guard command passed **11 tests**, while the affected control-plane and UI/provider regression shards passed **205** and **826** tests respectively.

Qodo's exact-head review produced eight valid closeout findings. The final patch routes canonical tool arguments through the shared Pydantic validation boundary, validates the Hub workspace root through the central path boundary, completes the public typing/docstring contract, preserves consumed approval facts after post-consumption failures, suppresses success copy when dispatch never starts, handles the production `deny` gate in the Inspector, and moves non-local invalid-intent refresh work off the UI loop. Regression evidence is **8 focused**, **242 affected MCP**, and **802 affected Console/UI/provider** tests passing. The exact **11-test** performance guard still reports **970/972** UI-ready modules, and touched-file Ruff check/format, scoped compilation, diagnostic-inventory zero-drift verification, and diff hygiene are clean.
