...
- [ ] #7 The check probes string LITERALS, not raw source text — a label that exists only in a comment or docstring must not count as emitted. This is a proven hole, not a theoretical one: task-32558's own stamp certified import-and-export.md's "press Export… in Notes" claim, the Notes toolbar ships a bare "Export" (library_notes_canvas.py:1842), and the checker stayed silent because "Export…" appears in a dozen comments and docstrings under UI/Library_Modules/ — one of them (library_notes_controller.py:5406) stale in the same way. An allowlist alone does not close it
<!-- AC:END -->
