...
- [ ] #7 A claim that names a surface resolves against THAT SURFACE's own modules, not against the tree. This is the hole, proven by the incident that produced this task: import-and-export.md said "press Export… in Notes", the Notes toolbar ships a bare "Export" (library_notes_canvas.py:1842), and the repo-wide checker stayed silent because Export… is a live Button label on Media, Conversations, Prompts, Meetings, Artifacts and the Console inspector. Measured: 35 raw lines carry Export… — 12 pure # comments, 13 docstring nodes, and 9 non-docstring string literals of which 7 are Button labels. NOTE FOR WHOEVER TAKES THIS, because this AC was wrong once: an ast.Constant pass that excludes comments and docstrings STILL returns 9 hits and would not have caught the incident — excluding prose is a precision improvement, not the fix. Scoping is. Verified both directions before filing: Export… probed against the 12 library_notes* modules returns 0 (the false claim fails), Export probed the same way returns 13 (the corrected sentence passes).
