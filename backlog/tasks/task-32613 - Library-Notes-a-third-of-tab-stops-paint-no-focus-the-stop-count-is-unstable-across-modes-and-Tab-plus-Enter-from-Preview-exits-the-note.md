...
<!-- SECTION:NOTES:BEGIN -->
Three findings, and the cause of two of them is the same fact: **Textual orders the focus chain by SCREEN POSITION, not compose order** (`Widget._focus_sort_key` = (y, x) of the virtual region, applied per container level in `Screen.focus_chain`). So the heading's '‹ Notes' is the note pane's first stop however the canvas is composed, and a reading region -- the bottom of the pane -- is its last. One Tab wrapped the cycle from the document straight onto the exit, and the obvious Tab-then-Enter closed the note. Reproduced headlessly before any change: Tab from `#library-note-preview-region` focused `#library-note-back`. There is no tab-index in Textual, so the ring is re-cut in `_move_library_screen_focus`: a forward Tab out of a reading region goes to `#library-note-edit` -- but ONLY when that region is the stop whose forward Tab would otherwise wrap onto the pane's Back button.

**Fix round 1 -- the first cut of this was a regression and is the reason the condition exists.** The redirect was unconditional. In Preview that is harmless: the region really is stop 7 of 7. In Info it is stop 7 of 12, and the measured forward ring collapsed to `edit -> preview -> context -> save -> use-in-console -> context-region -> edit`: Keywords, Copy, Export Markdown, Export text, Delete and '‹ Notes' became Shift+Tab-or-mouse only, while task-32607's footer went on naming 'enter delete note' on them. Entry focus in Info lands on that region, so the very first forward Tab from a freshly opened Info pane was the broken one. That is precisely the defect task-32607 exists to remove, re-created one layer down. Thirteen node ids in Tests/UI/test_library_shell.py went red on it, branch-only, dev-green -- among them `#library-note-context-copy was not reachable with tab; focused=VerticalScroll(id='library-note-context-region')`.

The branch's own suite could not see it because every Info case focused its target with `widget.focus()`, never with `pilot.press('tab')`: **a focus-order pin that sets focus directly is not testing focus order.** `test_tab_walks_every_stop_of_the_info_pane` now Tab-walks the Info pane and asserts every one of its stops is visited, and the Preview pin asserts the region really is the last stop before asserting the override fires.

Trade-off, recorded rather than hidden: in Preview, Back is consequently reached by Shift+Tab (one press from Edit) or Escape rather than by wrapping forward. In Info nothing is cut at all -- the region is not the wrap stop, so the ring is Textual's own. Cutting the ring anywhere else would need a per-control override table; a `ponytail:` comment on the constant names the upgrade path.
