...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Identify every flush call site for message_exchanges captures and confirm which ones re-process already-flushed captures on a later flush of the same message.
- [ ] #2 Either skip re-compressing captures already persisted on an earlier flush, or document why the current behavior is acceptable and close the task without a code change.
<!-- AC:END -->
