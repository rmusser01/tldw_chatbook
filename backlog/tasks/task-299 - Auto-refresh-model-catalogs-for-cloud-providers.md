...
<!-- SECTION:NOTES:BEGIN -->
Implemented the ADR-019 hybrid: startup background discovery → disk TTL cache → capped merge into selectors, with per-provider opt-in write-through to `[providers]`. All failures degrade to cached/saved models; startup is never blocked.
