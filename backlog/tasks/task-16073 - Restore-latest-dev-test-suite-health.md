...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A frozen collection manifest and resumable checkpointed baseline on the pinned dev commit record every collected node exactly once with durable node IDs, terminal outcomes, logs, environment/package fingerprints, and machine-readable reports; missing, duplicate, corrupt, multi-node-timeout, or abnormally exited chunks cannot pass the completeness gate, while a bounded single-node timeout is retained as an explicit red outcome.
