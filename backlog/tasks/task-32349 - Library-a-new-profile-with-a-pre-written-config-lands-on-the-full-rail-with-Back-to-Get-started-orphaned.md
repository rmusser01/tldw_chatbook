...
Pin updated, not weakened: test_library_onboarding_legacy_and_corrupt_preferences_open_expanded's [None] case now expects STARTER after settle (both cases still OPEN Expanded); its ['not-a-lifecycle'] case keeps EXPANDED, because a corrupt value is still a stored one. The constructor pin test_library_real_existing_config_without_lifecycle_defaults_expanded is untouched and green.
