...
(3) bulk delete for Library ▸ Notes -- select mode offers only Export
selected, so undoing a 54-note import is 54 individual confirmations, which
is why this task's recourse reads as a warning rather than a step. Wanted
...
**Fix round 2 (re-review).** Two Minors, both in the record rather than the
code. (a) The citation-accuracy round shipped a citation its own next commit
invalidated: AC#2's three `notes_sync_runtime.py` line numbers were exact at
`faa57da1c9` and two-to-three lines stale at head, because `85e9daa4cb` --
on this branch -- added imports above them. Re-pinning would rot again, so
they are now cited by symbol with no line number. This refines the wave-4
rule in `lessons-live-verification.md`, which said a citation into *another*
file is safe: it is not, if your own branch edits that file. The refinement
is appended under that entry, with this incident. (b) The rider list above
now enumerates all three riders the fix-round prose names; it read two.
