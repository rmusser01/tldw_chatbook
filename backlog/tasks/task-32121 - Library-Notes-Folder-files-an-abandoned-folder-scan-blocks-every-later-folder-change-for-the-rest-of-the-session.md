...
Review round 2 (PR #2543, Qodo): four behaviour corrections. (1) `_walk_candidates` checked the cancel flag once per `os.walk` yield, so a FLAT folder -- one yield -- still ran two stats per file with the lock held; the check now runs per file. (2) `_scan_for_root` gave the lock acquire a second fixed `ROOT_CHANGE_TIMEOUT_SECONDS` deadline, so Keep waiting bought nothing for a change queued behind an uninterruptible scan; the worker now waits until its attempt is abandoned (one clock), and keeps its own bound only for a caller that passes no cancel flag. (3) The landed-after-cancel branch wrote only the action status, leaving "previous folder kept" on the folder row while the new folder was linked; it reports through the same reason channel now. (4) `on_unmount`/`shutdown` did not stop the repeating patience timer. Also: `[notes] sync_directory` goes through `path_validation.validate_existing_absolute_directory`, so a relative spelling (resolved against the launch directory) is no longer offered by name. Live re-verified on the power profile: progress + the three controls at 6 s, Keep waiting carried the change past its first deadline, the timeout copy owned the folder row, and `file_notes` linked 3 s later (captures 10-14).
