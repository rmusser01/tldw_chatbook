...
Fix round (review of the first implementation, same branch): the wait must own only the SUFFIX of a status line, never the whole line. Rendering `wait.status_line()` in place of the surface's own status swallowed the export's per-phase progress (`Collecting notes…  3/12` -- the only progress readout an export has) and the skill import's in-flight refusal (`An import is already in progress.`). `StructuralWait` gained `is_slow(now)` and `with_patience_suffix(base, now)`; `status_line` is now that method applied to `'<label>…'`, so the copy still lives in one place. Two further fixes: `_repaint_library_skills_import_status` (a 3 s timer) had no row guard, so leaving Skills mid-import let it flip the File Notes workspace's identically-id'd `#library-structural-wait-cancel` on, or recompose an unrelated canvas via the `_sync_library_canvas` fallback -- it now returns early under the same `_library_skills_import_row_visible()` guard the in-place patch uses, and the recompose fallback is gone. And the screen's single wait slot became one slot per owner (`_library_structural_waits`), with the File Notes workspace's unused mirror (`adopt_library_structural_wait` / `_publish_structural_wait`) deleted: nothing consumed it, every file-notes exit path calls `workspace.cancel_structural_wait()` directly, and publishing into the shared slot let a folder change silently orphan a running skill import's Cancel.

Fix-round tests: Tests/Library/test_library_structural_wait.py +2 (is_slow, with_patience_suffix), Tests/UI/test_library_crit8_waits.py +2 (`test_skill_import_patience_repaint_stays_off_other_rows`, `test_a_folder_change_never_steals_another_surfaces_wait`) plus a progress-tick assertion in the export test and a widget-level refusal assertion in the skill-import test. All 9 pass; export/skills/skills-import/file-notes covering files show no new failing names.
<!-- SECTION:NOTES:END -->
