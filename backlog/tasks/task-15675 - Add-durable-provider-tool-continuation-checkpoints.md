...
Persist the private provider context required to resume interrupted native function-tool runs without exposing reasoning in the transcript, re-executing completed tools, or creating a provider-specific agent loop.
...
- [ ] #5 Continuation data participates in message versioning, both supported sync paths, payload hashing, branch/variant ownership, deletion, edit/regenerate behavior, and whole-record conflict handling without field-level merge.
- [ ] #6 Lossless `.chatbook` and ordinary JSON exports include the data under an explicit private field with a warning; text, Markdown, rendering, FTS, logs, errors, usage, and summaries exclude it.
...
- [ ] #9 The shared runtime supports provider-specific replay policy without an open metadata bag: Kimi and GLM can resume preserved active tool runs, while DeepSeek can replay completed tool-associated reasoning on later same-provider turns.
