...
5. PR review: keep the observed DuckDuckGo challenge reason through the rebased dispatcher, and  validate search envelope/result text fields with Pydantic before formatting or caching, preserve null/missing optional fields and saved-backend provenance, and update the Google-style API contract. Add malformed-field regressions before implementation.
<!-- SECTION:PLAN:END -->
...
Maintainer review: added strict Pydantic result/envelope validation and documented Args/Returns/Raises; 16 malformed-field RED cases now pass, with null/missing text controls. Preserved current dev saved-backend selection/provenance. Restored observed DuckDuckGo challenge reason through the rebased dispatcher using a typed exception. Search/backend/provider verification: 119 passed, three opt-in live skips. Existing ADR078/032.
<!-- SECTION:NOTES:END -->
