...
2026-09-04 governance repair: reproduce the missing-catalog failure; add a
complete documented-vocabulary check so one spacing sentinel cannot hide
missing categories; supply the accepted scalar catalog without changing
existing consumers; rebuild CSS and verify governance, bundle integrity, and
focus-token parity. This is direct implementation of the existing ADR-042;
no new ADR is required. Exemplar migration and the original task's full
closeout remain separately unverified.
