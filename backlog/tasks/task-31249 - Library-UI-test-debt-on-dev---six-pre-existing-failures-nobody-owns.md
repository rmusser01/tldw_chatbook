...
- Not a failing test, but found by the same wave and filed rather than left: the `canvas_sync.py` shared dispatchers take two receiver types with no guard, and 8 of the 10 controllers this program created declare no state accessor. One live production defect had already shipped from this shape (media Select-all/Clear silently full-screen recomposing) and was fixed at the program close. **Filed as TASK-32041**
