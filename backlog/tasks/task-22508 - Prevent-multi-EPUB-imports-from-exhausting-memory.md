...
<!-- SECTION:NOTES:BEGIN -->
Implemented isolated one-process EPUB pool generations while preserving configured ordinary-import parallelism, plus gated off-loop pool retirement and durable retryable failure handling when teardown cannot be proven complete. Added central ZIP/manifest admission limits and a checked ebooklib reader boundary that fails closed before extraction for unsafe member counts, expanded sizes, compression ratios, markup, and unresolved package paths. On a clean branch from current origin/dev, targeted verification completed with 266 passed and 1 Windows-only skip; py_compile, Ruff, and git diff --check passed. The spawned three-EPUB chunking reproduction completed 3/3 with no errors or resident workers, a 62.3 ms maximum event-loop gap, and 496.2 MiB peak tree RSS versus roughly 750 MiB before. Independent review found no remaining Critical or Important issues. Full suite was not run per repository policy. ADR: backlog/decisions/091-bounded-epub-archive-admission.md. Added the retained-worker-RSS incident to backlog/docs/lessons-testing-evidence.md.
<!-- SECTION:NOTES:END -->
