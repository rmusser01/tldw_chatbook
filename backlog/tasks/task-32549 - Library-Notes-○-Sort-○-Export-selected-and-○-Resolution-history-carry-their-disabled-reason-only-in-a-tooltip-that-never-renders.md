...
Seams added to `library_shell_state.py`: `library_disabled_reason_line(label, reason)` (the sentence) and `library_selection_count_line(count, action_label)` (the count-line spelling of it). A `reason=` keyword on `library_disabled_action_label` existed in the first shape and was REMOVED again in `2cca3fdadd` once the reasons moved off the labels — nothing called it, and `library_shell_state.library_disabled_action_label` ships with no `reason` parameter. **AC#1 was updated before the change landed** to name what was actually built.
