...
<!-- SECTION:NOTES:BEGIN -->
Added focused Textual readiness waits for the PR 574 UI CI failures instead of adding fixed sleeps. Settings tests now wait for the target category button, active category, and expected mounted selector/text before querying controls or saving. Library navigation waits for the requested conversation ID and visible title before asserting. The Library mode helper now allows full CI render latency and performs a final text sample before failing. The Console stale-search regression waits for the fresh conversation row ID before asserting stale rows are absent. The latest-dev smoke test waits for the Settings provider SelectOverlay child before exporting the screenshot. A follow-up CI pass exposed additional full-suite races, so the Console stale-search test now starts the fresh beta refresh with the current token after the stale task returns, Settings destination tests wait for mounted copy, Library replay waits for mode controls, and the cached navigation smoke test presses nav buttons directly instead of relying on bottom-row coordinates. Verification run locally: latest 9-test CI failure slice passed; older 9-test PR failure slice passed; remaining 5-test CI failure slice passed; affected UI files passed in 379-test and 445-test runs; git diff --check passed.
<!-- SECTION:NOTES:END -->
