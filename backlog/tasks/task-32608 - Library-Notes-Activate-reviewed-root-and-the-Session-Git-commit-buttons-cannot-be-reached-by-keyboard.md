...
AC#3, scoped honestly, corrected in fix round 1 after the reviewer drove the missing half: the commit side is done end to end from the keyboard in the pinned test -- type the subject, Tab to Review commit, Enter, and the draft leaves the form phase. The activate side is FURTHER along than the first write-up claimed. Driven headlessly against the real screen at the review phase: the view stays `lasting_add`, the work pane's stops are ['notes-sync-body', 'notes-sync-activate', 'notes-sync-back'], ONE Tab from the body reaches #notes-sync-activate (enabled, focusable), and pilot.press('enter') there fires its `press` -- recorded on ReviewActionButton, so the keystroke dispatched the activation rather than merely moving focus. A reviewed root CAN be activated by keyboard. The one remaining gap is the same walk against a REAL vault, where the activation does real work and the receipt can be read back: the journey harness in Tests/UI/test_library_notes_files_sync_journey.py driven with pilot.press('tab')/'enter' instead of Button.press(). It was left out of this branch because two other wave-5 branches are editing the sync runtime and reconciler concurrently. (One unexplained observation, INFERRED: after that Enter, focus landed on library-browse-items-grip, outside the work pane -- the stub runtime made the activation fail, so it may be an artefact. The real-vault walk settles it.)
