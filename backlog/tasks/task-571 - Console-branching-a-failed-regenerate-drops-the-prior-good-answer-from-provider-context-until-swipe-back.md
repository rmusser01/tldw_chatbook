...
- Scoped Ruff formatter checks exited 0 for every TASK-571-owned region: controller lines `11111-11290`, branching failure tests `143-214`, variant recovery/stop controls `365-467`, and the mounted test `80-197`. The full four-file formatter check remains read-only exit 1 (`test_console_variant_stream.py` is already formatted; the controller, branching, and mounted files report drift), but every remaining hunk is outside these owned regions and is present in the `origin/dev` version: ten controller hunks, the branching persistence assertion at current line 274, and mounted helper/retry hunks at current lines 58 and 218. Formatting each `origin/dev` file through Ruff on stdin reproduced those baseline hunks, so unrelated code was not reformatted.
