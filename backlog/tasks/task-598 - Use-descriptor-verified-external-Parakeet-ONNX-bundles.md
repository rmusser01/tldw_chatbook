...
<!-- SECTION:NOTES:BEGIN -->
Implemented descriptor-verified, user-owned external Parakeet v2/v3 INT8/F32 source selection across First Run, Lab Models, and Library job overrides under ADR-050. External roots are verified and used in place; Chatbook retains exact managed-VAD ownership, deterministic source resolution, path-private provenance with a null artifact root, resident source identity, optional install-only managed copy, and explicit later activation.
...
The exact 24-file changed Python test union ran once: 1248 passed, 6 failed, 1 skipped, 16 warnings in 356.88 seconds. Two localhost fixtures were sandbox-blocked, two union-order/import-state nodes passed in isolation, and two UI test-double mismatches remain reproducibly red in isolation. Scoped py_compile and git diff --check passed. Ruff retained two findings and format debt that both reproduce from origin/dev; no bulk format was applied.

Acceptance criteria 1-6 are implemented and evidenced. Criterion 7 stays open because the focused union is not fully green and native Linux x86_64, Linux aarch64, Windows x86_64, and macOS x86_64 source verification/copy-delete/real ONNX CPU gates remain unavailable. TASK-598 remains In Progress; no Linux or Windows readiness claim is made.
<!-- SECTION:NOTES:END -->
