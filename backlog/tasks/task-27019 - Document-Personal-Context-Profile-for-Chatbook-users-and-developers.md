...
- Extension guidance distinguishes a full local-first Sync peer—which owns local canonical storage, key custody, runtime/profile UX, and an ongoing caller—from an API-only client, which uses authenticated server APIs and does not need to implement Chatbook's local profile, interview, injection, or tool stack.
...
Evidence is the shipped owner boundary in `PersonalContextService.remove_local_profile()`, `finish_secure_removal()`, `PersonalContextRepository.destroy_profile_content()`, `_draft_repository()`, `InterviewDraftRepository`, the keyring wrapping/link custodians, and `ProfileContextService` ordering/whole-record budget code. The final verification must cover removal residuals, exact priority order, separate integrator modes, specification status, stable-link-only guide prose, shared-block parity, privacy, task-ID uniqueness, diff, and allowed-file scope.
