...
- [x] #2 ~~The same promptness holds for `close_session`'s deny path.~~ N/A — see "Scope note" above; `close_session`'s deny path is architecturally session-scoped with no independently-fixable promptness gap of the same shape as `shutdown()`'s -- descoped, not delivered.
- [x] #3 A regression test arms a round ahead of session registration, triggers shutdown/close, and asserts the round resolves before its timeout deadline.
<!-- AC:END -->
...
<!-- SECTION:NOTES:BEGIN -->
Fixed the shutdown-snapshot race by widening the single seam all three worker-thread bridges (request_mcp_approvals, request_skill_install_confirm, request_skill_script_confirm) already poll through: `_is_session_cancelled`'s real-session-id branch now ORs in the global, never-reset `_shutdown_requested` Event alongside that session's own `_active_cancel_events` entry, instead of checking only the latter. Verified (grep-audit) `_shutdown_requested` is set exactly once, only inside `shutdown()`, and never `.clear()`-ed anywhere in the module or its tests.

**Correction (review):** the safety argument is NOT "`_shutdown_requested` only fires at real process teardown, which is global by definition" -- `shutdown()` is also called from `ChatScreen.on_unmount` (chat_screen.py:10228) on every ordinary navigation away from the Console screen, not just app exit, so the flag can be set while the app keeps running. Safety instead rests on THIS controller instance's own lifecycle: `ChatScreen._ensure_console_chat_controller` only ever lazily builds a fresh `ConsoleChatController`, and `on_unmount` both runs the outgoing instance's `shutdown()` and drops the screen's reference to it before any later visit's rebuild -- so a permanently-set flag on a torn-down instance is never carried into a later visit's fresh instance, and no round on the torn-down instance could be resolved through a UI that no longer exists anyway. Both `_is_session_cancelled`'s and `shutdown()`'s docstrings in console_chat_controller.py were rewritten to state this correctly (and `task-1052-report.md` corrected to match).
