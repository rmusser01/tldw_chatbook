...
- Addressed PR review feedback by making missing RAG/source and pending approvals blocked inspector states centralizing inspector action IDs/reasons reusing the shared non-negative integer seam helper sharing provider/model source precedence between the Console control bar and inspector and consuming pending Artifacts Chatbook target IDs before latest fallback.
- Verification: `/Users/macbook-dev/Documents/GitHub/tldw_chatbook/.venv/bin/python -m pytest -q Tests/Chat/test_console_display_state.py Tests/UI/test_console_internals_decomposition.py Tests/UI/test_console_live_work_handoffs.py Tests/UI/test_chat_approvals_and_resume.py --tb=short` passed with 77 tests and known warnings; `git diff --check` passed.
<!-- SECTION:NOTES:END -->
