...
**Round 1 (review hardening, item f)**: two Important items closed. (1) Stale-generation guard -- `_submit_fast_decision` resolved via `self._batch_names[0]` with no guard against a batch swap; a fast button from a superseded batch, pressed during the narrow fire-and-forget `remove_children()` window, would resolve the NEW round using the newer `set_batch` call's overwritten `_batch_names`/`_batch_round_id`. Fixed with `self._batch_fast_buttons` (mirrors `self._batch_selects`) and an `on_button_pressed` membership guard identical in shape to `_on_batch_row_select_changed`'s existing one; a stale press is now a silent no-op. (2) Disable-after-press -- Submit and both fast buttons now disable immediately via `_disable_batch_submit_controls` (closing the double-submit window directly rather than relying on incidental safety); `set_batch` re-enables Submit at the start of every new round. Also added the requested bundled-CSS single-row geometry test for the fast buttons; while writing it, found (and worked around, in the new test only) an unrelated pre-existing test-harness race in the sibling multi-row geometry test (early `set_batch` racing `on_mount`'s deferred `_hide_batch_body`, confirmed pre-dating this task via stash-diff) -- out of scope to fix here. Gate 1 rerun: 309 passed (+4, exactly the new tests), the same 24 pre-existing failures by name.
<!-- SECTION:NOTES:END -->
