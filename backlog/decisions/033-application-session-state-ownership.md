...
Runtime-policy persistence is part of the ADR-029 private-data boundary. Its
...
A Chat handoff targets the native Console because TASK-577 retired the legacy
`ChatTabContainer` before this decision was integrated. The consumer
acknowledges the exact claim only after a character session is created or the
payload transfers into screen-local staged live work. Failure or cancellation
before that transfer releases the exact claim, leaving any newer replacement
untouched. Artifact handoffs use exact canonical
...
consumer. Chat can leave a revisioned claim in flight when native Console
delivery is settled through a removed raw application field, and Artifacts can
replace an absent requested target with the latest record from a limited first
page. `pending_notes_workspace_context` is
...
- [ADR-031: Bounded Evaluation and Tool Worker Execution](031-bounded-evaluation-and-tool-worker-execution.md)
