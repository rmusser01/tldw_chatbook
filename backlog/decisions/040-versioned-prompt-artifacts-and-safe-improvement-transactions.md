# ADR-040: Versioned Prompt Artifacts and Safe Improvement Transactions
...
Console block artifacts use schema version 2 because the server already owns a
different structured schema version 1 whose Pydantic model forbids extra
fields. Reusing version 1 would make valid Console block documents fail
validation and would represent incompatible document shapes as the same
version.

Prompt and Recipe identity needs a first-class artifact discriminator because
Library brief rows do not load full definitions, while the type controls browse
labels, selection, execution, usage accounting, and save behavior. A durable
column makes those decisions reliable without fetching and parsing every
definition.
