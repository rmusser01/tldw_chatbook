# ADR-102: Consume portable Notes organization and layer Agent Lessons on ordinary Notes
...
4. **Record synchronization intent in the Notes transaction.** A Notes mutation and
   ...
   transaction. A separate dispatcher copies those intents to the general outbox and
   acknowledges them idempotently. No transaction is claimed across two SQLite files.
...
7. **Make lesson saves guided, verified, and conflict-safe.** Agents search before
   ...
   preconditions. Additive keyword assurance never removes user keywords, and ordinary
   lesson updates preserve current folder membership.
