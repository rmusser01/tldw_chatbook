...
sync version, timestamps, deletion state, retained-history rows, collection
memberships, or usage state. Chatbook manifest entries use archive-local opaque
Prompt IDs such as `prompt-000001`; source database IDs are used only while
collecting the archive and never appear in its paths, manifest, or payloads.

The importer accepts both this strict record and the legacy unversioned
Chatbook Prompt payload (`name`, `description`, and one `content` value). A
legacy `content` value retains its historical meaning as a legacy Prompt's
System lane. Unknown record versions, partial new-format records, invalid field
types, and malformed archive structure fail closed for that record before a
database mutation. Artifact compatibility state is not silently repaired:
...
- Prompt export diagnostics may report a fixed operation, archive-local item
...
- The archive-local Prompt ID differs from the source database ID.
