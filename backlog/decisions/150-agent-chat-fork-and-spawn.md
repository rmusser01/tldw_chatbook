...
- `copy_conversation_active_path` (new, `Chat/chat_conversation_service.py`)
  is the first Console writer of the lineage columns; the shipped name
  describes exactly what it does (copy the active path) instead of avoiding
  collision with the `fork_conversation_into_workspace` misnomer. The copy
  must use the raised-caps/uncapped tree read — silent truncation of long
  chats is a bug and an over-cap fork is a tested case.
