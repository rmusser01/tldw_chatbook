...
1. Add two reserved, authenticated Console capabilities: `search_archived_conversations` and `request_conversation_restore`. V1 is an interactive main-agent feature for ordinary local Console sessions, targeting one local saved conversation at a time. External MCP, headless, temporary and subagent execution do not gain this capability implicitly.
...
4. Add nullable local `archived_at` timestamps to both stores through new migrations. Existing archives retain unknown times. Set atomically on archive, clear on restore, and preserve archive-only sync isolation and version semantics. Make archive-event stamps advance across rapid lifecycle cycles. Order by the latest known currently applicable archive timestamp; label incomplete chronology and expose a route to undated matches under date filters.
5. Runtime-owned immutable result references bind source session, result set and target snapshot. Tool requests select a returned reference; model-authored prose/links never authorize or create UI actions. Bound retention, require refresh after expiration/restart, and revalidate lifecycle state before confirmation and at the write boundary.
...
8. Reuse shared lifecycle storage services, with conversation version checks and transaction-level workspace lifecycle/name checks. Restore workspace first when both are approved, then the captured conversation. The stores are not an atomic distributed transaction. Preserve actual completed writes, report partial outcomes and retry only remaining work with fresh confirmation; never compensate by silently rearchiving or moving data.
9. Runtime ownership outlives a waiting tool or mounted view once a write begins. Deduplicate submissions, report cancellation according to actual committed scope and stop unstarted later writes. Old references and approvals do not replay after restart. No general persistent action journal is introduced for this reversible operation.
