...
**Destination taxonomy (13, as amended).** `SHELL_DESTINATION_ORDER` includes **Lab**, seated after ACP (`navigation_priority=45`), and **Logs**, seated between Lab and Settings. Lab's primary route is `llm` (the Models screen) and it folds `llm_management`, `stts`, and `evals` as legacy routes. Logs owns its existing `logs` route directly; operational diagnostics remain reachable without entering Settings. The full order is: Home, Console, Library, Artifacts, Personas, Watchlists, Schedules, Workflows, MCP, ACP, Lab, Logs, Settings. There is no Skills destination: upstream retired it into Library (Skills sub-project tasks 1-5, route alias `skills` -> `library`), and this ADR adopts that retirement on rebase.
...
The 2026-07-24 amendment records the subsequently established navigation contract that restores Logs as a top-level operational destination. Logs is a live diagnostic surface used during failure investigation, so direct shell and palette access is more appropriate than hiding it behind the broader Settings destination. Stats remains folded under Settings.

## Alternatives Considered

| Option | Why rejected |
| --- | --- |
| Grouped nav of ~20 destinations | A rail of ~20 tabs is unscannable and recreates the retired 18-tab chrome (ADR-014). Folding related screens under 13 destinations keeps the rail readable and gives each fold a clear owner. |
| Keep Logs folded under Settings | Diagnostics are a direct operational workflow during failures; requiring a Settings detour weakens recovery access and contradicts the established top-level route contract. |
...
- `MainNavigationBar` renders 13 destinations; Lab owns Models/Speech/Evals, while Logs remains directly reachable for operational diagnostics.
- Nav active state is truthful on folded screens: on Search the Library button boxes and the header says Search.
- The palette shows one entry per destination; typing a legacy name (`coding`, `media`, `stts`, …) finds the owning destination, while Logs has its own labeled command.
