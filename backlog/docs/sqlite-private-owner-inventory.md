...
| C35 | tldw_chatbook/TTS/profile_repository | TTSProfileRepository._worker_validate_standalone_snapshot | tts.profile_snapshot | read_only_uri | immutable validation read | Migrated via `connect_private_sqlite`. Run full integrity and incremental BLOB checks through a repository-controlled immutable snapshot handle. |
...
| C39 | tldw_chatbook/TTS/profile_schema | peek_profile_store_schema_version | tts.profile_store_version_peek | read_only_uri | schema-version peek | Migrated via `connect_private_sqlite`. Provides a validated read-only diagnostic peek; repository startup does not use it as migration authority and instead recovers and reads the exact version under its exclusive lease. |
