...
| C35 | tldw_chatbook/TTS/profile_repository | TTSProfileRepository._worker_validate_standalone_snapshot | tts.profile_snapshot | private_file | private validation read | Migrated via `connect_private_sqlite`. Run full integrity and incremental BLOB checks through a repository-controlled private snapshot handle. |
