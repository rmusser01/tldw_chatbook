...
| C45 | tldw_chatbook/TTS/profile_migration_recovery | recover_profile_migration_publication | tts.profile_migration_recovery | read_only_uri | immutable pre-open recovery authority validation | Migrated via `connect_private_sqlite`. Startup recovery opens only journal-classified exact active and retained-backup authority after descriptor-pinned content validation and before any profile-store open. Each observed or journal-recorded artifact is capped at 576 MiB (512 MiB reference quota plus 64 MiB bounded SQLite overhead) before incremental hashing. |
