...
- Rich populated-owner multi-profile coverage beyond Stage1, the complete create/inspect/isolated-open/replace/later-rollback sequence, native crash/multiple-volume matrix, product release-gate wiring and qualification CI remain Task26 requirements. Run named feature and inventory/lifecycle checks only; no full-suite run is implied.
