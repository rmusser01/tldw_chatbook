...
## Generated bytes do not establish that a speech model ran

**TASK-32111, Kokoro qualification, 2026-09-09.** Delivery tests had passed while
the advertised PyTorch path used an incompatible placeholder model and random
waveform helpers. Loading the official checkpoint exposed the architecture
mismatch; real MPS inference then exposed an unsupported Fourier operation that
CPU fixtures could not reveal. A Lab observer also initially retained its own
voice/speed defaults while Console correctly followed Global settings.

**What to do.** Pin real model provenance, record the actual upstream inference
class/device and request arguments, and transcribe complete saved speech after
