...
Status: governing document (ADR-042, TASK-2511)
...
Multi-value composite tokens are not used (ADR-042).
...
`$ds-opacity-disabled: 50%` (disabled controls), `$ds-opacity-dim: 70%`
(de-emphasized but readable secondary content). Do **not** stack
`$ds-opacity-disabled` with `$ds-disabled-fg` on text that must be read —
pick one mechanism, or the text falls below readable contrast (this has
happened; see `_agentic_terminal.tcss` line ~5608 comment).
...
- **disabled:** `$ds-opacity-disabled`, `$ds-disabled-bg`,
  `$ds-disabled-fg`; hover on disabled keeps the disabled surface
...
- Multi-value composite tokens (e.g. `border: round $border` as one token)
  — blocked on verified Textual substitution semantics (ADR-042).
