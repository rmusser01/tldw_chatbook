...
=======
=======
>>>>>>> 7c1d59c354 (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
>>>>>>> f6252b879b (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
>>>>>>> 7db697d69a (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
>>>>>>> dd30718abb (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
>>>>>>> f7bc766f18 (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
>>>>>>> c741d10d43 (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
>>>>>>> 61e95be55c (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
>>>>>>> 1d5af77c8f (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
...
<<<<<<< HEAD
<<<<<<< HEAD
>>>>>>> 9d0eae37c3 (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
=======
<<<<<<< HEAD
=======
...
=======
=======
>>>>>>> 751c0cbbfb (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
>>>>>>> 61e95be55c (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
>>>>>>> b463addfd6 (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
...
>>>>>>> b463addfd6 (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
>>>>>>> 88aaedea4b (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
>>>>>>> a6bdba33c1 (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
>>>>>>> fba5ef76a1 (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
>>>>>>> 7db697d69a (feat(llm): migrate summarize_with_groq onto the hosted engine (TASK-32853 phase B 1/6))
=======
    ...
    if isinstance(segments, list):
        for segment in segments:
            ...
            if isinstance(segment, dict) and "Text" in segment:
                ...
                isinstance(segment, dict) and "text" in segment
            ):  # Adding flexibility for key case
                text += segment["text"] + " "
            else:
                logging.warning(
        ...
        logging.warning(
            f"Unexpected type of 'segments': {type(segments)}. Trying to convert to string."
        )
...
def recursive_summarize_chunks(
    ...
    for i, chunk in enumerate(chunks):
        ...
        if i == 0:
            ...
            logging.debug(
                f"Processing combination of previous summary and chunk {i + 1} (total length {len(text_to_process)})."
            )
        ...
        try:
            ...
            if not isinstance(step_result, str):
                ...
                logging.error(
                    f"Recursive step {i + 1} did not return a string. Got: {type(step_result)}"
                )
                return f"Error: Processing step {i + 1} returned unexpected type {type(step_result)}"

            current_summary = step_result
            logging.debug(
                f"Chunk {i + 1} processed. Current summary length: {len(current_summary)}"
            )
...
def extract_text_from_input(input_data: Any) -> str:
    ...
    if isinstance(input_data, str):
        ...
        if "transcription" in input_data:
            ...
            logging.warning(
                "No specific text field found in dict, converting entire dict to string."
            )
            try:
        ...
        logging.warning(
            f"Unhandled input type: {type(input_data)}. Attempting string conversion."
        )
...
def _dispatch_to_api(
    ...
    try:
        ...
        if api_name_lower == "openai":
            ...
            return summarize_with_huggingface(
                api_key, text_to_summarize, custom_prompt_arg, temp, streaming
            )  # system_message not directly passed? Check func def.
        ...
        # --- Local LLM Calls ---
            ...
            return summarize_with_tabbyapi(
                text_to_summarize, custom_prompt_arg, temp, system_message, streaming
            )
        elif api_name_lower == "vllm":
            return summarize_with_vllm(
                api_key,
                text_to_summarize,
                custom_prompt_arg,
                temp,
            ...
            # Custom OpenAI likely needs base_url from config inside its function
            return summarize_with_custom_openai_2(
                api_key,
                text_to_summarize,
                custom_prompt_arg,
                temp,
            ...
            return summarize_with_ollama(
                ...
                system_message,
                streaming,
            )  # Passing None for model, assuming func handles it
        # --- MOCKING TEST LLM Calls ---
        elif api_name_lower == "mock-llm":
            return summarize_with_mock_llm(
                text_to_summarize,
                custom_prompt_arg,
                api_key,
                temp,
...
def analyze(
    ...
    logging.info(
        f"Starting summarization process. API: {api_name}, Recursive: {recursive_summarization}, Chunked: {chunked_summarization}, Streaming: {streaming}"
    )
    ...
    try:
                ...
                try:
                    for chunk in gen:
                        if isinstance(chunk, str):
                            ...
                            logging.warning(
                                f"Generator yielded non-string chunk: {type(chunk)}"
                            )
        ...
        if CHUNKER_AVAILABLE and (recursive_summarization or chunked_summarization):
            if recursive_summarization:
                ...
                chunks_data = improved_chunking_process(
                    text_content, current_chunk_options
                )  # Renamed variable for clarity
                if not chunks_data:
                    logging.warning(
                        "Recursive summarization: Chunking produced no chunks."
                    )
                    return (
                        "Error: Recursive summarization failed - no chunks generated."
                    )

                # Extract just the text from the chunk data
                text_chunks = [chunk["text"] for chunk in chunks_data]
                logging.debug(
                    f"Generated {len(text_chunks)} text chunks for recursive summarization."
                )
                ...
                def recursive_step_processor(text_to_summarize: str) -> str:
                    logging.debug(
                        f"recursive_step_processor called with text length: {len(text_to_summarize)}"
                    )
                    ...
                    if not isinstance(processed_result, str):
                        logging.error(
                            f"API dispatch/consumption did not return a string. Got: {type(processed_result)}"
                        )
                    ...
                    logging.debug(
                        f"recursive_step_processor finished. Result length: {len(processed_result)}"
                    )
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    if isinstance(
                        processed_chunk_summary, str
                    ) and not processed_chunk_summary.startswith("Error:"):
                        chunk_summaries.append(processed_chunk_summary)
                    else:
                        error_detail = (
                            processed_chunk_summary
                            if isinstance(processed_chunk_summary, str)
                            else "Unknown error"
                        )
                        logging.warning(
            ...
            final_result = _dispatch_to_api(
                ...
                temp,
                system_message,
                streaming=effective_streaming_for_api_call,
            )
        ...
        if (
            streaming
            and not recursive_summarization
            and not chunked_summarization
            and inspect.isgenerator(final_result)
        ):
            ...
            logging.debug(
                "Consuming final result (if generator) as streaming=False or chunking was used."
            )
            ...
            if final_string_summary is None:
                ...
                logging.error(
                    f"Unexpected final result type after processing: {type(final_string_summary)}"
                )
...
def summarize_with_openai(
    ...
    try:
        ...
        if not api_key or not api_key.strip():
            logging.error(
                "OpenAI: API key not found or is empty (checked param and config)."
            )
            ...
            def _stream_events():
                for line in response.iter_lines():
                    ...
                    if event.get("type") == "content-delta":
                        chunk = (
                            ((event.get("delta") or {}).get("message") or {}).get(
                                "content"
                            )
                            or {}
                        ).get("text")
...
def summarize_with_openrouter(
    ...
    try:
        ...
        openrouter_model = get_cli_setting(
...
def summarize_with_huggingface(
    ...
    try:
        ...
        huggingface_model = get_cli_setting(
        ...
        if streaming:
            ...
            if response.status_code == 200:
                ...
                if (
                    ...
                    and "generated_text" in response_json[0]
                ):
    ...
    try:
        ...
        if isinstance(input_data, str):
            if input_data.strip().startswith("{"):
                ...
                logging.debug(
                    "Google: Parsing provided JSON string data for summarization"
                )
                try:
                    data = json.loads(input_data)
...
def summarize_with_mock_llm(
    ...
    try:
        ...
        sample_text = (
            str(text_to_summarize)[:50] + "..."
            if len(str(text_to_summarize)) > 50
            else str(text_to_summarize)
        )
