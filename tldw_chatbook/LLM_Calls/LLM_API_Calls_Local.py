...
def _extract_text_from_message_content(
    content: Union[str, List[Dict[str, Any]]], provider_name: str, msg_index: int
) -> str:
...
def _chat_with_openai_compatible_local_server(
    ...
    system_message: Optional[
        str
    ] = None,  # This will be prepended to messages by this function
    ...
    # Log request metrics
    log_counter(
        "local_openai_compatible_api_request",
        labels={
    ...
    if images_present_in_payload:
        logging.info(
            f"{provider_name}: Multimodal content (images) detected in messages payload. "
    ...
    if top_k is not None:
        payload["top_k"] = (
            top_k  # OpenAI spec doesn't have top_k for chat, but some servers might
        )
    ...
    if top_logprobs is not None:  # Can only be used if logprobs is true
        if logprobs:
            ...
            logging.warning(
                f"{provider_name}: top_logprobs provided without logprobs=True. Ignoring top_logprobs."
            )
    ...
    try:
        ...
        if streaming:
            ...
            log_histogram(
                "local_openai_compatible_api_response_time",
                duration,
                labels={
                    "provider": provider_name,
                    "model": model_name or "default",
            ...
            log_counter(
                "local_openai_compatible_api_success",
                labels={
                    "provider": provider_name,
                    "model": model_name or "default",
                    "streaming": "true",
            ...
            def stream_generator():
                try:
                    ...
                    logging.error(
                        f"{provider_name}: ChunkedEncodingError during stream: {e_chunked}",
                        exc_info=False,
                    )
                    error_content = {
                        "error": {
                            "message": f"Stream connection error: {str(e_chunked)}",
                            "type": "stream_error",
                            "code": "chunked_encoding_error",
                        }
                    }
                    yield f"data: {json.dumps(error_content)}\n\n"
                except Exception as e_stream:
                    logging.error(
                        f"{provider_name}: Error during stream iteration: {e_stream}",
                        exc_info=True,
                    )
                    error_content = {
                        "error": {
            ...
            if usage:
                log_histogram(
                    "local_openai_compatible_api_input_tokens",
                ...
                log_histogram(
                    "local_openai_compatible_api_output_tokens",
                ...
                log_histogram(
                    "local_openai_compatible_api_total_tokens",
        ...
        log_counter(
            "local_openai_compatible_api_error",
            labels={
                "provider": provider_name,
                "model": model_name or "default",
        ...
        log_histogram(
            "local_openai_compatible_api_error_response_time",
            duration,
            labels={
                "provider": provider_name,
                "model": model_name or "default",
...
def chat_with_local_llm(
    ...
    # --- Settings Load ---
    cfg = get_runtime_config_snapshot().values.get("local-llm", {})
    ...
    current_streaming = (
        streaming if streaming is not None else cfg.get("streaming", False)
    )
    ...
    current_max_tokens = (
        max_tokens if max_tokens is not None else int(cfg.get("max_tokens", 4096))
    )
...
def chat_with_llama(
    ...
    n_probs: Optional[
        int
    ] = None,  # from map (maps to 'n' in generic, but llama.cpp might use it for top_logprobs count if logprobs enabled)
    ...
    api_url: Optional[
        str
    ] = None,  # This is specific to this function's call from API_CALL_HANDLERS if special handling exists
    ...
    llama_cpp_config_key_in_api_settings = (
        provider_name if provider_name else "llama_cpp"
    )
    ...
    if (
        not current_model
    ):  # Check after attempting to load from argument and then config
        # Using the original provider string "llama_api" from your snippet for the error.
        logging.info(
            "Llama.cpp: Model name not passed and could not be determined from arguments or configuration. Using default model if available."
        )

    current_temp = (
        temp if temp is not None else float(llama_config.get("temperature", 0.7))
    )  # llama.cpp native name is temperature
    current_streaming = (
        streaming if streaming is not None else llama_config.get("streaming", False)
    )
    ...
    current_max_tokens = (
        ...
        else int(llama_config.get("max_tokens", llama_config.get("n_predict", 4096)))
    )  # use n_predict if passed
    ...
    current_response_format = (
        response_format
        if response_format is not None
        else llama_config.get("response_format")
    )
    current_logit_bias = (
        logit_bias if logit_bias is not None else llama_config.get("logit_bias")
    )
    current_presence_penalty = (
        presence_penalty
        if presence_penalty is not None
        else llama_config.get("presence_penalty")
    )
    current_frequency_penalty = (
        frequency_penalty
        if frequency_penalty is not None
        else llama_config.get("frequency_penalty")
    )
    current_logprobs = (
        logprobs if logprobs is not None else llama_config.get("logprobs")
    )
    current_top_logprobs = (
        top_logprobs if top_logprobs is not None else llama_config.get("top_logprobs")
    )
    ...
    current_n = (
        ...
        else llama_config.get("n", llama_config.get("n_probs"))
    )
    ...
    if custom_prompt:
        logging.info(
            "Llama.cpp: 'custom_prompt' received. Ensure it's incorporated into 'input_data' or 'system_prompt' by the calling function."
        )
...
def chat_with_kobold(
    ...
    if not current_model:
        logging.info(
            "Kobold API model namenot passed and or could not be determined from arguments or configuration."
        )
    ...
    current_temp = (
        temp if temp is not None else float(cfg.get("temperature", 0.7))
    )  # Kobold native 'temp'
    current_top_k = top_k if top_k is not None else cfg.get("top_k")
    current_top_p = top_p if top_p is not None else cfg.get("top_p")
    current_max_length = (
        max_length if max_length is not None else int(cfg.get("max_length", 200))
    )
    current_stop_sequence = (
        stop_sequence if stop_sequence is not None else cfg.get("stop_sequence")
    )
    current_num_responses = (
        num_responses if num_responses is not None else cfg.get("num_responses")
    )
    current_seed = seed if seed is not None else cfg.get("seed")
    ...
    if current_streaming:
        logging.warning(
            "KoboldAI (Native): Streaming with /api/v1/generate is often non-standard. "
    ...
    max_context_length = int(
        cfg.get("max_context_length", 2048)
    )  # Kobold uses max_context_length for context window
    ...
    for i, msg in enumerate(input_data):
        ...
        text_content = _extract_text_from_message_content(
            msg.get("content"), "KoboldAI (Native)", i
        )
    ...
    if custom_prompt_input:  # This was mapped from 'prompt' in chat_api_call
        ...
        logging.info(
            "KoboldAI (Native): Appending 'custom_prompt_input' to the prompt."
        )
        full_prompt_parts.append(custom_prompt_input)

    final_prompt_string = "\n\n".join(
        filter(None, full_prompt_parts)
    ).strip()  # filter(None,...) removes empty strings
    ...
    try:
        ...
        if (
            ...
            log_counter(
                "kobold_api_success",
                labels={"model": current_model or "default", "streaming": "false"},
            )
        ...
        log_histogram(
            "kobold_api_error_response_time",
            duration,
            labels={
                "model": current_model or "default",
                "status_code": str(status_code),
        ...
        log_counter(
            "kobold_api_error",
            labels={"model": current_model or "default", "error_type": "network_error"},
        )
        log_histogram(
            "kobold_api_error_response_time",
            duration,
...
def chat_with_oobabooga(
    ...
    if custom_prompt:
        logging.info(
            "Oobabooga: 'custom_prompt' received. Ensure it's incorporated into 'input_data' or 'system_prompt'."
        )
...
def chat_with_tabbyapi(
    ...
    current_temp_val = (
        temp
        if temp is not None
        else float(cfg.get("temperature", cfg.get("temp", 0.7)))
    )
    current_streaming = (
        streaming if streaming is not None else cfg.get("streaming", False)
    )
    ...
    if custom_prompt_input:
        logging.info(
            "TabbyAPI: 'custom_prompt_input' received. Ensure incorporated if needed."
        )
...
def chat_with_vllm(
    ...
    system_prompt: Optional[
        str
    ] = None,  # from map (mapped from generic 'system_message')
    streaming: Optional[bool] = False,  # from map
    model: Optional[str] = None,  # from map
    top_k: Optional[int] = None,  # from map
    ...
    vllm_api_url: Optional[
        str
    ] = None,  # Specific config, not from generic map typically
    ...
    )  # func arg 'temperature' is vLLM's name
    current_streaming = (
        streaming if streaming is not None else cfg.get("streaming", False)
    )
    current_top_p = (
        top_p if top_p is not None else cfg.get("top_p")
    )  # func arg 'top_p' is vLLM's name
    current_top_k = top_k if top_k is not None else cfg.get("top_k")
    current_min_p = min_p if min_p is not None else cfg.get("min_p")
    current_max_tokens = (
        max_tokens if max_tokens is not None else int(cfg.get("max_tokens", 4096))
    ...
    if custom_prompt_input:
        logging.info(
            "vLLM: 'custom_prompt_input' received. Ensure incorporated if needed."
        )
...
def chat_with_aphrodite(
    ...
    if custom_prompt:
        logging.info(
            "Aphrodite: 'custom_prompt' received. Ensure incorporated if needed."
        )
...
def chat_with_ollama(
    ...
    ] = None,  # from map (mapped from generic 'response_format', e.g. "json" or {"type":"json_object"})
    presence_penalty: Optional[float] = None,  # from map
    frequency_penalty: Optional[float] = None,  # from map
    ...
    current_streaming_arg = (
        streaming if streaming is not None else cfg.get("streaming", False)
    )
    current_streaming = (
        bool(current_streaming_arg)
        if not isinstance(current_streaming_arg, bool)
        else current_streaming_arg
    )
    ...
    current_top_k = (
        top_k if top_k is not None else cfg.get("top_k")
    ...
    current_max_tokens = (
        num_predict if num_predict is not None else int(cfg.get("max_tokens", 4096))
    )
    ...
    if current_stop is None and cfg.get("stop") is not None:
        current_stop = cfg.get(
            "stop"
        )  # TOML can define this as string or list of strings
    ...
    if isinstance(candidate_format_value, str):
        ...
        if (
            candidate_format_value.get("type") == "json_object"
    ...
    api_retries = int(
        cfg.get("api_retries", 1)
    )  # Default to 1 from CONFIG_TOML_CONTENT
    api_retry_delay = int(
        cfg.get("api_retry_delay", 2)
    )  # Default to 2 from CONFIG_TOML_CONTENT

    if isinstance(
        current_streaming, str
    ):  # Should be bool by now from config or arg, but defensive
        current_streaming = current_streaming.lower() == "true"

    if custom_prompt:
        logging.info(
            "Ollama: 'custom_prompt' (function argument) received. Ensure this is correctly incorporated "
...
def chat_with_custom_openai(
    ...
    )  # Mapped param is 'temp'
    current_streaming = (
        streaming if streaming is not None else cfg.get("streaming", False)
    )
    current_top_p = (
        maxp if maxp is not None else cfg.get("top_p", cfg.get("maxp"))
    )  # Mapped param is 'maxp'
    current_top_k = (
        topk if topk is not None else cfg.get("top_k", cfg.get("topk"))
    )  # Mapped param is 'topk'
    current_min_p = (
        minp if minp is not None else cfg.get("min_p", cfg.get("minp"))
    )  # Mapped param is 'minp'
    current_max_tokens = (
        max_tokens if max_tokens is not None else int(cfg.get("max_tokens", 4096))
    )
    current_seed = seed if seed is not None else cfg.get("seed")
    ...
    current_user_identifier = (
        user_identifier
        if user_identifier is not None
        else cfg.get("user_identifier", cfg.get("user"))
    )
    current_logit_bias = logit_bias if logit_bias is not None else cfg.get("logit_bias")
    current_presence_penalty = (
        presence_penalty
...
def chat_with_custom_openai_2(
    ...
    if not current_model:
        raise ChatConfigurationError(
            provider=cfg_section, message=f"{cfg_section} Model required."
        )
    ...
    retry_cfg = loaded_config_data.get(
        "custom_openai_2_api", cfg
    )  # Fallback to main cfg if specific retry section missing
    api_retries = int(retry_cfg.get("api_retries", cfg.get("api_retries", 1)))
    api_retry_delay = int(
        retry_cfg.get("api_retry_delay", cfg.get("api_retry_delay", 1))
    )
    ...
    if custom_prompt_arg:
        logging.info(
            f"{cfg_section}: 'custom_prompt_arg' received. Ensure incorporated if needed."
        )
...
def save_summary_to_file(summary: str, file_path: str):  # Type hinting
    ...
    try:
        ...
        logging.error(
            f"Error saving summary to file '{summary_file_path}': {e}", exc_info=True
        )
    ...
    **kwargs: Any,  # To catch any other params from API_CALL_HANDLERS
    ...
    current_streaming = (
        ...
        if streaming is not None
            ...
            if isinstance(current_streaming_cfg, bool)
            else str(current_streaming_cfg).lower() == "true"
        )
    )
    current_max_tokens = (
        max_tokens if max_tokens is not None else int(mlx_cfg.get("max_tokens", 2048))
    )
    current_top_p = (
        top_p if top_p is not None else float(mlx_cfg.get("top_p", 1.0))
    )  # mlx-lm server default top_p is 1.0
    ...
    current_response_format = kwargs.get(
        "response_format", mlx_cfg.get("response_format")
    )
    ...
    current_presence_penalty = kwargs.get(
        "presence_penalty", mlx_cfg.get("presence_penalty")
    )
    current_frequency_penalty = kwargs.get(
        "frequency_penalty", mlx_cfg.get("frequency_penalty")
    )
    current_logit_bias = kwargs.get("logit_bias", mlx_cfg.get("logit_bias"))
    current_user_identifier = kwargs.get(
        "user_identifier", mlx_cfg.get("user_identifier", mlx_cfg.get("user"))
    )
