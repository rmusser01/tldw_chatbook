...
def _ollama_request(
    ...
    **kwargs: Any,
    ...
    log_counter(
        ...
        labels={"method": method, "endpoint": endpoint_name},
    )
    ...
    logging.debug(
        f"Ollama Request: {method} {full_url} | Payload: {kwargs.get('json', kwargs.get('params', 'None'))}"
    )
    try:
        with requests.Session() as session:
            ...
            if stream_log_callback:
                ...
                if response.status_code >= 400:  # Error before streaming (e.g. 404)
                    ...
                    logging.error(
                        f"Ollama stream initiation error: {err_msg} for {method} {full_url}"
                    )
                    return None, err_msg
                    ...
                    if line_bytes:
                        ...
                        try:
                            ...
                            if isinstance(json_line, dict):
                                ...
                                if (
                                    "error" in json_line
                                ):  # Check for errors within the stream content
                                    accumulated_error_messages.append(
                                        str(json_line["error"])
                                    )
                ...
                if accumulated_error_messages:
                    ...
                    log_counter(
                        "ollama_api_stream_error", labels={"endpoint": endpoint_name}
                    )
                ...
                if response.content and "application/json" in response.headers.get(
                    ...
                    return {
                        "status": "success",
                        "message": f"Operation {method} on {endpoint} successful (200 OK, No Content).",
                    }, None
                    ...
                    return {
                        "status": "success",
                        "message": f"Operation successful (200), non-JSON response: {response.text[:100]}",
                    }, None
        ...
        logging.error(
            f"Ollama request HTTP error for {method} {full_url}: {err_msg}",
            exc_info=False,
        )
        log_counter(
            "ollama_api_request_error",
        ...
        logging.error(
            f"Ollama connection error for {method} {full_url}: {err_msg}",
            exc_info=False,
        )
        log_counter(
            "ollama_api_request_error",
            labels={"endpoint": endpoint_name, "error": "connection_error"},
        )
        return None, err_msg
        ...
        log_counter(
            "ollama_api_request_error",
            labels={"endpoint": endpoint_name, "error": "timeout"},
        )
        return None, err_msg
    except requests.exceptions.RequestException as e:
        err_msg = f"Request Exception: An unexpected error occurred: {e}"
        logging.error(
            f"Ollama generic request error for {method} {full_url}: {err_msg}",
            exc_info=True,
        )
        log_counter(
            "ollama_api_request_error",
    ...
    ) as e:  # Fallback for non-streaming JSON decode issues
        ...
        log_counter(
            "ollama_api_request_error",
            labels={"endpoint": endpoint_name, "error": "json_decode_error"},
        )
        return None, err_msg
...
def ollama_model_info(
    ...
    logging.debug(
        f"ollama_model_info: Requesting /api/show for model '{model_name}' from {base_url}"
    )
...
def ollama_copy_model(
    ...
    logging.debug(
        f"ollama_copy_model: Requesting /api/copy from '{source}' to '{destination}' at {base_url}"
    )
...
def ollama_delete_model(
    ...
    logging.debug(
        f"ollama_delete_model: Requesting /api/delete for model '{model_name}' from {base_url}"
    )
...
def ollama_pull_model(
    ...
    log_counter(
        ...
        labels={"model": model_name, "insecure": str(insecure)},
    )
...
def ollama_generate_embeddings(
    ...
    logging.debug(
        f"ollama_generate_embeddings: Requesting /api/embeddings for model '{model_name}' from {base_url}"
    )
...
def ollama_list_running_models(
    ...
    if result and not error:
        ...
        for model in models:
            if "size" in model:
                log_histogram(
                    "ollama_running_model_memory_bytes",
                    model["size"],
                    labels={"model": model.get("name", "unknown")},
                )
