...
def get_video_generation_config(*, reload: bool = False) -> VideoGenerationConfig:
    ...
    # TASK-32808.4's one flag vocabulary, via the machinery wrapper that also
    # surfaces a malformed value. The local copy this replaced matched
    ...
    # nothing happened. A typo (`allow_uploads = "yess"`) was the same silent
    # no-op until `coerce_bool_flag_or_warn` started logging it.
    ...
    config = VideoGenerationConfig(
        ...
        confirm_cost_estimate=coerce_bool_flag_or_warn(
            section.get("confirm_cost_estimate"),
            DEFAULT_CONFIRM_COST_ESTIMATE,
            section="video_generation",
            key="confirm_cost_estimate",
        ),
        ...
        minimax_video_allow_uploads=coerce_bool_flag_or_warn(
            section.get("minimax_video_allow_uploads"),
            False,
            section="video_generation.minimax",
            key="allow_uploads",
        ),
