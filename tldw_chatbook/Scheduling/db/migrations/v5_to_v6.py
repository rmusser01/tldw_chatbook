...
def migrate(db: _MigrationCapableDB) -> None:
    """Apply the v5 -> v6 schema migration. Idempotent."""
    with closing(db._get_connection()) as conn:
        ...
        conn.commit()
    logger.debug("Scheduling schema migrated to version 6 (task_incidents)")


def rollback(db: _MigrationCapableDB) -> None:
    """Drop the incidents table, returning ``db`` to schema version 5."""
    ...
    # left `task_incidents` gone with the schema still claiming v6.
