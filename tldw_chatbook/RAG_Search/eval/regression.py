# regression.py
# Description: Regression detection for the RAG evaluation harness.
#
"""Ported from tldw_server2 rag_service/regression.py (P1, RAG server-port programme).
...
from .gating import GatingConfig, MetricCategory
...
def environment_mismatch(baseline: MetricBaseline, fingerprint: dict[str, Any]) -> list[str]:
    ...
    of the RAG server-port P1 plan builds the fingerprint dict (embedding
    model id, sentence-transformers version, corpus hash, platform, etc.)
    and passes it here together with a loaded baseline; this helper only
    reports which keys differ between what the baseline recorded and the
    current environment. It does not decide what to do about a mismatch --
    that policy (e.g. treating a mismatch as "environment changed" rather
    than a genuine regression) lives in Task 7.

    Args:
        baseline: The loaded baseline to compare against.
        fingerprint: Current environment fingerprint, keyed the same way as
            ``baseline.metadata["environment"]``.

    Returns:
        Sorted list of keys whose values differ between the baseline's
        recorded environment and ``fingerprint`` (a key present on only one
        side counts as differing). An empty list means the environments
        match.
    """
    baseline_environment = baseline.metadata.get("environment", {})
    differing_keys = {
        key
        for key in set(baseline_environment) | set(fingerprint)
        if baseline_environment.get(key) != fingerprint.get(key)
    }
