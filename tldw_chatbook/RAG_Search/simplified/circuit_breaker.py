...
class CircuitBreaker(Generic[T]):
    ...
    def __init__(self, name: str, config: Optional[CircuitBreakerConfig] = None):
        ...
        logger.info(
            f"Circuit breaker '{name}' initialized with threshold={self.config.failure_threshold}, "
    ...
    def _should_attempt_reset(self) -> bool:
        ...
        recovery_timeout = (
            self._state.current_recovery_timeout or self.config.recovery_timeout
        )
        ...
        with self._lock:
            ...
            if self._state.state == CircuitState.HALF_OPEN:
                ...
                if self._state.success_count >= self.config.success_threshold:
                    ...
                    self._state.current_recovery_timeout = (
    ...
    def get_stats(self) -> dict:
        ...
        if self._state.state == CircuitState.OPEN and self._state.last_failure_time:
            ...
            stats["time_until_recovery"] = max(
                0, current_timeout - (time.time() - self._state.last_failure_time)
            )
