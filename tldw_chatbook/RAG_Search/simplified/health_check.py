...
class RAGHealthChecker:
    ...
    async def check_vector_store_health(self) -> ComponentHealth:
        ...
        try:
            ...
            if "error" in stats:
                ...
                status = HealthStatus.HEALTHY
                message = (
                    f"Vector store operational ({stats.get('count', 0)} documents)"
                )
    ...
    async def check_cache_health(self) -> ComponentHealth:
        ...
        try:
            ...
            if not metrics.get("enabled", False):
                ...
                metrics.get("hit_rate", 0) < 0.1
                and metrics.get("total_requests", 0) > 100
            ):
                status = HealthStatus.DEGRADED
                ...
                status = HealthStatus.HEALTHY
                message = (
                    f"Cache operational (hit rate: {metrics.get('hit_rate', 0):.1%})"
                )
    ...
    def check_circuit_breakers_health(self) -> ComponentHealth:
        ...
        try:
            ...
            if open_breakers:
                ...
                status = HealthStatus.DEGRADED
                message = (
                    f"Circuit breakers recovering: {', '.join(half_open_breakers)}"
                )
            else:
                status = HealthStatus.HEALTHY
    ...
    async def check_all_health(self) -> Dict[str, Any]:
        ...
        if (
            self._cached_health
            and time.time() - self._last_check_time < self._cache_duration
        ):
        ...
        checks.extend(
            [self.check_circuit_breakers_health(), self.check_system_resources_health()]
        )
        ...
        health_report = {
            ...
            "summary": {
                ...
                "healthy": len(
                    [c for c in components if c.status == HealthStatus.HEALTHY]
                ),
