...
class EnhancedRAGService(RAGService):
    ...
    def __init__(
        ...
        self.expand_context_on_retrieval = getattr(
            config, "expand_context_on_retrieval", True
        )
        self.clean_pdf_artifacts = getattr(config, "clean_pdf_artifacts", True)

        logger.info(
            f"Initialized EnhancedRAGService with parent_retrieval={enable_parent_retrieval}"
        )
    ...
    async def index_document_with_parents(
        ...
        try:
            ...
            for i, chunk in enumerate(retrieval_chunks):
                ...
                parent_idx = chunk.get("parent_chunk_index") or chunk.get(
                    "metadata", {}
                ).get("parent_chunk_index")
            ...
            log_histogram(
                "enhanced_rag_retrieval_chunks_created", len(retrieval_chunks)
            )
            ...
            log_counter(
                "enhanced_rag_document_index_error", labels={"error": type(e).__name__}
            )
    ...
    async def index_batch_with_parents(
        ...
        logger.info(
            f"Starting enhanced batch indexing for {total} documents with parent retrieval"
        )
        ...
        (
            ...
            doc_chunk_info,
            failed_results,
        ) = await chunk_documents_with_parents(
            self,
            documents,
            self.parent_size_multiplier,
            show_progress,
            use_structural_chunking,
        )
        ...
        (
            ...
            failed_indices,
            _,
        ) = await generate_embeddings_for_parent_retrieval(
            self,
            retrieval_chunks,
            parent_chunks,
            batch_size,
    ...
    async def search_with_context_expansion(
        ...
        for result in results:
            ...
            if has_parent and parent_text:
                ...
                # Log expansion
                log_counter(
                    "enhanced_rag_context_expansion", labels={"type": "parent_chunk"}
                )
            else:
        ...
        logger.info(
            f"Expanded {sum(1 for r in expanded_results if r.metadata.get('context_expanded'))} results to parent context"
        )
    ...
    async def get_chunk_with_context(
        self,
        chunk_id: str,
        context_type: Literal["parent", "surrounding", "both"] = "parent",
        ...
        logger.warning(
            f"get_chunk_with_context not fully implemented for chunk_id: {chunk_id}"
        )
        return None
