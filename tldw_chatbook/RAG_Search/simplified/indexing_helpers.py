...
async def chunk_documents_batch(
    rag_service, documents: List[Dict[str, Any]], show_progress: bool = True
...
async def generate_embeddings_batch(
...
) -> Tuple[Union["np.ndarray", List[List[float]]], List[int]]:
    """
    ...
    """
    logger.info(
        f"Phase 2: Generating embeddings for {len(chunk_texts)} chunks in batches of {batch_size}..."
    )
    ...
    for i in range(0, len(chunk_texts), batch_size):
        ...
        try:
            batch_embeddings = await rag_service.embeddings.create_embeddings_async(
                batch
            )
            ...
            if retry_failed and len(batch) > 1:
                ...
                for j, (text, idx) in enumerate(zip(batch, batch_indices)):
                    try:
                        ...
                        single_embedding = (
                            await rag_service.embeddings.create_embeddings_async([text])
                        )
                ...
                if NUMPY_AVAILABLE:
                    fallback_embeddings = [
                        np.zeros(rag_service._embedding_dim) for _ in batch
                    ]
                else:
                    fallback_embeddings = [
                        [0.0] * rag_service._embedding_dim for _ in batch
                    ]
    ...
    if failed_indices:
        logger.warning(
            f"Failed to generate embeddings for {len(failed_indices)} chunks out of {len(chunk_texts)}"
        )
...
async def store_documents_batch(
    ...
    all_embeddings: Union["np.ndarray", List[List[float]]],
    batch_start_time: float,
    failed_embedding_indices: Optional[List[int]] = None,
) -> List[IndexingResult]:
    """
    ...
    for doc_info in doc_chunk_info:
        ...
        try:
            ...
            if not valid_chunks:
                logger.warning(
                    f"All chunks failed for document {doc['id']}, skipping storage"
                )
                results.append(
                    IndexingResult(
                        doc_id=doc["id"],
            ...
            await rag_service._store_chunks(
                chunk_ids, embeddings_to_store, chunk_texts, chunk_metadata
            )
            ...
            if failed_chunk_count > 0:
                logger.warning(
                    f"Document {doc['id']}: {failed_chunk_count} out of {chunk_count} chunks failed"
                )
