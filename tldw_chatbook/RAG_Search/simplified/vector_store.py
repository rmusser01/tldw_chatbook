...
class ChromaVectorStore:
    ...
    def __init__(
        ...
        # Log initialization
        logger.info(
            f"Initializing ChromaVectorStore: collection={collection_name}, "
    ...
    def add(
        ...
        log_counter(
            "vector_store_add_attempt", labels={"collection": self.collection_name}
        )
        ...
        if NUMPY_AVAILABLE and isinstance(embeddings, np.ndarray):
            if embeddings.shape[0] != len(ids):
                raise ValueError(
                    f"embeddings shape {embeddings.shape} doesn't match {len(ids)} documents"
                )
            ...
            if len(embeddings) != len(ids):
                raise ValueError(
                    f"embeddings length {len(embeddings)} doesn't match {len(ids)} documents"
                )
        ...
        log_histogram(
            "vector_store_document_length", sum(doc_lengths) / len(doc_lengths)
        )
        ...
        if metadata_sizes:
            log_histogram(
                "vector_store_metadata_size", sum(metadata_sizes) / len(metadata_sizes)
            )
    ...
    def search(
        ...
        *,
        ...
        # Log search attempt
        log_counter(
            "vector_store_search_attempt", labels={"collection": self.collection_name}
        )
        ...
        if NUMPY_AVAILABLE and isinstance(query_embedding, np.ndarray):
            ...
            raise TypeError(
                f"query_embedding must be numpy array or list, got {type(query_embedding)}"
            )
        ...
        try:
            ...
            if scores:
                log_histogram(
                    "vector_store_search_avg_score", sum(scores) / len(scores)
                )
            ...
            logger.info(
                f"Search completed in {elapsed:.3f}s, found {len(search_results)} results"
            )
    ...
    def search_with_citations(
        ...
        *,
        ...
        logger.debug(
            f"Search with citations returned {len(results_with_citations)} results"
        )
        return results_with_citations

    def _create_citations_from_result(
        self, result: SearchResult, query_text: str
    ) -> List[Citation]:
    ...
    def get_collection_stats(self) -> dict:
        ...
        try:
            ...
            logger.info(
                f"Collection stats: {count} documents, {len(metadata_fields)} metadata fields"
            )
...
class InMemoryVectorStore:
    ...
    def __init__(
        ...
        logger.info(
            f"InMemoryVectorStore initialized with max_documents={max_documents}, "
    ...
    def _check_memory_pressure(self) -> bool:
        ...
        try:
            ...
            if (
                ...
                or available_mb < MIN_SYSTEM_MEMORY_MB
            ):
                logger.warning(
                    f"Memory pressure detected: process={memory_mb:.1f}MB, "
    ...
    def _evict_for_memory_pressure(
        self, target_reduction_ratio: float = MEMORY_PRESSURE_REDUCTION
    ):
        """
    ...
    def _evict_lru(self) -> None:
        ...
        logger.debug(
            f"Evicted document {lru_id} (total evictions: {self._eviction_count})"
        )

    def _compute_similarity(
        self,
        query_embedding: Union[np.ndarray, List[float]],
        doc_embedding: Union[np.ndarray, List[float]],
    ) -> float:
        ...
        if NUMPY_AVAILABLE:
            ...
            if self.distance_metric == "cosine":
                ...
                # Compute L2 distance
                distance = (
                    sum((q - d) ** 2 for q, d in zip(query_embedding, doc_embedding))
                    ** 0.5
                )
    ...
    def search(
        self,
        query_embedding_or_collection: Optional[
...
def create_vector_store(
    ...
    **kwargs,
    ...
    if store_type == "chroma":
        ...
        logger.warning(
            f"Unknown vector store type: {store_type}. Using in-memory store."
        )
        max_documents = kwargs.pop("max_documents", 10000)
        return InMemoryVectorStore(
            distance_metric=distance_metric,
