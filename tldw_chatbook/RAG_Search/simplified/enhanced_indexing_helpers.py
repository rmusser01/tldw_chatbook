...
async def chunk_documents_with_parents(
    ...
    for i, doc in enumerate(documents):
        ...
        try:
            if use_enhanced_chunking and chunking_service:
                ...
                parent_chunk_size = (
                    rag_service.config.chunk_size * parent_size_multiplier
                )
                parent_overlap = (
                    rag_service.config.chunk_overlap * parent_size_multiplier
                )
    ...
    logger.info(
        f"Created {len(all_retrieval_chunks)} retrieval chunks and {len(all_parent_chunks)} parent chunks"
    )
...
async def store_documents_with_parents(
    ...
    for doc_info in doc_chunk_info:
        ...
        try:
            ...
            doc_embeddings = retrieval_embeddings[
            ...
            # Create result
            error_msg = (
                f"{failed_chunk_count} chunks failed"
