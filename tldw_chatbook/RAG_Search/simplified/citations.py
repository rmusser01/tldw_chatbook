...
class Citation:
    ...
    def __post_init__(self):
        ...
        if self.end_char < self.start_char:
            raise ValueError(
                f"end_char must be >= start_char, got start={self.start_char}, end={self.end_char}"
            )
    ...
    def format_citation(self, style: str = "inline") -> str:
        ...
        if style == "inline":
            ...
            return (
                f"{self.document_title} [{self.document_id}:{self.chunk_id}] "
...
class SearchResultWithCitations:
    ...
    def format_with_citations(
        self, style: str = "inline", max_citations: Optional[int] = None
    ) -> str:
        """
        ...
        if max_citations is not None and len(citations_to_use) > max_citations:
            ...
            citations_to_use = sorted(
                citations_to_use, key=lambda c: c.confidence, reverse=True
            )[:max_citations]
        ...
        exact_citations = [
            c for c in citations_to_use if c.match_type == CitationType.EXACT
        ]
        other_citations = [
            c for c in citations_to_use if c.match_type != CitationType.EXACT
        ]
...
def merge_citations(citations_list: List[List[Citation]]) -> List[Citation]:
    ...
    for citations in citations_list:
        for citation in citations:
            ...
            if (
                key not in citation_map
                or citation.confidence > citation_map[key].confidence
            ):
...
def filter_overlapping_citations(
    citations: List[Citation], prefer_exact: bool = True
) -> List[Citation]:
    """
    ...
    for doc_id, doc_citations in doc_groups.items():
        ...
        for citation in sorted_citations:
            ...
            for kept in kept_citations:
                if (
                    citation.start_char < kept.end_char
                    and citation.end_char > kept.start_char
                ):
                    overlaps = True
                    break
