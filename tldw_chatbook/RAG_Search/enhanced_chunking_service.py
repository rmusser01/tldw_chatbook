...
class DocumentStructureParser:
    ...
    def clean_text(self, text: str) -> Tuple[str, List[Tuple[str, str]]]:
        ...
        def replace_command(match):
            ...
            replacement = self.ARTIFACT_PATTERNS["command_mapping"].get(
                base_command, match.group(0)
            )
    ...
    def parse_structure(self, text: str) -> List[Dict[str, Any]]:
        ...
        for i, elem in enumerate(elements):
            ...
            for j, other in enumerate(elements):
                if (
                    i != j
                    and other["start"] <= elem["start"]
                    and other["end"] >= elem["end"]
                ):
...
class EnhancedChunkingService(ChunkingService):
    ...
    def __init__(self):
        ...
        logger.info(
            "Initialized EnhancedChunkingService with structure parsing and table serialization"
        )
    ...
    def chunk_text_with_structure(
        ...
        table_metadata = []
        if serialize_tables:
            processed_text, table_metadata = (
                self.table_processor.process_document_tables(
                    content, serialize_method="hybrid"
                )
            )
        ...
        if preserve_structure and method in ["hierarchical", "structural"]:
            ...
            if method == "hierarchical":
                return self._hierarchical_chunking(
                    content, elements, chunk_size, chunk_overlap
                )
            elif method == "structural":
                return self._structural_chunking(
                    content, elements, chunk_size, chunk_overlap
                )
            ...
            if len(elem_content.split()) > chunk_size:
                ...
                for sub_chunk in sub_chunks:
                    ...
                    if parent_index is not None:
                        chunks[parent_index].children_indices.append(
                            sub_chunk.chunk_index
                        )
        ...
        for elem in elements:
            ...
            if elem["start"] > prev_end:
                ...
                if gap_text:
                    ...
                    if (
                        current_word_count + len(gap_words) > chunk_size
                        ...
                        if chunk_overlap > 0 and len(current_chunk_parts) > 0:
                            overlap_words = " ".join(current_chunk_parts).split()[
                                -chunk_overlap:
                            ]
            ...
            if elem["type"].startswith("header") or elem["type"] == "numbered_section":
                ...
                current_word_count + len(elem_words) > chunk_size * 1.5
    ...
    def chunk_with_parent_retrieval(
        ...
        for retrieval_chunk in retrieval_chunks:
            ...
            for parent_idx, parent_chunk in enumerate(parent_chunks):
                if (
                    parent_chunk.start_char <= retrieval_chunk.start_char
                    and parent_chunk.end_char >= retrieval_chunk.end_char
                ):
    ...
    def serialize_table(
        self, table_text: str, format: str = "markdown"
    ) -> Dict[str, Any]:
        """
        ...
        return {
            "markdown": table_text,
            "serialized": {"information_blocks": serialized_blocks},
        }
