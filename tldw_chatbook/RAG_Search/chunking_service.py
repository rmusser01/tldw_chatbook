...
class ChunkingService:
    ...
    def chunk_text(
        ...
        try:
            if method in {"words", "sentences", "paragraphs"}:
                return self._chunk_text_in_process(
            ...
            if method == "words":
                ...
                for i, word in enumerate(words):
                    ...
                    while (
                        ...
                        and content[current_pos : current_pos + 1].isspace()
                    ):
                        current_pos += 1
                    ...
                    if current_pos < len(content):
                        ...
                        if (
                            current_pos + len(word) <= len(content)
                            and content[word_start:word_end] == word
                        ):
                            ...
                            if word_start != -1:
                                word_positions.append(
                                    (word_start, word_start + len(word))
                                )
                ...
                for i, chunk_text in enumerate(chunks):
                    ...
                    if word_positions and current_word_idx < len(word_positions):
                        ...
                        end_word_idx = min(
                            current_word_idx + len(chunk_words) - 1,
                            len(word_positions) - 1,
                        )
                        ...
                        logger.warning(
                            f"Word position calculation fallback for chunk {i}"
                        )
                        if i == 0:
                            start_char = 0
                            ...
                            if result_chunks:
                                ...
                                end_char = min(
                                    start_char + len(chunk_text), len(content)
                                )
                            else:
                                start_char = 0
                                end_char = min(len(chunk_text), len(content))
            ...
            logger.debug(
                f"Chunked text into {len(result_chunks)} chunks using method '{method}'"
            )
    ...
    def _chunk_text_in_process(
        ...
        if method == "paragraphs":
            ...
            units = [
                match
                for match in re.finditer(r"\S.+?(?:[.!?](?=\s)|\Z)", content, re.DOTALL)
            ]
        else:
        ...
        logger.debug(
            f"Chunked text into {len(chunks)} chunks using in-process method '{method}'"
        )
        return chunks
...
def improved_chunking_process(
    ...
    if method not in valid_methods:
        raise InvalidChunkingMethodError(
            f"Invalid chunking method: {method}. Valid methods are: {valid_methods}"
        )
