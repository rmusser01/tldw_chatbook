...
class TableParser:
    ...
    def parse_markdown_table(text: str) -> Table:
        ...
        header_line = lines[0]
        headers = [
            cell.strip() for cell in re.split(r"\s*\|\s*", header_line) if cell.strip()
        ]
...
class TableSerializer:
    ...
    @staticmethod
    def serialize_to_sentences(
        table: Table, template: Optional[str] = None, include_summary: bool = True
    ) -> List[str]:
        """
    ...
    @staticmethod
    def serialize_to_key_value_pairs(
        table: Table, group_by_column: Optional[str] = None
        ...
        if group_by_column and group_by_column not in table.headers:
            raise ValueError(
                f"Group by column '{group_by_column}' not found in headers"
            )
        ...
        if group_by_column:
            ...
            for row in table.rows:
                ...
                for j, (header, value) in enumerate(zip(table.headers, row)):
                    if (
                        j != group_idx and value.strip()
                    ):  # Skip group column and empty values
...
class TableProcessor:
    ...
    def process_document_tables(
        self,
        text: str,
        serialize_method: Literal["entities", "sentences", "hybrid"] = "hybrid",
        ...
        for match in table_pattern.finditer(text):
            try:
                ...
                serialized = self.serializer.serialize_for_rag(
                    table, method=serialize_method, include_original=True
                )
                ...
                # Replace in text
                processed_text = (
                    processed_text[:start] + replacement_text + processed_text[end:]
                )
                ...
                logger.debug(
                    f"Processed table with {table.num_rows} rows and {table.num_columns} columns"
                )
...
def process_document_with_tables(
    document_text: str,
    serialize_method: Literal["entities", "sentences", "hybrid"] = "hybrid",
) -> Dict[str, Any]:
    """
    ...
    processed_text, table_metadata = processor.process_document_tables(
        document_text, serialize_method
    )
