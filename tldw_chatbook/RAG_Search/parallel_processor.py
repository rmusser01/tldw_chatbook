...
class BatchProcessor:
    ...
    def _determine_worker_count(self):
        ...
        self.num_workers = max(
            self.config.min_workers, min(self.num_workers, self.config.max_workers)
        )
    ...
    async def process_documents_parallel(
        ...
        total = len(documents)
        progress = (
            ProgressTracker(total, desc, self.config.progress_interval)
        ...
        logger.info(
            f"Processing {total} documents in {len(batches)} batches of ~{batch_size} items"
        )
        ...
        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            ...
            for batch_idx, batch in enumerate(batches):
                future = executor.submit(
                    self._process_batch_wrapper, process_func, batch, batch_idx
                )
    ...
    def _process_batch_wrapper(
        process_func: Callable, batch: List[Any], batch_idx: int
    ) -> List[Any]:
...
class EmbeddingBatchProcessor:
    ...
    async def generate_embeddings_batch(
        self, texts: List[str], embedding_service, batch_size: Optional[int] = None
        ...
        progress = (
            ProgressTracker(
                total, "Generating embeddings", self.config.progress_interval
            )
            if self.config.show_progress
            else None
        )
        ...
        for i in range(0, total, batch_size):
            ...
            task = self._process_embedding_batch(
                embedding_service, batch_texts, batch_start_idx, progress
            )
            tasks.append(task)
        ...
        if progress:
            ...
            logger.info(
                f"Embedding generation completed: {success_rate:.1%} success rate"
            )
    ...
    async def _process_embedding_batch(
    ...
    ) -> Tuple[List[Union["np.ndarray", List[float]]], List[int]]:
        ...
        try:
            ...
            batch_embeddings = await embedding_service.create_embeddings_async(
                batch_texts
            )
            ...
            logger.error(
                f"Failed to generate embeddings for batch starting at {start_idx}: {e}"
            )
            # Mark entire batch as failed
...
class ChunkingBatchProcessor:
    ...
    async def chunk_documents_batch(
        ...
        chunk_overlap: int,
        method: str = "words",
    ) -> Tuple[List[Dict], Dict[str, Any], List[IndexingResult]]:
        """
        ...
        def process_document(
            doc: Dict[str, Any],
        ) -> Tuple[str, List[Dict], Optional[IndexingResult]]:
            doc_id = doc.get("id", "unknown")

            try:
                # Chunk the document
                chunks = chunking_service.chunk_text(
                    doc["content"], chunk_size, chunk_overlap, method
                )
        ...
        results = await self.batch_processor.process_documents_parallel(
            documents, process_document, "Chunking documents"
        )
        ...
        logger.info(
            f"Chunking completed: {len(all_chunks)} total chunks from {len(documents)} documents"
        )
