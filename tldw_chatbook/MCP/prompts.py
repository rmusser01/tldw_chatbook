...
class MCPPrompts:
    ...
    async def summarize_conversation_prompt(
        self, conversation_id: int, style: str = "concise", focus: Optional[str] = None
    ) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if style == "concise":
                ...
                prompt += (
                    "summary in bullet point format, organizing information by topic."
                )
    ...
    async def analyze_media_prompt(
        ...
        try:
            ...
            if detail_level == "brief":
                ...
                prompt += (
                    " (provide a comprehensive analysis with examples and quotes)."
                )
            else:
                prompt += "."
    ...
    async def search_and_synthesize_prompt(
        ...
        try:
            # Perform RAG search
            from ..RAG_Search.simplified.search_service import (
            ...
            if not results:
                return [
                    {"role": "user", "content": f"No results found for query: {query}"}
                ]
