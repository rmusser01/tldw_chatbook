...
class TabStateManager:
    ...
    async def get_streaming_tabs(self) -> list[str]:
        ...
        async with self._states_lock:
            return [
                tab_id for tab_id, state in self._states.items() if state.is_streaming
            ]
    ...
    async def has_unsaved_changes(self) -> bool:
        ...
        """
        async with self._states_lock:
            return any(state.has_unsaved_changes for state in self._states.values())
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"TabStateManager(tabs={len(self._states)}, active={self._active_tab_id})"
        )
