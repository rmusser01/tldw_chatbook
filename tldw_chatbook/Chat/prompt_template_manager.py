...
def load_template(template_name: str) -> Optional[PromptTemplate]:
    ...
    if not template_file.exists():
        logger.warning(
            f"Prompt template '{template_name}' not found at {template_file}"
        )
        return None
    try:
        ...
        logger.opt(exception=True).error(
            "Error loading template {}: {}", template_name, error_message_str
        )
    return None
...
def apply_template_to_string(
    template_string: Optional[str], data: Dict[str, Any]
) -> Optional[str]:
    """
    ...
    try:
        ...
        logger.warning(
            f"Placeholder {e} not found in data for template string: '{template_string}'"
        )
        return template_string  # Fallback to original
    except Exception as e:
        logger.opt(exception=True).error(
            f"Error applying template string '{template_string}': {e}"
        )
