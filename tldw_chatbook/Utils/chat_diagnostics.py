...
class ChatDiagnostics:
    ...
    def inspect_widget_tree(
        self, root_widget: Widget, max_depth: int = 10
    ) -> Dict[str, Any]:
        """
    ...
    def _inspect_recursive(
        self, widget: Widget, depth: int, max_depth: int, parent_path: str = ""
    ) -> None:
        ...
        widget_info = {
            ...
            "children_count": len(widget.children)
        ...
        if isinstance(widget, TextArea):
            text_area_info = {
                **widget_info,
                ...
                "text_preview": (widget.text[:100] + "...")
                if hasattr(widget, "text") and widget.text
                else "",
                "is_disabled": widget.disabled
                if hasattr(widget, "disabled")
                else False,
                "is_visible": widget.styles.display != "none"
            ...
            if widget.id and (
                "input" in widget.id.lower() or "message" in widget.id.lower()
            ):
        ...
        if isinstance(widget, Container):
            container_info = {
                **widget_info,
                "might_be_chat_log": any(
                    keyword in str(widget.id).lower()
                    for keyword in ["chat", "log", "message", "history"]
                )
                if widget.id
                else False,
                "might_be_tab_container": any(
                    keyword in str(widget.id).lower() for keyword in ["tab", "session"]
                )
                if widget.id
                else False,
            }
    ...
    def _summarize_containers(self) -> Dict[str, Any]:
        ...
        chat_containers = [
            c for c in self.containers_found if c.get("might_be_chat_log")
        ]
        tab_containers = [
            c for c in self.containers_found if c.get("might_be_tab_container")
        ]
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if not self.input_widgets:
            recommendations.append(
                "No clear input widgets found - check for TextArea with different IDs"
            )
        elif len(self.input_widgets) > 1:
            recommendations.append(
                f"Multiple input widgets found ({len(self.input_widgets)}) - determine which is primary"
            )
        ...
        if has_tabs:
            ...
            recommendations.append(
                "Non-tabbed interface - save state directly from widgets"
            )
        ...
        if not has_chat_log:
            recommendations.append(
                "No clear chat log container found - may need alternative message extraction"
            )
        ...
        if self.text_areas_found:
            ...
            if visible_areas:
                recommendations.append(
                    f"Focus on {len(visible_areas)} visible TextArea widgets for state capture"
                )
    ...
    def print_report(self, report: Dict[str, Any]) -> None:
        ...
        print(
            f"Root Widget: {report['root_widget']['class']} (id={report['root_widget']['id']})"
        )
    ...
    def run_diagnostic(chat_window: Widget) -> Dict[str, Any]:
        ...
        # Log summary
        logger.info(
            f"Diagnostic complete: {report['chat_structure']['type']} structure with {report['text_areas']['count']} TextAreas"
        )
