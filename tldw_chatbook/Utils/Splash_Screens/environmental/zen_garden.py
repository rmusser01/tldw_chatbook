...
class ZenGardenEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Garden elements
        self.stones = [(20, 10, "◯"), (50, 8, "○"), (35, 15, "●"), (60, 12, "◉")]
    ...
    def update(self) -> Optional[str]:
        ...
        for y in range(sand_area["y_start"], sand_area["y_end"]):
            for x in range(sand_area["x_start"], sand_area["x_end"]):
                ...
                reveal_progress = (x - sand_area["x_start"]) / (
                    sand_area["x_end"] - sand_area["x_start"]
                )
        ...
        for dy in range(-pond_radius, pond_radius + 1):
            for dx in range(-pond_radius, pond_radius + 1):
                ...
                if (
                    0 <= x < self.width
                    and 0 <= y < self.height
                    and dx * dx + dy * dy <= pond_radius * pond_radius
                ):
        ...
        if self.ripple_radius > 0:
            for angle in range(0, 360, 30):
                ...
                ry = pond_y + int(
                    self.ripple_radius * math.sin(math.radians(angle)) * 0.5
                )
                if (
                    0 <= rx < self.width
                    and 0 <= ry < self.height
                    and grid[ry][rx] == "≈"
                ):
                    grid[ry][rx] = "~"
