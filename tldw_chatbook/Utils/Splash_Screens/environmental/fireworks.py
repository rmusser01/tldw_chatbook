            ...
            if fw.age < fw.max_age * 2:  # Keep for a bit after explosion
                ...
                if fw.exploded:
                    ...
                    for px, py, pvx, pvy in fw.particles:
                        ...
                        py += pvy * delta_time
                        pvy += (
                            self.gravity * delta_time * 0.5
                        )  # Less gravity for particles

                        if (
                            0 <= px < self.display_width
                            and 0 <= py < self.display_height
                        ):
                ...
                for px, py, _, _ in fw.particles:
                    ...
                    if 0 <= x < self.display_width and 0 <= y < self.display_height:
                        ...
                        char_index = int(
                            (fw.age - fw.max_age)
                            / fw.max_age
                            * len(self.explosion_chars)
                        )
                        char_index = max(
                            0, min(char_index, len(self.explosion_chars) - 1)
                        )
            ...
            for i, char in enumerate(self.title):
                if i in self.revealed_chars:
                    ...
                    if (
                        ...
                        and 0 <= title_y < self.display_height
                    ):
                        grid[title_y][x] = char
