...
class ConstellationMapEffect(BaseEffect):
    ...
    def __init__(
        self, parent, title="TLDW Chatbook", width=80, height=24, speed=0.1, **kwargs
    ):
    ...
    def _create_constellation(self):
        ...
        for i in range(3):  # Top horizontal line
            self.connections.append(
                (t_stars[i], t_stars[i + 1] if i < 2 else t_stars[1])
            )
    ...
    def update(self) -> Optional[str]:
        ...
        for star in self.stars:
            if not star["constellation"]:
                star["twinkle"] = (
                    star["twinkle"] + elapsed_time * random.uniform(1, 3)
                ) % 1.0

        # Reveal constellation gradually
        self.reveal_progress += elapsed_time * 0.5
        revealed_count = int(
            self.reveal_progress * len([s for s in self.stars if s["constellation"]])
        )
    ...
    def render(self):
            ...
            for (x1, y1), (x2, y2) in self.connections:
                star1 = next(
                    (
                        s
                        for s in self.stars
                        if s["x"] == x1 and s["y"] == y1 and s.get("revealed")
                    ),
                    None,
                )
                star2 = next(
                    (
                        s
                        for s in self.stars
                        if s["x"] == x2 and s["y"] == y2 and s.get("revealed")
                    ),
                    None,
                )
                if star1 and star2:
        ...
        if self.reveal_progress > 0.7:
            self._add_centered_text(
                grid, style_grid, self.title, self.height - 3, "bold white"
            )
