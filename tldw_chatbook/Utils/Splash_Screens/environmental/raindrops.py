...
class RaindropsEffect(BaseEffect):
    ...
    def __init__(
        ...
        ripple_chars: List[str] = list(
            "·oO()"
        ),  # Smallest to largest, then fades maybe
    ...
    def _spawn_ripple(self):
        if len(self.ripples) < self.max_concurrent_ripples:
            ...
            self.ripples.append(
                RaindropsEffect.Ripple(
                    cx=cx, cy=cy, max_radius=max_r, speed=speed, life=life, style=style
                )
            )
    ...
    def update(self) -> Optional[str]:
        ...
        for ripple in self.ripples:
            ...
            if ripple.alive_time < ripple.life:
                ...
                char_progress = (ripple.radius / ripple.max_radius) * (
                    len(self.ripple_chars) - 1
                )
                ripple.current_char_index = min(
                    len(self.ripple_chars) - 1, int(char_progress)
                )
        ...
        char_grid = [
            [
                (self.base_water_char, self.water_style)
                for _ in range(self.display_width)
            ]
            for _ in range(self.display_height)
        ]

        for ripple in sorted(
            self.ripples, key=lambda r: r.radius, reverse=True
        ):  # Draw larger (older) ripples first
            ...
            for angle_deg in range(0, 360, 10):  # Draw points on the circle
                ...
                y = int(
                    ripple.cy + ripple.radius * math.sin(angle_rad) * 0.6
                )  # Y correction for char aspect
