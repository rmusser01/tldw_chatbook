    ...
    def update(self) -> Optional[str]:
            ...
            if self.formation_mode and particle.target_index is not None:
                ...
                if neighbors:
                    ...
                    for neighbor in neighbors:
                        dist = math.sqrt(
                            (neighbor.x - particle.x) ** 2
                            + (neighbor.y - particle.y) ** 2
                        )
                        if dist < 5 and dist > 0:
                            particle.vx -= (
                                (neighbor.x - particle.x) / dist * self.separation
                            )
                            particle.vy -= (
                                (neighbor.y - particle.y) / dist * self.separation
                            )
        ...
        for particle in self.particles:
            ...
            if 0 <= x < self.display_width and 0 <= y < self.display_height:
                if (
                    self.formation_mode
                    and particle.target_index is not None
                    and particle.target_index < len(self.title)
                ):
