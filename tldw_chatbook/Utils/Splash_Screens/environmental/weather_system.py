        ...
        if current_weather == "sunny":
            ...
            if current_weather == "rainy":
                ...
                for particle in self.snow_particles:
                    ...
                    x = int(particle["x"])
                    if 0 <= x < self.width and 0 <= y < self.height:
                        snow_char = self.snow_chars[
                            random.randint(0, len(self.snow_chars) - 1)
                        ]
                        grid[y][x] = snow_char
            ...
            if weather_progress < 0.5:
                # Still some clouds
                cloud_x = int(
                    (self.width - len(self.cloud[0])) // 2 + weather_progress * 20
                )
                cloud_y = 7
                for i, line in enumerate(self.cloud):
                    for j, char in enumerate(line):
                        if (
                            0 <= cloud_x + j < self.width
                            and 0 <= cloud_y + i < self.height
                        ):
            ...
            # Sun appearing
            sun_x = int(
                (self.width - len(self.sun[0])) // 2 - (1 - weather_progress) * 20
            )
            sun_y = 8
            for i, line in enumerate(self.sun):
                for j, char in enumerate(line):
