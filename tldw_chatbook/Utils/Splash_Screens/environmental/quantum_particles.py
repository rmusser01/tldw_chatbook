...
class QuantumParticlesEffect(BaseEffect):
    ...
    def update(self) -> Optional[str]:
        ...
        for i, particle in enumerate(self.particles):
            ...
            if 0 <= x < self.width and 0 <= y < self.height:
                self.interference_pattern[y][x] = (
                    self.interference_pattern[y][x] + 0.1
                ) % 1.0
    ...
    def render(self):
        ...
        self._add_centered_text(
            grid, style_grid, self.title, self.height // 2 - 1, "bold white"
        )
        if self.subtitle:
            self._add_centered_text(
                grid, style_grid, self.subtitle, self.height // 2 + 1, "white"
            )
