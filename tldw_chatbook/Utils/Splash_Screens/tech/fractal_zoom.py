...
class FractalZoomEffect(BaseEffect):
    ...
    def __init__(
        self, parent, title="TLDW Chatbook", width=80, height=24, speed=0.05, **kwargs
    ):
    ...
    def render(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                imag = (y - self.height / 2) / (
                    self.zoom * self.height / 4
                ) + self.center_y
        ...
        if self.zoom > 3:
            self._add_centered_text(
                grid, style_grid, self.title, self.height // 2, "bold white on black"
            )
