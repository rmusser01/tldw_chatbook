            ...
            self.typewriters.append(
                {
                    ...
                    "text": title
                    if i < 3
                    else subtitle,  # Some type title, some subtitle
                }
            )
        ...
        for tw in self.typewriters:
            ...
            if elapsed < self.convergence_start:
                ...
                convergence_progress = min(
                    1.0, (elapsed - self.convergence_start) / self.convergence_duration
                )
            ...
            for i, char in enumerate(text_to_show):
                if 0 <= x + i < self.width and 0 <= y < self.height:
                    ...
                    if elapsed > self.convergence_start:
                        convergence_progress = min(
                            1.0,
                            (elapsed - self.convergence_start)
                            / self.convergence_duration,
                        )
                        if (
                            tw["text"] == self.title
                            and abs(y - self.final_title_y) < 1
                            and abs(x - self.final_title_x) < 1
                        ) or (
                            tw["text"] == self.subtitle
                            and abs(y - self.final_subtitle_y) < 1
                            and abs(x - self.final_subtitle_x) < 1
                        ):
                            # This is the final position
                            grid[y][x + i] = char
                            style_grid[y][x + i] = (
                                "bold white" if tw["text"] == self.title else "cyan"
                            )
                        else:
