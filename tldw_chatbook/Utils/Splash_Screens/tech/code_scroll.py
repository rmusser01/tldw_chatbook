...
class CodeScrollEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_code_lines = min(
            num_code_lines, height - 4
    ...
    def update(self) -> Optional[str]:
        ...
        title_row = max(
            ...
            min(
                title_row_ideal,
                code_block_start_row - 2
                if self.num_code_lines > 0
                else title_row_ideal,
            ),
        )
        ...
        if (
            subtitle_row >= code_block_start_row and self.num_code_lines > 0
        ):  # Adjust if subtitle overlaps code block
        ...
        for r_idx in range(self.height):
            ...
            if r_idx == title_row:
                ...
                current_line_content = (
                    f"[{self.title_style}]{escaped_content}[/{self.title_style}]"
                )
                ...
                current_line_content = (
                    f"[{self.subtitle_style}]{escaped_content}[/{self.subtitle_style}]"
                )
