    ...
    def update(self) -> Optional[str]:
        ...
        if self.current_command_index < len(self.commands):
            ...
            if self.typing_progress < len(cmd):
                ...
                if self.current_command and self.current_command not in [
                    line["text"] for line in self.terminal_lines
                ]:
                    self.terminal_lines.append(
                        {"text": self.current_command, "style": "green"}
                    )

                    # Add response
                    _, response = self.commands[self.current_command_index]
                    self.terminal_lines.append({"text": response, "style": "dim white"})

                    # Progress bar
                    progress = int(
                        (self.current_command_index + 1) / len(self.commands) * 20
                    )
                    self.terminal_lines.append(
                        {
                            "text": "["
            ...
            if self.phase != "granted":
                self.phase = "granted"
                self.terminal_lines.append({"text": "", "style": None})
                self.terminal_lines.append(
                    {"text": "ACCESS GRANTED", "style": "bold green"}
                )
                self.terminal_lines.append(
                    {"text": f"Welcome to {self.title}", "style": "bold white"}
                )
