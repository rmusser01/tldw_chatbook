...
class TypewriterNewsEffect(BaseEffect):
    ...
    def render(self):
        ...
        # Type subheadline
        if chars_typed > 0:
            subheadline_typed = self.subheadline[
                : min(chars_typed, len(self.subheadline))
            ]
            self._add_centered_text(
                grid,
                style_grid,
        ...
        if chars_typed > 0:
            ...
            self._add_text_at(
                ...
                8,
                current_line,
                "italic black on rgb(245,245,220)",
            )
        ...
        if self.cursor_blink > 0.5 and chars_typed >= 0:
            cursor_pos = min(
                int(self.typed_chars),
                sum(
                    len(line)
                    for line in [self.headline, self.subheadline, self.dateline]
                    + self.article
                ),
            )
            ...
            if current_line < self.height - 4:
                ...
                style_grid[current_line][min(8 + (cursor_pos % 50), self.width - 6)] = (
                    "black on rgb(245,245,220)"
                )
