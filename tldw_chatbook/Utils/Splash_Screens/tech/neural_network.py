...
class NeuralNetworkEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for layer_idx, node_count in enumerate(layers):
            ...
            if layer_idx > 0:
                prev_layer_nodes = [
                    n for n in self.nodes if n["layer"] == layer_idx - 1
                ]
    ...
    def update(self) -> Optional[str]:
        ...
        self.activation_wave = (self.activation_wave + elapsed_time * 2) % (
            len(set(n["layer"] for n in self.nodes)) + 2
        )
    ...
    def render(self):
        ...
        for conn in self.connections:
            if conn["from"]["activation"] > 0.1 or conn["to"]["activation"] > 0.1:
                strength = (
                    (conn["from"]["activation"] + conn["to"]["activation"])
                    / 2
                    * conn["strength"]
                )
                self._draw_line(
