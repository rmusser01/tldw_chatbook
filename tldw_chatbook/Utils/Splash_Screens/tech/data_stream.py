...
class DataStreamEffect(BaseEffect):
    ...
    def __init__(
        self, parent, title="TLDW Chatbook", width=80, height=24, speed=0.02, **kwargs
    ):
    ...
    def update(self) -> Optional[str]:
        ...
        if self.decode_progress > 0.3 and len(self.highlight_positions) < len(
    ...
    def render(self):
        ...
        if len(self.highlight_positions) >= len(self.decoded_message):
            ...
            self._add_centered_text(
                grid, style_grid, self.decoded_message, msg_y - 1, "bold white on green"
            )
            self._add_centered_text(
                grid, style_grid, self.title, msg_y + 1, "bold white"
            )
