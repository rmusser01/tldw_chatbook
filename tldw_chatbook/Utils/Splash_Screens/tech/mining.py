            ...
            for line_idx, line in enumerate(self.content_lines):
                ...
                if abs(line_idx - center_y) <= distance:
                    for char_idx, char in enumerate(line):
                        ...
                        if (
                            char_idx < self.width
                            and not self.reveal_grid[line_idx][char_idx]
                            and char != " "
                        ):
                            self.reveal_grid[line_idx][char_idx] = True
                            # Set mining stage (gradually dig through stone)
                            self.mining_grid[line_idx][char_idx] = min(
                                4, int(self.mining_progress * 8)
                            )
                            revealed_count += 1
                ...
                self.sound_positions.append(
                    {
                        ...
                        "life": 1.0,
                        "style": random.choice(
                            ["bold red", "bold orange", "bold yellow"]
                        ),
                    }
                )
