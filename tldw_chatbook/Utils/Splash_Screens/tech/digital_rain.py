...
class DigitalRainEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.height = height
        self.speed = (
            speed  # Not directly used in update logic timing, but for animation timer
        )
        ...
        self.column_speeds: List[
            float
        ] = []  # How many frames until this column updates
        ...
        for _ in range(self.width):
            ...
            for _ in range(random.randint(self.height // 4, self.height // 2)):
                ...
                style = (
                    ...
                    if char in self.highlight_chars
                    or random.random() < self.highlight_chance
    ...
    def update(self) -> Optional[str]:
        ...
        for col_idx in range(self.width):
            if self.frame_count >= self.column_next_update[col_idx]:
                ...
                new_style = (
                    self.highlight_color
                    if new_char in self.highlight_chars
                    or random.random() < self.highlight_chance
                    else self.base_color
                )
                self.columns[col_idx].insert(0, (new_char, new_style))

                self.column_next_update[col_idx] = (
                    self.frame_count + self.column_speeds[col_idx]
                )

        # Prepare render grid (list of lists of styled characters)
        render_grid: List[
            List[str]
        ] = []  # Each string is already Rich-escaped and styled
        ...
        if elapsed > 0.5:  # Start revealing title
            self.title_reveal_progress = min(
                1.0, (elapsed - 0.5) / 1.5
            )  # Slower reveal
                ...
                for i in range(len(self.title)):
                    if title_start_col + i < self.width:
                        if i < title_len_to_show:
                            ...
                            render_grid[title_row][title_start_col + i] = (
        ...
        if elapsed > 1.0:  # Start revealing subtitle
            ...
            subtitle_len_to_show = int(
                len(self.subtitle) * self.subtitle_reveal_progress
            )
                ...
                for i in range(len(self.subtitle)):
                    if subtitle_start_col + i < self.width:
                        if i < subtitle_len_to_show:
                            ...
                            render_grid[subtitle_row][subtitle_start_col + i] = (
                                f"[{self.subtitle_style}]{char_to_draw}[/{self.subtitle_style}]"
                            )
                        else:
                            render_grid[subtitle_row][subtitle_start_col + i] = (
                                "[on black] [/on black]"
