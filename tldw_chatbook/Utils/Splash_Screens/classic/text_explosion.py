    ...
    def _prepare_chars(self):
        ...
        for i, char_val in enumerate(self.target_text):
            ...
            if self.effect_type == "explode":
                ...
                speed = (
                    random.uniform(
                        self.particle_spread * 0.5, self.particle_spread * 1.5
                    )
                    / self.duration
                )
                vx, vy = (
                ...
                vx, vy = (
                    0,
                    0,
                )  # Not used for velocity in implode, calculated by interpolation

            self.chars.append(
                TextExplosionEffect.AnimatedChar(
                    char=char_val,
                    origin_x=ox,
                    origin_y=oy,
                    current_x=cx,
    ...
    def update(self) -> Optional[str]:
        ...
        for achar in self.chars:
            if self.effect_type == "explode":
                ...
                if (
                    progress > 0.05
                ):  # Small delay before explosion starts or to ensure movement
                    achar.current_x += (
                        achar.vx * progress * 20
                    )  # Scale velocity effect by progress and arbitrary factor
                ...
                if not hasattr(
                    achar, "initial_x"
                ):  # Store initial random pos if not already
                ...
                achar.current_x = (
                    achar.initial_x + (achar.origin_x - achar.initial_x) * progress
                )
                achar.current_y = (
                    achar.initial_y + (achar.origin_y - achar.initial_y) * progress
                )
