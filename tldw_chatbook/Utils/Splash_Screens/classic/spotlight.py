...
class SpotlightEffect(BaseEffect):
    ...
    def update(self) -> Optional[str]:
        ...
        if self.path_type == "lissajous":
            ...
            self.spotlight_x = (self.content_width / 2) + (
                self.content_width / 2 - self.spotlight_radius
            ) * math.sin(self.time_elapsed_for_path * 0.15)
            self.spotlight_y = (self.content_height / 2) + (
                self.content_height / 2 - self.spotlight_radius
            ) * math.cos(self.time_elapsed_for_path * 0.1)
        elif self.path_type == "circle":
            radius = (
                min(self.content_width, self.content_height) / 2 - self.spotlight_radius
            )
            self.spotlight_x = (self.content_width / 2) + radius * math.cos(
                self.time_elapsed_for_path * self.movement_speed * 0.02
            )  # speed factor
            self.spotlight_y = (self.content_height / 2) + radius * math.sin(
                self.time_elapsed_for_path * self.movement_speed * 0.02
            )
            ...
            if not (0 <= self.spotlight_y < self.content_height):
                self.vy *= -1
                self.spotlight_y = max(
                    0, min(self.spotlight_y, self.content_height - 1)
                )
            # Occasionally change direction randomly
            if random.random() < 0.01:  # 1% chance per frame
                self.vx = (
                    random.uniform(-self.movement_speed, self.movement_speed) * 0.1
                )  # Slower random walk
                self.vy = (
                    random.uniform(-self.movement_speed, self.movement_speed) * 0.1
                )
            ...
            if content_start_row <= r_disp < content_start_row + render_height:
                ...
                for c_disp in range(self.display_width):
                    if (
                        content_start_col
                        <= c_disp
                        < content_start_col + self.content_width
                    ):
                        ...
                        dist_sq = (c_content - self.spotlight_x) ** 2 + (
                            (r_content - self.spotlight_y) * 2
                        ) ** 2  # Y weighted

                        if dist_sq <= self.spotlight_radius_sq:
                            styled_line_segments.append(
                                f"[{self.visible_style}]{escaped_char}[/{self.visible_style}]"
                            )
                        else:
                            # Optional: fade effect at edges of spotlight
                            # For now, simple binary visible/hidden
                            styled_line_segments.append(
                                f"[{self.hidden_style}]{escaped_char}[/{self.hidden_style}]"
                            )
                    else:  # Outside content width, but within display width (padding)
                        styled_line_segments.append(
                            f"[{self.hidden_style}] [/{self.hidden_style}]"
                        )
                output_lines.append("".join(styled_line_segments))
            else:  # Outside content height (padding)
                output_lines.append(
                    f"[{self.hidden_style}]{' ' * self.display_width}[/{self.hidden_style}]"
                )
