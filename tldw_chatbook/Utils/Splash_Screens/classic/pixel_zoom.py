...
class PixelZoomEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.content_width = (
            max(len(line) for line in self.target_lines) if self.target_lines else 0
        )
        ...
        if self.content_height > 0 and self.content_width > 0:
            for i in range(self.content_height):
                ...
                self.padded_target_lines.append(
                    line + " " * (self.content_width - len(line))
                )
    ...
    def _get_block_char(self, r_start: int, c_start: int, pixel_size: int) -> str:
        ...
        if not self.padded_target_lines:
            return " "
        ...
        for r_offset in range(pixel_size):
            for c_offset in range(pixel_size):
                ...
                if 0 <= r < self.content_height and 0 <= c < self.content_width:
                    ...
                    if (
                        char != " "
                    ):  # Ignore spaces for dominant char, or include if you want space to dominate
    ...
    def update(self) -> Optional[str]:
        ...
        if (
            current_pixel_size == self.max_pixel_size
            and self.effect_type == "pixelate"
            and progress >= 1.0
        ):
        ...
        output_art_chars = [
            [" " for _ in range(self.content_width)] for _ in range(self.content_height)
        ]
        ...
        for r_block_start in range(0, self.content_height, current_pixel_size):
            for c_block_start in range(0, self.content_width, current_pixel_size):
                block_char = self._get_block_char(
                    r_block_start, c_block_start, current_pixel_size
                )
