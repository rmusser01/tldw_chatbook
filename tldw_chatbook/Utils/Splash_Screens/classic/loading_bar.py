...
class LoadingBarEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bar_interior_width = (
            len(self.bar_frame_content) - 2
        )  # Assuming frame is like [---]
    ...
    def update(self) -> Optional[str]:
        ...
        current_progress = (
            self.parent.progress if hasattr(self.parent, "progress") else 0.0
        )
        ...
        empty_part = self.bar_frame_content[
            1 + num_filled : 1 + num_filled + num_empty
        ]  # Get actual empty chars from frame
        ...
        if len(current_bar_interior) < self.bar_interior_width:
            current_bar_interior += self.bar_frame_content[
                1 + len(current_bar_interior)
            ] * (self.bar_interior_width - len(current_bar_interior))
        ...
        progress_percentage_text = (
            self.parent.progress_text
            if hasattr(self.parent, "progress_text") and self.parent.progress_text
            else ""
        )

        # Use the template if available, otherwise use the SplashScreen's progress_text
        if "{progress}" in self.text_below_template:
            text_below_formatted = self.text_below_template.format(
                progress=current_progress * 100
            )
        else:
            text_below_formatted = (
                progress_percentage_text
                if progress_percentage_text
                else f"{current_progress * 100:.0f}%"
            )
        ...
        if self.text_above:
            ...
            output_lines.append(
                f"[{self.text_style}]{' ' * pad_above}{escaped_text}{' ' * pad_above}[/{self.text_style}]"
            )
        else:
        ...
        if text_below_formatted:
            ...
            output_lines.append(
                f"[{self.text_style}]{' ' * pad_below}{escaped_text_below}{' ' * pad_below}[/{self.text_style}]"
            )
        else:
            output_lines.append("")
        ...
        return "\n".join(
            output_lines[: self.parent.height if hasattr(self.parent, "height") else 7]
        )
