...
class ScrollingCreditsEffect(BaseEffect):
    ...
    def __init__(
        ...
        credits_list: List[
            Dict[str, str]
        ] = None,  # Each dict: {"role": "Concept", "name": "The Universe"} or just {"line": "Some text"}
        ...
        **kwargs,
        ...
        self.overall_title = (
            title  # Title for the splash screen itself, displayed statically
        )
        self.credits_list = credits_list if credits_list else [{"line": "Loading..."}]
        self.scroll_speed = (
            scroll_speed  # This will be used to calculate fractional line shifts
        )
        ...
        self.current_scroll_offset = float(
            self.display_height
        )  # Start with credits off-screen at the bottom
    ...
    def _format_credits(self):
        ...
        self.formatted_credit_lines.append(
            f"[{self.title_style}]{self.overall_title.center(self.display_width)}[/{self.title_style}]"
        )
        ...
        for item in self.credits_list:
            if "line" in item:
                ...
                self.formatted_credit_lines.append(
                    f"[{self.line_style}]{text.center(self.display_width)}[/{self.line_style}]"
                )
                ...
                self.formatted_credit_lines.append(
                    f"[{self.role_style}]{role_text.center(self.display_width)}[/{self.role_style}]"
                )
                self.formatted_credit_lines.append(
                    f"[{self.name_style}]{name_text.center(self.display_width)}[/{self.name_style}]"
                )
    ...
    def update(self) -> Optional[str]:
        ...
        if self.current_scroll_offset < -len(self.formatted_credit_lines):
            self.current_scroll_offset = float(
                self.display_height
            )  # Reset to start from bottom again
