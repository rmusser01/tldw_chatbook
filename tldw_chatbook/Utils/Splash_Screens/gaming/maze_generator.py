...
class MazeGeneratorEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.maze_grid = [
            [0 for _ in range(self.maze_cols)] for _ in range(self.maze_rows)
        ]
        ...
        self.current_cx = random.randrange(
            0, self.maze_cols, 2
        )  # Ensure starting on a "cell" column if we consider walls
        self.current_cy = random.randrange(
            0, self.maze_rows, 2
        )  # Ensure starting on a "cell" row
        ...
        self.logic_grid = [
            [0 for _ in range(self.logic_cols)] for _ in range(self.logic_rows)
        ]
    ...
    def _generation_step_dfs(self):
        ...
        neighbors = []
        # Check North
        if y > 0 and self.logic_grid[y - 1][x] & self.CELL_VISITED == 0:
            neighbors.append(("N", x, y - 1))
        # Check East
        if (
            x < self.logic_cols - 1
            and self.logic_grid[y][x + 1] & self.CELL_VISITED == 0
        ):
            neighbors.append(("E", x + 1, y))
        # Check South
        if (
            y < self.logic_rows - 1
            and self.logic_grid[y + 1][x] & self.CELL_VISITED == 0
        ):
            neighbors.append(("S", x, y + 1))
        # Check West
        if x > 0 and self.logic_grid[y][x - 1] & self.CELL_VISITED == 0:
    ...
    def update(self) -> Optional[str]:
        ...
        if self.is_generating and (
            current_time - self._last_gen_step_time >= self.generation_speed
        ):
        ...
        for r_draw in range(self.maze_rows):
            for c_draw in range(self.maze_cols):
                ...
                if not is_wall_row and not is_wall_col:  # Cell center
                    ...
                    if (
                        self.is_generating
                        and logic_x == self.current_logic_x
                        and logic_y == self.current_logic_y
                    ):
                        char_to_draw = self.cursor_char
                        style_to_use = self.cursor_style

                elif (
                    not is_wall_row and is_wall_col
                ):  # Horizontal wall/path between cells (y, x) and (y, x+1)
                    if logic_x < self.logic_cols - 1 and (
                        self.logic_grid[logic_y][logic_x] & self.CELL_PATH_E
                        or self.logic_grid[logic_y][logic_x + 1] & self.CELL_PATH_W
                    ):
                        char_to_draw = self.path_char
                        style_to_use = self.path_style

                elif (
                    is_wall_row and not is_wall_col
                ):  # Vertical wall/path between cells (y,x) and (y+1,x)
                    if logic_y < self.logic_rows - 1 and (
                        self.logic_grid[logic_y][logic_x] & self.CELL_PATH_S
                        or self.logic_grid[logic_y + 1][logic_x] & self.CELL_PATH_N
                    ):
                        char_to_draw = self.path_char
                        style_to_use = self.path_style
        ...
        if self.title:
            title_y = (
                maze_start_row - 2 if maze_start_row > 1 else self.display_height - 1
            )
            if not self.is_generating:
                title_y = self.display_height // 2  # Center title when done
            ...
            if 0 <= title_y < self.display_height:
                ...
                title_line_str = self.title.center(self.display_width).replace(
                    "[", r"\["
                )
                styled_output_lines[title_y] = (
                    f"[{self.title_style}]{title_line_str}[/{self.title_style}]"
                )
