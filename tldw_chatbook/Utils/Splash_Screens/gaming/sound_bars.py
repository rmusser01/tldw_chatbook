...
class SoundBarsEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_bar_height = (
            ...
            else self.display_height - self.title_area_height - 1
        )  # -1 for base line
        if self.max_bar_height <= 0:
            self.max_bar_height = 1
        ...
        self.bar_heights = [
            random.randint(1, self.max_bar_height) for _ in range(self.num_bars)
        ]
        self.bar_targets = list(
            self.bar_heights
        )  # Target heights for smooth transition
    ...
    def _update_bar_heights(self):
        ...
        for i in range(self.num_bars):
            ...
            if self.bar_heights[i] < self.bar_targets[i]:
                self.bar_heights[i] = min(
                    self.bar_targets[i], self.bar_heights[i] + 1
                )  # Step of 1 for simplicity
    ...
    def update(self) -> Optional[str]:
        ...
        if self.title:
            ...
            for c, char_val in enumerate(self.title):
                if title_x_start + c < self.display_width:
                    output_lines[title_start_row][title_x_start + c] = (
                        char_val.replace("[", r"\["),
                        self.title_style,
                    )
        ...
        spacing = (bar_display_area_width - total_bar_chars_width) // (
            self.num_bars + 1
        )
        if spacing < 0:
            spacing = 0  # Bars might overlap if too many

        current_c = spacing  # Start position for first bar

        for i in range(self.num_bars):
            if current_c >= self.display_width:
                break  # No more space for bars
            ...
            for r in range(self.max_bar_height):
                ...
                if row_idx_on_display < title_start_row:
                    continue  # Don't draw into title area from bottom
                if row_idx_on_display >= self.display_height:
                    continue

                if r < bar_h:  # This part of bar is filled
                    output_lines[row_idx_on_display][current_c] = (
                        self.bar_char_filled,
                        bar_style_to_use,
                    )
