...
class GameOfLifeEffect(BaseEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.grid = [
            [
                (random.choice([0, 1]) if initial_pattern == "random" else 0)
        ...
        for r_grid in range(self.grid_height):
            for c_grid in range(self.grid_width):
                ...
                if (
                    ...
                    char_to_draw = (
                        self.cell_alive_char if is_alive else self.cell_dead_char
                    )
            ...
            for c_idx in range(self.display_width):
                ...
                if isinstance(cell_content, tuple):
                    ...
                    line_segments.append(
                        f"[{self.dead_style}] [/{self.dead_style}]"
                    )  # Styled background space
            styled_output_lines[r_idx] = "".join(line_segments)

        # Overlay title (centered, typically above or below GoL grid)
        if self.title:
            title_y = (
                grid_start_r - 2 if grid_start_r > 1 else self.display_height - 1
            )  # Place above or at bottom
                ...
                for c_idx in range(self.display_width):
                    ...
                    if is_title_char_pos:
                        ...
                        title_line_segments.append(
                            f"[{self.title_style}]{char}[/{self.title_style}]"
                        )
                        title_char_idx += 1
