...
class KaleidoscopeEffect(BaseEffect):
    ...
    def update(self) -> Optional[str]:
        ...
        for p in self.particles:
            p["x"] += p["vx"] * 0.1
            p["y"] += p["vy"] * 0.1
            if p["x"] < 0 or p["x"] > self.display_width:
                p["vx"] *= -1
            if p["y"] < 0 or p["y"] > self.display_height:
                p["vy"] *= -1
            ...
            for i in range(self.num_segments):
                ...
                draw_x, draw_y = (
                    int(self.center_x + rotated_x),
                    int(self.center_y + rotated_y * 0.5),
                )
                if (
                    0 <= draw_x < self.display_width
                    and 0 <= draw_y < self.display_height
                ):
                ...
                draw_x_m, draw_y_m = (
                    int(self.center_x + rotated_x_m),
                    int(self.center_y + rotated_y_m * 0.5),
                )
                if (
                    0 <= draw_x_m < self.display_width
                    and 0 <= draw_y_m < self.display_height
                ):
                    grid[draw_y_m][draw_x_m] = p["char"]
                    styles[draw_y_m][draw_x_m] = p["color"]

        self._add_centered_text(
            grid, styles, self.title, self.display_height // 2, "bold white on black"
        )
