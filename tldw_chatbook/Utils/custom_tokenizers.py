...
class CustomTokenizerManager:
    ...
    def __init__(self, tokenizers_dir: Optional[str] = None):
        ...
        """
        self.tokenizers_dir = tokenizers_dir or os.path.expanduser(
            "~/.config/tldw_cli/tokenizers"
        )
    ...
    def _load_mappings(self) -> None:
        ...
        if os.path.exists(mappings_file):
            try:
                ...
                log_counter(
                    "custom_tokenizers_load_mappings_success",
                    labels={"count": str(len(self._model_mappings))},
                )
            except Exception as e:
                logger.error(f"Failed to load tokenizer mappings: {e}")
                log_counter(
                    "custom_tokenizers_load_mappings_error",
    ...
    def load_tokenizer(self, name: str) -> Optional[Any]:
        ...
        if name in self._tokenizers:
            log_counter(
                "custom_tokenizers_load_tokenizer_cache_hit", labels={"name": name}
            )
        ...
        if os.path.exists(tokenizer_path):
            try:
                ...
                log_counter(
                    "custom_tokenizers_load_tokenizer_success", labels={"name": name}
                )
                ...
                log_counter(
                    "custom_tokenizers_load_tokenizer_error",
                    labels={"name": name, "error_type": type(e).__name__},
                )
        else:
            log_counter(
                "custom_tokenizers_load_tokenizer_not_found", labels={"name": name}
            )
            ...
            for pattern in provider_patterns[provider]:
                if pattern in model.lower():
                    ...
                    for tokenizer_name in os.listdir(self.tokenizers_dir):
                        if (
                            tokenizer_name.endswith(".json")
                            and pattern in tokenizer_name.lower()
                        ):
    ...
    def count_tokens(self, text: str, model: str, provider: str) -> Optional[int]:
        ...
        start_time = time.time()
        log_counter(
            "custom_tokenizers_count_tokens_attempt",
        ...
        if tokenizer:
            try:
                ...
                log_counter(
                    "custom_tokenizers_count_tokens_success",
                    labels={"model": model, "provider": provider},
                )
                ...
                log_counter(
                    "custom_tokenizers_count_tokens_error",
                    labels={
                        "model": model,
                        "provider": provider,
            ...
            log_counter(
                "custom_tokenizers_count_tokens_no_tokenizer",
                labels={"model": model, "provider": provider},
            )
    ...
    def count_messages_tokens(
        ...
        start_time = time.time()
        log_counter(
            "custom_tokenizers_count_messages_attempt",
            labels={
                "model": model,
                "provider": provider,
        ...
        log_counter(
            "custom_tokenizers_count_messages_success",
            labels={
                "model": model,
                "provider": provider,
                "message_count": str(len(messages)),
    ...
    def install_tokenizer(self, source_path: str, name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not source.exists():
                ...
                log_counter(
                    "custom_tokenizers_install_error",
                    labels={"error": "file_not_found"},
                )
                return False
...
def count_messages_with_custom(
