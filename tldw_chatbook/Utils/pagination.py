    ...
    while len(all_items) < max_items:
        ...
        if status_callback:
            result = status_callback(
                f"⏳ Loading page {page} ({len(all_items)} items loaded so far)..."
            )
...
class LazyPaginator:
    ...
    def __init__(
        self, fetch_func: Callable[[int, int], List[Any]], page_size: int = 100
    ):
    ...
    async def get_all_items(
        self, max_items: int = 1000, status_callback: Callable[[str], None] = None
    ) -> List[Any]:
        """
        ...
        while len(all_items) < max_items:
            if status_callback:
                status_callback(
                    f"⏳ Loading page {page} ({len(all_items)} items so far)..."
                )
