...
def get_ingestion_mode_preference(media_type: str) -> bool:
    ...
    try:
        ...
        mode = get_cli_setting(
            "ingestion_preferences", f"{media_type}_simple_mode", True
        )
        return bool(mode)
...
def save_ingestion_mode_preference(media_type: str, simple_mode: bool) -> None:
    ...
    try:
        save_setting_to_cli_config(
            "ingestion_preferences", f"{media_type}_simple_mode", simple_mode
        )
        logger.debug(
            f"Saved ingestion mode preference for {media_type}: simple_mode={simple_mode}"
        )
    except Exception as e:
...
def save_global_ingestion_mode_preference(simple_mode: bool) -> None:
    ...
    try:
        save_setting_to_cli_config(
            "ingestion_preferences", "global_simple_mode", simple_mode
        )
        logger.debug(
            f"Saved global ingestion mode preference: simple_mode={simple_mode}"
        )
    except Exception as e:
