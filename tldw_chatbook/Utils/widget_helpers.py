...
class FeatureNotAvailableDialog(Container):
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            yield Label(
                f"Feature Not Available: {self.feature_name}", classes="dialog-title"
            )

            if len(self.missing_deps) == 1:
                yield Static(
                    f"The required dependency '{self.missing_deps[0]}' is not installed.",
                ...
                yield Static(
                    f"The required dependencies {deps_text} are not installed.",
                    classes="dialog-message",
                )
