...
def secure_temp_file(
    ...
    log_counter(
        "secure_temp_files_create_file_attempt",
        labels={"has_suffix": str(bool(suffix)), "text_mode": str(text)},
    )
    ...
    try:
        ...
        log_histogram(
            "secure_temp_files_create_file_duration",
            duration,
        ...
        if temp_file:
            try:
                ...
                if os.path.exists(temp_file.name):
                    ...
                    log_counter(
                        "secure_temp_files_cleanup_success", labels={"type": "file"}
                    )
            except Exception as e:
                logger.error(f"Error cleaning up temporary file {temp_file.name}: {e}")
                log_counter(
                    "secure_temp_files_cleanup_error",
                    labels={"type": "file", "error_type": type(e).__name__},
                )
...
def secure_temp_dir(suffix: str = "", prefix: str = "tmp", dir: Optional[str] = None):
    ...
    try:
        ...
        duration = time.time() - start_time
        log_histogram(
            "secure_temp_files_create_dir_duration",
            duration,
            labels={"status": "success"},
        )
        ...
        if temp_dir and os.path.exists(temp_dir):
            try:
                ...
                log_counter(
                    "secure_temp_files_cleanup_success", labels={"type": "directory"}
                )
            except Exception as e:
                logger.error(f"Error cleaning up temporary directory {temp_dir}: {e}")
                log_counter(
                    "secure_temp_files_cleanup_error",
                    labels={"type": "directory", "error_type": type(e).__name__},
                )
...
def create_secure_temp_file(
    ...
    log_counter(
        "secure_temp_files_create_with_content_attempt",
        labels={
...
def secure_delete_file(file_path: Union[str, Path]) -> bool:
    ...
    try:
        ...
        log_counter(
            "secure_temp_files_secure_delete_error",
...
class SecureTempFileManager:
    ...
    def create_temp_file(
        ...
        log_counter(
            "secure_temp_files_manager_track_file",
            labels={"files_tracked": str(len(self._temp_files))},
        )
        return temp_path
    ...
    def create_temp_dir(
        ...
        log_counter(
            "secure_temp_files_manager_track_dir",
            labels={"dirs_tracked": str(len(self._temp_dirs))},
        )
        return temp_dir
    ...
    def cleanup_all(self):
        ...
        log_counter(
            "secure_temp_files_manager_cleanup_attempt",
            labels={
        ...
        log_counter(
            "secure_temp_files_manager_cleanup_complete",
            labels={
