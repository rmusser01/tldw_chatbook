...
def get_project_databases_dir() -> Path:
    ...
    try:
        ...
        log.info(
            f"Ensured project-internal database directory exists: {PROJECT_DATABASES_DIR}"
        )
        return PROJECT_DATABASES_DIR
    except OSError as e:
        log.error(
            f"Could not create or access project-internal database directory {PROJECT_DATABASES_DIR}: {e}",
            exc_info=True,
        )
        raise OSError(
            f"Failed to ensure project-internal database directory exists at {PROJECT_DATABASES_DIR}"
        ) from e
...
def get_project_database_path(db_filename: str) -> Path:
    ...
    log.debug(
        f"Returning project-internal database path for '{safe_db_filename}': {full_path}"
    )
    return full_path
...
def get_project_relative_path(
    relative_path_str: Union[str, os.PathLike[AnyStr]],
) -> Path:
    """
    ...
    log.debug(
        f"Resolved project relative path for '{relative_path_str}': {absolute_path}"
    )
