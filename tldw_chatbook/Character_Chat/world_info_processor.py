...
class WorldInfoProcessor:
    ...
    def _process_character_book(self):
        ...
        logger.debug(
            f"Loaded {len(self.entries)} active world info entries from character book"
        )
    ...
    def _process_world_books(self, world_books: List[Dict[str, Any]]):
        ...
        for book in world_books:
            ...
            self.recursive_scanning = self.recursive_scanning or book.get(
            ...
            priority_offset = (
                book.get("priority", 0) * 1000
            )  # Use priority to offset insertion order
            book_id = book.get("id")
        ...
        logger.debug(
            f"Total {len(self.entries)} active world info entries after processing {len(world_books)} world books"
        )
    ...
    def process_messages(
        ...
        matched_entries = sorted(
            matched_entries,
            key=lambda e: (
                -_coerce_int(e.get("priority"), 0),
                _coerce_int(e.get("insertion_order"), 0),
            ),
        )
    ...
    def process_messages_with_diagnostics(
        self,
        current_message: str,
        conversation_history: List[Dict[str, str]],
        scan_depth: Optional[int] = None,
        ...
        """
        from .world_info_diagnostics import (
        ...
        result = self.process_messages(
            current_message, conversation_history, scan_depth, apply_token_budget
        )
        ...
        diagnostics = WorldBookScanDiagnostics(
            ...
            matched=len(records),
            fired=fired_count,
            skipped=len(records) - fired_count,
            ...
            books_scanned=len(
                {
                    (c.get("_book_id"), c.get("_book_name"))
                    for c in self._candidate_entries
                }
            ),
        )
    ...
    def _find_matching_entries(
        self, scan_text: str, _recursion_depth: int = 0
    ) -> List[Dict[str, Any]]:
        ...
        if self.recursive_scanning and matched and _recursion_depth < 3:
            ...
            additional_matches = self._find_matching_entries(
    ...
    def _classify_entry_match(
        self, entry: Dict[str, Any], scan_text: str, scan_text_lower: str
    ) -> Tuple[bool, Optional[str], bool, bool, Optional[str]]:
        ...
        secondary_required = bool(
            entry.get("selective", False) and entry.get("secondary_keys")
        )
    ...
    def _organize_by_position(
        ...
        positions = {"before_char": [], "after_char": [], "at_start": [], "at_end": []}
    ...
    def _apply_token_budget(
        self, matched_entries: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for entry in matched_entries:
            ...
            if total_tokens + entry_tokens <= self.token_budget:
                ...
                logger.debug(
                    f"Token budget exceeded. Skipping entry: {entry.get('content', '')[:50]}..."
                )
                break

        logger.debug(
            f"Token budget: {total_tokens}/{self.token_budget} tokens used for {len(selected_entries)} entries"
        )
