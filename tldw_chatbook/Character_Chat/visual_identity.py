    ...
    try:
        ...
        try:
            filenames = tuple(_publication_candidate_filename(index, stored, candidate._replacements.get(str(stored["expression_key"])))
                              for index, stored in enumerate(candidate.assets) if str(stored["expression_key"]) not in candidate._cleared and (stored.get("id") is not None or str(stored["expression_key"]) in candidate._replacements))
        ...
        source_files = tuple(_visual_identity_source_path(stored, candidate.source_kind, profile_root)
                             for stored in candidate.assets if stored.get("id") is not None and str(stored["expression_key"]) not in candidate._cleared and str(stored["expression_key"]) not in candidate._replacements)
