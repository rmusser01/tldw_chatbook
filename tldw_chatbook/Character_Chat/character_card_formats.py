...
class CharacterCardFormatDetector:
    ...
    def _is_v2_format(self, data: Dict[str, Any]) -> bool:
        ...
        return (
            ...
            or data.get("spec_version") == "2.0"
            or ("data" in data and isinstance(data["data"], dict))
        )
    ...
    def _is_v1_format(self, data: Dict[str, Any]) -> bool:
        ...
        return (
            ...
            and "spec" not in data
            and "spec_version" not in data
            and len(v1_fields.intersection(data.keys())) >= 3
        )
    ...
    def _is_agnai_format(self, data: Dict[str, Any]) -> bool:
        ...
        return (
            len(agnai_fields.intersection(data.keys())) >= 2
            or data.get("kind") == "character"
            or "_id" in data
            and "userId" in data
        )
    ...
    def _is_characterai_format(self, data: Dict[str, Any]) -> bool:
        ...
        return (
            "participant__name" in data
            or "external_id" in data
            or ("info" in data and "character" in data["info"])
            or "greeting" in data
    ...
    def _is_koboldai_format(self, data: Dict[str, Any]) -> bool:
        ...
        return (
            "memory" in data
            or "authors_note" in data
            or ("world_info" in data and isinstance(data["world_info"], dict))
        )
    ...
    def _is_textgen_format(self, data: Dict[str, Any]) -> bool:
        ...
        return (
            "char_name" in data
            or "char_persona" in data
            or "char_greeting" in data
            or "context" in data
            and "greeting" in data
        )

    def _is_chub_format(self, data: Dict[str, Any]) -> bool:
        """Check if data is Chub.ai format."""
        return data.get("spec") == "chara_card_v2" and (
    ...
    def _parse_characterai_format(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        name = (
            data.get("participant__name")
            or data.get("name")
            or data.get("info", {}).get("character", {}).get("name")
            or "Unknown"
        )

        description = (
            data.get("description")
            or data.get("info", {}).get("character", {}).get("description")
            or ""
        )

        greeting = (
            data.get("greeting")
            or data.get("info", {}).get("character", {}).get("greeting")
            or ""
        )
    ...
    def _parse_koboldai_format(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        v2_card["data"]["extensions"]["kobold_authors_note"] = data.get(
            "authors_note", ""
        )
