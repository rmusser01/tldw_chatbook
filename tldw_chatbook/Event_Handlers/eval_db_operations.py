...
class EvalDBOperations:
    ...
    async def get_recent_runs(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            runs = await loop.run_in_executor(
                None, lambda: self.db.list_runs(limit=limit)
            )
    ...
    async def get_datasets(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            datasets = await loop.run_in_executor(
                None, lambda: self.db.list_datasets(limit=100)
            )
    ...
    async def get_models(self, provider: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            models = await loop.run_in_executor(
                None, lambda: self.db.list_models(provider=provider, limit=100)
            )
    ...
    async def get_run_details(self, run_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Get run info
            run = await loop.run_in_executor(None, lambda: self.db.get_run(run_id))
            ...
            metrics = await loop.run_in_executor(
                None, lambda: self.db.get_run_metrics(run_id)
            )
    ...
    async def create_run(
        self, task_id: str, model_id: str, dataset_id: str, config: Dict[str, Any]
    ) -> str:
    ...
    async def update_run_status(
        ...
        try:
            ...
            await loop.run_in_executor(
                None, lambda: self.db.update_run(run_id, **update_data)
            )
    ...
    async def get_task_types(self) -> List[str]:
        ...
        try:
            ...
            tasks = await loop.run_in_executor(
                None, lambda: self.db.list_tasks(limit=100)
            )

            # Extract unique task types
            task_types = list(set(task.get("task_type", "custom") for task in tasks))
    ...
    async def search_runs(
        self, query: str, status: Optional[str] = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            all_runs = await loop.run_in_executor(
                None, lambda: self.db.list_runs(status=status, limit=limit)
            )
            ...
            for run in all_runs:
                if (
                    query_lower in run.get("run_id", "").lower()
                    or query_lower in run.get("model_id", "").lower()
                    or query_lower in run.get("task_id", "").lower()
                ):
