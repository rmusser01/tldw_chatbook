        ...
        def populate_ccp_widgets():
            ...
            try:
                ...
                self.call_async_handler(
                    ccp_handlers.populate_ccp_character_select, self.app
                )
                self.call_async_handler(
                    ccp_handlers.populate_ccp_prompts_list_view, self.app
                )
                self.call_async_handler(
                    ccp_handlers.populate_ccp_dictionary_select, self.app
                )
                self.call_async_handler(
                    ccp_handlers.populate_ccp_worldbook_list, self.app
                )
                self.call_async_handler(
                    ccp_handlers.perform_ccp_conversation_search, self.app
                )
        ...
        def initialize_search_tab():
            ...
            try:
                ...
                if not self.app.search_active_sub_tab:
                    self.app.search_active_sub_tab = (
                        self.app._initial_search_sub_tab_view
                    )
                    self.log_initialization(
                        f"Search sub-tab set to: {self.app._initial_search_sub_tab_view}"
                    )
...
class IngestTabInitializer(BaseTabInitializer):
    ...
    async def on_tab_shown(self) -> None:
        ...
        if not self.app.ingest_active_view:
            self.log_initialization(
                f"Ingest tab shown, activating initial view: {self.app._initial_ingest_view}"
            )
            ...
            try:
                ...
                if isinstance(tools_window, ToolsSettingsWindow):
                    ...
                    if not self.app.tools_settings_active_view:
                        self.app.tools_settings_active_view = (
                            self.app._initial_tools_settings_view
                        )
                        self.log_initialization(
                            f"Tools & Settings view set to: {self.app._initial_tools_settings_view}"
                        )
