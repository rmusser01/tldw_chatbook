...
class MediaReadItLaterToggleEvent(Message):
    ...
    def __init__(
        self, media_id: Any, *, record_id: Any = None, save_for_later: bool = True
    ) -> None:
        super().__init__()
        self.media_id = media_id
        self.record_id = record_id if record_id is not None else media_id
...
async def handle_media_nav_button_pressed(
    ...
    try:
        ...
        media_window = app.query_one(f"#{app.TAB_MEDIA}-window").query_one(
            "MediaWindow"
        )
        ...
        logger.opt(exception=True).error(
            f"Error in handle_media_nav_button_pressed for '{button_id}': {e}"
        )
        app.notify(f"Error switching media view: {str(e)[:100]}", severity="error")


async def handle_media_search_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        logger.info(
            f"Media search triggered for type '{type_slug}' with term: '{search_term}' and keywords: '{keyword_filter}'"
        )
        app.media_current_page = 1  # Reset to page 1 for new search
        await perform_media_search_and_display(
            app, type_slug, search_term, keyword_filter
        )
    except QueryError as e:
        logger.opt(exception=True).error(
            f"UI component not found for media search button '{button_id}': {e}"
        )
        app.notify("Search UI error.", severity="error")
    except Exception as e:
        logger.opt(exception=True).error(
            f"Error in handle_media_search_button_pressed for '{button_id}': {e}"
        )
        app.notify(f"Error performing media search: {str(e)[:100]}", severity="error")


async def handle_media_search_input_changed(
    app: "TldwCli", input_id: str, value: str
) -> None:
    ...
    async def debounced_search():
        ...
        logger.info(
            f"Debounced media search executing for type '{type_slug}', term: '{search_term}', keywords: '{keyword_filter}'"
        )
        app.media_current_page = 1
        await perform_media_search_and_display(
            app, type_slug, search_term, keyword_filter
...
async def handle_media_list_item_selected(
    app: "TldwCli", event: ListView.Selected
) -> None:
    """
    ...
    if list_view_id and list_view_id.startswith("media-list-view-"):
        ...
        app.notify(
            "Internal error: Could not identify media type for selection.",
            severity="error",
        )
        return
    ...
    try:
        ...
        logger.error(
            f"MediaDetailsWidget '#{details_widget_id}' not found for type_slug '{type_slug}'."
        )
    ...
    logger.info(
        f"Fetching full details for media item ID: {media_id} for view {type_slug}"
    )
...
async def handle_media_load_selected_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    """
    ...
    try:
        ...
        logger.opt(exception=True).error(
            f"UI component not found for media load selected '{button_id}': {e}"
        )
        app.notify("Load details UI error.", severity="error")
        return

    if not list_view.highlighted_child or not hasattr(
        list_view.highlighted_child, "media_data"
    ):
    ...
    class DummyListViewSelectedEvent:
        def __init__(self, list_view_widget, item_widget):
            ...
            self.control = (
                list_view_widget  # For compatibility with Textual's event structure
            )
    ...
    logger.info(
        f"Triggered detail loading via button for type '{type_slug}' by simulating selection."
    )


async def handle_media_page_change_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        logger.warning(
            f"Search input not found for type_slug '{type_slug}' during pagination."
        )
    except Exception as e:
        logger.error(
            f"Error getting search term during pagination for '{type_slug}': {e}"
        )
...
async def perform_media_search_and_display(
    app: "TldwCli", type_slug: str, search_term: str = "", keyword_filter: str = ""
) -> None:
    ...
    try:
        ...
        try:
            ...
            details_widget = app.query_one(
                f"#media-details-widget-{type_slug}", MediaDetailsWidget
            )
            details_widget.media_data = None  # Clear the widget
        except QueryError:
            logger.warning(
                f"MediaDetailsWidget '#media-details-widget-{type_slug}' not found for clearing."
            )
        ...
        if type_slug != "all-media" and type_slug != "analysis-review":
            db_media_type = type_slug.replace(
                "-", "_"
            )  # Convert slug to DB type format if necessary
        ...
        fields_arg = [
            ...
            "type",
            "analysis_content",
        ]  # Ensure analysis_content is searched
        ...
        if generation != app._media_search_generation.get(type_slug):
            logger.debug(
                f"Discarding stale media search results for type '{type_slug}'."
            )
            return
        ...
        if not results:
            ...
            for item in results:
                ...
                snippet = (
                    (snippet_label + content_preview[:70] + "...")
                    if content_preview
                ...
                rich_list_item = ListItem(
                    Vertical(
                        Label(
                            f"{title_display}",
                            classes="media-item-title media-item-deleted"
                            if is_deleted
                            else "media-item-title",
                        ),
                        Static(
                            snippet,
                            classes="media-item-snippet media-item-deleted"
                            if is_deleted
                            else "media-item-snippet",
                        ),
                        Static(
                            f"Type: {item.get('type')}  |  Ingested: {ingestion_date}",
                            classes="media-item-meta media-item-deleted"
                            if is_deleted
                            else "media-item-meta",
                        ),
                    ),
        ...
        try:
            total_pages = (
                math.ceil(total_matches / RESULTS_PER_PAGE) if total_matches > 0 else 1
            )
        ...
        logger.opt(exception=True).error(
            f"Error during media search for type '{type_slug}': {str(e)}"
        )
...
async def handle_media_metadata_update(
    app: "TldwCli", event: MediaMetadataUpdateEvent
) -> None:
    """
        ...
        if success:
            ...
            if updated_media:
                ...
                try:
                    ...
                    details_widget = app.query_one(
                        f"#media-details-widget-{event.type_slug}", MediaDetailsWidget
                    )
                    details_widget.update_media_data(updated_media)
                    details_widget.edit_mode = False  # Exit edit mode
                except QueryError:
                    logger.warning(
                        f"Could not find MediaDetailsWidget for type_slug: {event.type_slug}"
                    )
                ...
                try:
                    search_input = app.query_one(
                        f"#media-search-input-{event.type_slug}", Input
                    )
                    search_term = search_input.value
                    keyword_input = app.query_one(
                        f"#media-keyword-filter-{event.type_slug}", Input
                    )
                    keyword_filter = keyword_input.value
                except QueryError:
                    pass
                await perform_media_search_and_display(
                    app, event.type_slug, search_term, keyword_filter
                )
            else:
                logger.error(
                    f"Could not fetch updated media data for ID {event.media_id}"
                )
...
async def handle_media_delete_confirmation(
    app: "TldwCli", event: MediaDeleteConfirmationEvent
) -> None:
    """
    ...
    """
    logger.warning(
        "handle_media_delete_confirmation called directly - should be handled in MediaWindow_v2"
    )
...
async def handle_media_undelete(app: "TldwCli", event: MediaUndeleteEvent) -> None:
    ...
    try:
        ...
        if success:
            app.notify(
                f"'{media_item.get('title', 'Untitled')}' has been restored",
                severity="information",
            )
            # MediaWindow_v2 will handle the refresh when it receives the event
        else:
            app.notify(
                f"Failed to restore '{media_item.get('title', 'Untitled')}'",
                severity="error",
            )
