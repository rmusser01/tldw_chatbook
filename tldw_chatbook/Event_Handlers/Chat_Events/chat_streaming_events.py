    ...
    if current_widget and current_widget.is_mounted:
        try:
            ...
            if is_continuation:
                ...
                if (
                    not hasattr(self, "continue_thinking_removed")
                    or not self.continue_thinking_removed
                ):
                    # First chunk in continuation - append to existing text
                    if hasattr(self, "continue_original_text"):
                        current_widget.message_text = (
                            self.continue_original_text + event.text_chunk
                        )
                    else:
            ...
            if chat_log_id_to_query:
                try:
                    chat_log_container = self.query_one(
                        chat_log_id_to_query, VerticalScroll
                    )
            ...
            logger.opt(exception=True).error(
                f"Error accessing UI components during streaming chunk update: {e}"
            )
        except Exception as e_chunk:  # Catch any other unexpected error
            logger.opt(exception=True).error(
                "Unexpected error processing streaming chunk: {}", e_chunk
            )
    else:
        logger.warning(
...
async def handle_stream_done(self, event: StreamDone) -> None:
    ...
    logger = getattr(self, "loguru_logger", _loguru_fallback_logger)
    logger.info(
        f"StreamDone received. Final text length: {len(event.full_text)}. Error: '{event.error}'"
    )
    ...
    if not ai_widget or not ai_widget.is_mounted:
        logger.warning(
            "Received StreamDone but current_ai_message_widget is missing or not mounted."
        )
        if event.error:  # If there was an error, at least notify the user
            self.notify(
                f"Stream error (display widget missing): {event.error}",
                severity="error",
                timeout=10,
            )
        ...
        try:
            if self.current_tab == TAB_CHAT:
                ...
                self.query_one(
                    "#ccp-chat-input", TextArea
                ).focus()  # Adjust if ID is different
        except QueryError:
    ...
    try:
        ...
        if event.error:
            ...
            try:
                ...
                logger.warning(
                    "Could not update AI message header for stream error display."
                )
            ...
            if event.full_text:  # Check if there's any text to process
                strip_tags_setting = self.app_config.get("chat_defaults", {}).get(
                    "strip_thinking_tags", True
                )
                self.loguru_logger.info(
                    f"Strip thinking tags setting: {strip_tags_setting} (from config: {self.app_config.get('chat_defaults', {})})"
                )
                if strip_tags_setting:
                    ...
                    if len(think_blocks) > 1:
                        ...
                        for i, block in enumerate(think_blocks):
                            if i < len(think_blocks) - 1:  # This is a block to remove
                                text_parts.append(
                                    event.full_text[last_kept_block_end : block.start()]
                                )
                                last_kept_block_end = block.end()
                        text_parts.append(event.full_text[last_kept_block_end:])
                        event.full_text = "".join(
                            text_parts
                        )  # Modify the event's full_text
                        self.loguru_logger.debug(
                            f"Streamed response after stripping: {event.full_text[:200]}..."
                        )
                    else:
                        self.loguru_logger.info(
                    ...
                    self.loguru_logger.debug(
                        "Not stripping tags from stream: strip_thinking_tags setting is disabled."
                    )

            ai_widget.message_text = (
                event.full_text
            )  # Ensure internal state has the final, complete text
            ...
            if hasattr(event, "response_data") and event.response_data:
                logger.debug(
                    f"Checking for tool calls in response data: {type(event.response_data)}"
                )
                tool_calls = parse_tool_calls_from_response(event.response_data)
                if tool_calls:
                    logger.info(
                        f"Detected {len(tool_calls)} tool call(s) in streaming response"
                    )
                    ...
                    if chat_container:
                        ...
                        try:
                            ...
                            if (
                                self.chachanotes_db
                                and self.current_chat_conversation_id
                                and not self.current_chat_is_ephemeral
                            ):
                                try:
                            ...
                            await self._continue_conversation_with_tools(
                                results, tool_calls
                            )
                            ...
                            self.notify(
                                f"Tool execution error: {str(e)}", severity="error"
                            )
                    else:
                        logger.warning(
            ...
            ai_sender_name_for_db = (
                ai_widget.role
            )  # Role should be correctly set by now
            ...
            if (
                ...
                and self.current_chat_conversation_id
                and not self.current_chat_is_ephemeral
                and event.full_text.strip()
            ):
                if is_continuation:
                    # For continuation, update the existing message
                    if ai_widget.message_id_internal and hasattr(
                        ai_widget, "message_version_internal"
                    ):
                        try:
                            logger.debug(
                            ...
                            if success:
                                ...
                                logger.info(
                                    f"Continued message ID {ai_widget.message_id_internal} updated in DB. New version: {ai_widget.message_version_internal}"
                                )
                                self.notify(
                                    "Message continuation saved.",
                                    severity="information",
                                    timeout=2,
                                )
                            else:
                                logger.error(
                                    f"Failed to update continued message {ai_widget.message_id_internal} in DB"
                                )
                                self.notify(
                                    "Failed to save continuation to DB",
                                    severity="error",
                                )
                        except Exception as e_update:
                            logger.opt(exception=True).error(
                                f"Error updating continued message: {e_update}"
                            )
                            self.notify(
                                f"DB error saving continuation: {str(e_update)[:100]}",
                                severity="error",
                            )
                else:
                    ...
                    try:
                        ...
                        if ai_msg_db_id:
                            saved_ai_msg_details = (
                                self.chachanotes_db.get_message_by_id(ai_msg_db_id)
                            )
                            if saved_ai_msg_details:
                                ai_widget.message_id_internal = (
                                    saved_ai_msg_details.get("id")
                                )
                                ai_widget.message_version_internal = (
                                    saved_ai_msg_details.get("version")
                                )
                                logger.info(
                            ...
                            logger.error(
                                "Failed to save streamed AI message to DB (no ID returned)."
                            )

                        # NEW: Check if this is a regeneration - create variant
                        if (
                            hasattr(self, "regenerating_message_widget")
                            and self.regenerating_message_widget
                        ):
                            original_widget = self.regenerating_message_widget
                            original_id = getattr(
                                self, "regenerating_original_id", None
                            )
                            ...
                            if original_id is None:
                                ...
                                logger.info(
                                    "Regenerate: Content replaced in original widget (ephemeral chat)"
                                )
                                ...
                                try:
                                    ...
                                    if variant_id:
                                        ...
                                        variants = (
                                            self.chachanotes_db.get_message_variants(
                                                original_id
                                            )
                                        )
                                        variant_count = len(variants)
                                        current_variant_num = next(
                                            (
                                                i + 1
                                        ...
                                        logger.info(
                                            f"Created variant {variant_id} (#{current_variant_num} of {variant_count}) for message {original_id}"
                                        )
                                        self.notify(
                                            f"Response variant {current_variant_num} of {variant_count} generated",
                                            severity="information",
                                        )
                                    else:
                                        logger.error(
                                            f"Failed to create variant for message {original_id}"
                                        )
                                except Exception as e:
                                    logger.opt(exception=True).error(
                                        f"Error creating message variant: {e}"
                                    )
                        ...
                        logger.opt(exception=True).error(
                            f"DB Error saving streamed AI message: {e_save_ai_stream}"
                        )
                        self.notify(
                            f"DB error saving message: {e_save_ai_stream}",
                            severity="error",
                        )
                    except Exception as e_save_unexp:
                        logger.opt(exception=True).error(
                            f"Unexpected error saving streamed AI message: {e_save_unexp}"
                        )
                        self.notify(
                            "Unexpected error saving message.", severity="error"
                        )
            elif not event.full_text.strip() and not event.error:
                logger.info(
                    "Stream finished with no error but content was empty/whitespace. Not saving to DB."
                )
        ...
        logger.opt(exception=True).error(
            f"QueryError during StreamDone UI update (event.error='{event.error}'): {e}"
        )
        if (
            event.error
        ):  # If there was an underlying stream error, make sure user sees it
            self.notify(
                f"Stream Error (UI issue): {event.error}", severity="error", timeout=10
            )
        else:  # If stream was fine, but UI update failed
            self.notify("Error finalizing AI message display.", severity="error")
    except (
        Exception
    ) as e_done_unexp:  # Catch any other unexpected error during the try block
        logger.opt(exception=True).error(
            f"Unexpected error in on_stream_done (event.error='{event.error}'): {e_done_unexp}"
        )
        ...
        if is_continuation:
            ...
            self.current_ai_message_widget = (
                None  # Clear the reference to the AI message widget
            )
            logger.debug(
                "Cleared current_ai_message_widget in on_stream_done's finally block."
            )

        # Reset streaming state using thread-safe method
        self.set_current_chat_is_streaming(False)
        logger.debug(
            "Reset current_chat_is_streaming to False in on_stream_done's finally block."
        )
        ...
        if self.current_tab == TAB_CHAT:
            ...
            input_id_to_focus = (
                "#ccp-chat-input"  # Adjust if ID is different for CCP tab's input
            )
        ...
        if input_id_to_focus:
            try:
                ...
                logger.warning(
                    f"Could not focus input '{input_id_to_focus}' in on_stream_done (widget not found)."
                )
            except Exception as e_focus_final:
                logger.opt(exception=True).error(
                    f"Error focusing input '{input_id_to_focus}' in on_stream_done: {e_focus_final}"
                )
        else:
            logger.debug(
                f"No specific input to focus for tab {self.current_tab} in on_stream_done."
            )
    ...
    try:
        ...
        for i, result in enumerate(tool_results):
            ...
            if "error" in result:
                tool_results_text.append(
                    f"Tool call {tool_call_id} failed: {result['error']}"
                )
            else:
                tool_result_data = result.get("result", {})
                tool_results_text.append(
                    f"Tool call {tool_call_id} result: {json.dumps(tool_result_data, indent=2)}"
                )
        ...
        try:
            if self.current_tab == TAB_CHAT:
                ...
                logger.warning(
                    f"Unknown tab {self.current_tab}, cannot continue conversation"
                )
                return
            ...
            try:
                ...
                await send_button.post_message(
                    Click(send_button, 0, 0, 0, 0, 0, False, False, False)
                )
                ...
                self.notify(
                    "Could not automatically continue conversation with tool results",
                    severity="warning",
                )
            ...
            self.notify(
                "Could not continue conversation with tool results", severity="warning"
            )

    except Exception as e:
        logger.opt(exception=True).error(
            f"Error continuing conversation with tool results: {e}"
        )
