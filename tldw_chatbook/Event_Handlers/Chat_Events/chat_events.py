...
def safe_json_loads(
    ...
    if len(json_str.encode("utf-8")) > max_size:
        loguru_logger.warning(
            f"JSON string too large: {len(json_str)} bytes (max {max_size})"
        )
        return None
...
def _is_console_chatbook_artifact_eligible(
    ...
    return (
        action_widget.has_class("-ai")
        and str(message_role or "").strip().lower() != "system"
    )
    ...
    for line in str(message_text or "").splitlines():
        ...
        if title:
            if len(title) > MAX_CONSOLE_CHATBOOK_ARTIFACT_TITLE_CHARS:
                return (
                    title[: MAX_CONSOLE_CHATBOOK_ARTIFACT_TITLE_CHARS - 3].rstrip()
                    + "..."
                )
            return title
...
def _console_chatbook_artifact_description(message_text: str) -> str:
    ...
    if len(preview) > MAX_CONSOLE_CHATBOOK_ARTIFACT_DESCRIPTION_CHARS:
        preview = (
            preview[: MAX_CONSOLE_CHATBOOK_ARTIFACT_DESCRIPTION_CHARS - 3].rstrip()
            + "..."
        )
    if not preview:
    ...
    def save_worker() -> None:
        try:
            ...
            loguru_logger.opt(exception=True).error(
                "Failed to save Console response as Chatbook artifact"
            )
            app.call_from_thread(
                app.notify,
...
async def handle_chat_tab_sidebar_toggle(app: "TldwCli", event: Button.Pressed) -> None:
    ...
    if button_id == "toggle-chat-left-sidebar":
        ...
        loguru_logger.debug(
            "Chat tab settings sidebar (left) now %s",
        ...
        loguru_logger.debug(
            "Chat tab character sidebar (right) now %s",
            "collapsed" if app.chat_right_sidebar_collapsed else "expanded",
        )
    else:
        loguru_logger.warning(
            f"Unhandled sidebar toggle button ID '{button_id}' in Chat tab handler."
        )
...
def _stage_current_handoff_evidence_bundle(
    app: "TldwCli", payload: ChatHandoffPayload
) -> None:
...
def _current_handoff_evidence_bundle(app: "TldwCli", message_widget: Any = None) -> Any:
    if message_widget is not None:
        bundle = evidence_bundle_from_value(
            getattr(message_widget, "citation_evidence_bundle", None)
        )
    ...
    payload = ChatHandoffPayload.from_dict(
        getattr(app, "_current_chat_handoff_payload", None)
    )
    if payload is None:
        return None
...
def _tabbed_chat_selector(app: "TldwCli", selector: str) -> str:
    ...
    if (
        isinstance(current_tab_id, str)
        and current_tab_id
        and selector in {"#chat-input", "#chat-log"}
    ):
        return f"{selector}-{current_tab_id}"
    return selector


async def handle_chat_send_button_pressed(
    ...
    if (
        hasattr(app, "current_chat_worker")
        and app.current_chat_worker
        and app.current_chat_worker.is_running
    ):
    ...
    loguru_logger.info(
        f"Send button pressed for '{prefix}' (main chat)"
    )  # Use loguru_logger consistently
    ...
    try:
        ...
        system_prompt_widget = current_screen.query_one(
            f"#{prefix}-system-prompt", TextArea
        )
        ...
        llm_max_tokens_widget = current_screen.query_one(
            f"#{prefix}-llm-max-tokens", Input
        )
        ...
        llm_response_format_widget = current_screen.query_one(
            f"#{prefix}-llm-response-format", Select
        )
        llm_n_widget = current_screen.query_one(f"#{prefix}-llm-n", Input)
        llm_user_identifier_widget = current_screen.query_one(
            f"#{prefix}-llm-user-identifier", Input
        )
        llm_logprobs_widget = current_screen.query_one(
            f"#{prefix}-llm-logprobs", Checkbox
        )
        llm_top_logprobs_widget = current_screen.query_one(
            f"#{prefix}-llm-top-logprobs", Input
        )
        llm_logit_bias_widget = current_screen.query_one(
            f"#{prefix}-llm-logit-bias", TextArea
        )
        llm_presence_penalty_widget = current_screen.query_one(
            f"#{prefix}-llm-presence-penalty", Input
        )
        llm_frequency_penalty_widget = current_screen.query_one(
            f"#{prefix}-llm-frequency-penalty", Input
        )
        llm_tools_widget = current_screen.query_one(f"#{prefix}-llm-tools", TextArea)
        llm_tool_choice_widget = current_screen.query_one(
            f"#{prefix}-llm-tool-choice", Input
        )
        llm_fixed_tokens_kobold_widget = current_screen.query_one(
            f"#{prefix}-llm-fixed-tokens-kobold", Checkbox
        )
        # Query for the strip thinking tags checkbox
        try:
            strip_tags_checkbox = current_screen.query_one(
                "#chat-strip-thinking-tags-checkbox", Checkbox
            )
            strip_thinking_tags_value = strip_tags_checkbox.value
            loguru_logger.info(
                f"Read strip_thinking_tags checkbox value: {strip_thinking_tags_value}"
            )
        except QueryError:
        ...
        loguru_logger.error(
            f"Send Button: Could not find UI widgets for '{prefix}': {e}"
        )
        log_counter(
            "chat_ui_widget_error", labels={"tab": prefix, "error": "query_error"}
        )
        try:
            ...
            container_for_error = (
                chat_container
                if "chat_container" in locals() and chat_container.is_mounted
                else current_screen.query_one(
                    _tabbed_chat_selector(app, f"#{prefix}-log"), VerticalScroll
                )
            )  # Re-query if initial one failed
            ...
            loguru_logger.error(
                f"Send Button: Critical - could not even find chat container #{prefix}-log to display error."
            )
        return
    ...
    if message_text_from_input:
        if not validate_text_input(
            message_text_from_input, max_length=100000, allow_html=False
        ):
            ...
            log_counter(
                "chat_ui_message_validation_failed",
                labels={"tab": prefix, "reason": "invalid_content"},
            )
            return

        # Sanitize the message text to remove dangerous characters
        message_text_from_input = sanitize_string(
            message_text_from_input, max_length=100000
        )
        log_histogram(
            "chat_ui_message_length",
            len(message_text_from_input),
            labels={"tab": prefix},
        )

    reuse_last_user_bubble = False
    resend_conversation = (
        False  # New flag specifically for resending the entire conversation
    )

    if (
        not message_text_from_input
    ):  # Try to resend conversation if last message is from user
        try:
            ...
            if all_messages:
                ...
                loguru_logger.debug(
                    f"Found {len(all_messages)} messages. Last message role: {last_message.role}, type: {type(last_message).__name__}"
                )
                ...
                if last_message.has_class("-user"):
                    ...
                    loguru_logger.info(
                        f"Last message is from user (role: {last_message.role}), resending conversation"
                    )
                    ...
                    loguru_logger.debug(
                        "Last message is not from user (role: %s), not resending",
                        last_message.role,
                    )
                    text_area.focus()
                    return
            ...
            loguru_logger.opt(exception=True).error(
                "Failed to inspect last message for resend: %s", exc
            )
            text_area.focus()
            return

    selected_provider = (
        str(provider_widget.value) if provider_widget.value != Select.BLANK else None
    )
    selected_model = (
    ...
    if system_prompt and not validate_text_input(
        system_prompt, max_length=50000, allow_html=False
    ):
        await chat_container.mount(
            ChatMessage(
                Text.from_markup(
                    "Error: System prompt contains invalid content or is too long."
                ),
                role="System",
                classes="-error",
            )
    ...
    temperature = safe_float(
        temp_widget.value, 0.7, "temperature"
    )  # Use imported safe_float
    ...
    if not validate_number_range(temperature, 0.0, 2.0):
        await chat_container.mount(
            ChatMessage(
                Text.from_markup("Error: Temperature must be between 0.0 and 2.0."),
                role="System",
                classes="-error",
            )
        )
    ...
    if not validate_number_range(top_p, 0.0, 1.0):
        await chat_container.mount(
            ChatMessage(
                Text.from_markup("Error: Top-p must be between 0.0 and 1.0."),
                role="System",
                classes="-error",
            )
        )
    ...
    if not validate_number_range(min_p, 0.0, 1.0):
        await chat_container.mount(
            ChatMessage(
                Text.from_markup("Error: Min-p must be between 0.0 and 1.0."),
                role="System",
                classes="-error",
            )
        )
    ...
    if not validate_number_range(top_k, 1, 1000):
        await chat_container.mount(
            ChatMessage(
                Text.from_markup("Error: Top-k must be between 1 and 1000."),
                role="System",
                classes="-error",
            )
        )
    ...
    if selected_provider:
        ...
        provider_specific_settings = app.app_config.get("api_settings", {}).get(
            provider_settings_key, {}
        )
        should_stream = provider_specific_settings.get("streaming", False)
        loguru_logger.debug(
            f"Streaming for {selected_provider} set to {should_stream} based on config."
        )
    else:
        loguru_logger.debug(
            "No provider selected, streaming defaults to False for this request."
        )

    # Check streaming checkbox to override provider setting
    try:
        streaming_checkbox = current_screen.query_one(
            "#chat-streaming-enabled-checkbox", Checkbox
        )
        streaming_override = streaming_checkbox.value
        if streaming_override != should_stream:
            loguru_logger.info(
                f"Streaming override: checkbox={streaming_override}, provider default={should_stream}"
            )
    ...
    system_prompt_from_ui = (
        system_prompt_widget.text
    )  # This is the system prompt from the LEFT sidebar
    active_char_data = (
    ...
    if active_char_data:
        ...
        char_specific_system_prompt = active_char_data.get(
            "system_prompt"
        )  # This comes from the editable fields
        if (
            char_specific_system_prompt is not None
            and char_specific_system_prompt.strip()
    ...
    llm_seed_value = safe_int(
        llm_seed_widget.value, None, "llm_seed"
    )  # None is a valid default
    llm_stop_value = (
        [s.strip() for s in llm_stop_widget.value.split(",") if s.strip()]
        if llm_stop_widget.value.strip()
        else None
    )
    llm_response_format_value = (
        {"type": str(llm_response_format_widget.value)}
        if llm_response_format_widget.value != Select.BLANK
        else {"type": "text"}
    )
    ...
    llm_top_logprobs_value = (
        safe_int(llm_top_logprobs_widget.value, 0, "llm_top_logprobs")
        if llm_logprobs_value
        else 0
    )
    llm_presence_penalty_value = safe_float(
        llm_presence_penalty_widget.value, 0.0, "llm_presence_penalty"
    )
    llm_frequency_penalty_value = safe_float(
        llm_frequency_penalty_widget.value, 0.0, "llm_frequency_penalty"
    )
    ...
    if llm_logit_bias_text and llm_logit_bias_text != "{}":
        llm_logit_bias_value = safe_json_loads(
            llm_logit_bias_text, max_size=64 * 1024
        )  # 64KB limit
        if llm_logit_bias_value is None and llm_logit_bias_text:
            await chat_container.mount(
                ChatMessage(
                    Text.from_markup(
                        "Error: Invalid or too large JSON in LLM Logit Bias. Parameter not used."
                    ),
                    role="System",
                    classes="-error",
                )
    ...
    if llm_tools_text and llm_tools_text != "[]":
        llm_tools_value = safe_json_loads(
            llm_tools_text, max_size=256 * 1024
        )  # 256KB limit for tools
        if llm_tools_value is None and llm_tools_text:
            await chat_container.mount(
                ChatMessage(
                    Text.from_markup(
                        "Error: Invalid or too large JSON in LLM Tools. Parameter not used."
                    ),
                    role="System",
                    classes="-error",
                )
    ...
    if not selected_provider:
        await chat_container.mount(
            ChatMessage(
                Text.from_markup("Please select an API Provider."),
                role="System",
                classes="-error",
            )
        )
    ...
    if not selected_model:
        await chat_container.mount(
            ChatMessage(
                Text.from_markup("Please select a Model."),
                role="System",
                classes="-error",
            )
        )
    ...
    if not app.API_IMPORTS_SUCCESSFUL:  # Access as app attribute
        await chat_container.mount(
            ChatMessage(
                Text.from_markup("Error: Core API functions failed to load."),
                role="System",
                classes="-error",
            )
        )
    ...
    try:
        ...
        all_ui_messages = sorted(
            all_chat_messages + all_enhanced_messages,
            key=lambda w: (
                chat_container.children.index(w)
                if w in chat_container.children
                else float("inf")
            ),
        )
        ...
        if resend_conversation:
            loguru_logger.debug(
                "Resending conversation - including all messages in history"
            )
            messages_to_process_for_history = all_ui_messages
            ...
            try:
                ...
                ValueError,
                NameError,
            ):  # NameError if last_msg_widget wasn't set, ValueError if not found
                loguru_logger.warning(
                    "Could not definitively exclude reused message from history; sending full history."
                )
        ...
        for msg_widget in messages_to_process_for_history:
            if msg_widget.role in ("User", "AI") or (
                app.current_chat_active_character_data
                and msg_widget.role
                == app.current_chat_active_character_data.get("name")
            ):
                if msg_widget.generation_complete:  # Only send completed messages
                    # Map UI role to API role (user/assistant)
                    api_role = "user"
                    if (
                        msg_widget.role != "User"
                    ):  # Anything not "User" is treated as assistant for API history
                        api_role = "assistant"
                    ...
                    if (
                        hasattr(msg_widget, "image_data")
                        and msg_widget.image_data
                        and msg_widget.image_mime_type
                        and is_vision_capable(selected_provider, selected_model)
                    ):
                        try:
                            import base64
                            ...
                            loguru_logger.debug(
                                f"Including image in API history for {api_role} message"
                            )
                        except Exception as e:
                            loguru_logger.warning(
                                f"Failed to encode image for API: {e}"
                            )
        ...
        loguru_logger.debug(
            f"Built chat history for API with {len(chat_history_for_api)} messages."
        )

    except Exception as e:
        loguru_logger.opt(exception=True).error(
            f"Failed to build chat history for API: {e}"
        )
        await chat_container.mount(
            ChatMessage(
                Text.from_markup("Internal Error: Could not retrieve chat history."),
                role="System",
                classes="-error",
            )
        )
            ...
            try:
                ...
                if user_message_db_id_version_tuple:  # This is just the ID
                    ...
                    if saved_user_msg_details:
                        user_msg_widget_instance.message_id_internal = (
                            saved_user_msg_details.get("id")
                        )
                        user_msg_widget_instance.message_version_internal = (
                            saved_user_msg_details.get("version")
                        )
                        loguru_logger.debug(
                            f"User message saved to DB. ID: {saved_user_msg_details.get('id')}, Version: {saved_user_msg_details.get('version')}"
                        )
                    else:
                        loguru_logger.error(
                            f"Failed to retrieve saved user message details from DB for ID {user_msg_db_id}."
                        )
                else:
                    loguru_logger.error(
                        f"Failed to save user message to DB for conversation {active_conversation_id}."
                    )
            except (
                CharactersRAGDBError,
                InputError,
            ) as e_add_msg:  # Catch specific errors from ccl
                loguru_logger.opt(exception=True).error(
                    f"Error saving user message to DB: {e_add_msg}"
                )
            except Exception as e_add_msg_generic:
                loguru_logger.opt(exception=True).error(
                    f"Generic error saving user message to DB: {e_add_msg_generic}"
                )

    elif app.current_chat_is_ephemeral:
        loguru_logger.debug(
            "Chat is ephemeral. User message not saved to DB at this stage."
        )
    ...
    if provider_readiness.requires_api_key and not provider_readiness.ready:
        loguru_logger.error(
            f"API Key for '{selected_provider}' is missing and required."
        )
        error_message_markup = provider_readiness.user_message
        await chat_container.mount(
            ChatMessage(message=error_message_markup, role="System")
        )
    ...
    try:
        ...
        if rag_context:
            loguru_logger.info(
                f"RAG context retrieved, length: {len(rag_context)} chars"
            )
    ...
    chatdict_entries = []
    if (
        app.current_chat_conversation_id or app.current_chat_active_character_data
    ) and db:
        try:
            ...
            if chatdict_entries:
                loguru_logger.info(
                    f"Total chat dictionary entries loaded: {len(chatdict_entries)}"
                )
        except Exception as e:
            loguru_logger.opt(exception=True).error(
                f"Error loading chat dictionaries: {e}"
            )
            # Continue without dictionaries on error

    # --- 10.7. Apply staged handoff context and World Info if enabled ---
    message_text_with_handoff = apply_current_handoff_context(
    ...
    api_params = {
        ...
        "system_prompt_length": len(final_system_prompt_for_api)
    ...
    if llm_n_value and llm_n_value > 1:
        ...
        if should_stream:
            ...
            should_stream = False
            loguru_logger.info(
                f"Disabled streaming because n={llm_n_value} (multiple responses requested)"
            )
        ...
        if not confirmed:
            loguru_logger.info(
                f"User cancelled multiple response generation (n={llm_n_value})"
            )
            app.notify(
                "Multiple response generation cancelled.", severity="information"
            )
            return
        ...
        loguru_logger.info(
            f"User confirmed generation of {llm_n_value} response variants"
        )
    ...
    if media_content_for_api:
        loguru_logger.info(
            f"DEBUG: Passing media_content_for_api to chat_wrapper: mime_type={media_content_for_api.get('mime_type')}, has_base64_data={bool(media_content_for_api.get('base64_data'))}"
        )
    else:
        loguru_logger.info(
    ...
    log_counter(
        "chat_ui_message_sent",
        labels={"tab": prefix, "provider": selected_provider or "none"},
    )


async def handle_chat_action_button_pressed(
    app: "TldwCli",
    button: Button,
    action_widget: Union[ChatMessage, ChatMessageEnhanced],
) -> None:
    ...
    if "edit-button" in button_classes:
        ...
        if not is_editing:  # Start editing
            current_text_for_editing = (
                message_text  # Use the internally stored raw text
            )
            markdown_widget.display = False
            editor = TextArea(
                text=current_text_for_editing, id="edit-area", classes="edit-area"
            )
            ...
            try:
                ...
                if (
                    db
                    and action_widget.message_id_internal
                    and action_widget.message_version_internal is not None
                ):
                    try:
                        ...
                        if success:
                            ...
                            app.notify(
                                "Failed to save edit to DB (update operation returned false).",
                                severity="error",
                            )
                        ...
                        app.notify(
                            f"Save conflict: {e_conflict}. Please reload the chat or message.",
                            severity="error",
                            timeout=7,
                        )
                        ...
                        app.notify(
                            f"Failed to save edit to DB: {e_db_update}",
                            severity="error",
                        )
                    except (
                        Exception
                    ) as e_generic_update:  # Catch any other unexpected error
                        loguru_logger.opt(exception=True).error(
                            f"Unexpected error updating message {action_widget.message_id_internal} in DB: {e_generic_update}"
                        )
                        app.notify(
                            f"An unexpected error occurred while saving the edit: {e_generic_update}",
                            severity="error",
                        )

            except QueryError:
                loguru_logger.error(
                    "Edit TextArea not found when stopping edit. Restoring original."
                )
        ...
        app.copy_to_clipboard(
            message_text
        )  # message_text is already the raw, unescaped version
        app.notify(
            "Message content copied to clipboard.", severity="information", timeout=2
        )
        button.label = get_char(EMOJI_COPIED, FALLBACK_COPIED) + "Copied"
        app.set_timer(
            1.5, lambda: setattr(button, "label", get_char(EMOJI_COPY, FALLBACK_COPY))
        )

    elif "artifact-button" in button_classes:
        logging.info("Action: Save artifact clicked for %s message", message_role)
        await _save_console_chatbook_artifact(
            app, action_widget, message_text, message_role
        )

    elif "note-button" in button_classes:
        logging.info(
            "Action: Create Note clicked for %s message: '%s...'",
            message_role,
            message_text[:50],
        )
        ...
        conversation_context = {
            ...
            "timestamp": action_widget.timestamp
            ...
            if isinstance(result, tuple) and result[0] == "note":
                ...
                try:
                    ...
                    if note_id:
                            ...
                            for keyword in note_data["keywords"]:
                                try:
                                    ...
                                    if keyword_id:
                                        ...
                                        loguru_logger.debug(
                                            f"Linked keyword '{keyword}' to note {note_id}"
                                        )
                                except Exception as kw_e:
                                    loguru_logger.error(
                                        f"Error adding keyword '{keyword}': {kw_e}"
                                    )
                        ...
                        loguru_logger.info(
                            f"Created note '{note_data['title']}' with ID: {note_id} and {len(note_data.get('keywords', []))} keywords"
                        )
                    else:
                    ...
                    loguru_logger.opt(exception=True).error(
                        f"Error creating note from message: {e}"
                    )
                    app.notify(f"Failed to create note: {str(e)}", severity="error")

            elif isinstance(result, str):
                # Generate document using LLM (other document types)
                await generate_document_with_llm(
                    app, result, message_content, conversation_context
                )
        ...
        modal = DocumentGenerationModal(
            message_content=message_text, conversation_context=conversation_context
        )
        ...
        logging.info(
            "Action: Extract Files clicked for %s message: '%s...'",
            message_role,
            message_text[:50],
        )
        ...
        logging.info(
            f"Action: Speak clicked for {message_role} message: '{message_text[:50]}...'"
        )
        ...
        app.post_message(TTSRequestEvent(text=message_text, message_id=message_id))
        ...
        app.post_message(TTSPlaybackEvent(action="play", message_id=message_id))
        ...
        # Post TTS playback event
        app.post_message(TTSPlaybackEvent(action="pause", message_id=message_id))
        ...
        app.post_message(TTSPlaybackEvent(action="stop", message_id=message_id))
        ...
        def on_feedback_ready(result):
            ...
            if (
                ...
                try:
                    ...
                    if success:
                        ...
                        if new_feedback and new_feedback.startswith("1;"):
                            ...
                            button.label = "👍"
                            app.notify(
                                "Feedback cleared", severity="information", timeout=2
                            )

                        # Clear the other thumb button if it was selected
                        try:
                            other_button = action_widget.query_one(
                                "#thumb-down", Button
                            )
                            other_button.label = "👎"
                        except QueryError:
                            loguru_logger.debug(
                                "Thumb down button not found, likely already updated"
                            )

                        loguru_logger.info(
                            f"Message {action_widget.message_id_internal} feedback updated"
                        )
                    else:
                    ...
                    app.notify(
                        "Feedback conflict - please reload chat", severity="error"
                    )
                except Exception as e:
                ...
                try:
                    ...
                    loguru_logger.debug(
                        "Thumb down button not found when updating thumb-up feedback (no DB)"
                    )
        ...
        def on_feedback_ready(result):
            ...
            if (
                ...
                try:
                    ...
                    if success:
                        ...
                        try:
                            ...
                            loguru_logger.debug(
                                "Thumb up button not found when updating thumb-down feedback"
                            )
                ...
                try:
                    ...
                    loguru_logger.debug(
                        "Thumb up button not found when updating thumb-down feedback (no DB)"
                    )
                ...
                if db and message_id_to_delete:
                    try:
                        ...
                        expected_version = getattr(
                            action_widget, "message_version_internal", None
                        )
                        if expected_version is not None:
                            db.soft_delete_message(
                                message_id_to_delete, expected_version
                            )
                            loguru_logger.info(
                                f"Message ID {message_id_to_delete} soft-deleted from DB."
                            )
                            app.notify(
                                "Message deleted.", severity="information", timeout=2
                            )
                        else:
                            loguru_logger.error(
                                f"Cannot delete message {message_id_to_delete}: missing version information"
                            )
                            app.notify(
                                "Cannot delete message: missing version information",
                                severity="error",
                            )
                    except Exception as e_db_delete:
                        loguru_logger.opt(exception=True).error(
                            f"Failed to delete message {message_id_to_delete} from DB: {e_db_delete}"
                        )
                        app.notify(
                            "Failed to delete message from DB.", severity="error"
                        )
            except Exception as exc:
        ...
        try:
            ...
            loguru_logger.error(
                f"Regenerate: Could not find chat container #{prefix}-log. Aborting."
            )
        ...
        all_message_widgets_in_log = list(chat_container.query(ChatMessage)) + list(
            chat_container.query(ChatMessageEnhanced)
        )

        for msg_widget_iter in all_message_widgets_in_log:
            if (
                msg_widget_iter is action_widget
            ):  # This is the AI message we're regenerating
            ...
            if found_target_ai_message_for_regen:
                ...
                if msg_widget_iter.generation_complete:
                    ...
                    char_name_regen = (
                        active_char_data_regen.get("name", "AI")
                        if active_char_data_regen
                        else "AI"
                    )
                    role_for_api = (
                        "assistant"
                        if msg_widget_iter.role == char_name_regen
                        else "user"
                    )
                    history_for_regeneration.append(
                        {"role": role_for_api, "content": msg_widget_iter.message_text}
                    )

        if not history_for_regeneration:
            loguru_logger.warning(
                "Regenerate: No history found before the target AI message. Cannot regenerate."
            )
            app.notify(
                "Cannot regenerate: No preceding messages found.", severity="warning"
            )
            return
        ...
        if widgets_after_target:
            loguru_logger.info(
                f"Regenerate: Removing {len(widgets_after_target)} messages after target"
            )
        ...
        original_message_id = getattr(
            original_message_widget, "message_id_internal", None
        )
        ...
        if original_message_id is None:
            ...
            original_message_widget._generation_complete_internal = (
                False  # Mark as generating
            )
            ...
            if (
                ...
                in [original_message_widget] + widgets_after_target
            ):
                app.current_ai_message_widget = None
        ...
        try:
            ...
            system_prompt_widget_regen = app.query_one(
                f"#{prefix}-system-prompt", TextArea
            )
            ...
            # Full chat settings
            llm_max_tokens_widget_regen = app.query_one(
                f"#{prefix}-llm-max-tokens", Input
            )
            llm_seed_widget_regen = app.query_one(f"#{prefix}-llm-seed", Input)
            llm_stop_widget_regen = app.query_one(f"#{prefix}-llm-stop", Input)
            llm_response_format_widget_regen = app.query_one(
                f"#{prefix}-llm-response-format", Select
            )
            llm_n_widget_regen = app.query_one(f"#{prefix}-llm-n", Input)
            llm_user_identifier_widget_regen = app.query_one(
                f"#{prefix}-llm-user-identifier", Input
            )
            llm_logprobs_widget_regen = app.query_one(
                f"#{prefix}-llm-logprobs", Checkbox
            )
            llm_top_logprobs_widget_regen = app.query_one(
                f"#{prefix}-llm-top-logprobs", Input
            )
            llm_logit_bias_widget_regen = app.query_one(
                f"#{prefix}-llm-logit-bias", TextArea
            )
            llm_presence_penalty_widget_regen = app.query_one(
                f"#{prefix}-llm-presence-penalty", Input
            )
            llm_frequency_penalty_widget_regen = app.query_one(
                f"#{prefix}-llm-frequency-penalty", Input
            )
            llm_tools_widget_regen = app.query_one(f"#{prefix}-llm-tools", TextArea)
            llm_tool_choice_widget_regen = app.query_one(
                f"#{prefix}-llm-tool-choice", Input
            )
            # Query for the strip thinking tags checkbox for regeneration
            try:
                strip_tags_checkbox_regen = app.query_one(
                    "#chat-strip-thinking-tags-checkbox", Checkbox
                )
                strip_thinking_tags_value_regen = strip_tags_checkbox_regen.value
            except QueryError:
                loguru_logger.warning(
                    "Regenerate: Could not find '#chat-strip-thinking-tags-checkbox'. Defaulting to True."
                )
                strip_thinking_tags_value_regen = True
        except QueryError as e_query_regen:
            loguru_logger.error(
                f"Regenerate: Could not find UI settings widgets for '{prefix}': {e_query_regen}"
            )
            await chat_container.mount(
                ChatMessage(
                    Text.from_markup(
                        "[bold red]Internal Error:[/]\nMissing UI settings for regeneration."
                    ),
                    role="System",
                    classes="-error",
                )
        ...
        selected_provider_regen = (
            str(provider_widget_regen.value)
            if provider_widget_regen.value != Select.BLANK
            else None
        )
        selected_model_regen = (
            str(model_widget_regen.value)
            if model_widget_regen.value != Select.BLANK
            else None
        )
        ...
        if active_char_data_regen:
            loguru_logger.info(
                f"Active character data found for REGENERATION: {active_char_data_regen.get('name', 'Unnamed')}. Overriding system prompt."
            )
            ...
            if system_prompt_override_regen is not None:
                ...
                loguru_logger.debug(
                    f"System prompt for REGENERATION overridden by active character: '{system_prompt_regen[:100]}...'"
                )
            else:
                loguru_logger.debug(
                    f"Active character data present for REGENERATION, but 'system_prompt' is None or missing. Using: '{system_prompt_regen[:100]}...' (might be from UI or empty)."
                )
        else:
            loguru_logger.info(
                "No active character data for REGENERATION. Using system prompt from UI."
            )
        should_stream_regen = False  # Default for regen
        if selected_provider_regen:
            provider_settings_key_regen = selected_provider_regen.lower().replace(
                " ", "_"
            )
            provider_specific_settings_regen = app.app_config.get(
                "api_settings", {}
            ).get(provider_settings_key_regen, {})
            should_stream_regen = provider_specific_settings_regen.get(
                "streaming", False
            )
            loguru_logger.debug(
            ...
            loguru_logger.debug(
                "No provider selected for REGENERATION, streaming defaults to False."
            )

        # Check streaming checkbox to override provider setting for regeneration
        try:
            streaming_checkbox_regen = app.screen.query_one(
                "#chat-streaming-enabled-checkbox", Checkbox
            )
            streaming_override_regen = streaming_checkbox_regen.value
            if streaming_override_regen != should_stream_regen:
                loguru_logger.info(
                    f"Streaming override for REGENERATION: checkbox={streaming_override_regen}, provider default={should_stream_regen}"
                )
                should_stream_regen = streaming_override_regen
        except QueryError:
            loguru_logger.debug(
                "Streaming checkbox not found for REGENERATION, using provider default"
            )
        # --- End of Integration & Streaming Config for REGENERATION ---

        llm_max_tokens_value_regen = safe_int(
            llm_max_tokens_widget_regen.value, 1024, "llm_max_tokens"
        )
        llm_seed_value_regen = safe_int(llm_seed_widget_regen.value, None, "llm_seed")
        llm_stop_value_regen = (
            [s.strip() for s in llm_stop_widget_regen.value.split(",")]
            if llm_stop_widget_regen.value.strip()
            else None
        )
        llm_response_format_value_regen = (
            {"type": str(llm_response_format_widget_regen.value)}
            if llm_response_format_widget_regen.value != Select.BLANK
            else {"type": "text"}
        )
        llm_n_value_regen = safe_int(llm_n_widget_regen.value, 1, "llm_n")
        llm_user_identifier_value_regen = (
            llm_user_identifier_widget_regen.value.strip() or None
        )
        llm_logprobs_value_regen = llm_logprobs_widget_regen.value
        llm_top_logprobs_value_regen = (
            safe_int(llm_top_logprobs_widget_regen.value, 0, "llm_top_logprobs")
            if llm_logprobs_value_regen
            else 0
        )
        llm_presence_penalty_value_regen = safe_float(
            llm_presence_penalty_widget_regen.value, 0.0, "llm_presence_penalty"
        )
        llm_frequency_penalty_value_regen = safe_float(
            llm_frequency_penalty_widget_regen.value, 0.0, "llm_frequency_penalty"
        )
        ...
        try:
            ...
            llm_logit_bias_value_regen = (
                json.loads(llm_logit_bias_text_regen)
                if llm_logit_bias_text_regen and llm_logit_bias_text_regen != "{}"
                else None
            )
        ...
        try:
            ...
            llm_tools_value_regen = (
                json.loads(llm_tools_text_regen)
                if llm_tools_text_regen and llm_tools_text_regen != "[]"
                else None
            )
        except json.JSONDecodeError:
        ...
        provider_config_settings_regen = app.app_config.get("api_settings", {}).get(
            provider_settings_key_regen, {}
        )
        if provider_config_settings_regen.get("api_key"):
            ...
            api_key_for_regen = os.environ.get(
                provider_config_settings_regen["api_key_env_var"]
            )

        providers_requiring_key_regen = [
            "OpenAI",
            "Anthropic",
            "Google",
            "MistralAI",
        ...
        if (
            selected_provider_regen in providers_requiring_key_regen
            and not api_key_for_regen
        ):
            loguru_logger.error(
            ...
            await chat_container.mount(
                ChatMessage(
                    Text.from_markup(
                        f"[bold red]API Key for {selected_provider_regen} is missing for regeneration.[/]"
                    ),
                    role="System",
                    classes="-error",
                )
        ...
        if original_message_id is not None:  # Saved conversation - create new widget
            ...
            chat_container.scroll_end(animate=False)
            loguru_logger.debug(
                "Regenerate: Using existing widget for ephemeral chat streaming"
            )
        ...
        if not app.chachanotes_db:
            app.notify(
                "Database not available for variant navigation", severity="error"
            )
            return

        original_id = getattr(action_widget, "variant_of", None) or getattr(
            action_widget, "message_id_internal", None
        )
        if not original_id:
        ...
        current_variant_id = getattr(action_widget, "variant_id", None) or getattr(
            action_widget, "message_id_internal", None
        )
        current_index = next(
            (i for i, v in enumerate(variants) if v["id"] == current_variant_id), 0
        )
        ...
        app.notify(
            f"Showing variant {new_index + 1} of {len(variants)}",
            severity="information",
            timeout=2,
        )
        ...
        variant_id = getattr(action_widget, "variant_id", None) or getattr(
            action_widget, "message_id_internal", None
        )
        ...
        try:
            ...
            if chat_container:
                ...
                for widget in chat_container.query(ChatMessageEnhanced):
                    ...
                    if (
                        widget_variant_of == original_id or widget_id == original_id
                    ) and widget != action_widget:
                        widget.is_selected_variant = False
                        # Show select button on unselected variants
                        try:
                            other_select_btn = widget.query_one(
            ...
            app.notify(
                "Selected this response to continue the conversation",
                severity="information",
            )
            loguru_logger.info(
                f"Selected variant {variant_id} as the active variant for message {original_id}"
            )
...
async def handle_chat_new_conversation_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def handle_chat_save_current_chat_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    try:
        ...
        loguru_logger.debug(
            f"Platform: {platform.system()}, Version: {platform.version()}"
        )

        if not (
            app.current_chat_is_ephemeral and app.current_chat_conversation_id is None
        ):
            loguru_logger.warning(
                "Chat not eligible for saving (not ephemeral or already has ID)."
            )
            app.notify(
                "This chat is already saved or cannot be saved in its current state.",
                severity="warning",
            )
            return
        ...
        all_messages = list(chat_log_widget.query(ChatMessage)) + list(
            chat_log_widget.query(ChatMessageEnhanced)
        )
        messages_in_log = sorted(
            all_messages, key=lambda w: chat_log_widget.children.index(w)
        )
        loguru_logger.debug(
            f"Found {len(messages_in_log)} messages in chat log (including enhanced)"
        )
        ...
        char_name_for_sender = (
            "AI"  # Default sender name for AI messages if no specific character
        )

        if (
            app.current_chat_active_character_data
            and "id" in app.current_chat_active_character_data
        ):
            character_id_for_saving = app.current_chat_active_character_data["id"]
            char_name_for_sender = app.current_chat_active_character_data.get(
                "name", "AI"
            )  # Use actual char name for sender
            loguru_logger.info(
                f"Saving chat with active character: {char_name_for_sender} (ID: {character_id_for_saving})"
            )
        else:
            loguru_logger.info(
                f"Saving chat with default character association (ID: {character_id_for_saving})"
            )
        ...
        new_conv_title_from_ui = app.query_one(
        ...
        if not final_title_for_db:
            ...
            title_char_name_part = (
                char_name_for_sender
                if character_id_for_saving != ccl.DEFAULT_CHARACTER_ID
                else "Assistant"
            )
            # Check if first message is from a user (not the AI character)
            if (
                ui_messages_to_save
                and ui_messages_to_save[0]["sender"] != char_name_for_sender
            ):
        ...
        keywords_str_from_ui = app.query_one(
            "#chat-conversation-keywords-input", TextArea
        ).text.strip()
        keywords_list_for_db = [
            kw.strip()
            for kw in keywords_str_from_ui.split(",")
        ...
        try:
            ...
            if new_conv_id:
                ...
                app.current_chat_is_ephemeral = (
                    False  # Now it's saved, triggers watcher
                )
            ...
            loguru_logger.opt(exception=True).error(
                f"Exception while saving chat: {e_save_chat}"
            )
            app.notify(f"Error saving chat: {str(e_save_chat)[:100]}", severity="error")

    except Exception as e_outer:
        loguru_logger.opt(exception=True).error(
            f"Unexpected error in save current chat handler: {e_outer}"
        )
        app.notify(
            f"Unexpected error saving chat: {str(e_outer)[:100]}", severity="error"
        )


async def handle_chat_convert_to_note_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        for msg_widget in all_ui_messages:
            ...
            if (
                hasattr(msg_widget, "generation_complete")
                and not msg_widget.generation_complete
            ):
                continue
            ...
            note_content_parts.extend(
                [f"[{msg_timestamp}] {msg_role}:", msg_content, "", "-" * 30, ""]
            )
        ...
        note_id = notes_service.add_note(
            user_id=app.client_id, title=note_title, content=note_content
        )

        if note_id:
            app.notify(
                f"Conversation converted to note: {note_title[:50]}...",
                severity="success",
                timeout=3,
            )
        ...
        loguru_logger.opt(exception=True).error(
            f"Error converting conversation to note: {e}"
        )
        app.notify(f"Failed to convert conversation: {str(e)}", severity="error")


async def handle_chat_clone_current_chat_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        if app.current_chat_conversation_id and not app.current_chat_is_ephemeral:
            ...
            # Get keywords
            keywords_data = db.get_keywords_for_conversation(
                app.current_chat_conversation_id
            )
            keywords_list = [
                kw["keyword"]
            ...
            original_title = (
                app.query_one("#chat-conversation-title-input", Input).value.strip()
                or "Untitled Chat"
            )
            character_id = (
                app.current_chat_active_character_data.get("id")
                if app.current_chat_active_character_data
                else ccl.DEFAULT_CHARACTER_ID
            )

            keywords_str = app.query_one(
                "#chat-conversation-keywords-input", TextArea
            ).text.strip()
            keywords_list = [
                kw.strip()
        ...
        if new_conv_id:
            ...
            app.notify(
                f"Chat cloned successfully! Now editing: {new_title[:50]}...",
                severity="success",
                timeout=3,
            )
...
async def handle_chat_save_details_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    if app.current_chat_is_ephemeral or not app.current_chat_conversation_id:
        loguru_logger.warning(
            "Cannot save details for an ephemeral or non-existent chat."
        )
        app.notify(
            "No active saved conversation to update details for.", severity="warning"
        )
        return
    ...
    try:
        ...
        keywords_input_widget = app.query_one(
            "#chat-conversation-keywords-input", TextArea
        )
        ...
        if not conv_details:
            loguru_logger.error(
                f"Conversation {conversation_id} not found in DB for saving details."
            )
        ...
        if current_version is None:
            loguru_logger.error(
                f"Conversation {conversation_id} is missing version information."
            )
            app.notify(
                "Error: Conversation version information is missing.", severity="error"
            )
            return

        title_changed = False
        if new_title != conv_details.get(
            "title", ""
        ):  # Compare with empty string if title is None
            db.update_conversation(
                conversation_id, {"title": new_title}, current_version
            )
            current_version += 1  # Version is now incremented for the conversation row
            title_changed = True
            loguru_logger.info(
                f"Title updated for conversation {conversation_id}. New version: {current_version}"
            )
        ...
        db_user_keywords_map = {
            kw["keyword"]: kw["id"]
            for kw in all_db_keywords_list
            if not kw["keyword"].startswith("__")
        }
        db_user_keywords_set = set(db_user_keywords_map.keys())
        ui_user_keywords_set = {
            kw.strip()
            for kw in new_keywords_str.split(",")
            if kw.strip() and not kw.strip().startswith("__")
        }
        ...
        for keyword_text_add in keywords_to_add:
            keyword_detail_add = db.get_keyword_by_text(
                keyword_text_add
            )  # Does not take user_id
            keyword_id_to_link = None
            if not keyword_detail_add:  # Keyword doesn't exist globally
                added_kw_id = db.add_keyword(
                    keyword_text_add
                )  # Takes no user_id, returns int ID
                if isinstance(added_kw_id, int):
                    keyword_id_to_link = added_kw_id
                else:
                    logging.error(
                        f"Failed to add keyword '{keyword_text_add}', received: {added_kw_id}"
                    )
                    continue
            else:
        ...
        for keyword_text_remove in keywords_to_remove_text:
            ...
            if keyword_id_to_unlink:
                db.unlink_conversation_from_keyword(
                    conversation_id, keyword_id_to_unlink
                )
                keywords_changed = True
        ...
        if title_changed or keywords_changed:
            ...
            final_db_keywords_after_save = db.get_keywords_for_conversation(
                conversation_id
            )
            final_user_keywords_after_save = [
                kw["keyword"]
        ...
        loguru_logger.opt(exception=True).error(
            f"Save Conversation Details: UI component not found: {e_query}"
        )
        app.notify("Error accessing UI fields.", severity="error", timeout=3)
    except ConflictError as e_conflict:
        loguru_logger.opt(exception=True).error(
            f"Conflict saving conversation details for {conversation_id}: {e_conflict}"
        )
        app.notify(
            f"Save conflict: {e_conflict}. Please reload.", severity="error", timeout=5
        )
    except CharactersRAGDBError as e_db:  # More generic DB error
        loguru_logger.opt(exception=True).error(
            f"DB error saving conversation details for {conversation_id}: {e_db}"
        )
        app.notify("Database error saving details.", severity="error", timeout=3)
    except Exception as e_unexp:
        loguru_logger.opt(exception=True).error(
            f"Unexpected error saving conversation details for {conversation_id}: {e_unexp}"
        )
        app.notify("Unexpected error saving details.", severity="error", timeout=3)


async def handle_chat_load_selected_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    loguru_logger.info("Load selected chat button pressed.")
    try:
        results_list_view = app.query_one(
            "#chat-conversation-search-results-list", ListView
        )
        ...
        if not isinstance(highlighted_widget, ListItem):  # Check if it's a ListItem
            app.notify(
                "No chat selected to load (not a list item).", severity="warning"
            )
            loguru_logger.info(
                "No conversation selected in the list to load (highlighted_widget is not ListItem)."
            )
            return

        loaded_conversation_id: Optional[str] = getattr(
            highlighted_widget, "conversation_id", None
        )

        if loaded_conversation_id is None:
            app.notify(
                "No chat selected or item is invalid (missing conversation_id).",
                severity="warning",
            )
        ...
        loguru_logger.info(
            f"Attempting to load and display conversation ID: {loaded_conversation_id}"
        )
        ...
        conversation_title = getattr(
            highlighted_widget, "conversation_title", "Untitled"
        )
        app.notify(f"Chat '{conversation_title}' loaded.", severity="information")

    except QueryError as e_query:
        loguru_logger.opt(exception=True).error(
            f"UI component not found for loading chat: {e_query}"
        )
        ...
        loguru_logger.opt(exception=True).error(
            f"Unexpected error loading chat: {e_unexp}"
        )
...
async def perform_chat_conversation_search(app: "TldwCli") -> None:
    ...
    try:
        ...
        keyword_search_term = ""
        try:
            keyword_search_bar = app.query_one(
                "#chat-conversation-keyword-search-bar", Input
            )
        ...
        include_char_chats_checkbox = app.query_one(
            "#chat-conversation-search-include-character-checkbox", Checkbox
        )
        include_character_chats = (
            include_char_chats_checkbox.value
        )  # Currently unused in DB query, filtered client side

        all_chars_checkbox = app.query_one(
            "#chat-conversation-search-all-characters-checkbox", Checkbox
        )
        search_all_characters = all_chars_checkbox.value

        char_filter_select = app.query_one(
            "#chat-conversation-search-character-filter-select", Select
        )
        selected_character_id_filter = (
            char_filter_select.value
            if not char_filter_select.disabled
            and char_filter_select.value != Select.BLANK
            else None
        )
        ...
        if not app.notes_service:
            ...
            await results_list_view.append(
                ListItem(Label("Error: Notes service unavailable."))
            )
            return
        ...
        if include_character_chats:
            ...
            if not search_all_characters and selected_character_id_filter:
                ...
                loguru_logger.debug(
                    f"Filtering for specific character ID: {effective_character_id_for_search}"
                )
            else:
                # Either "All Characters" is checked or no specific character selected
                effective_character_id_for_search = (
                    None  # This will search all conversations
                )
                loguru_logger.debug(
                    "Searching all conversations (character chats included)"
                )
        else:
            ...
            loguru_logger.debug(
                "Will filter for regular chats only (client-side filtering)"
            )
        ...
        if not search_term:
            ...
            if (
                effective_character_id_for_search is not None
                and effective_character_id_for_search != ccl.DEFAULT_CHARACTER_ID
            ):
                # Specific character selected - show all conversations for that character
                conversations = db.get_conversations_for_character(
                    character_id=effective_character_id_for_search, limit=100
                )
                ...
                conversations = [
                    ...
                    if conv.get("character_id") == ccl.DEFAULT_CHARACTER_ID
                    or conv.get("character_id") is None
                ]
            else:
        ...
        if keyword_search_term and conversations:
            ...
            keyword_matches = db.search_conversations_by_content(
                keyword_search_term, limit=100
            )
            ...
            conversations = [
                conv for conv in conversations if conv["id"] in keyword_conv_ids
            ]
            filtered_count = original_count - len(conversations)
            if filtered_count > 0:
                loguru_logger.debug(
                    f"Keyword filter removed {filtered_count} conversations, keeping {len(conversations)} that match '{keyword_search_term}'"
                )
        ...
        if tag_search_term and conversations:
            ...
            search_tags = [
                tag.strip() for tag in tag_search_term.split(",") if tag.strip()
            ]
            ...
            if search_tags:
                ...
                for tag in search_tags:
                    ...
                    for keyword in keyword_results:
                        ...
                        tag_conversations = db.get_conversations_for_keyword(
                            keyword_id, limit=100
                        )
                ...
                original_count = len(conversations)
                conversations = [
                    conv for conv in conversations if conv["id"] in matching_conv_ids
                ]
                filtered_count = original_count - len(conversations)
                if filtered_count > 0:
                    loguru_logger.debug(
                        f"Tag filter removed {filtered_count} conversations, keeping {len(conversations)} that match tags: {search_tags}"
                    )
        ...
        if filter_regular_chats_only and conversations:
            ...
            if filtered_count > 0:
                loguru_logger.debug(
                    f"Filtered out {filtered_count} character conversations, keeping {len(conversations)} regular chats"
                )

        if conversations:
            original_count = len(conversations)
            conversations = [
                conv for conv in conversations if is_general_history_conversation(conv)
            ]
            filtered_count = original_count - len(conversations)
            if filtered_count > 0:
                loguru_logger.debug(
        ...
        if not conversations:
            ...
            for conv_data in conversations:
                title_str = (
                    conv_data.get("title") or f"Chat ID: {conv_data['id'][:8]}..."
                )
                ...
                item.conversation_title = conv_data.get(
                    "title"
        ...
        loguru_logger.info(
            f"Conversation search yielded {len(conversations)} results for display."
        )

    except QueryError as e_query:
        loguru_logger.opt(exception=True).error(
            f"UI component not found during conversation search: {e_query}"
        )
        if "results_list_view" in locals() and results_list_view.is_mounted:
            try:
                await results_list_view.append(
                    ListItem(Label("Error: UI component missing."))
                )
        ...
        loguru_logger.opt(exception=True).error(
            f"Database error during conversation search: {e_db}"
        )
        if "results_list_view" in locals() and results_list_view.is_mounted:
            try:
                await results_list_view.append(
                    ListItem(Label("Error: Database search failed."))
                )
            except (QueryError, AttributeError):
                # QueryError if results_list_view is not properly mounted/accessible
                # AttributeError if results_list_view is None or invalid
        ...
        loguru_logger.opt(exception=True).error(
            f"Unexpected error during conversation search: {e_unexp}"
        )
        if "results_list_view" in locals() and results_list_view.is_mounted:
            try:
                await results_list_view.append(
                    ListItem(Label("Error: Unexpected search failure."))
                )
            except (QueryError, AttributeError):
                # QueryError if results_list_view is not properly mounted/accessible
                # AttributeError if results_list_view is None or invalid
...
async def handle_chat_conversation_search_bar_changed(
    app: "TldwCli", event_value: str
) -> None:
    ...
    app._conversation_search_timer = app.set_timer(
        0.5, lambda: perform_chat_conversation_search(app)
    )
...
async def handle_chat_search_checkbox_changed(
    app: "TldwCli", checkbox_id: str, value: bool
) -> None:
    ...
    if checkbox_id == "chat-conversation-search-all-characters-checkbox":
        try:
            ...
            if value:
                char_filter_select.value = (
                    Select.BLANK
                )  # Clear selection when "All" is checked
        except QueryError as e:
            loguru_logger.opt(exception=True).error(
                f"Error accessing character filter select: {e}"
            )
...
async def display_conversation_in_chat_tab_ui(app: "TldwCli", conversation_id: str):
    if not app.chachanotes_db:  # Use correct DB instance name
        loguru_logger.error(
            "chachanotes_db unavailable, cannot display conversation in chat tab."
        )
        return
    ...
    if not full_conv_data or not full_conv_data.get("metadata"):
        loguru_logger.error(
            f"Cannot display conversation: Details for ID {conversation_id} not found or incomplete."
        )
        ...
        try:
            ...
            await chat_log_err.mount(
                ChatMessage(
                    Text.from_markup(
                        "[bold red]Failed to load conversation details.[/]"
                    ),
                    role="System",
                    classes="-error",
                )
            )
        except QueryError as qe_err_disp:
            loguru_logger.error(
                f"UI component missing during error display for conv {conversation_id}: {qe_err_disp}"
            )
        return
    ...
    try:
        ...
        if (
            ...
            and character_id_from_conv != ccl.DEFAULT_CHARACTER_ID
        ):
            loguru_logger.debug(
                f"Conversation {conversation_id} is associated with char_id: {character_id_from_conv}"
            )
            char_data_for_ui, _, _ = load_character_and_image(
                db, character_id_from_conv, current_user_name
            )
            if char_data_for_ui:
        ...
        for msg_data in db_messages:
            ...
            if msg_data.get("image_data") or use_enhanced_chat:
                chat_msg_widget_for_display = ChatMessageEnhanced(
                    ...
                    sender=msg_data.get(
                        "sender"
                    ),  # Pass sender for proper class assignment
                )
            ...
            if parent_msg_id:
                ...
                message_widgets_by_parent[parent_msg_id].append(
                    chat_msg_widget_for_display
                )
        ...
        app.query_one("#chat-input", TextArea).focus()
        app.notify(
            f"Chat '{conv_metadata.get('title', 'Untitled')}' loaded.",
            severity="information",
            timeout=3,
        )
...
async def handle_chat_character_search_input_changed(
    app: "TldwCli", event: Input.Changed
) -> None:
    ...
    try:
        ...
        loguru_logger.opt(exception=True).error(
            f"UI component not found for character search: {e_query}"
        )
        # Don't notify here as it's an input change, could be spammy. Log is enough.
    except Exception as e_unexp:
        loguru_logger.opt(exception=True).error(
            f"Unexpected error in character search input change: {e_unexp}"
        )
        # Don't notify here.


async def handle_chat_load_character_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        try:
            ...
            results_list_view = app.screen.query_one(
                "#chat-character-search-results-list", ListView
            )
        except QueryError:
        ...
        if not isinstance(highlighted_widget, ListItem):  # Check if it's a ListItem
            app.notify(
                "No character selected to load (not a list item).", severity="warning"
            )
            loguru_logger.info(
                "No character selected in the list to load (highlighted_widget is not ListItem)."
            )
            return
        ...
        selected_char_id: Optional[str] = getattr(
            highlighted_widget, "character_id", None
        )
        ...
        ):  # This should be app.chachanotes_db for character operations
            app.notify("Database service not available.", severity="error")
            loguru_logger.error(
                "ChaChaNotes DB (via notes_service) not available for loading character."
            )
            return
        ...
        character_data_full, _, _ = load_character_and_image(
            db, selected_char_id, app.notes_user_id
        )

        if character_data_full is None:
            app.notify(
                f"Character with ID {selected_char_id} not found in database.",
                severity="error",
            )
            loguru_logger.error(
                f"Could not retrieve data for character ID {selected_char_id} from DB (returned None)."
            )
            try:
                ...
                loguru_logger.warning(
                    f"Could not clear all character edit fields after failed load: {qe_clear}"
                )
            app.current_chat_active_character_data = None
            return
        ...
        try:
            app.query_one(
                "#chat-character-name-edit", Input
            ).value = character_data_full.get("name", "")
            app.query_one(
                "#chat-character-description-edit", TextArea
            ).text = character_data_full.get("description", "")
            app.query_one(
                "#chat-character-personality-edit", TextArea
            ).text = character_data_full.get("personality", "")
            app.query_one(
                "#chat-character-scenario-edit", TextArea
            ).text = character_data_full.get("scenario", "")
            app.query_one(
                "#chat-character-system-prompt-edit", TextArea
            ).text = character_data_full.get("system_prompt", "")
            app.query_one(
                "#chat-character-first-message-edit", TextArea
            ).text = character_data_full.get("first_message", "")
        except QueryError as qe_populate:
            loguru_logger.opt(exception=True).error(
                f"Error populating character edit fields: {qe_populate}"
            )
        ...
        app.notify(
            f"Character '{character_data_full.get('name', 'Unknown')}' loaded.",
            severity="information",
        )
        ...
        active_char_data_dict: Optional[Dict[str, Any]] = (
            app.current_chat_active_character_data
        )
        ...
        if app.current_chat_is_ephemeral:
            loguru_logger.debug(
                "Chat is ephemeral, checking if greeting is appropriate."
            )
            if active_char_data_dict:  # Check if the dictionary is not None
                try:
                    chat_log_widget = app.query_one("#chat-log", VerticalScroll)
                    messages_in_log = list(chat_log_widget.query(ChatMessage)) + list(
                        chat_log_widget.query(ChatMessageEnhanced)
                    )
                    ...
                    if not messages_in_log:
                        loguru_logger.debug(
                            "Chat log is empty. Greeting is appropriate."
                        )
                    else:
                        for msg_widget in messages_in_log:
                            if msg_widget.role != "User":
                                ...
                                loguru_logger.debug(
                                    f"Found message from role '{msg_widget.role}'. Greeting not appropriate."
                                )
                                break
                        if not character_has_spoken:
                            loguru_logger.debug(
                                "No non-User messages found in log. Greeting is appropriate."
                            )

                    if not messages_in_log or not character_has_spoken:
                        # Use active_char_data_dict here
                        first_message_content = active_char_data_dict.get(
                            "first_message"
                        )
                        ...
                        if first_message_content and character_name:
                            loguru_logger.info(
                                f"Displaying first_message for {character_name}."
                            )
                            ...
                            loguru_logger.debug(
                                f"Character {character_name} has no first_message defined."
                            )
                        elif not character_name:
                            loguru_logger.debug(
                                "Character name not found, cannot display first_message effectively."
                            )
                except QueryError as e_chat_log:
                    loguru_logger.error(
                        f"Could not find #chat-log to check for messages or mount greeting: {e_chat_log}"
                    )
                except Exception as e_greeting:
                    loguru_logger.opt(exception=True).error(
                        f"Error displaying character greeting: {e_greeting}"
                    )
            else:
                loguru_logger.debug(
                    "No active character data (active_char_data_dict is None), skipping greeting."
                )
        # --- End of fix ---

        loguru_logger.info(
            f"Character ID {selected_char_id} loaded and fields populated."
        )

    except QueryError as e_query:
        loguru_logger.opt(exception=True).error(
            f"UI component not found for loading character: {e_query}"
        )
        app.notify("Error: Character load UI elements missing.", severity="error")
    except Exception as e_unexp:
        loguru_logger.opt(exception=True).error(
            f"Unexpected error loading character: {e_unexp}"
        )
        app.notify("Unexpected error during character load.", severity="error")


async def handle_chat_character_attribute_changed(
    app: "TldwCli", event: Union[Input.Changed, TextArea.Changed]
) -> None:
    ...
    if isinstance(event, Input.Changed):
        ...
        loguru_logger.warning(
            f"Unhandled event type in handle_chat_character_attribute_changed: {type(event)}"
        )
    ...
    if control_id in field_map:
        ...
        if app.current_chat_active_character_data is not None:
            ...
            app.current_chat_active_character_data = (
                updated_data  # This updates the reactive variable
            )
            loguru_logger.debug(
                f"Temporarily updated active character attribute '{attribute_key}' to: '{str(new_value)[:50]}...'"
            )
            ...
            if attribute_key == "system_prompt":
                try:
                    ...
                    main_system_prompt_ta = app.query_one(
                        "#chat-system-prompt", TextArea
                    )
                    main_system_prompt_ta.text = new_value
                    loguru_logger.debug(
                        "Updated main system prompt in left sidebar from character edit."
                    )
                except QueryError:
                    loguru_logger.error(
                        "Could not find #chat-system-prompt to update from character edit."
                    )
    else:
        loguru_logger.warning(
            f"Attribute change event from unmapped control_id: {control_id}"
        )


async def handle_chat_clear_active_character_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        app.query_one("#chat-system-prompt", TextArea).text = default_system_prompt
        loguru_logger.debug(
            "Reset main system prompt to default on clear active character."
        )
    except QueryError:
        loguru_logger.error(
...
async def handle_chat_prompt_search_input_changed(
    ...
    if not app.prompts_service_initialized:
        ...
        try:
            results_list_view = app.query_one(
                "#chat-prompt-search-results-listview", ListView
            )
            await results_list_view.clear()
            await results_list_view.append(
                ListItem(Label("Prompts service unavailable."))
            )
    ...
    if not search_term:  # Clear list if search term is empty
        try:
            ...
            await results_list_view.clear()
            logger.debug(
                "Chat Tab: Cleared prompt search results as search term is empty."
            )
        except Exception as e_ui_clear:
            logger.error(
                f"Chat Tab: Error clearing prompt search listview: {e_ui_clear}"
            )
        return
    ...
    try:
        ...
        if prompt_results:
            ...
            logger.info(
                f"Chat Tab: Prompt search for '{search_term}' yielded {len(prompt_results)} results."
            )
        else:
            await results_list_view.append(ListItem(Label("No prompts found.")))
            logger.info(
                f"Chat Tab: Prompt search for '{search_term}' found no results."
            )

    except prompts_interop.DatabaseError as e_db:
        logger.opt(exception=True).error(
            f"Chat Tab: Database error during prompt search: {e_db}"
        )
        ...
        logger.opt(exception=True).error(
            f"Chat Tab: Unexpected error during prompt search: {e}"
        )
        try:  # Attempt to update UI with error
            results_list_view = app.query_one(
                "#chat-prompt-search-results-listview", ListView
...
async def perform_chat_prompt_search(app: "TldwCli") -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    try:
        search_input_widget = app.query_one(
            "#chat-prompt-search-input", Input
        )  # Ensure Input is imported where this is called
        await handle_chat_prompt_search_input_changed(app, search_input_widget.value)
    except Exception as e:
        logger.opt(exception=True).error(
            f"Chat Tab: Error performing prompt search via perform_chat_prompt_search: {e}"
        )


async def handle_chat_view_selected_prompt_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    try:
        ...
        identifier_to_fetch = (
            prompt_id_to_load if prompt_id_to_load is not None else prompt_uuid_to_load
        )

        if identifier_to_fetch is None:
            app.notify(
                "Selected prompt item is invalid (missing ID/UUID).", severity="error"
            )
            logger.error("Chat Tab: Selected prompt item missing ID and UUID.")
            return

        logger.debug(
            f"Chat Tab: Fetching details for prompt identifier: {identifier_to_fetch}"
        )
        ...
        if prompt_details:
            ...
            app.notify(
                f"Prompt '{prompt_details.get('name', 'Selected')}' loaded for viewing.",
                severity="information",
            )
            logger.info(
                f"Chat Tab: Displayed prompt '{prompt_details.get('name', 'Unknown')}' for viewing."
            )
        else:
            ...
            app.notify(
                "Failed to load details for the selected prompt.", severity="error"
            )
            logger.error(
                f"Chat Tab: Failed to fetch details for prompt identifier: {identifier_to_fetch}"
            )

    except prompts_interop.DatabaseError as e_db:
        logger.opt(exception=True).error(
            f"Chat Tab: Database error viewing selected prompt: {e_db}"
        )
        app.notify("Database error loading prompt.", severity="error")
    except Exception as e:
        logger.opt(exception=True).error(
            f"Chat Tab: Unexpected error viewing selected prompt: {e}"
        )
...
async def _populate_chat_character_search_list(
    app: "TldwCli", search_term: Optional[str] = None
) -> None:
    try:
        # Try to find the ListView in the current screen/chat window context
        try:
        ...
        if not app.notes_service:
            app.notify("Database service not available.", severity="error")
            loguru_logger.error(
                "Notes service not available for character list population."
            )
            await results_list_view.append(
                ListItem(Label("Error: DB service unavailable."))
            )
            return
        ...
        try:
            if search_term:
                ...
                loguru_logger.debug(
                    f"Populating character list by searching for: '{search_term}'"
                )
                characters = db.search_character_cards(
                    search_term=search_term, limit=50
                )
            else:
                loguru_logger.debug(
                    "Populating character list with default list (limit 40)."
                )
            ...
            if not characters:
                ...
                for char_data in characters:
                    ...
                    await results_list_view.append(item)
            loguru_logger.info(
                f"Character list populated using {operation_type}. Found {len(characters)} characters."
            )

        except Exception as e_db_call:
            loguru_logger.opt(exception=True).error(
                f"Error during DB call ({operation_type}): {e_db_call}"
            )
            await results_list_view.append(
                ListItem(Label(f"Error during {operation_type}."))
            )

    except QueryError as e_query:
        loguru_logger.opt(exception=True).error(
            f"UI component not found for character list population: {e_query}"
        )
        ...
        loguru_logger.opt(exception=True).error(
            f"Unexpected error in _populate_chat_character_search_list: {e_unexp}"
        )
        # Avoid app.notify here as well.


async def handle_chat_copy_system_prompt_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
...
async def handle_chat_copy_user_prompt_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
...
async def handle_chat_template_search_input_changed(
    app: "TldwCli", event_value: str
) -> None:
    ...
    try:
        ...
        if filtered_templates:
            ...
            logger.info(
                f"Chat Tab: Template search for '{search_term}' yielded {len(filtered_templates)} results."
            )
        else:
            await template_list_view.append(ListItem(Label("No matching templates.")))
            logger.info(
                f"Chat Tab: Template search for '{search_term}' found no results."
            )
...
async def handle_chat_apply_template_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        if (
            chat_input_widget.text.strip()
            and template.user_message_content_template != "{message_content}"
        ):
            ...
            chat_input_widget.text = template.user_message_content_template.replace(
...
async def handle_chat_sidebar_prompt_search_changed(
    ...
    )  # fall back to module-level loguru logger if unavailable
    ...
    # === A term is present → Run a full search. ===
        ...
        try:
            ...
            await results_view.append(
                ListItem(Label("Database error while searching."))
            )
            return
        except Exception as ex:
            logger.opt(exception=True).error(
                f"[Prompts] Unknown error during search: {ex}"
            )
...
async def handle_continue_response_button_pressed(
    ...
    message_widget: Union[ChatMessage, ChatMessageEnhanced],
) -> None:
    """Handles the 'Continue Response' button press on an AI chat message."""
    loguru_logger.info(
        f"Continue Response button pressed for message_id: {message_widget.message_id_internal}, current text: '{message_widget.message_text[:50]}...'"
    )
    prefix = (
        "chat"  # Assuming 'chat' is the prefix for UI elements in the main chat window
    )

    continue_button_widget: Optional[Button] = None
    original_button_label: Optional[str] = None
    markdown_widget: Optional[Markdown] = None
    original_display_text_obj: Optional[Union[str, Text]] = (
        None  # renderable can be str or Text
    )
    ...
    try:
        ...
        continue_button_widget.label = get_char(
            EMOJI_THINKING, FALLBACK_THINKING
        )  # "⏳" or similar

        markdown_widget = message_widget.query_one(".message-text", Markdown)
        original_display_text_obj = (
            message_widget.message_text
        )  # Save the original text
    except QueryError as qe:
        loguru_logger.opt(exception=True).error(
            f"Error querying essential UI component for continuation: {qe}"
        )
        app.notify(
            "Error initializing continuation: UI component missing.", severity="error"
        )
        if (
            continue_button_widget and original_button_label
        ):  # Attempt to restore button if found
        ...
        loguru_logger.opt(exception=True).error(
            f"Unexpected error during continue response initialization: {e_init}"
        )
    ...
    try:
        ...
        all_messages = list(chat_log.query(ChatMessage)) + list(
            chat_log.query(ChatMessageEnhanced)
        )
        all_messages_in_log = sorted(
            all_messages, key=lambda w: chat_log.children.index(w)
        )
        ...
        for msg_w in all_messages_in_log:
            ...
            if (
                msg_w.generation_complete or msg_w is message_widget
            ):  # Include incomplete target message
        ...
        if not any(
            msg_info["content"] == original_message_text
            and msg_info["role"] == "assistant"
            for msg_info in history_for_api
        ):
            loguru_logger.warning(
                "Target message for continuation not found in constructed history. This is unexpected."
            )
            # This might indicate an issue with message_widget identity or history construction logic.

        loguru_logger.debug(
            f"Built history for API continuation with {len(history_for_api)} messages. Last message is the one to continue."
        )

    except QueryError as e:
        loguru_logger.opt(exception=True).error(
            f"Continue Response: Could not find UI elements for history: {e}"
        )
        ...
        loguru_logger.opt(exception=True).error(
            f"Error building history for continuation: {e_hist}"
        )
        app.notify(
            "Error preparing message history for continuation.", severity="error"
        )
        if continue_button_widget:
            continue_button_widget.disabled = False
            continue_button_widget.label = original_button_label
    ...
    try:
        ...
        loguru_logger.opt(exception=True).warning(
            f"Could not update message with thinking indicator: {e_indicator}"
        )
    ...
    try:
        ...
        system_prompt_widget = app.query_one(
            f"#{prefix}-system-prompt", TextArea
        )  # Main system prompt from left sidebar
        ...
        llm_response_format_widget = app.query_one(
            f"#{prefix}-llm-response-format", Select
        )
        llm_n_widget = app.query_one(f"#{prefix}-llm-n", Input)
        llm_user_identifier_widget = app.query_one(
            f"#{prefix}-llm-user-identifier", Input
        )
        ...
        llm_presence_penalty_widget = app.query_one(
            f"#{prefix}-llm-presence-penalty", Input
        )
        llm_frequency_penalty_widget = app.query_one(
            f"#{prefix}-llm-frequency-penalty", Input
        )
        ...
        llm_fixed_tokens_kobold_widget = app.query_one(
            f"#{prefix}-llm-fixed-tokens-kobold", Checkbox
        )
    except QueryError as e:
        loguru_logger.opt(exception=True).error(
            f"Continue Response: Could not find UI settings widgets for '{prefix}': {e}"
        )
        app.notify("Error: Missing UI settings for continuation.", severity="error")
        if markdown_widget:
            markdown_widget.update(original_display_text_obj)  # Restore original text
        if continue_button_widget:
            continue_button_widget.disabled = False
            continue_button_widget.label = original_button_label
        return
    ...
    try:
        llm_logit_bias_text = llm_logit_bias_widget.text.strip()
        llm_logit_bias_value = (
            json.loads(llm_logit_bias_text)
            if llm_logit_bias_text and llm_logit_bias_text != "{}"
            else None
        )
    except json.JSONDecodeError:
        llm_logit_bias_value = None
        loguru_logger.warning("Invalid JSON in llm_logit_bias for continuation.")
    try:
        llm_tools_text = llm_tools_widget.text.strip()
        llm_tools_value = (
            json.loads(llm_tools_text)
            if llm_tools_text and llm_tools_text != "[]"
            else None
        )
    except json.JSONDecodeError:
        llm_tools_value = None
        loguru_logger.warning("Invalid JSON in llm_tools for continuation.")

    # System Prompt (Active Character > UI)
    final_system_prompt_for_api = (
        system_prompt_widget.text
    )  # Default to UI's system prompt
    if app.current_chat_active_character_data:
        char_specific_system_prompt = app.current_chat_active_character_data.get(
            "system_prompt"
        )
        if char_specific_system_prompt and char_specific_system_prompt.strip():
            final_system_prompt_for_api = char_specific_system_prompt
            loguru_logger.debug(
                "Using active character's system prompt for continuation."
            )
        else:
            loguru_logger.debug(
                "Active character has no system_prompt; using UI system prompt for continuation."
            )
    else:
        loguru_logger.debug(
            "No active character; using UI system prompt for continuation."
        )
    ...
    if selected_provider:  # Log provider's normal streaming setting for info
        ...
        loguru_logger.debug(
            f"Provider {selected_provider} normally streams: {provider_specific_settings.get('streaming', False)}. Default stream for continuation."
        )

    # Check streaming checkbox to override even for continuation
    try:
        streaming_checkbox_cont = app.screen.query_one(
            "#chat-streaming-enabled-checkbox", Checkbox
        )
        streaming_override_cont = streaming_checkbox_cont.value
        if not streaming_override_cont:
            loguru_logger.info(
                "Streaming override for CONTINUATION: checkbox=False, overriding default continuation streaming"
            )
            should_stream = False
    except QueryError:
        loguru_logger.debug(
            "Streaming checkbox not found for CONTINUATION, using default streaming=True"
        )
    ...
    if selected_provider:
        provider_settings_key = selected_provider.lower().replace(" ", "_")
        provider_config = app.app_config.get("api_settings", {}).get(
            provider_settings_key, {}
        )
        if (
            "api_key" in provider_config
            and provider_config["api_key"]
            and provider_config["api_key"] != "<API_KEY_HERE>"
        ):
            api_key_for_call = provider_config["api_key"]
        elif (
            "api_key_env_var" in provider_config and provider_config["api_key_env_var"]
        ):
    ...
    if selected_provider in providers_requiring_key and not api_key_for_call:
        loguru_logger.error(
            f"API Key for '{selected_provider}' is missing for continuation."
        )
    ...
    other_action_buttons_ids = [
        "thumb-up",
        "thumb-down",
        "regenerate",
    ]  # Add other relevant button IDs
    ...
    try:
        ...
        loguru_logger.warning(
            f"Could not find or disable one or more action buttons during continuation: {qe}"
        )
    ...
    try:
        ...
        app.set_current_chat_worker(worker)
        loguru_logger.info(
            f"Continue worker started for message_id: {message_widget.message_id_internal}"
        )
        ...
        loguru_logger.opt(exception=True).error(
            f"Error starting continue worker: {e_worker}"
        )
        app.notify(
            f"Failed to start continuation: {str(e_worker)[:100]}", severity="error"
        )

        # Restore original state on error
        message_widget.message_text = original_message_text  # Restore internal text
        if markdown_widget:
            markdown_widget.update(original_display_text_obj)  # Restore display
        if continue_button_widget:
            continue_button_widget.disabled = False
            continue_button_widget.label = original_button_label
...
async def handle_respond_for_me_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        try:
            ...
            if not all_messages_in_log:
                app.notify(
                    "Cannot generate suggestion: Chat history is empty.",
                    severity="warning",
                    timeout=4,
                )
            ...
            for msg_w in all_messages_in_log:
                ...
                if msg_w.generation_complete:  # Only include completed messages
                    history_for_api.append(
                        {"role": api_role, "content": msg_w.message_text}
                    )

            loguru_logger.debug(
                f"Built history for suggestion API with {len(history_for_api)} messages."
            )

        except QueryError as e_hist_query:
            loguru_logger.opt(exception=True).error(
                f"Respond for Me: Could not find UI elements for history: {e_hist_query}"
            )
            ...
            loguru_logger.opt(exception=True).error(
                f"Error building history for suggestion: {e_hist_build}"
            )
            app.notify(
                "Error preparing message history for suggestion.", severity="error"
            )
            raise  # Re-raise to go to finally
        ...
        conversation_history_str = "\n".join(
            [f"{item['role']}: {item['content']}" for item in history_for_api]
        )
        ...
        try:
            ...
            llm_logprobs_widget = app.query_one(f"#{prefix}-llm-logprobs", Checkbox)
            llm_top_logprobs_widget = app.query_one(
                f"#{prefix}-llm-top-logprobs", Input
            )
            llm_logit_bias_widget = app.query_one(f"#{prefix}-llm-logit-bias", TextArea)
            llm_presence_penalty_widget = app.query_one(
            ...
            try:
                strip_tags_checkbox_suggest = app.query_one(
                    "#chat-strip-thinking-tags-checkbox", Checkbox
                )
                strip_thinking_tags_value_suggest = strip_tags_checkbox_suggest.value
            except QueryError:
                loguru_logger.warning(
                    "Respond for Me: Could not find '#chat-strip-thinking-tags-checkbox'. Defaulting to True."
                )
                strip_thinking_tags_value_suggest = True
        except QueryError as e_params_query:
            loguru_logger.opt(exception=True).error(
                f"Respond for Me: Could not find UI settings widgets: {e_params_query}"
            )
        ...
        min_p = safe_float(min_p_widget.value, 0.05, "min_p")
        top_k = safe_int(top_k_widget.value, 50, "top_k")
        llm_max_tokens_value = safe_int(
            llm_max_tokens_widget.value, 200, "llm_max_tokens_suggestion"
        ...
        try:
            ...
            llm_logit_bias_value = None
            loguru_logger.warning("Invalid JSON in llm_logit_bias for suggestion.")
        try:
            llm_tools_text = llm_tools_widget.text.strip()
            llm_tools_value = (
                json.loads(llm_tools_text)
            ...
            llm_tools_value = None
            loguru_logger.warning("Invalid JSON in llm_tools for suggestion.")
        ...
        if selected_provider:
            ...
            if (
                ...
                "api_key_env_var" in provider_config
                and provider_config["api_key_env_var"]
            ):
                api_key_for_call = os.environ.get(provider_config["api_key_env_var"])
        ...
        if selected_provider in providers_requiring_key and not api_key_for_call:
            loguru_logger.error(
                f"API Key for '{selected_provider}' is missing for suggestion."
            )
            app.notify(f"API Key for {selected_provider} is missing.", severity="error")
            raise ApiKeyMissingError(
                f"API Key for {selected_provider} required."
            )  # Custom exception to catch in finally
        ...
        loguru_logger.debug(
            f"Suggestion prompt instruction: {suggestion_prompt_instruction[:500]}..."
        )
        loguru_logger.debug(
            f"Suggestion params: provider='{selected_provider}', model='{selected_model}', system_prompt (for suggestion)='{final_system_prompt_for_api[:100]}...'"
        )

        loguru_logger.info(
            "Respond for Me worker dispatched. Waiting for suggestion..."
        )
        ...
        ValueError
    ) as e_val:  # Catch specific ValueErrors like empty history or bad LLM response
        ...
        loguru_logger.opt(exception=True).error(
            f"Failed to generate suggestion: {e_main}"
        )
        app.notify(
            f"Failed to generate suggestion: {str(e_main)[:100]}",
            severity="error",
            timeout=5,
        )
    finally:
...
class ApiKeyMissingError(
    Exception
):  # Custom exception for cleaner handling in try/finally
    pass


async def handle_stop_chat_generation_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    if app.current_chat_worker and app.current_chat_worker.is_running:
        try:
            ...
            loguru_logger.info(
                f"Cancellation requested for worker: {app.current_chat_worker.name}"
            )

            if not app.current_chat_is_streaming:
                loguru_logger.debug(
                    "Handling cancellation for a non-streaming chat request."
                )
                if (
                    app.current_ai_message_widget
                    ...
                    try:
                        ...
                        markdown_widget = app.current_ai_message_widget.query_one(
                            ".message-text", Markdown
                        )
                        cancelled_text = "_Chat generation cancelled by user._"
                        markdown_widget.update(cancelled_text)

                        app.current_ai_message_widget.message_text = (
                            "Chat generation cancelled by user."  # Update raw text
                        )
                        app.current_ai_message_widget.role = "System"  # Change role

                        # Update header if it exists
                        try:
                            header_label = app.current_ai_message_widget.query_one(
                                ".message-header", Label
                            )
                            header_label.update("System Message")
                        except QueryError:
                            loguru_logger.warning(
                                "Could not find .message-header to update for non-streaming cancellation."
                            )

                        app.current_ai_message_widget.mark_generation_complete()  # Finalize UI state
                        loguru_logger.info(
                            "Non-streaming AI message widget UI updated for cancellation."
                        )
                    except QueryError as qe_widget_update:
                        loguru_logger.opt(exception=True).error(
                            f"Error updating non-streaming AI message widget UI on cancellation: {qe_widget_update}"
                        )
                else:
                    loguru_logger.warning(
                        "Non-streaming cancellation: current_ai_message_widget not found or not mounted."
                    )
            else:  # It was a streaming request
                loguru_logger.info(
                    "Cancellation for a streaming chat request initiated. Worker will handle stream termination."
                )
            ...
            loguru_logger.opt(exception=True).error(
                f"Error during worker cancellation or UI update: {e_cancel}"
            )
        ...
        if not app.current_chat_worker:
            ...
            loguru_logger.debug(
                f"current_chat_worker ({app.current_chat_worker.name}) is not running (state: {app.current_chat_worker.state})."
            )
...
async def populate_chat_conversation_character_filter_select(app: "TldwCli") -> None:
    ...
    logging.info(
        "Attempting to populate #chat-conversation-search-character-filter-select."
    )
    if not app.notes_service:
        ...
        try:
            char_filter_select_err = app.query_one(
                "#chat-conversation-search-character-filter-select", Select
            )
    ...
    try:
        ...
        char_filter_select.set_options(
            options if options else [("No characters", Select.BLANK)]
        )
        # Default to BLANK, user must explicitly choose or use "All Characters" checkbox
        char_filter_select.value = Select.BLANK
        logging.info(
            f"Populated #chat-conversation-search-character-filter-select with {len(options)} chars."
        )
    except QueryError as e_q:
        logging.error(
            f"Failed to find #chat-conversation-search-character-filter-select: {e_q}",
            exc_info=True,
        )
    except CharactersRAGDBError as e_db:  # Catch specific DB error
        logging.error(
            f"DB error populating char filter select (Chat Tab): {e_db}", exc_info=True
        )
    except Exception as e_unexp:
        logging.error(
            f"Unexpected error populating char filter select (Chat Tab): {e_unexp}",
            exc_info=True,
        )
...
async def generate_document_with_llm(
    ...
    try:
        ...
        if not all([provider, model, api_key, conversation_id]):
            app.notify(
                "Missing required information for document generation", severity="error"
            )
            return

        # Show loading notification
        app.notify(
            f"Generating {document_type.replace('_', ' ').title()}...",
            severity="information",
        )

        # Initialize document generator
        doc_generator = DocumentGenerator(
            db_path=app.chachanotes_db_path, client_id=app.client_id
        )
        ...
        conversation_name = (
            getattr(app, "current_conversation_name", None)
            or f"Chat-{conversation_id[:8]}"
        )
        ...
        if doc_generator.copy_to_clipboard(generated_content):
            app.notify(
                f"{document_type.replace('_', ' ').title()} created and copied to clipboard",
                severity="success",
                timeout=5,
            )
        else:
            app.notify(
                f"{document_type.replace('_', ' ').title()} created (clipboard copy failed)",
                severity="warning",
                timeout=5,
            )
        ...
        loguru_logger.opt(exception=True).error(
            f"Error generating {document_type}: {e}"
        )
