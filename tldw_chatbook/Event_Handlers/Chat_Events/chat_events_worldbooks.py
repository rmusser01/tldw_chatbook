...
async def handle_worldbook_search_input(app: "TldwCli", search_term: str) -> None:
    ...
    try:
        ...
        if search_term.strip():
            ...
            filtered_books = [
                ...
                or (
                    ...
                    and search_lower in book["description"].lower()
                )
            ]
        else:
        ...
        loguru_logger.opt(exception=True).error(
            f"Exception during world book search: {e}"
        )
...
async def handle_worldbook_add_button(app: "TldwCli") -> None:
    ...
    try:
        ...
        if not conv_id:
            app.notify(
                "No active conversation. Please start or load a conversation first.",
                severity="warning",
            )
            return
        ...
        if success:
            app.notify(
                f"Added '{worldbook_data['name']}' to conversation",
...
async def handle_worldbook_remove_button(app: "TldwCli") -> None:
    ...
    try:
        ...
        success = wb_manager.disassociate_world_book_from_conversation(
            conversation_id=conv_id, world_book_id=worldbook_data["id"]
        )

        if success:
            app.notify(
                f"Removed '{worldbook_data['name']}' from conversation",
                severity="information",
            )
            # Refresh the active world books list
            await refresh_active_worldbooks(app)
            ...
            app.notify(
                "Failed to remove world book from conversation", severity="error"
            )
...
async def refresh_active_worldbooks(app: "TldwCli") -> None:
    ...
    try:
        ...
        try:
            active_list = app.screen.query_one(
        ...
        if not active_books:
            ...
            for book in active_books:
                ...
                display_label = (
                    f"{status} {book['name']} (Priority: {book['priority']})"
                )
        ...
        loguru_logger.opt(exception=True).error(
            f"Error refreshing active world books: {e}"
        )
...
async def handle_worldbook_selection(app: "TldwCli", list_view_id: str) -> None:
    ...
    try:
        ...
        if list_view_id == "chat-worldbook-available-listview":
            add_button.disabled = selected_item is None or not hasattr(
                selected_item, "worldbook_data"
            )
            remove_button.disabled = True
        else:  # active list
            add_button.disabled = True
            remove_button.disabled = selected_item is None or not hasattr(
                selected_item, "worldbook_data"
            )
        ...
        details_text += (
            f"  Enabled: {'Yes' if worldbook_data.get('enabled', True) else 'No'}\n"
        )
        ...
        loguru_logger.opt(exception=True).error(
            f"Error displaying world book details: {e}"
        )
...
async def handle_worldbook_enable_checkbox(app: "TldwCli", enabled: bool) -> None:
    ...
    try:
        ...
        loguru_logger.opt(exception=True).error(
            f"Error updating world info setting: {e}"
        )
