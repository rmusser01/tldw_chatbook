...
#
########################################################################################################################
async def handle_api_call_worker_state_changed(
    app: "TldwCli", event: Worker.StateChanged
) -> None:
    ...
    log_counter(
        "chat_worker_state_change",
        labels={"worker_name": worker_name, "state": str(worker_state.name)},
    )
    ...
    if worker_name == "respond_for_me_worker":
        logger.info(
            f"Handling state change for 'respond_for_me_worker'. State: {worker_state}"
        )
        if worker_state is WorkerState.SUCCESS:
            suggestion_result = event.worker.result
            logger.debug(
                f"Respond_for_me_worker raw result: {str(suggestion_result)[:1000]}"
            )
            suggested_text = ""
            if isinstance(suggestion_result, dict) and suggestion_result.get("choices"):
                try:
                    suggested_text = (
                        suggestion_result["choices"][0]
                        .get("message", {})
                        .get("content", "")
                    )
                except (IndexError, KeyError, AttributeError):
                    logger.error(
                        f"Error parsing suggestion_result dict: {suggestion_result}"
                    )
                    suggested_text = ""
            ...
            logger.debug(
                f"Respond_for_me_worker initial extracted suggested_text: '{suggested_text[:500]}...'"
            )
            ...
            if suggested_text:
                ...
                common_fillers = [
                    ...
                    "Here's a suggestion for your response:",
                ]
                for filler in common_fillers:
                    if cleaned_suggested_text.lower().startswith(filler.lower()):
                        cleaned_suggested_text = cleaned_suggested_text[
                            len(filler) :
                        ].strip()
                if (
                    cleaned_suggested_text.startswith('"')
                    and cleaned_suggested_text.endswith('"')
                ) or (
                    cleaned_suggested_text.startswith("'")
                    and cleaned_suggested_text.endswith("'")
                ):
                ...
                try:
                    ...
                    chat_input_widget = current_screen.query_one(
                        "#chat-input", TextArea
                    )
                    chat_input_widget.text = cleaned_suggested_text
                    chat_input_widget.focus()
                    app.notify(
                        "Suggestion populated in the input field.",
                        severity="information",
                        timeout=3,
                    )
                    logger.info(
                        f"Suggestion populated from worker: '{cleaned_suggested_text[:100]}...'"
                    )
                except QueryError:
                    logger.error(
                        "Respond_for_me_worker: Failed to query #chat-input to populate suggestion."
                    )
                    app.notify(
                        "Error populating suggestion (UI element missing).",
                        severity="error",
                    )
            else:
                logger.warning(
                    "'respond_for_me_worker' succeeded but returned empty suggestion."
                )
                app.notify("AI returned an empty suggestion.", severity="warning")

        elif worker_state is WorkerState.ERROR:
            logger.opt(exception=event.worker.error).error(
                f"Worker 'respond_for_me_worker' failed: {event.worker.error}"
            )
            app.notify(
                f"Failed to generate suggestion: {str(event.worker.error)[:100]}",
                severity="error",
                timeout=5,
            )
        ...
        logger.info(
            f"Handling state change for API_Call worker: '{worker_name}'. State: {worker_state}"
        )
        ...
        try:
            ...
            if prefix == "chat" and get_cli_setting(
            ...
            markdown_widget_in_ai_msg = ai_message_widget.query_one(
            ...
            if worker_state is WorkerState.SUCCESS:
                ...
                if (
                    not app.current_chat_is_ephemeral
                    and app.current_chat_conversation_id
                ):
                    if app.chachanotes_db:
                        conv_char_name = ccl.get_character_name_for_conversation(
                            app.chachanotes_db, app.current_chat_conversation_id
                        )
                        if conv_char_name:
                            ai_sender_name_for_db = conv_char_name
                elif app.current_chat_active_character_data:
                    ai_sender_name_for_db = app.current_chat_active_character_data.get(
                        "name", "AI"
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "Could not update AI message header label with character name."
                    )

                current_thinking_text_pattern = (
                    f"AI {get_char(EMOJI_THINKING, FALLBACK_THINKING)}"
                )
                current_thinking_fallback_pattern = FALLBACK_THINKING
                current_message_text_strip = ai_message_widget.message_text.strip()

                if (
                    current_message_text_strip.startswith(current_thinking_text_pattern)
                    or current_message_text_strip.endswith(
                        current_thinking_fallback_pattern
                    )
                    or current_message_text_strip == current_thinking_fallback_pattern
                ):
                    logger.debug(
                        f"Found thinking placeholder: '{current_message_text_strip}' - not clearing to prevent flicker"
                    )
                ...
                if is_streaming_handled_by_events:
                    ...
                    if (
                        isinstance(worker_result_content, dict)
                        and "choices" in worker_result_content
                    ):
                        try:
                            ...
                            if num_choices > 1:
                                logger.info(
                                    f"Received {num_choices} response choices from API"
                                )
                                ...
                                if "reasoning_content" in first_choice["message"]:
                                    reasoning_content = first_choice["message"][
                                        "reasoning_content"
                                    ]
                                ...
                                if reasoning_content and hasattr(app, "app_config"):
                                    strip_tags_setting = app.app_config.get(
                                        "chat_defaults", {}
                                ...
                                for i, choice in enumerate(choices):
                                    ...
                                    if "reasoning_content" in choice["message"]:
                                        variant_reasoning = choice["message"][
                                            "reasoning_content"
                                        ]
                                ...
                                logger.info(
                                    f"Stored {len(app._pending_response_variants)} variants for creation"
                                )
                                ...
                                reasoning_content = None
                                logger.info(
                                    f"DeepSeek response structure: {json.dumps(choices[0], indent=2)[:500]}..."
                                )
                                if "reasoning_content" in choices[0]["message"]:
                                    reasoning_content = choices[0]["message"][
                                        "reasoning_content"
                                    ]
                                    logger.info(
                                        f"Found reasoning_content in message: {reasoning_content[:200]}..."
                                    )
                                elif "reasoning_content" in choices[0]:
                                    reasoning_content = choices[0]["reasoning_content"]
                                    logger.info(
                                        f"Found reasoning_content in choice: {reasoning_content[:200]}..."
                                    )
                            ...
                            original_text_for_storage = (
                                "[AI: Error parsing successful response structure.]"
                            )
                            Text.from_markup(
                    ...
                    current_markdown_text = getattr(
                        markdown_widget_in_ai_msg, "markup", ""
                    )
                    if (
                        f"AI {get_char(EMOJI_THINKING, FALLBACK_THINKING)}"
                        ...
                        markdown_widget_in_ai_msg.update(
                            ""
                        )  # Clear if it's still the placeholder
                    ...
                    if isinstance(worker_result_content, dict):
                        logger.debug(
                            "Checking for tool calls in non-streaming response"
                        )
                        tool_calls = parse_tool_calls_from_response(
                            worker_result_content
                        )
                        if tool_calls:
                            logger.info(
                                f"Detected {len(tool_calls)} tool call(s) in non-streaming response"
                            )
                            ...
                            if chat_container:
                                ...
                                try:
                                    ...
                                    logger.info(
                                        f"Tool execution completed with {len(results)} result(s)"
                                    )
                                    ...
                                    if (
                                        app.chachanotes_db
                                        and app.current_chat_conversation_id
                                        and not app.current_chat_is_ephemeral
                                    ):
                                        try:
                                            ...
                                            logger.debug(
                                                f"Saved tool call message to DB with ID: {tool_call_db_id}"
                                            )

                                            # Save tool results message
                                            tool_results_msg = f"Tool Results:\n{json.dumps(results, indent=2)}"
                                            tool_result_db_id = (
                                                ccl.add_message_to_conversation(
                                                    app.chachanotes_db,
                                                    app.current_chat_conversation_id,
                                                    "tool",
                                                    tool_results_msg,
                                            ...
                                            logger.debug(
                                                f"Saved tool results message to DB with ID: {tool_result_db_id}"
                                            )
                                        except Exception as e:
                                            logger.opt(exception=True).error(
                                                f"Error saving tool messages to DB: {e}"
                                            )

                                    # Import and use the streaming continue function
                                    from ..Event_Handlers.Chat_Events.chat_streaming_events import (
                                        _continue_conversation_with_tools,
                                    )

                                    await _continue_conversation_with_tools(
                                        app, results, tool_calls
                                    )
                                ...
                                logger.warning(
                                    "Could not find chat container for mounting tool widgets"
                                )
                    ...
                    if (
                        ...
                        try:
                            ...
                            if ai_msg_db_id_ns_version:
                                saved_ai_msg_details_ns = (
                                    app.chachanotes_db.get_message_by_id(
                                        ai_msg_db_id_ns_version
                                    )
                                )
                                if saved_ai_msg_details_ns:
                                    ai_message_widget.message_id_internal = (
                                        saved_ai_msg_details_ns.get("id")
                                    )
                                    ai_message_widget.message_version_internal = (
                                        saved_ai_msg_details_ns.get("version")
                                    )
                                    logger.debug(
                                    ...
                                    if (
                                        hasattr(app, "_pending_response_variants")
                                        and app._pending_response_variants
                                    ):
                                        logger.info(
                                            f"Creating {len(app._pending_response_variants)} response variants"
                                        )
                                        original_message_id = (
                                            saved_ai_msg_details_ns.get("id")
                                        )
                                        ...
                                        ai_message_widget.variant_of = (
                                            None  # First one is the original
                                        )
                                        ai_message_widget.variant_id = (
                                            original_message_id
                                        )
                                        ai_message_widget.variant_number = 1
                                        ai_message_widget.total_variants = len(
                                            app._pending_response_variants
                                        )
                                        ...
                                        chat_container = ai_message_widget.parent
                                        if chat_container:
                                            for i, variant_data in enumerate(
                                                app._pending_response_variants
                                            ):
                                                if i == 0:
                                                ...
                                                if variant_id:
                                                    ...
                                                    variant_widget.variant_of = (
                                                        original_message_id
                                                    )
                                                    variant_widget.variant_id = (
                                                        variant_id
                                                    )
                                                    variant_widget.variant_number = (
                                                        i + 1
                                                    )
                                                    variant_widget.total_variants = len(
                                                        app._pending_response_variants
                                                    )
                                                    ...
                                                    logger.info(
                                                        f"Created variant {i + 1} of {len(app._pending_response_variants)} with ID {variant_id}"
                                                    )

                                            # Update the first widget to show variant navigation
                                            if hasattr(
                                                ai_message_widget, "update_variant_info"
                                            ):
                                                ai_message_widget.update_variant_info(
                                                    1,
                                                    len(app._pending_response_variants),
                                                    True,
                                                )
                                    ...
                                    # Check if this is a regeneration - create variant relationship
                                        ...
                                        hasattr(app, "regenerating_message_widget")
                                        and app.regenerating_message_widget
                                    ):
                                        original_widget = (
                                            app.regenerating_message_widget
                                        )
                                        original_id = getattr(
                                            app, "regenerating_original_id", None
                                        )
                                        ...
                                        if original_id is None:
                                            ...
                                            logger.info(
                                                "Regenerate: Content replaced in original widget (ephemeral chat, non-streaming)"
                                            )
                                            ...
                                            try:
                                                ...
                                                existing_variants = app.chachanotes_db.get_message_variants(
                                                    original_id
                                                )
                                                variant_count = (
                                                    len(existing_variants) + 1
                                                )  # +1 for this new variant
                                                ...
                                                ai_message_widget.variant_number = (
                                                    variant_count
                                                )
                                                ai_message_widget.total_variants = (
                                                    variant_count
                                                )
                                                ai_message_widget.is_selected_variant = True
                                                ...
                                                if hasattr(
                                                    original_widget, "has_variants"
                                                ):
                                                    original_widget.has_variants = True
                                                    original_widget.variant_id = (
                                                        original_id
                                                    )
                                                    original_widget.variant_number = 1
                                                    original_widget.total_variants = (
                                                        variant_count
                                                    )
                                                    original_widget.is_selected_variant = False
                                                    original_widget.display = (
                                                        False  # Hide original
                                                    )
                                                    ...
                                                    if hasattr(
                                                        ...
                                                        original_widget.update_variant_info(
                                                            1, variant_count, False
                                                        )

                                                logger.info(
                                                    f"Created regeneration variant {variant_count} of original message {original_id}"
                                                )
                                                app.notify(
                                                ...
                                                logger.opt(exception=True).error(
                                                    f"Failed to create regeneration variant: {e}"
                                                )
                                ...
                                logger.error(
                                    "Failed to save non-streamed AI message to DB (no ID returned)."
                                )
                        except (CharactersRAGDBError, InputError) as e_save_ai_ns:
                            logger.opt(exception=True).error(
                                f"Failed to save non-streamed AI message to DB: {e_save_ai_ns}"
                            )
                ...
                try:
                    ...
                    logger.warning(
                        "Could not update AI message header label for worker error state."
                    )
                ...
                if (
                    app.current_chat_is_streaming
                ):  # Should be false if non-streaming worker failed
                    logger.info(
                    ...
                    app.post_message(
                        StreamDone(
                            full_text=ai_message_widget.message_text,
                            error=str(error_from_worker),
                            response_data=None,
                        )
                    )
                    ...
                    logger.info(
                        f"Worker '{worker_name}' failed (non-streaming). Clearing current_ai_message_widget."
                    )
            ...
            if app.current_ai_message_widget is None:
                ...
                if app.is_mounted:
                    try:
                        ...
                        logger.debug(
                            f"Could not focus input #{prefix}-input after API call, widget might not exist."
                        )
                    except Exception as e_final_focus:
                        logger.opt(exception=True).error(
                            f"Error focusing input after API call processing: {e_final_focus}"
                        )
            ...
            if (
                app.current_ai_message_widget
                and app.current_ai_message_widget.is_mounted
            ):
                try:
                    await app.current_ai_message_widget.remove()
                except Exception as e_remove_final:
                    logger.error(
                        f"Error removing AI widget during outer QueryError handling: {e_remove_final}"
                    )
            app.current_ai_message_widget = None
            ...
            if ai_message_widget and ai_message_widget.is_mounted:
                try:
                    markdown_widget_unexp_err = ai_message_widget.query_one(
                        ".message-text", Markdown
                    )
                    Text.from_markup(
                        f"[bold red]Internal error handling AI response:[/]\n{escape_markup(str(exc_outer))}"
                    )
                    markdown_widget_unexp_err.update(
                        f"**Internal error handling AI response:**\n{str(exc_outer)}"
                    )
                    ai_message_widget.role = "System"
                    try:
                        header_label = ai_message_widget.query_one(
        ...
        logger.debug(
            f"Ignoring worker state change for unhandled worker type: {worker_name}"
        )
