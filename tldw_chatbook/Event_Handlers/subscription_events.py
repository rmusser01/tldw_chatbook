...
class SubscriptionHealthUpdate(SubscriptionEvent):
    ...
    def __init__(
        ...
        paused_count: int,
        error_count: int,
        failing_subscriptions: List[Dict],
    ) -> None:
        super().__init__()
...
class BulkOperationComplete(SubscriptionEvent):
    ...
    def __init__(
        self, operation: str, item_count: int, success_count: int, error_count: int = 0
    ) -> None:
        super().__init__()
...
class SubscriptionImportComplete(SubscriptionEvent):
    ...
    def __init__(
        self, format: str, total: int, imported: int, errors: List[str] = None
    ) -> None:
        super().__init__()
...
async def handle_add_subscription(app: "TldwCli", event: Button.Pressed) -> None:
    ...
    try:
        ...
        # Get custom headers
        custom_headers = None
        headers_text = app.query_one(
            "#subscription-custom-headers", TextArea
        ).text.strip()
        if headers_text:
            try:
        ...
        ignore_selectors = app.query_one(
            "#subscription-ignore-selectors", TextArea
        ).text.strip()
        rate_limit = app.query_one("#subscription-rate-limit", Input).value
        auto_pause_threshold = app.query_one(
            "#subscription-auto-pause-threshold", Input
        ).value
        ...
        try:
            app.subscriptions_db.add_subscription(
                ...
                rate_limit_config={"requests_per_minute": int(rate_limit)}
                if rate_limit
                else None,
            ...
            app.notify(
                f"Successfully added subscription: {name_input.value}",
...
async def handle_check_all_subscriptions(app: "TldwCli", event: Button.Pressed) -> None:
    ...
    try:
        # Start check worker
        app.run_worker(
            check_all_subscriptions_worker, app, name="check_all_subscriptions"
        )
    except Exception as e:
...
async def handle_subscription_item_action(
    ...
    try:
        ...
        if action_name == "accept":
            ...
            # Ingest items
            try:
                success_count, error_count = await bulk_ingest_subscription_items(
                    app, selected_items
                )
            except Exception as e:
    ...
    try:
        ...
        if not pending:
            app.call_from_thread(
                app.notify, "No subscriptions due for checking", severity="information"
            )
            return
        ...
        # Update status
        app.call_from_thread(
            update_subscription_status, app, f"Checking {len(pending)} subscriptions..."
        )
        ...
        app.call_from_thread(
            app.notify, status_msg, severity="warning" if errors else "information"
        )
        ...
        app.call_from_thread(
            app.notify, f"Error checking subscriptions: {str(e)}", severity="error"
        )
    finally:
...
async def refresh_subscription_list(app: "TldwCli") -> None:
    ...
    try:
        ...
        subscriptions = app.subscriptions_db.get_all_subscriptions(
