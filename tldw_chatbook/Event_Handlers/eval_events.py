...
from tldw_chatbook.Widgets.Evals.eval_config_dialogs import (
    ModelConfigDialog,
    TaskConfigDialog,
    RunConfigDialog,
)
...
# File picker dialogs
from ..Widgets.file_picker_dialog import (
...
class EvaluationProgressAdapter:
    ...
    async def on_progress(
        self, progress: EvalProgress, sample_result: Optional[EvalSampleResult] = None
    ):
        ...
        try:
            ...
            if sample_result:
                # Update accuracy
                if sample_result.metrics and sample_result.metrics.get(
                    "correct", False
                ):
            ...
            accuracy = (
                (self.correct_count / self.sample_count * 100)
            ...
            sample_data = {
                ...
                if sample_result
                else f"Sample {progress.current}",
                ...
                "correct": sample_result.metrics.get("correct", False)
                if sample_result and sample_result.metrics
                else None,
            ...
            if hasattr(self.app_or_lab, "post_message_from_worker"):
                ...
                self.app_or_lab.post_message_from_worker(
                    self.app_or_lab.EvaluationProgress(
                        self.eval_id, progress.percentage, sample_data
                    )
                )
            else:
...
async def _process_task_upload(app: "TldwCli", file_path: str):
    ...
    try:
        app.notify(
            f"Processing task file: {Path(file_path).name}", severity="information"
        )
        ...
        log_histogram(
            "eval_ui_task_upload_duration", duration, labels={"status": "success"}
        )
        ...
        # Log task load error
        log_histogram(
            "eval_ui_task_upload_duration", duration, labels={"status": "error"}
        )
        log_counter("eval_ui_task_upload_error", labels={"error_type": "TaskLoadError"})
        ...
        log_counter(
            "eval_ui_task_upload_error", labels={"error_type": type(e).__name__}
        )
...
async def _create_custom_task(app: "TldwCli", config: Dict[str, Any]):
    ...
    try:
        ...
        duration = time.time() - start_time
        log_histogram(
            "eval_ui_custom_task_creation_duration",
            duration,
            labels={"task_type": config["task_type"], "status": "success"},
        )
        log_counter(
            "eval_ui_custom_task_created", labels={"task_type": config["task_type"]}
        )

        app.notify(
            f"Task created successfully: {config['name']}", severity="information"
        )
        ...
        log_counter(
            "eval_ui_custom_task_error", labels={"error_type": type(e).__name__}
        )
...
async def handle_template_selection(app: "TldwCli", template_name: str) -> None:
    ...
    log_counter("eval_ui_template_selected", labels={"template_name": template_name})

    try:
        app.notify(
            f"Creating task from template: {template_name}", severity="information"
        )
        ...
        log_counter(
            "eval_ui_template_task_created", labels={"template_name": template_name}
        )

        app.notify(
            f"Task created from template: {template_name}", severity="information"
        )
        ...
        # Log error
        log_counter(
            "eval_ui_template_task_error",
            labels={"template_name": template_name, "error_type": type(e).__name__},
        )
...
async def _save_model_config(app: "TldwCli", config: Dict[str, Any]):
    ...
    try:
        ...
        log_counter(
            "eval_ui_model_configured",
            labels={"provider": config["provider"], "model_id": config["model_id"]},
        )
        ...
        # Log error
        log_counter(
            "eval_ui_model_config_error", labels={"error_type": type(e).__name__}
        )
...
async def handle_provider_setup(app: "TldwCli", provider: str) -> None:
    ...
    # Log provider setup
    log_counter("eval_ui_provider_setup_clicked", labels={"provider": provider})
...
async def handle_start_evaluation(app: "TldwCli", event: Button.Pressed) -> None:
    ...
    if not tasks:
        log_counter("eval_ui_start_evaluation_blocked", labels={"reason": "no_tasks"})
        app.notify("No tasks available. Create a task first.", severity="error")
        return

    if not models:
        log_counter("eval_ui_start_evaluation_blocked", labels={"reason": "no_models"})
    ...
    dialog = RunConfigDialog(
        callback=on_config_ready, available_tasks=tasks, available_models=models
    )
    app.push_screen(dialog)
...
async def handle_view_detailed_results(app: "TldwCli", event) -> None:
    ...
    try:
        ...
        if runs:
            ...
            log_counter("eval_ui_detailed_results_viewed", labels={"run_id": run_id})
...
async def handle_export_results(app: "TldwCli", format_type: str) -> None:
    ...
    # Log export request
    log_counter("eval_ui_export_results_clicked", labels={"format_type": format_type})
    ...
    if not runs:
        log_counter("eval_ui_export_blocked", labels={"reason": "no_completed_runs"})
    ...
    def on_file_selected(file_path: Optional[str]):
        if file_path:
            asyncio.create_task(
                _export_run_results(app, run["id"], file_path, format_type)
            )
...
async def _export_run_results(
    app: "TldwCli", run_id: str, file_path: str, format_type: str
):
    ...
    try:
        ...
        duration = time.time() - start_time
        log_histogram(
            "eval_ui_export_duration",
            duration,
            labels={"format_type": format_type, "status": "success"},
        )
        log_counter(
            "eval_ui_results_exported",
            labels={"format_type": format_type, "run_id": run_id},
        )
        ...
        log_histogram(
            "eval_ui_export_duration",
            duration,
            labels={"format_type": format_type, "status": "error"},
        )
        log_counter(
            "eval_ui_export_error",
...
async def _export_to_csv(orchestrator, run_id: str, file_path: str):
    ...
    with open(file_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        csvfile.write(
            f"# Task: {run_summary['run_info'].get('task_name', 'Unknown')}\n"
        )
        csvfile.write(
            f"# Model: {run_summary['run_info'].get('model_name', 'Unknown')}\n"
        )
...
async def _auto_export_results(app: "TldwCli", run_id: str):
    ...
    try:
        ...
        app.notify(
            f"Results auto-exported to: {export_path.name}", severity="information"
        )
...
async def _process_dataset_upload(app: "TldwCli", file_path: str):
    ...
    try:
        app.notify(
            f"Processing dataset: {Path(file_path).name}", severity="information"
        )
        ...
        log_counter(
            "eval_ui_dataset_uploaded",
            labels={"file_extension": Path(file_path).suffix},
        )
        ...
        log_counter(
            "eval_ui_dataset_upload_error", labels={"error_type": type(e).__name__}
        )
...
async def handle_template_button(app: "TldwCli", event: Button.Pressed) -> None:
    ...
    log_counter("eval_ui_template_button_clicked", labels={"button_id": button_id})
...
async def handle_compare_runs(app: "TldwCli", event: Button.Pressed) -> None:
    ...
    if len(runs) < 2:
        log_counter(
            "eval_ui_compare_runs_blocked",
            labels={"reason": "insufficient_runs", "available_runs": str(len(runs))},
        )
    ...
    if len(runs) > 2:
        ...
        try:
            from tldw_chatbook.Widgets.Evals.eval_additional_dialogs import (
                RunSelectionDialog,
            )
...
async def _perform_comparison(app: "TldwCli", runs_to_compare: List[Dict[str, Any]]):
    ...
    try:
        ...
        log_counter("eval_ui_runs_compared", labels={"num_runs": str(len(run_ids))})
...
async def initialize_evals_system(app: "TldwCli") -> None:
    ...
    try:
        ...
        if not tasks:
            app.notify(
                "Evaluation system ready. Create a task to get started!",
                severity="information",
            )
        elif not models:
            app.notify(
                "Evaluation system ready. Add a model to start evaluating!",
                severity="information",
            )
        else:
            app.notify(
                f"Evaluation system ready. {len(tasks)} tasks, {len(models)} models available.",
                severity="information",
            )
        ...
        log_counter(
            "eval_ui_system_initialization_error",
            labels={"error_type": type(e).__name__},
        )

        app.notify(
            f"Failed to initialize evaluation system: {str(e)}", severity="error"
        )
    ...
    try:
        ...
        for dataset in datasets:
            if dataset.get("name") == dataset_name:
                ...
                dataset["format"] = (
                    Path(dataset.get("source_path", "")).suffix.lstrip(".") or "Unknown"
                )
...
async def upload_dataset(
    app: "TldwCli", source: str, source_type: str = "file"
) -> Dict[str, Any]:
...
async def export_dataset(
    app: "TldwCli", dataset_name: str, export_path: str, format: str = "json"
) -> Dict[str, Any]:
...
async def handle_cancel_evaluation(app: "TldwCli", run_id: Optional[str]) -> None:
    ...
    if not run_id:
        log_counter(
            "eval_ui_cancel_evaluation_blocked",
            labels={"reason": "no_active_evaluation"},
        )
    ...
    try:
        ...
        if success:
            ...
            log_counter("eval_ui_evaluation_cancel_failed", labels={"run_id": run_id})
        ...
        log_counter(
            "eval_ui_evaluation_cancel_error", labels={"error_type": type(e).__name__}
        )
...
def handle_evaluation_error(error: Exception, eval_id: str) -> Dict[str, Any]:
    ...
    # Log error
    log_counter(
        "eval_error", labels={"error_type": error_info["type"], "eval_id": eval_id}
    )
