...
class AIGenerationHandler(BaseWorkerHandler):
    ...
    async def _handle_success_state(
        ...
        self.logger.info(
            f"AI generation worker '{worker_info['name']}' completed successfully"
        )

        result = event.worker.result
        if not result or not isinstance(result, dict) or "choices" not in result:
            self.logger.warning(
                f"AI generation worker returned invalid result: {result}"
            )
        ...
        try:
            ...
            self.app.notify(
                "Failed to extract content from AI response", severity="error"
            )
            await self._re_enable_button(worker_info["name"])

        except Exception as e:
            self.logger.error(f"Error handling AI generation success: {e}")
            self.app.notify(f"Error updating UI: {str(e)[:100]}", severity="error")
            await self._re_enable_button(worker_info["name"])
    ...
    async def _handle_single_generation(self, worker_name: str, content: str) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Updated {config['field_name']} field with generated content"
            )
    ...
    async def _handle_generate_all(self, content: str) -> None:
        ...
        for worker_name, config in self.GENERATION_CONFIGS.items():
            ...
            if field_name in sections and sections[field_name]:
                try:
                    text_area = self.app.query_one(
                        f"#{config['textarea_id']}", TextArea
                    )
                    text_area.text = sections[field_name]
                    self.logger.debug(
                        f"Updated {field_name} from comprehensive generation"
                    )
                except QueryError as e:
        ...
        self.app.notify(
            "Character fields generated successfully", severity="information"
        )
    ...
    def _parse_comprehensive_response(self, content: str) -> Dict[str, str]:
        ...
        for line in content.split("\n"):
            ...
            for section_name in sections.keys():
                if section_name in line_clean_lower and any(
    ...
    async def _handle_error_state(
        ...
        error_msg = (
            str(event.worker.error)[:200] if event.worker.error else "Unknown error"
        )
        self.logger.error(
            f"AI generation worker '{worker_info['name']}' failed: {error_msg}"
        )
