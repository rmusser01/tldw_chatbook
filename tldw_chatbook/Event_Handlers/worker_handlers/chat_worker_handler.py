...
class ChatWorkerHandler(BaseWorkerHandler):
    ...
    def can_handle(self, worker_name: str, worker_group: Optional[str] = None) -> bool:
        ...
        return isinstance(worker_name, str) and (
            worker_name.startswith("API_Call_chat")
            or worker_name.startswith("API_Call_ccp")
            or worker_name == "respond_for_me_worker"
        )
    ...
    async def handle(self, event: Worker.StateChanged) -> None:
        ...
        if worker_info["state"] == WorkerState.RUNNING:
            await self._handle_running_state(
                send_button_id, get_char, EMOJI_STOP, FALLBACK_STOP
            )

        elif worker_info["state"] in [
            WorkerState.SUCCESS,
            WorkerState.ERROR,
            ...
            self.logger.debug(
                f"Chat-related worker '{worker_info['name']}' in other state: {worker_info['state']}"
            )
    ...
    async def _handle_running_state(
        ...
        try:
            ...
            self.logger.error(
                f"Could not find button '#{send_button_id}' to change it to stop state"
            )

    async def _handle_finished_state(
        self,
        ...
        try:
            ...
            self.logger.error(
                f"Could not find button '#{send_button_id}' to change it back to send state"
            )
        ...
        if (
            worker_info["name"].startswith("API_Call_chat")
            or worker_info["name"] == "respond_for_me_worker"
        ):
            ...
            if worker_info["name"].startswith("API_Call_chat"):
                ...
                self.logger.debug(
                    f"Reset current_chat_is_streaming to False after {worker_info['name']} finished"
                )

            self.logger.debug(
                f"Cleared current_chat_worker after {worker_info['name']} finished"
            )
    ...
    async def _handle_cancelled_state(self, worker_name: str) -> None:
        ...
        if (
            hasattr(self.app, "current_ai_message_widget")
            and self.app.current_ai_message_widget
            and not self.app.current_ai_message_widget.generation_complete
        ):
            self.logger.debug(
                "Finalizing AI message widget UI due to worker CANCELLED state"
            )

            try:
                markdown_widget = self.app.current_ai_message_widget.query_one(
                ...
                if (
                    ...
                    not in self.app.current_ai_message_widget.message_text
                ):
                ...
                self.logger.error(
                    f"Error updating AI message UI on CANCELLED state: {e}"
                )
