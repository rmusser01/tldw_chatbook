...
async def handle_tldw_api_worker_failure(app: "TldwCli", event: "Worker.StateChanged"):
    ...
    logger.opt(exception=True).error(
        f"TLDW API request worker failed for {media_type}: {error}"
    )

    try:
        loading_indicator = app.query_one(
            f"#tldw-api-loading-indicator-{media_type}", LoadingIndicator
        )
        ...
        logger.error(
            f"UI component not found in on_worker_failure for {media_type}: {e_ui}"
        )
        return
...
async def handle_tldw_api_worker_success(app: "TldwCli", event: "Worker.StateChanged"):
    ...
    try:
        ...
        logger.error(
            f"UI component not found in on_worker_success for {media_type}: {e_ui}"
        )
        return
    except Exception as e:
        logger.opt(exception=True).error(
            f"Error resetting UI state in worker success handler: {e}"
        )
        return
    ...
    if not request_model:
        logger.error(
            "Could not retrieve request_model from app context. Cannot properly ingest results."
        )
        status_area.load_text(
    ...
    elif isinstance(response_data, list) and all(
        isinstance(item, ProcessedMediaWikiPage) for item in response_data
    ):
        for mw_page in response_data:
            if mw_page.status == "Error":
                error_count += 1
                logger.error(
                    f"MediaWiki page '{mw_page.title}' processing error: {mw_page.error_message}"
                )
                continue
            # Adapt ProcessedMediaWikiPage to the common result structure
            results_to_ingest.append(
                MediaItemProcessResult(
                    status="Success",
            ...
            results_to_ingest.append(
                MediaItemProcessResult(
                    ...
                    metadata={
                        ...
                        "keywords": xml_item.keywords,
                    },
                    content=xml_item.content,
                    analysis=xml_item.summary,
                )
            )
    else:
        logger.error(
            f"Unexpected TLDW API response data type for {media_type}: {type(response_data)}."
        )
        status_area.load_text(
        ...
        if item_result.status == "Success":
            try:
                ...
                media_id, _, msg = app.media_db.add_media_with_keywords(
                    ...
                    keywords=item_result.metadata.get("keywords", [])
                    or request_model.keywords,
                ...
                if media_id:
                    logger.info(
                        f"Successfully ingested '{item_result.input_ref}' into local DB. Media ID: {media_id}. Msg: {msg}"
                    )
                    processed_count += 1
                    ...
                    logger.error(
                        f"Failed to ingest '{item_result.input_ref}' into local DB. Message: {msg}"
                    )
                    error_count += 1

            except Exception as e_ingest:
                logger.opt(exception=True).error(
                    f"Error ingesting item '{item_result.input_ref}' into local DB: {e_ingest}"
                )
                error_count += 1
        else:
            logger.error(
                f"API processing error for '{item_result.input_ref}': {item_result.error}"
            )
            error_count += 1
    ...
    if not results_to_ingest and error_count == 0:
        summary_parts.append(
            "API request successful, but no items were provided or found for processing.\n"
        )
    else:
        summary_parts.append(
            f"- **Successfully Processed & Ingested:** {processed_count}\n"
        )
        summary_parts.append(f"- **Errors (API or DB):** {error_count}\n\n")

    if error_count > 0:
        summary_parts.append(
            "**Please check the application logs for details on any errors.**\n\n"
        )
    ...
    if successful_ingestions_details:
        ...
        for detail in successful_ingestions_details[:10]:  # Show max 10 details
            title_str = (
                f" (Title: `{detail['title']}`)" if detail["title"] != "N/A" else ""
            )
            summary_parts.append(f"- **Input:** `{detail['input_ref']}`{title_str}\n")
            summary_parts.append(
                f"  - **Type:** {detail['media_type']}, **DB ID:** {detail['db_id']}\n"
            )
    ...
    notify_msg = (
        f"{media_type.title()} Ingestion: {processed_count} done, {error_count} errors."
    )
    app.notify(
        notify_msg, severity="information" if error_count == 0 else "warning", timeout=7
    )
