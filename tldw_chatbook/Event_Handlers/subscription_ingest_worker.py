...
async def process_subscription_item(
    ...
    try:
        ...
        if media_id:
            ...
            subscriptions_db.mark_item_status(item["id"], "ingested", media_id=media_id)

            # Log success
            duration = (datetime.now() - start_time).total_seconds()
            log_histogram(
                "subscription_item_ingestion_duration",
                duration,
                labels={
                    "subscription_type": subscription["type"],
            ...
            log_counter(
                "subscription_item_ingestion_count",
                labels={"subscription_type": subscription["type"], "status": "success"},
            )

            logger.info(
                f"Successfully ingested item '{processed['title']}' as media ID {media_id}"
            )
            ...
            log_counter(
                "subscription_item_ingestion_count",
                labels={"subscription_type": subscription["type"], "status": "failed"},
            )
...
def _extract_basic_keywords(
    item: Dict[str, Any], subscription: Dict[str, Any]
) -> List[str]:
    """
...
async def bulk_ingest_subscription_items(
    app, selected_item_ids: List[int]
) -> tuple[int, int]:
    """
