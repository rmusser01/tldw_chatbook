...
async def handle_tldw_api_auth_method_changed(
    app: "TldwCli", event: Union[Select.Changed, str]
) -> None:
...
async def handle_tldw_api_media_type_changed(
    app: "TldwCli", event: Union[Select.Changed, str]
) -> None:
    ...
    try:
        ...
        target_container_id = (
            f"tldw-api-{event_value.lower().replace('_', '-')}-options"
        )
        if event_value:  # If a type is selected
            try:
                ...
                logger.warning(
                    f"Options container #{target_container_id} not found for media type {event_value}."
                )
        ...
        logger.opt(exception=True).error(
            f"Unexpected error handling media type change: {ex}"
        )
...
def _collect_common_form_data(app: "TldwCli", media_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        data["urls"] = [
            url.strip()
            for url in app.query_one(
                f"#tldw-api-urls-{media_type}", TextArea
            ).text.splitlines()
            if url.strip()
        ]
        ...
        if (
            ingest_window
            and hasattr(ingest_window, "selected_local_files")
            and media_type in ingest_window.selected_local_files
        ):
            # Convert Path objects to strings as expected by the API client processing functions
            data["local_files"] = [
                str(p) for p in ingest_window.selected_local_files[media_type]
            ]
        else:
                ...
                if media_type in window_map:
                    ...
                    if window_class:
                        ...
                        if hasattr(media_window, "selected_local_files"):
                            data["local_files"] = [
                                str(p) for p in media_window.selected_local_files
                            ]
            except Exception:
        ...
        if data["urls"] and data["local_files"]:
            logger.warning(
                f"Both URLs and local files present for {media_type}. URLs: {data['urls']}, Files: {data['local_files']}"
            )

        current_field_template_for_error = f"#tldw-api-title-{media_type}"
        data["title"] = (
            app.query_one(f"#tldw-api-title-{media_type}", Input).value or None
        )

        current_field_template_for_error = f"#tldw-api-author-{media_type}"
        data["author"] = (
            app.query_one(f"#tldw-api-author-{media_type}", Input).value or None
        )

        current_field_template_for_error = f"#tldw-api-keywords-{media_type}"
        data["keywords_str"] = app.query_one(
            f"#tldw-api-keywords-{media_type}", TextArea
        ).text

        current_field_template_for_error = f"#tldw-api-custom-prompt-{media_type}"
        data["custom_prompt"] = (
            app.query_one(f"#tldw-api-custom-prompt-{media_type}", TextArea).text
            or None
        )

        current_field_template_for_error = f"#tldw-api-system-prompt-{media_type}"
        data["system_prompt"] = (
            app.query_one(f"#tldw-api-system-prompt-{media_type}", TextArea).text
            or None
        )

        current_field_template_for_error = f"#tldw-api-perform-analysis-{media_type}"
        data["perform_analysis"] = app.query_one(
            f"#tldw-api-perform-analysis-{media_type}", Checkbox
        ).value

        current_field_template_for_error = f"#tldw-api-overwrite-db-{media_type}"
        data["overwrite_existing_db"] = app.query_one(
            f"#tldw-api-overwrite-db-{media_type}", Checkbox
        ).value

        current_field_template_for_error = f"#tldw-api-perform-chunking-{media_type}"
        data["perform_chunking"] = app.query_one(
            f"#tldw-api-perform-chunking-{media_type}", Checkbox
        ).value

        current_field_template_for_error = f"#tldw-api-chunk-method-{media_type}"
        data["chunk_method"] = app.query_one(
            f"#tldw-api-chunk-method-{media_type}", Select
        ).value

        current_field_template_for_error = f"#tldw-api-max-chunk-size-{media_type}"
        max_chunk_size_val = app.query_one(
            f"#tldw-api-max-chunk-size-{media_type}", Input
        ).value
        try:
            data["max_chunk_size"] = (
                int(max_chunk_size_val) if max_chunk_size_val else None
            )
        except ValueError:
            logger.warning(
                f"Invalid max_chunk_size value for {media_type}: '{max_chunk_size_val}'. Using default."
            )
            data["max_chunk_size"] = None

        current_field_template_for_error = f"#tldw-api-chunk-overlap-{media_type}"
        chunk_overlap_val = app.query_one(
            f"#tldw-api-chunk-overlap-{media_type}", Input
        ).value
        try:
            data["chunk_overlap"] = (
                int(chunk_overlap_val) if chunk_overlap_val else None
            )
        except ValueError:
            logger.warning(
                f"Invalid chunk_overlap value for {media_type}: '{chunk_overlap_val}'. Using default."
            )
        ...
        if data["keywords_str"]:
            data["keywords"] = [
                k.strip() for k in data["keywords_str"].split(",") if k.strip()
            ]
        else:
            data["keywords"] = []
        ...
        logger.error(
            f"UI field not found: {current_field_template_for_error}. Exception: {e}"
        )
        raise ValueError(
            f"Required field not found: {current_field_template_for_error}. Check UI composition for media type: {media_type}"
        )
    except Exception as e:
        logger.opt(exception=True).error(
            f"Unexpected error collecting common data for {media_type}: {e}"
        )
        raise


def _collect_video_specific_data(
    app: "TldwCli", common_data: Dict[str, Any], media_type: str
) -> ProcessVideoRequest:
    ...
    try:
        ...
        transcription_model = app.query_one(
            f"#tldw-api-transcription-model-{media_type}", Select
        ).value
        ...
        start_time = (
            app.query_one(f"#tldw-api-video-start-time-{media_type}", Input).value
            or None
        )
        end_time = (
            app.query_one(f"#tldw-api-video-end-time-{media_type}", Input).value or None
        )
...
def _collect_audio_specific_data(
    app: "TldwCli", common_data: Dict[str, Any], media_type: str
) -> ProcessAudioRequest:
    ...
    try:
        ...
        # Get start and end time fields
        start_time = (
            app.query_one(f"#tldw-api-audio-start-time-{media_type}", Input).value
            or None
        )
        end_time = (
            app.query_one(f"#tldw-api-audio-end-time-{media_type}", Input).value or None
        )
...
def _collect_pdf_specific_data(
    app: "TldwCli", common_data: Dict[str, Any], media_type: str
) -> ProcessPDFRequest:
...
def _collect_ebook_specific_data(
    app: "TldwCli", common_data: Dict[str, Any], media_type: str
) -> ProcessEbookRequest:
...
def _collect_document_specific_data(
    app: "TldwCli", common_data: Dict[str, Any], media_type: str
) -> ProcessDocumentRequest:
...
def _collect_plaintext_specific_data(
    app: "TldwCli", common_data: Dict[str, Any], media_type: str
) -> ProcessPlaintextRequest:
...
def _collect_xml_specific_data(
    app: "TldwCli", common_api_data: Dict[str, Any], media_type: str
) -> ProcessXMLRequest:
    ...
    try:
        ...
        per_segment_summary_input = app.query_one(
            f"#tldw-api-per-segment-summary-{media_type}", Input
        )
        ...
        if per_segment_summary_str.lower() in ["true", "1", "yes", "y"]:
            ...
            logger.warning(
                f"Invalid per_segment_summary value: '{per_segment_summary_str}'. Using default (False)."
            )
...
def _collect_mediawiki_specific_data(
    app: "TldwCli", common_api_data: Dict[str, Any], media_type: str
) -> ProcessMediaWikiRequest:
    ...
    try:
        namespaces_val = app.query_one(
            f"#tldw-api-namespaces-{media_type}", Input
        ).value.strip()
        namespaces_list = None
        if namespaces_val:
            try:
                namespaces_list = [
                    int(ns.strip()) for ns in namespaces_val.split(",") if ns.strip()
                ]
            except ValueError:
                logger.warning(
                    f"Invalid namespaces value for {media_type}: '{namespaces_val}'. Using default."
                )
                namespaces_list = None
...
async def handle_tldw_api_submit_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    if not event.button.id:
        logger.error(
            "Submit button pressed but has no ID. Cannot determine media_type."
        )
    ...
    try:
        loading_indicator = app.query_one(
            f"#tldw-api-loading-indicator-{selected_media_type}", LoadingIndicator
        )
        status_area = app.query_one(
            f"#tldw-api-status-area-{selected_media_type}", TextArea
        )
        submit_button = event.button  # This is already the correct button
        endpoint_url_input = app.query_one(
            f"#tldw-api-endpoint-url-{selected_media_type}", Input
        )
        auth_method_select = app.query_one(
            f"#tldw-api-auth-method-{selected_media_type}", Select
        )
    except QueryError as e:
        logger.error(
            f"Critical UI component missing for media_type '{selected_media_type}': {e}"
        )
        app.notify(
            f"Error: UI component missing for {selected_media_type}: {e.widget.id if hasattr(e, 'widget') and e.widget else 'Unknown'}. Cannot proceed.",
            severity="error",
        )
        return
    ...
    if not (endpoint_url.startswith("http://") or endpoint_url.startswith("https://")):
        app.notify(
            "API Endpoint URL must start with http:// or https://.", severity="error"
        )
        endpoint_url_input.focus()
    ...
    try:
        if auth_method == "custom_token":
            custom_token_input = app.query_one(
                f"#tldw-api-custom-token-{selected_media_type}", Input
            )
            auth_token = custom_token_input.value.strip()
            if not auth_token:
                app.notify(
                    "Custom Auth Token is required for selected method.",
                    severity="error",
                )
                custom_token_input.focus()
        ...
        logger.error(
            f"UI component not found for TLDW API auth token for {selected_media_type}: {e}"
        )
        app.notify(
            f"Error: Missing UI field for auth for {selected_media_type}: {e.widget.id if hasattr(e, 'widget') and e.widget else 'Unknown'}",
            severity="error",
        )
        loading_indicator.display = False
        submit_button.disabled = False
    ...
    try:
        common_data = _collect_common_form_data(
            app, selected_media_type
        )  # Pass selected_media_type
        local_file_paths = common_data.pop("local_files", [])

        if selected_media_type == "video":
            request_model = _collect_video_specific_data(
                app, common_data, selected_media_type
            )
            logger.debug(f"Video request model URLs: {request_model.urls}")
        elif selected_media_type == "audio":
            request_model = _collect_audio_specific_data(
                app, common_data, selected_media_type
            )
        elif selected_media_type == "pdf":
            request_model = _collect_pdf_specific_data(
                app, common_data, selected_media_type
            )
        elif selected_media_type == "ebook":
            request_model = _collect_ebook_specific_data(
                app, common_data, selected_media_type
            )
        elif selected_media_type == "document":
            request_model = _collect_document_specific_data(
                app, common_data, selected_media_type
            )
        elif selected_media_type == "plaintext":
            request_model = _collect_plaintext_specific_data(
                app, common_data, selected_media_type
            )
        elif selected_media_type == "xml":
            request_model = _collect_xml_specific_data(
                app, common_data, selected_media_type
            )
            ...
            app.notify(
                f"Media type '{selected_media_type}' not yet supported by this client form.",
                severity="warning",
            )
            loading_indicator.display = False
            submit_button.disabled = False
        ...
        logger.opt(exception=True).error(
            f"Error collecting form data for {selected_media_type}: {e}"
        )
        app.notify(
            f"Error in form data for {selected_media_type}: {str(e)[:100]}. Please check fields.",
            severity="error",
        )
        loading_indicator.display = False
        submit_button.disabled = False
        status_area.load_text(
            f"Error processing form data: {str(e)[:100]}. Submission halted."
        )
        return
    except Exception as e:
        logger.opt(exception=True).error(
            f"Unexpected error preparing request model for TLDW API ({selected_media_type}): {e}"
        )
    ...
    if not request_model:
        ...
        submit_button.disabled = False
        status_area.load_text(
            "Internal error: Failed to create request model. Submission halted."
        )
        return
    ...
    if not getattr(request_model, "urls", None) and not local_file_paths:
        ...
        if not is_xml_or_mediawiki or (is_xml_or_mediawiki and not local_file_paths):
            app.notify(
                "Please provide at least one URL or one local file path.",
                severity="warning",
            )
            try:
    ...
    async def process_media_worker():  # This worker is fine
        ...
        try:
            if selected_media_type == "video":
                logger.debug(
                    f"Processing video with URLs: {getattr(request_model, 'urls', None)}"
                )
                logger.debug(
                    f"Processing video with local_file_paths: {local_file_paths}"
                )
                ...
                return await api_client.process_document(
                    request_model, local_file_paths
                )
            elif selected_media_type == "plaintext":
                return await api_client.process_plaintext(
                    request_model, local_file_paths
                )
            elif selected_media_type == "xml":
                ...
                results = []
                async for item in api_client.process_mediawiki_dump(
                    request_model, local_file_paths[0]
                ):
                    results.append(item)
                return results
            else:
                raise NotImplementedError(
                    f"Client-side processing for {selected_media_type} not implemented."
                )
        finally:
    ...
    def on_worker_success(response_data: Any):
        ...
        try:
            current_loading_indicator = app.query_one(
                f"#tldw-api-loading-indicator-{selected_media_type}", LoadingIndicator
            )
            ...
            submit_button.disabled = (
                False  # submit_button is already defined from event.button
            )
        except QueryError as e_ui:
            logger.error(
                f"UI component not found in on_worker_success for {selected_media_type}: {e_ui}"
            )

        app.notify(
            f"TLDW API request for {selected_media_type} successful. Processing results...",
            timeout=2,
        )
        logger.info(f"TLDW API Response for {selected_media_type}: {response_data}")

        try:
            current_status_area = app.query_one(
                f"#tldw-api-status-area-{selected_media_type}", TextArea
            )
            current_status_area.clear()
        except QueryError:
            logger.error(
                f"Could not find status_area for {selected_media_type} in on_worker_success."
            )
        ...
        if isinstance(response_data, BatchMediaProcessResponse):
                ...
                results_to_ingest.append(
                    MediaItemProcessResult(
                        ...
                        metadata={
                            ...
                            "revision_id": mw_page.revision_id,
                        ...
                        if hasattr(mw_page, "chunks") and mw_page.chunks
                        else None,
                    )
                )
        else:
            logger.error(
                f"Unexpected TLDW API response data type for {selected_media_type}: {type(response_data)}."
            )
            current_status_area.load_text(
            ...
            app.notify(
                "Error: Received unexpected data format from API.", severity="error"
            )
            return
        ...
        for item_result in results_to_ingest:
            if item_result.status == "Success":
                media_id_ingested = (
                    None  # For storing the ID if ingestion is successful
                )
                try:
                    ...
                    api_keywords = (
                        item_result.metadata.get("keywords", [])
                        if item_result.metadata
                        else []
                    )
                    if isinstance(
                        api_keywords, str
                    ):  # If server returns comma-sep string
                        api_keywords = [
                            k.strip() for k in api_keywords.split(",") if k.strip()
                        ]
                    ...
                    if item_result.chunks:
                        ...
                        for idx, chunk_item in enumerate(item_result.chunks):
                            if isinstance(chunk_item, dict) and "text" in chunk_item:
                                unvectorized_chunks_to_save.append(
                                    {
                                        ...
                                        "start_char": chunk_item.get(
                                            "metadata", {}
                                        ).get(
                                            "start_char"
                                        ),  # Assuming metadata structure
                                        "end_char": chunk_item.get("metadata", {}).get(
                                            "end_char"
                                        ),
                                        "chunk_type": chunk_item.get(
                                            "metadata", {}
                                        ).get("type", selected_media_type),
                                        "metadata": chunk_item.get(
                                            "metadata", {}
                                ...
                                chunk_item, str
                            ):  # If chunks are just strings
                                unvectorized_chunks_to_save.append({"text": chunk_item})
                            else:
                                logger.warning(
                                    f"Skipping malformed chunk item: {chunk_item}"
                                )
                    ...
                    media_id, media_uuid, msg = app.media_db.add_media_with_keywords(
                        ...
                        if item_result.metadata
                        else item_result.input_ref,
                        media_type=item_result.media_type,
                        content=item_result.content
                        or item_result.transcript,  # Use transcript if content is empty
                        keywords=api_keywords
                        or (
                            request_model.keywords
                            if hasattr(request_model, "keywords")
                            else []
                        ),  # Fallback to request
                        prompt=request_model.custom_prompt
                        if hasattr(request_model, "custom_prompt")
                        else None,  # From original request
                        analysis_content=item_result.analysis or item_result.summary,
                        transcription_model=item_result.analysis_details.get(
                            "transcription_model"
                        )
                        if item_result.analysis_details
                        else (
                            request_model.transcription_model
                            if hasattr(request_model, "transcription_model")
                            else None
                        ),
                        author=item_result.metadata.get("author")
                        if item_result.metadata
                        else (
                            request_model.author
                            if hasattr(request_model, "author")
                            else None
                        ),
                    ...
                    if media_id:
                        logger.info(
                            f"Successfully ingested '{item_result.input_ref}' into local DB for {selected_media_type}. Media ID: {media_id}. Message: {msg}"
                        )
                        processed_count += 1
                        media_id_ingested = media_id  # Store the ID
                    else:
                        logger.error(
                            f"Failed to ingest '{item_result.input_ref}' into local DB for {selected_media_type}. Message: {msg}"
                        )
                        error_count += 1
                except Exception as e_ingest:
                    logger.opt(exception=True).error(
                        f"Error ingesting item '{item_result.input_ref}' for {selected_media_type} into local DB: {e_ingest}"
                    )
                    error_count += 1
                ...
                logger.error(
                    f"API processing error for '{item_result.input_ref}' ({selected_media_type}): {item_result.error}"
                )
                error_count += 1

        summary_parts = [
            f"## TLDW API Request Successful ({selected_media_type.title()})\n\n"
        ]
        ...
        if processed_count == 0 and error_count == 0 and not results_to_ingest:
            ...
            summary_parts.append(
                "API request successful, but no new items were ingested due to errors.\n"
            )
            summary_parts.append(
                f"- Successfully processed items by API: {processed_count}\n"
            )  # This might be confusing if API said success but ingest failed
            summary_parts.append(
                f"- Items with errors during API processing or local ingestion: {error_count}\n"
            )
        else:
            summary_parts.append(
                f"- Successfully processed and ingested items: {processed_count}\n"
            )
            summary_parts.append(
                f"- Items with errors during API processing or local ingestion: {error_count}\n\n"
            )
        ...
        if successful_ingestions_details:
            if len(successful_ingestions_details) <= 5:
                ...
                for detail in successful_ingestions_details:
                    ...
                    summary_parts.append(
                        f"- **Input:** `{detail['input_ref']}`{title_str}\n"
                ...
                summary_parts.append(
                    f"Details for {len(successful_ingestions_details)} successfully ingested items are available in the logs.\n"
                )
        elif (
            processed_count > 0
        ):  # Processed but no details (should not happen if logic is correct)
            summary_parts.append(
                "Successfully processed items, but details are unavailable.\n"
            )
    ...
    def on_worker_failure(error: Exception):
        ...
        try:
            ...
            logger.error(
                f"UI component not found in on_worker_failure for {selected_media_type}: {e_ui}"
            )

        logger.opt(exception=True).error(
            f"TLDW API request worker failed for {selected_media_type}: {error}"
        )

        error_message_parts = [
            f"## API Request Failed! ({selected_media_type.title()})\n\n"
        ]
        ...
        try:
            ...
            logger.error(
                f"Could not find status_area for {selected_media_type} to display error."
            )
            app.notify(
                f"Critical: Status area for {selected_media_type} not found. Error: {brief_notify_message}",
                severity="error",
                timeout=10,
            )
            return
