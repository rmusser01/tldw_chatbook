...
class SwarmUIEventHandler:
    ...
    async def handle_generation_request(self, event: SwarmUIGenerateRequest) -> None:
        ...
        try:
            ...
            self.post_message(
                SwarmUIStatusUpdate(
                    status="generating", message=f"Generating: {event.prompt[:50]}..."
                )
            )
            ...
            if result.success:
                self.post_message(
                    SwarmUIGenerationComplete(
                        result=result, conversation_id=event.conversation_id
                    )
                )
                self.post_message(
                    SwarmUIStatusUpdate(
                ...
                self.post_message(
                    SwarmUIStatusUpdate(
                        status="idle", message=f"Generation failed: {result.error}"
                    )
                )
            ...
            self.post_message(
                SwarmUIStatusUpdate(status="idle", message=f"Error: {str(e)}")
            )
    ...
    async def handle_generation_complete(
        self, event: SwarmUIGenerationComplete
    ) -> None:
    ...
    async def save_generation_to_db(
        self, result: GenerationResult, conversation_id: int
    ) -> None:
