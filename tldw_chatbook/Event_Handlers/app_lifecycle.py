...
async def handle_copy_logs_button_pressed(
    ...
    try:
        ...
        if log_widget.lines:
            ...
            for i, strip in enumerate(log_widget.lines):
                ...
                if i < 3:
                    logger.debug(
                        f"Line {i} type: {type(strip)}, has .text: {hasattr(strip, 'text')}"
                    )
                ...
                if hasattr(strip, "text"):  # Strip objects have a .text attribute
                    ...
                    all_log_text_parts.append(text)
                    logger.warning(
                        f"Line {i} doesn't have .text attribute, using str(): {text[:50]}..."
                    )
            ...
            logger.info(
                f"Attempting to copy {len(all_log_text)} characters to clipboard..."
            )
        ...
        app.notify(
            "Log widget not found. Cannot copy.",
            title="Error",
            severity="error",
            timeout=4,
        )
        ...
        app.notify(
            f"Error processing log line: {str(ae)}",
            title="Error",
            severity="error",
            timeout=6,
        )
        logger.opt(exception=True).error(
            f"AttributeError while processing RichLog lines: {ae}"
        )
    except Exception as e:  # General catch-all
        app.notify(
            f"Error copying logs: {str(e)}", title="Error", severity="error", timeout=6
        )
