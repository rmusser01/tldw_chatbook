...
async def populate_ccp_character_select(app: "TldwCli") -> None:
    ...
    if not app.notes_service:
        logger.error(
            "Notes service not available, cannot populate character select for CCP tab."
        )
        try:
            ...
            logger.error(
                "Failed to find #conv-char-character-select to show service error."
            )
        return
    ...
    try:
        ...
        if character_cards:
            options = [
                ...
                if char.get("name") and char.get("id") is not None
            ]
        ...
        if options:
            ...
            logger.info(
                f"Populated #conv-char-character-select with {len(options)} characters."
            )
        else:
        ...
        logger.opt(exception=True).error(
            f"Failed to find #conv-char-character-select widget: {e_query}"
        )
    except CharactersRAGDBError as e_db:
        logger.opt(exception=True).error(
            f"Database error populating #conv-char-character-select: {e_db}"
        )
        try:
            char_select_widget_err = app.query_one(
                "#conv-char-character-select", Select
            )
            char_select_widget_err.set_options(
                [("Error Loading Characters", Select.BLANK)]
            )
        ...
        logger.opt(exception=True).error(
            f"Unexpected error populating #conv-char-character-select: {e_unexp}"
        )
        try:
            char_select_widget_unexp = app.query_one(
                "#conv-char-character-select", Select
            )
            char_select_widget_unexp.set_options(
                [("Error Loading (Unexpected)", Select.BLANK)]
            )
...
async def populate_ccp_prompts_list_view(
    ...
    if not app.prompts_service_initialized:
        try:
            ...
            await list_view_prompt_err.append(
                ListItem(Label("Prompts service not available."))
            )
        except QueryError:
    ...
    try:
        ...
        if search_term:
            ...
            results_tuple = prompts_interop.list_prompts(
        ...
        logger.info(
            f"Populated CCP prompts list. Search: '{search_term}', Found: {len(results)}"
        )
    except QueryError as e_query:
        logger.opt(exception=True).error(
            f"UI component error populating CCP prompts list: {e_query}"
        )
    except (prompts_interop.DatabaseError, RuntimeError) as e_prompt_service:
        logger.opt(exception=True).error(
            f"Error populating CCP prompts list: {e_prompt_service}"
        )
        try:
            ...
            await list_view_err_detail.append(
                ListItem(
                    Label(f"Error loading prompts: {type(e_prompt_service).__name__}")
                )
            )
        except QueryError:
            pass
        ...
        logger.opt(exception=True).error(
            f"Unexpected error populating CCP prompts list: {e_unexp}"
        )
...
#
########################################################################################################################
async def _character_import_callback(
    ...
    if selected_path:
        ...
        try:
            ...
            if char_id is not None:
                app.notify(
                    f"Character card imported successfully (ID: {char_id}).",
                ...
                app.notify(
                    "Failed to import character card. Check logs.", severity="error"
                )
            ...
            await populate_ccp_character_select(
                app
            )  # Refresh in case it was a duplicate name
        except ImportError as ie:  # E.g., PyYAML missing for Markdown with frontmatter
            app.notify(
                f"Import error: {ie}. A required library might be missing.",
                severity="error",
                timeout=8,
            )
            logger.opt(exception=True).error(
                f"Import error for character card '{selected_path}': {ie}"
            )
        except Exception as e:
            app.notify(
                f"Error importing character card: {type(e).__name__}",
                severity="error",
                timeout=6,
            )
            logger.opt(exception=True).error(
                f"Error importing character card '{selected_path}': {e}"
            )
    else:
...
async def handle_ccp_import_character_button_pressed(
    ...
    defined_filters = Filters(
        (
            "Character Card (JSON, PNG, WebP, MD)",
            lambda p: (
                p.suffix.lower() in (".json", ".png", ".webp", ".md", ".yaml", ".yml")
            ),
        ),
...
async def handle_ccp_left_load_character_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(
        app, "loguru_logger", loguru_logger
    )  # Or however logger is typically obtained
    ...
    try:
        ...
        logger.info(
            f"Attempting to load character ID: {selected_character_id} into center editor view."
        )
        ...
        try:
            ...
            char_data, initial_ui_history, img = ccl.load_character_and_image(
                db, selected_character_id, app.notes_user_id
            )
            ...
            if char_data:
                ...
                app.notify(
                    f"Displaying character: {char_data.get('name', 'Unknown')}",
                    severity="information",
                )
                logger.info(
                    f"Character card for '{char_data.get('name', 'Unknown')}' loaded and view switched to card display."
                )
            else:
                app.notify("Failed to load character details.", severity="error")
                logger.warning(
                    f"Failed to load character details for ID: {selected_character_id}"
                )

        except CharactersRAGDBError as e_db_rag:
            logger.opt(exception=True).error(
                f"Database error loading character {selected_character_id}: {e_db_rag}"
            )
            app.notify(
                f"Database error loading character: {e_db_rag}", severity="error"
            )
        except Exception as e_load_char:  # Catch other potential errors from ccl.load_character_and_image
            logger.opt(exception=True).error(
                f"Error loading character data/image for ID {selected_character_id}: {e_load_char}"
            )
            app.notify(
                f"Error loading character: {type(e_load_char).__name__}",
                severity="error",
            )

    except QueryError as e_query:
        logger.opt(exception=True).error(
            f"UI component not found during load character (left pane): {e_query}"
        )
        app.notify(
            "Error: UI component missing for loading character.", severity="error"
        )
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error during load character (left pane): {e_unexp}"
        )
        app.notify(
            "An unexpected error occurred while trying to load character view.",
            severity="error",
        )


async def handle_ccp_create_character_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        logger.error(
            "Failed to find #ccp-editor-char-cancel-button to remove 'hidden' class."
        )
    ...
    app.notify(
        "Create a new character by filling in the details.", severity="information"
    )
    logger.info("Character editor cleared and ready for new character creation.")


async def handle_ccp_refresh_character_list_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def handle_ccp_card_save_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    if not app.notes_service:  # This should be app.chachanotes_db for character ops
        app.notify("Database service not available.", severity="error")
        logger.error(
            "ChaChaNotes DB (via notes_service or direct) not available for saving character card."
        )
        return

    # Use cast to help the type checker understand the type of the reactive's value
    current_details_value = cast(
        Optional[Dict[str, Any]], app.current_ccp_character_details
    )

    if not current_details_value:
        app.notify(
            "No character loaded or details missing. Cannot save.", severity="warning"
        )
        logger.warning(
            "Save Character Card: app.current_ccp_character_details value is None or empty dict."
        )
        return
    ...
    if not char_id or char_version is None:
        app.notify(
            "Character ID or version missing from loaded details. Cannot save.",
            severity="warning",
        )
        logger.warning(
            f"Save Character Card: ID ({char_id}) or Version ({char_version}) is missing."
        )
        return
    ...
    try:
        ...
        first_message_textarea = app.query_one(
        ...
        if updated_character_data:
            app.notify(
                f"Character '{update_data['name']}' updated successfully.",
                severity="information",
            )
            logger.info(
                f"Character ID {char_id} updated successfully. New version: {updated_character_data.get('version')}"
            )
            app.current_ccp_character_details = updated_character_data

            if original_name != update_data["name"]:
                logger.info(
                    f"Character name changed from '{original_name}' to '{update_data['name']}'. Refreshing dropdown."
                )
                await populate_ccp_character_select(
                    app
                )  # Ensure this is imported/defined
        else:
            app.notify(
                "Failed to update character. DB operation did not confirm success.",
                severity="error",
            )
            logger.error(
                f"Update for character ID {char_id} did not return updated data or confirmation."
            )

    except ConflictError as e_conflict:
        logger.opt(exception=True).warning(
            f"ConflictError saving character ID {char_id}: {e_conflict}"
        )
        app.notify(
            "Save conflict: Data has been modified elsewhere. Please reload and try again.",
            severity="error",
            timeout=7,
        )
    except QueryError as e_query:
        logger.opt(exception=True).error(
            f"UI component not found during save character card: {e_query}"
        )
        app.notify(
            "Error: UI component missing for saving character.", severity="error"
        )
    except CharactersRAGDBError as e_db:
        logger.opt(exception=True).error(
            f"CharactersRAGDBError saving character ID {char_id}: {e_db}"
        )
        app.notify(
            f"Error saving character: {type(e_db).__name__}. Check logs.",
            severity="error",
        )
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error saving character ID {char_id}: {e_unexp}"
        )
        app.notify(
            f"An unexpected error occurred: {type(e_unexp).__name__}. Check logs.",
            severity="error",
        )


async def handle_ccp_card_clone_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    if not app.chachanotes_db:  # Check the correct DB instance
        app.notify("Database service not available.", severity="error")
        logger.error(
            "ChaChaNotes DB (via app.chachanotes_db) not available for cloning character card."
        )
        return
    ...
    if not current_details_value:
        ...
        logger.warning(
            "Clone Character Card: app.current_ccp_character_details value is None."
        )
        return
    ...
    try:
        ...
        name_input_val = app.query_one(
            "#ccp-card-name-display", Static
        ).renderable  # If it's a Static for display
        ...
        if isinstance(name_input_val, Text):
            ...
            original_name_from_ui = str(
                current_details_value.get("name", "Unnamed")
            ).strip()

        description_from_ui = app.query_one(
            "#ccp-card-description-display", TextArea
        ).text.strip()
        personality_from_ui = app.query_one(
            "#ccp-card-personality-display", TextArea
        ).text.strip()
        scenario_from_ui = app.query_one(
            "#ccp-card-scenario-display", TextArea
        ).text.strip()
        first_message_from_ui = app.query_one(
            "#ccp-card-first-message-display", TextArea
        ).text.strip()

        if not original_name_from_ui:
            app.notify(
                "Original character name is empty. Cannot clone.", severity="error"
            )
            # app.query_one("#ccp-card-name-input", Input).focus() # If it's an Input
            return

        cloned_name = (
            f"{original_name_from_ui} Clone {datetime.now().strftime('%Y%m%d%H%M%S')}"
        )
        ...
        if "image" in current_details_value and isinstance(
            current_details_value["image"], bytes
        ):
            cloned_character_data["image"] = current_details_value[
                "image"
            ]  # Bytes are immutable enough for this copy
        ...
        if new_character_id:
            app.notify(
                f"Character cloned as '{cloned_name}' successfully (ID: {new_character_id}).",
                severity="information",
            )
            logger.info(
            ...
            app.notify(
                f"Failed to clone character '{original_name_from_ui}'. DB operation did not confirm success.",
                severity="error",
            )
            logger.error(
                f"Cloning character '{original_name_from_ui}' did not return new character details or ID."
            )

    except ConflictError as e_conflict:
        logger.opt(exception=True).warning(
            f"ConflictError cloning character '{original_name_from_ui}': {e_conflict}"
        )
        app.notify(
            f"Save conflict while cloning: {e_conflict}. Please try again.",
            severity="error",
            timeout=7,
        )
    except QueryError as e_query:
        logger.opt(exception=True).error(
            f"UI component not found during clone character card: {e_query}"
        )
        app.notify(
            "Error: UI component missing for cloning character.", severity="error"
        )
    except CharactersRAGDBError as e_db:
        logger.opt(exception=True).error(
            f"CharactersRAGDBError cloning character '{original_name_from_ui}': {e_db}"
        )
        app.notify(
            f"Error cloning character: {type(e_db).__name__}. Check logs.",
            severity="error",
        )
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error cloning character '{original_name_from_ui}': {e_unexp}"
        )
        app.notify(
            f"An unexpected error occurred while cloning: {type(e_unexp).__name__}. Check logs.",
            severity="error",
        )


async def handle_ccp_right_delete_character_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    if (
        not app.current_ccp_character_details
        or not app.current_ccp_character_details.get("id")
    ):
        app.notify(
            "No character is currently loaded in the view to delete.",
            severity="warning",
        )
        logger.warning(
            "Delete Character (Right Pane): app.current_ccp_character_details is None or missing ID."
        )
        return

    character_id_to_delete = app.current_ccp_character_details.get("id")
    character_name_to_delete = app.current_ccp_character_details.get(
        "name", "The selected character"
    )
    ...
    try:
        ...
        if success:
            app.notify(
                f"Character '{character_name_to_delete}' deleted successfully.",
                severity="information",
            )
            logger.info(
                f"Character ID {character_id_to_delete} (Name: '{character_name_to_delete}') deleted successfully."
            )
            ...
            app.ccp_active_view = (
                "conversation_messages_view"  # Or another appropriate default
            )
            ...
            try:
                ...
                logger.warning(
                    f"Could not explicitly clear some card UI fields after delete: {qe_clear}"
                )

        else:
            # This case might be hit if db.delete_character_card returns False (e.g., char not found)
            app.notify(
                f"Failed to delete character '{character_name_to_delete}'. It might have already been deleted or an error occurred.",
                severity="error",
            )
            logger.error(
                f"Delete operation for character ID {character_id_to_delete} (Name: '{character_name_to_delete}') returned False."
            )
        ...
        logger.opt(exception=True).error(
            f"Database error deleting character ID {character_id_to_delete}: {e_db}"
        )
        app.notify(
            f"Error deleting character: {type(e_db).__name__}. Check logs.",
            severity="error",
        )
    except QueryError as e_query:  # If query_one for clearing fields fails
        logger.opt(exception=True).error(
            f"UI component error during delete character (likely clearing fields): {e_query}"
        )
        app.notify("UI error during character deletion process.", severity="error")
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error deleting character ID {character_id_to_delete}: {e_unexp}"
        )
        app.notify(
            f"An unexpected error occurred while deleting: {type(e_unexp).__name__}. Check logs.",
            severity="error",
        )
    ...
    *,
    ...
    if not search_term:
        ...
        if effective_character_id is not None:
            ...
            logger.debug(
                f"CCP Search: All conversations for CharID {effective_character_id}"
            )
            conversations = db.get_conversations_for_character(
                character_id=effective_character_id, limit=200
            )
        else:
            # All conversations
        ...
        # Search by title
        logger.debug(
            f"CCP Search: Term '{search_term}', CharID {effective_character_id}"
        )
        conversations = db.search_conversations_by_title(
            title_query=search_term, character_id=effective_character_id, limit=200
        )
    ...
    if keyword_search_term and conversations:
        ...
        if filtered_count > 0:
            logger.debug(
                f"Keyword filter removed {filtered_count} conversations, keeping {len(conversations)}"
            )
    ...
    if tag_search_term and conversations:
        ...
        if search_tags:
            ...
            if filtered_count > 0:
                logger.debug(
                    f"Tag filter removed {filtered_count} conversations, keeping {len(conversations)}"
                )
    ...
    if not include_char_chats and conversations:
        ...
        if filtered_count > 0:
            logger.debug(
                f"Character chat filter removed {filtered_count} conversations"
            )
...
async def perform_ccp_conversation_search(app: "TldwCli") -> None:
    ...
    try:
        ...
        try:
            include_checkbox = app.query_one(
                "#conv-char-search-include-character-checkbox", Checkbox
            )
        ...
        try:
            all_chars_checkbox = app.query_one(
                "#conv-char-search-all-characters-checkbox", Checkbox
            )
        ...
        if not conversations:
            msg = (
                "Enter search term or select a character."
                if not search_term and not selected_character_id
                else "No items found matching your criteria."
            )
            ...
            for conv_data in conversations:
                base_title = (
                    conv_data.get("title")
                    or f"Conversation ID: {conv_data['id'][:8]}..."
                )
        ...
        logger.opt(exception=True).error(
            f"UI component not found during CCP conversation search: {e_query}"
        )
        ...
        logger.opt(exception=True).error(
            f"Database error during CCP conversation search: {e_db}"
        )
        if "results_list_view" in locals() and results_list_view.is_mounted:
            await results_list_view.clear()
            await results_list_view.append(
        ...
        logger.opt(exception=True).error(
            f"Unexpected error during CCP conversation search: {e_unexp}"
        )
        if "results_list_view" in locals() and results_list_view.is_mounted:
            await results_list_view.clear()
            await results_list_view.append(
...
#
########################################################################################################################
async def handle_ccp_conversation_search_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def handle_ccp_load_conversation_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        if not (highlighted_item and hasattr(highlighted_item, "details")):
            logger.warning(
                "No conversation selected in CCP list or item has no details."
            )
        ...
        if app.notes_service:
            ...
            keywords_input_ccp.text = (
                ", ".join([kw["keyword"] for kw in keywords_list_ccp])
                if keywords_list_ccp
                else ""
            )
        else:
        ...
        if not app.notes_service:
            ...
            await messages_container.mount(
                Static("Error: Notes service unavailable for messages.")
            )
            return
        ...
        logger.info(
            f"Loaded messages into #conv-char-center-pane for conversation {loaded_conversation_id}."
        )
        app.notify(
            f"Conversation '{title_input_ccp.value}' loaded.", severity="information"
        )

    except QueryError as e_query:
        logger.opt(exception=True).error(
            f"UI component not found during CCP load conversation: {e_query}"
        )
        app.notify("Error: UI component missing for loading.", severity="error")
    except CharactersRAGDBError as e_db:
        logger.opt(exception=True).error(
            f"Database error during CCP load conversation: {e_db}"
        )
        app.notify("Error loading conversation data from database.", severity="error")
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error during CCP load conversation: {e_unexp}"
        )
        app.notify(
...
async def handle_ccp_save_conversation_details_button_pressed(
    ...
    try:
        ...
        if not current_conv_details_ccp:
            logger.error(
                f"CCP: Conversation {target_conversation_id_ccp} not found in DB for saving details."
            )
        ...
        if current_db_version_ccp is None:
            logger.error(
                f"CCP: Conversation {target_conversation_id_ccp} is missing version information."
            )
            app.notify(
        ...
        if new_title_ccp != current_conv_details_ccp.get("title", ""):
            ...
            db.update_conversation(
                conversation_id=target_conversation_id_ccp,
                update_data=update_payload_ccp,
        ...
        existing_db_keywords_ccp = db.get_keywords_for_conversation(
            target_conversation_id_ccp
        )
        existing_kw_texts_set_ccp = {
            kw["keyword"].lower() for kw in existing_db_keywords_ccp
        }
        ui_kw_texts_set_ccp = {
            kw.strip().lower() for kw in new_keywords_str_ccp.split(",") if kw.strip()
        }

        keywords_to_add_ccp = ui_kw_texts_set_ccp - existing_kw_texts_set_ccp
        keywords_to_remove_details_ccp = [
            kw
            for kw in existing_db_keywords_ccp
            if kw["keyword"].lower() not in ui_kw_texts_set_ccp
        ]
        ...
        for kw_text_add in keywords_to_add_ccp:
            ...
            if isinstance(kw_id_add, int):  # Check if it's an int ID
                db.link_conversation_to_keyword(
                    conversation_id=target_conversation_id_ccp, keyword_id=kw_id_add
                )
                keywords_changed_ccp = True
            else:
                logger.error(
                    f"Failed to get or add keyword '{kw_text_add}', received: {kw_id_add}"
                )
        ...
        if title_updated_ccp or keywords_changed_ccp:
            logger.info(
                f"CCP: Details saved for conversation {target_conversation_id_ccp}."
            )
            app.notify("Details saved successfully!", severity="information")
            final_keywords_list_ccp = db.get_keywords_for_conversation(
                target_conversation_id_ccp
            )
            keywords_widget_ccp_save.text = (
                ", ".join([kw["keyword"] for kw in final_keywords_list_ccp])
                if final_keywords_list_ccp
                else ""
            )
        else:
        ...
        logger.opt(exception=True).error(
            f"CCP: UI component not found for saving details: {e_query}"
        )
        ...
        logger.opt(exception=True).error(
            f"CCP: Unexpected error saving details: {e_unexp}"
        )
        app.notify("An unexpected error occurred.", severity="error")


async def handle_ccp_prompt_create_new_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:  # This one is for the RIGHT pane
    ...
    try:
        app.query_one(
            "#ccp-prompt-name-input", Input
        ).value = "New Prompt"  # Default name in right pane
        author_name = app.app_config.get("user_defaults", {}).get("author_name", "User")
        app.query_one(
            "#ccp-prompt-author-input", Input
        ).value = author_name  # In right pane

        app.query_one(
            "#ccp-prompt-details-collapsible", Collapsible
        ).collapsed = False  # Expand right pane prompt details
        app.query_one(
            "#ccp-conversation-details-collapsible", Collapsible
        ).collapsed = True  # Collapse right pane conv details

        app.ccp_active_view = (
            "conversation_details_view"  # Ensure center pane is NOT on its editor view.
        )
        ...
        app.notify(
            "Ready to create a new prompt in the right-side editor.",
            severity="information",
        )
    except QueryError as e_query:  # This can happen if the right sidebar is collapsed
        logger.opt(exception=True).error(
            f"CCP: UI error preparing for new prompt (right pane): {e_query}"
        )
        app.notify("UI error creating new prompt.", severity="error")


async def handle_ccp_center_pane_new_prompt_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    """
...
async def handle_ccp_prompt_load_selected_button_pressed(
    ...
    try:
        ...
        if selected_item and (
            ...
            await app._load_prompt_for_editing(
                prompt_id=prompt_id_to_load, prompt_uuid=prompt_uuid_to_load
            )
        else:
        ...
        app.notify(
            f"Error loading prompt: {str(e)[:100]}", severity="error"
        )  # Show part of error
        logger.opt(exception=True).error(f"Error loading prompt from list: {e}")


async def handle_ccp_prompt_save_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        details = app.query_one(
            "#ccp-prompt-description-textarea", TextArea
        ).text.strip()
        system_prompt = app.query_one(
            "#ccp-prompt-system-textarea", TextArea
        ).text.strip()
        user_prompt = app.query_one("#ccp-prompt-user-textarea", TextArea).text.strip()
        keywords_str = app.query_one(
            "#ccp-prompt-keywords-textarea", TextArea
        ).text.strip()
        ...
        if (
            app.current_prompt_id is None
        ):  # This implies a new prompt being saved from the right pane editor
            ...
            logger.info(
                f"CCP: Updating prompt ID (from right pane): {app.current_prompt_id}, Name: {name}"
            )
            update_payload = {
                "name": name,
                "author": author,
            ...
            updated_uuid, message_from_save = (
                prompts_interop.get_db_instance().update_prompt_by_id(
        ...
        if saved_id is not None or saved_uuid is not None:
            ...
            await app._load_prompt_for_editing(
                prompt_id=saved_id, prompt_uuid=saved_uuid
            )
        else:
            app.notify(
                f"Failed to save prompt: {message_from_save or 'Unknown error'}",
                severity="error",
            )
        ...
        logger.opt(exception=True).error(
            f"CCP Save Prompt (right pane): UI component error: {e_query}"
        )
        app.notify("UI Error saving prompt.", severity="error")
    except Exception as e_save:
        logger.opt(exception=True).error(
            f"CCP: Error saving prompt (right pane): {e_save}"
        )
        app.notify(f"Error saving prompt: {type(e_save).__name__}", severity="error")


async def handle_ccp_prompt_clone_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    if not app.prompts_service_initialized or app.current_prompt_id is None:
        app.notify(
            "No prompt loaded in right pane editor to clone or service unavailable.",
            severity="warning",
        )
        return
    try:
        ...
        cloned_id, cloned_uuid, msg_clone = prompts_interop.add_prompt(
            ...
            keywords=[
                kw.strip()
                for kw in (app.current_prompt_keywords_str or "").split(",")
                if kw.strip()
            ],
            overwrite=False,
        )
        if cloned_id:
            app.notify(
                f"Prompt cloned as '{cloned_name}'. {msg_clone}", severity="information"
            )
            await populate_ccp_prompts_list_view(app)  # Refresh list in left pane
            # Load the newly cloned prompt into the RIGHT PANE editor
            await app._load_prompt_for_editing(
                prompt_id=cloned_id, prompt_uuid=cloned_uuid
            )
        else:
            app.notify(f"Failed to clone prompt: {msg_clone}", severity="error")
    except Exception as e_clone:
        logger.opt(exception=True).error(
            f"CCP: Error cloning prompt (from right pane): {e_clone}"
        )
        app.notify(f"Error cloning prompt: {type(e_clone).__name__}", severity="error")


async def handle_ccp_prompt_delete_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    """Handles deleting (soft) the currently loaded prompt from the RIGHT PANE editor."""
    logger = getattr(app, "loguru_logger", loguru_logger)
    logger.info(
        "CCP Delete Prompt button pressed (deletes based on right pane editor state)."
    )
    if not app.prompts_service_initialized or app.current_prompt_id is None:
        app.notify(
            "No prompt loaded in right pane editor to delete or service unavailable.",
            severity="warning",
        )
        return
    ...
    try:
        ...
        success = prompts_interop.soft_delete_prompt(
            prompt_id_to_delete
        )  # client_id is handled internally

        if success:
            app.notify(
                f"Prompt '{app.current_prompt_name or 'selected'}' deleted.",
                severity="information",
            )
            ...
            try:  # Collapse details in right pane
                ...
                logger.warning(
                    "Could not find #ccp-prompt-details-collapsible to collapse after delete."
                )
        else:
            ...
            if (
                app.current_prompt_id == prompt_id_to_delete
            ):  # Only clear if it was the one we tried to delete
                clear_ccp_prompt_fields(app)

    except (
        prompts_interop.ConflictError
    ) as e_cf_del:  # Specific exception from your DB class
        logger.opt(exception=True).error(
            f"CCP: Conflict error deleting prompt: {e_cf_del}"
        )
        app.notify(f"Conflict error deleting prompt: {e_cf_del}", severity="error")
        await populate_ccp_prompts_list_view(app)  # Refresh list
    except (
        prompts_interop.DatabaseError
    ) as e_db_del:  # Specific exception from your DB class
        logger.opt(exception=True).error(
            f"CCP: Database error deleting prompt: {e_db_del}"
        )
        app.notify(
            f"Database error deleting prompt: {type(e_db_del).__name__}",
            severity="error",
        )
    except Exception as e_del:  # Catch any other unexpected exceptions
        logger.opt(exception=True).error(
            f"CCP: Unexpected error deleting prompt: {e_del}"
        )
        app.notify(
            f"Unexpected error deleting prompt: {type(e_del).__name__}",
            severity="error",
        )


async def handle_ccp_conversation_search_input_changed(
    ...
    app._conv_char_search_timer = app.set_timer(
        0.5, lambda: perform_ccp_conversation_search(app)
    )


async def handle_ccp_conversation_keyword_search_input_changed(
    app: "TldwCli", event: Input.Changed
) -> None:
...
async def handle_ccp_conversation_tags_search_input_changed(
    app: "TldwCli", event: Input.Changed
) -> None:
...
async def handle_ccp_search_checkbox_changed(
    ...
    if checkbox_id == "conv-char-search-all-characters-checkbox":
        try:
            ...
            if value:
                char_select_widget.value = (
                    Select.BLANK
                )  # Clear selection when "All" is checked
        except QueryError as e:
            logger.opt(exception=True).error(
                f"Error accessing character select widget: {e}"
            )
...
async def handle_ccp_prompt_search_input_changed(
    app: "TldwCli", event: Input.Changed
) -> None:
    ...
    app._prompt_search_timer = app.set_timer(
        0.5, lambda: populate_ccp_prompts_list_view(app, search_term.strip())
    )


async def handle_ccp_character_select_changed(
    app: "TldwCli", selected_value: Any
) -> None:
...
async def handle_ccp_prompts_list_view_selected(
    app: "TldwCli", list_view_id: str, item: Any
) -> None:
    ...
    if item and (hasattr(item, "prompt_id") or hasattr(item, "prompt_uuid")):
        ...
        logger.info(
            f"CCP Prompt selected from list: ID={prompt_id_to_load}, UUID={prompt_uuid_to_load}. Loading to right pane."
        )
...
#
########################################################################################################################
async def _conversation_import_callback(
    app: "TldwCli", selected_path: Optional[Path]
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    if selected_path:
        ...
        try:
            ...
            if conv_id:
                app.notify(
                    f"Conversation imported successfully (ID: {conv_id}).",
                    severity="information",
                )
                await perform_ccp_conversation_search(app)  # Refresh conversation list
            else:
                app.notify(
                    "Failed to import conversation. Check logs.", severity="error"
                )
        except Exception as e:
            app.notify(
                f"Error importing conversation: {type(e).__name__}",
                severity="error",
                timeout=6,
            )
            logger.opt(exception=True).error(
                f"Error importing conversation from '{selected_path}': {e}"
            )
    else:
...
async def handle_ccp_import_conversation_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
...
def _parse_prompt_from_file_content(file_content_str: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if isinstance(parsed_json, dict) and parsed_json.get("name"):
            ...
            logger.debug(
                "Content parsed as JSON but not a valid prompt structure (missing 'name' or not a dict)."
            )
    ...
    try:
        ...
        if isinstance(parsed_yaml, dict) and parsed_yaml.get("name"):
            ...
            logger.debug(
                "Content parsed as YAML but not a valid prompt structure (missing 'name' or not a dict)."
            )
    ...
    try:
        ...
        for match in re.finditer(
            pattern, file_content_str, re.MULTILINE | re.DOTALL | re.IGNORECASE
        ):
        ...
        if "keywords_str" in parsed_custom_data:
            parsed_custom_data["keywords"] = [
                kw.strip()
                for kw in parsed_custom_data["keywords_str"].split(",")
                if kw.strip()
            ]
        ...
        if "name" not in parsed_custom_data or not parsed_custom_data["name"]:
            ...
            if first_few_lines:
                if first_few_lines[0].upper().strip().startswith("### TITLE ###"):
                    if len(first_few_lines) > 1 and not first_few_lines[
                        1
                    ].upper().strip().startswith("###"):
        ...
        if "name" in parsed_custom_data and parsed_custom_data["name"]:
            ...
            logger.debug(
                "Custom plain text parsing did not yield a valid 'name' or was empty."
            )
    except Exception as e_custom:
        logger.opt(exception=True).error(
            f"Error parsing prompt file as custom plain text: {e_custom}"
        )
        return None
...
async def _prompt_import_callback(
    app: "TldwCli", selected_path: Optional[Path]
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    if selected_path:
        ...
        try:
            ...
            if not prompt_data:
                app.notify(
                    "Failed to parse prompt file. Invalid format or missing 'name'.",
                    severity="error",
                    timeout=7,
                )
                return
            ...
            prompt_id, prompt_uuid, msg = prompts_interop.add_prompt(
                name=prompt_data.get(
                    "name", "Unnamed Prompt"
                ),  # name is required by _parse_prompt...
            ...
            if (
                prompt_id is not None or prompt_uuid is not None
            ):  # Check if save was successful (id or uuid exists)
            ...
            app.notify(
                f"Error importing prompt: {type(e).__name__}",
                severity="error",
                timeout=6,
            )
            logger.opt(exception=True).error(
                f"Error importing prompt from '{selected_path}': {e}"
            )
    else:
...
async def handle_ccp_import_prompt_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    defined_filters = Filters(
        (
            "Prompt files (TXT, MD, JSON, YAML)",
            lambda p: p.suffix.lower() in (".txt", ".md", ".json", ".yaml", ".yml"),
        ),
        ("JSON files (*.json)", lambda p: p.suffix.lower() == ".json"),
        ("YAML files (*.yaml, *.yml)", lambda p: p.suffix.lower() in (".yaml", ".yml")),
...
########################################################################################################################
async def handle_ccp_editor_prompt_save_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    """Handles saving a new or existing prompt from the CENTER PANE editor."""
    logger = getattr(app, "loguru_logger", loguru_logger)
    logger.info(
        "CCP Editor Save Prompt button pressed (saves from center pane editor)."
    )
    if not app.prompts_service_initialized:
        app.notify("Prompts service not available.", severity="error")
        return
    ...
    try:
        ...
        if not name:
            app.notify(
                "Prompt Name is required for editor save.", severity="error", timeout=4
            )
        ...
        if app.current_prompt_id is None:  # Saving a NEW prompt via center editor
            ...
            logger.info(
                f"CCP Editor: Updating prompt ID: {app.current_prompt_id}, Name: {name}"
            )
            update_payload = {
                "name": name,
                "author": author,
        ...
        if saved_id is not None or saved_uuid is not None:
            ...
            app.notify(
                f"Prompt Editor: Failed to save prompt: {message_from_save or 'Unknown error'}",
                severity="error",
            )
        ...
        logger.opt(exception=True).error(
            f"CCP Editor Save Prompt: UI component error: {e_query}"
        )
        ...
        app.notify(
            f"Editor: Error saving prompt: {type(e_save).__name__}", severity="error"
        )


async def handle_ccp_editor_prompt_clone_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    if not app.prompts_service_initialized or app.current_prompt_id is None:
        app.notify(
            "No prompt loaded in editor to clone or service unavailable.",
            severity="warning",
        )
        return
    ...
    try:
        ...
        if cloned_id:
            app.notify(
                f"Editor: Prompt cloned as '{cloned_name}'. {msg_clone}",
                severity="information",
            )
            await populate_ccp_prompts_list_view(app)
        ...
        app.notify(
            f"Editor: Error cloning prompt: {type(e_clone).__name__}", severity="error"
        )


async def handle_ccp_editor_prompt_delete_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    if not app.prompts_service_initialized or app.current_prompt_id is None:
        app.notify(
            "No prompt loaded in editor to delete or service unavailable.",
            severity="warning",
        )
        return
    ...
    try:
        ...
        if success:
            app.notify(
                f"Editor: Prompt '{app.current_prompt_name or 'selected'}' deleted.",
                severity="information",
            )
            ...
            app.notify(
                "Editor: Failed to delete prompt. It might have been already deleted.",
                severity="error",
            )
            await populate_ccp_prompts_list_view(app)
            if (
                app.current_prompt_id == prompt_id_to_delete
            ):  # If it was the one we tried to delete
                app._clear_prompt_fields()
                app.ccp_active_view = "conversation_details_view"

    except prompts_interop.ConflictError as e_cf_del:
        logger.opt(exception=True).error(
            f"CCP Editor: Conflict deleting prompt: {e_cf_del}"
        )
        app.notify(
            f"Editor: Conflict error deleting prompt: {e_cf_del}", severity="error"
        )
    except prompts_interop.DatabaseError as e_db_del:
        logger.opt(exception=True).error(
            f"CCP Editor: Database error deleting prompt: {e_db_del}"
        )
        app.notify(
            f"Editor: Database error deleting prompt: {type(e_db_del).__name__}",
            severity="error",
        )
    except Exception as e_del:
        logger.opt(exception=True).error(
            f"CCP Editor: Unexpected error deleting prompt: {e_del}"
        )
        app.notify(
            f"Editor: Unexpected error deleting prompt: {type(e_del).__name__}",
            severity="error",
        )
...
async def handle_ccp_editor_char_save_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        character_version = app.query_one(
        ...
        if (
            hasattr(app, "temp_character_image_bytes")
            and app.temp_character_image_bytes
        ):
        ...
        current_editing_data = cast(
            Optional[Dict[str, Any]], app.current_editing_character_data
        )
        ...
        if current_editing_id is None:  # New character
            ...
            if new_character_id:
                logger.info(
                    f"New character '{char_name}' added. ID: {new_character_id}"
                )
                app.notify(
                ...
                logger.error(
                    f"Failed to save new character '{char_name}'. DB response: {new_character_id}"
                )
                app.notify(
                    f"Failed to save new character '{char_name}'.", severity="error"
                )
                return
        else:  # Existing character
            logger.info(
                f"Attempting to update character ID: {current_editing_id}, Name: {char_name}"
            )
            if not current_editing_data:
                logger.error(
                    f"Cannot update character {current_editing_id}: current editing data is missing."
                )
                app.notify(
                    "Error: Current character data is missing. Please reload.",
                    severity="error",
                )
                return
            ...
            if current_version is None:
                logger.error(
                    f"Cannot update character {current_editing_id}: version is missing from loaded data."
                )
                app.notify(
                    "Error: Character version is missing. Please reload and try again.",
                    severity="error",
                )
                return
            ...
            if saved_character_details:
                logger.info(
                    f"Character '{char_name}' (ID: {current_editing_id}) updated successfully."
                )
                app.notify(
                    f"Character '{char_name}' updated successfully.",
                    severity="information",
                )
                # Clear temporary image bytes after successful save
                if hasattr(app, "temp_character_image_bytes"):
                ...
                logger.error(
                    f"Failed to update character '{char_name}'. DB response: {saved_character_details}"
                )
                app.notify(
                    f"Failed to update character '{char_name}'.", severity="error"
                )
                return
        ...
        if saved_character_details and saved_character_details.get("id"):
            ...
            try:
                ...
                logger.error(
                    "Failed to find #ccp-editor-char-cancel-button to add 'hidden' class post-save."
                )
        else:
            # This case should ideally not be reached if errors are returned above.
            logger.warning(
                "Save/Update operation completed but no valid character details received."
            )
            # Optionally, try to reload current if update failed but no specific error caught
            if current_editing_id:
                await _helper_ccp_load_character_into_center_pane_editor(
                    app, current_editing_id
                )

    except ConflictError as e_conflict:
        logger.opt(exception=True).warning(
            f"Conflict saving character '{char_name}': {e_conflict}"
        )
        app.notify(
            "Save conflict: Data was modified elsewhere. Please reload and try again.",
            severity="error",
            timeout=7,
        )
        if app.current_editing_character_id:  # Reload to show current state from DB
            await _helper_ccp_load_character_into_center_pane_editor(
                app, app.current_editing_character_id
            )
    except CharactersRAGDBError as e_db:
        logger.opt(exception=True).error(
            f"Database error saving character '{char_name}': {e_db}"
        )
        app.notify(
            f"Database error saving character: {type(e_db).__name__}", severity="error"
        )
    except QueryError as e_query:
        logger.opt(exception=True).error(
            f"UI component error saving character: {e_query}"
        )
        app.notify(
            "UI Error: Could not access character editor fields.", severity="error"
        )
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error saving character '{char_name}': {e_unexp}"
        )
        app.notify(
            f"An unexpected error occurred: {type(e_unexp).__name__}", severity="error"
        )
...
async def _helper_ccp_clear_center_pane_character_editor_fields(app: "TldwCli") -> None:
    ...
    try:
        ...
        loguru_logger.error(
            f"Error clearing CCP center pane character editor fields: {e}"
        )
    except Exception as e_clear:
        loguru_logger.opt(exception=True).error(
            f"Unexpected error clearing CCP center pane character editor fields: {e_clear}"
        )


async def _helper_ccp_load_character_into_center_pane_editor(
    app: "TldwCli", character_id: str
) -> None:
    ...
    if not app.chachanotes_db:  # Use the correct DB instance
        ...
        loguru_logger.error(
            "ChaChaNotes DB not available for loading character into editor."
        )
        return
    ...
    try:
        ...
        if char_data:
            ...
            app.query_one("#ccp-editor-char-name-input", Input).value = char_data.get(
                "name", ""
            )
            app.query_one("#ccp-editor-char-avatar-input", Input).value = char_data.get(
                "image_path", char_data.get("avatar", "")
            )  # Use image_path or avatar
            app.query_one(
                "#ccp-editor-char-description-textarea", TextArea
            ).text = char_data.get("description", "")
            app.query_one(
                "#ccp-editor-char-personality-textarea", TextArea
            ).text = char_data.get("personality", "")
            app.query_one(
                "#ccp-editor-char-scenario-textarea", TextArea
            ).text = char_data.get("scenario", "")
            app.query_one(
                "#ccp-editor-char-first-message-textarea", TextArea
            ).text = char_data.get("first_message", char_data.get("first_mes", ""))
            ...
            app.query_one(
                "#ccp-editor-char-creator-notes-textarea", TextArea
            ).text = char_data.get("creator_notes", "")
            app.query_one(
                "#ccp-editor-char-system-prompt-textarea", TextArea
            ).text = char_data.get("system_prompt", "")
            app.query_one(
                "#ccp-editor-char-post-history-instructions-textarea", TextArea
            ).text = char_data.get("post_history_instructions", "")
            ...
            ).value = char_data.get("creator", "")
            app.query_one(
                "#ccp-editor-char-version-input", Input
            ).value = char_data.get("character_version", "")
            ...
            app.query_one("#ccp-editor-char-name-input", Input).focus()
            app.notify(
                f"Character '{char_data.get('name', 'Unknown')}' loaded into center editor.",
                severity="information",
            )
            loguru_logger.info(
                f"Loaded character '{char_data.get('name', 'Unknown')}' (ID: {char_data.get('id')}) into CCP center editor."
            )
        else:
            app.notify(
                f"Failed to load character details for ID: {character_id} into editor.",
                severity="error",
            )
            await _helper_ccp_clear_center_pane_character_editor_fields(app)
            app.current_editing_character_id = None
            ...
            loguru_logger.warning(
                f"Character details not found for ID {character_id} for editor."
            )

    except CharactersRAGDBError as e_db:
        loguru_logger.opt(exception=True).error(
            f"Database error loading character for CCP editing (ID: {character_id}): {e_db}"
        )
        app.notify(
            f"Database error loading character: {type(e_db).__name__}", severity="error"
        )
        await _helper_ccp_clear_center_pane_character_editor_fields(app)
        app.current_editing_character_id = None
        app.current_editing_character_data = None
        ...
        loguru_logger.opt(exception=True).error(
            f"UI component error loading character for CCP editing (ID: {character_id}): {e_query}"
        )
        app.notify(
            "UI Error: Could not populate character editor fields.", severity="error"
        )
    except Exception as e:
        loguru_logger.opt(exception=True).error(
            f"Unexpected error loading character for CCP editing (ID: {character_id}): {e}"
        )
        app.notify(
            f"Error loading character into editor: {type(e).__name__}", severity="error"
        )
        await _helper_ccp_clear_center_pane_character_editor_fields(app)
        app.current_editing_character_id = None
        app.current_editing_character_data = None
...
async def handle_ccp_editor_char_clone_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    current_editing_id = app.current_editing_character_id
    current_editing_data_value = cast(
        Optional[Dict[str, Any]], app.current_editing_character_data
    )

    if current_editing_id is None or current_editing_data_value is None:
        app.notify(
            "No character loaded in editor to clone. Please load a character first.",
            severity="warning",
            timeout=7,
        )
        logger.warning(
            "Clone Character from editor: No character data loaded in editor's state."
        )
        return
    ...
    try:
        ...
        original_data = (
            current_editing_data_value  # This is already Optional[Dict[str, Any]]
        )
        ...
        cloned_character_data_for_db: Dict[str, Any] = {
            ...
            if key not in ["id", "version", "uuid", "created_at", "last_modified"]
        }
        cloned_character_data_for_db["name"] = cloned_name  # Set the new name

        # Deep copy mutable fields if they exist
        if "alternate_greetings" in cloned_character_data_for_db and isinstance(
            cloned_character_data_for_db["alternate_greetings"], list
        ):
            cloned_character_data_for_db["alternate_greetings"] = list(
                cloned_character_data_for_db["alternate_greetings"]
            )
        if "tags" in cloned_character_data_for_db and isinstance(
            cloned_character_data_for_db["tags"], list
        ):
            cloned_character_data_for_db["tags"] = list(
                cloned_character_data_for_db["tags"]
            )
        if "extensions" in cloned_character_data_for_db and isinstance(
            cloned_character_data_for_db["extensions"], dict
        ):
            cloned_character_data_for_db["extensions"] = dict(
                cloned_character_data_for_db["extensions"]
            )

        logger.info(
            f"Attempting to clone character '{original_name}' as '{cloned_name}' from editor state."
        )
        ...
        if new_cloned_char_id:
            ...
            await _helper_ccp_load_character_into_center_pane_editor(
                app, str(new_cloned_char_id)
            )
            await populate_ccp_character_select(app)
        else:
            logger.error(
                f"Failed to save cloned character '{cloned_name}'. DB returned: {new_cloned_char_id}"
            )
            app.notify(
                f"Failed to clone character '{original_name}'.", severity="error"
            )

    except CharactersRAGDBError as e_db:
        logger.opt(exception=True).error(
            f"Database error cloning character from editor: {e_db}"
        )
        app.notify(f"Database error cloning: {type(e_db).__name__}", severity="error")
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error cloning character from editor: {e_unexp}"
        )
        app.notify(
            f"Unexpected error cloning: {type(e_unexp).__name__}", severity="error"
        )


async def handle_ccp_editor_char_cancel_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        if app.current_editing_character_id is not None:
            ...
            logger.info(
                f"Cancelling edit for existing character ID: {stored_character_id}. Restoring card view."
            )

            if not app.chachanotes_db:
                app.notify(
                    "Database service not available to restore character.",
                    severity="error",
                )
                logger.error(
                    "ChaChaNotes DB not available to restore character for card view."
                )
            ...
            try:
                ...
                original_char_data, _, original_char_image_pil = (
                ...
                if original_char_data:
                    ...
                    app.ccp_active_view = "character_card_view"
                    app.notify(
                        "Character editing cancelled. Displaying original card.",
                        severity="information",
                    )
                else:
                    # Failed to reload original data, fallback to clearing editor and neutral view
                    app.notify(
                        "Could not reload original character data. Clearing editor.",
                        severity="warning",
                    )
                    logger.warning(
                        f"Failed to reload original data for char ID {stored_character_id} on cancel."
                    )
                    await _helper_ccp_clear_center_pane_character_editor_fields(app)
                ...
                logger.opt(exception=True).error(
                    f"Error reloading original character data (ID: {stored_character_id}) on cancel: {e_load}"
                )
                app.notify(
                    "Error restoring character view. Clearing editor.", severity="error"
                )
                await _helper_ccp_clear_center_pane_character_editor_fields(app)
                app.ccp_active_view = "conversation_messages_view"  # Fallback
            ...
            logger.info(
                "Cancelling creation of new character. Clearing fields and switching view."
            )
            await _helper_ccp_clear_center_pane_character_editor_fields(app)
            ...
            if (
                app.current_ccp_character_details
                and app.current_ccp_character_details.get("id")
            ):
                app.ccp_active_view = (
                    "character_card_view"  # Show previous card if one was loaded
                )
            else:
        ...
        logger.opt(exception=True).error(
            f"UI component error during cancel character edit (querying cancel button): {e_query}"
        )
        app.notify(
            "UI Error: Could not properly cancel character edit.", severity="error"
        )
        # Attempt to recover by switching to a default view
        app.ccp_active_view = "conversation_messages_view"
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error during cancel character edit: {e_unexp}"
        )
        app.notify(
            f"An unexpected error occurred: {type(e_unexp).__name__}", severity="error"
        )
...
async def handle_ccp_editor_char_delete_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    character_name_to_delete = (
        current_editing_data_value.get("name", "the loaded character")
        if current_editing_data_value
        else "the loaded character"
    )
    ...
    try:
        logger.info(
            f"Attempting to delete character ID from editor: {character_id_to_delete} (Name: '{character_name_to_delete}')."
        )
        db = app.chachanotes_db  # Already checked

        success = db.delete_character_card(
            character_id=character_id_to_delete
        )  # delete_character_card expects string ID

        if success:
            logger.info(
                f"Character {character_id_to_delete} deleted successfully from editor."
            )
            app.notify(
                f"Character '{character_name_to_delete}' deleted.",
                severity="information",
            )
            ...
            logger.error(
                f"Failed to delete character '{character_name_to_delete}' (ID: {character_id_to_delete}) from editor. DB returned False."
            )
            app.notify(
                f"Failed to delete character '{character_name_to_delete}'. It might have already been deleted.",
                severity="error",
                timeout=7,
            )
            await populate_ccp_character_select(app)
        ...
        logger.opt(exception=True).error(
            f"Conflict error deleting character {character_id_to_delete} from editor: {e_conflict}"
        )
        app.notify(f"Conflict deleting character: {e_conflict}", severity="error")
        await populate_ccp_character_select(app)
    except CharactersRAGDBError as e_db:
        logger.opt(exception=True).error(
            f"Database error deleting character {character_id_to_delete} from editor: {e_db}"
        )
        app.notify(f"Database error deleting: {type(e_db).__name__}", severity="error")
    except Exception as e_unexp:
        logger.opt(exception=True).error(
            f"Unexpected error deleting character {character_id_to_delete} from editor: {e_unexp}"
        )
        app.notify(
            f"Unexpected error deleting: {type(e_unexp).__name__}", severity="error"
        )


async def handle_ccp_editor_char_image_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    async def _image_upload_callback(
        app: "TldwCli", selected_path: Optional[str]
    ) -> None:
        if selected_path:
            ...
            try:
                ...
                try:
                    status_widget = app.query_one(
                        "#ccp-editor-char-image-status", Static
                    )
                    filename = Path(selected_path).name
                    status_widget.update(
                        f"Selected: {filename} ({len(image_bytes) // 1024}KB)"
                    )
                except QueryError:
                    logger.error("Could not find image status widget")

                app.notify(
                    f"Image selected: {Path(selected_path).name}",
                    severity="information",
                )
...
async def handle_ccp_editor_char_clear_image_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def _finish_new_prompt_setup(app: "TldwCli") -> None:
    ...
    await app._load_prompt_for_editing(
        prompt_id=None
    )  # Call with None to clear fields for a new prompt
    app.notify(
        "Ready to create a new prompt in the center editor.", severity="information"
    )
...
async def handle_ccp_tab_sidebar_toggle(app: "TldwCli", event: Button.Pressed) -> None:
    ...
    if button_id == "toggle-conv-char-left-sidebar":
        ...
        logger.warning(
            f"Unhandled sidebar toggle button ID '{button_id}' in CCP tab handler."
        )


async def handle_ccp_card_edit_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    logger.info("CCP Card Edit Character button pressed.")

    current_card_details = cast(
        Optional[Dict[str, Any]], app.current_ccp_character_details
    )
...
def _get_llm_settings_for_generation(
    app: "TldwCli",
    ...
    if provider:
        ...
        provider_config_settings = app.app_config.get("api_settings", {}).get(
        ...
        if not api_key:
            ...
            if env_var_name:
                ...
                if env_api_key:
                    api_key = env_api_key
                    logger.debug(
                        f"Using API key for '{provider}' from ENV var '{env_var_name}'."
                    )

    return provider, model, api_key


async def handle_ccp_generate_description_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        if not provider:
            app.notify(
                "No API provider configured. Please configure in chat settings or config file.",
                severity="warning",
            )
            return
        ...
        app.notify(
            "Generating description... This may take a moment.", severity="information"
        )
...
async def handle_ccp_generate_personality_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def handle_ccp_generate_scenario_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def handle_ccp_generate_first_message_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        app.notify(
            "Generating first message... This may take a moment.",
            severity="information",
        )
...
async def handle_ccp_generate_system_prompt_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        app.notify(
            "Generating system prompt... This may take a moment.",
            severity="information",
        )
...
async def handle_ccp_generate_all_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        app.notify(
            "Generating complete character profile... This may take up to a minute.",
            severity="information",
        )
...
async def handle_conv_char_export_text_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        text_content = ccl.export_conversation_to_text(
            db, app.current_ccp_conversation_id, user_name
        )
        ...
        if conv_details:
            title = (
                conv_details.get("title", "conversation")
                .replace("/", "_")
                .replace("\\", "_")
            )
        ...
        app.notify(
            f"Conversation exported to: {export_path}",
            severity="information",
            timeout=10,
        )
        ...
        app.notify(
            f"Error exporting conversation: {type(e).__name__}", severity="error"
        )


async def handle_conv_char_export_json_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        json_content = ccl.export_conversation_to_json(
            db, app.current_ccp_conversation_id, include_character_card=True
        )
...
async def handle_ccp_export_character_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        json_content = ccl.export_character_card_to_json(
            db, character_id, include_image=True
        )
        if json_content:
            json_filename = (
                f"{char_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            ...
            if char_data.get("image"):
                ...
                png_filename = (
                    f"{char_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                )
                png_path = export_dir / png_filename
                success = ccl.export_character_card_to_png(
                    db, character_id, str(png_path)
                )
                ...
                if success:
                    ...
                    app.notify(
                        f"Character JSON exported to: {json_path}\nPNG export failed.",
                        severity="warning",
                        timeout=10,
                    )
            else:
                ...
                app.notify(
                    f"Character exported to:\n{json_path}\n(No image available - exported as JSON only)",
                    severity="information",
                    timeout=10,
                )
...
async def populate_ccp_dictionary_select(app: "TldwCli") -> None:
    ...
    if not app.notes_service:
        logger.error(
            "Notes service not available, cannot populate dictionary select for CCP tab."
        )
        try:
    ...
    try:
        ...
        if options:
            ...
            logger.info(
                f"Populated #ccp-dictionary-select with {len(options)} dictionaries."
            )
        else:
        ...
        logger.opt(exception=True).error(
            f"Failed to find #ccp-dictionary-select widget: {e_query}"
        )
    except CharactersRAGDBError as e_db:
        logger.opt(exception=True).error(
            f"Database error populating #ccp-dictionary-select: {e_db}"
        )
        try:
            dict_select_widget_err = app.query_one("#ccp-dictionary-select", Select)
            dict_select_widget_err.set_options(
                [("Error Loading Dictionaries", Select.BLANK)]
            )
        ...
        logger.opt(exception=True).error(
            f"Unexpected error populating #ccp-dictionary-select: {e_unexp}"
        )
        try:
            dict_select_widget_unexp = app.query_one("#ccp-dictionary-select", Select)
            dict_select_widget_unexp.set_options(
                [("Error Loading (Unexpected)", Select.BLANK)]
            )
...
async def _dictionary_import_callback(
    app: "TldwCli", selected_path: Optional[Path]
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    if selected_path:
        ...
        try:
            ...
            if dict_id is not None:
                app.notify(
                    f"Dictionary imported successfully (ID: {dict_id}).",
                    severity="information",
                )
            ...
            await populate_ccp_dictionary_select(
                app
            )  # Refresh in case it was a duplicate name
        except Exception as e:
            app.notify(
                f"Error importing dictionary: {type(e).__name__}",
                severity="error",
                timeout=6,
            )
            logger.opt(exception=True).error(
                f"Error importing dictionary '{selected_path}': {e}"
            )
    else:
        logger.info("Dictionary import cancelled.")


async def handle_ccp_import_dictionary_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    defined_filters = Filters(
        (
            "Dictionary files (MD, YAML)",
            lambda p: p.suffix.lower() in (".md", ".yaml", ".yml"),
        ),
        ("Markdown files (*.md)", lambda p: p.suffix.lower() == ".md"),
        ("YAML files (*.yaml, *.yml)", lambda p: p.suffix.lower() in (".yaml", ".yml")),
        ("All files (*.*)", lambda p: True),
...
async def handle_ccp_create_dictionary_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    try:
        ...
        app.query_one(
            "#ccp-editor-dict-strategy-select", Select
        ).value = "sorted_evenly"
...
async def handle_ccp_load_dictionary_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    try:
        ...
        if dict_data:
            ...
            app.query_one(
                "#ccp-dict-description-display", TextArea
            ).text = dict_data.get("description", "")
            app.query_one("#ccp-dict-strategy-display", Static).update(
                dict_data.get("strategy", "sorted_evenly")
            )
            app.query_one("#ccp-dict-max-tokens-display", Static).update(
                str(dict_data.get("max_tokens", 1000))
            )
            ...
            for entry in dict_data.get("entries", []):
                entry_text = (
                    f"{entry.raw_key} → {entry.content[:50]}..."
                    if len(entry.content) > 50
                    else f"{entry.raw_key} → {entry.content}"
                )
            ...
            app.notify(
                f"Loaded dictionary: {dict_data['name']}", severity="information"
            )
...
async def handle_ccp_refresh_dictionary_list_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
...
async def handle_ccp_dict_edit_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    try:
        ...
        if dict_data:
            ...
            app.query_one("#ccp-editor-dict-name-input", Input).value = dict_data[
                "name"
            ]
            app.query_one(
                "#ccp-editor-dict-description-textarea", TextArea
            ).text = dict_data.get("description", "")
            app.query_one(
                "#ccp-editor-dict-strategy-select", Select
            ).value = dict_data.get("strategy", "sorted_evenly")
            app.query_one("#ccp-editor-dict-max-tokens-input", Input).value = str(
                dict_data.get("max_tokens", 1000)
            )
            ...
            app.notify(
                f"Editing dictionary: {dict_data['name']}", severity="information"
            )
        else:
...
async def handle_ccp_editor_dict_save_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    try:
        ...
        max_tokens = int(
            app.query_one("#ccp-editor-dict-max-tokens-input", Input).value or "1000"
        )
        ...
        if hasattr(app, "editing_dictionary_id") and app.editing_dictionary_id:
            ...
            if success:
                app.notify(
                    f"Dictionary '{name}' updated successfully.", severity="information"
                )
                app.loaded_dictionary_id = app.editing_dictionary_id
                app.editing_dictionary_id = None
                # Switch back to view mode
                app.switch_ccp_center_view("dictionary")
                await handle_ccp_load_dictionary_button_pressed(
                    app, event
            ...
            if dict_id:
                app.notify(
                    f"Dictionary '{name}' created successfully.", severity="information"
                )
...
async def handle_ccp_dict_export_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    try:
        ...
        if export_path:
            app.notify(
                f"Dictionary exported to: {export_path}",
                severity="information",
                timeout=8,
            )
        else:
...
async def handle_ccp_dict_delete_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
...
async def handle_ccp_dict_add_entry_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
    ...
    try:
        ...
        group = (
            app.query_one("#ccp-dict-entry-group-input", Input).value.strip() or None
        )
        probability = int(
            app.query_one("#ccp-dict-entry-probability-input", Input).value or "100"
        )
        ...
        entry = cdl.ChatDictionary(
            key=key, content=value, probability=probability, group=group
        )
...
async def handle_ccp_dict_remove_entry_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
...
async def handle_ccp_editor_dict_cancel_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    logger = getattr(app, "loguru_logger", loguru_logger)
...
async def handle_ccp_dict_clone_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        if dict_data:
            ...
            if dict_id:
                app.notify(
                    f"Dictionary cloned as '{clone_name}'.", severity="information"
                )
                await populate_ccp_dictionary_select(app)  # Refresh dropdown
...
async def handle_ccp_dict_update_priority_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    app.notify(
        "Dictionary priority ordering not yet implemented.", severity="information"
    )
...
async def handle_ccp_import_worldbook_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def handle_ccp_create_worldbook_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
    ...
    try:
        ...
        app.notify(
            f"Created new world book with ID {world_book_id}", severity="success"
        )
...
async def handle_ccp_worldbook_search_input_changed(
    app: "TldwCli", event: Input.Changed
) -> None:
...
async def handle_ccp_worldbook_load_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def handle_ccp_worldbook_edit_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
...
async def handle_ccp_refresh_worldbook_list_button_pressed(
    app: "TldwCli", event: Button.Pressed
) -> None:
        ...
        if not filtered_books:
            if search_term:
                await listview.append(
                    ListItem(Label("No world books found matching search"))
                )
            else:
