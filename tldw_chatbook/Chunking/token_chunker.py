...
class TransformersTokenizer:
    ...
    def __init__(self, tokenizer_name_or_path: str = "gpt2"):
        ...
        if not self.available:
            logger.debug(
                "transformers not available, token-based chunking will be disabled"
            )
    ...
    def tokenizer(self):
        ...
        if self._tokenizer is None:
            ...
            try:
                ...
                logger.debug(
                    f"Tokenizer {self.tokenizer_name_or_path} loaded successfully"
                )
            except Exception as e:
                logger.error(
                    f"Failed to load tokenizer '{self.tokenizer_name_or_path}': {e}"
                )
...
class TokenBasedChunker:
    ...
    def chunk_by_tokens(
        ...
        if max_tokens > MAX_CHUNK_SIZE_TOKENS:
            raise ValueError(
                f"max_tokens {max_tokens} exceeds maximum allowed {MAX_CHUNK_SIZE_TOKENS}"
            )
        ...
        if overlap_tokens >= max_tokens:
            raise ValueError(
                f"Token overlap {overlap_tokens} must be less than max_tokens {max_tokens}"
            )
        ...
        try:
            ...
            for i in range(0, len(tokens), step):
                ...
                logger.debug(
                    f"Created token chunk {len(chunks)} with {len(chunk_tokens)} tokens"
                )
    ...
    def _chunk_by_word_approximation(
    ...
    def is_transformers_available(self) -> bool:
        ...
        return (
            isinstance(self.tokenizer, TransformersTokenizer)
            and self.tokenizer.available
        )
