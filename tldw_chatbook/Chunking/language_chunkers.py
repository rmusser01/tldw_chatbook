...
class ChineseChunker:
    ...
    def __init__(self):
        ...
        if not self.available:
            logger.debug(
                "jieba not available, Chinese chunking will fall back to space splitting"
            )
    ...
    def tokenize_words(self, text: str) -> List[str]:
        ...
        if not self.available:
            logger.debug(
                "jieba not available, falling back to character splitting for Chinese"
            )
        ...
        try:
            ...
            logger.warning(
                f"jieba tokenization failed: {e}, falling back to character splitting"
            )
            return list(text.replace(" ", "").replace("\n", "").replace("\t", ""))
...
class JapaneseChunker:
    ...
    def __init__(self):
        ...
        if self.fugashi:
            ...
            logger.debug(
                "fugashi not available, Japanese chunking will fall back to space splitting"
            )
    ...
    def tokenize_words(self, text: str) -> List[str]:
        ...
        if not self.available:
            logger.debug(
                "fugashi not available, falling back to character splitting for Japanese"
            )
        ...
        try:
            ...
            logger.warning(
                f"fugashi tokenization failed: {e}, falling back to character splitting"
            )
            return list(text.replace(" ", "").replace("\n", "").replace("\t", ""))
    ...
    def tokenize_sentences(self, text: str) -> List[str]:
        ...
        if self.nltk:
            try:
                ...
                logger.warning(
                    f"NLTK punkt tokenizer not found for language '{self.language}', using basic fallback"
                )
            except Exception as e:
                logger.warning(
                    f"NLTK sentence tokenization failed: {e}, using basic fallback"
                )
