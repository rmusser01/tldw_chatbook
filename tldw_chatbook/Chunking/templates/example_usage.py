...
def example_custom_template():
    ...
    template = ChunkingTemplate(
        ...
        pipeline=[
            ChunkingStage(
                stage="preprocess",
                operations=[ChunkingOperation(type="normalize_whitespace", params={})],
            ),
            ChunkingStage(
                stage="chunk", method="words", options={"max_size": 50, "overlap": 10}
            ),
            ChunkingStage(
                stage="postprocess",
                operations=[
                    ChunkingOperation(type="add_context", params={"context_size": 1})
                ],
            ),
        ],
        metadata={"created_by": "example_script", "purpose": "demonstration"},
    )
...
def example_domain_specific_template():
    ...
    try:
        chunks = improved_chunking_process(academic_text, template="academic_paper")
...
def example_custom_operation():
    ...
    template = ChunkingTemplate(
        ...
        pipeline=[
            ChunkingStage(stage="chunk", method="sentences", options={"max_size": 2}),
            ChunkingStage(
                stage="postprocess",
                operations=[
                    ChunkingOperation(
                        type="add_line_numbers", params={"prefix": "Chunk"}
                    )
                ],
            ),
        ],
