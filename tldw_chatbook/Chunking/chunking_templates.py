...
class ChunkingTemplateManager:
    ...
    def __init__(
        ...
        logger.info(
            f"ChunkingTemplateManager initialized with templates_dir: {self.templates_dir}"
        )
    ...
    def _merge_templates(
        self, parent: ChunkingTemplate, child: ChunkingTemplate
    ) -> ChunkingTemplate:
        ...
        if "pipeline" in child_data:
            ...
            for stage in child_data["pipeline"]:
                ...
                if stage_name in parent_stages:
                    # Merge stage options
                    parent_stages[stage_name]["options"].update(
                        stage.get("options", {})
                    )
    ...
    # Built-in operations
    def _op_normalize_whitespace(
        self, text: str, chunks: List[str], options: Dict[str, Any]
    ) -> str:
    ...
    def _op_section_detection(
        ...
        if headers:
            ...
            header_regex = re.compile(
                rf"^({pattern})\s*$", re.MULTILINE | re.IGNORECASE
            )
...
class ChunkingPipeline:
    ...
    def execute(
        self, text: str, template: ChunkingTemplate, chunker_instance: Any, **kwargs
    ) -> List[Dict[str, Any]]:
        """
            ...
            try:
                ...
                if "max_size" in temp_options and "overlap" in temp_options:
                    ...
                    if overlap >= max_size:
                        ...
                        logger.debug(
                            f"Adjusted overlap from {overlap} to {temp_options['overlap']} to be less than max_size {max_size}"
                        )

                # Update chunker options temporarily
                chunker_instance.options = temp_options
                chunks = chunker_instance.chunk_text(
                    processed_text, method=method, use_template=False
                )
            finally:
    ...
    def _execute_preprocess(self, text: str, stage: ChunkingStage) -> str:
        ...
        for operation in stage.operations:
            if self.template_manager.evaluate_condition(
                operation.condition, self.context
            ):
    ...
    def _execute_chunk(
        self, text: str, stage: ChunkingStage, chunker_instance: Any
    ) -> List[Any]:
        ...
        try:
            ...
            chunks = chunker_instance.chunk_text(
                text, method=method, use_template=False
            )
        finally:
            # Restore original options
            chunker_instance.options = original_options
    ...
    def _execute_postprocess(
        self, text: str, chunks: List[str], stage: ChunkingStage
    ) -> List[str]:
    ...
    def _format_chunks(
        self, chunks: List[Any], template: ChunkingTemplate
    ) -> List[Dict[str, Any]]:
