        ...
        with Horizontal(classes="config-search-result-item"):
            with Vertical(classes="result-info"):
                ...
                if self.current_value is not None:
                    ...
                    yield Label(
                        f"{type_str} • Value: [yellow]{value_str}[/yellow]",
                ...
                if self.widget_id:
                    yield Label(
                        f"[dim]ID: {self.widget_id}[/dim]", classes="result-context"
                    )
            ...
            with Horizontal(classes="result-actions"):
                yield Button(
                    "Go to",
                    id=f"goto_{id(self.widget)}",
                    classes="result-goto-btn",
                    variant="primary",
                )
...
class UIElementSearchEngine:
    ...
    def _scan_widget(self, widget: Widget, depth: int = 0) -> None:
        ...
        if (
            widget.id == "config-search-container"
            or widget.id == "config-search-results-list"
        ):
            return
