    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="result-card-wrapper"):
            ...
            with Vertical(classes="result-content-column"):
                ...
                with Container(
                    id=f"expanded-{self.index}",
                    classes="result-expanded-content hidden",
                ):
                    # Full content
                    yield Static(
                        "[bold]Full Content:[/bold]", classes="expanded-section-title"
                    )
                    ...
                    if self.result.get("citations"):
                        yield Static(
                            "[bold]Sources:[/bold]", classes="expanded-section-title"
                        )
                        ...
                        for doc_title, doc_citations in citations_by_doc.items():
                            yield Static(
                                f"📄 {doc_title}", classes="citation-doc-title"
                            )
                            for citation in doc_citations:
                                match_type = citation.get("match_type", "unknown")
                                confidence = citation.get("confidence", 0)
                                text_snippet = (
                                    citation.get("text", "")[:100] + "..."
                                    if len(citation.get("text", "")) > 100
                                    else citation.get("text", "")
                                )

                                citation_text = f"  • [{match_type}] ({confidence:.0%}) {text_snippet}"
                                yield Static(
                                    citation_text,
                                    classes=f"citation-item citation-{match_type}",
                                )

                    # Full metadata
                    if self.result.get("metadata"):
                        yield Static(
                            "[bold]All Metadata:[/bold]",
                            classes="expanded-section-title",
                        )
                        for key, value in self.result["metadata"].items():
                            yield Static(
                                f"• {key}: {value}", classes="metadata-full-item"
                            )
                ...
                with Horizontal(classes="result-actions-enhanced"):
                    ...
                    yield Button(
                        "📋 Copy", id=f"copy-{self.index}", classes="result-button"
                    )
                    yield Button(
                        "📝 Note", id=f"add-note-{self.index}", classes="result-button"
                    )
                    yield Button(
                        "Use in Chat",
                        id=f"use-in-chat-{self.index}",
                        classes="result-button",
                    )
                    yield Button(
                        "Use in Console",
                        id=f"use-in-console-{self.index}",
                        classes="result-button",
                    )
                    yield Button(
                        "📤 Export", id=f"export-{self.index}", classes="result-button"
                    )
