...
SAVED_SEARCH_LOAD_ENABLED_TOOLTIP = (
    "Load the selected saved search into the search controls."
)
...
class SavedSearchesPanel(Container):
    ...
    def __init__(self):
        super().__init__(
            id="saved-searches-panel", classes="saved-searches-panel-enhanced"
        )
    ...
    def save_search(self, name: str, config: Dict[str, Any]) -> None:
        ...
        if sanitized_name in self.saved_searches:
            ...
            self.saved_searches[sanitized_name]["last_used"] = (
                datetime.now().isoformat()
            )
            self.saved_searches[sanitized_name]["updated_at"] = (
                datetime.now().isoformat()
            )
        ...
        if len(self.saved_searches) > 50:  # Arbitrary limit
            ...
            oldest_name = min(
                ...
                key=lambda k: datetime.fromisoformat(
                    self.saved_searches[k]["created_at"]
                ),
            )
    ...
    def _set_saved_search_action_state(self, *, selected: bool) -> None:
        ...
        load_button.tooltip = (
            SAVED_SEARCH_LOAD_ENABLED_TOOLTIP
            if selected
            else SAVED_SEARCH_LOAD_DISABLED_TOOLTIP
        )
        delete_button.tooltip = (
            SAVED_SEARCH_DELETE_ENABLED_TOOLTIP
            if selected
            else SAVED_SEARCH_DELETE_DISABLED_TOOLTIP
        )
    ...
    async def refresh_list(self) -> None:
        ...
        try:
            ...
            for name, data in self.saved_searches.items():
                created = datetime.fromisoformat(data["created_at"]).strftime(
                    "%Y-%m-%d %H:%M"
                )
                list_item = ListItem(
    ...
    def handle_load_saved_search(self, event: Button.Pressed) -> None:
        ...
        self._persist_saved_searches()
        self.post_message(
            self.LoadRequested(selected_name, saved_data.get("config", {}))
        )
