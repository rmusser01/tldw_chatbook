...
class EvaluationSetupView(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield SectionContainer(
            ...
            ActionButtonRow(
                [
                    ...
                    ButtonConfig(
                        "🔧 Configure", "configure-model", "default", disabled=True
                    ),
        ...
        yield SectionContainer(
            "Run Configuration", self._create_run_config_form(), id="run-config-section"
        )
    ...
    def _create_model_selector(self) -> Container:
        ...
        container._add_child(
            Select([("Select a model...", "")], id="model-select", value="")
        )
        return container
    ...
    def _create_run_config_form(self) -> ConfigurationForm:
        ...
        return ConfigurationForm(fields, id="run-config-form")
            ...
            if self.state.draft_config and self.state.draft_config.model_id:
                ...
                if model:
                    ...
                    cost_estimator.estimate_cost(
                        model.provider, model.model_id, samples
                    )
        except Exception as e:
...
class ResultsDashboardView(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="results-grid"):
            ...
            yield SectionContainer(
                "Quick Stats", self._create_quick_stats(), id="quick-stats-section"
            )
    ...
    def _update_quick_stats(self) -> None:
        ...
        try:
            ...
            if total_runs > 0:
                ...
                self.query_one("#stat-success-rate").update(
                    f"Success Rate: {avg_success_rate:.1f}%"
                )
                self.query_one("#stat-avg-duration").update(
    ...
    def compose(self) -> ComposeResult:
        ...
        yield SectionContainer(
            ...
            Container(
                ...
                Button(
                    "🧠 Anthropic", id="provider-anthropic", classes="provider-button"
                ),
        ...
        yield SectionContainer(
            ...
            ActionButtonRow(
                [
                    ...
                    ButtonConfig(
                        "📚 Browse Templates", "browse-model-templates", "default"
                    ),
                ]
            ),
            id="import-section",
...
class DatasetManagementView(Container):
        ...
        yield SectionContainer(
            ...
            ActionButtonRow(
                [
                    ...
                    ButtonConfig(
                        "👁️ Preview", "preview-dataset", "default", disabled=True
                    ),
    ...
    def _create_template_categories(self) -> Container:
        ...
        # Reasoning & Math
        categories._add_child(
            Static("🧮 Reasoning & Mathematics", classes="template-category-title")
        )
        math_grid = Container(classes="template-grid")
        math_grid._add_child(
            Button("GSM8K", id="template-gsm8k", classes="template-button")
        )
        math_grid._add_child(
            Button("MATH", id="template-math", classes="template-button")
        )
        math_grid._add_child(
            Button("LogiQA", id="template-logiqa", classes="template-button")
        )
        math_grid._add_child(
            Button("ARC", id="template-arc", classes="template-button")
        )
        math_grid._add_child(
            Button("HellaSwag", id="template-hellaswag", classes="template-button")
        )
        math_grid._add_child(
            Button("PIQA", id="template-piqa", classes="template-button")
        )
        categories._add_child(math_grid)

        # Code & Programming
        categories._add_child(
            Static("💻 Code & Programming", classes="template-category-title")
        )
        code_grid = Container(classes="template-grid")
        code_grid._add_child(
            Button("HumanEval", id="template-humaneval", classes="template-button")
        )
        code_grid._add_child(
            Button("MBPP", id="template-mbpp", classes="template-button")
        )
        code_grid._add_child(
            Button("CodeXGLUE", id="template-codexglue", classes="template-button")
        )
        code_grid._add_child(
            Button("DS-1000", id="template-ds1000", classes="template-button")
        )
        code_grid._add_child(
            Button("Spider", id="template-spider", classes="template-button")
        )
        code_grid._add_child(
            Button("CoNaLa", id="template-conala", classes="template-button")
        )
        categories._add_child(code_grid)

        # Safety & Alignment
        categories._add_child(
            Static("🛡️ Safety & Alignment", classes="template-category-title")
        )
        safety_grid = Container(classes="template-grid")
        safety_grid._add_child(
            Button("TruthfulQA", id="template-truthfulqa", classes="template-button")
        )
        safety_grid._add_child(
            Button(
                "RealToxicity", id="template-realtoxicity", classes="template-button"
            )
        )
        safety_grid._add_child(
            Button("BOLD", id="template-bold", classes="template-button")
        )
        safety_grid._add_child(
            Button("WinoGender", id="template-winogender", classes="template-button")
        )
        safety_grid._add_child(
            Button("BBQ", id="template-bbq", classes="template-button")
        )
        safety_grid._add_child(
            Button("Ethics", id="template-ethics", classes="template-button")
        )
