"""Console realtime lifecycle, with named late-bound presentation seams.

The controller owns the live provider session and closing worker. The app
identity is stable for this controller's lifetime. Framework services are
read from the screen at each call; other dependencies are named callables.
Only the screen paints the voice chip. Transport imports stay lazy.
"""
...
from ...Chat.console_hands_free import ExitLoop, ModeChanged, SilenceSpeech
...
class ConsoleRealtimeController:
    """Own realtime connection, audio, transcript continuity, and teardown."""
    ...
    def __init__(
        ...
        *,
        ...
        dictation_state: Callable[[], str],
        request_console_dictation_stop: Callable[..., Any],
        sync_native_console_chat_ui: Callable[..., Any],
        repaint_console_realtime_chip: Callable[[], None],
        restore_console_voice_chip: Callable[[], None],
        console_pipeline_hands_free_blocker: Callable[[], Any],
        enter_console_hands_free_pipeline_loop: Callable[..., Any],
    ) -> None:
        self._screen = screen
        ...
        self._dictation_state = dictation_state
        self._request_console_dictation_stop = request_console_dictation_stop
        self._sync_native_console_chat_ui = sync_native_console_chat_ui
        self._repaint_console_realtime_chip = repaint_console_realtime_chip
        self._restore_console_voice_chip = restore_console_voice_chip
        self._console_pipeline_hands_free_blocker = console_pipeline_hands_free_blocker
        self._enter_console_hands_free_pipeline_loop = (
            enter_console_hands_free_pipeline_loop
        )
