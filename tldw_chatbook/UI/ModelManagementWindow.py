            ...
            with VerticalScroll(classes="model-panel", id="model-panel"):
                ...
                with Container(classes="section-container", id="provider-info-section"):
                    ...
                    with Grid(classes="info-grid"):
                        ...
                        yield Label("Status:")
                        yield Static(
                            "Not Selected", id="provider-status", classes="info-value"
                        )
                ...
                with Container(classes="section-container", id="models-section"):
                    ...
                    with Container(id="model-categories"):
                        ...
                        with Collapsible(title="📝 Completion Models"):
                            yield Container(
                                id="completion-models", classes="model-category"
                            )

                        with Collapsible(title="🎯 Specialized Models"):
                            yield Container(
                                id="specialized-models", classes="model-category"
                            )

                # Model details section (hidden initially)
                with Container(
                    classes="section-container hidden", id="model-details-section"
                ):
                    ...
                    with Container(id="model-description", classes="description-box"):
                        yield Static(
                            "Select a model to view details", classes="muted-text"
                        )
    ...
    async def _load_provider_models(self, provider: str) -> None:
        ...
        try:
            ...
            for model in models:
                ...
                model_item.mount(
                    Static(model.get("description", ""), classes="model-desc")
                )
                model_item.mount(
                    Button("Details", classes="mini-button", id=f"model-{model['id']}")
                )
    ...
    async def handle_test_connection(self) -> None:
        ...
        try:
            result = await test_model_connection(
                self.app_instance, self.selected_provider
            )

            if result["success"]:
                self.notify_success(
                    f"Connection successful! Response time: {result['latency']}ms"
                )
            else:
    ...
    async def _show_model_details(self, model_id: str) -> None:
        ...
        try:
            ...
            model_info = await get_model_info(
                self.app_instance, self.selected_provider, model_id
            )
            ...
            self.query_one("#capabilities").update(
                ", ".join(capabilities) or "Standard"
            )
