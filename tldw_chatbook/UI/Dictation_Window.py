...
class DictationWindow(Widget):
                ...
                with ScrollableContainer(classes="dictation-sidebar"):
                    ...
                    with Vertical(classes="control-section"):
                        ...
                        with Vertical(classes="export-buttons"):
                            ...
                            yield Button(
                                "Save with Timestamps", id="save-timestamps-button"
                            )
    ...
    def _add_transcript_segment(self, text: str, timestamp: Optional[float] = None):
        ...
        segment = {
            "text": text,
            "timestamp": timestamp
            or (datetime.now() - self._start_time).total_seconds(),
            "is_command": False,
        }
    ...
    def _update_stats(self):
        ...
        self.query_one("#duration-display", Static).update(
    ...
    def _update_history_display(self):
        ...
        for item in self.transcript_history:
            ...
            label = (
                f"{timestamp.strftime('%Y-%m-%d %H:%M')} - {item['word_count']} words"
            )
    ...
    def _load_history(self):
        ...
        try:
            history_file = (
                Path.home() / ".config" / "tldw_cli" / "dictation_history.json"
            )
