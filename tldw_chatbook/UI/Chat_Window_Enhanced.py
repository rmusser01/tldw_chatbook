...
class ChatWindowEnhanced(Container):
    ...
    async def on_mount(self) -> None:
        ...
        self._sidebar_collapsed = getattr(
    ...
    async def on_chat_input_message_send_requested(
        self, message: ChatInputMessage.SendRequested
    ) -> None:
        """Handle send request via message system."""
        logger.debug(
            f"Send requested via message: {len(message.text)} chars, {len(message.attachments)} attachments"
        )
        # Forward to input handler
        await self.input_handler.handle_enhanced_send_button(None)

    async def on_chat_input_message_stop_requested(
        self, message: ChatInputMessage.StopRequested
    ) -> None:
    ...
    async def on_chat_attachment_message_file_selected(
        self, message: ChatAttachmentMessage.FileSelected
    ) -> None:
    ...
    async def on_chat_voice_message_transcript_received(
        self, message: ChatVoiceMessage.TranscriptReceived
    ) -> None:
        """Handle voice transcript via message system."""
        logger.debug(
            f"Transcript received via message: {message.text} (final: {message.is_final})"
        )
        if message.is_final:
    ...
    async def on_chat_sidebar_message_sidebar_toggled(
        self, message: ChatSidebarMessage.SidebarToggled
    ) -> None:
    ...
    async def on_chat_message_display_message_edit_requested(
        self, message: ChatMessageDisplayMessage.EditRequested
    ) -> None:
    ...
    async def on_chat_streaming_message_stream_started(
        self, message: ChatStreamingMessage.StreamStarted
    ) -> None:
    ...
    async def on_chat_streaming_message_stream_completed(
        self, message: ChatStreamingMessage.StreamCompleted
    ) -> None:
    ...
    def _cache_widget_references(self) -> None:
        ...
        try:
            self._attachment_indicator = self.query_one(
                "#image-attachment-indicator", Static
            )
        except NoMatches:
    ...
    def build_first_run_orientation_text(self, provider: Optional[str] = None) -> str:
        ...
        readiness = get_provider_readiness(
            selected_provider, self.app_instance.app_config
        )
    ...
    def _has_sendable_content(self) -> bool:
        ...
        try:
            ...
            all_messages.sort(key=lambda msg: getattr(msg, "_mount_time", 0))
    ...
    def _toggle_chat_left_sidebar(self) -> None:
        ...
        try:
            ...
            sidebar.display = not self._sidebar_collapsed
            logger.info(
                f"Left sidebar display set to {sidebar.display} (collapsed={self._sidebar_collapsed})"
            )
        except Exception as e:
        ...
        if tab_container:
            ...
            for session_id in tab_container.sessions.keys():
                if button_id.endswith(f"-{session_id}"):
                    logger.debug(
                        f"Tab-specific button detected for session {session_id}"
                    )
                    return True
        return False
    ...
    async def _handle_core_buttons(self, button_id: str, event: Button.Pressed) -> bool:
        ...
        if hasattr(chat_events, "CHAT_BUTTON_HANDLERS"):
            if button_id in chat_events.CHAT_BUTTON_HANDLERS:
                ...
                await chat_events.CHAT_BUTTON_HANDLERS[button_id](
    ...
    async def _handle_sidebar_buttons(
        self, button_id: str, event: Button.Pressed
    ) -> bool:
        ...
        sidebar_handlers = {
            ...
            "chat-notes-search-button": chat_events.handle_chat_notes_search_button_pressed
            if hasattr(chat_events, "handle_chat_notes_search_button_pressed")
            else None,
            "chat-notes-load-button": chat_events.handle_chat_notes_load_button_pressed
            if hasattr(chat_events, "handle_chat_notes_load_button_pressed")
            else None,
            "chat-notes-create-new-button": chat_events.handle_chat_notes_create_new_button_pressed
            if hasattr(chat_events, "handle_chat_notes_create_new_button_pressed")
            else None,
            "chat-notes-save-button": chat_events.handle_chat_notes_save_button_pressed
            if hasattr(chat_events, "handle_chat_notes_save_button_pressed")
            else None,
            "chat-notes-copy-button": chat_events.handle_chat_notes_copy_button_pressed
            if hasattr(chat_events, "handle_chat_notes_copy_button_pressed")
            else None,
        }
        ...
        try:
            ...
            if not is_safe_path(file_path, os.path.expanduser("~")):
                self.app_instance.notify(
                    "Error: File path is outside allowed directory", severity="error"
                )
                return
            ...
            try:
                ...
                self.app_instance.notify(f"Error reading image: {e}", severity="error")
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="chat-main-content"):
            ...
            if enable_tabs:
                logger.info(
                    "Chat tabs are enabled - using ChatTabContainer in enhanced mode"
                )
    ...
    def on_voice_input_message(self, event: VoiceInputMessage) -> None:
        ...
        if event.is_final and event.text and self._chat_input:
            ...
            with app.batch_update():
                current_text = self._chat_input.text
                separator = (
                    " " if current_text and not current_text.endswith(" ") else ""
                )
