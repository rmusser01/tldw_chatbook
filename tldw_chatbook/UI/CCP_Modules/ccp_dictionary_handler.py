...
class CCPDictionaryHandler:
    ...
    def _load_dictionary_sync(self, dictionary_id: int) -> None:
        ...
        try:
            ...
            if dict_data:
                ...
                self.window.call_from_thread(
                    ...
                    ViewChangeMessage.Requested(
                        "dictionary_view", {"dictionary_id": dictionary_id}
                    ),
                )
            ...
            logger.opt(exception=True).error(
                f"Error loading dictionary {dictionary_id}: {e}"
            )
    ...
    def _display_dictionary(self) -> None:
        ...
        try:
            ...
            self._update_textarea(
                "#ccp-dict-description-display", data.get("description", "")
            )
            self._update_field(
                "#ccp-dict-strategy-display", data.get("strategy", "sorted_evenly")
            )
            self._update_field(
                "#ccp-dict-max-tokens-display", str(data.get("max_tokens", 1000))
            )
    ...
    async def handle_edit_dictionary(self) -> None:
        ...
        try:
            ...
            self.window.post_message(
                ViewChangeMessage.Requested(
                    "dictionary_editor", {"dictionary_id": self.current_dictionary_id}
                )
            )
            ...
            logger.opt(exception=True).error(
                f"Error switching to dictionary editor: {e}"
            )
    ...
    def _populate_editor_fields(self) -> None:
        ...
        try:
            ...
            self._set_textarea_value(
                "#ccp-editor-dict-description-textarea", data.get("description", "")
            )

            # Strategy select
            strategy_select = self.window.query_one(
            ...
            self._set_input_value(
                "#ccp-editor-dict-max-tokens-input", str(data.get("max_tokens", 1000))
            )
    ...
    def _display_editor_entries(self) -> None:
        ...
        try:
            entries_list = self.window.query_one(
                "#ccp-editor-dict-entries-list", ListView
            )
            entries_list.clear()
    ...
    async def handle_add_entry(self) -> None:
        ...
        try:
            ...
            value = self.window.query_one(
                "#ccp-dict-entry-value-textarea", TextArea
            ).text
            group = self.window.query_one("#ccp-dict-entry-group-input", Input).value
            probability_str = self.window.query_one(
                "#ccp-dict-entry-probability-input", Input
            ).value
                        ...
                        if self.current_dictionary_id:
                            self.window.post_message(
                                DictionaryMessage.EntryRemoved(
                                    self.current_dictionary_id, removed_entry["key"]
                                )
                            )
    ...
    async def handle_save_dictionary(self) -> None:
        ...
        try:
            ...
            if self.current_dictionary_id:
                ...
                await self._update_dictionary(
                    self.current_dictionary_id, dictionary_data
                )
            else:
    ...
    def _gather_editor_data(self) -> Dict[str, Any]:
        ...
        try:
            data["name"] = self.window.query_one(
                "#ccp-editor-dict-name-input", Input
            ).value
            ...
            max_tokens_str = self.window.query_one(
                "#ccp-editor-dict-max-tokens-input", Input
            ).value
            try:
    ...
    async def handle_delete_dictionary(self) -> None:
        ...
        try:
            ...
            if success:
                ...
                logger.error(
                    f"Failed to delete dictionary {self.current_dictionary_id}"
                )
    ...
    async def handle_import(self) -> None:
        ...
        try:
            ...
            filters = Filters(
                (
                    "Dictionary Files",
                    lambda p: p.suffix.lower() in (".json", ".csv", ".yaml", ".yml"),
                ),
