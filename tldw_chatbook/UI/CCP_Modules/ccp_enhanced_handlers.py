...
from .ccp_loading_indicators import with_loading, LoadingManager
...
def enhance_conversation_handler(handler_class):
    ...
    async def enhanced_search(
        self, search_term: str, search_type: str = "title", validated_data=None
    ):
    ...
    @with_loading(
        "Loading conversation...", "Conversation loaded", "Failed to load conversation"
    )
    ...
    if hasattr(handler_class, "handle_save_details"):
        ...
        @with_loading(
            "Saving conversation details...", "Details saved", "Failed to save details"
        )
...
def enhance_character_handler(handler_class):
    ...
    if hasattr(handler_class, "handle_import_character"):
        ...
        async def enhanced_import(
            self, file_path: str, file_type: str = "character_card"
        ):
...
def enhance_prompt_handler(handler_class):
    ...
    if hasattr(handler_class, "handle_save_prompt"):
        ...
        @with_loading(
            "Saving prompt...", "Prompt saved successfully", "Failed to save prompt"
        )
        async def enhanced_save(self, validated_data):
            return await original_save()
...
def enhance_dictionary_handler(handler_class):
    ...
    if hasattr(handler_class, "handle_save_dictionary"):
        ...
        @with_loading(
            "Saving dictionary...", "Dictionary saved", "Failed to save dictionary"
        )
        async def enhanced_save(self, validated_data):
            return await original_save()
