...
class ConversationInput(BaseModel):
    ...
    character_id: Optional[int] = Field(
        None, gt=0, description="Associated character ID"
    )
...
class CharacterCardInput(BaseModel):
    ...
    personality: Optional[str] = Field(
        None, max_length=5000, description="Character personality"
    )
    scenario: Optional[str] = Field(
        None, max_length=5000, description="Scenario/setting"
    )
    first_message: Optional[str] = Field(
        None, max_length=5000, description="First message"
    )
    keywords: Optional[str] = Field(None, max_length=1000, description="Keywords/tags")
    system_prompt: Optional[str] = Field(
        None, max_length=10000, description="System prompt"
    )
    ...
    def validate_version(cls, v):
        ...
        if v:
            ...
            if not version_pattern.match(v):
                raise ValueError(
                    "Invalid version format. Use format like '1.0' or 'v2.1.3'"
                )
        return v
...
class PromptInput(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=2000, description="Prompt description"
    )
    system_prompt: Optional[str] = Field(
        None, max_length=10000, description="System prompt"
    )
    user_prompt: Optional[str] = Field(
        None, max_length=10000, description="User prompt template"
    )
...
class DictionaryInput(BaseModel):
    ...
    entries: List[Dict[str, Any]] = Field(
        default_factory=list, description="Dictionary entries"
    )
...
class SearchInput(BaseModel):
    ...
    search_term: str = Field(
        ..., min_length=1, max_length=500, description="Search term"
    )
...
class FileImportInput(BaseModel):
    ...
    overwrite_existing: bool = Field(
        False, description="Whether to overwrite existing entries"
    )
