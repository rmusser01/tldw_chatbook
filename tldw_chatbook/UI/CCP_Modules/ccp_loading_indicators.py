...
class LoadingManager:
    ...
    @asynccontextmanager
    async def loading(
        self, text: str = "Loading...", operation_id: Optional[str] = None
    ):
        """
...
def with_loading(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            try:
                # Start loading
                await loading_manager.start_loading(
                    loading_text, operation_id, notify=notify
                )
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            async def update_progress(current: int, total: int = None, text: str = ""):
                ...
                if total_steps:
                    ...
                    progress_text = (
                        f"{text} ({percentage}%)"
                        if text
                        else f"Processing... ({percentage}%)"
                    )
                else:
                    progress_text = (
                        f"{text} ({current})" if text else f"Processing... ({current})"
                    )

                if loading_manager:
                    await loading_manager.update_loading_text(
                        operation_id, progress_text
                    )
            ...
            try:
                ...
                if hasattr(self.window, "notify"):
                    self.window.notify(
                        "✅ Operation completed", severity="information", timeout=2
                    )
