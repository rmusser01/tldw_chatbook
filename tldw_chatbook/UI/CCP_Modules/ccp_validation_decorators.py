...
from .ccp_validators import SearchInput, FileImportInput, validate_with_model
        ...
        async def wrapper(self, *args, **kwargs):
            try:
                ...
                logger.opt(exception=True).error(
                    f"Error in validation decorator for {func.__name__}: {e}"
                )
    ...
    async def wrapper(self, *args, **kwargs):
        try:
            ...
            data = {
                ...
                "search_type": args[1]
                if len(args) > 1
                else kwargs.get("search_type", "title"),
            }

            # Try to get additional parameters from checkboxes
            try:
                include_char = self.window.query_one(
                    "#conv-char-search-include-character-checkbox"
                )
                data["include_character_chats"] = (
                    include_char.value if hasattr(include_char, "value") else True
                )
            except Exception:
                data["include_character_chats"] = True

            try:
                all_chars = self.window.query_one(
                    "#conv-char-search-all-characters-checkbox"
                )
                data["all_characters"] = (
                    all_chars.value if hasattr(all_chars, "value") else True
                )
            except Exception:
            ...
            return await func(
                self,
                validated_data.search_term,
                validated_data.search_type,
                validated_data,
                *args[2:],
                **kwargs,
            )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if not selected_id:
                ...
                if hasattr(self, "window") and hasattr(self.window, "app_instance"):
                    self.window.app_instance.notify(
                        f"Please select a {item_type} first", severity="warning"
                    )
                return None
