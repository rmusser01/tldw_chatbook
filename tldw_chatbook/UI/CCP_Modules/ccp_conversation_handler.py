...
def normalize_conversation_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized["title"] = (
        normalized.get("title") or normalized.get("name") or "Untitled"
    )
...
class CCPConversationHandler:
    ...
    def _filter_conversations_for_scope(
    ...
    def _search_conversations_sync(
        ...
        try:
            ...
            if search_type == "title":
                ...
                results = (
                    db.search_conversations_by_content(search_term, limit=100)
                    if search_term
                    else []
                )
    ...
    async def _update_search_results_ui(self) -> None:
        ...
        try:
            results_list = self.window.query_one(
                "#conv-char-search-results-list", ListView
            )
            results_list.clear()
    ...
    async def handle_load_selected(self) -> None:
        ...
        try:
            ...
            logger.opt(exception=True).error(
                f"Error loading selected conversation: {e}"
            )
    ...
    def _load_conversation_sync(self, conversation_id: ConversationId) -> None:
        ...
        try:
            ...
            conversation_data = (
                normalize_conversation_row(conversation_row) if conversation_row else {}
            )
            ...
            logger.opt(exception=True).error(
                f"Error loading conversation {conversation_id}: {e}"
            )

    def get_conversation_contract(
        self, conversation_id: Optional[ConversationId] = None
    ) -> Dict[str, Any]:
        ...
        if (
            ...
            and self.current_conversation_data.get("id") == target_id
        ):
    ...
    def _display_conversation_messages(self) -> None:
        ...
        try:
            ...
            messages = db.get_messages_for_conversation(
                str(self.current_conversation_id), limit=200
            )
            ...
            logger.opt(exception=True).error(
                f"Error displaying conversation messages: {e}"
            )
    ...
    async def handle_save_details(self, title: str, keywords: str) -> None:
        ...
        try:
            ...
            success = update_conversation_metadata(
                self.current_conversation_id, title=title, keywords=keywords
            )

            if success:
                logger.info(
                    f"Saved details for conversation {self.current_conversation_id}"
                )

                # Post update message
                self.window.post_message(
                    ConversationMessage.Updated(
                        self.current_conversation_id, title, keywords
                    )
                )
            else:
                logger.error(
                    f"Failed to save details for conversation {self.current_conversation_id}"
                )
    ...
    async def handle_export(self, format: str = "json") -> None:
        ...
        try:
            ...
            if format == "json":
                file_path = await generator.export_conversation_json(
                    self.current_conversation_id
                )
            else:
                file_path = await generator.export_conversation_text(
                    self.current_conversation_id
                )
