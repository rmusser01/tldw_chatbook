...
class CodeRepoCopyPasteWindow(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        """Compose the UI."""
        logger.info(
            "CodeRepoCopyPasteWindow.compose() called - NEW VERSION WITH COMPILATION"
        )
        with Container(classes="repo-window-container"):
            ...
            with Container(classes="repo-header"):
                ...
                with Container(
                    id="repo-input-container", classes="repo-input-container hidden"
                ):
                    yield Input(
                    ...
                    yield Button(
                        "🔑 Token", id="token-config-btn", classes="token-button"
                    )

                # Controls (hidden until repo loaded)
                with Container(
                    id="repo-controls-container", classes="repo-controls hidden"
                ):
                    yield Select(
                        options=[("main", "main")],
                        id="branch-selector",
                    ...
                    with Container(classes="selection-summary"):
                        ...
                        yield Static(
                            "0 KB", id="selection-size", classes="summary-stat"
                        )
                        yield Static(
                            "~0 tokens", id="selection-tokens", classes="summary-stat"
                        )
            ...
            with Container(id="empty-state", classes="empty-state-container"):
                with Container(classes="empty-state-content"):
                    yield Static(
                        "🗂️\n\nNo Repository Loaded", classes="empty-state-icon"
                    )
                    yield Static(
                        "Load a GitHub Repository", classes="empty-state-title"
                    )
                    yield Static(
            ...
            with Container(id="filter-bar", classes="filter-bar hidden"):
                ...
                with Container(classes="filter-buttons"):
                    yield Button(
                        "📝 Docs", id="filter-docs", classes="filter-quick-btn"
                    )
                    yield Button(
                        "💻 Code", id="filter-code", classes="filter-quick-btn"
                    )
                    yield Button(
                        "⚙️ Config", id="filter-config", classes="filter-quick-btn"
                    )
            ...
            with Container(id="action-bar", classes="action-bar"):
                with Container(classes="action-buttons"):
                    ...
                    yield Button(
                        "Export ZIP", id="export-zip-btn", classes="action-button"
                    )
                    yield Button(
                        "Copy to Clipboard",
        ...
        try:
            ...
            for root, dirs, files in os.walk(repo_path):
                ...
                for dir_name in sorted(dirs):
                    rel_path = (
                        str(rel_root / dir_name) if str(rel_root) != "." else dir_name
                    )
                    tree_data.append(
                        {"path": rel_path, "type": "tree", "name": dir_name}
                    )
                ...
                for file_name in sorted(files):
                    ...
                    rel_path = (
                        str(rel_root / file_name) if str(rel_root) != "." else file_name
                    )
            ...
            self.notify(
                f"Loaded {len(tree_data)} items from local repository",
    ...
    async def load_repository(self, event: Button.Pressed) -> None:
        ...
        if not repo_input:
            self.notify(
                "Please enter a GitHub repository URL or local path", severity="error"
            )
            return
        ...
        try:
            if self.is_local_repo:
                ...
                content = await self.api_client.get_file_content(
                    self.current_repo["owner"], self.current_repo["repo"], path, branch
                )
    ...
    async def generate_compilation(self, event: Button.Pressed) -> None:
        ...
        try:
            ...
            self.notify(
                f"Generated compilation with {len(selected_files)} files",
                severity="success",
            )
    ...
    async def _export_to_zip_worker(self, selected_files: List[str]) -> None:
        ...
        try:
            ...
            self.loading_message = (
                f"Creating ZIP file with {len(selected_files)} files..."
            )
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
                ...
                if self.is_local_repo:
                    ...
                    for idx, file_path in enumerate(selected_files):
                        ...
                        (idx + 1) / total_files
                        self.loading_message = (
                            f"Adding to ZIP: {file_path} ({idx + 1}/{total_files})"
                        )
                    ...
                    def update_progress(completed, total, current_file):
                        self.loading_message = (
                            f"Fetching files: {completed}/{total} - {current_file}"
                        )

                    # Fetch all files concurrently
                    self.loading_message = (
                        f"Fetching {total_files} files from GitHub..."
                    )
                    ...
                    for file_path in selected_files:
                        if file_path in file_contents:
                            ...
                            zip_file.writestr(
                                file_path, file_contents[file_path].encode("utf-8")
                            )
                        else:
                ...
                if not self.is_local_repo:
                    manifest["github_url"] = (
                        f"https://github.com/{self.current_repo['owner']}/{self.current_repo['repo']}"
                    )
                    manifest["branch"] = self.query_one(
    ...
    async def load_node_children(self, node_path: str) -> None:
        ...
        try:
            ...
            children = await self.api_client.get_directory_contents(
                self.current_repo["owner"], self.current_repo["repo"], node_path, branch
            )
