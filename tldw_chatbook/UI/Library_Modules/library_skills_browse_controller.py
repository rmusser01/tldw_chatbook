        ...
        *,
        ...
        if not callable(list_skills):
            ...
            try:
                ...
                record = await self._service_page(list_skills, scope, offset=offset)
                if isinstance(record, Mapping):
                    total = record.get("total")
                    if type(total) is int and total > 0 and offset >= total:
                        final_page = (total + scope.page_size - 1) // scope.page_size
                        final_offset = (final_page - 1) * scope.page_size
