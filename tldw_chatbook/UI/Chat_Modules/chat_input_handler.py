...
class ChatInputHandler:
    ...
    async def handle_send_stop_button(self, event):
        ...
        if current_time - self._last_send_stop_click < self.DEBOUNCE_MS:
            logger.debug(
                "Button click debounced",
                extra={"time_diff": current_time - self._last_send_stop_click},
            )
            return
        ...
        try:
            ...
            if is_streaming or self._worker_is_running():
                # Stop operation
                logger.info(
                    "Send/Stop button pressed - stopping generation",
                ...
                logger.info(
                    "Send/Stop button pressed - sending message",
                    extra={
                        "action": "send",
                        "has_attachment": bool(
                            self.chat_window.pending_attachment
                            or self.chat_window.pending_image
                        ),
                    },
                )
            ...
            logger.error(
                f"Widget access error in send/stop handler: {e}",
                extra={
                    "button_state": "send"
                    if self.chat_window.is_send_button
                    else "stop"
                },
            )
    ...
    def update_button_state(self):
        ...
        try:
            ...
            if self.chat_window.is_send_button != should_be_send:
                ...
                logger.debug(
                    f"Button state updated - is_send: {should_be_send}, is_streaming: {is_streaming}"
                )
        except (AttributeError, RuntimeError) as e:
