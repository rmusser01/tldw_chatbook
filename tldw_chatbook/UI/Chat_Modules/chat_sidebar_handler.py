...
class ChatSidebarHandler:
    ...
    async def handle_notes_expand_button(self, event):
        ...
        try:
            ...
            button = (
                self.chat_window._notes_expand_button
                if self.chat_window._notes_expand_button
                else self.app_instance.query_one("#chat-notes-expand-button", Button)
            )
            textarea = (
                self.chat_window._notes_textarea
                if self.chat_window._notes_textarea
                else self.app_instance.query_one(
            ...
            logger.debug(
                f"Notes area toggled - expanded: {'notes-textarea-expanded' in textarea.classes}"
            )
        ...
        try:
            ...
            if direction == "shrink":
                ...
                if isinstance(current_width, int) and current_width < 60:
                    sidebar.styles.width = current_width + 5
                    logger.debug(
                        f"Sidebar {sidebar_id} expanded to {current_width + 5}"
                    )
    ...
    def toggle_sidebar_visibility(self, sidebar_id: str):
        ...
        try:
            ...
            logger.debug(
                f"Sidebar {sidebar_id} visibility toggled to {sidebar.display}"
            )
    ...
    def get_sidebar_state(self, sidebar_id: str) -> dict:
        ...
        """
        state = {"visible": False, "width": None, "collapsed": False}
