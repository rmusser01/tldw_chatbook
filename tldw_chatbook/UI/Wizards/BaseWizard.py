...
class WizardNavigation(Horizontal):
    ...
    def watch_can_go_forward(self) -> None:
        ...
        logger.debug(
            f"WizardNavigation: can_go_forward changed to {self.can_go_forward}"
        )
    ...
    def update_button_states(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"WizardNavigation.update_button_states: can_go_forward={self.can_go_forward}, next_btn.disabled={next_btn.disabled}"
            )
...
class WizardProgress(Horizontal):
    ...
    /* .active step styling lives in css/features/_wizards.tcss (needs
       the bundle's $ds-focus-* tokens; TASK-16811). */
    ...
    def compose(self) -> ComposeResult:
        ...
        for i in range(1, self.total_steps + 1):
            ...
            with Container(classes="step-indicator-container"):
                ...
                yield Static("✓" if is_complete else str(i), classes=step_classes)
                ...
                if i <= len(self.step_titles):
                    ...
                    yield Label(self.step_titles[i - 1], classes=title_classes)
...
class WizardContainer(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        logger.info(
            f"BaseWizard.compose() called with {len(self.steps)} steps, current_step={self.current_step}"
        )
        # Title
    ...
    def on_mount(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"BaseWizard.on_mount: Navigation configured - current_step={nav.current_step}, can_go_forward={nav.can_go_forward}"
            )
    ...
    def validate_step(self) -> None:
        ...
        logger.debug(
            f"WizardContainer.validate_step called for step {self.current_step}"
        )
        ...
        if 0 <= current_step_index < len(self.steps):
            ...
            try:
                nav = self.query_one(".wizard-navigation", WizardNavigation)
                logger.info(
                    f"BaseWizard.validate_step: Setting nav.can_go_forward={is_valid}"
                )
                nav.can_go_forward = is_valid
                logger.info("BaseWizard.validate_step: Navigation updated successfully")
            except NoMatches as e:
                logger.error(
                    f"BaseWizard.validate_step: Could not find navigation to update - {e}"
                )
        ...
        if self.current_step < len(self.steps) - 1:
            ...
            if self.can_proceed:
                ...
                step_id = (
                    ...
                    else f"step_{self.current_step}"
                )
    ...
    def action_cancel(self) -> None:
        ...
        parent_screen = (
            self.ancestors_with_self[1] if len(self.ancestors_with_self) > 1 else None
        )
    ...
    def complete_wizard(self) -> None:
        ...
        self.wizard_data[
            self.steps[self.current_step].config.id
            if self.steps[self.current_step].config
            else f"step_{self.current_step}"
        ] = self.steps[self.current_step].get_step_data()
