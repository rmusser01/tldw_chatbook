...
class BackupRestoreScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id="backup-body"):
            ...
            with Vertical(id="backup-create-form", classes="backup-form"):
                ...
                yield Static(
                    "Groups contain whole stores. Linked groups needed by your selection are included in the review."
                )
            ...
            with Vertical(id="backup-inspect-form", classes="backup-form"):
                ...
                with Vertical(id="backup-restore-form", classes="backup-form"):
                    yield Static("Data to restore", classes="destination-section")
                    yield Static(
                        "Everything restores all available groups in this archive. Choose groups to restore a subset."
                    )
                    yield BackupDataGroupSelector(id="backup-restore-data")
                    yield Select(
                        [
                    ...
                    yield Static(
                        "Each selected and required group is replaced as a whole. Unselected stored data stays in place."
                    )
                    yield Input(
    ...
    def _options(self):
        ...
        return {
            "data_groups": self.query_one(
                "#backup-data", BackupDataGroupSelector
            ).selection(),
    ...
    def _form_changed(self, event):
        if self.is_mounted:
            if event.control.id in {
                ...
                return
            identifier = event.control.id or ""
            if identifier == "backup-restore-mode" or identifier.startswith(
                ("backup-target-config", "backup-restore-data-")
            ):
                self._restore_group_requirements = None
            self._invalidate()
            self._sync_replacement_host()
            ...
            if control.has_class("backup-restore-data-group") or (
                isinstance(control, (Input, Select))
                and not (isinstance(control, Input) and control.password)
            ):
    ...
    def _show_preview(self, revision, preview, reviewed):
        ...
        whole_profile = details.get("whole_profile", True)
        coverage = "Complete" if preview.complete else "Partial"
        rows = [
            f"{coverage} coverage — whole profile"
            if whole_profile
            else f"{coverage} selected coverage",
        ...
        if "effective_groups" in details:
            rows.extend(
                self._group_review_rows(
                    details.get("requested_groups"),
    ...
    async def _summary_ready(self, operation, summary):
        ...
        scope = summary.get("group_scope")
        if scope is not None:
            rows.append(
                "Coverage: whole profile"
                if scope["whole_profile"]
                else "Coverage: selected groups, not the whole profile"
            )
        else:
            rows.append("Restore scope: all available archived groups")
        rows.extend(
            f"Data group: {group['label']} · {group['file_count']} files · {group['payload_bytes']} bytes"
            for group in summary.get("data_groups", ())
        )
        self.query_one("#backup-inspection-summary", Static).update("\n".join(rows))
        await self.query_one(
            "#backup-restore-data", BackupDataGroupSelector
        ).set_groups(summary.get("data_groups", ()))
    ...
    def _review_restore(self):
        ...
        for index, slot in enumerate(self._inspection_summary["destination_slots"]):
            if not self._restore_slot_selected(slot):
                continue
        ...
        external = {
            slot["logical_id"]: destinations[slot["logical_id"]]
            for slot in self._inspection_summary["destination_slots"]
            if slot["kind"] == "external_root" and self._restore_slot_selected(slot)
        }
        setup_parent = None
        ...
        self._preview_restore(
            ...
            data_groups,
            None if displayed_groups is None else tuple(sorted(displayed_groups)),
        )
    ...
    def _preview_restore(
        ...
        try:
            ...
            if inventory is not None:
                requirements = self.service.restore_group_requirements(
                    inspection, data_groups=data_groups, target=inventory
                )
                if (
                    requirements is not None
                    and displayed_groups is not None
                    and requirements.effective_groups != displayed_groups
                ):
                    self._deliver(
                        app, self._restore_requirements_ready, revision, requirements
                    )
                    return
    ...
    def _restore_requirements_ready(self, revision, requirements):
        """Show target-linked groups in the existing form before a new review."""
        if not self.is_mounted or revision != self._revision:
            return
        self._restore_group_requirements = requirements
        self._sync_replacement_host()
        rows = self._group_review_rows(
            requirements.requested_groups,
            requirements.effective_groups,
            requirements.required_groups,
        )
        rows.append(
            "Linked group requirements changed. Review the updated destinations, "
            "then review restore again."
        )
    ...
    async def _restore_ready(self, revision, plan, issue, availability=None):
        ...
        if plan.requested_groups is None and not effective_groups:
            effective_groups = tuple(
                group["group_id"]
                for group in (self._inspection_summary or {}).get("data_groups", ())
            )
        rows.extend(
            self._group_review_rows(
                plan.requested_groups, effective_groups, plan.required_groups
            )
        )
        if plan.mode == "replace":
            rows.append(
    ...
    @staticmethod
    def _group_review_rows(requested, effective, required):
        """Name the reviewed groups separately from support and safety copies."""
        labels = {group.group_id: group.label for group in BACKUP_GROUPS}
        rows = [
            "Selected data groups: "
            + (
                "Everything available"
                if requested is None
                else ", ".join(labels[group] for group in requested)
            ),
            "Included data groups: " + ", ".join(labels[group] for group in effective),
        ]
        if required:
            rows.append(
                "Required linked groups: "
                + ", ".join(labels[group] for group in required)
                + " — included to keep the selected data usable."
            )
        return rows

    def _effective_restore_groups(self):
        """Use the verified summary to show destinations before plan review."""
        selected = self.query_one(
            "#backup-restore-data", BackupDataGroupSelector
        ).selection()
        if selected is None:
            return None
        if self._restore_group_requirements is not None:
            return set(self._restore_group_requirements.effective_groups)
        required = {
            required
            for group in (self._inspection_summary or {}).get("data_groups", ())
            if group["group_id"] in selected
            for required in group.get("required_groups", ())
        }
        return set(selected) | required

    def _restore_slot_selected(self, slot):
        groups = self._effective_restore_groups()
        return (
            groups is None
            or slot["kind"] == "profile_base"
            or "group_ids" not in slot
            or bool(groups.intersection(slot["group_ids"]))
        )

    def _restore_setup_required(self):
        summary = self._inspection_summary or {}
        groups = self._effective_restore_groups()
        return bool(summary.get("setup_destination_required")) and (
            groups is None
            or "setup_group_ids" not in summary
            or bool(groups.intersection(summary["setup_group_ids"]))
        )
    ...
    def _sync_replacement_host(self):
        ...
        self.query_one("#backup-setup-destination").display = (
            replacing and self._restore_setup_required()
        )
        for index, slot in enumerate(
            (self._inspection_summary or {}).get("destination_slots", ())
        ):
            visible = self._restore_slot_selected(slot)
            self.query_one(f"#backup-root-{index}", Input).display = visible
            self.query_one(f"#backup-root-label-{index}", Static).display = visible
            if slot["kind"] == "profile_base":
