...
class ChatScreen(BaseAppScreen):
    ...
    def action_open_console_session_switcher(self) -> None:
        ...
        self.app.push_screen(
            ConsoleSessionSwitcherModal(
                ...
                on_full_search=lambda query, scope: self.app_instance.open_conversation_archive(query, scope),
            ),
    ...
    def _console_setup_blocked_reason(self) -> str:
        ...
        if "endpoint is not saved" in blocker:
    ...
    def on_mount(self) -> None:
        ...
        self.set_timer(0.16, self._consume_pending_conversation_resume)
    ...
    def _native_console_transcript_fingerprint(
        ...
        return (
            ...
            presentation_signature,
            coerce_bool_setting(
                (getattr(self.app_instance, "app_config", None) or {})
                .get("console", {})
                .get("show_thinking", True),
                True,
            ),
        )
    ...
    async def _submit_console_native_draft(
        ...
        sending_session = next((item for item in controller.store.sessions() if item.id == session_id), None)
        sending_id = sending_session.persisted_conversation_id if sending_session else None
        send_reservations = getattr(self.app_instance, "_conversation_send_inflight", None)
        ...
        try:
            ...
            session = next((item for item in controller.store.sessions() if item.id == session_id), None)
            reason = await conversation_send_refusal(self.app_instance, session.persisted_conversation_id if session else None)
            if reason:
                ...
                if self._console_visible_draft_session_id == session_id:
                    ...
                    controller.store.set_session_draft(session_id, draft + ("\n" + session.draft if session.draft else ""))
    ...
    def _console_send_blocked_reason(self) -> str:
        ...
        if conversation_id in getattr(self.app_instance, "_conversation_archive_inflight", ()):
            return "Archive change in progress. Your draft is preserved."
        if getattr(self.app_instance, "_conversation_archive_states", {}).get(conversation_id):
