...
class ChatScreen(BaseAppScreen):
    ...
    #: ``components/_agentic_terminal.tcss``. Textual parses a screen's
    #: ``CSS_PATH`` lazily on first visit (``App._load_screen_css``), and
    #: this screen is constructed ~1.5 s AFTER first paint -- so these
    #: bytes no longer ride the pre-first-paint boot parse. The sheet is
    #: GENERATED by ``css/build_css.py``; edit the source module, not it.
    CSS_PATH = [
        str(
