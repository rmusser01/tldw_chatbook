...
class SiteConfigSettings(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            ...
            with ScrollableContainer(classes="site-config-container"):
                yield Static(
                    "Site Configuration", classes="config-header", id="config-title"
                )
                ...
                with Horizontal(classes="action-buttons"):
                    ...
                    yield Button(
                        "Apply Preset", variant="primary", id="apply-preset-btn"
                    )
    ...
    def _compose_general_settings(self) -> ComposeResult:
        ...
        with Container(classes="config-section"):
            ...
            yield self.form_builder.create_form_field(
                "Requests per minute",
                Input(value="60", id="rate-limit-requests", type="number"),
            )

            yield self.form_builder.create_form_field(
                "Concurrent requests",
                Input(value="1", id="concurrent-requests", type="number"),
            )

            yield Static("Request Settings", classes="section-title")

            yield self.form_builder.create_form_field(
                "Timeout (seconds)", Input(value="30", id="timeout", type="number")
            )

            yield self.form_builder.create_form_field(
                "Retry count", Input(value="3", id="retry-count", type="number")
            )
            ...
            yield self.form_builder.create_form_field(
                "Requires JavaScript", Switch(value=False, id="requires-javascript")
            )

            yield Input(placeholder="CSS selector to wait for", id="wait-for-selector")
    ...
    def _compose_auth_settings(self) -> ComposeResult:
        ...
        with Container(classes="config-section"):
            ...
            with Container(id="bearer-auth-container", classes="hidden"):
                yield Input(
                    placeholder="Bearer Token", password=True, id="bearer-token"
                )

            # API key fields
            with Container(id="api-key-container", classes="hidden"):
                yield Input(
                    placeholder="Header Name (e.g., X-API-Key)", id="api-key-name"
                )
                yield Input(
                    placeholder="API Key Value", password=True, id="api-key-value"
                )
    ...
    def _compose_extraction_settings(self) -> ComposeResult:
        ...
        with Container(classes="config-section"):
            ...
            yield Input(placeholder="Content CSS selector", id="content-selector")

            yield Input(placeholder="Title CSS selector", id="title-selector")

            yield Input(placeholder="Date CSS selector", id="date-selector")

            yield Input(placeholder="Author CSS selector", id="author-selector")
            ...
            yield Input(
                placeholder="Ignore selectors (comma-separated)", id="ignore-selectors"
            )
    ...
    def _compose_advanced_settings(self) -> ComposeResult:
        ...
        with Container(classes="config-section"):
            ...
            yield self.form_builder.create_form_field(
                "Remove scripts", Switch(value=True, id="remove-scripts")
            )

            yield self.form_builder.create_form_field(
                "Remove styles", Switch(value=True, id="remove-styles")
            )

            yield self.form_builder.create_form_field(
                "Preserve links", Switch(value=True, id="preserve-links")
            )

            yield self.form_builder.create_form_field(
                "Extract images", Switch(value=False, id="extract-images")
            )
            ...
            yield self.form_builder.create_form_field(
                "Viewport width",
                Input(value="1920", id="viewport-width", type="number"),
            )

            yield self.form_builder.create_form_field(
                "Viewport height",
                Input(value="1080", id="viewport-height", type="number"),
            )

            yield Static("Notes", classes="section-title")
            yield Input(placeholder="Notes about this configuration", id="config-notes")
    ...
    def display_config(self, config: SiteConfig):
        ...
        # General settings
        self.query_one("#rate-limit-requests", Input).value = str(
            config.rate_limit_requests
        )
        self.query_one("#concurrent-requests", Input).value = str(
        ...
        # JavaScript settings
        self.query_one(
            "#requires-javascript", Switch
        ).value = config.requires_javascript
        self.query_one("#wait-for-selector", Input).value = (
            config.wait_for_selector or ""
        )
...
Created: {config.created_at}"""
    ...
    async def on_save_config(self):
        ...
        config.rate_limit_requests = int(
            self.query_one("#rate-limit-requests", Input).value or 60
        )
        config.concurrent_requests = int(
            self.query_one("#concurrent-requests", Input).value or 1
        )
        ...
        # JavaScript settings
        config.requires_javascript = self.query_one(
            "#requires-javascript", Switch
        ).value
        config.wait_for_selector = (
            self.query_one("#wait-for-selector", Input).value or None
        )
