...
class ScraperBuilderWindow(Screen):
        ...
        with Container(classes="builder-container"):
            ...
            with ScrollableContainer(classes="left-panel"):
                ...
                with Horizontal(classes="action-buttons"):
                    yield Button(
                        "Save Configuration", variant="success", id="save-config-btn"
                    )
                    yield Button(
                        "Load Configuration", variant="primary", id="load-config-btn"
                    )
                    yield Button(
                        "Export as Code", variant="default", id="export-code-btn"
                    )
            ...
            with Container(classes="right-panel"):
                ...
                with TabbedContent():
                    ...
                    with TabPane("Text", id="text-preview-tab"):
                        yield TextArea(id="text-preview", read_only=True)
    ...
    def _create_selector_templates(self) -> ComposeResult:
        ...
        templates = [
            (
                "Article Title",
                "h1, article h1, .article-title, .post-title, .entry-title",
            ),
            (
                "Article Content",
    ...
    def create_rule_widget(self, rule: Dict, index: int) -> Container:
        ...
        with rule_container:
            with Horizontal():
                with Vertical():
                    rule_container.mount(Static(f"[bold]{rule['field']}[/bold]"))
                    rule_container.mount(
                        Static(f"Selector: [cyan]{rule['selector']}[/cyan]")
                    )
                    rule_container.mount(
                        Static(
                            f"Extract: {rule['extract']} | Multiple: {rule['multiple']}"
                        )
                    )
                ...
                try:
                    ...
                    if not elements:
                        ...
                        if rule["extract"] == "text":
                            ...
                            results[field] = [
                                elem.get(rule["extract"]) for elem in elements
                            ]
                    else:
    ...
    def display_extraction_results(self, results: Dict[str, Any]):
        ...
        for field, value in results.items():
            if value is None:
                ...
                content += (
                    f"[yellow]{field}:[/yellow] [green]{len(value)} items[/green]\n"
                )
                for i, item in enumerate(value[:3]):  # Show first 3
                    content += (
                        f"  - {item[:100]}{'...' if len(str(item)) > 100 else ''}\n"
                    )
                if len(value) > 3:
        ...
        panel = Panel(content.strip(), title="Extracted Data", border_style="green")
    ...
    async def on_save_config(self):
        ...
        config = {
            ...
            "options": {
                ...
                "exclude_selectors": self.query_one("#exclude-selectors", TextArea)
                .text.strip()
                .split("\n"),
    ...
    async def on_template_clicked(self, event: Button.Pressed):
        ...
        template_name = (
            event.button.id.replace("template-", "").replace("-", " ").title()
        )
