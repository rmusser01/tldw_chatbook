...
from .eval_shared_components import BaseEvaluationWindow, format_status_badge
...
class ResultsDashboardWindow(BaseEvaluationWindow):
        ...
        with TabbedContent():
            ...
            with TabPane("Current Results", id="current-results-tab"):
                with VerticalScroll(classes="eval-content-area"):
                    # Run Summary
                    yield RunSummaryWidget(
                        id="run-summary", classes="section-container"
                    )

                    # Metrics Overview
                    yield MetricsDisplay(
                        id="metrics-display", classes="section-container"
                    )
                    ...
                    with Container(classes="section-container"):
                        ...
                        with Container(id="viz-container", classes="viz-area"):
                            yield Static(
                                "Visualization placeholder", classes="muted-text"
                            )
            ...
            with TabPane("Compare", id="compare-tab"):
                with VerticalScroll(classes="eval-content-area"):
                    yield ComparisonView(
                        id="comparison-view", classes="section-container"
                    )
                ...
                with VerticalScroll(classes="eval-content-area"):
                    with Container(classes="section-container"):
                        ...
                        with Container(id="performance-insights"):
                            yield Static(
                                "Performance Insights", classes="subsection-title"
                            )
                            yield Static(
                                "Loading insights...",
                                id="insights-text",
    ...
    async def handle_generate_report(self) -> None:
        ...
        try:
            ...
            report_path = await generate_evaluation_report(
                self.app_instance, self.current_run_id
            )
    ...
    async def handle_refresh(self) -> None:
        ...
        await self._load_history(self.query_one("#history-filter", Select).value)
    ...
    def _update_visualization_type(
        self, viz_type: str, results: Optional[Dict] = None
    ) -> None:
        ...
        try:
            ...
            if viz_type == "accuracy":
                viz_container.mount(
                    Static("Accuracy chart placeholder", classes="viz-placeholder")
                )

            elif viz_type == "confusion":
                viz_container.mount(
                    Static("Confusion matrix placeholder", classes="viz-placeholder")
                )
            ...
            viz_container.mount(
                Static("Failed to load visualization", classes="error-text")
            )
            ...
            if errors:
                ...
                for category, category_errors in error_categories.items():
                    ...
                    for error in category_errors[:5]:  # Show first 5
                        category_node.add(
                            f"Sample {error['sample_id']}: {error['message']}"
                        )
