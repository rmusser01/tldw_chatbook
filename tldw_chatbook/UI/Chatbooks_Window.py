...
class ChatbooksWindow(Widget):
        ...
        yield Horizontal(
            ...
            Container(
                Container(
                    ...
                    Horizontal(
                        Button(
                            "✨ Create New Chatbook", id="btn-create", variant="primary"
                        ),
        ...
        try:
            ...
            if self.current_view == "welcome":
                ...
                if self.selected_chatbook:
                    ...
                    details = Container(
                        Static(
                            f"📚 {cb.get('name', 'Untitled')}", classes="chatbook-title"
                        ),
                        Static(
                            f"Description: {cb.get('description', 'No description')}",
                            classes="chatbook-desc",
                        ),
                        Static(
                            f"Size: {cb.get('size_mb', 0):.1f} MB",
                            classes="chatbook-size",
                        ),
                        Static(
                            f"Created: {cb.get('created_at', 'Unknown')}",
                            classes="chatbook-date",
                        ),
                        Container(
    ...
    def _update_chatbooks_list(self) -> None:
        ...
        try:
            ...
            for i, cb in enumerate(self.chatbooks):
                ...
                safe_id = f"cb_{i}"
                item = ListItem(Static(f"📚 {name} ({size:.1f} MB)"), id=safe_id)
    ...
    def _show_templates_view(self, content: Container) -> None:
        ...
        try:
            # Templates header
            header = Static("📋 Template Management", classes="chatbook-title")
            ...
            templates_data = [
                (
                    "🔬 Research Project",
                    "Organize research conversations, notes, and references",
                ),
                (
                    "✍️ Creative Writing",
                    "Bundle character profiles, story notes, and world-building",
                ),
                (
                    "📚 Learning Journey",
                    "Track learning progress with conversations and study materials",
                ),
                (
                    "📋 Project Documentation",
                    "Document project decisions and technical notes",
                ),
                (
                    "🤖 Personal Assistant",
                    "Export AI assistant conversations and custom prompts",
                ),
    ...
    async def _use_selected_template(self) -> None:
        ...
        try:
            ...
            template = await self.app.push_screen(
                template_window, wait_for_dismiss=True
            )
    ...
    async def _create_chatbook_with_template(self, template) -> None:
        ...
        try:
            ...
            if result and result.get("success"):
                self.app.notify(
                    f"Chatbook created from template: {template.name}",
                    severity="success",
                )
                await self._refresh_chatbooks()
                self.current_view = "welcome"
