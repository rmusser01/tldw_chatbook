...
class MindmapViewerWindow(Screen):
        ...
        with Container(classes="mindmap-main-container"):
            ...
            with Vertical(id="content-selector"):
                ...
                if SmartContentTree:
                    yield SmartContentTree(
                        load_content=self._load_content, id="content-tree"
                    )
                else:
                ...
                yield Button(
                    ...
                    id="import-canvas",
                    variant="default",
                    tooltip="Import from Obsidian JSON Canvas",
                )
            ...
            with Vertical(id="mindmap-display-container"):
                if MindmapViewer:
                    ...
                    yield Label(
                        "Mindmap viewer not available", id="mindmap-placeholder"
                    )
                ...
                with Horizontal(id="export-controls"):
                    ...
                    yield Button(
                        ...
                        classes="export-button",
                        variant="default",
                        tooltip="Export to Obsidian JSON Canvas",
                    )
    ...
    async def create_mindmap(self) -> None:
        ...
        try:
            ...
            if total_selections == 0:
                self.notify(
                    "Please select content to create a mindmap", severity="warning"
                )
                return
    ...
    async def import_canvas_file(self) -> None:
        ...
        try:
            ...
            if not canvas_files:
                self.notify(
                    "No .canvas files found in Documents/tldw_exports",
                    severity="warning",
                )
                return
    ...
    async def _export_mindmap(self, format_type: str) -> None:
        ...
        try:
            ...
            if format_type == "markdown":
                content = MindmapExporter.to_markdown(
                    self.current_mindmap_root, include_metadata=True
                )
                filename = f"mindmap_{timestamp}.md"

            elif format_type == "flashcards":
                flashcards = MindmapExporter.to_flashcards(
                    self.current_mindmap_root, card_type="basic", include_context=True
                )
                if not flashcards:
                ...
                content = MindmapExporter.to_html(
                    self.current_mindmap_root, include_style=True, collapsible=True
                )
                filename = f"mindmap_{timestamp}.html"

            elif format_type == "graphviz":
                content = MindmapExporter.to_graphviz(
                    self.current_mindmap_root, rankdir="TB", include_style=True
                )
                filename = f"mindmap_{timestamp}.dot"

            elif format_type == "json":
                content = MindmapExporter.to_json(
                    self.current_mindmap_root, pretty=True
                )
