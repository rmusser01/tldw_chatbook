...
from textual.widgets import Static, Button, DataTable, OptionList, Footer
...
CHATBOOK_EXPORT_DELETE_DISABLED_TOOLTIP = (
    "Select an exported Chatbook before deleting it."
)
CHATBOOK_EXPORT_REEXPORT_DISABLED_TOOLTIP = (
    "Select an exported Chatbook before re-exporting it."
)
CHATBOOK_EXPORT_SHARE_DISABLED_TOOLTIP = (
    "Select an exported Chatbook before sharing it."
)
CHATBOOK_EXPORT_OPEN_DISABLED_TOOLTIP = (
    "Select an exported Chatbook before opening its folder."
)
...
SERVER_JOB_CANCEL_DISABLED_TOOLTIP = (
    "Select a running server Chatbook job before cancelling it."
)
SERVER_JOB_DOWNLOAD_DISABLED_TOOLTIP = (
    "Select a completed server export job before downloading it."
)
SERVER_JOB_REMOVE_DISABLED_TOOLTIP = (
    "Select a completed, failed, or cancelled server job before removing it."
)
SERVER_JOB_REQUIRES_SERVER_TOOLTIP = (
    "Server job actions require a server-owned Chatbook job."
)
...
SERVER_JOB_DOWNLOAD_INELIGIBLE_TOOLTIP = (
    "Only completed server export jobs can be downloaded."
)
SERVER_JOB_REMOVE_INELIGIBLE_TOOLTIP = (
    "Only completed, failed, or cancelled server jobs can be removed."
)
...
class ChatbookExportManagementWindow(ModalScreen):
    ...
    async def on_mount(self) -> None:
        ...
        await header_container.mount(
            Static("", id="chatbook-name", classes="details-title")
        )
        ...
        await preview_container.mount(
            Static("Content Summary:", classes="preview-title")
        )
        ...
        await row1.mount(
            Button(
                "📤 Re-export with Options", id="re-export-options", variant="primary"
            )
        )
        ...
        await row2.mount(
            Button("☁️ Upload to Cloud", id="share-cloud", variant="default")
        )
        ...
        try:
            ...
            for i, chatbook in enumerate(self.chatbook_files):
                ...
                option_list.add_option(
                    f"{chatbook['name']}\n  {size_mb:.1f} MB • {modified}", id=str(i)
                )
    ...
    async def _fetch_live_server_job_records(self) -> List[Dict[str, Any]]:
        ...
        try:
            lease = server_chatbook_service_lease(
                ...
                policy_enforcer=getattr(
                    self.app_instance, "service_policy_enforcer", None
                ),
            )
        ...
        try:
            ...
            return [
                *self._normalize_live_server_jobs(
                    "export", export_payload.get("jobs", [])
                ),
                *self._normalize_live_server_jobs(
                    "import", import_payload.get("jobs", [])
                ),
            ]
        except Exception as exc:
    ...
    def _update_server_job_action_buttons(self) -> None:
        ...
        download_button.disabled = not (
            is_remote and has_job_id and is_export and is_downloadable
        )
    ...
    async def _cancel_selected_server_job(self) -> None:
        ...
        if not record or record.get("source") != "server":
            self.app_instance.notify(
                "Select a remote server job to cancel.", severity="warning"
            )
            return
        ...
        if not job_id or job_type not in {"export", "import"}:
            self.app_instance.notify(
                "Selected server job cannot be cancelled.", severity="warning"
            )
            return
        ...
        try:
            ...
            self.app_instance.notify(
                f"Cancelled server {job_type} job {job_id}", severity="success"
            )
            await self.refresh_server_job_list()
        except Exception as exc:
            logger.opt(exception=True).error(
                f"Failed to cancel server chatbook job {job_id}: {exc}"
            )
            self.app_instance.notify(
                f"Failed to cancel server job: {exc}", severity="error"
            )
    ...
    async def _remove_selected_server_job(self) -> None:
        ...
        if not record or record.get("source") != "server":
            self.app_instance.notify(
                "Select a remote server job to remove.", severity="warning"
            )
            return
        ...
        if not job_id or job_type not in {"export", "import"}:
            self.app_instance.notify(
                "Selected server job cannot be removed.", severity="warning"
            )
            return
        ...
        try:
            ...
            self.app_instance.notify(
                f"Removed server {job_type} job {job_id}", severity="success"
            )
            await self.refresh_server_job_list()
        except Exception as exc:
            logger.opt(exception=True).error(
                f"Failed to remove server chatbook job {job_id}: {exc}"
            )
            self.app_instance.notify(
                f"Failed to remove server job: {exc}", severity="error"
            )

    def _server_download_filename(self, record: Dict[str, Any]) -> str:
        raw_name = str(
            record.get("chatbook_name") or record.get("job_id") or "server-export"
        ).strip()
    ...
    async def _download_selected_server_export(self) -> None:
        ...
        if not record or record.get("source") != "server":
            self.app_instance.notify(
                "Select a remote server export to download.", severity="warning"
            )
            return
        ...
        if (
            not job_id
            or job_type != "export"
            or status not in DOWNLOADABLE_SERVER_JOB_STATUSES
        ):
            self.app_instance.notify(
                "Selected server export is not ready to download.", severity="warning"
            )
            return
        ...
        try:
            ...
            self.app_instance.notify(
                f"Downloaded server export to {downloaded_path}", severity="success"
            )
            await self.refresh_chatbook_list()
        except Exception as exc:
            logger.opt(exception=True).error(
                f"Failed to download server chatbook export {job_id}: {exc}"
            )
            self.app_instance.notify(
                f"Failed to download server export: {exc}", severity="error"
            )
    ...
    async def _load_chatbook_details(self, index: int) -> None:
        ...
        self.query_one("#meta-size", Static).update(
            f"{chatbook['size'] / (1024 * 1024):.2f} MB"
        )
        self.query_one("#meta-created", Static).update(
            chatbook["created"].strftime("%Y-%m-%d %H:%M")
        )
        ...
        try:
            ...
            if manifest and not error:
                ...
                self.query_one("#meta-author", Static).update(
                ...
                if manifest.total_conversations > 0:
                    table.add_row(
                        "Conversations", str(manifest.total_conversations), "-"
                    )
                ...
                if (
                    len([i for i in manifest.content_items if i.type.value == "prompt"])
                    > 0
                ):
                    prompt_count = len(
                        [i for i in manifest.content_items if i.type.value == "prompt"]
                    )
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if button_id == "refresh-list":
            ...
            self.app_instance.notify(
                "Share functionality coming soon!", severity="info"
            )
            ...
            self.app_instance.notify(
                "Re-export with options coming soon!", severity="info"
            )
    ...
    async def _delete_selected(self) -> None:
        ...
        if result:
            try:
                ...
                self.app_instance.notify(
                    f"Deleted '{chatbook['name']}'", severity="success"
                )
    ...
    async def _re_export_chatbook(self) -> None:
        ...
        self.app_instance.notify(
            "Re-export functionality coming soon!", severity="info"
        )
    ...
    async def _open_location(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error opening folder: {str(e)}", severity="error"
            )
