...
class ProgressDashboard(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        if self.show_sparkline:
            with Container(classes="sparkline-container"):
                ...
                yield Sparkline([], id="throughput-sparkline", summary_function=max)

        # Status message
        yield Static(
            "Ready to start evaluation", id="status-message", classes="status-line"
        )
        ...
        with Horizontal(classes="control-buttons"):
            ...
            yield Button(
                "⏹️ Stop", id="stop-button", classes="control-button", variant="error"
            )
            yield Button(
                "📋 Details",
                id="details-button",
    ...
    def start_tracking(self, total_samples: int, task: str, model: str) -> None:
        ...
        self.metrics = ProgressMetrics(
            total_samples=total_samples, current_task=task, current_model=model
        )
        self.throughput_history.clear()
        self._update_display()
        self._update_status("Evaluation started")
        logger.info(
            f"Started tracking: {total_samples} samples, task={task}, model={model}"
        )
    ...
    def complete(self) -> None:
        """Mark as complete."""
        self._update_status(
            f"Evaluation completed! Success: {self.metrics.success_count}, Errors: {self.metrics.error_count}"
        )
