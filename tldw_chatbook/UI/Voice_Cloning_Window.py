...
class VoiceCloningWindow(Vertical):
    """
            ...
            with TabbedContent():
                ...
                with TabPane("Profile Management", id="profiles-tab"):
                    ...
                    with Horizontal(id="action-buttons"):
                        yield Button(
                            "➕ New Profile", id="new-profile-btn", variant="primary"
                        )
                        ...
                        yield Button(
                            "📤 Export", id="export-profile-btn", disabled=True
                        )
                ...
                with TabPane("Voice Testing", id="testing-tab"):
                    with Container(id="test-playground"):
                        ...
                        with Horizontal():
                            ...
                            yield Select(options=[], id="test-profile-select")

                        with Horizontal(classes="form-group"):
                            yield Button(
                                "🔊 Generate", id="test-generate-btn", variant="primary"
                            )
    ...
    async def _initialize_backends(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Initialized {len(self.backend_managers)} voice backend managers"
            )
...
"""
    ...
    async def _export_profile(self) -> None:
            ...
            if path:
                ...
                if manager:
                    success, message = manager.export_profile(
                        self.selected_profile, str(path)
                    )
                    if success:
                        self.notify(message, severity="information")
                    else:
        ...
        file_save = FileSave(title="Select Export Directory", show_dirs_only=True)
    ...
    async def _test_play_audio(self) -> None:
        ...
        self.notify(
            "Use the main TTS Playground to play generated audio",
    ...
    async def _test_save_audio(self) -> None:
        ...
        self.notify(
            "Use the main TTS Playground to save generated audio",
            severity="information",
        )
