...
class ToolsSettingsWindow(Container):
    ...
    def _compose_general_settings(self) -> ComposeResult:
        ...
        with TabbedContent(
            id="general-settings-tabs", classes="settings-tabbed-content"
        ):
            # General Tab
    ...
    def _compose_chat_defaults_settings(self) -> ComposeResult:
        ...
        with VerticalScroll(classes="settings-tab-content"):
            ...
            with Container(classes="settings-group"):
                yield Static(
                    "🤖 Default Provider & Model", classes="settings-group-title"
                )
                ...
                with Container(classes="settings-form-grid"):
                    ...
                    _readiness = get_provider_readiness(
                        current_chat_provider, self.config_data
                    )
    ...
    def _save_chat_api_key(self) -> bool:
        ...
        field_state = chat_api_key_field_state(
            readiness, locked=self._config_is_locked()
        )
        key_to_persist = chat_api_key_value_to_persist(
            api_key_widget.value, field_state
        )
        ...
        if not save_setting_to_cli_config(
            f"api_settings.{provider_key}", "api_key", key_to_persist
        ):
            return False
            ...
            with Container(classes="settings-group"):
                ...
                with Container(classes="settings-form-grid"):
                    ...
                    if current_char_provider not in providers:
                        current_char_provider = (
                            providers[0] if providers else "Anthropic"
                        )
                    yield Select(
                        options=provider_options,
    ...
    def _compose_security_settings(self) -> ComposeResult:
        ...
        with VerticalScroll(classes="settings-tab-content"):
            ...
            with Container(classes="settings-group"):
                yield Static(
                    "🔐 Config File Encryption", classes="settings-group-title"
                )
                ...
                if not encryption_enabled and check_encryption_needed():
                    ...
                    with Container(classes="settings-warning-box"):
                        yield Static("⚠️ Security Warning", classes="warning-title")
                        yield Static(
    ...
    def _compose_config_file_settings(self) -> ComposeResult:
        ...
        with Container(classes="config-file-settings-inner"):
            ...
            yield Static(
                "Edit all configuration values with organized sections or raw TOML",
                classes="section-description",
            )
            ...
            with TabbedContent(id="config-tabs"):
                ...
                with TabPane("Raw TOML", id="tab-raw-toml"):
                    yield Static(
                        "Direct TOML Configuration Editor", classes="tab-description"
                    )

                    # Search container
                    with Container(
                        id="config-search-container", classes="config-search-container"
                    ):
                        yield Input(
                    ...
                    with Container(classes="config-button-container"):
                        yield Button(
                            "Save TOML", id="save-config-button", variant="primary"
                        )
                ...
                with TabPane("General", id="tab-general-config"):
                    yield Static(
                        "Application General Settings", classes="tab-description"
                    )
                    with VerticalScroll():
                ...
                with TabPane("API Settings", id="tab-api-config"):
                    yield Static(
                        "API Provider Configurations", classes="tab-description"
                    )
                    with VerticalScroll():
                ...
                with TabPane("Providers", id="tab-providers-config"):
                    yield Static(
                        "Available Models by Provider", classes="tab-description"
                    )
                    with VerticalScroll():
                ...
                with TabPane("Character", id="tab-character-config"):
                    yield Static(
                        "Character Default Settings", classes="tab-description"
                    )
                    with VerticalScroll():
                ...
                with TabPane("Notes", id="tab-notes-config"):
                    yield Static(
                        "Notes Synchronization Settings", classes="tab-description"
                    )
                    with VerticalScroll():
                ...
                with TabPane("Advanced", id="tab-advanced-config"):
                    yield Static(
                        "Advanced Configuration Options", classes="tab-description"
                    )
                    with VerticalScroll():
    ...
    def _compose_general_config_form(self) -> ComposeResult:
        ...
        yield Container(
            ...
            Static(
                "Shows an animated splash screen with app information when starting tldw-cli",
                classes="help-text",
            ),
            Container(
                Button(
                    "Save General Config",
                    id="save-general-config-form",
                    variant="primary",
                ),
    ...
    def _compose_api_config_form(self) -> ComposeResult:
        ...
        for display_name, key, fields in providers:
            widgets.append(
                Static(f"{display_name} Configuration", classes="form-section-title")
            )
            ...
            for field in fields:
                if field == "api_key":
                    ...
                    widgets.append(
                        Label("API Key Environment Variable:", classes="form-label")
                    )
                    widgets.append(
                        Input(
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to get models for provider {display_name}: {e}"
                        )
                        models = []
        ...
        for display_name, key, fields in local_providers:
            widgets.append(
                Static(f"{display_name} Configuration", classes="form-subsection-title")
            )
            provider_config = api_settings.get(key, {})
            ...
            for field in fields:
                if field == "api_url":
                    ...
                    widgets.append(
                        Label("Default System Prompt:", classes="form-label")
                    )
                    widgets.append(
                        TextArea(
                            text=provider_config.get(
                                field, "You are a helpful AI assistant"
                            ),
                            id=f"config-{key}-{field}",
    ...
    def _compose_database_config_form(self) -> ComposeResult:
        ...
        yield Container(
            ...
            Input(
                value=str(
                    get_cli_setting("media_cleanup", "cleanup_interval_hours", 24)
                ),
                placeholder="24",
            ...
            Container(
                Button(
                    ...
                    id="save-database-config-form",
                    variant="primary",
                ),
                Button("Reset Section", id="reset-database-config-form"),
                Button(
                    "Vacuum All Databases", id="vacuum-all-databases", variant="warning"
                ),
                Button(
                    "Run Media Cleanup Now",
                    id="run-media-cleanup-now",
                    variant="default",
                ),
                classes="form-actions",
            ),
    ...
    def _compose_rag_config_form(self) -> ComposeResult:
        ...
        widgets.append(
            Input(value=str(rag_config.get("default_top_k", 10)), id="config-rag-top-k")
        )
        ...
        widgets.append(
            Collapsible(
                *retriever_widgets, title="Retriever Configuration", collapsed=True
            )
        )

        # Processor Settings
        processor_widgets = []
        processor_widgets.append(
            Checkbox(
                "Enable Re-ranking",
        ...
        processor_widgets.append(
            Label("Deduplication Threshold (0.0-1.0):", classes="form-label")
        )
        processor_widgets.append(
            Input(
        ...
        widgets.append(
            Collapsible(
                *processor_widgets, title="Processor Configuration", collapsed=True
            )
        )
        ...
        widgets.append(
            Collapsible(
                *generator_widgets, title="Generator Configuration", collapsed=True
            )
        )
        ...
        widgets.append(
            Input(
                value=str(chunking_config.get("size", 512)), id="config-rag-chunk-size"
            )
        )
        ...
        widgets.append(
            Collapsible(*chroma_widgets, title="ChromaDB Configuration", collapsed=True)
        )
    ...
    def _compose_providers_config_form(self) -> ComposeResult:
        ...
        for provider, models in providers_config.items():
            ...
            widgets.append(
                Label("Available Models (comma-separated):", classes="form-label")
            )
            widgets.append(
                TextArea(
                    text=models_str,
    ...
    def _compose_chat_config_form(self) -> ComposeResult:
        ...
        image_widgets.append(
            Label("Supported Formats (comma-separated):", classes="form-label")
        )
        from tldw_chatbook.Chat.attachment_core import DEFAULT_SUPPORTED_IMAGE_FORMATS

        formats = chat_images_config.get(
        ...
        widgets.append(
            Collapsible(*image_widgets, title="Image Support Settings", collapsed=True)
        )
        ...
        widgets.append(
            Container(
                Button(
                    "Save Chat Config", id="save-chat-config-form", variant="primary"
                ),
    ...
    def _compose_character_config_form(self) -> ComposeResult:
        ...
        widgets.append(
            Static("Character Default Settings", classes="form-section-title")
        )
    ...
    def _compose_tts_config_form(self) -> ComposeResult:
        ...
        widgets.append(
            Static("Text-to-Speech Configuration", classes="form-section-title")
        )
        ...
        widgets.append(
            Input(
                value=tts_config.get(
                    "KOKORO_ONNX_MODEL_PATH_DEFAULT", "path/to/kokoro-v0_19.onnx"
                ),
        ...
        widgets.append(
            Input(
                value=tts_config.get(
                    "KOKORO_ONNX_VOICES_JSON_DEFAULT", "path/to/voices.json"
                ),
    ...
    def _compose_embedding_config_form(self) -> ComposeResult:
        ...
        widgets.append(
            Label("Default LLM for Contextualization:", classes="form-label")
        )
        widgets.append(
            Input(
                value=embedding_config.get(
        ...
        for model_id, model_config in models.items():
            ...
            widgets.append(
                Collapsible(*model_widgets, title=f"Model: {model_id}", collapsed=True)
            )
    ...
    def _compose_advanced_config_form(self) -> ComposeResult:
        ...
        widgets = [
            Static(
                "Advanced settings - modify with caution", classes="form-description"
            ),
        ...
        with Container(classes="settings-container"):
            ...
            yield Collapsible(
                ...
                Button(
                    ...
                    id="db-check-integrity",
                    variant="warning",
                ),
                Static(
                    "Perform operations on all databases at once", classes="help-text"
                ),
            ...
            yield Collapsible(
                ...
                Container(
                    ...
                    Button(
                        "Import Data", id="db-import-chachanotes", variant="primary"
                    ),
    ...
    def _compose_appearance_settings(self) -> ComposeResult:
        ...
        with Container(classes="settings-container"):
            ...
            current_theme = self.config_data.get("general", {}).get(
            ...
            yield Checkbox(
                "Enable UI Animations", value=True, id="appearance-enable-animations"
            )

            yield Checkbox(
                "Enable Smooth Scrolling", value=True, id="appearance-smooth-scrolling"
            )
    ...
    def _compose_encryption_config_form(self) -> ComposeResult:
        ...
        if encryption_enabled:
            ...
            yield Static(
                "🔓 Encryption is DISABLED", classes="encryption-status-disabled"
            )
            if has_api_keys:
                yield Static(
                    f"⚠️ Detected unencrypted API keys for: {', '.join(detected_providers)}",
                    classes="encryption-warning",
                )
            else:
                yield Static(
                    "No API keys detected in configuration.",
                    classes="section-description",
                )
        ...
        with Container(classes="encryption-controls"):
            if not encryption_enabled:
                ...
                with Container(classes="encryption-button-group"):
                    yield Button(
                        "Change Password",
                        id="change-encryption-password-button",
                        variant="default",
                        classes="settings-button",
                    )
                    yield Button(
                        "Disable Encryption",
                        id="disable-encryption-button",
                        variant="warning",
                        classes="settings-button",
                    )
    ...
    def _compose_tool_settings(self) -> ComposeResult:
        ...
        yield Static(
            "Configure which tools are available for LLM function calling",
            classes="section-description",
        )
        ...
        with Container(classes="settings-container"):
            ...
            with Horizontal(classes="button-row"):
                yield Button(
                    "Save Tool Settings", id="save-tool-settings", variant="primary"
                )
                yield Button(
                    "Reset Tool Settings", id="reset-tool-settings", variant="warning"
                )
...
Thank you for using tldw-chatbook! 🎉
    ...
    def compose(self) -> ComposeResult:
        with Container(id="tools-settings-nav-pane", classes="tools-nav-pane"):
            ...
            yield Button(
                "General Settings",
                id="ts-nav-general-settings",
                classes="ts-nav-button active-nav",
            )
            yield Button(
                "Configuration File Settings",
                id="ts-nav-config-file-settings",
                classes="ts-nav-button",
            )
            yield Button(
        ...
        with ContentSwitcher(
            id="tools-settings-content-pane",
            classes="tools-content-pane",
            initial="ts-view-general-settings",
        ):
            yield Container(
    ...
    async def on_markdown_link_clicked(self, event: Markdown.LinkClicked) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Opening {event.href} in browser...", severity="information"
            )
        except Exception as e:
    ...
    async def _save_general_settings(self) -> None:
        ...
        try:
            ...
            if save_setting_to_cli_config(
                "general",
                "default_tab",
                self.query_one("#general-default-tab", Select).value,
            ):
                saved_count += 1
            ...
            if save_setting_to_cli_config(
                "general",
                "default_theme",
                self.query_one("#general-theme", Select).value,
            ):
                saved_count += 1
            ...
            if save_setting_to_cli_config(
                "general",
                "log_level",
                self.query_one("#general-log-level", Select).value,
            ):
                saved_count += 1
            ...
            try:
                splash_duration = float(
                    self.query_one("#general-splash-duration", Input).value
                )
                if save_setting_to_cli_config(
                    "splash_screen", "duration", splash_duration
                ):
                    saved_count += 1
            except ValueError:
                self.app_instance.notify(
                    "Invalid splash screen duration value", severity="warning"
                )
            ...
            if save_setting_to_cli_config(
                "chat_defaults",
                "provider",
                self.query_one("#general-chat-provider", Select).value,
            ):
                saved_count += 1
            ...
            if save_setting_to_cli_config(
                "chat_defaults",
                "model",
                self.query_one("#general-chat-model", Input).value,
            ):
                saved_count += 1
            try:
                temp_value = float(
                    self.query_one("#general-chat-temperature", Input).value
                )
                if save_setting_to_cli_config(
                    "chat_defaults", "temperature", temp_value
                ):
                    saved_count += 1
            except ValueError:
                self.app_instance.notify(
            ...
            if save_setting_to_cli_config(
                "character_defaults",
                "provider",
                self.query_one("#general-character-provider", Select).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "character_defaults",
                "model",
                self.query_one("#general-character-model", Input).value,
            ):
                saved_count += 1
            try:
                temp_value = float(
                    self.query_one("#general-character-temperature", Input).value
                )
                if save_setting_to_cli_config(
                    "character_defaults", "temperature", temp_value
                ):
                    saved_count += 1
            except ValueError:
                self.app_instance.notify(
                    "Invalid character temperature value", severity="warning"
                )

            # Config Encryption
            encryption_enabled = self.query_one(
                "#general-encryption-enabled", Checkbox
            ).value
            current_encryption = self.config_data.get("encryption", {}).get(
                "enabled", False
            )
                ...
                if encryption_enabled:
                    ...
                    if password:
                        ...
                        if enable_config_encryption(password):
                            ...
                            self.app_instance.notify(
                                "❌ Failed to enable encryption", severity="error"
                            )
                        ...
                        self.app_instance.notify(
                            "Encryption setup cancelled", severity="warning"
                        )
                else:
                    ...
                    if password:
                        ...
                        if disable_config_encryption(password):
                            ...
                            self.app_instance.notify(
                                "❌ Invalid password or failed to disable encryption",
                                severity="error",
                            )
                        ...
                        self.app_instance.notify(
                            "Encryption disable cancelled", severity="warning"
                        )
            ...
            if saved_count > 0:
                self.app_instance.notify(
                    f"General Settings saved successfully! ({saved_count} settings updated)"
                )
            else:
            ...
            self.app_instance.notify(
                f"Error saving General Settings: {e}", severity="error"
            )
    ...
    async def _reset_general_settings(self) -> None:
        ...
        try:
            ...
            self.query_one(
                "#general-splash-enabled", Checkbox
            ).value = True  # Default is enabled
            self.query_one(
                "#general-splash-duration", Input
            ).value = "1.5"  # Default duration
            ...
            default_chat_provider = (
                ...
                else (providers[0] if providers else "OpenAI")
            )
            self.query_one(
                "#general-chat-provider", Select
            ...
            default_char_provider = (
                ...
                else (providers[0] if providers else "Anthropic")
            )
            self.query_one(
            ...
            self.app_instance.notify(
                f"Error resetting General Settings: {e}", severity="error"
            )
    ...
    async def _preview_theme(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Previewing theme: {selected_theme}. Save settings to make permanent.",
                timeout=5,
            )
        except Exception as e:
    ...
    async def _test_chat_connection(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"❌ Connection test failed: {str(e)}", severity="error"
            )
    ...
    async def _view_chat_models(self) -> None:
        ...
        try:
            ...
            if models:
                ...
                self.app_instance.notify(
                    f"No models found for {provider}", severity="warning"
                )
        except Exception as e:
    ...
    async def _import_character_quick(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error importing character: {e}", severity="error"
            )
    ...
    async def _enable_encryption_quick(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error enabling encryption: {e}", severity="error"
            )
    ...
    async def _disable_encryption_quick(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error disabling encryption: {e}", severity="error"
            )
    ...
    async def _manage_api_keys(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error opening API settings: {e}", severity="error"
            )
    ...
    async def _test_all_api_keys(self) -> None:
        ...
        try:
            ...
            if not providers:
                self.app_instance.notify(
                    "No API providers configured", severity="warning"
                )
                return
    ...
    async def _save_raw_toml_config(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error: Invalid TOML format: {e}", severity="error"
            )
        except IOError as e:
            self.app_instance.notify(
                f"Error: Could not write to configuration file: {e}", severity="error"
            )
        except Exception as e:
            self.app_instance.notify(
                f"Error saving configuration: {e}", severity="error"
            )
    ...
    async def _reload_raw_toml_config(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error reloading configuration: {e}", severity="error"
            )
    ...
    async def _validate_toml_config(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                "TOML configuration is valid!", severity="information"
            )
        except toml.TomlDecodeError as e:
    ...
    async def _save_general_config_form(self) -> None:
        ...
        try:
            save_setting_to_cli_config(
                "general",
                "default_tab",
                self.query_one("#config-general-default-tab", Select).value,
            )
            save_setting_to_cli_config(
                "general",
                "default_theme",
                self.query_one("#config-general-theme", Input).value,
            )
            save_setting_to_cli_config(
                "general",
                "palette_theme_limit",
                int(self.query_one("#config-general-palette-limit", Input).value),
            )
            save_setting_to_cli_config(
                "general",
                "log_level",
                self.query_one("#config-general-log-level", Input).value,
            )
            save_setting_to_cli_config(
                "general",
                "users_name",
                self.query_one("#config-general-users-name", Input).value,
            )

            # Save splash screen settings
            splash_enabled = self.query_one(
                "#config-general-splash-enabled", Checkbox
            ).value
            save_setting_to_cli_config("splash_screen", "enabled", splash_enabled)

            try:
                splash_duration = float(
                    self.query_one("#config-general-splash-duration", Input).value
                )
            ...
            self.app_instance.notify(
                f"Error saving general config: {e}", severity="error"
            )
    ...
    async def _reset_general_config_form(self) -> None:
        ...
        try:
            ...
            self.query_one(
                "#config-general-splash-duration", Input
            ).value = "1.5"  # Default duration
            self.app_instance.notify("General configuration reset to defaults!")
        except Exception as e:
            self.app_instance.notify(
                f"Error resetting general config: {e}", severity="error"
            )
    ...
    async def _save_api_config_form(self) -> None:
        ...
        try:
            ...
            for provider_key, fields in all_providers:
                for field in fields:
                    ...
                    try:
                        ...
                        if save_setting_to_cli_config(
                            f"api_settings.{provider_key}", field, value
                        ):
                            saved_count += 1
            ...
            if errors:
                self.app_instance.notify(
                    f"API config saved with {len(errors)} errors. Check logs for details.",
                    severity="warning",
                )
                ...
                self.app_instance.notify(
                    f"API configuration saved! ({saved_count} settings updated)"
                )
            else:
                self.app_instance.notify(
                    "No API settings were updated", severity="warning"
                )
    ...
    async def _save_database_config_form(self) -> None:
        ...
        try:
            ...
            save_setting_to_cli_config(
                "database",
                "USER_DB_BASE_DIR",
                self.query_one("#config-db-base-dir", Input).value,
            )
            ...
            save_setting_to_cli_config(
                "media_cleanup",
                "enabled",
                self.query_one("#config-media-cleanup-enabled", Checkbox).value,
            )
            save_setting_to_cli_config(
                "media_cleanup",
                "cleanup_days",
                int(self.query_one("#config-media-cleanup-days", Input).value),
            )
            save_setting_to_cli_config(
                "media_cleanup",
                "cleanup_interval_hours",
                int(self.query_one("#config-media-cleanup-interval", Input).value),
            )
            save_setting_to_cli_config(
                "media_cleanup",
                "cleanup_on_startup",
                self.query_one("#config-media-cleanup-on-startup", Checkbox).value,
            )

            self.app_instance.notify("Database configuration saved!")
        except Exception as e:
            self.app_instance.notify(
                f"Error saving database config: {e}", severity="error"
            )
    ...
    async def _reset_database_config_form(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error resetting database config: {e}", severity="error"
            )
    ...
    async def _save_rag_config_form(self) -> None:
        ...
        try:
            ...
            if save_setting_to_cli_config(
                "rag_search",
                "default_mode",
                self.query_one("#config-rag-search-mode", Select).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search",
                "default_top_k",
                int(self.query_one("#config-rag-top-k", Input).value),
            ):
                saved_count += 1

            # Sources
            if save_setting_to_cli_config(
                "rag_search.default_sources",
                "media",
                self.query_one("#config-rag-source-media", Checkbox).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.default_sources",
                "conversations",
                self.query_one("#config-rag-source-conversations", Checkbox).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.default_sources",
                "notes",
                self.query_one("#config-rag-source-notes", Checkbox).value,
            ):
                saved_count += 1

            # Retriever settings
            if save_setting_to_cli_config(
                "rag_search.retriever",
                "fts_top_k",
                int(self.query_one("#config-rag-retriever-fts-top-k", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.retriever",
                "vector_top_k",
                int(self.query_one("#config-rag-retriever-vector-top-k", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.retriever",
                "hybrid_alpha",
                float(
                    self.query_one("#config-rag-retriever-hybrid-alpha", Input).value
                ),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.retriever",
                "media_collection",
                self.query_one("#config-rag-retriever-media-collection", Input).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.retriever",
                "chat_collection",
                self.query_one("#config-rag-retriever-chat-collection", Input).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.retriever",
                "notes_collection",
                self.query_one("#config-rag-retriever-notes-collection", Input).value,
            ):
                saved_count += 1

            # Processor settings
            if save_setting_to_cli_config(
                "rag_search.processor",
                "enable_reranking",
                self.query_one(
                    "#config-rag-processor-enable-reranking", Checkbox
                ).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.processor",
                "reranker_model",
                self.query_one("#config-rag-processor-reranker-model", Input).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.processor",
                "reranker_top_k",
                int(
                    self.query_one("#config-rag-processor-reranker-top-k", Input).value
                ),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                ...
                float(
                    self.query_one("#config-rag-processor-deduplication", Input).value
                ),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.processor",
                "max_context_length",
                int(self.query_one("#config-rag-processor-max-context", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.processor",
                "combination_method",
                self.query_one("#config-rag-processor-combination", Select).value,
            ):
                saved_count += 1

            # Generator settings
            if save_setting_to_cli_config(
                "rag_search.generator",
                "default_model",
            ...
            if save_setting_to_cli_config(
                "rag_search.generator",
                "default_temperature",
                float(self.query_one("#config-rag-generator-temperature", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.generator",
                "max_tokens",
                int(self.query_one("#config-rag-generator-max-tokens", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.generator",
                "enable_streaming",
                self.query_one("#config-rag-generator-streaming", Checkbox).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.generator",
                "stream_chunk_size",
                int(self.query_one("#config-rag-generator-chunk-size", Input).value),
            ):
                saved_count += 1
            ...
            if save_setting_to_cli_config(
                "rag_search.chunking",
                "size",
                int(self.query_one("#config-rag-chunk-size", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.chunking",
                "overlap",
                int(self.query_one("#config-rag-chunk-overlap", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.chunking",
                "method",
                self.query_one("#config-rag-chunk-method", Select).value,
            ):
                saved_count += 1
            ...
            if save_setting_to_cli_config(
                "rag_search.chroma",
                "collection_prefix",
                self.query_one("#config-rag-chroma-prefix", Input).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.chroma",
                "embedding_model",
                self.query_one("#config-rag-chroma-embedding-model", Input).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.chroma",
                "embedding_dimension",
                int(self.query_one("#config-rag-chroma-dimension", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "rag_search.chroma",
                "distance_metric",
                self.query_one("#config-rag-chroma-distance", Select).value,
            ):
                saved_count += 1
            ...
            if saved_count > 0:
                self.app_instance.notify(
                    f"RAG configuration saved! ({saved_count} settings updated)"
                )
            else:
                self.app_instance.notify(
                    "No RAG settings were updated", severity="warning"
                )
    ...
    async def _reset_rag_config_form(self) -> None:
        ...
        try:
            ...
            self.query_one(
                "#config-rag-retriever-media-collection", Input
            ).value = "media_embeddings"
            self.query_one(
                "#config-rag-retriever-chat-collection", Input
            ).value = "chat_embeddings"
            self.query_one(
                "#config-rag-retriever-notes-collection", Input
            ...
            self.query_one(
                "#config-rag-processor-reranker-model", Input
            ...
            self.query_one(
                "#config-rag-processor-combination", Select
            ).value = "weighted"
            ...
            self.query_one(
                "#config-rag-chroma-embedding-model", Input
            ...
            self.app_instance.notify(
                f"Error resetting RAG config: {e}", severity="error"
            )
    ...
    async def _save_providers_config_form(self) -> None:
        ...
        try:
            ...
            for provider in providers_config.keys():
                ...
                try:
                    ...
                    if models_text:
                        models_list = [
                            model.strip()
                            for model in models_text.split(",")
                            if model.strip()
                        ]
            ...
            self.app_instance.notify(
                f"Error saving providers config: {e}", severity="error"
            )
    ...
    async def _reset_providers_config_form(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error resetting providers config: {e}", severity="error"
            )
    ...
    async def _save_advanced_config_form(self) -> None:
        ...
        try:
            ...
            save_setting_to_cli_config(
                "logging",
                "log_filename",
                self.query_one("#config-logging-filename", Input).value,
            )
            save_setting_to_cli_config(
                "logging",
                "file_log_level",
                self.query_one("#config-logging-file-level", Input).value,
            )
            save_setting_to_cli_config(
                "logging",
                "log_max_bytes",
                int(self.query_one("#config-logging-max-bytes", Input).value),
            )
            save_setting_to_cli_config(
                "logging",
                "log_backup_count",
                int(self.query_one("#config-logging-backup-count", Input).value),
            )
            ...
            self.app_instance.notify(
                f"Error saving advanced config: {e}", severity="error"
            )
    ...
    async def _reset_advanced_config_form(self) -> None:
        ...
        try:
            ...
            # Reset endpoints
            self.query_one(
                "#config-endpoint-llama_cpp", Input
            ).value = "http://localhost:8080"
            self.query_one(
                "#config-endpoint-koboldcpp", Input
            ).value = "http://localhost:5001/api"
            self.query_one(
                "#config-endpoint-oobabooga", Input
            ).value = "http://localhost:5000/api"
            self.query_one(
                "#config-endpoint-ollama", Input
            ).value = "http://localhost:11434"
            self.query_one(
                "#config-endpoint-vllm", Input
            ).value = "http://localhost:8000"
            self.query_one(
                "#config-endpoint-custom", Input
            ).value = "http://localhost:1234/v1"

            self.app_instance.notify("Advanced configuration reset to defaults!")
        except Exception as e:
            self.app_instance.notify(
                f"Error resetting advanced config: {e}", severity="error"
            )
    ...
    async def _export_configuration(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error exporting configuration: {e}", severity="error"
            )
    ...
    async def _import_configuration(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                "Import feature not yet implemented. Please manually copy TOML content.",
                severity="warning",
            )
        except Exception as e:
            self.app_instance.notify(
                f"Error importing configuration: {e}", severity="error"
            )
    ...
    async def _save_chat_config_form(self) -> None:
        ...
        try:
            ...
            if save_setting_to_cli_config(
                "chat_defaults",
                "provider",
                self.query_one("#config-chat-provider", Select).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat_defaults",
                "model",
                self.query_one("#config-chat-model", Input).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat_defaults",
                "system_prompt",
                self.query_one("#config-chat-system-prompt", TextArea).text,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat_defaults",
                "temperature",
                float(self.query_one("#config-chat-temperature", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat_defaults",
                "top_p",
                float(self.query_one("#config-chat-top-p", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat_defaults",
                "min_p",
                float(self.query_one("#config-chat-min-p", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat_defaults",
                "top_k",
                int(self.query_one("#config-chat-top-k", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat_defaults",
            ...
            if save_setting_to_cli_config(
                "chat.images",
                "enabled",
                self.query_one("#config-chat-images-enabled", Checkbox).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat.images",
                "show_attach_button",
                self.query_one("#config-chat-images-show-button", Checkbox).value,
            ):
                saved_count += 1
            ...
            if save_setting_to_cli_config(
                "chat.voice",
                "show_mic_button",
                self.query_one("#config-chat-voice-show-button", Checkbox).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat.images",
                "default_render_mode",
                self.query_one("#config-chat-images-render-mode", Select).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat.images",
                "max_size_mb",
                float(self.query_one("#config-chat-images-max-size", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat.images",
                "auto_resize",
                self.query_one("#config-chat-images-auto-resize", Checkbox).value,
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat.images",
                "resize_max_dimension",
                int(self.query_one("#config-chat-images-max-dimension", Input).value),
            ):
                saved_count += 1
            if save_setting_to_cli_config(
                "chat.images",
                "save_location",
                self.query_one("#config-chat-images-save-location", Input).value,
            ):
                saved_count += 1
            ...
            if save_setting_to_cli_config(
                "chat.images", "supported_formats", formats_list
            ):
                saved_count += 1
            ...
            if saved_count > 0:
                self.app_instance.notify(
                    f"Chat configuration saved! ({saved_count} settings updated)"
                )
            else:
                self.app_instance.notify(
                    "No chat settings were updated", severity="warning"
                )
    ...
    async def _reset_chat_config_form(self) -> None:
        ...
        try:
            ...
            self.query_one(
                "#config-chat-system-prompt", TextArea
            ...
            self.query_one(
                "#config-chat-images-save-location", Input
            ).value = "~/Downloads"
            self.query_one(
                "#config-chat-images-formats", Input
            ).value = ".png, .jpg, .jpeg, .gif, .webp, .bmp"

            self.app_instance.notify("Chat configuration reset to defaults!")
        except Exception as e:
            self.app_instance.notify(
                f"Error resetting chat config: {e}", severity="error"
            )
    ...
    async def _save_character_config_form(self) -> None:
        ...
        try:
            save_setting_to_cli_config(
                "character_defaults",
                "provider",
                self.query_one("#config-character-provider", Select).value,
            )
            save_setting_to_cli_config(
                "character_defaults",
                "model",
                self.query_one("#config-character-model", Input).value,
            )
            save_setting_to_cli_config(
                "character_defaults",
                "system_prompt",
                self.query_one("#config-character-system-prompt", TextArea).text,
            )
            save_setting_to_cli_config(
                "character_defaults",
                "temperature",
                float(self.query_one("#config-character-temperature", Input).value),
            )
            save_setting_to_cli_config(
                "character_defaults",
                "top_p",
                float(self.query_one("#config-character-top-p", Input).value),
            )
            save_setting_to_cli_config(
                "character_defaults",
                "min_p",
                float(self.query_one("#config-character-min-p", Input).value),
            )
            save_setting_to_cli_config(
                "character_defaults",
                "top_k",
                int(self.query_one("#config-character-top-k", Input).value),
            )
            ...
            self.app_instance.notify(
                f"Error saving character config: {e}", severity="error"
            )
    ...
    async def _reset_character_config_form(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error resetting character config: {e}", severity="error"
            )
    ...
    async def _save_tts_config_form(self) -> None:
        ...
        try:
            save_setting_to_cli_config(
                ...
                self.query_one("#config-tts-openai-key", Input).value,
            )
            save_setting_to_cli_config(
                "app_tts",
                "KOKORO_ONNX_MODEL_PATH_DEFAULT",
                self.query_one("#config-tts-kokoro-model-path", Input).value,
            )
            save_setting_to_cli_config(
                "app_tts",
                "KOKORO_ONNX_VOICES_JSON_DEFAULT",
                self.query_one("#config-tts-kokoro-voices-path", Input).value,
            )
            save_setting_to_cli_config(
                "app_tts",
                "KOKORO_DEVICE_DEFAULT",
                self.query_one("#config-tts-kokoro-device", Select).value,
            )
            save_setting_to_cli_config(
                "app_tts",
                "ELEVENLABS_API_KEY_fallback",
                self.query_one("#config-tts-elevenlabs-key", Input).value,
            )
    ...
    async def _reset_tts_config_form(self) -> None:
        ...
        try:
            ...
            self.query_one(
                "#config-tts-kokoro-model-path", Input
            ).value = "path/to/kokoro-v0_19.onnx"
            self.query_one(
                "#config-tts-kokoro-voices-path", Input
            ).value = "path/to/voices.json"
            ...
            self.app_instance.notify(
                f"Error resetting TTS config: {e}", severity="error"
            )
    ...
    async def _save_embedding_config_form(self) -> None:
        ...
        try:
            save_setting_to_cli_config(
                "embedding_config",
                "default_model_id",
                self.query_one("#config-embedding-default-model", Select).value,
            )
            save_setting_to_cli_config(
                "embedding_config",
                "default_llm_for_contextualization",
                self.query_one("#config-embedding-default-llm", Input).value,
            )
            ...
            for model_id in models.keys():
                try:
                    save_setting_to_cli_config(
                        f"embedding_config.models.{model_id}",
                        "provider",
                        self.query_one(
                            f"#config-embedding-{model_id}-provider", Input
                        ).value,
                    )
                    save_setting_to_cli_config(
                        f"embedding_config.models.{model_id}",
                        "model_name_or_path",
                        self.query_one(
                            f"#config-embedding-{model_id}-name", Input
                        ).value,
                    )
                    save_setting_to_cli_config(
                        f"embedding_config.models.{model_id}",
                        ...
                        int(
                            self.query_one(
                                f"#config-embedding-{model_id}-dimension", Input
                            ).value
                        ),
                    )
                    ...
                    if models[model_id].get("provider") == "openai":
                        api_key_widget = self.query_one(
                            f"#config-embedding-{model_id}-api-key", Input
                        )
                        if api_key_widget:
                            save_setting_to_cli_config(
                                f"embedding_config.models.{model_id}",
            ...
            self.app_instance.notify(
                f"Error saving embedding config: {e}", severity="error"
            )
    ...
    async def _reset_embedding_config_form(self) -> None:
        ...
        try:
            self.query_one(
                "#config-embedding-default-model", Select
            ).value = "e5-small-v2"
            self.query_one(
                "#config-embedding-default-llm", Input
            ).value = "gpt-3.5-turbo"

            # Reset model-specific settings would be complex, so just notify
            self.app_instance.notify(
                "Embedding configuration partially reset. Model-specific settings retained."
            )
        except Exception as e:
            self.app_instance.notify(
    ...
    async def _show_view(self, view_id: str) -> None:
        ...
        content_switcher = self.query_one(
            "#tools-settings-content-pane", ContentSwitcher
        )
    ...
    async def _run_media_cleanup_now(self) -> None:
        ...
        try:
            self.app_instance.notify(
                "Starting media cleanup...", severity="information"
            )
            ...
            self.app_instance.notify(
                f"Error running media cleanup: {e}", severity="error"
            )
    ...
    async def _vacuum_databases(self) -> None:
        ...
        try:
            self.app_instance.notify(
                "Starting database vacuum operation...", severity="information"
            )
            ...
            self.app_instance.notify(
                f"Error starting vacuum operation: {e}", severity="error"
            )
    ...
    async def _check_database_integrity(self) -> None:
        ...
        try:
            self.app_instance.notify(
                "Checking database integrity...", severity="information"
            )
            ...
            self.app_instance.notify(
                f"Error starting integrity check: {e}", severity="error"
            )
    ...
    async def _export_conversations(self) -> None:
        ...
        try:
            self.app_instance.notify(
                "Exporting conversations...", severity="information"
            )

            # Run export in a worker
            self.run_worker(
                self._export_conversations_worker,
                thread=True,
                name="export_conv_worker",
            )

        except Exception as e:
            self.app_instance.notify(
                f"Error exporting conversations: {e}", severity="error"
            )
    ...
    async def _export_notes(self) -> None:
        ...
        try:
            ...
            # Run export in a worker
            self.run_worker(
                self._export_notes_worker, thread=True, name="export_notes_worker"
            )
    ...
    async def _export_characters(self) -> None:
        ...
        try:
            ...
            # Run export in a worker
            self.run_worker(
                self._export_characters_worker, thread=True, name="export_chars_worker"
            )

        except Exception as e:
            self.app_instance.notify(
                f"Error exporting characters: {e}", severity="error"
            )
    ...
    async def _import_data(self) -> None:
        ...
        self.app_instance.notify(
            "Import functionality coming soon!", severity="information"
        )
    ...
    async def _clear_conversations(self) -> None:
        ...
        self.app_instance.notify(
            "Clear conversations requires confirmation dialog (coming soon)",
            severity="warning",
        )
    ...
    async def _clear_notes(self) -> None:
        ...
        # TODO: Implement confirmation dialog
        self.app_instance.notify(
            "Clear notes requires confirmation dialog (coming soon)", severity="warning"
        )
    ...
    async def _reset_databases(self) -> None:
        ...
        self.app_instance.notify(
            "Database reset requires double confirmation (coming soon)",
            severity="warning",
        )
    ...
    async def _create_chatbook(self) -> None:
        ...
        try:
            ...
            if result:
                self.app_instance.notify(
                    f"Chatbook saved to: {result}", severity="success"
                )
    ...
    async def _setup_encryption(self) -> None:
        ...
        try:
            ...
            if not detected_providers:
                self.app_instance.notify(
                    "No API keys detected to encrypt.", severity="warning"
                )
                return
            ...
            password = await self.app_instance.push_screen(
                PasswordDialog(
                    mode="setup", on_submit=lambda p: None, on_cancel=lambda: None
                ),
                wait_for_dismiss=True,
            )
            ...
            if password:
                ...
                if enable_config_encryption(password):
                    self.app_instance.notify(
                        "✅ Config encryption enabled successfully!",
                        severity="information",
                    )
                    # Refresh the UI
            ...
            self.app_instance.notify(
                f"Error setting up encryption: {e}", severity="error"
            )
        ...
        try:
            ...
            if new_password:
                ...
                if change_encryption_password(current_password, new_password):
                    self.app_instance.notify(
                        "✅ Password changed successfully!", severity="information"
                    )
                else:
                    self.app_instance.notify(
    ...
    async def _disable_encryption(self) -> None:
        ...
        try:
                ...
                def compose(self) -> ComposeResult:
                    with Container():
                        with Vertical():
                            ...
                            yield Static(
                                "This will decrypt all API keys and save them in plain text."
                            )
                            ...
                            with Container(classes="button-container"):
                                ...
                                yield Button(
                                    "Disable Encryption", id="confirm", variant="error"
                                )
            ...
            confirm = await self.app_instance.push_screen(
                ConfirmDisableDialog(), wait_for_dismiss=True
            )
            ...
            if password:
                ...
                if disable_config_encryption(password):
                    self.app_instance.notify(
                        "✅ Encryption disabled. API keys are now in plain text.",
                        severity="warning",
                    )
                    # Refresh the UI
                    await self._refresh_encryption_ui()
                    ...
                    self.app_instance.notify(
                        "❌ Failed to disable encryption. Check password.",
                        severity="error",
                    )
    ...
    async def _clear_config_search_results(self) -> None:
        ...
        try:
            ...
            collapsible = self.query_one(
                "#config-search-results-collapsible", Collapsible
            )
    ...
    async def on_config_result_selected(
        self, event: ConfigSearchResult.Selected
    ) -> None:
        ...
        try:
            ...
            # Show notification
            self.app_instance.notify(
                f"Navigated to: {event.label}", severity="information", timeout=2
            )
