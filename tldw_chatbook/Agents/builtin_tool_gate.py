...
class BuiltinToolGate:
    ...
    def resolve(self, tool: Tool, *, strict: bool = False) -> EffectiveToolState:
        ...
        """
        if strict:
            # dev: strict callers read fresh, existing authority; _strict_payload
            # raises PermissionStoreSnapshotError on unavailability (their contract).
        ...
        if self._load_failed:
            # TASK-32806.3 / Qodo #1: a non-strict resolve against an unreadable
            # store must fail CLOSED, not resolve untagged built-ins from the
            # allow floor.
