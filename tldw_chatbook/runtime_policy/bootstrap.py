...
class _CompatibilityRuntimeAPIClient:
...
def build_runtime_api_client(
    *,
    ...
    client_class = _runtime_api_client_class()
    if resolved_auth_method in {"bearer", "custom_token"}:
        client = client_class(base_url=resolved_endpoint)
        client.bearer_token = resolved_auth_token
        return client

    return client_class(base_url=resolved_endpoint, token=resolved_auth_token)


def build_runtime_api_client_from_config(app_config: Mapping[str, Any] | None) -> Any:
