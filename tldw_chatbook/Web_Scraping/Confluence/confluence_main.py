...
async def scrape_confluence_with_config(
    operation: str, target: str, config_file: Optional[str] = None, **kwargs
    ...
    if not auth.test_authentication():
        raise RuntimeError(
            "Confluence authentication failed. Please check your credentials."
        )
    ...
    if operation == "page":
        ...
        return await scrape_confluence_search(
            target, auth=auth, config=config, **kwargs
        )
    else:
...
def ingest_confluence_to_db(
    page_data: Dict[str, Any], keywords: Optional[str] = None
) -> str:
    """
    ...
    try:
        ...
        if metadata.get("labels"):
            confluence_keywords.extend(
                [f"label:{label}" for label in metadata["labels"]]
            )
