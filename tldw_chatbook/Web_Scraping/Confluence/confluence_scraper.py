...
class ConfluenceScraper(Scraper):
    ...
    async def scrape_page_by_id(self, page_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully scraped Confluence page: {result.get('title', 'Unknown')}"
            )
            return result
    ...
    async def scrape_space(
        ...
        while len(pages) < limit:
            try:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"Failed to fetch pages from space {space_key}: {response.status_code}"
                    )
                    break
    ...
    async def scrape_by_cql(
        self, cql_query: str, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
