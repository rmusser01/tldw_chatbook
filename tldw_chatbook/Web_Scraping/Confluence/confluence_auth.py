...
class ConfluenceAuth:
    ...
    def configure_api_token(self, username: str, api_token: str) -> None:
        ...
        if not username or not api_token:
            raise ValueError(
                "Username and API token are required for API token authentication"
            )
    ...
    def configure_oauth(
        self, oauth_token: str, oauth_token_secret: Optional[str] = None
    ) -> None:
        """
    ...
    def configure_cookies(self, browser: str = "all") -> None:
        ...
        if not cookies:
            raise ValueError(
                f"No cookies found for domain {domain} in browser {browser}"
            )
        ...
        self._auth_configured = True
        logger.info(
            f"Configured cookie authentication with {len(cookies)} cookies from {browser}"
        )
    ...
    def configure_from_env(self) -> None:
        ...
        if auth_method == "api_token":
            ...
            if username and api_token:
                ...
                raise ValueError(
                    "CONFLUENCE_USERNAME and CONFLUENCE_API_TOKEN environment variables required"
                )
            ...
            if username and password:
                ...
                raise ValueError(
                    "CONFLUENCE_USERNAME and CONFLUENCE_PASSWORD environment variables required"
                )
    ...
    def get_session(self) -> requests.Session:
        ...
        if not self._auth_configured:
            raise RuntimeError(
                "Authentication not configured. Call one of the configure_* methods first."
            )
    ...
    def test_authentication(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Authentication successful. Logged in as: {user_data.get('displayName', 'Unknown')}"
                )
                return True
            else:
                logger.error(
