...
class ConfluenceCrawler:
    ...
    def __init__(
        self, auth: ConfluenceAuth, scraper: Optional[ConfluenceScraper] = None
    ):
        """
    ...
    async def crawl_space(
        ...
        """
        logger.info(
            f"Starting crawl of space {space_key} (max_pages={max_pages}, max_depth={max_depth})"
        )
            ...
            try:
                ...
                if page_data["extraction_successful"]:
                    ...
                    if page_filter is None or page_filter(page_data):
                        ...
                        logger.info(
                            f"Scraped page: {page_data['title']} (depth={depth})"
                        )
        ...
        logger.info(
            f"Crawl complete. Scraped {len(scraped_pages)} pages, failed {len(self.failed_pages)}"
        )
        ...
        if include_siblings:
            ...
            if parent_id:
                ...
                for sibling in siblings:
                    if (
                        sibling["id"] != root_page_id
                        and sibling["id"] not in self.visited_pages
                    ):
    ...
    def get_crawl_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": len(self.visited_pages)
            / (len(self.visited_pages) + len(self.failed_pages))
            if (len(self.visited_pages) + len(self.failed_pages)) > 0
            else 0,
        }
...
def create_page_filter(
    ...
    def page_filter(page_data: Dict[str, Any]) -> bool:
        ...
        if include_patterns:
            if not any(
                pattern.lower() in title.lower() for pattern in include_patterns
            ):
                return False
