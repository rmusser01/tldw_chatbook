...
def get_chrome_cookies(domain_name):
    ...
    if sys.platform == "win32":
        ...
        cookie_path = os.path.join(
            appdata_path, r"Google\Chrome\User Data\Default\Cookies"
        )
        local_state_path = os.path.join(
            appdata_path, r"Google\Chrome\User Data\Local State"
        )
    ...
    duration = time.time() - start_time
    log_histogram(
        "cookie_cloner_chrome_duration",
        duration,
        labels={"status": "success", "platform": sys.platform},
    )
    log_histogram("cookie_cloner_chrome_cookies_found", len(cookies))
    log_counter(
        "cookie_cloner_chrome_success",
        labels={
...
def get_edge_cookies(domain_name):
    ...
    try:
        if sys.platform == "win32":
            ...
            appdata_path = os.getenv("LOCALAPPDATA")
            cookie_path = os.path.join(
                appdata_path, r"Microsoft\Edge\User Data\Default\Cookies"
            )
            local_state_path = os.path.join(
                appdata_path, r"Microsoft\Edge\User Data\Local State"
            )
        elif sys.platform == "darwin":
            import keyring
...
def parse_safari_cookie(data, domain_name):
    try:
        ...
        cookie_name = data[name_offset : data.find(b"\x00", name_offset)].decode(
            "utf-8"
        )
        cookie_value = data[value_offset:end_of_cookie].decode("utf-8")
        cookie_domain = data[url_offset : data.find(b"\x00", url_offset)].decode(
            "utf-8"
        )
