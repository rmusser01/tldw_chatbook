...
async def crawl_site(
    ...
    logging.info(
        f"Starting crawl of {base_url} (max_pages={max_pages}, max_depth={max_depth})"
    )
    ...
    # Log crawl start
    log_counter(
        "crawler_site_crawl_start",
        labels={"max_pages": str(max_pages), "max_depth": str(max_depth)},
    )
    ...
    async with aiohttp.ClientSession() as session:
        while to_visit and len(visited) < max_pages:
            ...
            if current_url in visited or not url_filter(current_url):
                log_counter(
                    "crawler_url_filtered", labels={"reason": "visited_or_filtered"}
                )
                continue

            if current_depth > max_depth:
                log_counter(
                    "crawler_url_filtered", labels={"reason": "max_depth_exceeded"}
                )
                continue
            ...
            try:
                ...
                page_duration = time.time() - page_start
                log_histogram(
                    "crawler_page_duration", page_duration, labels={"status": "timeout"}
                )
                log_counter(
                    "crawler_page_error",
                    labels={"error_type": "timeout", "depth": str(current_depth)},
                )
                ...
                page_duration = time.time() - page_start
                log_histogram(
                    "crawler_page_duration", page_duration, labels={"status": "error"}
                )
                log_counter(
                    "crawler_page_error",
                    labels={
    ...
    log_histogram(
        "crawler_site_duration",
        duration,
        labels={"pages_found": str(len(visited)), "pages_crawled": str(pages_crawled)},
    )
    log_counter(
        "crawler_site_complete",
        labels={
...
async def get_urls_from_sitemap(
    ...
    try:
        ...
        log_histogram(
            "crawler_sitemap_parse_duration",
            parse_duration,
            labels={"status": "success"},
        )
        ...
        duration = time.time() - start_time
        log_histogram(
            "crawler_sitemap_duration", duration, labels={"status": "http_error"}
        )
        log_counter(
            "crawler_sitemap_error",
            labels={"error_type": "http_error", "error_class": type(e).__name__},
        )
        ...
        duration = time.time() - start_time
        log_histogram(
            "crawler_sitemap_duration", duration, labels={"status": "parse_error"}
        )
        ...
        log_counter(
            "crawler_sitemap_error",
            labels={"error_type": "unexpected", "error_class": type(e).__name__},
        )
        logging.error(
    ...
    log_histogram(
        "crawler_sitemap_duration",
        total_duration,
        labels={"status": "complete", "urls_found": str(len(urls))},
    )
    log_counter(
        "crawler_sitemap_complete",
        labels={
