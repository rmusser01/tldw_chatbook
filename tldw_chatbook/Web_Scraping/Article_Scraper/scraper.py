...
class Scraper:
    ...
    async def __aenter__(self):
        ...
        log_counter(
            "article_scraper_browser_init", labels={"stealth": str(self.config.stealth)}
        )
        ...
        try:
            ...
            log_histogram(
                "article_scraper_browser_init_duration",
                duration,
            ...
            log_counter(
                "article_scraper_browser_init_error",
            ...
            try:
                ...
                if self.config.stealth:
                    ...
                    await page.wait_for_load_state(
                        "networkidle", timeout=self.config.request_timeout_ms
                    )
                ...
                log_histogram(
                    "article_scraper_fetch_duration",
                    total_duration,
                    labels={"status": "success", "attempts": str(attempt + 1)},
                )
                log_counter(
                    "article_scraper_fetch_success",
    ...
    def _extract_data(self, html: str, url: str) -> Dict[str, Any]:
        ...
        if not html:
            log_counter(
                "article_scraper_extraction_error", labels={"error_type": "empty_html"}
            )
        ...
        try:
            ...
            if not main_content_html or not metadata:
                log_counter(
                    "article_scraper_extraction_error",
                    labels={"error_type": "trafilatura_failure"},
                )
                return {
                    "extraction_successful": False,
                    "error": "Trafilatura failed to extract content or metadata.",
                }
            ...
            article_data["content_with_meta"] = (
                ContentMetadataHandler.format_content_with_metadata(
                    url=url,
                    content=main_content_md,
                    pipeline="trafilatura-playwright",
                    additional_metadata={
                        "author": article_data["author"],
                        "extracted_date": article_data["date"],
                    },
                )
            )
            ...
            log_histogram(
                "article_scraper_extraction_duration",
                duration,
                labels={"status": "success"},
            )
            log_histogram("article_scraper_content_length", content_length)
            log_counter(
                "article_scraper_extraction_success",
                labels={
    ...
    async def scrape(self, url: str) -> Dict[str, Any]:
        ...
        log_counter(
            "article_scraper_scrape_request",
            labels={"has_cookies": str(bool(self.custom_cookies))},
        )
        ...
        try:
            ...
            if result["extraction_successful"]:
                logging.info(
                    f"Successfully extracted article: '{result.get('title', 'N/A')}'"
                )
                log_histogram(
                    "article_scraper_scrape_duration",
                    duration,
                    labels={"status": "success"},
                )
                ...
                logging.warning(
                    f"Failed to extract article from {url}. Reason: {result.get('error')}"
                )
                log_histogram(
                    "article_scraper_scrape_duration",
                    duration,
            ...
            duration = time.time() - start_time
            log_histogram(
                "article_scraper_scrape_duration", duration, labels={"status": "error"}
            )
            log_counter(
                "article_scraper_scrape_error", labels={"error_type": type(e).__name__}
            )
    ...
    async def scrape_many(self, urls: List[str]) -> List[Dict[str, Any]]:
        ...
        url_count = len(urls)
        log_counter(
            "article_scraper_batch_request", labels={"url_count": str(url_count)}
        )
        ...
        log_histogram(
            "article_scraper_batch_duration",
            duration,
            labels={"url_count": str(url_count), "success_count": str(success_count)},
        )
        log_counter(
            "article_scraper_batch_complete",
            labels={
