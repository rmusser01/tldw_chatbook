...
class MeetingsScopeService:
    ...
    def _require_server_service(self, mode: MeetingsBackend) -> Any:
        if mode == MeetingsBackend.LOCAL:
            raise ValueError(
                "Server meeting records are server-only; switch to server mode to manage them."
            )
    ...
    def _normalize_response(
        cls, mode: MeetingsBackend, payload: Any, *, kind: str | None = None
    ) -> Any:
        ...
        if kind == "meeting_share":
            return cls._with_record_id(
                mode, "meeting_share", payload, payload.get("dispatch_id")
            )
        ...
        if record.get("session_id") is not None:
            record = cls._with_record_id(
                mode, "meeting_session", record, record.get("session_id")
            )
        if isinstance(record.get("artifacts"), list):
            record["artifacts"] = [
                cls._with_record_id(mode, "meeting_artifact", item)
    ...
    def list_unsupported_capabilities(
        ...
        *,
        ...
        for item in _SERVER_UNSUPPORTED_CAPABILITIES:
            if (
                item["operation_id"] == "meetings.websocket_live_ingest.server"
                and self._has_websocket_live_ingest_adapter()
            ):
                continue
    ...
    async def get_meetings_health(
        self, *, mode: MeetingsBackend | str | None = None
    ) -> dict[str, Any]:
    ...
    async def create_meeting_session(
        self, request_data: Any, *, mode: MeetingsBackend | str | None = None
    ) -> dict[str, Any]:
        return await self._call(
            mode=mode,
    ...
    async def list_meeting_sessions(
        self, *, mode: MeetingsBackend | str | None = None, **kwargs: Any
    ) -> list[dict[str, Any]]:
        return await self._call(
            mode=mode,
    ...
    async def get_meeting_session(
        self, session_id: str, *, mode: MeetingsBackend | str | None = None
    ) -> dict[str, Any]:
        return await self._call(
            mode=mode,
    ...
    async def get_meeting_template(
        self, template_id: str, *, mode: MeetingsBackend | str | None = None
    ) -> dict[str, Any]:
        return await self._call(
            mode=mode,
