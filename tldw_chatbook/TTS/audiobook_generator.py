...
class CharacterDetector:
    ...
    def assign_dialogue_voices(
        cls, content: str, character_voices: Dict[str, str], narrator_voice: str
    ) -> List[Tuple[str, str]]:
        """
...
class AudioBookGenerator:
    ...
    async def generate_audiobook(
        self, request: AudioBookRequest, progress_callback: Optional[callable] = None
    ) -> Path:
        """
        ...
        try:
            ...
            if request.multi_voice:
                ...
                for i, name in enumerate(character_names):
                    voice = request.character_voices.get(
                        name, self._get_default_character_voice(i, request.provider)
                    )
            ...
            self.progress.estimated_completion = datetime.now() + timedelta(
                seconds=self.progress.estimated_duration
            )
            ...
            for i, chapter in enumerate(chapters):
                ...
                chapter_audio = await self._generate_chapter_audio(
                    chapter, request, characters
                )

                chapter_audio_files.append(chapter_audio)
                self._chapter_audio_files.append(
                    chapter_audio
                )  # Store for M4B creation
            ...
            output_path = await self._combine_chapters(
                chapter_audio_files, request, chapters
            )
    ...
    async def _generate_chapter_audio(
        self, chapter: Chapter, request: AudioBookRequest, characters: List[Character]
    ) -> Path:
        ...
        if request.multi_voice and characters:
            ...
            segments = CharacterDetector.assign_dialogue_voices(
                chapter.content, character_voices, request.narrator_voice
            )
        else:
        ...
        for text, voice in segments:
            ...
            if request.enable_ssml and request.provider in ["elevenlabs"]:
                ...
                processed_text = self.text_processor.format_for_tts(
            ...
            internal_model_id = self._get_internal_model_id(
        ...
        combined_audio = await self._combine_segments(
            segment_audio_data, request.output_format, request.sentence_pause_duration
        )

        # Apply post-processing if enabled
        if request.normalize_audio:
            combined_audio = await self._normalize_audio(
                combined_audio, request.output_format, request.target_db
            )
    ...
    async def _combine_segments(
        self, segments: List[bytes], format: str, pause_duration: float
    ) -> bytes:
    ...
    async def _combine_chapters(
        ...
        chapter_pause = await self._generate_silence(
            request.chapter_pause_duration, request.output_format
        )
    ...
    async def _add_metadata(
        self, audio_path: Path, metadata: AudioBookMetadata
    ) -> Path:
        ...
        if file_ext == "m4b" and hasattr(self, "_chapter_audio_files"):
            ...
            chapter_titles = (
                [ch.title for ch in self._chapters]
    ...
    async def _generate_silence(self, duration: float, format: str) -> bytes:
        ...
        try:
            ...
            if format == "mp3":
                ...
                logger.error(
                    f"Cannot generate silence for format {format} without pydub"
                )
                return b""

    async def _normalize_audio(
        self, audio_data: bytes, format: str, target_db: float
    ) -> bytes:
        ...
        try:
            ...
            logger.warning(
                "Audio normalization requires pydub and numpy. Install with: pip install pydub numpy"
            )
    ...
    async def _get_audio_duration(self, audio_path: Path) -> float:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Cannot determine accurate duration. Install mutagen or pydub."
                )
