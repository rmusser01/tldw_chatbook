...
class ElevenLabsTTSBackend(APITTSBackend):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.default_voice = self.config.get(
            ...
            get_cli_setting(
                "app_tts", "ELEVENLABS_DEFAULT_VOICE", "21m00Tcm4TlvDq8ikWAM"
            ),
        )
        ...
        # Boolean conversion
        speaker_boost_str = self.config.get(
    ...
    async def initialize(self):
        ...
        if not self.api_key:
            logger.warning(
                "ElevenLabsTTSBackend: No API key available. Requests will fail."
            )
    ...
    async def generate_speech_stream(
        ...
        if voice_id == request.voice and len(voice_id) < 15:
            ...
            logger.warning(
                f"Voice '{request.voice}' not recognized, using default voice ID"
            )
        ...
        if hasattr(request, "extra_params") and request.extra_params:
            ...
            if "similarity_boost" in request.extra_params:
                voice_settings["similarity_boost"] = float(
                    request.extra_params["similarity_boost"]
                )
            ...
            if "use_speaker_boost" in request.extra_params:
                voice_settings["use_speaker_boost"] = bool(
                    request.extra_params["use_speaker_boost"]
                )
        ...
        logger.info(
            f"ElevenLabsTTSBackend: Requesting TTS for {len(request.input)} characters"
        )
        logger.debug(
            f"ElevenLabsTTSBackend: Request params: model={model_id}, voice={voice_id}, "
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"ElevenLabs API HTTP error {e.response.status_code}: {error_msg}"
                )
                ...
                try:
                    ...
                    if "detail" in error_data:
                        if (
                            isinstance(error_data["detail"], dict)
                            and "message" in error_data["detail"]
                        ):
            ...
            logger.opt(exception=True).error(
                f"ElevenLabsTTSBackend: Unexpected error: {e}"
            )
