            ...
            send_response(
                {
                    ...
                    "format": format
                    if i == 0
                    else None,  # Only send metadata with first chunk
                    "sample_rate": sample_rate if i == 0 else None,
                }
            )

        # Send completion message
        send_response({"type": "audio_complete", "total_chunks": total_chunks})
        ...
        try:
            ...
            if command == "initialize":
                ...
                if device == "cuda" and not torch.cuda.is_available():
                    device = "cpu"
                    send_response(
                        {"type": "warning", "message": "CUDA not available, using CPU"}
                    )
                ...
                try:
                    send_response(
                        {
                            "type": "status",
                            "message": f"Loading model on device: {device}",
                        }
                    )
                    ...
                    send_response(
                        {"type": "success", "message": "Model loaded successfully"}
                    )
                except Exception as e:
                    import traceback
                ...
                try:
                    ...
                    send_chunked_audio(
                        audio_bytes, format="wav", sample_rate=sample_rate
                    )
