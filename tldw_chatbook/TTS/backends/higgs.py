...
try:
    ...
    librosa = None
    logger.warning(
        "librosa not available. Advanced audio analysis features will be limited."
    )
...
try:
    ...
    sf = None
    logger.warning(
        "soundfile not available. Some audio format support will be limited."
    )
...
def protect_file_descriptors():
    ...
    try:
        ...
        # back (the previous ordering) compared the just-restored value
        # against itself, always False, so this cleanup silently never ran
        # (task-634 round 3). closefd=False above already makes that
        # harmless for fd 1/2 specifically, but this ordering is still
        # correct hygiene for the devnull fallback (which this process DOES
        # fully own and should close).
...
class HiggsAudioTTSBackend(LocalTTSBackend):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not NUMPY_AVAILABLE:
            raise ImportError(
                "numpy is required for Higgs Audio backend but is not installed. "
                "Install it with: pip install numpy"
            )

        if not TORCH_AVAILABLE:
            raise ImportError(
                "PyTorch is required for Higgs Audio backend but is not installed. "
        ...
        self.audio_tokenizer_path = self.config.get(
        ...
        self.device = self.config.get(
            "HIGGS_DEVICE", "cuda" if torch.cuda.is_available() else "cpu"
        )
        ...
        self.enable_background_music = self.config.get(
        ...
        logger.info(
            f"Loaded {len(self.voice_profiles)} voice profiles from {self.voice_profiles_file}"
        )

        # Services
        self.audio_service = get_audio_service()
        self.text_chunker = TextChunker(
            max_tokens=self.config.get("HIGGS_MAX_TOKENS", 500)
        )
        ...
        logger.info(
            f"HiggsAudioTTSBackend initialized with device: {self.device}, model: {self.model_path}"
        )
    ...
    async def load_model(self):
        ...
        try:
            ...
            init_params = inspect.signature(
                self._higgs_serve_engine.__init__
            ).parameters
            ...
            if "audio_tokenizer_name_or_path" in init_params:
                engine_kwargs["audio_tokenizer_name_or_path"] = (
                    self.audio_tokenizer_path
                )
                logger.debug(
                    "HiggsAudioServeEngine supports audio_tokenizer_name_or_path parameter"
                )
            else:
                logger.debug(
            ...
            if "dtype" in init_params:
                ...
                logger.debug(
                    "HiggsAudioServeEngine does not support dtype parameter, skipping"
                )
            ...
            if "enable_flash_attn" in init_params:
                ...
                logger.debug(
                    "HiggsAudioServeEngine supports enable_flash_attn parameter"
                )
            else:
                logger.debug(
                    "HiggsAudioServeEngine does not support enable_flash_attn parameter, skipping"
                )
            ...
            logger.info(
                "🔄 Step 3/4: Loading model (this may take a while on first run)..."
            )
            logger.info(f"📥 Model path: {engine_kwargs.get('model_name_or_path')}")
            logger.info(
                f"🎵 Audio tokenizer: {engine_kwargs.get('audio_tokenizer_name_or_path', 'default')}"
            )
    ...
    async def _generate_single_speaker_stream(
        ...
        try:
            ...
            await self._report_progress(
                ...
                metrics={
                    "voice": voice_config.get("display_name", "custom"),
                    "language": language,
                },
            )
            ...
            logger.opt(exception=True).error(
                f"HiggsAudioTTSBackend: Single speaker generation failed: {e}"
            )
            raise
    ...
    async def _generate_multi_speaker_stream(
        ...
        try:
            ...
            logger.info(
                f"HiggsAudioTTSBackend: Generating multi-speaker dialog with {len(sections)} sections"
            )
            ...
            for i, (speaker, section_text) in enumerate(sections):
                ...
                logger.debug(
                    f"Section {i + 1}: Speaker='{speaker}', Voice={speaker_voice_config.get('display_name', 'custom')}"
                )

                # Generate audio for this section
                messages = self._prepare_messages(
                    section_text, speaker_voice_config, language
                )
                chat_ml_sample = self._make_chat_ml_sample(messages)
            ...
            logger.opt(exception=True).error(
                f"HiggsAudioTTSBackend: Multi-speaker generation failed: {e}"
            )
            raise
    ...
    def _parse_multi_speaker_text(self, text: str) -> List[Tuple[str, str]]:
        ...
        Format: "Speaker1|||text1 Speaker2|||text2 Speaker3|||text3"
        Expected: [("Speaker1", "text1"), ("Speaker2", "text2"), ("Speaker3", "text3")]

        Returns: List of (speaker_name, text) tuples
        """
        ...
        sections = []
        # Split by delimiter
        # "narrator|||Welcome. Alice|||Hello!" -> ['narrator', 'Welcome. Alice', 'Hello!']
        parts = text.split(self.speaker_delimiter)

        # Track current speaker for proper pairing
        ...
        for i, part in enumerate(parts):
            ...
            if i == 0:
                ...
                if part:
                    if len(parts) > 1:
                        # Check if this part ends with a speaker name
                        # Format could be "text speaker|||" or just "speaker|||"
                        last_space = part.rfind(" ")
                        if last_space > 0:
                            # Has space, might be "text speaker" format
                            text_part = part[:last_space].strip()
                            potential_speaker = part[last_space + 1 :].strip()

                            # Check if this looks like narrator text followed by speaker
                            if text_part and (
                                "." in text_part
                                or "!" in text_part
                                or "?" in text_part
                                ...
                                # Text part looks like narrator text
                                sections.append(("narrator", text_part))
                                current_speaker = potential_speaker
                            else:
                                # Whole thing might be a speaker name or short text
                                current_speaker = part
                        else:
                            # No space, check if it's a simple speaker name
                            if len(part) < 30 and part.isalnum():
                                # Simple word/name, likely a speaker
                                current_speaker = part
                            else:
                                # Treat as narrator text
                                sections.append(("narrator", part))
                                current_speaker = None
                    else:
                        # Only one part total, treat as narrator text
                        sections.append(("narrator", part))
                else:
                    # Empty first part (text starts with |||)
                    current_speaker = "narrator"
            else:
                # Parts after delimiter
                if not part:
                    # Empty part after ||| - like "|||text"
                    if current_speaker:
                        sections.append((current_speaker, ""))
                    continue

                # If we don't have a current speaker, extract it from the beginning of this part
                if current_speaker is None:
                    # This part should start with "speaker text" or just be a speaker name
                    first_space = part.find(" ")
                    if first_space > 0:
                        # Has space, split into speaker and text
                        current_speaker = part[:first_space].strip()
                        remaining_text = part[first_space + 1 :].strip()

                        # Check if remaining text contains next speaker at the end
                        if i < len(parts) - 1 and " " in remaining_text:
                            last_space = remaining_text.rfind(" ")
                            text_part = remaining_text[:last_space].strip()
                            next_speaker = remaining_text[last_space + 1 :].strip()
                            sections.append((current_speaker, text_part))
                            current_speaker = next_speaker
                        else:
                            sections.append((current_speaker, remaining_text))
                    else:
                        # No space, entire part is speaker name
                        current_speaker = part
                        # Don't add section yet, wait for text
                    continue

                # Check if this part contains the next speaker at the end
                # This handles "text1 Speaker2" pattern
                if i < len(parts) - 1:
                    # Not the last part, might have next speaker embedded
                    # Look for last space-separated word as potential next speaker
                    last_space = part.rfind(" ")
                    if last_space > 0:
                        # Has space, split into text and potential next speaker
                        text_part = part[:last_space].strip()
                        potential_speaker = part[last_space + 1 :].strip()

                        # Add current speaker with their text
                        if current_speaker:
                            sections.append((current_speaker, text_part))
                        ...
                        # No space, entire part is text for current speaker
                        if current_speaker:
                            sections.append((current_speaker, part))
                else:
                    # Last part - just text for current speaker
                    if current_speaker:
                        sections.append((current_speaker, part))
    ...
    def _looks_like_speaker_name(self, text: str) -> bool:
        ...
        # Speaker names are typically:
        # - Single words or compound names with underscores/dashes
        # - Not too long
        # - Don't contain spaces (unless it's a voice profile name)
        # - May contain underscores or dashes

        # Check if it's a known voice profile
        if text in self.voice_profiles:
            return True

        # Basic validation for speaker names
        import re

        # Allow alphanumeric, underscore, dash
        if re.match(r"^[\w-]+$", text) and len(text) < 50:
            return True

        # Also allow names with single spaces for compound names like "professional_female"
        if re.match(r"^[\w]+(?:[\s_-][\w]+)*$", text) and len(text) < 50:
            return True
    ...
    def _invoke_serve_engine_generate(
        self, chat_ml_sample: Any, **generation_kwargs
    ) -> Any:
    ...
    async def _prepare_voice_config(self, voice_name: str) -> Dict[str, Any]:
        ...
        if self.enable_voice_cloning and (
            voice_name.endswith((".wav", ".mp3", ".flac", ".ogg"))
            or os.path.exists(voice_name)
        ):
    ...
    def _prepare_messages(
        self, text: str, voice_config: Dict[str, Any], language: str
    ) -> List[Any]:
        ...
        if voice_config.get("reference_audio"):
            ...
            if os.path.exists(ref_audio_path):
                ...
                try:
                    ...
                    messages.append(
                        self._make_message(role="user", content=reference_text)
                    )
                    messages.append(
                        self._make_message(
                            role="assistant",
                            content=self._make_audio_content(
                                raw_audio=audio_base64, audio_url="placeholder"
                            ),
                        )
                    )
                    logger.info(
                        f"Added reference audio for voice cloning: {ref_audio_path}"
                    )
                except Exception as e:
            ...
            messages.append(self._make_message(role="system", content=system_content))

        # Add the text to generate
        messages.append(self._make_message(role="user", content=text))
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_generations"] > 0:
            ...
            stats["average_tokens_per_generation"] = (
                stats["total_tokens"] / stats["total_generations"]
            )
    ...
    async def close(self):
        ...
        if self.track_performance and self._performance_metrics["generation_count"] > 0:
            ...
            logger.info(
                f"HiggsAudioTTSBackend: Final stats - "
