...
class ChatterboxVoiceManager(VoiceManagerBase):
    ...
    def create_profile(
        ...
        try:
            ...
            if not is_valid:
                return (
                    False,
                    f"Invalid audio file: {audio_info.get('error', 'Unknown error')}",
                )
            ...
            if metadata and "duration" in metadata:
                ...
                if duration < 7:
                    logger.warning(
                        f"Audio duration ({duration}s) is less than recommended 7 seconds for Chatterbox"
                    )
                elif duration > 20:
                    logger.warning(
                        f"Audio duration ({duration}s) exceeds recommended 20 seconds for Chatterbox"
                    )
    ...
    def export_profile(self, profile_name: str, export_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(
                f"Exported Chatterbox profile '{profile_name}' to {package_dir}"
            )
    ...
    def import_profile(
        ...
        try:
            ...
            if (
                import_profile.get("backend") != "chatterbox"
                and "chatterbox_settings" not in import_profile
            ):
            ...
            if not profile_name:
                ...
                if package_dir.name.startswith("chatterbox_voice_"):
                    profile_name = package_dir.name[
                        17:
                    ]  # Remove "chatterbox_voice_" prefix
                else:
                    profile_name = (
                        f"imported_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    )
