...
class AllTalkTTSBackend(APITTSBackend):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.base_url = self.config.get(
            "ALLTALK_TTS_URL",
            get_cli_setting(
                "app_tts", "ALLTALK_TTS_URL_DEFAULT", "http://127.0.0.1:7851"
            ),
        )

        self.default_voice = self.config.get(
            "ALLTALK_TTS_VOICE",
            get_cli_setting("app_tts", "ALLTALK_TTS_VOICE_DEFAULT", "female_01.wav"),
        )
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if isinstance(voices_data, list):
                            logger.info(
                                f"AllTalk available voices: {', '.join(voices_data)}"
                            )
                        else:
                    ...
                    logger.warning(
                        f"AllTalk voices endpoint returned status {response.status_code}"
                    )
        except httpx.ConnectError:
            logger.warning(
                f"Could not connect to AllTalk server at {self.base_url}. "
    ...
    async def generate_speech_stream(
        ...
        logger.info(
            f"AllTalkTTSBackend: Generating TTS for {len(request.input)} characters "
        ...
        try:
            ...
            logger.info(
                f"AllTalkTTSBackend: Generation complete, received {bytes_received / 1024:.1f}KB"
            )

        except httpx.ConnectError:
            logger.error(
                f"AllTalk connection error: Could not connect to {self.base_url}"
            )
            raise ValueError(
            ...
            try:
                if hasattr(e.response, "text"):
                    ...
                    logger.error(
                        f"AllTalk API error {e.response.status_code}: {error_msg}"
                    )
            except Exception:
                pass
        ...
        try:
            ...
            async with self.client.get(voices_url, timeout=5.0) as response:
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Could not fetch voices, status: {response.status_code}"
                    )
                    return [self.default_voice]
        except Exception as e:
