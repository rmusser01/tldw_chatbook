...
class TTSService:
    ...
    async def generate_audio_stream(
        ...
        """
        backend: Optional[TTSBackendBase] = await self.backend_manager.get_backend(
            internal_model_id
        )
        if not backend:
            logger.error(
                f"TTSService: No backend found for internal_model_id: {internal_model_id}"
            )
            # Don't expose internal model IDs to users
            raise ValueError(
                f"TTS model '{request.model}' is not available. Please choose a different model."
            )

        logger.info(
            f"TTSService: Using backend {type(backend).__name__} for model '{request.model}' (internal: {internal_model_id})"
        )
        try:
            ...
            logger.error(
                f"TTSService: Error streaming from backend {type(backend).__name__}: {e}",
                exc_info=True,
            )
...
async def get_tts_service(app_config: Optional[Dict[str, Any]] = None) -> TTSService:
    ...
    try:
        ...
        _tts_service_instance = TTSService(
            backend_manager=_tts_backend_manager_instance
        )
