...
try:
    ...
    np = None
    logger.warning("numpy not available - some Kokoro features may be limited")
...
VOICE_PATTERN = re.compile(
    r"([a-z]+_[a-z]+)(?:\((\d+)\))?"
)  # Matches voice names like "af_bella(2)"
...
def mix_voices(
    voice_tensors: List[Tuple[torch.Tensor, float]], normalize: bool = True
) -> torch.Tensor:
    """
    ...
    **kwargs,
    ...
    with torch.cuda.amp.autocast(enabled=(model.device != "cpu")):
        ...
        audio_features = _generate_audio_features(
            model.model, token_tensor, voice_tensor, speed=speed
        )
...
def validate_voice_mix(
    voice_string: str, available_voices: Optional[List[str]] = None
) -> bool:
