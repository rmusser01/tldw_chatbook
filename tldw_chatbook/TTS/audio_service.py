...
try:
    ...
    logger.warning(
        "soundfile not available. Some audio format conversions will be limited."
    )
...
try:
    ...
    logger.warning(
        "pydub not available. Some audio format conversions will be limited."
    )
...
class AudioService:
    ...
    def _numpy_to_bytes(self, audio_array: "np.ndarray") -> bytes:
        ...
        if not NUMPY_AVAILABLE:
            raise RuntimeError(
                "numpy is required for array conversion but is not installed"
            )
    ...
    async def _convert_with_pydub(
        ...
        try:
            ...
            target_sample_rate = sample_rate or self.SAMPLE_RATES.get(
                target_format, 44100
            )
    ...
    async def _convert_with_soundfile(
        ...
        try:
            ...
            if source_format == "pcm":
                if not NUMPY_AVAILABLE:
                    raise RuntimeError(
                        "numpy is required for PCM conversion but is not installed"
                    )
            ...
            output_buffer = io.BytesIO()
            sf.write(
                output_buffer, audio_float, target_sr, format=target_format.upper()
            )
            return output_buffer.getvalue()
    ...
    async def validate_audio(self, audio_data: bytes, expected_format: str) -> bool:
        ...
        try:
            if expected_format == "pcm":
                ...
                logger.warning(
                    f"Cannot validate {expected_format} format without pydub"
                )
