...
class ModelDownloader:
    ...
    async def download_file(
        ...
        if destination.exists() and expected_size:
            ...
            if actual_size == expected_size:
                ...
                logger.warning(
                    f"File {destination} exists but size mismatch "
            ...
            try:
                ...
                async with client.stream("GET", url) as response:
                    ...
                    if expected_size and bytes_downloaded != expected_size:
                        logger.error(
                            f"Downloaded size mismatch: {bytes_downloaded} != {expected_size}"
                        )
    ...
    async def download_kokoro_model(
        self, version: str = "v0.19", progress_callback: Optional[callable] = None
    ...
    def get_model_paths(
        self, model_name: str = "kokoro", version: str = "v0.19"
    ) -> Dict[str, Path]:
        """
...
def get_kokoro_model_paths(
    version: str = "v0.19", base_dir: Optional[Path] = None
) -> Dict[str, Path]:
    """
    ...
    async def main():
        def progress(downloaded, total):
            if total > 0:
                ...
                print(
                    f"Progress: {downloaded:,} / {total:,} bytes ({percent:.1f}%)",
                    end="\r",
                )

        print("Downloading Kokoro model files...")
        model_path, voices_path = await download_kokoro_model(
