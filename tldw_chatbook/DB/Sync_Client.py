...
class ClientSyncEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"  Initial State: Last Sent={self.last_local_log_id_sent}, Last Processed={self.last_server_log_id_processed}"
        )
        ...
        try:
            if os.path.exists(self.state_file):
                with open(self.state_file, "r") as f:
                    ...
                    self.last_server_log_id_processed = state.get(
                        "last_server_log_id_processed", 0
                    )
                    logger.debug(f"Loaded sync state from {self.state_file}")
            else:
                logger.info(
                    f"State file {self.state_file} not found, starting from scratch."
                )
                # Ensure the initial save happens if the file doesn't exist
                self._save_sync_state()
        except (json.JSONDecodeError, IOError, KeyError) as e:
            logger.opt(exception=True).error(
                f"Error loading sync state from {self.state_file}: {e}. Starting from scratch."
            )
    ...
    def _save_sync_state(self):
        ...
        try:
            ...
            logger.opt(exception=True).error(
                f"Error saving sync state to {self.state_file}: {e}"
            )
    ...
    def run_sync_cycle(self):
        ...
        if not network_error:
            try:
                ...
                logger.opt(exception=True).error(
                    f"Unexpected error during pull/apply phase: {e}"
                )
        else:
            logger.warning(
                "Skipping pull phase due to earlier network error during push."
            )

        logger.info(
            f"Sync cycle finished. Current state: Last Sent={self.last_local_log_id_sent}, Last Processed={self.last_server_log_id_processed}"
        )

    def _push_local_changes(self):
        """Fetches local changes from sync_log and sends them to the server."""
        logger.debug(
            f"Checking for local changes since log ID {self.last_local_log_id_sent}"
        )
        try:
            local_changes = self.db.get_sync_log_entries(
                since_change_id=self.last_local_log_id_sent, limit=SYNC_BATCH_SIZE
            )
        except DatabaseError as e:
            logger.opt(exception=True).error(
                f"Failed to get local sync log entries: {e}"
            )
        ...
        try:
            ...
            response = requests.post(
                full_url, json=payload, headers=headers, timeout=45
            )
            ...
            self._save_sync_state()
            logger.info(
                f"Successfully pushed {len(local_changes)} changes. Last sent ID updated to {new_last_sent}."
            )
    ...
    def _pull_and_apply_remote_changes(self):
        ...
        logger.info(
            f"Pulling remote changes since server log ID {self.last_server_log_id_processed}."
        )
        ...
        try:
            ...
            response = requests.get(
                full_url, params=params, headers=headers, timeout=45
            )
            ...
            server_latest_id = sync_data.get(
                "latest_change_id", self.last_server_log_id_processed
            )
            ...
            if not remote_changes:
                ...
                if server_latest_id > self.last_server_log_id_processed:
                    logger.info(
                        f"Updating processed ID to server's latest known ID: {server_latest_id}"
                    )
            ...
            if processed_successfully:
                ...
                self._save_sync_state()
                logger.info(
                    f"Successfully processed remote changes. Last processed ID updated to {new_last_processed}."
                )
            else:
                logger.error(
                    "Failed to apply one or more remote changes in the batch. Last processed ID NOT updated."
                )
                # Consider: Implement partial batch success handling? (More complex)

        except requests.exceptions.HTTPError as e:
            logger.error(
                f"HTTP error pulling changes: {e.response.status_code} - {e.response.text}"
            )
    ...
    def _apply_remote_changes_batch(self, changes: List[Dict]) -> bool:
        ...
        try:
            with self.db.transaction() as conn:  # Use the library's transaction context
                cursor = (
                    conn.cursor()
                )  # Get cursor from the transaction-managed connection
                for change in changes:
                    try:
                        ...
                        ConflictError
                    ) as cf_err:  # Catch conflicts raised by _apply_single_change
                        logger.warning(
                            f"Conflict occurred applying change ID {change['change_id']}: {cf_err}. Attempting resolution."
                        )
                        resolved = self._resolve_conflict(cursor, change, cf_err)
                        if not resolved:
                            logger.error(
                                f"Conflict resolution failed for change ID {change['change_id']}. Rolling back batch."
                            )
                        ...
                        logger.opt(exception=True).error(
                            f"Failed to apply change ID {change['change_id']} ({change['entity']} {change['operation']}): {item_error}"
                        )
                        all_applied_or_skipped = False
            ...
            logger.error(
                f"Transaction rolled back while applying remote changes batch: {e}"
            )
            all_applied_or_skipped = False
        except Exception as e:
            logger.opt(exception=True).error(
                f"Unexpected error during remote changes batch application: {e}"
            )
            all_applied_or_skipped = False
    ...
    def _apply_single_change(self, cursor: sqlite3.Cursor, change: Dict):
        ...
        if not payload_str:
            ...
            if entity == "MediaKeywords" and operation in ["link", "unlink"]:
                ...
                logger.error(
                    f"Missing payload for change ID {change['change_id']} ({entity} {operation})"
                )
            ...
            if remote_version <= local_version:
                ...
                if remote_version < local_version or (
                    ...
                    and remote_client_id == local_client_id
                ):
                    logger.debug(
    ...
    def _resolve_conflict(
        self, cursor: sqlite3.Cursor, change: Dict, conflict_error: ConflictError
    ) -> bool:
        """
        ...
        try:
            cursor.execute(
                f"SELECT last_modified FROM {entity} WHERE uuid = ?", (entity_uuid,)
            )
            ...
            if isinstance(local_timestamp, datetime):
                local_timestamp = (
                    local_timestamp.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
                )
            if isinstance(authoritative_timestamp, datetime):
                authoritative_timestamp = (
                    authoritative_timestamp.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
                )

            # Compare server's authoritative timestamp with the local record's timestamp
            if authoritative_timestamp >= local_timestamp:
                logger.warning(
                    f"  Resolving Conflict (LWW): Remote change wins (RemoteTS: {authoritative_timestamp} >= LocalTS: {local_timestamp}). Forcing apply."
                )
                ...
                logger.warning(
                    f"  Resolving Conflict (LWW): Local change wins (RemoteTS: {authoritative_timestamp} < LocalTS: {local_timestamp}). Skipping remote change."
                )
            ...
            logger.opt(exception=True).error(
                f"Error during LWW conflict resolution for {entity} {entity_uuid}: {e}"
            )
            return False  # Resolution failed

    def _execute_change_sql(
        self,
        ...
        if not table_columns:
            ...
            raise DatabaseError(
                f"Cannot proceed: Failed to get columns for entity '{entity}'."
            )
        ...
        if operation in ["update", "delete"] or force_apply:
            try:
                ...
                cursor.execute(
                    f"SELECT version FROM `{entity}` WHERE uuid = ?", (uuid,)
                )
                ...
                logger.opt(exception=True).error(
                    f"Error fetching current version for {entity} {uuid}: {e}"
                )
                # Depending on strategy, either raise or default to 0 and proceed carefully
                raise DatabaseError(
                    f"Could not fetch current version for {entity} {uuid}"
                ) from e

        # Determine the target version to SET in the SQL statement
        target_sql_version = (
            remote_version  # Default: use remote version if not forcing
        )
        if force_apply:
            ...
            logger.debug(
                f"Forcing apply: Setting target SQL version to current_db_version + 1 = {target_sql_version}"
            )
        ...
        if operation == "create":
            ...
            if not set_clauses:
                ...
                main_sql = (
                    f"UPDATE `{entity}` SET {', '.join(set_clauses)}{where_clause}"
                )
            ...
            main_params_tuple = tuple(
                [timestamp, target_sql_version, client_id] + where_params
            )
            execute_main_sql = True

        else:  # Invalid Operation
            raise ValueError(
                f"Unsupported operation '{operation}' received for entity '{entity}'"
            )
        ...
        if execute_main_sql and main_sql:
            try:
                ...
                if (
                    fts_update_sql and entity == "Keywords"
                ):  # Keyword FTS params are known
                    logger.debug(
                ...
                if fts_delete_sql:
                    logger.debug(
                        f"Executing FTS Delete SQL FIRST: {fts_delete_sql} | Params: {fts_delete_params}"
                    )
                ...
                logger.debug(
                    f"Executing Main SQL: {main_sql} | Params: {main_params_tuple}"
                )
                ...
                if (
                    ...
                    raise ConflictError(
                        "Optimistic lock failed applying change.",
                        entity=entity,
                        identifier=uuid,
                    )
                ...
                logger.opt(exception=True).error(
                    f"Integrity error applying change for {entity} {uuid}: {ie}"
                )
                ...
                logger.opt(exception=True).error(
                    f"SQLite error applying change or FTS for {entity} {uuid}: {e}"
                )
                raise  # Re-raise original SQLite error

        elif not main_sql:
            logger.debug(
                f"No Main SQL generated or needed execution for {entity} {uuid} (Op: {operation})"
            )

    def _execute_media_keyword_sql(
        self, cursor: sqlite3.Cursor, operation: str, payload: Dict
    ):
        ...
        if not media_uuid or not keyword_uuid:
            raise ValueError(
                f"Missing media_uuid or keyword_uuid for MediaKeywords operation '{operation}'"
            )
        ...
        if operation == "link":
            ...
            raise ValueError(
                f"Unsupported operation '{operation}' for MediaKeywords entity"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            db.add_keyword(
                "test_sync_keyword"
            )  # 'db' is guaranteed to be Database object here if no exception occurred above
        ...
        logger.opt(exception=True).error(
            f"Critical error during client sync setup or execution: {main_err}"
        )
        ...
        if db:
            ...
            logger.warning(
                "DB object was not successfully initialized, no connection to close."
            )
