-- Migration: ChaChaNotes V17 to V18 per-conversation system prompt
-- Adds a nullable system_prompt column for the native Console per-session
-- system prompt feature, and redefines the conversations_sync_* triggers so
-- edits to the new column are reflected in sync_log.
