...
class MindmapDatabase(BaseDB):
    ...
    def save_mindmap(
        ...
        with self.transaction() as cursor:
            ...
            cursor.execute(
                "DELETE FROM mindmap_nodes WHERE mindmap_id = ?", (mindmap_id,)
            )
            ...
            logger.info(
                f"Saved mindmap {mindmap_id} with {self._count_nodes(root_node)} nodes"
            )

    def _save_node_recursive(
        self, cursor, mindmap_id: str, node: Node, parent_id: Optional[str], index: int
    ):
    ...
    def restore_version(self, mindmap_id: str, version_number: int) -> None:
        ...
        with self.transaction() as cursor:
            ...
            if not result:
                raise ValueError(
                    f"Version {version_number} not found for mindmap {mindmap_id}"
                )
    ...
    def add_collaborator(
        self, mindmap_id: str, user_id: str, permission: str = "view"
    ) -> None:
        ...
        with self.transaction() as cursor:
            ...
            logger.info(
                f"Added collaborator {user_id} to mindmap {mindmap_id} with {permission} permission"
            )
    ...
    def export_mindmap_json(self, mindmap_id: str) -> Dict[str, Any]:
        ...
        return {"metadata": metadata, "root": node_to_dict(root)}
