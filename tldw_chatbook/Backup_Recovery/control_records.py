...
def _replacement_activation_names(
    ...
    from .restore_plan import _retained_config_items, recheck_retained_configs

    configs.update(
        {key: item.path for key, item in _retained_config_items(plan, doc).items()}
    )
...
def _bind_activation(
    ...
    from .storage_admission import MaintenanceSession, _contains_capture_path
    ...
    physical = effective_roots(
        roots, (registry[name] for name in names)
    )
    if not _contains_capture_path(physical, selected) or any(
        not _contains_capture_path(session._roots, path) for path in physical
    ):
