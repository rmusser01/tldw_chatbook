...
def _capture_names(authority, inventory, *, include_absent_sqlite=False):
    ...
    if include_absent_sqlite:
        from .preserved_groups import _absence_names
        from .restore_plan import _absent_sqlite_paths

        owners = {owner.owner_id: owner for owner in install_adapters()}
        for item in inventory.items:
            if paths := _absent_sqlite_paths(item, owners.get(item.owner)):
                # Absence needs exclusion of existing writers, not enrollment
                # of a nonexistent source or a broader parent directory.
                names.update(_absence_names(registry, paths))
    from .effective_roots import check_redundant_profiles

    _, profiles = bootstrap._records(authority.control_root.parent)
    omitted = check_redundant_profiles(registry, profiles)
    for root in sorted(roots, key=lambda path: (len(path.parts), str(path))):
        covered = False
        narrower = False
        for name, entry in registry.items():
            for raw in entry["roots"]:
                if Path(raw) in omitted:
                    continue
...
def capture(
    ...
    *,
    ...
    maintenance_inventory = inventory
    if selections.data_groups is not None:
        # Maintained discovery still inspects all installed owners to detect
        # aliases and dependencies; selection narrows captured payloads, not the
        # native read/exclusion scope needed for that discovery.
        with _preview_reads(limits=limits, byte_budget=budget):
            maintenance_inventory = discover(
                selectors, selections=replace(selections, data_groups=None)
            )
    names = _capture_names(authority, maintenance_inventory)
    _check(cancel)
