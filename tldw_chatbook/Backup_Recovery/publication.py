...
def _plan_digest(plan):
    ...
    if plan.retained_configs:
        value["retained_configs"] = [row.record() for row in plan.retained_configs]
    if plan.requested_groups is not None:
        value["requested_groups"] = plan.requested_groups
    if plan.effective_groups:
        value["effective_groups"] = plan.effective_groups
    if plan.required_groups:
        value["required_groups"] = plan.required_groups
...
def _config_publication_scopes(
    ...
    *,
    ...
    config_paths.update(item.path for item in retained.values())
    existing = {
        path
        for row in profiles
    ...
    for key, item in files.items():
        ...
        if (
            ...
            and config.logical_id not in retained
        ):
            raise ValueError("publication_config_relation_unverified")
        selector = selected.get(config.logical_id) or retained[config.logical_id].path
        path = selected[key]
...
def _activation_proof(journal, context, generation, selectors, owners):
    ...
    # Recheck exhaustive current ownership after paired writes and before this
    # operation can clear its fence; a newly foreign profile is not ignored.
    check_redundant_profiles(registry, profiles)
...
def finalize_candidate(candidate: Path, plan: RestorePlan, journal, *, session) -> str:
    ...
    with journal._locked(exclusive=True) as parent:
        ...
        selectors = sorted(
            ...
            | {str(row.config_path) for row in plan.retained_configs}
        )
        ...
        def check_installed():
            recheck_retained_configs(plan)
            _check_preserved_settings(plan, session, candidate, context.descriptor)
            for item in prepared.artifacts:
                if item.action == "retire":
                    _check_parents(item)
                    if (
                        _states(prepared, logical_id=item.logical_id)[item.logical_id]
                        != "retired"
                    ):
...
def _preserved_settings_digest(descriptor):
    """Require staging's finite preservation proof before accepting Settings."""
    expected = descriptor.get("preserved_settings_fingerprint")
    if (
        type(expected) is not str or len(expected) != 64
        or any(char not in "0123456789abcdef" for char in expected)
    ):
        raise ValueError("preserved_settings_proof_required")
    return expected
...
def _descriptor(candidate, plan):
    ...
    if document.get("local_snapshot") != (
        ...
        raise ValueError("candidate_plan_mismatch")
    if document.get("retained_configs", []) != [
        row.record() for row in plan.retained_configs
    ]:
        raise ValueError("candidate_plan_mismatch")
    if plan.mode == "replace" and "settings" in plan.effective_groups:
        _preserved_settings_digest(document)
...
def _prepare(
    ...
    *,
    ...
    with journal._locked(exclusive=True) as parent:
        ...
        restored_selectors.update(
            str(item.path) for item in _retained_config_items(plan, doc).values()
        )
...
def _check_preserved_settings(plan, session, candidate, descriptor_record):
    """Recheck authenticated staging proof while ordinary startup stays fenced."""
    ...
    if descriptor_record.path != str(path) or not _matches(descriptor_record, str(path)):
        raise ValueError("candidate_receipt_changed")
    with pinned_directory(candidate) as parent:
        descriptor = _read(parent, path.name, max_bytes=RECOVERY_RECORD_BYTES)
    expected = _preserved_settings_digest(descriptor)
    with session._discovery_reads():
        if preserved_settings_fingerprint(plan) != expected:
            raise ValueError("preserved_group_state_changed")
    if not _matches(descriptor_record, str(path)):
        raise ValueError("candidate_receipt_changed")
...
def _validate_installed(journal, candidate, plan, *, session=None):
    ...
    with journal._locked(exclusive=True) as parent:
        ...
        try:
            ...
            from .staging import _retained_config_validation
            ...
            topology.update(
                {
                    ...
                    for key, item in retained_items.items()
                }
            )
            verify()
            ...
            verify()
            _check_preserved_settings(plan, session, candidate, receipt.descriptor)
            verify()
            for row in sorted(
                rows,
                key=lambda row: (
