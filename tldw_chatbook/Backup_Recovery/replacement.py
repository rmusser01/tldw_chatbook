...
def _absent_sqlite_dependency_scope(inventory, missing):
    """Accept unavailable edges only between independently proved absent stores.

    This checks recorded graph labels only. Callers must separately prove every
    missing row is an absent installed SQLite store, live or from its receipt.
    """
    from .inventory import BLOCKING

    by_id = {item.logical_id: item for item in inventory.items}
    if len(by_id) != len(inventory.items):
        return False
    absent_ids = {item.logical_id for item in missing}
    expected = {"missing_required"}
    for item in inventory.items:
        if item.status in {"unused", "intentionally_excluded"}:
            continue
        for key in item.dependencies:
            dependency = by_id.get(key)
            if (
                dependency is None
                or dependency.status in BLOCKING | {"unused", "intentionally_excluded"}
                or dependency.status == "intentionally_deleted"
                and not dependency.deletion_validated
            ):
                if item.logical_id not in absent_ids or key not in absent_ids:
                    return False
                expected.add("dependency_unavailable")
    return set(inventory.issues) == expected


def _first_binding_scope_available(inventory):
    """Permit absent installed SQLite primaries, never an uncertain footprint."""
    from .inventory import BLOCKING
    from .restore_plan import _absent_sqlite_paths

    if inventory.complete:
        return True
    missing = [item for item in inventory.items if item.status in BLOCKING]
    if not _absent_sqlite_dependency_scope(inventory, missing):
        return False
    owners = {owner.owner_id: owner for owner in install_adapters()}
    if not missing:
        return False
    for item in missing:
        if (
            ...
            or _absent_sqlite_paths(item, owners.get(item.owner)) is None
        ):
            return False
    return True
...
def _first_user_data_root(selector, inventory):
    ...
    if not _first_binding_scope_available(inventory):
        raise ValueError("replacement_current_scope_unavailable")
    ...
    with authority.maintenance(names, 30, cancel=cancel) as session:
        ...
        for selector in missing:
            ...
            if (
                ...
                raise ValueError("replacement_data_container_changed")
            if not _first_binding_scope_available(inventories[selector]):
                raise ValueError("replacement_current_scope_unavailable")
            roots = tuple(
                Path(path)
...
def replace(
    ...
    *,
    ...
    config_destinations.extend(row.config_path for row in plan.retained_configs)
    ...
    with authority.maintenance((*names, UNBOUND_NAMESPACE), 30) as session:
        ...
        try:
            # This held session has now performed the same durable moves that
            # fresh recovery admission proves. Qualify missing roots from this
            # journal before private installed or reverse credential readback.
            session._recovery_roots = tuple(
                sorted(
                    {
                        *(session._recovery_roots or ()),
                        *(path for path, _ in session._publication_roots),
                        *_recovery_staging_roots(
                            journal,
                            session._control,
                            tuple(sorted({*names, UNBOUND_NAMESPACE})),
                            session._all_roots,
                        ),
                    }
                )
...
def _abort_context(journal, plan, records):
    ...
    config_paths.update(row.config_path for row in plan.retained_configs)
    selectors = sorted(
        {
            str(item.path)
            for item in plan.target.items
