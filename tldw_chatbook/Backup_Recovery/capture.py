...
def _manifest_for(inventory, staged, aliases, options, issues):
    ...
    if options.get("data_groups") is not None:
        from .data_groups import resolve_inventory_groups

        scope = resolve_inventory_groups(inventory.items, options["data_groups"])
        doc["format_version"] = 2
        doc["group_scope"] = {
            "requested_groups": scope.requested_groups,
            "effective_groups": scope.effective_groups,
            "support_ids": sorted(
                set(scope.support_ids) & {row["logical_id"] for row in files}
            ),
        }
    encoded = json.dumps(
...
def _capture_options(options):
    ...
    selections = DiscoverySelections(
        ...
        data_groups=options.get("data_groups"),
    )
    if (
