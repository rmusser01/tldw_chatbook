...
def _config_targets(data, profile, config_target, doc, plan, owners, *, derive=False):
    ...
    for payload in records:
        ...
        elif type(owner) is _ConfiguredPets:
...
def _retained_config_targets(data, profile, config_target, doc, plan, owners):
    ...
    for selector, destination in plan.selectors:
        ...
        if expected != destination:
            raise ValueError("retained_config_selector_unverified")
    name = dict(plan.profile_names).get(profile)
    if name is not None and name != data.get("general", {}).get(
        "users_name", "default_user"
    ):
        raise ValueError("retained_config_profile_changed")


def _retained_config_validation(plan, doc, work, session, cancel, *, installed=False):
    """Copy only checked local config dependencies under existing native locks."""
    import tomllib
    ...
    items = _retained_config_items(plan, doc)
    if not items:
        return {}, {}
    if type(session) is not MaintenanceSession:
        raise ValueError("retained_config_native_session_required")
    session._check()
    if (
        session._control != bootstrap.default_bootstrap_root() / "admission"
        or UNBOUND_NAMESPACE not in session._names
        or not installed
        and not set(retained_config_names(plan)) <= set(session._names)
        or any(
            not _contains_capture_path(session._roots, item.path)
            for item in items.values()
        )
    ):
        raise ValueError("retained_config_native_scope_required")
    recheck_retained_configs(plan)
    owners = {owner.owner_id: owner for owner in install_adapters()}
    files = tuple(
        (
            item.path,
            os.stat(item.path, follow_symlinks=False).st_dev,
            os.stat(item.path, follow_symlinks=False).st_ino,
        )
        for item in items.values()
    )
    ...
    with session._capture_bound_sources(files, work, limits, limits.expanded_bytes):
        for key, item in items.items():
            candidate = work / (
                "retained-config-" + hashlib.sha256(key.encode()).hexdigest()
            )
            copy_capture_file(
                "config", item.path, candidate, cancel, max_bytes=16 * 1024**2
            )
            issues = owners["config"].validate(candidate)
            if issues:
                raise ValueError(issues[0])
            with reader._regular(candidate) as stream:
                data = _config_tables(
                    tomllib.loads(stream.read(16 * 1024**2 + 1).decode("utf-8"))
                )
            profile = key.split(":")[1]
            _retained_config_targets(data, profile, item.path, doc, plan, owners)
            ...
            if type(owner) is _Definitions:
                issues = restore_check(
                    ...
                    mapping={
                        **dict(plan.restore),
                        **{
                            row.archive_config_id: row.config_path
                            for row in plan.retained_configs
                        },
                    }
                    if plan
                    else {},
...
def stage_restore(
    ...
    *,
    ...
    if plan.retained_configs and session is None:
        raise ValueError("retained_config_native_session_required")
    ...
    for _, destination in (*plan.destinations, *plan.retire):
    ...
    try:
        ...
        retained_items, retained_candidates = _retained_config_validation(
            plan,
            doc,
            stage,
            session,
        ...
        relocation_mapping = {
            **selected,
            **{key: item.path for key, item in retained_items.items()},
        }
        ...
        with validation_scope:
            ...
            for payload in doc.files:
                ...
                if (
                    ...
                    and "settings" in plan.effective_groups
                ):
                    import tomllib
                    ...
                    # Historical Settings retain exact snapshot bytes. Verify
                    # their unselected data locations without remapping them.
                    data = tomllib.loads(candidate.read_text(encoding="utf-8"))
                    with (
                        ...
                        check_preserved_group_paths(
                            plan, data, selected[payload.logical_id]
                        )
                ...
                if plan.local_snapshot is not None and payload.owner_id == "config":
                    ...
                    owner.relocate_restore(
                        item, candidate, relocation_mapping, tuple(items.values())
                    )
                    ...
                    relocate(item, candidate, relocation_mapping)
                else:
                    owner.relocate(candidate, relocation_mapping)
                if (
                    plan.local_snapshot
        ...
        # Retirement needs retained storage on its own volume even when the
        # reviewed rollback has no incoming files to publish there.
        for _, destination in (*ordered_roots, *plan.containers, *plan.retire):
            parent = _ancestor(destination.parent)
        ...
        for root_id, destination in (*ordered_roots, *plan.containers):
            device = os.stat(_ancestor(destination.parent)).st_dev
        ...
        descriptor = {
            ...
            "retained_configs": [row.record() for row in plan.retained_configs],
            "archive_source": {
        ...
        if preserved_fingerprint is not None:
            descriptor["preserved_settings_fingerprint"] = preserved_fingerprint
