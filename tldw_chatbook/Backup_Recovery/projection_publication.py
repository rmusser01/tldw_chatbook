...
def dependent_retirements(
    target, restore, retire, preserve, *, safety_scope=(), preserve_config_dependents=False
):
    ...
    # Settings-only changes do not invalidate preserved projections, but their
    # actual config writes still count in the existing complete-source proof.
    config_only = {
        item.logical_id for item in target.items
        if preserve_config_dependents and item.owner == "config"
    }
    affected = changed - config_only
    while True:
        dependents = {
            item.logical_id
            for item in target.items
