...
def discover(
    ...
    for selected in config_paths or (profile_paths.effective_config_path(),):
        ...
        try:
            ...
            # Workflow authoring has no qualified backup adapter. Its default
            # file is caught by the child census; custom locations need an
            # explicit unsupported declaration so Complete cannot omit them.
            workflow_path = profile_paths.setting(
                config, "database", "workflows_db_path"
            )
            if workflow_path:
                ...
                declared.append(
                    _source_item(
                        "workflows.local",
                        storage_logical_id(context, "workflows.local"),
                        profile_paths.lexical_path(workflow_path),
                        (config_id,),
                    )
                )
    ...
    if selections.data_groups is not None:
        from .group_selection import select_inventory
