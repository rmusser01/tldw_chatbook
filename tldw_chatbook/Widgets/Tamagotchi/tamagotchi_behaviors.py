...
class BehaviorEngine:
    ...
    def __init__(self, personality_type: str = "balanced"):
        ...
        self.personality = PERSONALITIES.get(
            personality_type, PERSONALITIES["balanced"]
        )
    ...
    def process_action(
        self, action: str, current_stats: Dict[str, float], force: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if action not in self.action_effects:
            return {"changes": {}, "success": False, "message": "Unknown action"}
        ...
        changes = self._apply_situational_modifiers(action, changes, current_stats)
    ...
    def _apply_situational_modifiers(
        self, action: str, changes: Dict[str, float], current_stats: Dict[str, float]
    ) -> Dict[str, float]:
        """
    ...
    def calculate_interaction_bonus(
        self, interaction_count: int, time_since_last: float
    ) -> float:
        """
