...
class TamagotchiValidator:
    ...
    def validate_name(cls, name: str) -> str:
        ...
        if len(name) < cls.MIN_NAME_LENGTH:
            raise ValidationError(
                f"Pet name must be at least {cls.MIN_NAME_LENGTH} character(s)"
            )
        if len(name) > cls.MAX_NAME_LENGTH:
            raise ValidationError(
                f"Pet name cannot exceed {cls.MAX_NAME_LENGTH} characters"
            )
    ...
    def validate_personality(
        cls, personality: str, available_personalities: dict
    ) -> str:
        """
    ...
    def validate_size(cls, size: str) -> str:
        ...
        if size not in cls.VALID_SIZES:
            ...
            raise ValidationError(f"Invalid size '{size}'. Available: {available}")
    ...
    def validate_sprite_theme(cls, theme: str) -> str:
        ...
        if theme not in cls.VALID_THEMES:
            ...
            raise ValidationError(f"Invalid theme '{theme}'. Available: {available}")
...
class RateLimiter:
    ...
    def can_interact(
        self, current_time: float, action: Optional[str] = None
    ) -> tuple[bool, float]:
        """
        ...
        global_remaining = max(
            0, self.global_cooldown - (current_time - self.last_interaction)
        )
