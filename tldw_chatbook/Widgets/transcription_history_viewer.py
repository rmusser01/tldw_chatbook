...
from textual.widgets import Button, Label, Input, Select, Static, DataTable, TextArea
from textual.widget import Widget
from textual.reactive import reactive
from textual import work
from loguru import logger

from ..Audio.transcription_history import TranscriptionEntry, get_transcription_history
...
TRANSCRIPTION_COPY_DISABLED_TOOLTIP = (
    "Select a transcription entry before copying text."
)
TRANSCRIPTION_EXPORT_DISABLED_TOOLTIP = (
    "Select a transcription entry before exporting it."
)
TRANSCRIPTION_DELETE_DISABLED_TOOLTIP = (
    "Select a transcription entry before deleting it."
)
...
class TranscriptionHistoryViewer(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(classes="history-container"):
            ...
            with Container(classes="history-header"):
                ...
                # Stats
                yield Static("0 entries", id="stats-display", classes="stats-display")
            ...
            with Horizontal(classes="history-content"):
                ...
                with Container(classes="history-list"):
                    yield DataTable(
                        id="history-table", classes="history-table", cursor_type="row"
                    )
    ...
    def _update_encryption_status(self):
        ...
        if self.history.is_encrypted():
            status = Static(
                "🔒 History is encrypted", classes="encryption-status encrypted"
            )
        else:
            status = Static(
                "⚠️ History is not encrypted", classes="encryption-status unencrypted"
            )
    ...
    async def load_history(self, password: Optional[str] = None):
        ...
        try:
            # Load from file
            success = await self.run_worker(self.history.load, password=password).wait()

            if not success and self.history.is_encrypted():
                # Need password
                self.app.notify(
                    "History is encrypted. Please enter password.", severity="warning"
                )
            ...
            self.app.notify(f"Failed to load history: {str(e)}", severity="error")
        finally:
    ...
    def _populate_table(self):
        ...
        for entry in self.current_entries:
            ...
            table.add_row(
                date_str, time_str, duration_str, words_str, preview, key=entry.id
            )
