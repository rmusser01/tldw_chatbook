...
class VoiceInputWidget(Widget):
    ...
    def start_recording(self):
        ...
        try:
            ...
            if not self.dictation_service:
                self._call_from_worker_or_now(
                    self._set_error, "Voice input not initialized"
                )
                return
            ...
            if success:
                ...
                self._call_from_worker_or_now(
                    self._set_error, "Failed to start recording"
                )
    ...
    async def _monitor_audio_levels(self):
        ...
        while self.state == DictationState.LISTENING:
            if self.dictation_service:
                level = self._coerce_audio_level(
                    self.dictation_service.get_audio_level()
                )
