...
GENERATE_ANALYSES_NO_SELECTION_TOOLTIP = (
    "Select at least one media item before generating analyses."
)
...
class AnalysisGenerationEvent(Message):
    ...
    def __init__(
        self, media_id: int, status: str, content: Optional[str] = None
    ) -> None:
        super().__init__()
...
class MultiItemReviewWindow(Container):
        ...
        with VerticalScroll(classes="multi-review-container"):
            ...
            with Container(classes="analysis-section"):
                ...
                with Horizontal(classes="analysis-controls"):
                    yield Checkbox(
                        "Save analyses permanently",
                        id="save-analyses-checkbox",
                        value=False,
                    )
                    yield Button(
                        "Generate Analyses",
                ...
                yield ProgressBar(
                    ...
                    classes="analysis-progress hidden",
                    show_eta=False,
                )
    ...
    async def perform_search(self) -> None:
        ...
        try:
            ...
            tags = (
                [tag.strip() for tag in tag_filter.split(",") if tag.strip()]
                if tag_filter
                else None
            )
            ...
            for item in results:
                ...
                if item.get("id"):
                    try:
                        ...
                        keywords = fetch_keywords_for_media(
                            self.app_instance.media_db, item["id"]
                        )
                    except Exception:
                        pass
                ...
                list_item = ListItem(
                    Vertical(
                        ...
                        Static(
                            f"{media_type} | {date}{keywords_str}", classes="item-meta"
                        ),
                    )
                )
            ...
            if not results:
                await list_view.append(
                    ListItem(Label("No items found matching your search criteria."))
                )
    ...
    def handle_item_selection(self, event: ListView.Selected) -> None:
        ...
        if hasattr(event.item, "media_data"):
            ...
            if any(i["id"] == item["id"] for i in self.selected_items):
                self.selected_items = [
                    i for i in self.selected_items if i["id"] != item["id"]
                ]
    ...
    async def _generate_analyses_worker(
        ...
        for index, item in enumerate(items):
            if (
                ...
                and self.generation_worker.state == WorkerState.CANCELLED
            ):
                break
                ...
                if analysis_content:
                    ...
                    if save and self.app_instance.media_db:
                        try:
                            ...
                            logger.error(
                                f"Error saving analysis for media {media_id}: {e}"
                            )

                    # Post completion event
                    self.post_message(
                        AnalysisGenerationEvent(media_id, "completed", analysis_content)
                    )
                else:
                    self.post_message(
                        AnalysisGenerationEvent(
                            media_id, "error", "Failed to generate analysis"
                        )
                    )
    ...
    def handle_cancel_generation(self) -> None:
        ...
        if self.generation_worker:
            ...
            self.app_instance.notify(
                "Analysis generation cancelled", severity="warning"
            )
    ...
    async def handle_analysis_event(self, event: AnalysisGenerationEvent) -> None:
        ...
        if event.status == "completed" and event.content:
            # Find the media item
            item = next(
                (i for i in self.selected_items if i["id"] == event.media_id), None
            )
            if item:
                ...
                with Container(classes="analysis-result-card") as card:
                    ...
                    with Horizontal(classes="result-header"):
                        title = item.get("title", "Untitled")
                        card.mount(
                            Label(f"[bold]{title}[/bold]", classes="result-title")
                        )

                    # Metadata
                    meta_info = (
                        f"Type: {item.get('type', 'Unknown')} | ID: {item['id']}"
                    )
