...
class TreeNode(Widget):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.path = path
        self.node_name = (
            name  # Changed from 'name' to avoid conflict with Widget.name property
        )
        self.is_directory = is_directory
        self.level = level
        self.file_size = (
            size if size is not None else file_size
        )  # Changed from 'size' to avoid conflict with Widget.size property
        self.expanded = False
        self.selected = False
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="tree-node-row"):
            ...
            if self.is_directory:
                yield Button(
                    "▶", classes="tree-expand-btn", id=_safe_dom_id("expand", self.path)
                )
            else:
            ...
            icon = self._get_icon()
            size_text = (
                f" ({self._format_size(self.file_size)})" if self.file_size else ""
            )
            yield Static(f"{icon} {self.node_name}{size_text}", classes="tree-content")
