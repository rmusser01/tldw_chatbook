...
def create_chat_right_sidebar(
    id_prefix: str, initial_ephemeral_state: bool = True
) -> ComposeResult:
    """
    ...
    """
    with VerticalScroll(
        id="chat-right-sidebar", classes="sidebar"
    ):  # Main ID for the whole sidebar
        # Sidebar header with resize controls
        with Horizontal(classes="sidebar-header-with-resize"):
            yield Button(
                "<<<",
                id=f"{id_prefix}-sidebar-expand",
                classes="sidebar-resize-button",
                tooltip="Expand sidebar",
            )
            yield Static("Session & Character", classes="sidebar-title flex-grow")
            yield Button(
                ">>>",
                id=f"{id_prefix}-sidebar-shrink",
                classes="sidebar-resize-button",
        ...
        with Collapsible(
            title="Current Chat Details",
            collapsed=False,
            id=f"{id_prefix}-chat-details-collapsible",
        ):
            # "New Chat" Button
            yield Button(
                "New Temp Chat",
            ...
            yield Label(
                ...
                id=f"{id_prefix}-uuid-label-displayonly",
            )
            yield Input(
            ...
            yield Label(
                "Chat Title:",
                classes="sidebar-label",
                id=f"{id_prefix}-title-label-displayonly",
            )  # Keep consistent ID for query if needed elsewhere
            yield Input(
            ...
            yield Label(
                "Keywords (comma-sep):",
                classes="sidebar-label",
                id=f"{id_prefix}-keywords-label-displayonly",
            )
            yield TextArea(
                "",
        ...
        if id_prefix == "chat":
            with Collapsible(
                title="Search Media",
                collapsed=True,
                id=f"{id_prefix}-media-collapsible",
            ):
                ...
                yield ListView(
                    id="chat-media-search-results-listview", classes="sidebar-listview"
                )
                ...
                with Horizontal(
                    ...
                    yield Button(
                        "Prev", id="chat-media-prev-page-button", disabled=True
                    )
                    yield Label("Page 1/1", id="chat-media-page-label")
                    yield Button(
                        "Next", id="chat-media-next-page-button", disabled=True
                    )
                ...
                media_details_view.styles.height = (
                    35  # Set height to 35 lines to fit content without excess space
                )
                with media_details_view:
                    ...
                    title_display_ta = TextArea(
                        "",
                        id="chat-media-title-display",
                        read_only=True,
                        classes="detail-textarea",
                    )
                    title_display_ta.styles.height = (
                        3  # Set height to 3 lines for title
                    )
                    ...
                    content_display_ta = TextArea(
                        "",
                        id="chat-media-content-display",
                        read_only=True,
                        classes="detail-textarea content-display",
                    )
                    content_display_ta.styles.height = (
                        20  # Set height to 20 lines minimum
                    )
                    ...
                    author_display_ta = TextArea(
                        "",
                        id="chat-media-author-display",
                        read_only=True,
                        classes="detail-textarea",
                    )
                    author_display_ta.styles.height = (
                        2  # Set height to 2 lines for author
                    )
        ...
        if id_prefix == "chat":
            with Collapsible(
                title="Prompts", collapsed=True, id=f"{id_prefix}-prompts-collapsible"
            ):  # Added ID
                ...
                results_list_view = ListView(
                    id=f"{id_prefix}-prompts-listview", classes="sidebar-listview"
                )
        ...
        if id_prefix == "chat":
            with Collapsible(
                title="Notes", collapsed=True, id=f"{id_prefix}-notes-collapsible"
            ):
                ...
                notes_list_view = ListView(
                    id=f"{id_prefix}-notes-listview", classes="sidebar-listview"
                )
        ...
        with Collapsible(
            title="Active Character Info",
            collapsed=True,
            id=f"{id_prefix}-active-character-info-collapsible",
        ):  # Changed to collapsed=True to start collapsed
            if id_prefix == "chat":
                yield Input(
                ...
                yield Button("Load Character", id="chat-load-character-button")
                yield Button(
                ...
                yield Input(id="chat-character-name-edit", placeholder="Name")
        ...
        if id_prefix == "chat":
            with Collapsible(
                ...
                collapsed=True,
                id=f"{id_prefix}-dictionaries-collapsible",
            ):
        ...
        logging.debug(
            f"Character sidebar (id='chat-right-sidebar', prefix='{id_prefix}') created with ephemeral state: {initial_ephemeral_state}"
        )
