...
def _create_essential_info(id_prefix: str, initial_ephemeral: bool) -> ComposeResult:
    ...
    with Container(classes="conversation-status"):
        ...
        yield Static(
            ...
            classes="status-indicator ephemeral"
            if initial_ephemeral
            else "status-indicator saved",
        )
    ...
    with Container(classes="quick-actions"):
        ...
        yield Button(
            "🗑️ Clear", id=f"{id_prefix}-clear-chat-button", classes="clear-button"
        )
...
def _create_character_details(id_prefix: str) -> ComposeResult:
    ...
    yield Container(
        id=f"{id_prefix}-character-traits-container", classes="character-traits"
    )
...
def _create_prompt_templates(id_prefix: str) -> ComposeResult:
    ...
    yield Container(
        id=f"{id_prefix}-prompt-list-container", classes="prompt-list-container"
    )
...
def _create_media_review(id_prefix: str) -> ComposeResult:
    ...
    yield Container(
        id=f"{id_prefix}-media-metadata-container", classes="media-metadata-container"
    )
...
def _create_notes_section(id_prefix: str) -> ComposeResult:
    ...
    # Notes content
    yield TextArea(
        "", id=f"{id_prefix}-notes-content-textarea", classes="notes-textarea"
    )
    ...
    with Container(classes="notes-metadata"):
        yield Static(
            "Last saved: Never",
            id=f"{id_prefix}-notes-last-saved",
            classes="last-saved",
        )
        yield Checkbox(
            "Auto-save",
...
def _create_advanced_section(id_prefix: str) -> ComposeResult:
    ...
    with Container(classes="export-options"):
        ...
        yield Button(
            "Export as JSON", id=f"{id_prefix}-export-json", classes="export-button"
        )

        yield Button(
            "Export as Text", id=f"{id_prefix}-export-text", classes="export-button"
        )
