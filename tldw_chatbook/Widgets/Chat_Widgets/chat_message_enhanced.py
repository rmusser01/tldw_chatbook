...
class ChatMessageEnhanced(Widget):
    ...
    class Action(Message):
        ...
        def __init__(
            self, message_widget: "ChatMessageEnhanced", button: Button
        ) -> None:
            super().__init__()
    ...
    # TTS state tracking
    tts_state: reactive[str] = reactive(
        "idle"
    )  # "idle", "generating", "ready", "playing", "paused"
    tts_audio_file: reactive[Optional[Path]] = reactive(None)
    tts_progress: reactive[float] = reactive(0.0)

    # Variant tracking
    variant_of: reactive[Optional[str]] = reactive(
        None
    )  # Original message ID if this is a variant
    ...
    is_selected_variant: reactive[bool] = reactive(
        True
        ...
        with Vertical():
            ...
            if self.has_class("-ai") and (self.has_variants or self.total_variants > 1):
                with Horizontal(classes="variant-navigation"):
                    yield Button(
                        "◀",
                        id="prev-variant",
                        classes="variant-nav-button",
                        disabled=(self.variant_number <= 1),
                        tooltip="Previous variant",
                    )
                    yield Label(
                        f"Response {self.variant_number} of {self.total_variants}",
                ...
                with Container(classes="message-image-container"):
                    ...
                    with Container(classes="image-controls"):
                        yield Button(
                            ...
                            tooltip="Switch between image display modes",
                        )
                        yield Button(
                            "Save Image",
                            id="save-image",
                            tooltip="Save image to Downloads",
                        )
            ...
            with Horizontal(classes=actions_class) as actions_bar:
                ...
                if self.tts_state == "idle":
                    ...
                    yield Button(
                        "⏳",
                        classes="action-button tts-generating-button",
                        id="tts-generating",
                        tooltip=f"Generating audio... {self.tts_progress:.0%}",
                        disabled=True,
                    )
                elif self.tts_state in ["ready", "paused"]:
                    yield Button(
                        "▶️",
                        classes="action-button tts-play-button",
                        id="tts-play",
                        tooltip="Play audio",
                    )
                    yield Button(
                        "💾",
                        classes="action-button tts-save-button",
                        id="tts-save",
                        tooltip="Save audio",
                    )
                    yield Button(
                        "⏹️",
                        classes="action-button tts-stop-button",
                        id="tts-stop",
                        tooltip="Stop and clear audio",
                    )
                elif self.tts_state == "playing":
                    yield Button(
                        "⏸️",
                        classes="action-button tts-pause-button",
                        id="tts-pause",
                        tooltip="Pause audio",
                    )
                    yield Button(
                ...
                if self.has_class("-ai"):
                    ...
                    yield Button(
                        "💡",
                        classes="action-button suggest-response-button",
                        id="suggest-response",
...
[/dim]"""
    ...
    def watch_tts_state(self, old_state: str, new_state: str) -> None:
        ...
        logging.info(
            f"TTS state changed from {old_state} to {new_state} for message {self.message_id_internal}"
        )
    ...
    def watch__generation_complete_internal(self, complete: bool) -> None:
        ...
        logging.info(
            f"watch__generation_complete_internal called with complete={complete}"
        )
        if self.has_class("-ai"):
            try:
                ...
                logging.warning(
                    f"QueryError in watch__generation_complete_internal: {qe}"
                )
            except Exception as e:
                logging.error(
                    f"Error in watch__generation_complete_internal: {e}", exc_info=True
                )
    ...
    async def handle_save_image(self) -> None:
        ...
        try:
            ...
            extension = (
                self.image_mime_type.split("/")[-1] if self.image_mime_type else "png"
            )
    ...
    def update_tts_progress(self, progress: float, status: str = "") -> None:
        ...
        if status:
            logging.debug(
                f"TTS progress for message {self.message_id_internal}: {progress:.0%} - {status}"
            )
    ...
    def update_variant_info(
        self, variant_num: int, total: int, is_selected: bool = False
    ):
