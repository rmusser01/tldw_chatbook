...
def _derive_session_title(
    ...
    effective_fallback_title = (
        session_data.title if fallback_title is None else fallback_title
    )
    assistant_name = None
...
class ChatTabContainer(Container):
    ...
    async def create_new_tab(
        ...
        try:
            ...
            if session_data is not None:
                ...
                session_data = ChatSessionData(
                    tab_id=tab_id, title=title or f"Chat {len(self.sessions) + 1}"
                )
            ...
            try:
                ...
                self.app_instance.notify(
                    "Failed to create chat session", severity="error"
                )
                return ""
    ...
    async def close_tab(self, tab_id: str) -> None:
        ...
        if (
            not session.session_data.is_ephemeral
            and session.session_data.has_unsaved_changes
        ):
            ...
            dialog = UnsavedChangesDialog(
                tab_title=session.session_data.title, confirm_callback=confirm_close
            )
    ...
    async def on_chat_tab_bar_tab_selected(
        self, message: ChatTabBar.TabSelected
    ) -> None:
    ...
    async def on_chat_tab_bar_new_tab_requested(
        self, message: ChatTabBar.NewTabRequested
    ) -> None:
    ...
    def update_active_tab_title(
        self, new_title: str, character_name: Optional[str] = None
    ) -> None:
        ...
        if session:
            ...
            if self.tab_bar:
                self.tab_bar.update_tab_title(
                    self.active_session_id, new_title, character_name
                )
    ...
    def update_tab_title(
        self, tab_id: str, new_title: str, character_name: Optional[str] = None
    ) -> None:
        """
    ...
    async def restore_sessions_from_state(
        self, state: Dict[str, ChatSessionData]
    ) -> None:
        """
