...
class MinimalSettingsSidebar(Container):
    ...
    def __init__(self, id_prefix: str, config: dict, **kwargs):
        ...
        self.defaults = config.get(
            f"{id_prefix}_defaults", config.get("chat_defaults", {})
        )
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id=f"{self.id_prefix}-sidebar", classes="minimal-sidebar"):
            ...
            with Horizontal(classes="quick-actions"):
                yield Button(
                    "New Chat", id=f"{self.id_prefix}-new-chat", variant="primary"
                )
                yield Button(
                    "Clear", id=f"{self.id_prefix}-clear-chat", variant="warning"
                )
            ...
            with Container(classes="core-settings"):
                ...
                yield Checkbox(
                    "Stream responses", value=True, id=f"{self.id_prefix}-streaming"
                )
            ...
            with Container(
                ...
                yield Checkbox(
                    "Enable RAG", value=False, id=f"{self.id_prefix}-rag-enable"
                )

                # Max tokens
                yield Label("Max Tokens", classes="setting-label")
                yield Input(
                    value="2048", id=f"{self.id_prefix}-max-tokens", placeholder="2048"
                )
    ...
    def _compose_provider_model(self) -> ComposeResult:
        ...
        default_provider = self.defaults.get(
