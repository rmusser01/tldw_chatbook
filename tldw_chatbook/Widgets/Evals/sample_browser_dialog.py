...
class SampleBrowserDialog(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Container(classes="browser-dialog"):
            ...
            with Container(classes="dialog-header"):
                yield Label(
                    f"Sample Browser: {self.dataset_path.name}", classes="dialog-title"
                )
            ...
            with Container(classes="stats-panel"):
                with Grid(classes="stats-grid"):
                    ...
                    with Container(classes="stat-item"):
                        yield Static(
                            self.task_type, id="task-type", classes="stat-value"
                        )
                    ...
                    with Container(classes="stat-item"):
                        yield Static(
                            self.dataset_path.suffix[1:].upper(), classes="stat-value"
                        )
            ...
            with Tabs(classes="main-content"):
                with TabPane("List View", id="list-tab"):
                    with Horizontal():
                        ...
                        yield ScrollableContainer(
                            id="samples-list", classes="samples-list"
                        )
                ...
                with TabPane("JSON View", id="json-tab"):
                    yield TextArea(
                        "", id="json-editor", language="json", theme="monokai"
                    )

            # Navigation controls
            with Horizontal(classes="nav-controls"):
                yield Button(
                    "◀ Previous", id="prev-btn", classes="nav-button", disabled=True
                )
                yield Static("Page 1 / 1", id="page-display", classes="page-info")
                yield Button(
                    "Next ▶", id="next-btn", classes="nav-button", disabled=True
                )
    ...
    def update_list_view(self) -> None:
        ...
        for sample_index, sample, preview in visible_rows:
            ...
            if row is None:
                samples_list.mount(
                    self._build_sample_row(sample_index, sample, preview)
                )
            else:
    ...
    def update_statistics(self) -> None:
        ...
        if self.filtered_samples:
            avg_len = sum(
                len(s.get("input_text", "")) for s in self.filtered_samples
        ...
        if self.task_type == "classification" and self.filtered_samples:
            unique_answers = len(
                set(s.get("expected_output", "") for s in self.filtered_samples)
            )
    ...
    def update_pagination(self) -> None:
        ...
        total_pages = (
            len(self.filtered_samples) + self.samples_per_page - 1
