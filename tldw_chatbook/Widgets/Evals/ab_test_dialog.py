...
class ABTestDialog(ModalScreen):
        ...
        with Container(classes="ab-dialog"):
            ...
            with Horizontal(classes="model-selection"):
                with Container(classes="model-column"):
                    ...
                    yield Select([], id="model-a-select", classes="form-select")

                with Container(classes="model-column"):
                    yield Static("Model B", classes="model-title")
                    yield Select([], id="model-b-select", classes="form-select")

            # Progress section (hidden initially)
            with Container(classes="progress-section", id="progress-section"):
                yield ProgressBar(total=100, id="test-progress", classes="progress-bar")
                yield Static(
                    "Ready to start", id="progress-status", classes="progress-status"
                )
            ...
            with Horizontal(classes="button-row"):
                yield Button(
                    "Run Test", id="run-btn", classes="action-button", variant="primary"
                )
                yield Button(
                    "Export Results",
                    id="export-btn",
    ...
    async def load_data(self) -> None:
        ...
        try:
            ...
            self.tasks = await self.app.run_in_executor(
                None, self.orchestrator.db.list_tasks
            )

            # Load models
            self.models = await self.app.run_in_executor(
                None, self.orchestrator.db.list_models
            )
    ...
    def update_selects(self) -> None:
        ...
        task_options = [
            (f"{t['name']} ({t['task_type']})", t["id"]) for t in self.tasks
        ]
        ...
        model_options = [
            (f"{m['name']} ({m['provider']})", m["id"]) for m in self.models
        ]
    ...
    async def run_ab_test(self) -> None:
        ...
        try:
            ...
            result = await self.runner.run_ab_test(
                config, progress_callback=self.update_progress
            )
        ...
        for metric in result.model_a_metrics:
            if metric in result.model_b_metrics:
                ...
                with grid.mount(Container(classes="metric-box")):
                    grid.mount(
                        Static(
                            f"{result.model_a_name}: {metric}", classes="metric-name"
                        )
                    )
                ...
                with grid.mount(Container(classes="metric-box")):
                    grid.mount(
                        Static(
                            f"{result.model_b_name}: {metric}", classes="metric-name"
                        )
                    )
                ...
                if isinstance(value_a, (int, float)) and isinstance(
                    ...
                    with grid.mount(Container(classes="metric-box")):
                        grid.mount(
                            Static(f"Difference: {metric}", classes="metric-name")
                        )
                        grid.mount(
                            Static(
    ...
    def display_details(self, result: ABTestResult) -> None:
        ...
        for i, sample in enumerate(result.sample_results[:100]):
            input_preview = (
                sample.get("input", "")[:50] + "..."
                if len(sample.get("input", "")) > 50
                else sample.get("input", "")
            )
            model_a_correct = "✓" if sample.get("model_a_correct") else "✗"
            model_b_correct = "✓" if sample.get("model_b_correct") else "✗"
            agreement = (
                "="
                if sample.get("model_a_correct") == sample.get("model_b_correct")
                else "≠"
            )

            table.add_row(
                str(i + 1), input_preview, model_a_correct, model_b_correct, agreement
            )
    ...
    def handle_export(self, event: Button.Pressed) -> None:
        ...
        if self.current_result:
            if self.callback:
                self.callback({"action": "export", "result": self.current_result})
