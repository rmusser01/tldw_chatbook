...
from tldw_chatbook.Evals.eval_errors import ErrorContext, ErrorSeverity, EvaluationError
...
class ErrorDetailsDialog(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Container(id="error-dialog"):
            ...
            if self.error_context:
                with Collapsible(
                    title="Technical Details", id="technical-details", collapsed=True
                ):
            ...
            with Horizontal(id="error-actions"):
                if (
                    self.error_context
                    and self.error_context.is_retryable
                    and self.on_retry
                ):
...
class ErrorSummaryWidget(Container):
    ...
    def add_error(self, error: Exception, context: Optional[Dict[str, Any]] = None):
        ...
        error_info = {"error": error, "context": context, "timestamp": datetime.now()}
    ...
    def update_display(self):
        ...
        for error_info in recent_errors:
            ...
            error_item = Static(f"• [{category}] {message}", classes="error-item")
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "show-all-errors":
            ...
            self.notify(
                "Error history dialog not implemented yet", severity="information"
            )
...
def show_error_dialog(
    app, error: Exception, title: str = "Evaluation Error", allow_retry: bool = True
) -> None:
