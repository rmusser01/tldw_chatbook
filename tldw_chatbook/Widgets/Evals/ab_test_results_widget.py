...
class ABTestResultsWidget(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(classes="test-header"):
            yield Static(
                self.test_data.get("name", "Unnamed Test"), classes="test-name"
            )
            status = self.test_data.get("status", "pending")
            yield Static(status.upper(), classes=f"test-status status-{status}")

        # Progress bar for running tests
        with Container(
            classes=f"progress-container {'visible' if self.test_data.get('status') == 'running' else ''}"
        ):
            yield ProgressBar(total=100, show_percentage=True)

        # Winner section (if completed)
        if self.test_data.get("status") == "completed" and self.test_data.get(
            "result_data"
        ):
                    ...
                    with Container(classes="metrics-list"):
                        for metric, value in result.get("model_a_metrics", {}).items():
                            with Horizontal(classes="metric-row"):
                                ...
                                if value_b is not None and isinstance(
                                    value, (int, float)
                                ):
                                    if value > value_b:
                    ...
                    with Container(classes="metrics-list"):
                        for metric, value in result.get("model_b_metrics", {}).items():
                            with Horizontal(classes="metric-row"):
                                ...
                                css_class = "metric-value"
                                if value_a is not None and isinstance(
                                    value, (int, float)
                                ):
                                    if value > value_a:
                                        css_class += " metric-better"
            ...
            if result.get("statistical_tests"):
                with Container(classes="statistical-summary"):
                    with Grid(classes="stat-grid"):
                        ...
                        # Confidence level
                        first_test = next(
                            iter(result["statistical_tests"].values()), {}
                        )
                        confidence = (
                            "95%"
                            if first_test.get("p_value", 1) < 0.05
                        ...
                        p_values = [
                            t.get("p_value", 1)
                            for t in result["statistical_tests"].values()
                        ]
