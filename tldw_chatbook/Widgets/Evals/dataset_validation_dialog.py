...
from tldw_chatbook.Evals.dataset_validator import (
    DatasetValidator,
    ValidationReport,
    ValidationIssue,
)
...
class DatasetValidationDialog(ModalScreen):
        ...
        with Container(classes="validation-dialog"):
            ...
            with Container(classes="summary-section"):
                with Grid(classes="summary-grid"):
                    ...
                    with Container(classes="summary-stat"):
                        yield Static(str(info_count), classes="stat-value")
    ...
    def populate_issues(self) -> None:
        ...
        if not sorted_issues:
            issues_list.append(
                ListItem(Static("✅ No issues found!", classes="success-message"))
            )
    ...
    def populate_statistics(self) -> None:
        ...
        if "quality" in self.validation_report.statistics:
            ...
            if "unique_questions" in quality:
                ...
                stats_md += (
                    f"- **Duplicate Questions**: {quality['duplicate_questions']}\n"
                )
        ...
        if "balance" in self.validation_report.statistics:
            ...
            if "answer_distribution" in balance:
                ...
                for answer, count in balance["answer_distribution"].items():
                    percentage = (
                        (count / self.validation_report.total_samples * 100)
                        if self.validation_report.total_samples > 0
                        else 0
                    )
        ...
        if "quality" in self.validation_report.statistics:
            ...
            if "topic_distribution" in quality:
                ...
                for topic, count in sorted(
                    quality["topic_distribution"].items(),
                    key=lambda x: x[1],
                    reverse=True,
                )[:10]:
    ...
    def populate_suggestions(self) -> None:
        ...
        suggestions_content = self.query_one(
            "#suggestions-content", ScrollableContainer
        )
        ...
        if suggestions:
            ...
            suggestions_content.mount(
                Static(
                    "✅ No fixes needed - dataset is valid!", classes="success-message"
                )
            )
