...
class FileUploadDialog(ModalScreen):
    ...
    def __init__(
        self, callback: Optional[Callable[[Optional[str]], None]] = None, **kwargs
    ):
    ...
    def compose(self) -> ComposeResult:
        with Container(classes="config-dialog"):
            ...
            with Grid(classes="config-grid"):
                ...
                yield Input(placeholder="/path/to/task/file.yaml", id="file-path")
    ...
    def handle_browse(self):
        ...
        self.app.notify(
            "File browser not implemented - enter path manually", severity="information"
        )
    ...
    def compose(self) -> ComposeResult:
        with Container(classes="config-dialog"):
            ...
            with Grid(classes="config-grid"):
                ...
                yield Input(placeholder="/path/to/export/file", id="output-path")
    ...
    def compose(self) -> ComposeResult:
        with Container(classes="config-dialog"):
            ...
            with Grid(classes="config-grid"):
                ...
                yield Input(placeholder="Search in input/output text", id="text-search")
...
class RunSelectionDialog(ModalScreen):
    ...
    def handle_compare(self):
        ...
        if len(self.selected_runs) < 2:
            self.app.notify(
                "Please select at least 2 runs to compare", severity="error"
            )
            return
