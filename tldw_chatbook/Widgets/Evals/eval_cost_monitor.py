...
class CostWarning(Message):
    ...
    def __init__(
        self, limit_type: str, current: float, limit: float, suggestions: List[str]
    ):
        super().__init__()
...
class CostMonitor(Container):
    ...
    def __init__(self, cost_history: Optional[List[Dict[str, Any]]] = None, **kwargs):
        ...
        self._warning_shown = {"daily": False, "weekly": False, "monthly": False}
        ...
        with Vertical(classes="cost-monitor"):
            ...
            with Grid(classes="budget-grid"):
                ...
                with Vertical(classes="budget-item"):
                    ...
                    yield Static(
                        "$0.00 / $10.00", id="daily-text", classes="budget-text"
                    )
                ...
                with Vertical(classes="budget-item"):
                    ...
                    yield Static(
                        "$0.00 / $50.00", id="weekly-text", classes="budget-text"
                    )
                ...
                with Vertical(classes="budget-item"):
                    ...
                    yield Static(
                        "$0.00 / $200.00", id="monthly-text", classes="budget-text"
                    )
    ...
    def _check_budget_warning(self, period: str, current: float, limit: float) -> None:
        ...
        if (
            percentage >= self.limits.warning_threshold
            and not self._warning_shown[period]
        ):
    ...
    def _generate_cost_suggestions(
        self, period: str, current: float, limit: float
    ) -> List[str]:
        ...
        if breakdown.by_model:
            ...
            if expensive_model[1] > current * 0.5:  # If one model is >50% of cost
                suggestions.append(
                    f"Consider using a cheaper model than {expensive_model[0]}"
                )
        ...
        if remaining > 0:
            ...
            suggestions.append(
                f"Budget remaining: ${remaining:.2f} (~{samples_remaining} samples)"
            )
    ...
    def get_cost_breakdown(self) -> CostBreakdown:
        ...
        return CostBreakdown(
            by_model=by_model, by_task=by_task, by_date=by_date, total=total
        )
