...
class ProgressTracker(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(classes="progress-details"):
            ...
            yield Static(
                "-- samples/min", id="progress-rate", classes="progress-detail"
            )
...
class MetricsDisplay(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(
            "No metrics available", id="metrics-content", classes="metrics-content"
        )
    ...
    def _format_metrics(self, metrics: Dict[str, Any]) -> str:
        ...
        for key, value in metrics.items():
            if "accuracy" in key.lower() or "exact_match" in key.lower():
                ...
                "time" in key.lower() or "rate" in key.lower() or "count" in key.lower()
            ):
...
class ResultsTable(Container):
    ...
    def on_mount(self):
        ...
        try:
            ...
            table.add_columns(
                "Sample ID", "Input", "Expected", "Actual", "Score", "Status"
            )
        except Exception as e:
...
class ComparisonView(Container):
    ...
    def _format_comparison(self) -> str:
        ...
        # Headers
        headers = ["Metric"] + [
            run.get("name", f"Run {i + 1}") for i, run in enumerate(self.runs)
        ]
    ...
    async def handle_add_run(self):
        ...
        dialog = RunSelectionDialog(
            callback=on_runs_selected, available_runs=available_runs
        )
        await self.app.push_screen(dialog)
    ...
    def handle_remove_run(self):
        ...
        if self.runs:
            ...
            self._update_comparison_display()
            self.app.notify(
                f"Removed run: {removed_run.get('name', 'Unknown')}",
    ...
    async def handle_export_comparison(self):
        ...
        if len(self.runs) < 2:
            self.app.notify(
                "Add at least 2 runs to export comparison", severity="error"
            )
            return
