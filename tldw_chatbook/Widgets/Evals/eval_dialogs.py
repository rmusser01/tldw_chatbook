...
def open_dataset_file_picker(app, callback: Callable[[Path, Dict[str, Any]], None]):
    ...
    def handle_file_selected(path: Optional[Path]) -> None:
        ...
        if path:
            ...
            app.push_screen(DatasetFilePickerDialog(file_path=path, callback=callback))
...
class AdvancedConfigDialog(ModalScreen):
            ...
            with VerticalScroll(classes="config-scroll"):
                ...
                with Container(classes="config-section"):
                    ...
                    with Grid(classes="advanced-grid"):
                        ...
                        yield Input(
                            value=str(
                                self.current_config.get("frequency_penalty", 0.0)
                            ),
                            id="freq-penalty",
                            type="number",
                        )
        ...
        try:
            ...
            config = {
                ...
                "frequency_penalty": float(
                    self.query_one("#freq-penalty", Input).value
                ),
    ...
    def handle_reset(self) -> None:
        ...
        self.query_one("#freq-penalty", Input).value = str(
...
class TemplateSelectorDialog(ModalScreen):
    ...
    def __init__(
        self, callback: Optional[Callable[[str, Dict[str, Any]], None]] = None, **kwargs
    ):
        super().__init__(**kwargs)
        self.callback = callback
        ...
        with Container(classes="template-selector-dialog"):
            ...
            with Container(classes="template-content"):
                ...
                with Container(classes="template-list-container"):
                    ...
                    yield ListView(id="template-list", classes="template-list")
                ...
                with Container(classes="template-preview-container"):
                    ...
                    yield Static("", id="template-config", classes="config-display")

                    with Horizontal(classes="preview-actions"):
                        yield Button("Customize", id="customize-btn", disabled=True)

            # Action buttons
            with Horizontal(classes="dialog-buttons"):
                yield Button(
                    "Use Template", id="use-btn", variant="primary", disabled=True
                )
                yield Button("Cancel", id="cancel-btn", variant="default")
    ...
    def on_mount(self) -> None:
        ...
        for template_id, template_data in self.TEMPLATES.items():
            item = ListItem(Label(f"{template_data['name']}"))
        ...
        with Container(classes="export-dialog"):
            ...
            with Container(classes="format-section"):
                ...
                with RadioSet(id="format-radio"):
                    ...
                    yield RadioButton(
                        "JSON - Full data with metadata", id="format-json"
                    )
            ...
            with Container(classes="options-section"):
                ...
                with Grid(classes="export-options-grid"):
                    ...
                    yield Checkbox(
                        "Include configuration", value=True, id="include-config"
                    )
                    yield Checkbox(
                        "Include timestamps", value=False, id="include-timestamps"
                    )
                    yield Checkbox(
                        "Aggregate by metric", value=False, id="aggregate-metrics"
                    )
                    yield Checkbox(
                        "Add summary statistics", value=True, id="add-summary"
                    )
