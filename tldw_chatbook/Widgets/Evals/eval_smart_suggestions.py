...
class SmartSuggestions(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(classes="smart-suggestions"):
            ...
            with Container(classes="recommendations-section"):
                ...
                yield Container(
                    id="model-recommendations", classes="recommendations-list"
                )
    ...
    def watch_current_task(
        self, old_task: Optional[str], new_task: Optional[str]
    ) -> None:
    ...
    def _update_suggestions(self) -> None:
        ...
        if self.current_task:
            recommendations = self._get_model_recommendations(
                self.current_task, self.current_budget
            )
    ...
    def _get_history_score(self, task: str, model: str) -> float:
        ...
        for entry in self.usage_history:
            if entry["config"].get("task") == task and model in entry["config"].get(
                "model", ""
            ):
                total += 1
                if entry.get("success", False):
            ...
            with container:
                with Horizontal(classes="recommendation-item"):
                    ...
                    with Vertical(classes="model-info"):
                        yield Static(
                            f"{rec.provider}/{rec.model_id}", classes="model-name"
                        )
                        yield Static(
                    ...
                    with Vertical(classes="model-stats"):
                        yield Static(
                            f"~${rec.estimated_cost:.2f}", classes="model-cost"
                        )
                        yield Static(f"~{rec.estimated_time}min", classes="model-time")

                    # Use button
                    yield Button(
                        "Use", classes="use-model-btn", id=f"use-{rec.model_id}"
                    )

    def _display_config_suggestions(self, suggestions: List[ConfigSuggestion]) -> None:
    ...
    def _update_insights(self) -> None:
        ...
        if self.usage_history:
            avg_cost = sum(
                entry["result"].get("cost", 0) for entry in self.usage_history
