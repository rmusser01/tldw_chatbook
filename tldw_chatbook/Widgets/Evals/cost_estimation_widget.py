...
from tldw_chatbook.Utils.cost_estimation import (
...
class CostEstimationWidget(Container):
        ...
        with Vertical(classes="cost-widget"):
            ...
            with Container(classes="cost-section hidden", id="tracking-section"):
                ...
                with Horizontal(classes="cost-progress"):
                    ...
                    yield Static(
                        "$0.00", id="total-estimate", classes="cost-display muted"
                    )

                yield ProgressBar(id="cost-progress-bar", show_percentage=False)
                yield Static(
                    "0 / 0 samples", id="sample-progress", classes="progress-text"
                )

            # Cost breakdown
            with Container(classes="cost-section", id="details-section"):
                yield Button(
                    "Details ▼", id="toggle-details-btn", classes="section-label"
                )

                with Container(
                    classes="details-content collapsed", id="details-content"
                ):
            ...
            with Container(classes="warning-section hidden", id="budget-warning"):
                ...
                yield Button(
                    "View Alternatives", id="view-alternatives", variant="warning"
                )
    ...
    def estimate_cost(
        ...
        avg_tokens_per_sample = avg_input_tokens + avg_output_tokens

        # Get estimation using simplified function
        estimated_cost = estimate_evaluation_cost(
            model_id, num_samples, avg_tokens_per_sample
        )

        self.estimated_total = estimated_cost

        # Update display with simple cost info
        cost_per_1k = get_model_cost_per_1k_tokens(model_id)
        estimation_info = {
        ...
        self._update_estimation_display(estimation_info)
    ...
    def update_sample_cost(
        self, input_tokens: int, output_tokens: int, sample_index: int
    ) -> None:
        ...
        self.cost_estimator.add_sample_cost(
            self.run_id, input_tokens, output_tokens, self.provider, self.model
        )
