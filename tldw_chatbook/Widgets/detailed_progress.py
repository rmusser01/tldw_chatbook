...
class DetailedProgressBar(Widget):
            ...
            with Horizontal(classes="progress-metrics-row"):
                ...
                if self.show_speed:
                    with Vertical(classes="metric-group"):
                        ...
                        yield Static(
                            "0 items/sec", id="speed-metric", classes="metric-value"
                        )
                ...
                with Vertical(classes="metric-group"):
                    ...
                    yield Static(
                        "Calculating...", id="remaining-time", classes="metric-value"
                    )
    ...
    def complete_stage(self) -> None:
        ...
        if self.current_stage < len(self.stages):
            ...
            self.stages[self.current_stage].current = self.stages[
                self.current_stage
            ].total
    ...
    def _update_current_item_display(self) -> None:
        ...
        try:
            ...
            # Truncate long items
            display_text = (
                self.current_item[:50] + "..."
                if len(self.current_item) > 50
                else self.current_item
            )
    ...
    def _update_progress_bars(self) -> None:
        ...
        try:
            ...
            stage = self.stages[self.current_stage]
            stage_progress = (
                (stage.current / stage.total * 100) if stage.total > 0 else 0
            )
        ...
        try:
            ...
            if self.show_speed and self.speed_history:
                ...
                if self.bytes_processed > 0:
                    mb_per_sec = (
                        self.bytes_processed / 1024 / 1024
            ...
            if self.items_processed > 0:
                ...
                if total_processed > 0:
                    ...
                    self.query_one("#remaining-time", Static).update(
                        str(remaining_time)
                    )
            ...
            if self.show_memory:
                try:
                    ...
                    self.query_one("#memory-usage", Static).update(
                        f"{memory_mb:.1f} MB"
                    )
                except Exception:
                    pass
