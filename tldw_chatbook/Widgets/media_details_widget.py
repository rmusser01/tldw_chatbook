...
class MediaDetailsWidget(Container):
    ...
    def on_mount(self) -> None:
        ...
        try:
            ...
            chunk_method_select = self.query_one(
                f"#chunk-method-{self.type_slug}", Select
            )
    ...
    def compose(self) -> ComposeResult:
        ...
        # Metadata section
        with Container(
            id=f"metadata-section-{self.type_slug}", classes="metadata-section"
        ):
            # View mode elements
            with Container(
                id=f"metadata-view-{self.type_slug}", classes="metadata-view"
            ):
                yield Static(
                    "",
                    id=f"metadata-display-{self.type_slug}",
                    classes="metadata-display",
                )
                with Horizontal(classes="metadata-buttons"):
                    yield Button(
                        "Edit",
                        id=f"edit-button-{self.type_slug}",
                        classes="metadata-edit-button",
                    )
                    yield Button(
                        "Delete",
                        id=f"delete-button-{self.type_slug}",
                        classes="metadata-delete-button",
                        variant="error",
                    )

            # Edit mode elements (initially hidden)
            with Container(
                id=f"metadata-edit-{self.type_slug}", classes="metadata-edit hidden"
            ):
                # Input fields for editing
                with Vertical(classes="edit-fields"):
                    yield Label("Title:")
                    yield Input(
                        id=f"edit-title-{self.type_slug}", placeholder="Enter title"
                    )

                    yield Label("Type:")
                    yield Input(
                        id=f"edit-type-{self.type_slug}",
                    ...
                    yield Label("Author:")
                    yield Input(
                        id=f"edit-author-{self.type_slug}", placeholder="Enter author"
                    )

                    yield Label("URL:")
                    yield Input(
                        id=f"edit-url-{self.type_slug}", placeholder="Enter URL"
                    )

                    yield Label("Keywords (comma-separated):")
                    yield Input(
                        id=f"edit-keywords-{self.type_slug}",
                ...
                with Horizontal(classes="edit-actions"):
                    yield Button(
                        "Save", id=f"save-button-{self.type_slug}", variant="primary"
                    )
                    yield Button(
                        "Cancel",
                        id=f"cancel-button-{self.type_slug}",
                        variant="default",
                    )
        ...
        with Collapsible(
            ...
            id=f"chunking-config-section-{self.type_slug}",
            classes="chunking-config-section",
            collapsed=True,
        ):
            # Current config display
            yield Static(
                "Current: Default configuration",
                id=f"chunking-config-display-{self.type_slug}",
                classes="chunking-config-display",
            )
            ...
            with Collapsible(
                ...
                collapsed=True,
                id=f"chunking-advanced-{self.type_slug}",
            ):
            ...
            with Horizontal(classes="chunking-actions"):
                yield Button(
                    "Save Config",
                    id=f"save-chunking-{self.type_slug}",
                    variant="primary",
                )
                yield Button(
                    "Preview Chunks",
        ...
        with Container(
            id=f"content-search-section-{self.type_slug}",
            classes="content-search-section",
        ):
            with Horizontal(classes="search-controls"):
                ...
                yield Static(
                    "",
                    id=f"content-search-status-{self.type_slug}",
                    classes="content-search-status",
                )
    ...
    def watch_media_data(
        self, old_data: Optional[Dict], new_data: Optional[Dict]
    ) -> None:
        """React to media data changes by updating the display."""
        # Initialize chunking service if needed
        if (
            not self.chunking_service
            and hasattr(self.app_instance, "media_db")
        ...
        if self.media_data:
            ...
            try:
                content_area = self.query_one(
                    f"#content-display-{self.type_slug}", TextArea
                )
            ...
            if self.app_instance.media_db and media_id:
                try:
                    ...
                    keywords = fetch_keywords_for_media(
                        self.app_instance.media_db, media_id
                    )
    ...
    def _clear_displays(self) -> None:
        ...
        try:
            metadata_display = self.query_one(
                f"#metadata-display-{self.type_slug}", Static
            )
    ...
    def _save_metadata(self) -> None:
        ...
        try:
            ...
            author = self.query_one(
                f"#edit-author-{self.type_slug}", Input
            ).value.strip()
            url = self.query_one(f"#edit-url-{self.type_slug}", Input).value.strip()
            keywords_str = self.query_one(
                f"#edit-keywords-{self.type_slug}", Input
            ).value.strip()
            ...
            self.app_instance.notify(
                f"Error saving metadata: {str(e)}", severity="error"
            )
    ...
    def _perform_undelete(self) -> None:
        ...
        self.post_message(
            MediaUndeleteEvent(media_id=self.media_data["id"], type_slug=self.type_slug)
        )
    ...
    def _save_chunking_config(self) -> None:
        ...
        try:
            ...
            template = self.query_one(
                f"#chunking-template-select-{self.type_slug}", Select
            ).value
            chunk_size = self.query_one(f"#chunk-size-{self.type_slug}", Input).value
            chunk_overlap = self.query_one(
                f"#chunk-overlap-{self.type_slug}", Input
            ).value
            chunk_method = self.query_one(
                f"#chunk-method-{self.type_slug}", Select
            ).value
            ...
            self.app_instance.notify(
                "Chunking configuration saved", severity="information"
            )

        except ChunkingTemplateError as e:
            logger.error(f"Error saving chunking config: {e}")
            self.app_instance.notify(
                f"Error saving configuration: {str(e)}", severity="error"
            )
        except Exception as e:
    ...
    def _reset_chunking_config(self) -> None:
        ...
        try:
            ...
            if self.media_data and self.chunking_service:
                ...
                self.app_instance.notify(
                    "Chunking configuration reset to defaults", severity="information"
                )

        except ChunkingTemplateError as e:
            logger.error(f"Error resetting chunking config: {e}")
            self.app_instance.notify(
                f"Error resetting configuration: {str(e)}", severity="error"
            )
        except Exception as e:
                    ...
                    for stage in template_data["pipeline"]:
                        if stage.get("stage") == "chunk":
                            ...
                            if "max_size" in options:
                                self.query_one(
                                    f"#chunk-size-{self.type_slug}", Input
                                ).value = str(options["max_size"])
                            if "overlap" in options:
                                self.query_one(
                                    f"#chunk-overlap-{self.type_slug}", Input
                                ).value = str(options["overlap"])
                            if "method" in stage:
                                self.query_one(
                                    f"#chunk-method-{self.type_slug}", Select
                                ).value = stage["method"]
    ...
    def _update_chunking_display(self, config: Optional[Dict[str, Any]]) -> None:
        ...
        try:
            display = self.query_one(
                f"#chunking-config-display-{self.type_slug}", Static
            )
            ...
            if config:
                ...
                display.update(
                    f"Current: {template} - {method} ({size} words, {overlap} overlap)"
                )
            else:
    ...
    def _load_chunking_config(self) -> None:
        ...
        try:
            ...
            if config:
                ...
                self.query_one(f"#chunk-size-{self.type_slug}", Input).value = str(
                    config.get("chunk_size", 400)
                )
                self.query_one(f"#chunk-overlap-{self.type_slug}", Input).value = str(
                    config.get("chunk_overlap", 100)
                )
                self.query_one(
                    f"#chunk-method-{self.type_slug}", Select
                ).value = config.get("method", "words")
    ...
    async def _perform_content_search(self, event) -> None:
        ...
        try:
            search_input = self.query_one(
                f"#content-search-input-{self.type_slug}", Input
            )
            ...
            if matches:
                ...
                self.current_match_index = -1
                self.app_instance.notify(
                    f"No matches found for '{search_term}'", severity="information"
                )
    ...
    def _update_search_status(self) -> None:
        ...
        try:
            ...
            if not self.search_matches:
                ...
                status.update(
                    f"{self.current_match_index + 1} of {len(self.search_matches)}"
                )
    ...
    def _update_search_navigation(self) -> None:
        ...
        try:
            prev_button = self.query_one(
                f"#content-search-prev-{self.type_slug}", Button
            )
            next_button = self.query_one(
                f"#content-search-next-{self.type_slug}", Button
            )
    ...
    def watch_search_matches(
        self, old_matches: List[Tuple[int, int]], new_matches: List[Tuple[int, int]]
    ) -> None:
