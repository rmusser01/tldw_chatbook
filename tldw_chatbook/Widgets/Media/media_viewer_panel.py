...
ANALYSIS_SAVE_EXISTING_TOOLTIP = (
    "This analysis is already saved. Use Overwrite to update it."
)
...
ANALYSIS_OVERWRITE_ENABLED_TOOLTIP = (
    "Overwrite the saved analysis with the edited text."
)
...
READING_HIGHLIGHT_DELETE_DISABLED_TOOLTIP = (
    "Select a reading highlight before deleting it."
)
...
class MediaViewerPanel(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(classes="tab-header"):
            yield Button("◀", id="collapse-media-list", classes="collapse-media-list")
        ...
        with TabbedContent():
                ...
                with VerticalScroll():
                    ...
                    with Container(id="metadata-view", classes="metadata-section"):
                        ...
                        with Collapsible(title="Actions", collapsed=True):
                            with Horizontal(classes="metadata-buttons"):
                                ...
                                yield Button(
                                    "Delete", id="delete-button", variant="error"
                                )

                        with Collapsible(
                            title="Reading Highlights",
                            collapsed=True,
                            id="reading-highlights-editor",
                        ):
                            yield Static(
                                "No reading highlights loaded.",
                                id="reading-highlights-status",
                            )
                            yield Select(
                                [],
                    ...
                    with Container(id="metadata-edit", classes="edit-section hidden"):
                        with Vertical(classes="edit-fields"):
                            ...
                            yield Label(
                                "Keywords (comma-separated):", classes="edit-label"
                            )
                        ...
                        with Horizontal(classes="edit-actions"):
                            ...
                            yield Button(
                                "Cancel", id="cancel-button", variant="default"
                            )
            ...
            with TabPane("Content", id="content-tab"):
                ...
                with Container(classes="content-search"):
                    ...
                    yield Checkbox(
                        "Format for reading", id="format-reading-checkbox", value=False
                    )
            ...
            with TabPane("Analysis", id="analysis-tab"):
                ...
                with VerticalScroll(id="analysis-scroll-fix"):
                    ...
                    with Collapsible(
                        title="API Settings",
                        collapsed=False,
                        id="analysis-api-settings",
                        classes="compact-collapsible",
                    ):
                    ...
                    yield TextArea(
                        "", id="system-prompt-area", classes="prompt-textarea"
                    )

                    # User prompt
                    yield Label(
                        "User Prompt: (use {content} for specific placement, otherwise content is appended)",
                        classes="prompt-label",
                    )
                    yield TextArea("", id="user-prompt-area", classes="prompt-textarea")
    ...
    def update_metadata_display(self) -> None:
        ...
        try:
            ...
            # Title
            lines.append(
                f"[bold]Title:[/bold] {self.media_data.get('title', 'Untitled')}"
            )

            # Type
            lines.append(
                f"[bold]Type:[/bold] {self.media_data.get('media_type', 'Unknown')}"
            )
            ...
            if self.media_data.get("backing_media_id") not in (None, ""):
                ...
                if current_page is not None or total_pages is not None:
                    ...
                    if percent_complete is not None:
                        progress_text = (
                            f"{progress_text} ({float(percent_complete):.1f}%)"
                        )
    ...
    def populate_edit_fields(self) -> None:
        ...
        try:
            self.query_one("#edit-title", Input).value = self.media_data.get(
                "title", ""
            )
            self.query_one("#edit-author", Input).value = self.media_data.get(
                "author", ""
            )
    ...
    def _update_read_it_later_button(self) -> None:
        ...
        if supports_toggle:
            ...
            button.tooltip = (
                READ_IT_LATER_REMOVE_TOOLTIP if is_saved else READ_IT_LATER_SAVE_TOOLTIP
            )
        else:
    ...
    def load_analysis_versions(self, analyses: List[Dict[str, Any]]) -> None:
        ...
        try:
            self.all_analyses = [
                dict(version) for version in analyses if version.get("analysis_content")
            ]
            self.all_analyses.sort(
            ...
            if self.all_analyses:
                ...
                if self.media_data.get("analysis"):
                    ...
                    try:
                        self.query_one("#analysis-display", Markdown).update(
    ...
    def _display_analysis_at_index(self, index: int) -> None:
        ...
        if 0 <= index < len(self.all_analyses):
            ...
            try:
                ...
                logger.debug(
                    f"Displaying analysis at index {index}: {len(analysis_content)} chars"
                )
    ...
    def populate_providers(self) -> None:
        ...
        try:
            ...
            if providers_models:
                provider_options = [
                    (provider, provider) for provider in providers_models.keys()
                ]
                ...
                default_provider = analysis_defaults.get(
                    "provider", provider_options[0][0] if provider_options else None
                )
                if default_provider and any(
                    p[0] == default_provider for p in provider_options
                ):
    ...
    def prepare_analysis_messages(self) -> Tuple[str, str]:
        ...
        try:
            ...
            logger.debug(
                f"Raw prompts: system={len(system_prompt)} chars, user={len(user_prompt)} chars"
            )
            ...
            if self.media_data:
                ...
                if len(content) > 10000:
                    ...
                    logger.info(
                        f"Content truncated from {original_content_length} to {len(content)} characters"
                    )
    ...
    def _update_analysis_button_states(self) -> None:
        ...
        try:
            ...
            if self.analysis_edit_mode:
                ...
                save_btn.disabled = (
                    not self.current_analysis or self.has_existing_analysis
                )
                ...
                save_as_note_btn.tooltip = (
                    ANALYSIS_SAVE_NOTE_ENABLED_TOOLTIP
                    if self.current_analysis
                    else ANALYSIS_SAVE_NOTE_DISABLED_TOOLTIP
                )
                edit_btn.label = "Edit"
                edit_btn.disabled = not self.current_analysis
                edit_btn.tooltip = (
                    ANALYSIS_EDIT_ENABLED_TOOLTIP
                    if self.current_analysis
                    else ANALYSIS_EDIT_DISABLED_TOOLTIP
                )
                overwrite_btn.disabled = not self.has_existing_analysis
                overwrite_btn.tooltip = (
                    ANALYSIS_OVERWRITE_ENABLED_TOOLTIP
                    if self.has_existing_analysis
                    else ANALYSIS_OVERWRITE_DISABLED_TOOLTIP
                )
                ...
                if self.has_existing_analysis and len(self.all_analyses) > 0:
                    if 0 <= self.current_analysis_index < len(self.all_analyses):
                        current_analysis = self.all_analyses[
                            self.current_analysis_index
                        ]
                        # Only enable delete if it has a UUID or is unsaved
                        has_deletable_analysis = (
                            current_analysis.get("uuid") is not None
                            or current_analysis.get("version_number") == "unsaved"
                        )
                delete_btn.disabled = not has_deletable_analysis
                delete_btn.tooltip = (
                    ANALYSIS_DELETE_ENABLED_TOOLTIP
                    if has_deletable_analysis
                    else ANALYSIS_DELETE_DISABLED_TOOLTIP
                )
    ...
    def handle_generate_analysis(self) -> None:
        ...
        logger.debug(
            f"Media data available: id={self.media_data.get('id')}, "
        ...
        try:
            ...
            provider = (
                provider_select.value if provider_select.value != Select.BLANK else None
            )
            ...
            if not provider or not model:
                ...
                self.app_instance.notify(
                    "Please select a provider and model", severity="warning"
                )
                return
            ...
            logger.debug(
                f"Prompts prepared: system_prompt length={len(system_prompt or '')}, "
            ...
            if not system_prompt and not user_prompt:
                ...
                self.app_instance.notify(
                    "Please provide at least one prompt", severity="warning"
                )
                return
            ...
            try:
                temperature = float(
                    self.query_one("#analysis-temperature", Input).value or "0.7"
                )
            except ValueError:
                temperature = 0.7
            ...
            try:
                max_tokens = int(
                    self.query_one("#analysis-max-tokens", Input).value or "4096"
                )
            except ValueError:
                max_tokens = 4096
            ...
            logger.info(
                f"Posting MediaAnalysisRequestEvent for media_id={self.media_data['id']}"
            )
            logger.debug(
                f"Event parameters: provider={provider.lower()}, model={model}, "
    ...
    def handle_prev_analysis(self) -> None:
        ...
        logger.debug(
            f"Prev button pressed. Current index: {self.current_analysis_index}, Total: {len(self.all_analyses)}"
        )
        if self.current_analysis_index > 0:
            self.current_analysis_index -= 1
            logger.info(
                f"Navigating to previous analysis: index {self.current_analysis_index}"
            )
    ...
    def handle_next_analysis(self) -> None:
        ...
        logger.debug(
            f"Next button pressed. Current index: {self.current_analysis_index}, Total: {len(self.all_analyses)}"
        )
        if self.current_analysis_index < len(self.all_analyses) - 1:
            self.current_analysis_index += 1
            logger.info(
                f"Navigating to next analysis: index {self.current_analysis_index}"
            )
            self._display_analysis_at_index(self.current_analysis_index)
            self._update_analysis_navigation()
        else:
    ...
    def handle_edit_analysis(self) -> None:
        ...
        logger.info(
            f"Edit button pressed. Current analysis: {len(self.current_analysis) if self.current_analysis else 0} chars"
        )
