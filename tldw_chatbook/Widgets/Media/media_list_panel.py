...
class MediaListPanel(Container):
        ...
        try:
            ...
            if not self.items:
                await list_view.append(
                    ListItem(Static("No media items found", classes="no-results"))
                )
                return
            ...
            for index, item in enumerate(self.items):
                ...
                is_deleted = (
                    item.get("is_deleted", False) or item.get("deleted", 0) == 1
                )
    ...
    def load_items(
        self, items: List[Dict[str, Any]], page: int, total_pages: int
    ) -> None:
