...
class StatusDashboard(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="status-dashboard-container"):
            ...
            with Horizontal(classes="status-main-row"):
                # Status message
                yield Label(
                    self.status_message, id="status-message", classes="status-message"
                )
                ...
                if self.show_actions:
                    with Container(
                        id="action-buttons", classes="action-buttons hidden"
                    ):
                        yield Button(
                            "Cancel",
                            id="cancel-button",
    ...
    def _update_alert_messages(self) -> None:
        ...
        try:
            ...
            if self.warnings:
                ...
                messages.append(
                    f"[bold yellow]Warnings ({len(self.warnings)}):[/bold yellow]"
                )
    ...
    def start_processing(
        self, total_files: int = 0, message: str = "Processing..."
    ) -> None:
    ...
    def finish_processing(
        self, message: str = "Complete", success: bool = True
    ) -> None:
