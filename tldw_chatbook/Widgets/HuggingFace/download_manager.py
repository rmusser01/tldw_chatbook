...
class DownloadManager(Container):
    ...
    def _create_download_item(
        self, download_id: str, download: DownloadItem
    ) -> ListItem:
        ...
        header_widgets = [
            Static(f"{download.repo_id}/{download.filename}", classes="download-title"),
            Static(
                download.status.upper(), classes=f"download-status {download.status}"
            ),
        ]
    ...
    def add_download(self, repo_id: str, file_info: Dict[str, Any]) -> str:
        ...
        if self.download_queue is not None:
            try:
                ...
                logger.info(
                    f"Successfully queued download task: {download_id}, queue size after put: {queue_size}"
                )
        ...
        while not self._shutdown.is_set():
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Worker {worker_id} successfully got download task: {download_id}"
                    )
                except queue.Empty:
                ...
                logger.info(
                    f"Download {download_id} status changed to 'downloading', active downloads: {self.active_downloads}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Starting download: repo_id={download.repo_id}, filename={download.filename}, destination={download.destination}"
                    )
                    ...
                    self.active_downloads -= 1
                    logger.info(
                        f"Download {download_id} finished, active downloads: {self.active_downloads}"
                    )
        ...
        if hasattr(button, "download_id"):
            ...
            if "retry" in button.classes:
                ...
                if download_id in self.downloads:
                    ...
                    # Re-queue
                    file_info = {"path": download.filename, "size": download.size}
                    self.download_queue.put((download_id, file_info))
                    self.call_later(self._refresh_downloads_list)
