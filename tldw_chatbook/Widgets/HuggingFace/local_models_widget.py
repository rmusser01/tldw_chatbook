...
class ModelInfo:
    ...
    def __init__(self, path: Path):
        ...
        if parent.name != "tldw_models":
            self.repo_id = parent.name.replace(
                "_", "/", 1
            )  # Convert back first underscore
        else:
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="delete-confirm-dialog", id="delete-confirm-dialog"):
            with Vertical():
                ...
                yield Label(
                    "", id="delete-confirm-message", classes="delete-confirm-message"
                )
    ...
    async def _refresh_model_list(self) -> None:
        ...
        if not self.models:
            ...
            empty_container = Container(
                ...
                Label(
                    "Download models from the 'Download Models' tab",
        ...
        try:
            if model_path.exists():
                ...
                self.app.call_from_thread(
                    self.notify, f"Deleted: {model_path.name}", severity="information"
                )
            else:
                logger.warning(f"Model file not found: {model_path}")
                self.app.call_from_thread(
                    self.notify, "Model file not found", severity="warning"
                )
