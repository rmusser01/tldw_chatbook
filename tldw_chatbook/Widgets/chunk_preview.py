...
class ChunkPreview(Widget):
                ...
                for i, (chunk_text, start, end) in enumerate(
                    ...
                    with Container(classes="chunk-item"):
                        ...
                        with Horizontal(classes="chunk-header"):
                            ...
                            yield Static(
                                f"{len(chunk_text.split())} words", classes="chunk-size"
                            )
                        ...
                        if i > 0 and self.overlap_size > 0:
                            ...
                            if overlap_text:
                                yield Static(overlap_text, classes="chunk-overlap")
    ...
    def update_chunks(
        self, chunks: List[Tuple[str, int, int]], overlap_size: int = 0
    ) -> None:
    ...
    def _get_overlap_text(self, chunk_index: int, chunk_text: str) -> Optional[str]:
        ...
        overlap_chars = min(
            self.overlap_size * 5, len(chunk_text) // 4
        )  # Rough estimate
        if overlap_chars > 0:
