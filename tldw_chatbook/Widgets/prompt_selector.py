...
class PromptSelector(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield TextArea(
            id=self.system_prompt_id, classes="ingest-textarea-medium prompt-textarea"
        )
        ...
        yield TextArea(
            id=self.user_prompt_id, classes="ingest-textarea-medium prompt-textarea"
        )
    ...
    def load_available_prompts(self) -> None:
        ...
        try:
            ...
            if (
                not hasattr(self.app_instance, "prompts_db")
                or not self.app_instance.prompts_db
            ):
            ...
            try:
                if self.media_type == "general":
                    ...
                    all_prompts = self.app_instance.prompts_db.get_all_prompts(
                        limit=500
                    )
                    # Add keywords to each prompt
                    for prompt in all_prompts:
                        prompt["keywords"] = (
                            self.app_instance.prompts_db.fetch_keywords_for_prompt(
                    ...
                    media_prompts = (
                        self.app_instance.prompts_db.search_prompts_by_keyword(
                ...
                if all_prompts:
                    logger.info(
                        f"Loaded {len(all_prompts)} prompts for media type '{self.media_type}'"
                    )
                    self._load_prompts_from_db(all_prompts)
                else:
                    logger.warning(
                        f"No prompts found for media type '{self.media_type}', using defaults"
                    )
                    self._set_default_prompts()
    ...
    def _load_prompts_from_db(self, prompts: List[Dict[str, Any]]) -> None:
        ...
        system_prompts = system_prompts[:1] + sorted(
            system_prompts[1:], key=lambda x: x[0]
        )
        ...
        try:
            ...
            logger.debug(
                f"Set {len(system_prompts)} system prompt options for {self.media_type}"
            )
        except Exception as e:
        ...
        try:
            ...
            logger.debug(
                f"Set {len(user_prompts)} user prompt options for {self.media_type}"
            )
        except Exception as e:
        ...
        if event.value == "custom":
            ...
            if prompt_text:
                ...
                if other_text and prompt_type == "user":
                    ...
                    logger.warning(
                        f"No {prompt_type} prompt text found for: {event.value}"
                    )
