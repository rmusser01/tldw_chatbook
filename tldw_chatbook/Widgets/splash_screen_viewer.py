...
class SplashScreenViewer(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(
            "Select a splash screen below to preview it. Double-click to play.",
            classes="help-text",
        )
        ...
        for card_name, card_info in self.splash_cards.items():
            ...
            if card_info.title.lower() in [
                "tldw chatbook",
                "tldw",
                "loading tldw chatbook",
            ]:
        ...
        yield Static(
            "Select a card to see details", id="card-info", classes="card-info"
        )
...
[dim]Double-click or press "Play Selected" to view this splash screen[/dim]"""
    ...
    async def _play_card_worker(self, card_id: str) -> None:
        ...
        self.app.notify(
            f"Playing '{card_info.title}' splash screen...", severity="information"
        )

        # Create splash screen with specific card
        splash = SplashScreen(card_name=card_id, duration=duration, show_progress=True)
    ...
    def on_set_default_button(self, event: Button.Pressed) -> None:
        ...
        try:
            ...
            self.app.notify(
                f"Set '{card_info.title}' as default splash screen", severity="success"
            )
        except Exception as e:
    ...
    def on_duration_changed(self, event: Input.Changed) -> None:
        ...
        try:
            ...
            if value < 0.5:
                ...
                self.app.notify(
                    "Duration must be at least 0.5 seconds", severity="warning"
                )
            elif value > 30:
                event.input.add_class("error")
