...
class CharacterVoiceWidget(Widget):
        ...
        with Container(classes="voice-widget-container"):
            ...
            with Vertical(classes="character-list-section"):
                ...
                with Vertical(classes="detection-controls"):
                    with Horizontal(classes="form-row"):
                        yield Switch(id="auto-detect-switch", value=True)
                    ...
                    yield Button(
                        "➕ Add Character", id="add-character-btn", variant="default"
                    )
                ...
                character_table.add_columns("Character", "Lines", "Voice")
                yield character_table

                # Quick actions
                with Horizontal(classes="form-row"):
                    yield Button(
                        "🗑️ Remove", id="remove-character-btn", variant="warning"
                    )
                    yield Button(
                        "🔄 Reset All", id="reset-voices-btn", variant="warning"
                    )
            ...
            with Vertical(classes="voice-assignment-section"):
                ...
                with Collapsible(title="Character Details", collapsed=False):
                    yield Label(
                        "", id="selected-character-name", classes="character-name-label"
                    )
                ...
                with Collapsible(title="Bulk Assignment", collapsed=True):
                    ...
                    yield Select(options=[], id="bulk-voice-select")
                    with Horizontal(classes="form-row"):
                        yield Button(
                            "Apply to All", id="apply-all-btn", variant="default"
                        )
                        yield Button(
                            "Apply to Selected",
    ...
    def _refresh_character_table(self) -> None:
        ...
        for i, character in enumerate(self.characters):
            ...
            table.add_row(character.name, line_count, voice_label, key=str(i))
    ...
    def _update_assignment_ui(self) -> None:
        ...
        if 0 <= self.selected_character_index < len(self.characters):
            ...
            try:
                ...
                sample_text.text = (
                    f'"{character.name} speaking: {sample_text.text.strip()}"'
                )
            except Exception as e:
    ...
    def _preview_character_voice(self) -> None:
        ...
        self.app.notify(
            f"Preview: {character.name} with voice {self._get_voice_label(voice_id)}",
    ...
    def _save_voice_assignment(self) -> None:
        ...
        self.app.notify(
            f"Voice assigned: {character.name} → {self._get_voice_label(voice_id)}",
            severity="success",
        )
    ...
    def _apply_voice_to_all(self) -> None:
        ...
        self._update_assignment_summary()
        self.app.notify(
            f"Applied {self._get_voice_label(voice_id)} to all characters",
            severity="success",
        )
    ...
    def detect_characters_from_text(
        self, text: str, auto_assign: bool = False
    ) -> List[Character]:
