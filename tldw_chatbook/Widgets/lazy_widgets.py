...
class LazyCollapsible(Collapsible):
    ...
    def compose(self) -> ComposeResult:
        ...
        if self.collapsed and self._content_factory:
            ...
            self._loading_message = Static(
                "Content will load when expanded...", classes="lazy-placeholder"
            )
    ...
    def _load_content(self) -> ComposeResult:
        ...
        if self._content_factory:
            ...
            try:
                ...
                logger.error(
                    f"Error loading content for LazyCollapsible '{self.title}': {e}"
                )
    ...
    async def _async_load_content(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error async loading content for LazyCollapsible '{self.title}': {e}"
            )
            await self.mount(
                Static(f"Error loading content: {e}", classes="error-message")
            )
...
class VirtualListView(ListView):
    ...
    def compose(self) -> ComposeResult:
        ...
        if self._items and self._item_factory:
            ...
            logger.debug(
                f"VirtualListView rendered {end_idx} of {len(self._items)} items"
            )
