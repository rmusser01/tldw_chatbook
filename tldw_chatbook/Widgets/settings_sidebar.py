...
def create_settings_sidebar(id_prefix: str, config: dict) -> ComposeResult:
    ...
    with VerticalScroll(id=sidebar_id, classes="sidebar"):
        ...
        with Horizontal(classes="quick-actions-bar"):
            yield Button(
                "➕",
                id=f"{id_prefix}-expand-all",
                classes="quick-action-btn",
                tooltip="Expand all sections",
            )
            yield Button(
                "➖",
                id=f"{id_prefix}-collapse-all",
                classes="quick-action-btn",
                tooltip="Collapse all sections",
            )
            yield Button(
                "🔄",
                id=f"{id_prefix}-reset-settings",
                classes="quick-action-btn",
        ...
        with Container(classes="settings-group primary-group"):
            ...
            with Collapsible(
                ...
                collapsed=False,
                id=f"{id_prefix}-quick-settings",
                classes="settings-collapsible priority-high basic-mode advanced-mode",
            ):
            ...
            initial_strip_value = config.get("chat_defaults", {}).get(
        ...
        with Container(classes="settings-group secondary-group"):
            ...
            if id_prefix == "chat":
                with Collapsible(
                    ...
                    collapsed=True,
                    id=f"{id_prefix}-image-generation-collapsible",
                    classes="settings-collapsible basic-mode advanced-mode",
                ):
            ...
            with Collapsible(
                ...
                classes="settings-collapsible rag-settings-panel basic-mode advanced-mode",
            ):
                # Main RAG toggle
                yield Checkbox(
                    "Enable RAG",
            ...
            if PIPELINE_INTEGRATION_AVAILABLE:
                try:
                    ...
                    if custom_pipelines:
                        ...
                        pipeline_options.extend(
                            sorted(custom_pipelines, key=lambda x: x[0])
                        )
            ...
            with Container(classes="rag-scope-options"):
                yield Checkbox(
                    "Media Items",
                    id=f"{id_prefix}-rag-search-media-checkbox",
                    value=True,
                )
                yield Checkbox(
                    "Conversations",
                    id=f"{id_prefix}-rag-search-conversations-checkbox",
                    value=False,
                )
                yield Checkbox(
                    "Notes", id=f"{id_prefix}-rag-search-notes-checkbox", value=False
                )
            ...
            yield Static(
                "Expansion Provider",
                classes="sidebar-label rag-expansion-provider-label",
            )
            ...
            yield Static(
                "Expansion Model", classes="sidebar-label rag-expansion-llm-label"
            )
            ...
            yield Static(
                "Llamafile Model",
                classes="sidebar-label rag-expansion-local-label hidden",
            )
            yield Input(
            ...
            yield Static("", classes="sidebar-separator")
            yield Static(
                "Advanced RAG Settings", classes="sidebar-label sidebar-section-header"
            )
        ...
        if id_prefix == "chat":
            with Collapsible(
                title="Prompts",
                collapsed=True,
                id=f"{id_prefix}-prompts-collapsible",
                classes="settings-collapsible basic-mode advanced-mode",
            ):
        ...
        with Container(classes="settings-group advanced-group"):
            ...
            yield Input(
                id=f"{id_prefix}-custom-token-limit",
                value="12888",
            ...
            yield Checkbox(
                "Fixed Tokens (Kobold)",
                id=f"{id_prefix}-llm-fixed-tokens-kobold",
                value=False,
            )
            ...
            yield Input(
                id=f"{id_prefix}-llm-stop",
                placeholder="e.g., <|endoftext|>,<|eot_id|>",
                classes="sidebar-input",
            )
            yield Static("Response Format", classes="sidebar-label")
            yield Select(
                options=[("text", "text"), ("json_object", "json_object")],
                id=f"{id_prefix}-llm-response-format",
                value="text",
                allow_blank=False,
            )
        ...
        with Collapsible(
            ...
            if id_prefix == "chat":
                ...
                character_search_results_list = ListView(
                    id="chat-character-search-results-list", classes="sidebar-listview"
                )
                ...
                description_edit_ta = TextArea(
                    id="chat-character-description-edit", classes="sidebar-textarea"
                )
                description_edit_ta.styles.height = 30
                yield description_edit_ta

                yield Label("Personality:", classes="sidebar-label")
                personality_edit_ta = TextArea(
                    id="chat-character-personality-edit", classes="sidebar-textarea"
                )
                personality_edit_ta.styles.height = 30
                yield personality_edit_ta

                yield Label("Scenario:", classes="sidebar-label")
                scenario_edit_ta = TextArea(
                    id="chat-character-scenario-edit", classes="sidebar-textarea"
                )
                scenario_edit_ta.styles.height = 30
                yield scenario_edit_ta

                yield Label("System Prompt:", classes="sidebar-label")
                system_prompt_edit_ta = TextArea(
                    id="chat-character-system-prompt-edit", classes="sidebar-textarea"
                )
                system_prompt_edit_ta.styles.height = 30
                yield system_prompt_edit_ta

                yield Label("First Message:", classes="sidebar-label")
                first_message_edit_ta = TextArea(
                    id="chat-character-first-message-edit", classes="sidebar-textarea"
                )
        ...
        with Collapsible(
            ...
            yield Input(
                id=f"{id_prefix}-llm-top-logprobs",
                value="0",
                placeholder="e.g., 5",
                classes="sidebar-input",
            )
            ...
            yield TextArea(
                id=f"{id_prefix}-llm-logit-bias", text="{}", classes="sidebar-textarea"
            )
            yield Static("Presence Penalty", classes="sidebar-label")
            yield Input(
                id=f"{id_prefix}-llm-presence-penalty",
                value="0.0",
            ...
            yield Input(
                id=f"{id_prefix}-llm-frequency-penalty",
                value="0.0",
                placeholder="e.g., 0.0 to 2.0",
                classes="sidebar-input",
            )
        ...
        with Collapsible(
            ...
            collapsed=True,
            id=f"{id_prefix}-tools",
            classes="settings-collapsible advanced-mode advanced-only",
        ):
            yield Static("Tool Usage", classes="sidebar-label")
            yield TextArea(
                id=f"{id_prefix}-llm-tools", text="[]", classes="sidebar-textarea"
            )
            yield Static("Tool Choice", classes="sidebar-label")
            yield Input(
                id=f"{id_prefix}-llm-tool-choice",
                placeholder="e.g., auto, none, or specific tool",
                classes="sidebar-input",
            )
            ...
            template_list_view = ListView(
                id=f"{id_prefix}-template-list-view", classes="sidebar-listview"
            )
