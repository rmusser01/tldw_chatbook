...
class ConsoleAppearancePickerModal(
    ...
    def _apply_pressed(self, event: Button.Pressed) -> None:
        ...
        self.dismiss(
            ConsoleConversationAppearance(icon=icon, color=self._selected_color)
        )
    ...
    def _sync_preview(self) -> None:
        ...
        if color:
            preview.update(f"{icon} {escape_markup(title)}  [{color}]{color}[/]")
        else:
            preview.update(f"{icon} {escape_markup(title)}  no color")
