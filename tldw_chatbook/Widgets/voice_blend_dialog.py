...
class VoiceBlendDialog(ModalScreen[Optional[Dict[str, Any]]]):
    ...
    def __init__(
        self, blend_data: Optional[Dict[str, Any]] = None, name: str = "", **kwargs
    ) -> None:
        ...
        with Container(id="voice-blend-container"):
            ...
            with Vertical():
                with Horizontal(classes="form-row"):
                    ...
                    yield Input(
                        value=self.blend_data.get("name", "")
                        if self.blend_data
                        else "",
                ...
                with Horizontal(classes="form-row"):
                    ...
                    yield Input(
                        value=self.blend_data.get("description", "")
                        if self.blend_data
                        else "",
    ...
    async def on_voice_blend_entry_removed(
        self, message: VoiceBlendEntry.Removed
    ) -> None:
        ...
        try:
            ...
            for entry in self.voice_entries:
                ...
                try:
                    ...
                    if weight <= 0:
                        self.app.notify(
                            "All weights must be positive", severity="error"
                        )
                        return
