...
class ActivityLogWidget(Widget):
            ...
            with Horizontal(classes="activity-log-header"):
                ...
                if self.show_actions:
                    with Horizontal(classes="activity-log-actions"):
                        yield Button(
                            "Clear Log", id="clear-log", classes="small-button"
                        )
                        if self.show_export:
                            yield Button(
                                "Export", id="export-log", classes="small-button"
                            )
                        yield Button(
                            "⬇ Auto-scroll"
                            if self.auto_scroll
    ...
    def _should_display_entry(self, entry: ActivityEntry) -> bool:
        ...
        if self.search_query:
            ...
            if (
                query not in entry.message.lower()
                and query not in entry.category.lower()
            ):
                return False
    ...
    def _trim_display_entries(self) -> None:
        ...
        visible_count = (
            len(self._get_filtered_entries())
            if self._has_active_filters()
    ...
    def export_log(
        self, target: Optional[str] = None, filtered_only: bool = False
    ) -> None:
    ...
    def _resolve_export_target(self, target: Optional[str]) -> tuple[str, str]:
        ...
        if normalized_target in {"json", "csv", "text", "txt"}:
            export_format = (
                "text" if normalized_target in {"text", "txt"} else normalized_target
            )
