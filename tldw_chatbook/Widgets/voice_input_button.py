...
class VoiceInputButton(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container():
            yield Button("🎤 Voice", id="voice-input-btn", classes="voice-button")
        ...
        try:
            ...
            if self._dictation_service is None:
                self._dictation_service = LazyLiveDictationService(
                    language=self.app.app_config.get("dictation", {}).get(
                        "language", "en"
                    ),
    ...
    def _handle_result(self, text: str):
        ...
        self.post_message(
            VoiceInputResponseEvent(source_widget_id=self.id, text=text, success=True)
        )
        ...
        if self.auto_insert:
            if self.target_widget_id:
                ...
                self.post_message(
                    InsertDictationTextEvent(text=text, append_space=True)
                )
            else:
...
class FloatingVoiceInput(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="floating-container"):
            ...
            yield VoiceInputButton(show_status=True, on_result=self._on_voice_result)
            yield Static(
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "insert-btn":
            if self.transcript_preview:
                self.app.post_message(
                    InsertDictationTextEvent(
                        text=self.transcript_preview, append_space=True
                    )
                )
            self.remove()
