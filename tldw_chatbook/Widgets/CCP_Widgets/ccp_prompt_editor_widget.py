...
class CCPPromptEditorWidget(Container):
    ...
    def __init__(self, parent_screen: Optional[Any] = None, **kwargs):
        ...
        """
        super().__init__(
            id="ccp-prompt-editor-view", classes="ccp-view-area hidden", **kwargs
        )
    ...
    def compose(self) -> ComposeResult:
            ...
            with Container(id="prompt-editor-container", classes="hidden"):
                ...
                with Container(classes="prompt-section"):
                    ...
                    with Container(classes="field-container"):
                        ...
                        yield Select(
                            options=[
                                (label, value) for value, label in self.CATEGORIES
                            ],
                ...
                with Container(classes="prompt-section"):
                    ...
                    with Container(classes="field-container"):
                        yield Label(
                            "Prompt Text (use {{variable}} for variables):",
                            classes="field-label",
                        )
                        yield TextArea(
                            "", id="ccp-prompt-content", classes="prompt-textarea large"
                        )
                ...
                with Container(classes="prompt-section"):
                    ...
                    # Variables list
                    with Container(
                        classes="variables-container", id="ccp-variables-list"
                    ):
                        yield Static(
                            "No variables defined", classes="no-variables-placeholder"
                        )
                ...
                with Container(classes="prompt-section"):
                    ...
                    yield Static(
                        "Usage: -",
                        id="ccp-editor-prompt-usage-display",
                        classes="field-label",
                    )
                    with Container(classes="add-variable-container"):
                        yield Input(
                    ...
                    yield Static(
                        "", id="ccp-editor-prompt-version-status", classes="field-label"
                    )
                ...
                with Container(classes="prompt-section test-section"):
                    ...
                    with Container(
                        id="ccp-test-inputs-container", classes="test-input-container"
                    ):
                        yield Static("Define variables first to test the prompt")

                    # Test button
                    yield Button(
                        "Test Prompt", id="test-prompt-btn", classes="test-button"
                    )
        ...
        with Container(classes="prompt-actions"):
            yield Button(
                "Save Prompt",
                classes="prompt-action-button primary",
                id="save-prompt-btn",
            )
            yield Button(
                "Delete", classes="prompt-action-button danger", id="delete-prompt-btn"
            )
            yield Button("Reset", classes="prompt-action-button", id="reset-prompt-btn")
            yield Button(
                "Cancel", classes="prompt-action-button cancel", id="cancel-prompt-btn"
            )
    ...
    def load_prompt(self, prompt_data: Dict[str, Any]) -> None:
        ...
        if self._prompt_area:
            self._prompt_area.text = prompt_data.get(
        ...
        try:
            ...
            if self.variables:
                for i, var in enumerate(self.variables):
                    ...
                    name_widget = Static(
                        f"{{{{ {var['name']} }}}}", classes="variable-name"
                    )
                    var_container.mount(name_widget)

                    type_widget = Static(
                        f"({var.get('type', 'text')})", classes="variable-type"
                    )
                    var_container.mount(type_widget)

                    remove_btn = Button(
                        "Remove",
                        classes="variable-remove-btn remove-var-btn",
                        id=f"remove-var-{i}",
                    )
            ...
            if prompt_text:
                ...
                for var in self.variables:
                    ...
                    prompt_text = prompt_text.replace(
                        var_placeholder, f"[bold cyan]{var_placeholder}[/bold cyan]"
                    )
