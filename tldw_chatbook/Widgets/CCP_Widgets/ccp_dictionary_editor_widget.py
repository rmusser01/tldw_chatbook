...
class CCPDictionaryEditorWidget(Container):
    ...
    def __init__(self, parent_screen: Optional[Any] = None, **kwargs):
        ...
        """
        super().__init__(
            id="ccp-dictionary-editor-view", classes="ccp-view-area hidden", **kwargs
        )
    ...
    def compose(self) -> ComposeResult:
        ...
        # Header
        yield Static(
            "Dictionary/World Book Editor",
            classes="dictionary-editor-header pane-title",
        )
            ...
            with Container(id="dictionary-editor-container", classes="hidden"):
                ...
                with Container(classes="dictionary-section"):
                    ...
                    with Container(classes="search-container"):
                        ...
                        yield Button(
                            "Clear", id="clear-search-btn", classes="search-button"
                        )
                    ...
                    with Container(classes="entry-editor-container"):
                        ...
                        yield TextArea(
                            "", id="ccp-entry-value", classes="entry-value-textarea"
                        )
                        ...
                        with Container(classes="entry-actions"):
                            yield Button(
                                "Add Entry",
                                id="add-entry-btn",
                                classes="entry-action-button add",
                            )
                            yield Button(
                                "Update Entry",
                                id="update-entry-btn",
                                classes="entry-action-button update",
                            )
                            yield Button(
                                "Remove Entry",
                                id="remove-entry-btn",
                                classes="entry-action-button remove",
                            )
                ...
                with Container(classes="dictionary-section"):
                    ...
                    with Container(classes="stats-container"):
                        ...
                        with Container(classes="stats-row"):
                            ...
                            yield Static(
                                "Never", id="stats-modified", classes="stats-value"
                            )
        ...
        with Container(classes="dictionary-actions"):
            yield Button(
                "Save Dictionary",
                classes="dictionary-action-button primary",
                id="save-dictionary-btn",
            )
            yield Button(
                "Delete",
                classes="dictionary-action-button danger",
                id="delete-dictionary-btn",
            )
            yield Button(
                "Reset", classes="dictionary-action-button", id="reset-dictionary-btn"
            )
            yield Button(
                "Cancel",
                classes="dictionary-action-button cancel",
                id="cancel-dictionary-btn",
            )
    ...
    def _cache_field_references(self) -> None:
        ...
        try:
            ...
            self._description_area = self.query_one(
                "#ccp-dictionary-description", TextArea
            )
            self._active_toggle = self.query_one(
                "#ccp-dictionary-active-toggle", Switch
            )
    ...
    def load_dictionary(self, dictionary_data: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"Loaded dictionary for editing: {dictionary_data.get('name', 'Unknown')}"
        )
    ...
    def _update_entries_table(self, filter_text: str = "") -> None:
        ...
        if entries_to_show:
            ...
            if not self.entries:
                self._entries_table.add_row(
                    "No entries", "Add entries using the editor below"
                )
            else:
