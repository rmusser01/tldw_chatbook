...
DELETE_KEYWORDS_DISABLED_TOOLTIP = (
...
class KeywordMergeDialog(ModalScreen):
    ...
    def __init__(
        self, source_keywords: List[Dict[str, Any]], all_keywords: List[Dict[str, Any]]
    ) -> None:
        super().__init__()
    ...
    def confirm_merge(self) -> None:
        ...
        list_view = self.query_one("#target-keyword-list", ListView)
        if list_view.highlighted_child and hasattr(
            list_view.highlighted_child, "keyword_data"
        ):
    ...
    def load_keywords(self) -> None:
        ...
        try:
            ...
            self.app_instance.notify(
                f"Error loading keywords: {str(e)}", severity="error"
            )
        ...
        if hasattr(event.item, "keyword_data"):
            ...
            if any(k["id"] == keyword["id"] for k in self.selected_keywords):
                self.selected_keywords = [
                    k for k in self.selected_keywords if k["id"] != keyword["id"]
                ]
...
"""
            ...
            if self.app_instance.media_db:
                try:
                    media_query = """
                        ...
                        LIMIT 10
                    """
                    media_items = self.app_instance.media_db.execute_query(
                        media_query, (keyword["id"],)
                    )
            ...
            stats.update(
                f"**Total usage across selected keywords:** {total_usage} items"
            )
        ...
        class DeleteConfirmationModal(ModalScreen):
            ...
            def compose(self) -> ComposeResult:
                with Vertical(id="delete-dialog"):
                    ...
                    yield Label(
                        f"Delete {self.keyword_count} keyword{'s' if self.keyword_count > 1 else ''}?",
                        classes="dialog-title",
                    )
                    yield Label(f"Keywords: {keyword_names}", classes="dialog-subtitle")
                    yield Label(
                        "This will remove the keywords from all media items.",
    ...
    async def merge_keywords(
        self, source_keywords: List[Dict[str, Any]], merge_type: str, target: Any
    ) -> None:
        ...
        if merge_type == "new":
            ...
            self.post_message(
                KeywordMergeEvent(source_ids, target, create_if_not_exists=True)
            )
        else:
            # Merge into existing keyword
            self.post_message(
                KeywordMergeEvent(
                    source_ids, target["keyword"], create_if_not_exists=False
                )
            )
