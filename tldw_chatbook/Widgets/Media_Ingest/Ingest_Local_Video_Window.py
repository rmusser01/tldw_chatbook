# tldw_chatbook/Widgets/Media_Ingest/VideoIngestWindowRedesigned.py
"""
Redesigned video ingestion window implementing the new architecture patterns.
This serves as the template for other media ingestion windows.
"""
...
class VideoFormData(MediaFormData):
    """Video-specific form data model."""
    # Video processing options
    extract_audio_only: bool = Field(True, description="Extract audio only (faster)")
    download_video: bool = Field(False, description="Download full video file")
    ...
    enable_vad: bool = Field(False, description="Enable voice activity detection")
    ...
    user_prompt: Optional[str] = Field(None, description="User prompt for analysis")
...
class VideoIngestWindowRedesigned(BaseMediaIngestWindow):
    """
    Redesigned video ingestion window with proper architecture.
    ...
    def __init__(self, app_instance: 'TldwCli', **kwargs):
        ...
        # Load defaults
        self.video_defaults = get_media_ingestion_defaults("video")
        
        logger.debug("[Video] VideoIngestWindowRedesigned initialized")
    ...
    def get_file_filters(self) -> List[Tuple[str, str]]:
        """Get video file filters for the file browser."""
        return [
    ...
    def create_media_specific_options(self) -> ComposeResult:
        """Create video-specific processing options."""
        
        # Video Processing Options
        with Container(id="video-processing-options", classes="options-section"):
            yield Static("Video Processing Options", classes="section-header")
            
            # Basic video options
            yield Checkbox(
                "Extract Audio Only (faster, recommended)",
                value=self.video_defaults.get("extract_audio_only", True),
                id="extract-audio-only",
                classes="form-checkbox"
            )
            
            yield Checkbox(
                "Download Full Video (if URL provided)",
                value=self.video_defaults.get("download_video", False),
                id="download-video",
                classes="form-checkbox"
            )
    ...
    def create_transcription_options(self) -> ComposeResult:
        ...
        if self._available_providers:
            default_provider = self.video_defaults.get("transcription_provider", "faster-whisper")
        ...
        yield Label("Advanced Transcription Options:", classes="form-label")
        yield Checkbox(
    ...
    def create_analysis_options(self) -> ComposeResult:
        ...
        yield Label("User Prompt for Analysis:", classes="form-label")
        yield TextArea(
            text="# Enter custom user prompt for analysis (optional)...",
            id="user-prompt",
            classes="form-textarea"
        )
        
        yield Label("System Prompt for Analysis:", classes="form-label")
        yield TextArea(
            text="# Enter custom system prompt for analysis (optional)...",
            id="system-prompt", 
            classes="form-textarea"
        )
    ...
    async def process_media(self, validated_data: dict) -> AsyncIterator[ProcessingStatus]:
        """Process video files with status updates."""
        try:
            ...
            for file_path in files:
                yield ProcessingStatus(
                    ...
                    message=f"Processing file: {file_path.name}"
                )
            ...
            for url in urls:
                yield ProcessingStatus(
                    ...
                    files_processed=processed,
                    total_files=total_items,
                    message=f"Processing URL: {url[:50]}..."
                )
            ...
            yield ProcessingStatus(
                ...
                files_processed=processed,
                total_files=total_items,
                message=f"Successfully processed {processed} items!"
            )
    ...
    # Event handlers for video-specific controls
    ...
    def handle_provider_change(self, event):
        ...
        try:
            all_models = self.transcription_service.list_available_models(provider)
            ...
            logger.error(f"[Video] Error loading models for {provider}: {e}")
    ...
    def update_analysis_models(self, provider: str):
        ...
        try:
            ...
            logger.error(f"[Video] Error loading analysis models for {provider}: {e}")
    ...
    def watch_simple_mode(self, simple: bool):
        """Handle simple/advanced mode changes for video."""
        ...
        try:
            ...
            logger.error(f"[Video] Error updating mode display: {e}")
    
    def on_mount(self):
        """Initialize video-specific settings after mount."""
    ...
    def update_transcription_models(self, provider: str):
        ...
        try:
            ...
            # Set default model
            default_model = self.video_defaults.get("transcription_model", "base")
            ...
            logger.error(f"[Video] Error updating transcription models: {e}")
    
    # Extended CSS for video-specific styling
    ...
    /* Note: Textual doesn't support @media queries
       Responsive behavior is handled programmatically in Python code */
    """