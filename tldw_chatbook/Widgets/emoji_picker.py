...
def _load_emojis() -> Tuple[
    List[ProcessedEmoji], Dict[str, List[ProcessedEmoji]], List[str]
]:
    ...
    if EMOJI_SOURCE_TYPE == "unicode_codes.EMOJI_DATA":
            ...
            try:
                ...
                if 0x1F600 <= char_code <= 0x1F64F:  # Emoticons
                    ...
                    if any(
                        ...
                        word in name_lower
                        for word in ["food", "fruit", "vegetable", "meal", "drink"]
                    ):
                        category = "Food & Drink"
                    else:
                        category = "Objects"
        ...
        for (
            char,
            data_val,
        ) in EMOJI_METADATA.items():  # Renamed data to data_val to avoid conflict
    ...
    sorted_category_names = sorted(
        ...
        key=lambda c: (
            PREFERRED_CATEGORY_ORDER.index(c)
            if c in PREFERRED_CATEGORY_ORDER
            else float("inf"),
            c,
        ),
    )
    ...
    for cat_emojis in categorized_emojis.values():
        cat_emojis.sort(
            key=lambda e: e.get("sort_order", e["name"])
        )  # Use name if sort_order absent
...
class EmojiSelected(Message):
    ...
    def __init__(self, emoji: str, picker_id: Optional[str] = None) -> None:
        ...
        self.picker_id: Optional[str] = (
            picker_id  # Optional: if we need to identify the source picker
        )
...
class EmojiGrid(VerticalScroll):
    ...
    def populate_grid(
        self, emojis_to_display: Optional[List[ProcessedEmoji]] = None
    ) -> None:
        for child in self.query("Horizontal, EmojiButton, Static.no_emojis_message"):
            child.remove()

        current_emojis = (
            emojis_to_display if emojis_to_display is not None else self.emojis
        )
...
class EmojiPickerScreen(ModalScreen[str]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="dialog"):
            ...
            yield Input(
                placeholder="Search emojis (e.g., smile, cat, :thumbsup:)",
                id="search-input",
            )
    ...
    def on_mount(self) -> None:
        ...
        self.query_one(
            "#search-results-grid", EmojiGrid
        ).display = False  # Ensure it starts hidden
    ...
    def _filter_emojis(self, query: str) -> List[ProcessedEmoji]:
        ...
        for emoji_data in self._all_emojis:
            if (
                query in emoji_data["name"].lower()
                or any(query in alias.lower() for alias in emoji_data["aliases"])
                or (len(query) == 1 and query == emoji_data["char"])
            ):
            ...
            if tab_content:
                ...
                if active_pane_id:
                    try:
                        ...
                        active_pane = tab_content.query_one(
                            f"#{active_pane_id}", TabPane
                        )
    ...
    def action_dismiss_picker(
        self,
    ) -> None:  # This is the action method bound to "escape"
