...
# Local imports
from ..Utils.embedding_templates import (
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="embedding-template-card-container"):
            ...
            with Horizontal(classes="embedding-template-card-header"):
                yield Static(
                    self.template.name, classes="embedding-template-card-title"
                )
                yield Static(
                    self.template.category.value.replace("_", " ").title(),
                    classes="embedding-template-card-category",
                )

            # Description
            yield Static(
                self.template.description, classes="embedding-template-card-description"
            )
            ...
            if self.template.recommended_for:
                ...
                with Container(classes="embedding-template-card-list"):
                    for item in self.template.recommended_for[:3]:
                        yield Static(
                            f"• {item}", classes="embedding-template-card-item"
                        )
...
class EmbeddingTemplateSelectorDialog(ModalScreen):
    ...
    selected_category: reactive[TemplateCategory] = reactive(
        TemplateCategory.QUICK_START
    )
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="embedding-template-selector-container"):
            yield Label(
                "Select Embedding Configuration Template",
                id="embedding-template-selector-title",
            )
            ...
            with Horizontal(id="embedding-template-selector-content"):
                ...
                with Container(id="embedding-template-grid-container"):
                    yield Grid(
                        id="embedding-template-grid", classes="embedding-template-grid"
                    )
...
class EmbeddingTemplateQuickSelect(Widget):
    ...
    def __init__(
        self, template_manager: Optional[EmbeddingTemplateManager] = None, **kwargs
    ):
        super().__init__(**kwargs)
        self.template_manager = template_manager or EmbeddingTemplateManager()
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(classes="embedding-template-quick-select-container"):
            ...
            yield Button(
                "Quick Start",
                id="template-quick_local",
                classes="embedding-template-quick-button",
            )
            yield Button(
                "High Quality",
                id="template-high_quality_local",
                classes="embedding-template-quick-button",
            )
            yield Button(
                "Performance",
                id="template-balanced_performance",
                classes="embedding-template-quick-button",
            )
            ...
            yield Button(
                "Browse All...",
                id="template-browse",
                classes="embedding-template-browse-button",
                variant="primary",
            )

            # Current template indicator
            yield Static(
                "", id="current-template-name", classes="current-template-name"
            )
