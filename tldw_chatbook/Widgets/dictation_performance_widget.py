...
class DictationPerformanceWidget(Widget):
    ...
    async def refresh_metrics(self):
        ...
        # Update UI
        self.metrics_data = {"summary": summary, "comparison": comparison}
    ...
    def watch_metrics_data(self, old_data: Dict, new_data: Dict):
        ...
        self._apply_performance_class(
            latency_widget, avg_latency, 200, 100, inverse=True
        )
    ...
    def _apply_performance_class(
        ...
        widget.remove_class(
            "performance-good", "performance-warning", "performance-poor"
        )
        ...
        for provider, stats in comparison.items():
            with container:
                with Horizontal(classes="comparison-row"):
                    # Provider name
                    container.mount(Label(provider.title(), classes="provider-name"))
