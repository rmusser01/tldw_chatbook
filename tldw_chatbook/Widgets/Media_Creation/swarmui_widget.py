...
class SwarmUIWidget(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="swarmui-widget"):
            ...
            yield TextArea(id="prompt-input", classes="swarmui-textarea")
        ...
        if event.select.id == "template-select":
            ...
            if template_id != "custom":
                ...
                if template:
                    ...
                    neg_prompt_input = self.query_one(
                    ...
                    if (
                        "width" in template.default_params
                        and "height" in template.default_params
                    ):
                    ...
                    if "steps" in template.default_params:
                        self.query_one("#steps-input", Input).value = str(
                            template.default_params["steps"]
                        )
                    if "cfg_scale" in template.default_params:
                        self.query_one("#cfg-input", Input).value = str(
                            template.default_params["cfg_scale"]
                        )
    ...
    def use_last_message(self) -> None:
        ...
        self.show_status_message(
            "Feature coming soon: Will use last chat message", "info"
        )
    ...
    async def generate_image(self) -> None:
        ...
        try:
            ...
            negative_prompt = self.query_one(
                "#negative-prompt-input", TextArea
            ).text.strip()
    ...
    async def save_current_image(self) -> None:
        ...
        if self.last_result and self.last_result.images:
            ...
            if saved_paths:
                self.show_status_message(
                    f"Saved to: {Path(saved_paths[0]).name}", "success"
                )
            else:
