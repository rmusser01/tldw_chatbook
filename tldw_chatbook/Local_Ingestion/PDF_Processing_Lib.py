...
def process_pdf(
    ...
    try:
        ...
        try:
            logger.info(f"Attempting metadata extraction for {filename}.")
    ...
    # --- Catch PDF library errors that indicate fundamental file issues (but weren't caught during specific steps) ---
        ...
        # --- MODIFICATION END ---
        # Check the message specifically for password errors if needed for logger differentiation
        err_msg = str(pdf_lib_err)
        # Distinguish error types for logger and user messages
...
async def process_pdf_task(
    ...
    *No DB interaction.*
    """
    try:
        logger.info(f"process_pdf_task started for {filename} using {parser}")
