...
def xml_to_text(xml_file):
    ...
    try:
        ...
        log_histogram(
            "xml_ingestion_to_text_duration", duration, labels={"status": "success"}
        )
        ...
        duration = time.time() - start_time
        log_histogram(
            "xml_ingestion_to_text_duration", duration, labels={"status": "error"}
        )
        ...
        log_counter(
            "xml_ingestion_to_text_error", labels={"error_type": type(e).__name__}
        )
        logging.error(f"Unexpected error in xml_to_text: {str(e)}")
        return None


def import_xml_handler(
    import_file,
    title,
    author,
    keywords,
    ...
    try:
        ...
        duration = time.time() - start_time
        log_histogram(
            "xml_ingestion_import_duration", duration, labels={"status": "success"}
        )
        log_counter(
            "xml_ingestion_import_success",
            labels={
                "root_tag": root.tag,
        ...
        return (
            f"XML file '{import_file.name}' import complete. Database result: {result}"
        )
        ...
        duration = time.time() - start_time
        log_histogram(
            "xml_ingestion_import_duration", duration, labels={"status": "error"}
        )
        ...
        log_counter(
            "xml_ingestion_import_error", labels={"error_type": type(e).__name__}
        )
