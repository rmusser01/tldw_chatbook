...
async def process_videos_endpoint(
    ...
    **Process Videos Endpoint (Fixed)**
    ...
    with TempDirManager(cleanup=True, prefix="process_video_") as temp_dir:
        ...
        # --- Save Uploads ---
        saved_files_info, file_handling_errors_raw = await _save_uploaded_files(
            files or [],
            temp_dir,
        ...
        if file_handling_errors_raw:
            ...
            for err in file_handling_errors_raw:
                ...
                err.get(
                    "input", "Unknown Filename"
                )  # Assume 'input' holds original name from _save_uploaded_files error
        ...
        if not all_inputs_to_process:
            if file_handling_errors_raw:  # Only file errors occurred
                ...
                return JSONResponse(
                    status_code=status.HTTP_207_MULTI_STATUS, content=batch_result
                )
            else:  # No inputs provided at all
                logger.warning("No video sources provided.")
                raise HTTPException(
                    status.HTTP_400_BAD_REQUEST, "No valid video sources supplied."
                )
        ...
        try:
            logger.debug(
                f"Calling process_videos for /process-videos endpoint with {len(all_inputs_to_process)} inputs."
            )
            ...
            try:
                print(
                    f"!!! DEBUG PRINT !!! My debug message: {json.dumps(processing_output, indent=2, default=str)}"
                )
            ...
            if isinstance(processing_output, dict):
                ...
                for res in processed_results_from_lib:
                    ...
                    current_input_ref = res.get(
                        "input_ref"
                    )  # This is likely the temp path or URL
                ...
                logger.error(
                    f"process_videos function returned unexpected type: {type(processing_output)}"
                )
            ...
            batch_result["errors_count"] = len(
                final_results_list
            )  # Count all items as errors now
            unique_errors = set(str(e) for e in final_errors_list if e is not None)
            batch_result["errors"] = list(unique_errors)
        ...
        if (
            ...
            )  # All errors, could also be 4xx/5xx depending on cause
        ...
        try:
            ...
            if (
                len(logged_result.get("results", [])) > 5
            ):  # Log details for first 5 results only
                logged_result["results"] = logged_result["results"][:5] + [
                    {"message": "... remaining results truncated for logging ..."}
                ]
            logger.debug(
                json.dumps(logged_result, indent=2, default=str)
            )  # Use default=str for non-serializable items
            ...
            if success_item_debug:
                logger.debug(
                    f"Value of input_ref for success item before return: {success_item_debug.get('input_ref')}"
                )
            else:
...
def get_process_audios_form(
    ...
    urls: Optional[List[str]] = Form(
        None, description="List of URLs of the audio items"
    ),
    title: Optional[str] = Form(
        None, description="Optional title (applied if only one item processed)"
    ),
    author: Optional[str] = Form(
        None, description="Optional author (applied similarly to title)"
    ),
    ...
    overwrite_existing: bool = Form(
        False,
        description="Overwrite existing media (Not used in this endpoint, but needed for model)",
    ),
    ...
    use_cookies: bool = Form(
        False, description="Use cookies for URL download requests"
    ),
    cookies: Optional[str] = Form(
        None, description="Cookie string if `use_cookies` is True"
    ),
    transcription_model: str = Form(
        "deepdml/faster-distil-whisper-large-v3.5", description="Transcription model"
    ),
    ...
    use_multi_level_chunking: bool = Form(
        False, description="Enable multi-level chunking"
    ),
    chunk_language: Optional[str] = Form(
        None, description="Chunking language override"
    ),
    ...
    perform_rolling_summarization: bool = Form(
        False, description="Perform rolling summarization"
    ),  # Keep if AddMediaForm has it
    summarize_recursively: bool = Form(
        False, description="Perform recursive summarization"
    ),
    # PDF options (Needed for AddMediaForm compatibility, ignored for audio)
    pdf_parsing_engine: Optional[PdfEngine] = Form(
        "pymupdf4llm", description="PDF parsing engine (for model compatibility)"
    ),
    custom_chapter_pattern: Optional[str] = Form(
        None,
        description="Regex pattern for custom chapter splitting (for model compatibility)",
    ),
    ...
    start_time: Optional[str] = Form(
        None, description="Optional start time (HH:MM:SS or seconds)"
    ),
    end_time: Optional[str] = Form(
        None, description="Optional end time (HH:MM:SS or seconds)"
    ),
) -> ProcessAudiosForm:
    """
...
async def process_audios_endpoint(
    ...
    **Process Audio Endpoint (Refactored)**
    ...
    with TempDirManager(cleanup=True, prefix="process_audio_") as temp_dir:
        ...
        for sf in saved_files:
            if sf.get("path") and sf.get("original_filename"):
                ...
                logger.warning(
                    f"Missing path or original_filename in saved_files_info item for audio: {sf}"
                )
        ...
        if file_errors:
            ...
            for err in file_errors:
                ...
                original_filename = err.get("original_filename") or err.get(
                    "input", "Unknown Upload"
                )
        ...
        try:
            logger.debug(
                f"Calling process_audio_files for /process-audios with {len(all_inputs)} inputs."
            )
            ...
            logging.error(
                f"Error executing process_audio_files: {exec_err}", exc_info=True
            )
            error_msg = (
                f"Error during audio processing execution: {type(exec_err).__name__}"
            )
            # Calculate errors based on *attempted* inputs for this batch
            num_attempted = len(all_inputs)
            batch_result["errors_count"] += (
                num_attempted  # Assume all failed if executor errored
            )
        ...
        if (
            ...
            processing_output is None and not batch_result["results"]
        ):  # Handle case where executor failed AND no file errors
            ...
            logging.error(
                f"process_audio_files returned unexpected format: Type={type(processing_output)}"
            )
                ...
                if input_src in uploaded_paths:
                    for sf in saved_files:
                        if str(sf["path"]) == input_src:
                            ...
                            break
                if (
                    original_ref not in existing_refs
                ):  # Only add errors for inputs not already covered (e.g., by file errors)
                    error_results.append(
                        {
                            "status": "Error",
                            "input_ref": original_ref,
    ...
    final_status_code = (
        ...
        and batch_result.get("processed_count", 0) > 0
        else status.HTTP_207_MULTI_STATUS
        if batch_result.get(
            "results"
        )  # Return 207 if there are *any* results (success, warning, or error)
    ...
    if final_status_code == status.HTTP_200_OK:
        ...
        logging.warning(
            "Not all submissions were processed succesfully! Please Try Again!"
        )
        logger.warning(
            f"/process-audios request finished with status {final_status_code}. Results count: {len(batch_result.get('results', []))}, Total Errors: {batch_result.get('errors_count', 0)}"
        )
