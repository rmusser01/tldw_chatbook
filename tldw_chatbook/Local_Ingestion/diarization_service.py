...
class DiarizationService:
                ...
                try:
                    ...
                    if not EncoderClassifier:
                        raise DiarizationError(
                            "SpeechBrain EncoderClassifier not available"
                        )
            ...
            if self._vad_model is None:
                try:
                    ...
                    if not model or not utils:
                        raise DiarizationError(
                            "Silero VAD model or utilities not available"
                        )
                    ...
                    try:
                        ...
                        for name, func in self._vad_utils.items():
                            if name != "VADIterator" and not callable(func):
                                raise DiarizationError(
                                    f"VAD utility '{name}' is not callable"
                                )
    ...
    def diarize(
        ...
        if not self.is_available:
            raise DiarizationError(
                "Diarization service is not available due to missing dependencies"
            )
        ...
        try:
            ...
            speech_timestamps = self._detect_speech(
                waveform, sample_rate, streaming=streaming_vad
            )
            ...
            if self.config.get("detect_overlapping_speech", False):
                ...
                segments = self._detect_overlapping_speech(
                    segments, embeddings, speaker_labels
                )
            ...
            if transcription_segments:
                ...
                aligned_segments = self._align_with_transcription(
                    merged_segments, transcription_segments
                )
            else:
            ...
            if progress_callback:
                progress_callback(
                    ...
                    {"num_speakers": unique_speakers, "duration": duration},
                )
        ...
        if torchaudio and torch:
            ...
            if not self._vad_utils:
                try:
                    ...
                    raise DiarizationError(
                        f"Cannot load audio: VAD model load failed: {e}"
                    ) from e
    ...
    def _detect_speech(
        self, waveform, sample_rate: int, streaming: bool = False
    ) -> List[Dict]:
            ...
            try:
                ...
                vad_iterator = VADIterator(
                    ...
                    min_silence_duration_ms=int(
                        self.config["vad_min_silence_duration"] * 1000
                    ),
                ...
                logger.warning(
                    f"Streaming VAD failed, falling back to standard VAD: {e}"
                )
                streaming = False
    ...
    def _create_segments(
        self, waveform: "torch.Tensor", speech_timestamps: List[Dict], sample_rate: int
    ) -> List[SegmentDict]:
        ...
        min_segment_samples = int(
            self.config.get("min_segment_duration", 1.0) * sample_rate
        )
        ...
        for speech in speech_timestamps:
            ...
            if speech_duration < min_segment_samples:
                ...
                if memory_efficient:
                    ...
                    try:
                        padded_waveform = torch.nn.functional.pad(
                            segment_waveform, (0, padding_needed)
                        )
                    except Exception as e:
                ...
                for i in range(
                    start_sample, end_sample - segment_samples + 1, step_samples
                ):
                    if memory_efficient:
                        # Store indices instead of waveform copy
                        segments.append(
                ...
                last_segment_start = (
                    start_sample
                    + ((end_sample - start_sample - segment_samples) // step_samples)
                    * step_samples
                    + step_samples
                )
                            ...
                            segments.append(
                                {
                                    ...
                                    "waveform_ref": waveform,  # Reference to original
                                    "is_padded": True,
                                    "padding_needed": segment_samples
                                    - remaining_samples,
                                    "original_duration": remaining_samples
                                    / sample_rate,
                                    "speech_region": speech,
                                }
                            )
        ...
        for batch_idx in range(0, len(segments), batch_size):
            ...
            try:
                if memory_efficient:
                    ...
                    waveforms = torch.stack(
                        [seg["waveform"].unsqueeze(0) for seg in batch_segments]
                    )
            except Exception as e:
            ...
            try:
                if hasattr(torch, "no_grad"):
                    try:
                        with torch.no_grad():
                            batch_embeddings = self._embedding_model.encode_batch(
                                waveforms
                            )
                    except (AttributeError, RuntimeError) as e:
                ...
                logger.error(
                    f"Failed to extract embeddings for batch starting at {batch_idx}: {e}"
                )
    ...
    def _cluster_speakers(
        ...
        num_speakers = max(
            self.config["min_speakers"], min(num_speakers, self.config["max_speakers"])
        )
    ...
    def _is_single_speaker(
        self, embeddings: "np.ndarray", threshold: Optional[float] = None
    ) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Average cosine similarity: {avg_similarity:.3f}, threshold: {threshold}"
            )
    ...
    def _merge_segments(self, segments: List[Dict]) -> List[Dict]:
        ...
        for segment in segments[1:]:
            ...
            close_enough = (
                segment["start"] - current["end"] <= self.config["merge_threshold"]
            )
    ...
    def _align_with_transcription(
        self, diarization_segments: List[Dict], transcription_segments: List[Dict]
    ) -> List[Dict]:
    ...
    def _detect_overlapping_speech(
        ...
        try:
            ...
            overlapping_count = sum(
                1 for s in segments if s.get("is_overlapping", False)
            )
