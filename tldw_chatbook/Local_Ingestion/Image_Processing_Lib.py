...
def get_image_config():
    ...
    return (
        get_media_ingestion_defaults("image")
        if get_media_ingestion_defaults
        else {
...
def extract_visual_features(image_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    """
    features = {"has_visual_features": False, "error": None}
    ...
    try:
        if PIL_AVAILABLE:
            with Image.open(image_path) as img:
                ...
                features["aspect_ratio"] = (
                    img.width / img.height if img.height > 0 else 0
                )
                ...
                if img.mode in ["RGB", "RGBA"]:
                    ...
                    if pixels:
                        ...
                        avg_color = tuple(
                            sum(col) // len(pixels) for col in zip(*pixels)
                        )
        ...
        if CV2_AVAILABLE and NUMPY_AVAILABLE:
            ...
            if img_cv is not None:
                ...
                features["edge_density"] = np.count_nonzero(edges) / (
            ...
            if img_cv is not None:
                try:
                    ...
                    features["edge_density"] = (
                        edge_count / total_pixels if total_pixels > 0 else 0
                    )
                    ...
                    logger.warning(
                        f"Could not extract OpenCV features without numpy: {e}"
                    )
...
def extract_text_from_image(
    ...
    try:
        ...
        if ocr_backend == "auto":
            ...
            result = ocr_manager.process_image(
                processed_path, language=language, backend=ocr_backend
            )
    ...
    try:
        ...
        if extract_features:
            ...
            if visual_features.get("error"):
                result["warnings"].append(
                    f"Visual feature extraction warning: {visual_features['error']}"
                )
        ...
        if ocr_text and chunk_options:
            try:
                ...
                result["chunks"] = [{"text": ocr_text, "metadata": {"chunk_num": 0}}]
        elif ocr_text:
        ...
        if perform_analysis and api_name and api_key and ocr_text:
            ...
            try:
                analysis_prompt = (
                    custom_prompt
                    or f"Analyze this text extracted from an image: {ocr_text[:500]}..."
                )
...
def process_image_batch(
    image_paths: List[Union[str, Path]], parallel: bool = True, **kwargs
) -> List[Dict[str, Any]]:
    """
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        try:
            ...
            results.append(
                {"status": "Error", "input_ref": str(image_path), "error": str(e)}
            )
