...
class EvaluationError(Exception):
    ...
    def __init__(
        self, context: ErrorContext, original_error: Optional[Exception] = None
    ):
...
class APIError(EvaluationError):
    ...
    def rate_limit_exceeded(
        provider: str, retry_after: Optional[float] = None
    ) -> "APIError":
        return APIError(
            ErrorContext(
        ...
        while retry_count <= max_retries:
            try:
                ...
                if not eval_error.context.is_retryable:
                    logger.error(
                        f"Operation failed after {retry_count + 1} attempts: {eval_error.context.message}"
                    )
                    if isinstance(e, EvaluationError):
                        raise eval_error
                    raise e
