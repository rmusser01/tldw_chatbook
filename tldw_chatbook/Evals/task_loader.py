...
class TaskLoader:
    ...
    def load_task(
        self, source: Union[str, Path, Dict], format_type: str = "auto"
    ) -> TaskConfig:
        """
    ...
    def _detect_format(self, source: Union[str, Path, Dict]) -> str:
        ...
        if isinstance(source, dict):
            ...
            if any(
                key in source for key in ["dataset_name", "dataset_path", "test_split"]
            ):
                return "eleuther"
            return "custom"
            ...
            if path.suffix.lower() in [".yaml", ".yml"]:
                ...
                try:
                    ...
                    if isinstance(content, dict) and any(
                        key in content
                        for key in ["dataset_name", "dataset_path", "test_split"]
                    ):
                        return "eleuther"
                    return "custom"
                except Exception:
    ...
    def _load_eleuther_task(self, source: Union[str, Path, Dict]) -> TaskConfig:
        ...
        if isinstance(source, dict):
            ...
            try:
                ...
                log_histogram(
                    "eval_task_file_parse_duration",
                    parse_duration,
                    labels={"format": "eleuther", "file_format": file_format},
                )
                log_histogram(
                    "eval_task_file_size",
                    path.stat().st_size,
                ...
                parse_duration = time.time() - parse_start
                log_counter(
                    "eval_task_file_parse_error",
                    labels={"format": "eleuther", "error_type": type(e).__name__},
                )
        ...
        try:
            name = config_data.get(
                "task", path.stem if isinstance(source, Path) else "unnamed_task"
            )
            ...
            stop_sequences = []
            if (
                "output_type" in config_data
                and config_data["output_type"] == "generate_until"
            ):
    ...
    def _load_huggingface_task(self, source: str) -> TaskConfig:
        ...
        if path.exists() and path.is_file():
            ...
            try:
                ...
                raise TaskLoadError(
                    f"Failed to load HuggingFace config from {path}: {e}"
                )
        ...
        try:
            ...
            dataset = load_dataset(
                dataset_name, dataset_config, split="train[:1]"
            )  # Load minimal sample
    ...
    def _infer_task_type_from_columns(self, columns: set) -> str:
        ...
        if any(
            col in columns_lower for col in ["choices", "options", "answers", "labels"]
        ):
            return "classification"
        elif any(
            col in columns_lower for col in ["question", "query", "input", "prompt"]
        ):
            return "question_answer"
    ...
    def create_task_from_template(self, template_name: str, **kwargs) -> TaskConfig:
        ...
        log_counter(
            "eval_task_template_create_attempt", labels={"template_name": template_name}
        )
        ...
        try:
            if hasattr(template_manager, "create_task_config"):
                task_config = template_manager.create_task_config(
            ...
            duration = time.time() - start_time
            log_histogram(
                "eval_task_template_create_duration",
                duration,
                labels={
            ...
            log_counter(
                "eval_task_template_create_success",
                labels={
                    "template_name": template_name,
                    "task_type": task_config.task_type,
                },
    ...
    def export_task(
        ...
        try:
            ...
            log_histogram(
                "eval_task_export_file_size",
                file_size,
                labels={"format_type": format_type, "file_format": file_format},
            )
            log_counter(
                "eval_task_export_success",
                labels={"format_type": format_type, "task_type": task_config.task_type},
            )
            ...
            duration = time.time() - start_time
            log_counter(
                "eval_task_export_error",
                labels={"format_type": format_type, "error_type": type(e).__name__},
            )
            log_histogram(
                "eval_task_export_error_duration",
                duration,
                labels={"format_type": format_type},
            )
            raise
    ...
    def validate_task(self, task_config: TaskConfig) -> List[str]:
        ...
        if task_config.task_type == "code_generation":
            if not task_config.generation_kwargs:
                ...
                issues.append(
                    "Code generation tasks must specify language in generation_kwargs"
                )

        # Log validation metrics
        duration = time.time() - start_time
        log_histogram(
            "eval_task_validation_duration",
            duration,
            labels={"task_type": task_config.task_type},
        )
        log_histogram(
            "eval_task_validation_issue_count",
            len(issues),
            labels={"task_type": task_config.task_type},
        )
        if len(issues) > 0:
