...
def _retained_definition_paths(context) -> tuple[tuple[str, Path], ...]:
    ...
    retained_configs = _retained_config_items(plan, doc)
    configs.update(
        key for key, item in retained_configs.items() if item.path == selector
    )
    if len(configs) != 1:
    ...
    if plan.local_snapshot is not None or (
        plan.effective_groups and "evaluations" not in plan.effective_groups
    ):
        retained.extend(
...
def _original_definition_paths(
    ...
    """Retain receipt-bound originals through safety coverage or selected preservation."""
    ...
    for item in target.items:
        ...
        if unselected and (item.logical_id, path) in plan.preserve:
            # These local rows were preserved, not published or captured. Carry
            # only their exact checked paths into the next inactive generation.
            covered = not any(
                path == selected or path in selected.parents or selected in path.parents
