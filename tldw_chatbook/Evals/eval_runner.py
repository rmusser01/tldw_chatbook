...
class DatasetLoader:
    ...
    def load_dataset_samples(
        task_config: TaskConfig, split: str = None, max_samples: int = None
    ) -> List[EvalSample]:
    ...
    def _load_local_dataset(
        ...
        if file_size > 1_000_000_000:  # 1GB warning
            logger.warning(
                f"Large dataset file ({file_size / 1_000_000:.1f} MB): {path}"
            )
    ...
    def _load_json_dataset(path: Path, max_samples: int = None) -> List[EvalSample]:
        ...
        try:
            ...
            raise DatasetLoadingError.invalid_format(
                str(path), f"Invalid JSON at line {e.lineno}, column {e.colno}: {e.msg}"
            )
        ...
        for i, item in enumerate(data[:max_samples] if max_samples else data):
            try:
                ...
                if not input_text:
                    errors.append(
                        f"Sample {sample_id}: Missing input text (checked 'input', 'question', 'text' fields)"
                    )
                    continue

                expected_output = item.get(
                    "output", item.get("answer", item.get("target"))
                )
        ...
        if errors:
            ...
            logger.warning(
                f"Failed to load {len(errors)} samples from {path}:\n{error_summary}"
            )
    ...
    def _load_csv_dataset(path: Path, max_samples: int = None) -> List[EvalSample]:
        ...
        for i, row in enumerate(rows[:max_samples] if max_samples else rows):
            ...
            choice_keys = [
                k
                for k in row.keys()
                if k.startswith("choice") or k.startswith("option")
            ]
            if choice_keys:
    ...
    def _load_huggingface_dataset(
        task_config: TaskConfig, split: str, max_samples: int = None
    ) -> List[EvalSample]:
        ...
        try:
            dataset = load_dataset(
                task_config.dataset_name, task_config.dataset_config, split=split
            )
            ...
            for i, item in enumerate(dataset):
                ...
                if task_config.doc_to_text:
                    input_text = DatasetLoader._apply_template(
                        task_config.doc_to_text, item
                    )
                else:
                    input_text = item.get(
                        "input", item.get("question", item.get("text", str(item)))
                    )

                # Apply doc_to_target template if provided
                expected_output = None
                if task_config.doc_to_target:
                    expected_output = DatasetLoader._apply_template(
                        task_config.doc_to_target, item
                    )
                else:
                    expected_output = item.get(
                        "output", item.get("answer", item.get("target"))
                ...
                if task_config.doc_to_choice:
                    choices_text = DatasetLoader._apply_template(
                        task_config.doc_to_choice, item
                    )
            ...
            raise ValueError(
                f"Failed to load HuggingFace dataset {task_config.dataset_name}: {e}"
            )
...
class MetricsCalculator:
    ...
    @staticmethod
    def calculate_regex_match(
        predicted: str, expected: str, pattern: str = None
    ) -> float:
    ...
    def calculate_semantic_similarity(
        predicted: str, expected: str, embedding_model=None
    ) -> float:
        ...
        try:
            ...
            logger.debug(
                "Sentence transformers not available, using token overlap for semantic similarity"
            )
    ...
    def _calculate_lexical_semantic_fallback(predicted: str, expected: str) -> float:
        ...
        if (
            number_aliases.get(predicted_norm) == expected_norm
            or number_aliases.get(expected_norm) == predicted_norm
        ):
            return 0.8
...
class ErrorHandler:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if provider and provider in self.rate_limit_delays:
                    ...
                    if delay > 0:
                        logger.info(
                            f"Rate limited for {provider}, waiting {delay:.1f}s"
                        )
                ...
                if provider:
                    self.consecutive_errors[provider] = (
                        self.consecutive_errors.get(provider, 0) + 1
                    )
                ...
                if attempt < self.max_retries:
                    ...
                    raise APIError.rate_limit_exceeded(
                        provider or "unknown", self.rate_limit_delays.get(provider)
                    )
                ...
                if provider:
                    ...
                    if self.consecutive_errors[provider] > 5:
                        logger.warning(
                            f"Multiple consecutive errors for {provider}, adding delay"
                        )
                ...
                error_context = self.error_handler.handle_error(
                    e,
                    {"sample_id": sample_id, "provider": provider, "attempt": attempt},
                )
    ...
    def get_provider_health(self, provider: str) -> Dict[str, Any]:
        ...
        return {
            ...
            if self.consecutive_errors.get(provider, 0) < 3
            else "degraded",
        }
...
class BaseEvalRunner(ABC):
    ...
    def calculate_metrics(
        ...
        if self.task_config.metric == "exact_match":
            metrics["exact_match"] = MetricsCalculator.calculate_exact_match(
                predicted, expected
            )
        elif self.task_config.metric == "contains":
            metrics["contains"] = MetricsCalculator.calculate_contains_match(
                predicted, expected
            )
            ...
            metrics["bleu"] = MetricsCalculator.calculate_bleu_score(
                predicted, expected
            )
        elif self.task_config.metric == "bleu-1":
            metrics["bleu-1"] = MetricsCalculator.calculate_bleu_score(
                predicted, expected, n=1
            )
        elif self.task_config.metric == "bleu-2":
            metrics["bleu-2"] = MetricsCalculator.calculate_bleu_score(
                predicted, expected, n=2
            )
        elif self.task_config.metric == "bleu-3":
            metrics["bleu-3"] = MetricsCalculator.calculate_bleu_score(
                predicted, expected, n=3
            )
        elif self.task_config.metric == "bleu-4":
            metrics["bleu-4"] = MetricsCalculator.calculate_bleu_score(
                predicted, expected, n=4
            )
        elif self.task_config.metric == "rouge-1":
            metrics["rouge-1"] = MetricsCalculator.calculate_rouge_1(
                predicted, expected
            )
        elif self.task_config.metric == "rouge-2":
            metrics["rouge-2"] = MetricsCalculator.calculate_rouge_2(
                predicted, expected
            )
        elif self.task_config.metric == "rouge-l":
            metrics["rouge-l"] = MetricsCalculator.calculate_rouge_l(
                predicted, expected
            )
            ...
            metrics["semantic_similarity"] = (
                MetricsCalculator.calculate_semantic_similarity(predicted, expected)
            )
        elif self.task_config.metric == "bertscore":
            # Alias for semantic similarity
            metrics["bertscore"] = MetricsCalculator.calculate_semantic_similarity(
                predicted, expected
            )
        elif self.task_config.metric == "accuracy" and choice_values:
            # For multiple choice
            metrics["accuracy"] = MetricsCalculator.calculate_exact_match(
                predicted, expected
            )
        elif self.task_config.metric == "instruction_adherence":
            # Evaluate how well the response follows instructions
            metrics["instruction_adherence"] = self._calculate_instruction_adherence(
                predicted, expected, metric_sample
            )
        elif self.task_config.metric == "format_compliance":
            # Evaluate if the response follows required format
            metrics["format_compliance"] = self._calculate_format_compliance(
                predicted, metric_sample
            )
            ...
            metrics["dialogue_quality"] = self._calculate_dialogue_quality(
                predicted, metric_sample
            )
        else:
        ...
        for filter_config in self.task_config.filter_list:
            if filter_config.get("filter") == "regex":
                ...
                if pattern:
                    try:
                        match = re.search(
                            pattern, filtered_output, re.IGNORECASE | re.DOTALL
                        )
                        if match:
    ...
    def _calculate_instruction_adherence(
        self, predicted: str, expected: str, sample: EvalSample
    ) -> float:
    ...
    def _calculate_coherence_score(self, predicted: str) -> float:
        ...
        transition_count = sum(
            1 for word in transition_words if word in predicted.lower()
        )
    ...
    def _calculate_dialogue_quality(self, predicted: str, sample: EvalSample) -> float:
        ...
        has_speakers = any(
            re.search(pattern, predicted, re.MULTILINE) for pattern in speaker_patterns
        )
        ...
        has_responses = bool(
            re.search(r"(yes|no|okay|sure|maybe|I think|I believe)", predicted.lower())
        )
...
class QuestionAnswerRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        async def _run_sample_operation():
            ...
            if self.task_config.num_fewshot > 0:
                ...
                prompt = self._format_few_shot_prompt(
                    few_shot_samples, sample.input_text
                )
        ...
        try:
            ...
            result_data, retry_count = await self.error_handler.with_retry(
                _run_sample_operation, sample.id, self.model_config.get("provider")
            )
    ...
    def _format_few_shot_prompt(
        self, few_shot_samples: List[EvalSample], query: str
    ) -> str:
...
class ClassificationRunner(BaseEvalRunner):
    ...
    def calculate_metrics(
        self, predicted: str, expected: str, choices: List[str] = None
    ) -> Dict[str, float]:
        ...
        if choices and expected in choices:
            ...
            if predicted_idx >= 0:
                distance = (
                    abs(predicted_idx - expected_idx) / (len(choices) - 1)
                    if len(choices) > 1
                    else 0
                )
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        log_counter(
            "eval_sample_processing_started",
            labels={
        ...
        async def _run_classification_operation():
            ...
            metrics = self.calculate_metrics(
                predicted_choice, sample.expected_output, sample.choices
            )
        ...
        try:
            ...
            log_histogram(
                "eval_sample_processing_duration",
                processing_time,
                labels={
                    "task_type": "classification",
                    "provider": self.model_config.get("provider", "unknown"),
            ...
            log_counter(
                "eval_sample_processing_success",
                labels={
                    "task_type": "classification",
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            logger.error(
                f"Error processing classification sample {sample.id} after retries: {e}"
            )
            logger.error(f"Error classification: {error_info}")
    ...
    async def _generate_response(self, prompt: str) -> str:
        ...
        try:
            ...
            log_histogram(
                "eval_llm_generation_error_duration",
                generation_duration,
                labels={
                    "task_type": "classification",
                    "provider": self.model_config.get("provider", "unknown"),
...
class LogProbRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        async def _run_logprob_operation():
            ...
            if sample.choices:
                ...
                metrics = self.calculate_metrics(
                    predicted, sample.expected_output, sample.choices
                )
                ...
                logprob = await self._get_continuation_logprob(
                    prompt, sample.expected_output
                )
        ...
        try:
            # Use error handler with retry logic
            result_data, retry_count = await self.error_handler.with_retry(
                _run_logprob_operation, sample.id, self.model_config.get("provider")
            )
            ...
            logger.error(
                f"Error processing logprob sample {sample.id} after retries: {e}"
            )
            logger.error(f"Error classification: {error_info}")
    ...
    async def _get_choice_logprobs(
    ...
    async def _get_text_logprob(self, text: str) -> float:
        ...
        try:
            ...
            if self.provider_name == "openai":
                ...
                logger.warning(
                    f"Log probabilities require provider-specific implementation for {self.provider_name}"
                )
                return float("-inf")
            else:
                ...
                logger.warning(
                    f"Log probabilities not supported by {self.provider_name}"
                )
                return float("-inf")
...
class GenerationRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            # Use error handler with retry logic
            result_data, retry_count = await self.error_handler.with_retry(
                _run_generation_operation, sample.id, self.model_config.get("provider")
            )
            ...
            logger.error(
                f"Error processing generation sample {sample.id} after retries: {e}"
            )
            logger.error(f"Error classification: {error_info}")
...
class EvalRunner:
    ...
    def __init__(self, task_config: TaskConfig, model_config: Dict[str, Any]):
        ...
        if specialized_available:
            if category == "coding" or subcategory in [
    ...
    async def run_evaluation(
        ...
        samples = DatasetLoader.load_dataset_samples(
            self.task_config, max_samples=max_samples
        )
    ...
    def calculate_aggregate_metrics(
        ...
        aggregate_metrics["error_count"] = sum(
            1 for r in results if "error" in r.metrics
        )
        aggregate_metrics["success_rate"] = (
            len(results) - aggregate_metrics["error_count"]
        ) / len(results)

        # Log aggregation metrics
        duration = time.time() - start_time
        log_histogram(
            "eval_aggregation_duration",
            duration,
            labels={
        ...
        for metric_name, metric_value in aggregate_metrics.items():
            if isinstance(metric_value, (int, float)):
                log_histogram(
                    f"eval_aggregate_{metric_name}",
                    metric_value,
                    labels={"sample_count": str(len(results))},
                )
...
async def handle_batch_errors(
    operations: List[asyncio.Task], continue_on_error: bool = True
