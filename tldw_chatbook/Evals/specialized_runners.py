...
class CodeExecutionRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            ...
            response = await self.llm_interface.generate(
                prompt=prompt, **self.task_config.generation_kwargs
            )
            generation_duration = time.time() - generation_start

            # Log generation metrics
            log_histogram(
                "eval_code_generation_duration",
                generation_duration,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            log_histogram(
                "eval_code_extraction_duration",
                extraction_duration,
                labels={"extraction_method": "regex_and_ast"},
            )
            log_histogram(
                "eval_extracted_code_length",
                len(extracted_code),
                labels={"provider": self.model_config.get("provider", "unknown")},
            )
            ...
            log_counter(
                "eval_specialized_runner_success",
                labels={
                    "runner_type": "code_execution",
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            for metric_name, metric_value in metrics.items():
                if isinstance(metric_value, (int, float)):
                    log_histogram(
                        f"eval_code_metric_{metric_name}",
                        metric_value,
                        labels={
                            "provider": self.model_config.get("provider", "unknown"),
                            "model": self.model_config.get("model_id", "unknown"),
    ...
    def _execute_code(self, code: str, sample: EvalSample) -> Dict[str, Any]:
        ...
        try:
            ...
            log_histogram(
                "eval_code_syntax_check_duration",
                syntax_check_duration,
                labels={"valid": "true"},
            )
            log_counter(
                "eval_code_syntax_valid",
                labels={"provider": self.model_config.get("provider", "unknown")},
            )
            ...
            if not test_cases and hasattr(sample, "expected_output"):
                ...
                test_cases = [
                    {"input": sample.input_text, "expected": sample.expected_output}
                ]
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    results["error_message"] = (
                        f"Execution timeout ({self.timeout_seconds}s)"
                    )
                except Exception as e:
    ...
    def _extract_function_arg_count(
    ...
    def _find_static_safety_violation(self, code: str) -> Optional[str]:
        ...
        for node in ast.walk(tree):
            ...
            left_is_sequence = isinstance(
                node.left, (ast.List, ast.Tuple, ast.Set, ast.Dict)
            )
            right_is_sequence = isinstance(
                node.right, (ast.List, ast.Tuple, ast.Set, ast.Dict)
            )

            if (
                left_is_sequence
                and right_repetition is not None
                and right_repetition > max_literal_repetition
            ):
                return (
                    "Static safety check blocked excessive literal sequence allocation"
                )
            if (
                right_is_sequence
                and left_repetition is not None
                and left_repetition > max_literal_repetition
            ):
                return (
                    "Static safety check blocked excessive literal sequence allocation"
                )
    ...
    def _calculate_code_metrics(
        self, execution_results: Dict[str, Any], sample: EvalSample
    ) -> Dict[str, float]:
        ...
        metrics["execution_success"] = (
            1.0 if execution_results["execution_success"] else 0.0
        )
...
class SafetyEvaluationRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            ...
            # Log generation metrics
            log_histogram(
                "eval_safety_generation_duration",
                generation_duration,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            if safety_analysis["contains_harmful_content"]:
                log_counter(
                    "eval_safety_harmful_content_detected",
                    labels={
                        "provider": self.model_config.get("provider", "unknown"),
                        "model": self.model_config.get("model_id", "unknown"),
                    },
            ...
            if safety_analysis["privacy_leakage"]:
                log_counter(
                    "eval_safety_privacy_leakage_detected",
                    labels={
                        "provider": self.model_config.get("provider", "unknown"),
                        "model": self.model_config.get("model_id", "unknown"),
                    },
            ...
            if safety_analysis["appropriate_refusal"]:
                log_counter(
                    "eval_safety_appropriate_refusal",
                    labels={
                        "provider": self.model_config.get("provider", "unknown"),
                        "model": self.model_config.get("model_id", "unknown"),
                    },
            ...
            return EvalSampleResult(
                ...
                metrics=metrics,
                metadata={"safety_analysis": safety_analysis, "prompt": prompt},
                processing_time=processing_time,
            )
    ...
    def _calculate_safety_metrics(
        self, analysis: Dict[str, Any], sample: EvalSample
    ) -> Dict[str, float]:
        """Calculate safety metrics."""
        metrics = {}
        ...
        # Safety metrics
        metrics["harmful_content_detected"] = (
            1.0 if analysis["contains_harmful_content"] else 0.0
        )
        metrics["bias_detected"] = 1.0 if analysis["contains_bias"] else 0.0
        metrics["privacy_leakage_detected"] = (
            1.0 if analysis["privacy_leakage"] else 0.0
        )
        ...
        if hasattr(sample, "label"):
            ...
            if expected_label in ["safe", "harmful"]:
                ...
                metrics["safety_classification_accuracy"] = (
                    1.0 if predicted_safe == actual_safe else 0.0
                )
...
class MultilingualEvaluationRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            ...
            # Log generation metrics
            log_histogram(
                "eval_multilingual_generation_duration",
                generation_duration,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            log_histogram(
                "eval_multilingual_analysis_duration",
                analysis_duration,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "target_language": self.target_language,
            ...
            if language_analysis.get("contains_latin_script"):
                log_counter(
                    "eval_multilingual_latin_script_detected",
                    labels={"target_language": self.target_language},
                )
            if language_analysis.get("contains_chinese"):
                log_counter(
                    "eval_multilingual_chinese_detected",
                    labels={"target_language": self.target_language},
                )
            if language_analysis.get("contains_japanese"):
                log_counter(
                    "eval_multilingual_japanese_detected",
                    labels={"target_language": self.target_language},
                )
            if language_analysis.get("contains_arabic"):
                log_counter(
                    "eval_multilingual_arabic_detected",
                    labels={"target_language": self.target_language},
                )
            ...
            if "fluency_score" in metrics:
                log_histogram(
                    "eval_multilingual_fluency_score",
            ...
            return EvalSampleResult(
                ...
                actual_output=response,
                metrics=metrics,
                metadata={"language_analysis": language_analysis, "prompt": prompt},
                processing_time=processing_time,
            )
    ...
    def _format_multilingual_prompt(self, sample: EvalSample) -> str:
        ...
        if task_type == "translation":
            ...
            return (
                f"Answer this question in {self.target_language}: {sample.input_text}"
            )
        else:
            return sample.input_text
    ...
    def _analyze_language(self, response: str, sample: EvalSample) -> Dict[str, Any]:
        ...
        for lang, markers in language_markers.items():
            ...
            if common_found > 0:
                ...
                frequency_score = (
                    sum(1 for w in words_lower if w in markers) / len(words_lower)
                    if words_lower
                    else 0
                )
        ...
        if language_scores:
            ...
            if self.target_language.lower() in language_scores:
                analysis["contains_target_language"] = (
                    language_scores[self.target_language.lower()] > 0.1
                )

        # Check for mixed languages
        significant_scripts = sum(
            1 for _, conf in analysis["script_analysis"].items() if conf > 0.05
        )
        significant_languages = sum(
            1 for score in language_scores.values() if score > 0.1
        )
        analysis["mixed_language"] = (
            significant_scripts > 1 or significant_languages > 1
        )
        ...
        analysis["fluency_indicators"] = {
            ...
            "avg_words_per_sentence": sum(sentence_lengths) / len(sentence_lengths)
            if sentence_lengths
            else 0,
            "sentence_length_std": self._calculate_std_dev(sentence_lengths)
            if len(sentence_lengths) > 1
            else 0,
            "has_punctuation": bool(re.search(r"[.!?,;:]", response)),
            "capitalization_ratio": sum(1 for w in words if w and w[0].isupper())
            / len(words)
            if words
            else 0,
    ...
    def _calculate_multilingual_metrics(
        self, analysis: Dict[str, Any], sample: EvalSample
    ) -> Dict[str, float]:
        """Calculate multilingual metrics."""
        metrics = {}
        ...
        metrics["target_language_detected"] = (
            1.0 if analysis["contains_target_language"] else 0.0
        )
        ...
        if self.task_config.metadata.get("subcategory") == "translation":
            ...
            overlap_ratio = (
                len(input_words & output_words) / len(input_words) if input_words else 0
            )
            ...
            if input_length > 0:
                length_ratio = min(input_length, output_length) / max(
        ...
        if sample.expected_output:
            ...
            base_metrics = self.calculate_metrics(
                analysis.get("translated_text", ""), sample.expected_output
            )
            ...
            if analysis["contains_target_language"]:
                ...
                if expected_words:
                    jaccard = len(expected_words & actual_words) / len(
...
class CreativeEvaluationRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            ...
            # Log generation metrics
            log_histogram(
                "eval_creative_generation_duration",
                generation_duration,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
                    "task_subcategory": self.task_config.metadata.get(
            ...
            log_histogram(
                "eval_creative_analysis_duration",
                analysis_duration,
                labels={"provider": self.model_config.get("provider", "unknown")},
            )
            ...
            log_histogram(
                "eval_creative_vocabulary_diversity",
                creativity_analysis["vocabulary_diversity"],
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
                },
    ...
    def _analyze_creativity(self, response: str, sample: EvalSample) -> Dict[str, Any]:
        ...
        if analysis["word_count"] > 0:
            analysis["vocabulary_diversity"] = (
                analysis["unique_words"] / analysis["word_count"]
            )

        # Coherence indicators
        analysis["coherence_indicators"] = {
            "has_proper_structure": bool(re.search(r"[.!?]", response)),
            "avg_sentence_length": analysis["word_count"]
        ...
        analysis["creativity_indicators"] = {
            ...
            "emotional_language": len(
                re.findall(
                    r"\b(happy|sad|excited|afraid|angry|surprised)\b", response.lower()
                )
            ),
        }
    ...
    def _calculate_creative_metrics(
        self, analysis: Dict[str, Any], sample: EvalSample
    ) -> Dict[str, float]:
        """Calculate creative metrics."""
        metrics = {}
...
class RobustnessEvaluationRunner(BaseEvalRunner):
    ...
    def __init__(self, task_config: TaskConfig, model_config: Dict[str, Any]):
        ...
        self.perturbation_types = task_config.metadata.get(
            "perturbation_types",
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            ...
            # Log generation metrics
            log_histogram(
                "eval_robustness_generation_duration",
                generation_duration,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            log_histogram(
                "eval_robustness_total_duration",
                duration,
                labels={"provider": self.model_config.get("provider", "unknown")},
            )
    ...
    async def _test_robustness(
        self, sample: EvalSample, original_response: str
    ) -> Dict[str, Any]:
            ...
            for pert_type, perturbed_input in perturbations.items():
                try:
                    ...
                    perturbed_response = await self.llm_interface.generate(
                        prompt=perturbed_input, **self.task_config.generation_kwargs
                    )
            ...
            for format_type, formatted_input in format_tests.items():
                try:
                    formatted_response = await self.llm_interface.generate(
                        prompt=formatted_input, **self.task_config.generation_kwargs
                    )

                    # Check if response maintains expected format
                    format_valid = self._validate_format(
                        formatted_response, sample.metadata.get("expected_format", {})
                    )
    ...
    def _generate_perturbations(self, text: str) -> Dict[str, str]:
        ...
        if "case_change" in self.perturbation_types:
            ...
            perturbations["case_change"] = "".join(
                c.upper() if i % 3 == 0 else c.lower() for i, c in enumerate(text)
            )
    ...
    def _generate_format_variations(self, sample: EvalSample) -> Dict[str, str]:
        ...
        # JSON format
        variations["json"] = json.dumps({"input": base_input, "format": "json"})
    ...
    def _validate_format(self, response: str, expected_format: Dict[str, Any]) -> bool:
        ...
        if format_type == "json":
            ...
            response_lower = response.lower()
            return all(
                section.lower() in response_lower for section in required_sections
            )

        return True  # Default to valid

    def _detect_adversarial_trap_avoidance(
        self, response: str, sample: EvalSample
    ) -> bool:
    ...
    def _calculate_robustness_metrics(
        ...
        if self.robustness_type == "input_perturbation":
            ...
            if consistency_scores:
                metrics["robustness_score"] = sum(consistency_scores.values()) / len(
                    consistency_scores
                )
            ...
            avoided_trap = self._detect_adversarial_trap_avoidance(
                original_response, sample
            )
...
class MathReasoningRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            ...
            log_histogram(
                "eval_math_extraction_duration",
                extraction_duration,
                labels={"found_answer": str(extracted_answer is not None)},
            )

            # Check if answer is correct
            is_correct = self._check_math_answer(
                extracted_answer, sample.expected_output
            )

            if is_correct:
                log_counter(
                    "eval_math_correct_answer",
                    labels={
                        "provider": self.model_config.get("provider", "unknown"),
                        "model": self.model_config.get("model_id", "unknown"),
                    },
            ...
            log_histogram(
                "eval_math_reasoning_analysis_duration",
                analysis_duration,
                labels={"provider": self.model_config.get("provider", "unknown")},
            )
            ...
            # Log step count
            log_histogram(
                "eval_math_step_count",
                reasoning_analysis["step_count"],
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
                },
            ...
            return EvalSampleResult(
                ...
                input_text=sample.input_text,
                expected_output=sample.expected_output,
                actual_output=str(extracted_answer)
    ...
    def _analyze_reasoning(self, response: str) -> Dict[str, Any]:
        ...
        uses_equations = any(
            re.search(pattern, response) for pattern in equation_patterns
        )
...
class SummarizationRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            ...
            # Log generation metrics
            log_histogram(
                "eval_summarization_generation_duration",
                generation_duration,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            compression_ratio = (
                summary_length / source_length if source_length > 0 else 0
            )
            ...
            if sample.expected_output:
                ...
                metrics["rouge-1"] = MetricsCalculator.calculate_rouge_1(
                    summary, sample.expected_output
                )
            ...
            analysis_start = time.time()
            coverage_analysis = self._analyze_content_coverage(
                summary, sample.input_text
            )
            analysis_duration = time.time() - analysis_start
            metrics["key_info_coverage"] = coverage_analysis["coverage_score"]

            log_histogram(
                "eval_summarization_analysis_duration",
                analysis_duration,
                labels={"provider": self.model_config.get("provider", "unknown")},
            )
            ...
            log_histogram(
                "eval_summarization_compression_ratio",
                compression_ratio,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            log_histogram(
                "eval_summarization_coverage_score",
            ...
            if "rouge-1" in metrics:
                log_histogram(
                    "eval_summarization_rouge1_score",
                    metrics["rouge-1"],
                    labels={
                        "provider": self.model_config.get("provider", "unknown"),
                        "model": self.model_config.get("model_id", "unknown"),
                ...
                log_histogram(
                    "eval_summarization_rouge2_score",
                    metrics["rouge-2"],
                    labels={
                        "provider": self.model_config.get("provider", "unknown"),
                        "model": self.model_config.get("model_id", "unknown"),
                ...
                log_histogram(
                    "eval_summarization_rougeL_score",
                    metrics["rouge-l"],
                    labels={
                        "provider": self.model_config.get("provider", "unknown"),
                        "model": self.model_config.get("model_id", "unknown"),
...
class DialogueRunner(BaseEvalRunner):
    ...
    async def run_sample(self, sample: EvalSample) -> EvalSampleResult:
        ...
        try:
            ...
            # Log generation metrics
            log_histogram(
                "eval_dialogue_generation_duration",
                generation_duration,
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
            ...
            log_histogram(
                "eval_dialogue_analysis_duration",
                analysis_duration,
                labels={"provider": self.model_config.get("provider", "unknown")},
            )
            ...
            if sample.expected_output:
                ...
                metrics["semantic_similarity"] = (
                    MetricsCalculator.calculate_semantic_similarity(
            ...
            log_histogram(
                "eval_dialogue_relevance_score",
                dialogue_analysis["relevance_score"],
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
                },
            ...
            log_histogram(
                "eval_dialogue_coherence_score",
                dialogue_analysis["coherence_score"],
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
                },
            ...
            log_histogram(
                "eval_dialogue_appropriateness_score",
                dialogue_analysis["appropriateness_score"],
                labels={
                    "provider": self.model_config.get("provider", "unknown"),
                    "model": self.model_config.get("model_id", "unknown"),
                },
            ...
            if dialogue_analysis["maintains_context"]:
                log_counter(
                    "eval_dialogue_maintains_context",
                    labels={
                        "provider": self.model_config.get("provider", "unknown"),
                        "model": self.model_config.get("model_id", "unknown"),
                    },
            ...
            return EvalSampleResult(
                ...
                actual_output=response,
                metrics=metrics,
                metadata={"prompt": prompt, "dialogue_analysis": dialogue_analysis},
                processing_time=processing_time,
            )
...
def get_specialized_runner(
    task_type: str, task_config: TaskConfig, model_config: Dict[str, Any]
) -> Optional[BaseEvalRunner]:
    """Get appropriate specialized runner for task type with metrics tracking."""
    # Log runner request
    log_counter(
        "eval_specialized_runner_requested",
        labels={
            "task_type": task_type,
    ...
    if runner_class:
        log_counter(
            "eval_specialized_runner_found",
            labels={"task_type": task_type, "runner_class": runner_class.__name__},
        )
    ...
    if task_config.metadata:
        ...
        if runner_hint in SPECIALIZED_RUNNER_REGISTRY:
            ...
            log_counter(
                "eval_specialized_runner_found_by_hint",
                labels={
                    "task_type": task_type,
    ...
    log_counter("eval_specialized_runner_not_found", labels={"task_type": task_type})
    return None
...
def list_specialized_runners() -> List[str]:
    ...
    log_counter(
        "eval_specialized_runners_listed", labels={"count": str(len(runner_names))}
    )
