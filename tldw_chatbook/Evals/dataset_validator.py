...
class DatasetValidator:
    ...
    def validate_dataset(
        ...
        try:
            ...
            if not samples:
                issues.append(
                    ValidationIssue(
                        severity="error", category="data", message="Dataset is empty"
                    )
                )
    ...
    def _validate_schema(
        ...
        optional = self.optional_fields.get("all", []) + self.optional_fields.get(
            task_type, []
        )
    ...
    def _check_format_consistency(
        ...
        if task_type == "classification":
            choice_counts = [
                len(s.get("choices", [])) for s in samples if "choices" in s
            ]
    ...
    def _calculate_quality_metrics(
        ...
        if task_type in ["question_answer", "classification"]:
            question_lengths = [
                len(s.get("question", "")) for s in samples if "question" in s
            ]
            if question_lengths:
        ...
        if "question" in samples[0]:
            ...
            metrics["duplicate_questions"] = (
                len(questions) - metrics["unique_questions"]
            )
    ...
    def suggest_fixes(self, report: ValidationReport) -> List[str]:
        ...
        if "balance" in issues_by_category:
            for issue in issues_by_category["balance"]:
                if "imbalanced" in issue.message:
                    suggestions.append(
                        "Consider balancing the dataset by adding more samples for underrepresented classes"
                    )
        ...
        if "data" in issues_by_category:
            ...
            if empty_fields:
                suggestions.append(
                    f"Fill in empty values for fields: {', '.join(empty_fields)}"
                )

        # Duplicate suggestions
        if report.statistics.get("quality", {}).get("duplicate_questions", 0) > 0:
            suggestions.append(
                "Remove or modify duplicate questions to increase dataset diversity"
            )
