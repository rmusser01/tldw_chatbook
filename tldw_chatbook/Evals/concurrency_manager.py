...
class ConcurrentRunManager:
        ...
        async with self._lock:
            ...
            for existing_id, run_info in self._active_runs.items():
                if (
                    ...
                    and run_info["model_id"] == model_id
                    and existing_id != run_id
                ):
            ...
            logger.info(
                f"Registered run {run_id} for task {task_id} with model {model_id}"
            )
            return True
    ...
    async def get_active_runs(self) -> List[Dict[str, Any]]:
        ...
        async with self._lock:
            return [
                {"run_id": run_id, **info} for run_id, info in self._active_runs.items()
            ]

    async def is_task_running(self, task_id: str) -> bool:
        """Check if a specific task is currently running."""
        async with self._lock:
            return any(run["task_id"] == task_id for run in self._active_runs.values())
