...
class EvaluationExporter:
    ...
    def _export_ab_test_csv(self, result: Any, output_path: Path) -> None:
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            writer.writerow(
                ["Metric", "Model A", "Model B", "Difference", "P-value", "Significant"]
            )

            for metric in result.model_a_metrics:
                if (
                    metric in result.model_b_metrics
                    and metric in result.statistical_tests
                ):
            ...
            writer.writerow(
                [
                    "Latency (ms)",
                    f"{result.model_a_latency:.2f}",
                    f"{result.model_b_latency:.2f}",
                ]
            )
            writer.writerow(
                ["Cost ($)", f"{result.model_a_cost:.4f}", f"{result.model_b_cost:.4f}"]
            )
    ...
    def _export_ab_test_json(
        self, result: Any, output_path: Path, options: Dict[str, Any]
    ) -> None:
    ...
    def _export_ab_test_markdown(self, result: Any, output_path: Path) -> None:
        ...
        lines.append("")
        winner_text = (
            f"**{result.winner}**" if result.winner else "No significant difference"
        )
        lines.append(f"- **Winner:** {winner_text}")
        lines.append(f"- **Sample Size:** {result.sample_size}")
        lines.append(
            f"- **Models Tested:** {result.model_a_name} vs {result.model_b_name}"
        )
        lines.append("")
        ...
        for metric in result.model_a_metrics:
            if metric in result.model_b_metrics and metric in result.statistical_tests:
                ...
                value_a_str = (
                    f"{value_a:.4f}" if isinstance(value_a, float) else str(value_a)
                )
                value_b_str = (
                    f"{value_b:.4f}" if isinstance(value_b, float) else str(value_b)
                )
                ...
                lines.append(
                    f"| {metric} | {value_a_str} | {value_b_str} | {diff_str} | {p_value_str} | {sig_str} |"
                )
        ...
        latency_better = (
            "Model A" if result.model_a_latency < result.model_b_latency else "Model B"
        )
        cost_better = (
            "Model A" if result.model_a_cost < result.model_b_cost else "Model B"
        )

        lines.append(
            f"| Latency (ms) | {result.model_a_latency:.2f} | {result.model_b_latency:.2f} | {latency_better} |"
        )
        lines.append(
            f"| Cost ($) | {result.model_a_cost:.4f} | {result.model_b_cost:.4f} | {cost_better} |"
        )
        lines.append("")
        ...
        if hasattr(result, "statistical_summary"):
            ...
            lines.append(
                "- Statistical tests performed using appropriate methods for each metric"
            )
    ...
    def _export_ab_test_latex(self, result: Any, output_path: Path) -> None:
        ...
        for metric in result.model_a_metrics:
            if metric in result.model_b_metrics and metric in result.statistical_tests:
                ...
                lines.append(
                    f"{metric_escaped} & {value_a_str} & {value_b_str} & {diff_str} & {p_value_str} & {sig_str} \\\\"
                )
    ...
    def _export_run_csv(
        self, run_data: Dict[str, Any], output_path: Path, options: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            if not results:
                ...
                with open(output_path, "w", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(
                        ["Run ID", "Model", "Task", "Status", "Total Cost", "Samples"]
                    )
                    writer.writerow(
                        [
                            run_data.get("run_id", ""),
    ...
    def _export_run_markdown(self, run_data: Dict[str, Any], output_path: Path) -> None:
        ...
        lines.append(
            f"- **Duration:** {run_data.get('duration_seconds', 0):.2f} seconds"
        )
        lines.append("")
...
def export_to_csv(
    run_data: Dict[str, Any], output_path: Path, options: Dict[str, Any]
) -> None:
