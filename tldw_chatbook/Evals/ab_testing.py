...
class ABTestResult:
    ...
    winner: Optional[str] = (
        None  # 'model_a', 'model_b', or None (no significant difference)
    )
...
class ABTestRunner:
    ...
    async def run_ab_test(
        ...
        try:
            ...
            if summary_a.get("duration"):
                result.model_a_latency = (
                    summary_a["duration"] / len(results_a) if results_a else 0
                )
            if summary_b.get("duration"):
                result.model_b_latency = (
                    summary_b["duration"] / len(results_b) if results_b else 0
                )
            ...
            log_histogram(
                "ab_test_duration", duration, labels={"winner": result.winner or "tie"}
            )
            log_counter("ab_test_completed", labels={"winner": result.winner or "tie"})

            logger.info(
                f"A/B test completed. Winner: {result.winner or 'No significant difference'}"
            )
            ...
            log_counter("ab_test_error", labels={"error_type": type(e).__name__})
            raise
    ...
    def _perform_statistical_tests(
        ...
        for metric in config.metrics_to_compare:
            ...
            if SCIPY_AVAILABLE:
                ...
                t_stat = (
                    (mean_a - mean_b) / math.sqrt(var_a / n_a + var_b / n_b)
            ...
            pooled_std = (
                math.sqrt((std_a**2 + std_b**2) / 2) if std_a > 0 or std_b > 0 else 1
            )
            ...
            tests[metric] = {
                ...
                "relative_difference": ((mean_a - mean_b) / mean_b * 100)
                if mean_b != 0
                else 0,
            }
    ...
    def _calculate_confidence_intervals(
        ...
        for metric in metrics:
            ...
            if values_a and values_b:
                ...
                intervals[f"{metric}_difference"] = (
                    mean_diff - margin,
                    mean_diff + margin,
                )
    ...
    def _merge_sample_results(
        self, results_a: List[Dict[str, Any]], results_b: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
            ...
            merged.append(
                {
                    ...
                    "model_a_correct": res_a.get("metrics", {}).get("exact_match", 0)
                    == 1,
                    "model_b_correct": res_b.get("metrics", {}).get("exact_match", 0)
                    == 1,
                }
            )
    ...
    async def create_and_run_ab_test(self, config: ABTestConfig) -> str:
        ...
        async def run_test():
            try:
                ...
                self.eval_orchestrator.db.update_ab_test_status(
                    test_id, "failed", str(e)
                )
                raise
