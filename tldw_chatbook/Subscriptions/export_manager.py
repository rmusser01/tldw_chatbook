...
class ExportManager:
    ...
    async def _export_pdf(
        self, content: str, metadata: Dict[str, Any], output_path: Path
    ) -> Path:
    ...
    async def _export_epub(
        ...
        try:
            ...
            book.toc = (
                epub.Link(
                    "briefing.xhtml", metadata.get("name", "Briefing"), "briefing"
                ),
            )
    ...
    async def _export_docx(
        ...
        try:
            ...
            footer = doc.add_paragraph()
            footer.add_run(
                f"This briefing contains {metadata.get('item_count', 0)} items from {metadata.get('source_count', 0)} sources."
            ).italic = True
