...
class BaseScrapingPipeline(ABC):
    ...
    async def scrape(self, url: Optional[str] = None) -> List[ScrapedItem]:
        ...
        try:
            ...
            # Check rate limiting
            can_request, wait_time = self.site_config_manager.check_rate_limit(
                target_url
            )
            if not can_request:
            ...
            log_histogram(
                "scraping_pipeline_duration",
                duration,
                labels={"pipeline_type": self.pipeline_type, "success": "true"},
            )
            log_counter(
                "scraping_pipeline_items",
                labels={
            ...
            log_histogram(
                "scraping_pipeline_duration",
                duration,
                labels={"pipeline_type": self.pipeline_type, "success": "false"},
            )
            log_counter(
                "scraping_pipeline_errors",
                labels={
                    "pipeline_type": self.pipeline_type,
...
class ScrapingPipelineFactory:
    ...
    def create_pipeline(cls, config: ScrapingConfig) -> BaseScrapingPipeline:
        ...
        if pipeline_type not in cls._pipeline_registry:
            ...
            raise ValueError(
                f"Unknown pipeline type: {pipeline_type}. Available types: {available}"
            )
...
class ContentExtractor:
    ...
    def extract_text_content(
        html: str, remove_scripts: bool = True, remove_styles: bool = True
    ) -> str:
        """
