        ...
        if TIKTOKEN_AVAILABLE:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not load tiktoken encoding for {model}, using approximation"
                    )
        else:
            logger.info(
    ...
    def allocate_by_priority(
        ...
        for item in items:
            ...
            total_allocation = min(
                total_allocation, current_tokens
        ...
        for item in items:
            if isinstance(item, dict) and "id" in item:
                ...
                formatted_items.append(
                    {
                        "id": item,
                        "priority": getattr(item, "priority", 3)
                        if hasattr(item, "priority")
                        else 3,
                        "current_tokens": getattr(item, "token_count", 1000)
                        if hasattr(item, "token_count")
                        else 1000,
                    }
                )

        return temp_manager.allocate_by_priority(
            formatted_items, min_per_item, max_per_item
        )

    def allocate_proportional(
        self, items: List[Dict[str, Any]], weights: Optional[Dict[Any, float]] = None
    ) -> Dict[Any, int]:
        """
...
class TokenBudgetTracker:
    ...
    def use_tokens(self, amount: int, operation: str = "unknown"):
        ...
        if self.used_tokens > self.total_budget:
            logger.warning(
                f"Token budget exceeded: {self.used_tokens}/{self.total_budget}"
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        for op in self.operations:
            operation_totals[op["operation"]] = (
                operation_totals.get(op["operation"], 0) + op["tokens"]
            )
