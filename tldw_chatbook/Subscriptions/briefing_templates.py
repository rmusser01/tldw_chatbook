...
class BriefingTemplate:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "BriefingTemplate":
        ...
        sections = [
            BriefingSection(**section_data) for section_data in data.get("sections", [])
        ]
...
class BriefingRenderer:
        ...
        for section in template.sections:
            try:
                ...
                section_items = self._get_section_items(
                    section, content, render_context
                )
    ...
    def _prepare_context(
        ...
        context["security_items"] = [
            ...
            for item in all_items
            if any(
                tag in str(item).lower() for tag in ["security", "vulnerability", "cve"]
            )
        ]
        ...
        context["technical_items"] = [
            ...
            in ["technology", "programming", "development"]
        ]
                ...
                trending.append(
                    {
                        ...
                        "mention_count": count,
                        "source_count": min(
                            count, len(set(item.get("source", "") for item in items))
                        ),
                    }
                )
    ...
    def _format_date_filter(
        self, timestamp: Union[str, datetime], format: str = "%Y-%m-%d"
    ) -> str:
        """Format date."""
        if isinstance(timestamp, str):
            try:
    ...
    def _convert_to_html(self, markdown_text: str) -> str:
        ...
        html = markdown.markdown(
            markdown_text, extensions=["extra", "sane_lists", "codehilite"]
        )
