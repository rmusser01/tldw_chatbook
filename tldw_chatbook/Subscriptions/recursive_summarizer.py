...
class RecursiveSummarizer:
    ...
    async def summarize_content(
        self, content: str, target_tokens: int, context: Optional[str] = None
    ) -> SummarizationResult:
        """
        ...
        if self.config.provider and self.config.model:
            try:
                result = await self._llm_recursive_summarize(
                ...
                # Log metrics
                log_histogram(
                    "recursive_summarization_duration",
                    result.processing_time,
                    labels={
                ...
                log_counter(
                    "recursive_summarization_performed",
                    labels={
                        "method": result.method_used,
    ...
    async def _llm_recursive_summarize(
        self,
        content: str,
        target_tokens: int,
        ...
        chunk_params = self._calculate_chunk_params(
            original_tokens, target_tokens, level
        )
        ...
        for i, chunk in enumerate(chunks):
            chunk_summary = await self._summarize_single_chunk(
                chunk, chunk_params["chunk_target"], context, i, len(chunks)
            )
        ...
        if (
            merged_tokens > target_tokens
            and level < self.config.max_recursion_depth - 1
        ):
            # Recursive call
            sub_result = await self._llm_recursive_summarize(
                merged_content, target_tokens, context, level + 1
            )
    ...
    def _calculate_chunk_params(
        self, content_tokens: int, target_tokens: int, level: int
    ) -> Dict[str, Any]:
    ...
    async def _summarize_single_chunk(
        ...
        prompt = self._build_chunk_prompt(
            chunk, target_tokens, context, chunk_index, total_chunks
        )
    ...
    def _build_chunk_prompt(
        ...
        if self.config.style == "concise":
            parts.append(
                f"Summarize the following content in approximately {target_tokens} tokens. Be extremely concise, focusing only on the most critical information:"
            )
        elif self.config.style == "detailed":
            parts.append(
                f"Provide a comprehensive summary of the following content in approximately {target_tokens} tokens. Preserve important details and nuances:"
            )
        else:  # balanced
            parts.append(
                f"Summarize the following content in approximately {target_tokens} tokens. Balance brevity with completeness:"
            )
...
5. Respect the requested token limit"""
    ...
    def _merge_summaries(self, chunks: List[SummarizedChunk]) -> str:
        ...
        if self.config.format == "bullets":
            ...
            merged = []
            for i, chunk in enumerate(chunks):
                if i > 0 and not chunk.summary.startswith(
                    ("However", "Additionally", "Furthermore", "Moreover")
                ):
                    # Add simple transition
                    merged.append(
                        "Additionally, " + chunk.summary[0].lower() + chunk.summary[1:]
                    )
                else:
