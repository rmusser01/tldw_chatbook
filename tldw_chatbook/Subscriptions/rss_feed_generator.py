...
class RSSFeedGenerator:
    ...
    def __init__(
        ...
        self.feed_description = (
            feed_description or f"Generated RSS feed for {feed_link}"
        )
    ...
    def _setup_channel(self):
        ...
        ET.SubElement(self.channel, "lastBuildDate").text = self._format_rfc822_date(
        ...
        ET.SubElement(
            ...
            {"href": self.feed_link, "rel": "self", "type": "application/rss+xml"},
        )
    ...
    def add_item(
        ...
        ET.SubElement(item, "description").text = self._escape_text(
        ...
        if enclosure_url and enclosure_type:
            enclosure = ET.SubElement(
                item, "enclosure", {"url": enclosure_url, "type": enclosure_type}
            )
            if enclosure_length:
    ...
    def generate_feed(self, pretty_print: bool = True) -> str:
        ...
        log_counter(
            "rss_feed_generated",
            labels={"item_count": str(len(self.channel.findall("item")))},
        )
...
class AtomFeedGenerator:
    ...
    def _setup_feed(self):
        ...
        ET.SubElement(self.feed, "updated").text = self._format_iso8601_date(
            datetime.now(timezone.utc)
        )
        ...
        ET.SubElement(
            self.feed,
            "link",
            {"rel": "alternate", "type": "text/html", "href": self.feed_link},
        )

        ET.SubElement(
            self.feed,
            "link",
            {"rel": "self", "type": "application/atom+xml", "href": self.feed_id},
        )
        ...
        generator = ET.SubElement(
            self.feed,
            "generator",
            {"uri": "https://github.com/rmusser01/tldw_chatbook", "version": "1.0"},
        )
    ...
    def add_entry(
        ...
        # Link
        ET.SubElement(
            entry, "link", {"rel": "alternate", "type": "text/html", "href": link}
        )
        ...
        if published:
            ET.SubElement(entry, "published").text = self._format_iso8601_date(
                published
            )
