...
class ContentBaseline:
    ...
    def from_content(
        ...
        structural_hash = (
            extractor.calculate_structural_hash(content) if extract_structure else ""
        )
    ...
    def extract_key_elements(self, html_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            elements = {
                ...
                "headers": [
                    h.get_text()[:100] for h in soup.find_all(["h1", "h2", "h3"])[:10]
                ],
...
class ChangeDetector:
    ...
    def __init__(self, change_threshold: float = 0.1, semantic_threshold: float = 0.85):
        """
    ...
    def detect_changes(
        ...
        text_similarity = self._calculate_text_similarity(
            baseline.extracted_text, new_text
        )
        ...
        # Token count change
        token_change_ratio = abs(new_token_count - baseline.token_count) / max(
            baseline.token_count, 1
        )
        ...
        log_counter(
            "change_detection_performed",
            labels={"change_type": change_type, "significant": str(should_update)},
        )
    ...
    def _compare_key_elements(
        self, old_elements: Dict[str, Any], new_elements: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class BaselineManager:
    ...
    async def create_baseline(
        self, subscription_id: int, url: str, content: str
    ) -> ContentBaseline:
        """
        ...
        log_counter(
            "baseline_created", labels={"subscription_id": str(subscription_id)}
        )
    ...
    async def check_for_changes(
        ...
        change_threshold = (
            subscription.get("change_threshold", 0.1) if subscription else 0.1
        )
    ...
    async def update_baseline(
        self, subscription_id: int, url: str, new_content: str
    ) -> ContentBaseline:
        ...
        log_counter(
            "baseline_updated", labels={"subscription_id": str(subscription_id)}
        )
    ...
    def _get_latest_baseline(
        self, subscription_id: int, url: str
    ) -> Optional[ContentBaseline]:
    ...
    def _cleanup_old_baselines(self, subscription_id: int):
        ...
        with self.db.transaction() as conn:
            ...
            if deleted > 0:
                logger.info(
                    f"Cleaned up {deleted} old baselines for subscription {subscription_id}"
                )
