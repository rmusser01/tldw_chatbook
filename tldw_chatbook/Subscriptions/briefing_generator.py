...
class BriefingGenerator:
        ...
        try:
            ...
            if self.llm_provider and self.llm_model:
                ...
                sections = await self._generate_sections_with_llm(
                    items, items_by_source, analysis_prompt
                )
            else:
            ...
            log_histogram(
                ...
                duration,
                labels={"format": format, "used_llm": str(bool(self.llm_provider))},
            )
            log_counter(
                "briefing_generation_count",
                labels={"format": format, "item_count": str(len(items))},
            )
            ...
            return {"success": False, "message": str(e), "content": None}

    async def _get_briefing_items(
        self,
        ...
        if source_filter:
            if "subscription_ids" in source_filter:
                placeholders = ",".join(
                    ["?" for _ in source_filter["subscription_ids"]]
                )
    ...
    async def _generate_sections_with_llm(
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an expert analyst creating executive briefings from aggregated content.",
            },
    ...
    def _generate_sections_basic(
        ...
        sections["actions_section"] = (
            "• Review highlighted items for detailed information\n• Consider following up on trending topics"
        )

        return sections

    def _prepare_content_summary(
        self, items: List[Dict[str, Any]], max_items: int = 20
    ) -> str:
    ...
    def _format_sources_section(
        self, items_by_source: Dict[str, List[Dict[str, Any]]]
    ) -> str:
    ...
    def _format_html(
        ...
        trends_html = "".join(
            [f'<span class="trend-tag">{trend}</span>' for trend in trends_items[:10]]
        )
    ...
    async def save_briefing(
        self, briefing_data: Dict[str, Any], save_location: str = "notes"
    ) -> bool:
        """
        ...
        try:
            ...
            if briefing["success"]:
                ...
                await self.generator.save_briefing(
                    briefing, config.get("save_location", "notes")
                )
