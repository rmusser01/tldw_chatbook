...
class DistributionManager:
    ...
    def _get_encrypted_config(self, section: str, default: dict) -> dict:
        ...
        try:
            ...
            if section == "smtp" and "password" in config_data:
                config_data["password"] = self.encryption.decrypt_value(
            ...
            try:
                if channel == "email":
                    results["email"] = await self._distribute_email(
                        content, format, metadata, attachments
                    )
                elif channel.startswith("webhook:"):
                    webhook_type = channel.split(":", 1)[1]
                    results[channel] = await self._distribute_webhook(
                        content, format, metadata, webhook_type
                    )
                elif channel.startswith("cloud:"):
                    cloud_type = channel.split(":", 1)[1]
                    results[channel] = await self._distribute_cloud(
                        content, format, metadata, attachments, cloud_type
                    )
                else:
    ...
    async def _distribute_email(
        ...
        try:
            ...
            msg["Subject"] = (
                f"{metadata.get('name', 'Briefing')} - {datetime.now().strftime('%Y-%m-%d')}"
            )

            # Add metadata headers
            msg["X-Briefing-Generated"] = metadata.get(
            ...
            log_histogram(
                "distribution_duration", duration, labels={"channel": "email"}
            )
            log_counter(
                "distributions_sent", labels={"channel": "email", "status": "success"}
            )
            ...
            log_counter(
                "distributions_sent", labels={"channel": "email", "status": "error"}
            )
            logger.error(f"Error sending email: {str(e)}")
            return {"success": False, "error": str(e)}

    async def _distribute_webhook(
        self, content: str, format: str, metadata: Dict[str, Any], webhook_type: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not webhook_url:
                return {
                    "success": False,
                    "error": f"No webhook URL configured for {webhook_type}",
                }
            ...
            if webhook_type == "discord":
                ...
                # Generic webhook
                payload = {"content": content, "format": format, "metadata": metadata}
            ...
            log_counter(
                "distributions_sent",
                labels={"channel": f"webhook_{webhook_type}", "status": "success"},
            )
            ...
            log_counter(
                "distributions_sent",
                labels={"channel": f"webhook_{webhook_type}", "status": "error"},
            )
    ...
    def _format_discord_payload(
        ...
        return {"content": content, "embeds": [embed], "username": "Briefing Bot"}
    ...
    async def _distribute_cloud(
        ...
        logger.warning(
            f"Cloud storage distribution not yet implemented for {cloud_type}"
        )
        return {
            "success": False,
...
<body>
    ...
    def configure_smtp(
        ...
        try:
            ...
            save_setting_to_cli_config(
                "distribution.smtp", "password", password
            )  # Will be encrypted automatically
            save_setting_to_cli_config(
                "distribution.smtp", "from_address", from_address or username
            )
            save_setting_to_cli_config(
                "distribution.smtp", "to_addresses", to_addresses or []
            )
    ...
    def configure_webhook(self, webhook_type: str, url: str) -> bool:
        ...
        try:
            ...
            # Save to config
            save_setting_to_cli_config(
                "distribution.webhooks", f"{webhook_type}_url", url
            )
