...
class ScheduledTask:
    ...
    task_type: str = field(
        compare=False, default="check"
    )  # 'check', 'briefing', 'cleanup'
...
class UpdatePattern:
    ...
    def __init__(self):
        ...
        self.update_history = defaultdict(
            list
        )  # subscription_id -> list of update timestamps
        self.pattern_cache = {}  # subscription_id -> pattern info

    def record_update(
        self, subscription_id: int, timestamp: datetime, had_updates: bool
    ):
        """
    ...
    def analyze_pattern(self, subscription_id: int) -> Dict[str, Any]:
        ...
        if len(history) < 5:
            # Not enough data
            pattern = {"type": "unknown", "confidence": 0.0, "suggested_interval": None}
        else:
    ...
    def get_next_check_time(
        self, subscription_id: int, default_interval: int
    ) -> datetime:
        """
...
class SubscriptionScheduler:
    ...
    async def _process_subscription(self, task: ScheduledTask):
        ...
        try:
            ...
            logger.info(
                f"Checking subscription '{subscription['name']}' (ID: {subscription_id})"
            )
            ...
            self.update_patterns.record_update(
                subscription_id, datetime.now(timezone.utc), len(items) > 0
            )
            ...
            # Log success
            logger.info(
                f"Subscription check complete: '{subscription['name']}' - {len(items)} new items"
            )

            # Schedule next check
            next_check = self.update_patterns.get_next_check_time(
                subscription_id, subscription["check_frequency"]
            )
            ...
            if task.attempt < 3:
                ...
                heapq.heappush(self.task_queue, next_task)
                logger.info(
                    f"Scheduled retry for subscription {subscription_id} in {retry_delay} seconds"
                )
            ...
            log_histogram(
                ...
                duration,
                labels={"status": "success" if "items" in locals() else "error"},
            )
...
def create_scheduler(
    db: SubscriptionsDB, max_concurrent: int = 10
