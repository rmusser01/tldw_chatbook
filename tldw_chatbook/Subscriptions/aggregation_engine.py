...
class AggregationEngine:
    ...
    async def aggregate_items(
        ...
        subscriptions: Optional[Dict[int, Dict[str, Any]]] = None,
    ) -> AggregatedContent:
        """
        ...
        for group_key, items in grouped_items.items():
            section = await self._create_section(
                group_key, items, section_budgets.get(group_key, 1000)
            )
            sections.append(section)

        # Sort sections by priority
        sections.sort(
            key=lambda s: -sum(i.priority for i in s.items) / max(len(s.items), 1)
        )
    ...
    def _prepare_items(
        self,
        items: List[Dict[str, Any]],
        subscriptions: Optional[Dict[int, Dict[str, Any]]],
    ) -> List[AggregatedItem]:
    ...
    def _apply_filters(self, items: List[AggregatedItem]) -> List[AggregatedItem]:
        ...
        if self.config.min_relevance_score > 0:
            filtered = [
                i
                for i in filtered
                if i.relevance_score >= self.config.min_relevance_score
            ]
        ...
        if self.config.date_range_days:
            ...
            filtered = [
                i for i in filtered if i.published_date and i.published_date >= cutoff
            ]
        ...
        if self.config.exclude_categories:
            ...
            filtered = [
                i for i in filtered if not any(c in excluded for c in i.categories)
            ]

        return filtered

    async def _calculate_relevance_scores(
        self, items: List[AggregatedItem]
    ) -> List[AggregatedItem]:
        ...
        for item in items:
            ...
            if item.published_date:
                age_hours = (
                    datetime.now(timezone.utc) - item.published_date
                ).total_seconds() / 3600
                if age_hours < 24:
                    score *= 1.5
                elif age_hours < 72:
    ...
    def _group_items(
        ...
        for key, group_items in groups.items():
            if self.config.sort_within_groups == "relevance":
                ...
                group_items.sort(
                    key=lambda x: (
                        x.published_date or datetime.min.replace(tzinfo=timezone.utc)
                    ),
                    reverse=True,
                )
    ...
    def _allocate_section_budgets(
        self, grouped_items: Dict[str, List[AggregatedItem]]
    ) -> Dict[str, int]:
        """Allocate token budgets to each section."""
        # Calculate available content budget
        content_budget = int(
            self.config.total_token_budget * self.config.content_allocation
        )
        ...
        for key, weight in section_weights.items():
            ...
            budget = max(
                budget, self.config.min_tokens_per_item * len(grouped_items[key])
            )
        ...
        for item, allocation in allocations.items():
            if allocation < item.token_count:
                ...
                item.summary = await self._summarize_content(
                    item.content, target_tokens=allocation
                )
    ...
    async def _summarize_content(self, content: str, target_tokens: int) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Summarized content: {result.original_tokens} -> {result.final_tokens} tokens "
            ...
            logger.error(
                f"Recursive summarization failed: {str(e)}. Falling back to truncation."
            )
    ...
    async def _generate_executive_summary(
        ...
        summary_parts.append(
            f"This briefing contains {total_items} items from {len(sections)} sections."
        )
        ...
        budget = int(
            self.config.total_token_budget * self.config.executive_summary_allocation
        )
        summary = "\n".join(summary_parts)
