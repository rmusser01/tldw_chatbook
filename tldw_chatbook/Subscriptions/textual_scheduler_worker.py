...
class SubscriptionSchedulerWorker:
    ...
    async def start_scheduler(self):
        ...
        try:
            ...
            self.error_count += 1
            self.post_message(
                SubscriptionError(worker=self, error=str(e), subscription_id=None)
            )
        finally:
            self.is_running = False
                ...
                if pending:
                    ...
                    for task, result in zip(pending, results):
                        if isinstance(result, Exception):
                            logger.error(
                                f"Task {task.subscription_id} failed: {result}"
                            )
                            self.error_count += 1
                        else:
                            self.check_count += 1
    ...
    async def _process_task_with_limit(
        self, task: ScheduledTask, semaphore: asyncio.Semaphore
    ):
    ...
    async def _process_task(self, task: ScheduledTask):
        ...
        try:
            ...
            log_histogram(
                ...
                duration,
                labels={"task_type": task.task_type, "success": "true"},
            )
            ...
            log_histogram(
                "scheduler_task_duration",
                duration,
                labels={"task_type": task.task_type, "success": "false"},
            )
    ...
    def _on_check_complete(self, subscription_id: int, result: Dict[str, Any]):
        ...
        if hasattr(self, "update_patterns"):
            self.update_patterns.record_update(
                subscription_id, datetime.now(timezone.utc), bool(result.get("items"))
            )
    ...
    async def generate_briefing_now(self, config: Dict[str, Any]):
        ...
        if not BRIEFING_AVAILABLE or not self.briefing_generator:
            logger.warning(
                "Briefing generation unavailable - install with: pip install tldw_chatbook[subscriptions]"
            )
            self.post_message(
                SubscriptionError(
                    worker=self,
        ...
        try:
            ...
            aggregated = await self.aggregation_engine.aggregate_items(
                items, subscriptions
            )
            ...
            self.post_message(
                SubscriptionError(
                    worker=self, error=f"Briefing generation failed: {str(e)}"
                )
            )
