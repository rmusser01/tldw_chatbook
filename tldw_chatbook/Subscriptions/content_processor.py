...
class ContentProcessor:
    ...
    async def process_item(
        ...
        processed["processing_duration_ms"] = int(
            (datetime.now() - start_time).total_seconds() * 1000
        )

        # Log metrics
        log_histogram(
            "subscription_item_processing_duration",
            processed["processing_duration_ms"] / 1000.0,
            labels={
    ...
    def _extract_content(
        self, item: Dict[str, Any], subscription: Dict[str, Any]
    ) -> str:
        """
    ...
    def _extract_keywords(
        ...
        if subscription.get("tags"):
            tags = [
                tag.strip() for tag in subscription["tags"].split(",") if tag.strip()
            ]
        ...
        if item.get("title"):
            ...
            title_words = re.findall(
                r"\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b", item["title"]
            )
    ...
    async def _analyze_content(
        self, content: str, item: Dict[str, Any], subscription: Dict[str, Any]
    ) -> str:
        """
        ...
        log_histogram(
            "subscription_llm_analysis_duration",
            duration,
            labels={"provider": self.llm_provider, "model": self.llm_model},
        )
        log_counter(
            "subscription_llm_analysis_count",
            labels={
...
class ContentSummarizer:
    ...
    def summarize(text: str, max_sentences: int = 3) -> str:
        ...
        top_indices = sorted(
            [idx for score, idx, sent in sentence_scores[:max_sentences]]
        )
