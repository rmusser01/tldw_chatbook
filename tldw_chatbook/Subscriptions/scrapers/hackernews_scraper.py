...
class HackerNewsScrapingPipeline(BaseScrapingPipeline):
    ...
    def __init__(self, config: ScrapingConfig):
        ...
        # HN-specific options
        self.feed_type = config.options.get(
            "feed_type", "frontpage"
        )  # frontpage, newest, best, ask, show, jobs
        ...
        self.search_tags = config.options.get(
            "search_tags", []
        )  # story, comment, poll, job
        self.time_range = config.options.get(
            "time_range"
        )  # last24h, pastWeek, pastMonth, pastYear
    ...
    async def _fetch_search_results(self) -> str:
        ...
        params = {"query": self.search_query, "hitsPerPage": self.max_items}
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            log_counter(
                "hn_scraper_fetch",
                labels={"status": "success", "feed_type": "search", "method": "api"},
            )
    ...
    async def _fetch_user_content(self) -> str:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            log_counter(
                "hn_scraper_fetch",
                labels={"status": "success", "feed_type": "user", "method": "api"},
            )
    ...
    def _parse_rss_feed(self, raw_content: str) -> List[ScrapedItem]:
        ...
        try:
            ...
            log_counter(
                "hn_scraper_items",
                labels={"feed_type": self.feed_type, "item_count": str(len(items))},
            )
    ...
    def _extract_hn_metadata(
        self, description: str, comments_link: str
    ) -> Dict[str, Any]:
