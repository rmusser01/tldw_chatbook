...
class GenericWebScrapingPipeline(BaseScrapingPipeline):
    ...
    async def fetch_content(self, url: str) -> str:
        ...
        if ARTICLE_EXTRACTOR_AVAILABLE and not self.config.javascript_enabled:
            try:
                ...
                logger.warning(
                    f"Article extractor failed, falling back to httpx: {str(e)}"
                )
    ...
    def parse_content(self, raw_content: str, url: str) -> List[ScrapedItem]:
        ...
        try:
            ...
            log_counter(
                "generic_scraper_items",
                labels={"domain": self.get_domain(), "item_count": str(len(items))},
            )

        except Exception as e:
            logger.error(f"Error parsing content: {str(e)}")
            log_counter(
                "generic_scraper_errors",
                labels={"error_type": "parse_error", "domain": self.get_domain()},
            )
            raise
    ...
    def _extract_item_from_element(
    ...
    def _extract_metadata(self, element, base_url: str) -> Dict[str, Any]:
        ...
        metadata = {"source": "generic_web", "domain": self.get_domain()}
