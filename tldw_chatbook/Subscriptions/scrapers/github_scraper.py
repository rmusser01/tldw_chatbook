...
class GitHubScrapingPipeline(BaseScrapingPipeline):
    ...
    def __init__(self, config: ScrapingConfig):
        ...
        self.feed_type = config.options.get(
            "feed_type", "releases"
        )  # releases, commits, tags, activity, issues, pulls
    ...
    def validate_config(self) -> Tuple[bool, Optional[str]]:
        ...
        if self.feed_type in ["issues", "pulls"] and not self.github_token:
            logger.warning(
                "GitHub API token recommended for issues/pulls to avoid rate limits"
            )
    ...
    async def _fetch_atom_feed(self) -> str:
        ...
        if self.feed_type == "releases":
            ...
            if self.branch and self.branch != "main":
                ...
                feed_url = (
                    f"{self.GITHUB_BASE_URL}/{self.owner}/{self.repo}/commits.atom"
                )
    ...
    def _parse_api_item(self, item_data: dict) -> Optional[ScrapedItem]:
        ...
        try:
            ...
            return ScrapedItem(
                ...
                url=url,
                content=body
                or f"No description provided for this {self.feed_type[:-1]}",
                published_date=published_date,
                metadata=metadata,
            )
    ...
    def _should_include_item(self, item: ScrapedItem) -> bool:
        ...
        if self.feed_type == "releases":
            ...
            if not self.include_pre_releases:
                ...
                if any(
                    term in title_lower
                    for term in ["alpha", "beta", "rc", "pre-release"]
                ):
                    return False
