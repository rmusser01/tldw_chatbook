...
class YouTubeScrapingPipeline(BaseScrapingPipeline):
    ...
    async def fetch_content(self, url: str) -> str:
        ...
        if self.channel_id:
            ...
            if not channel_id:
                raise ValueError(
                    f"Could not resolve channel ID for {self.username or self.handle}"
                )
    ...
    def _parse_entry(self, entry) -> Optional[ScrapedItem]:
        ...
        try:
            ...
            # Extract statistics
            stats_elem = (
                media_group.find("media:community", self.NAMESPACES)
                if media_group
                else None
            )
            views = 0
    ...
    def _should_include_video(self, item: ScrapedItem) -> bool:
        ...
        if self.keyword_filter:
            if not any(
                keyword.lower() in text_to_check for keyword in self.keyword_filter
            ):
                return False
        ...
        if self.exclude_keywords:
            if any(
                keyword.lower() in text_to_check for keyword in self.exclude_keywords
            ):
                return False
