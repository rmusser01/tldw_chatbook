...
class WebsiteMonitor:
    ...
    async def monitor_website(self, subscription: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            log_histogram(
                "website_monitor_duration",
                duration,
                labels={"type": sub_type, "success": "true"},
            )
            log_counter(
    ...
    async def _monitor_sitemap(
        ...
        if ARTICLE_SCRAPER_AVAILABLE:
            try:
                ...
                max_urls = subscription.get("processing_options", {}).get(
                    "max_urls", 50
                )
                urls = urls[:max_urls]
    ...
    async def _scrape_website(
        ...
        config = ScrapingConfig(
            ...
            javascript_enabled=subscription.get("processing_options", {}).get(
                "javascript", False
            ),
    ...
    async def _fallback_article_scraper(
        self, subscription: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _fetch_url_content(
        self, url: str, subscription: Dict[str, Any]
    ) -> Optional[str]:
    ...
    async def _extract_items_from_content(
        self, url: str, content: str, subscription: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if method == "custom" and subscription.get("extraction_rules"):
            ...
            config = ScrapingConfig(
                ...
                options={"rules": subscription["extraction_rules"]},
            )
            pipeline = ScrapingPipelineFactory.create_pipeline(config)
    ...
    async def _scrape_and_extract_url(
        self, url: str, subscription: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if content:
                return await self._extract_items_from_content(
    ...
    async def _generate_and_cache_feed(
        self, subscription: Dict[str, Any], items: List[Dict[str, Any]]
    ) -> Path:
    ...
    async def _monitor_feed(self, subscription: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            duration = (datetime.now(timezone.utc) - start_time).total_seconds()
            log_histogram(
                "feed_monitor_duration",
                duration,
                labels={"type": subscription["type"], "success": "true"},
            )
            log_counter(
                "feed_monitor_checks",
                labels={
            ...
            logger.info(
                f"Successfully monitored {subscription['type']} feed: {subscription['name']} - {len(formatted_items)} items"
            )
