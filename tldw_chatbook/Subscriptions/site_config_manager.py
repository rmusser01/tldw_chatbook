    ...
    def check_rate_limit(self, domain: str, config: SiteConfig) -> Tuple[bool, float]:
        ...
        with self._locks[domain]:
            ...
            self._request_times[domain] = [
                t for t in self._request_times[domain] if t > cutoff
            ]
...
class SiteConfigManager:
    ...
    def _save_config(self, config: SiteConfig) -> bool:
        ...
        try:
            ...
            if config_data.get("auth_credentials"):
                config_data["auth_credentials"] = self._encrypt_dict(
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                if row:
                    ...
                    if config_data.get("auth_credentials"):
                        config_data["auth_credentials"] = self._decrypt_dict(
                            config_data["auth_credentials"]
                        )
                    if config_data.get("cookies"):
                        config_data["cookies"] = self._decrypt_dict(
                            config_data["cookies"]
                        )
    ...
    def get_presets(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "reddit.com": {
                "rate_limit_requests": 60,
                "custom_headers": {"User-Agent": "SubscriptionBot/1.0"},
        ...
        try:
            ...
            with self.db.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    configs.append({"domain": row[0], "config": config_data})
