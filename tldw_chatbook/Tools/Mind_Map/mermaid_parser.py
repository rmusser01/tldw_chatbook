...
class MermaidMindmapParser:
    ...
    def parse(self, mermaid_code: str) -> Node:
        ...
        try:
            for line_number, line in enumerate(lines, 1):
                ...
                if not node_info:
                    logger.warning(
                        f"Line {line_number}: Could not parse '{line.strip()}'"
                    )
                    continue
            ...
            raise ValueError(
                f"Error parsing Mermaid mindmap at line {line_number}: {str(e)}"
            )
    ...
    def _find_parent_node(self, node_stack: List[tuple], indent_level: int) -> Node:
...
class ExtendedMermaidParser(MermaidMindmapParser):
    ...
    def _parse_node_line(self, line: str) -> Optional[Dict]:
        ...
        if icon_match:
            ...
            node_info["text"] = re.sub(
                self.EXTENDED_PATTERNS["icon"], "", node_info["text"]
            ).strip()
        ...
        if class_match:
            ...
            node_info["text"] = re.sub(
                self.EXTENDED_PATTERNS["class"], "", node_info["text"]
            ).strip()
    ...
    def _process_markdown(self, text: str) -> Text:
        ...
        while remaining:
            ...
            italic_match = re.search(
                self.EXTENDED_PATTERNS["markdown_italic"], remaining
            )
