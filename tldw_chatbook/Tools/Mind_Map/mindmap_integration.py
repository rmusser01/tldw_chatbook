...
class MindmapIntegration:
    ...
    def create_from_notes(
        self, note_ids: List[str], include_headers: bool = True
    ) -> Node:
    ...
    def create_from_search_results(
        self, query: str, results: Dict[str, List[Any]]
    ) -> Node:
    ...
    def create_study_mindmap(self, topic: str, note_ids: List[str]) -> Node:
        ...
        for note_id in note_ids:
            ...
            if note.get("content"):
                self._extract_study_content(
                    note["content"], concepts_node, questions_node
                )

        return root

    def _extract_study_content(
        self, content: str, concepts_node: Node, questions_node: Node
    ):
        ...
        for line in lines:
            ...
            if line.endswith("?") or line.lower().startswith(
