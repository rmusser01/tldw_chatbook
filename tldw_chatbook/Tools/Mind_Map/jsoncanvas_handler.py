...
class JSONCanvasHandler:
    ...
    def from_json_canvas(cls, canvas_data: str) -> Node:
        ...
        for node_data in nodes_data:
            ...
            node = Node(
                ...
                metadata={"type": node_type, "original_data": node_data},
            )
        ...
        for edge_data in edges_data:
            ...
            if from_node and to_node:
                if to_node not in parent_map:
                    ...
                    logger.warning(
                        f"Node {to_node} already has a parent, skipping edge from {from_node}"
                    )
    ...
    @classmethod
    def to_json_canvas(
        cls, root: Node, layout: str = "hierarchical", include_metadata: bool = True
    ) -> str:
    ...
    def _hierarchical_layout(cls, root: Node) -> Dict[Node, Tuple[int, int]]:
        ...
        for depth in sorted(levels.keys()):
            ...
            total_width = len(nodes_at_level) * (
                cls.DEFAULT_NODE_WIDTH + cls.NODE_PADDING_X
            )
    ...
    def _validate_node(cls, node: Dict, index: int) -> List[str]:
        ...
        if "color" in node:
            ...
            if not (
                ...
                or (isinstance(color, str) and color.startswith("#"))
            ):
