...
class MindmapExporter:
    ...
    def to_markdown(root: Node, include_metadata: bool = False) -> str:
        ...
        def render_node(node: Node, level: int):
            ...
            if level <= 6:
                ...
                lines.append(
                    f"{indent}{bullet} {node.text if hasattr(node, 'text') else node.name}"
                )
    ...
    def to_ascii_art(root: Node, max_width: int = 80) -> str:
        ...
        lines.append(
            f" MINDMAP: {root.text if hasattr(root, 'text') else root.name} ".center(
                max_width
            )
        )
        lines.append("=" * max_width)
    ...
    @staticmethod
    def to_flashcards(
        root: Node, card_type: str = "basic", include_context: bool = True
    ) -> List[Dict[str, Any]]:
            ...
            if node.parent and node != root:
                ...
                if include_context and node.parent.parent:
                    context = (
                        ...
                        if hasattr(node.parent.parent, "text")
                        else node.parent.parent.name
                    )
    ...
    @staticmethod
    def to_html(
        root: Node, include_style: bool = True, collapsible: bool = True
    ) -> str:
        ...
        lines.append(
            f"<title>{html.escape(root.text if hasattr(root, 'text') else root.name)}</title>"
        )
        ...
        lines.append(
            f"<h1>{html.escape(root.text if hasattr(root, 'text') else root.name)}</h1>"
        )
    ...
    @staticmethod
    def to_json_canvas(
        root: Node, layout: str = "hierarchical", include_metadata: bool = True
    ) -> str:
