...
class MindmapRenderer:
    ...
    def _render_node_tree(
        ...
        shape_symbol = (
            node.shape.value if hasattr(node, "shape") else NodeShape.DEFAULT.value
        )
            ...
            for i, child in enumerate(node.children):
                ...
                if node == self.model.root:
                    ...
                    if is_last:
                        ...
                        child_prefix = (
                            prefix + symbols["tree_vertical"] + symbols["space"] * 3
                        )
    ...
    def _render_node_sr(
        self, node: Node, lines: List[str], level: int, position: List[int]
    ) -> None:
