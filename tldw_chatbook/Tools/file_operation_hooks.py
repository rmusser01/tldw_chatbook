...
class FileOperationMonitor:
    ...
    async def _monitor_async_operation(
        self, func: Callable, operation_type: str, args: tuple, kwargs: dict
    ) -> Any:
    ...
    async def _monitor_sync_operation(
        self, func: Callable, operation_type: str, args: tuple, kwargs: dict
    ) -> Any:
    ...
    def _extract_operation_details(
        ...
        if operation_type == "Read":
            ...
            content_after = bound_args.arguments.get(
                "content", bound_args.arguments.get("data")
            )
            ...
            if operation_type == "Edit":
                ...
                content_after = self._apply_multi_edits(
                    content_before, bound_args.arguments.get("edits", [])
                )
    ...
    try:
        ...
        try:
            ...
            if hasattr(file_tools, "ReadFileTool"):
                ...
                file_tools.ReadFileTool.execute = _monitor.hook_function(
                    original_read, "Read"
                )
        ...
        try:
            ...
            if hasattr(file_tools, "WriteFileTool"):
                ...
                file_tools.WriteFileTool.execute = _monitor.hook_function(
        ...
        if not hook_attempts:
            logger.warning(
                "No file operations were hooked. Manual integration may be required."
            )
