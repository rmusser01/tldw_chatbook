...
class FileAuditSystem:
    ...
    async def analyze_change_alignment(
        self,
        file_path: str,
        content_before: Optional[str],
        content_after: str,
        ...
        analysis_prompt = self._build_analysis_prompt(
            file_path, content_before, content_after, user_prompt
        )
...
Focus on detecting deception and incomplete implementations, not just syntax errors or style issues."""
    ...
    async def record_file_operation(
        ...
        if (
            content_after is not None
            and prompt_to_use
            and operation_type in ["Write", "Edit", "MultiEdit"]
        ):
            try:
        ...
        record = FileAuditRecord(
            ...
            or (len(content_after.encode("utf-8")) if content_after else None),
        ...
        if deception_risk in ["HIGH", "CRITICAL"]:
            logger.warning(
                f"Potentially deceptive file operation detected: {operation_type} on {file_path}"
            )
    ...
    def get_recent_operations(
        self, hours: int = 24, risk_level: Optional[str] = None
    ...
    def get_deception_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for record in recent_records:
            ...
            if (
                ...
                and record.analysis_result != "No analysis performed"
            ):
                ...
                summary["high_risk_operations"].append(
                    {
                        ...
                        "user_prompt": record.user_prompt[:100] + "..."
                        if record.user_prompt and len(record.user_prompt) > 100
...
class CodeAuditTool(Tool):
    ...
    async def execute(
        ...
        try:
            ...
            if audit_type == "recent_changes":
                return await self._audit_recent_changes(
                    timeframe_hours, risk_filter, operation_filter
                )
    ...
    def _parse_audit_type(self, prompt: str) -> str:
        ...
        if "recent" in prompt_lower and "change" in prompt_lower:
            ...
            "todo" in prompt_lower
            or "incomplete" in prompt_lower
            or "fixme" in prompt_lower
            or "placeholder" in prompt_lower
        ):
            return "incomplete_analysis"
        else:
            return "comprehensive"

    async def _audit_recent_changes(
        self, hours: int, risk_filter: Optional[str], operation_filter: Optional[str]
    ) -> Dict[str, Any]:
        ...
        if operation_filter:
            recent_ops = [
                op for op in recent_ops if op.operation_type == operation_filter
            ]
            ...
            changes.append(
                {
                    ...
                    "user_prompt": op.user_prompt[:100] + "..."
                    if op.user_prompt and len(op.user_prompt) > 100
                    else op.user_prompt,
    ...
    async def _analyze_high_risk_operations(self, hours: int) -> Dict[str, Any]:
        ...
        high_risk_ops = [
            op for op in high_risk_ops if op.deception_risk in ["HIGH", "CRITICAL"]
        ]
        ...
        analysis = {
            ...
            "critical_operations": len(
                [op for op in high_risk_ops if op.deception_risk == "CRITICAL"]
            ),
            "high_risk_operations": len(
                [op for op in high_risk_ops if op.deception_risk == "HIGH"]
            ),
            "operations": [],
        }
    ...
    async def _analyze_incomplete_implementations(self, hours: int) -> Dict[str, Any]:
        ...
        for op in recent_ops:
            if op.analysis_result and op.analysis_result != "No analysis performed":
                ...
                if (
                    "INCOMPLETE_IMPLEMENTATION: YES" in analysis_upper
    ...
    async def _comprehensive_audit(
        self, hours: int, risk_filter: Optional[str]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_risk_assessment": self._assess_overall_deception_risk(
                deception_report["summary"]
            ),
        }
    ...
    def _generate_deception_recommendations(self, summary: Dict[str, Any]) -> List[str]:
        ...
        if critical_count > 0:
            recommendations.append(
                f"URGENT: {critical_count} potentially deceptive operations detected. Review immediately."
            )

        if high_count > 0:
            recommendations.append(
                f"WARNING: {high_count} high-risk deception indicators found. Manual review recommended."
            )

        if analyzed_ops < total_ops * 0.5:
            recommendations.append(
                "Many operations lack user prompt context for proper deception analysis. Consider improving prompt tracking."
            )
        ...
        if not recommendations:
            recommendations.append(
                "No significant deception indicators detected in the reviewed timeframe."
            )
    ...
    def _analyze_operation_risk(self, operation: FileAuditRecord) -> str:
        ...
        if (
            ...
            and operation.analysis_result != "No analysis performed"
        ):
