...
class SyncStateRepository(BaseDB):
    ...
    def mark_sync_v2_outbox_push_results(
        ...
        *,
        ...
        accepted_by_id = {
            str(item["client_envelope_id"]): dict(item)
        ...
        retained = 0
        with self._get_connection() as conn:
            for client_envelope_id, acceptance in sorted(accepted_by_id.items()):
                row = conn.execute(
                    "SELECT envelope FROM sync_v2_local_outbox "
                    "WHERE source_scope_key = ? AND dataset_id = ? "
                    "AND client_envelope_id = ?",
                    (source_scope_key, dataset_id, client_envelope_id),
                ).fetchone()
                server_cursor = acceptance.get("server_cursor")
                if server_cursor is None:
                    server_cursor = acceptance.get("server_sequence")
                if row is not None and server_cursor is not None:
                    envelope = self._parse_outbox_envelope(json.loads(row["envelope"]))
                    object_revision = acceptance.get("object_revision")
