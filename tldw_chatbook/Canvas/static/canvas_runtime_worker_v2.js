/* Canvas V2 candidate: private diagrams and authored code execute only in QuickJS.
 * Frozen V1 remains in canvas_runtime_worker.js; keep this closure separate. */
...
function validateDiagramRecords(value) {
  if (!ownRecord(value.source_identity, ["source_bytes", "sha256"]) ||
      !Number.isSafeInteger(value.source_identity.source_bytes) ||
      value.source_identity.source_bytes < 0 || value.source_identity.source_bytes > 524288 ||
      typeof value.source_identity.sha256 !== "string" ||
      !/^[0-9a-f]{64}$/.test(value.source_identity.sha256) ||
      typeof value.profile_manifest_sha256 !== "string" ||
      !/^[0-9a-f]{64}$/.test(value.profile_manifest_sha256) ||
      !Array.isArray(value.diagrams) || value.diagrams.length > 4) throw new Error("diagram-schema");
  const declarations = [], stack = [value.root], ids = new Set();
  let count = 0;
  while (stack.length) {
    const node = stack.pop();
    if (!ownRecord(node, ["node_id", "tag", "attributes", "text", "children"]) ||
        !boundedIdentifier(node.node_id) || ids.has(node.node_id) ||
        !Array.isArray(node.attributes) || !Array.isArray(node.children) ||
        ++count > 1800) throw new Error("diagram-tree");
    ids.add(node.node_id);
    const names = new Set();
    let kind = null;
    for (const pair of node.attributes) {
      if (!Array.isArray(pair) || pair.length !== 2 || typeof pair[0] !== "string" ||
          typeof pair[1] !== "string" || names.has(pair[0])) throw new Error("diagram-attributes");
      names.add(pair[0]);
      if (pair[0] === "data-canvas-diagram") kind = pair[1];
    }
    if (kind !== null) declarations.push({node, kind});
    stack.push(...node.children.slice().reverse());
  }
  if (declarations.length !== value.diagrams.length) throw new Error("diagram-count");
  let bytes = 0;
  value.diagrams.forEach((record, ordinal) => {
    if (!ownRecord(record, ["ordinal", "target_node_id", "kind", "source"]) ||
        record.ordinal !== ordinal || record.kind !== "mermaid" ||
        !boundedString(record.source, 8192) || !record.source.trim()) throw new Error("diagram-record");
    bytes += encoder.encode(record.source).byteLength;
    const {node, kind} = declarations[ordinal];
    if (bytes > 16384 || kind !== record.kind || node.node_id !== record.target_node_id ||
        node.tag !== "pre" || node.text !== null ||
        node.children.some(child => child.tag !== "#text" || child.children.length || typeof child.text !== "string") ||
        node.children.map(child => child.text).join("") !== record.source) throw new Error("diagram-target");
  });
}

async function verifiedDiagramSource(value, data) {
  if (!ownRecord(data, ["manifest", "library", "source"]) ||
      !boundedString(data.manifest, 262144) || !boundedString(data.library, 262144) ||
      !boundedString(data.source, 524288)) throw new Error("runtime-data-schema");
  const digest = async text => Array.from(new Uint8Array(
    await crypto.subtle.digest("SHA-256", encoder.encode(text))
  ), byte => byte.toString(16).padStart(2, "0")).join("");
  if (await digest(data.manifest) !== value.profile_manifest_sha256 ||
      encoder.encode(data.source).byteLength !== value.source_identity.source_bytes ||
      await digest(data.source) !== value.source_identity.sha256) throw new Error("runtime-data-identity");
  const manifest = JSON.parse(data.manifest), library = JSON.parse(data.library);
  if (manifest.runtime_profile !== "canvas-v2-mermaid-1" || manifest.schema_version !== 1 ||
      !ownRecord(manifest.library, ["bytes", "sha256"]) ||
      encoder.encode(data.library).byteLength !== manifest.library.bytes ||
      await digest(data.library) !== manifest.library.sha256 ||
      !ownRecord(library, ["schema_version", "profile_id", "source", "source_bytes", "source_sha256", "inputs_sha256", "inventory"]) ||
      library.profile_id !== value.runtime_profile || library.schema_version !== 1 ||
      !boundedString(library.source, 262144) ||
      encoder.encode(library.source).byteLength !== library.source_bytes ||
      library.source_bytes !== manifest.mermaid_candidate.source_bytes ||
      library.source_sha256 !== manifest.mermaid_candidate.source_sha256 ||
      await digest(library.source) !== library.source_sha256) throw new Error("library-integrity");
  const scriptBytes = value.scripts.reduce((total, script) => {
    if (typeof script !== "string") throw new Error("script-type");
    return total + encoder.encode(script).byteLength;
  }, library.source_bytes);
  if (scriptBytes > 262144) throw new Error("script-limit");
...
function failureCode(error) {
  ...
  // Elapsed time alone does not classify an unexpected native engine exception.
  if (error?.interrupted || interrupted) return "runtime-timeout";
...
function runStartup(operationId) {
  ...
  const library = handleEval(librarySource, "canvas-private-diagrams.js");
  librarySource = null;
  const render = context.getProp(library, "renderDiagrams");
  const records = context.newString(JSON.stringify(plan.diagrams));
  const apply = context.getProp(virtualControls, "renderDiagrams");
  try {
...
(() => {
  ...
  controls.renderDiagrams = (render, encoded) => {
    const records = safeJsonParse(encoded);
    // The verified library is private and runs before authored code. Prepare all
    // scenes before touching virtual nodes, then use the existing typed facade.
    const scenes = render(records);
    function build(scene, namespace = HTML) {
      if (scene.tag === "svg") namespace = SVG;
      const node = namespace === SVG
        ? document.createElementNS(SVG, scene.tag) : document.createElement(scene.tag);
      for (const [name, value] of scene.attributes) {
        if (name === "style") {
          for (const declaration of value.split(";")) {
      ...
      if (scene.text) node.textContent = scene.text;
      for (const child of scene.children) node.appendChild(build(child, namespace));
      return node;
    }
...
async function prepare(value, runtimeData) {
  validatePlan(value);
  librarySource = await verifiedDiagramSource(value, runtimeData);
  plan = value;
...
self.onmessage = async (event) => {
  ...
  try {
    if (ownRecord(message, ["type", "plan", "runtime_data"]) && message.type === "prepare" && !prepared) {
      await prepare(message.plan, message.runtime_data);
      return;
    }
