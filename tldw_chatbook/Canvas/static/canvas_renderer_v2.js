...
function validatePlan(plan) {
  if (!ownRecord(plan, ["runtime_profile", "source_identity", "root", "assets", "css_rules", "scripts", "diagrams", "profile_manifest_sha256"])) {
    throw new Error("render plan schema");
  }
  if (plan.runtime_profile !== "canvas-v2-mermaid-1") throw new Error("runtime profile");
  validateDiagramRecords(plan);
...
function handleWorkerMessage(event) {
  ...
  if (message.type === "bootstrap-ready" && ownRecord(message, ["type"])) {
    ...
    worker.postMessage({type: "prepare", plan: pendingPlan, runtime_data: pendingRuntimeData});
    pendingPlan = null;
    return;
  }
...
async function start(plan, runtimeData) {
  try {
    validatePlan(plan);
    await verifiedDiagramSource(plan, runtimeData);
    pendingRuntimeData = runtimeData;
  ...
  const workerModuleUrl = new URL("./canvas_runtime_worker_v2.js", import.meta.url).href;
  ...
  worker = new Worker(workerBootstrapUrl, {
    type: "module",
    name: "chatbook-canvas-v2-mermaid-1",
  });
...
window.addEventListener("message", (event) => {
  ...
  if (
    ...
    !ownRecord(event.data, ["type", "nonce", "plan", "runtime_data"]) ||
  ...
  channel.start();
  void start(event.data.plan, event.data.runtime_data);
});
