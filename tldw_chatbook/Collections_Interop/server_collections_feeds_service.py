...
class ServerCollectionsFeedsService:
    ...
    async def list_feeds(
        ...
        *,
        ...
        return self._dump(
            await self._require_client().list_collections_feeds(
    ...
    async def get_feed(self, feed_id: int) -> dict[str, Any]:
        ...
        return self._dump(
            await self._require_client().get_collections_feed(int(feed_id))
        )
    ...
    async def update_feed(
        ...
        *,
        ...
        return self._dump(
            await self._require_client().update_collections_feed(int(feed_id), request)
        )
    ...
    async def subscribe_feed_websub(
        self, feed_id: int, *, lease_seconds: int | None = None
    ) -> dict[str, Any]:
        # Deferred import: avoid module-scope tldw_api schema import (task-285 phase 2).
        ...
        return self._dump(
            await self._require_client().subscribe_collections_feed_websub(
                int(feed_id), request
            )
        )
    ...
    async def get_feed_websub_status(self, feed_id: int) -> dict[str, Any]:
        ...
        return self._dump(
            await self._require_client().get_collections_feed_websub(int(feed_id))
        )

    async def unsubscribe_feed_websub(self, feed_id: int) -> dict[str, Any]:
        self._enforce("collections.feeds.websub.delete.server")
        return self._dump(
            await self._require_client().unsubscribe_collections_feed_websub(
                int(feed_id)
            )
        )
