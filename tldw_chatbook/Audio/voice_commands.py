...
class VoiceCommandProcessor:
    ...
    BUILTIN_COMMANDS = [
        ...
        # Punctuation
        VoiceCommand(
            "comma", "insert_comma", CommandType.PUNCTUATION, parameters={"text": ","}
        ),
        VoiceCommand(
            "period", "insert_period", CommandType.PUNCTUATION, parameters={"text": "."}
        ),
        VoiceCommand(
            "question mark",
            "insert_question",
        ...
        VoiceCommand(
            "colon", "insert_colon", CommandType.PUNCTUATION, parameters={"text": ":"}
        ),
        VoiceCommand(
            "open quote",
            "insert_open_quote",
    ...
    REGEX_COMMANDS = [
        ...
        (r"replace (.+) with (.+)", "replace_text", CommandType.TEXT_MANIPULATION),
        # "Go to line [number]"
        (r"go to line (\d+)", "goto_line", CommandType.APP_NAVIGATION),
        # "Select from [start] to [end]"
        (r"select from (.+) to (.+)", "select_range", CommandType.TEXT_MANIPULATION),
    ]
    ...
    def __init__(self):
        ...
        for pattern, action, cmd_type in self.REGEX_COMMANDS:
            self.regex_patterns.append(
                (re.compile(pattern, re.IGNORECASE), action, cmd_type)
            )
    ...
    def delete_custom_command(self, phrase: str):
        ...
        custom_commands = get_cli_setting("dictation.custom_commands", [])
        custom_commands = [
            cmd for cmd in custom_commands if cmd["phrase"].lower() != phrase.lower()
        ]
        save_setting_to_cli_config("dictation", "custom_commands", custom_commands)
