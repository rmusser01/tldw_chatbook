...
class DictationPerformanceMonitor:
    ...
    def start_session(
        self, session_id: str, provider: str, model: str = "", language: str = "en"
    ) -> DictationMetrics:
    ...
    def record_transcription(
        self, text: str, processing_time: float, audio_size: int = 0
    ):
    ...
    def record_resource_usage(self, cpu_percent: float, memory_mb: float):
        ...
        if self.current_session:
            self.current_session.peak_cpu_percent = max(
                self.current_session.peak_cpu_percent, cpu_percent
            )
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        total_latency = sum(
            s.average_latency for s in self.sessions if s.average_latency > 0
        )
        ...
        for session in self.sessions:
            provider_counts[session.provider] = (
                provider_counts.get(session.provider, 0) + 1
            )
        ...
        avg_efficiency = statistics.mean(
            s.calculate_efficiency() for s in self.sessions
        )
        ...
        for provider, data in provider_metrics.items():
            if data["total_duration"] > 0:
                comparison[provider] = {
                    ...
                    "average_latency": statistics.mean(data["latencies"])
                    if data["latencies"]
                    else 0,
    ...
    def get_time_analysis(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for session in recent_sessions:
            ...
            if hour_key not in hourly_stats:
                hourly_stats[hour_key] = {"sessions": 0, "words": 0, "duration": 0}
        ...
        return {
            ...
            "peak_usage_hour": max(
                hourly_stats.items(), key=lambda x: x[1]["sessions"]
            )[0],
    ...
    def _load_statistics(self):
        ...
        for stat_dict in stats_data[-self.history_size :]:
            try:
                ...
                stat_dict["start_time"] = datetime.fromisoformat(
                    stat_dict["start_time"]
                )
                if stat_dict.get("end_time"):
                    stat_dict["end_time"] = datetime.fromisoformat(
                        stat_dict["end_time"]
                    )
