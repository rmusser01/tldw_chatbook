...
class LiveDictationService:
    ...
    def __init__(
        ...
        # Initialize services
        try:
            module_audio_cls = getattr(
                recording_service_module, "AudioRecordingService", AudioRecordingService
            )
            audio_service_cls = (
            ...
            self.audio_service = audio_service_cls(
                backend=audio_backend, use_vad=True, vad_aggressiveness=2
            )
        except AudioRecordingError as e:
            logger.error(f"Failed to initialize audio service: {e}")
            raise

        module_transcription_cls = getattr(
            transcription_service_module, "TranscriptionService", TranscriptionService
        )
        ...
        logger.info(
            f"LiveDictationService initialized with provider: {transcription_provider}"
        )
    ...
    def start_dictation(
        ...
        try:
            ...
            success = self.audio_service.start_recording(callback=self._audio_callback)
    ...
    def _processing_loop(self):
        ...
        while True:
            try:
                ...
                if accumulated_audio and (current_time - last_process_time) >= (
        ...
        try:
            ...
            if self.streaming_transcriber:
                try:
                    ...
                    logger.warning(
                        f"Streaming transcription failed, falling back to buffer transcription: {e}"
                    )
                    result = None
                ...
                if isinstance(result, dict):
                    if result.get("partial"):
                        self._handle_partial_transcript(
                            result["partial"], normalize_punctuation=True
                        )
                    if result.get("final"):
            ...
            if not self.streaming_transcriber or not isinstance(result, dict):
                ...
                try:
                    ...
                    if result and result.get("text"):
                        self._handle_partial_transcript(
                            result["text"], normalize_punctuation=True
                        )
    ...
    def _handle_partial_transcript(
        self, text: str, *, normalize_punctuation: bool = False
    ):
    ...
    def stop_dictation(self) -> DictationResult:
        ...
        try:
            ...
            logger.info(
                f"Dictation stopped. Duration: {duration:.1f}s, Words: {len(full_transcript.split())}"
            )
