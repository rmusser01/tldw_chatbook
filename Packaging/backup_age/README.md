# Recovery age helper source qualification

This directory implements protocol v1 under [ADR-126](../../backlog/decisions/126-complete-local-backup-and-recovery.md).
It does **not** deliver an installed encryption capability. The unbuilt application
reports `helper_unavailable`. Native packaging, release integrity, wheel/source
installation, and the supported platform matrix are the next task's release gate.
Do not copy a PATH executable into place or fall back to plaintext.
...
Python resolves only its package's `Backup_Recovery/_age` directory. The root
contains `backup-age` (`backup-age.exe` on Windows) and `manifest.json` with the
info fields plus lowercase SHA-256 `sha256`. Manifest data is strict, bounded to
4 KiB, and rejects unknown fields. The binary must be a regular executable of at
most 32 MiB with matching platform, digest, and runtime `info`. No manifest value
selects an executable path. Package installation is the trust boundary; this is
integrity verification, not protection against a compromised local installer.
...
Source qualification ran on macOS arm64, Python 3.12.11, Go 1.26.2 with
...
source-fixture evidence only, not an advertised end-user platform matrix or proof
of other Python/platform combinations.
