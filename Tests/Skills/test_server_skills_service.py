...
def test_server_skills_service_direct_client_takes_precedence_over_provider():
    client = object()
    provider = ExplodingClientProvider()
    service = ServerSkillsService(client=client, client_provider=provider)

    assert service._require_client() is client
    assert provider.build_calls == 0


def test_server_skills_service_from_server_context_provider_is_lazy():
    client = object()
    provider = FakeClientProvider(client)
    service = ServerSkillsService.from_server_context_provider(provider)

    assert isinstance(service, ServerSkillsService)
...
def test_server_skills_service_re_resolves_provider_without_service_local_client_cache():
...
def test_server_skills_service_from_config_returns_provider_backed_service():
    service = ServerSkillsService.from_config(
