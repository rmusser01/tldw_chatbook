...
class TestParseCsvUrlsPandasGuard:
    ...
    def test_returns_empty_dict_when_find_spec_true_but_import_fails(
...
class TestURLValidation:
    ...
    async def test_scrape_article_valid_url(self):
        ...
        # Mock the config
        with patch(
            "tldw_chatbook.Web_Scraping.Article_Extractor_Lib.load_and_log_configs"
        ) as mock_config:
            ...
            with patch(
                "tldw_chatbook.Web_Scraping.Article_Extractor_Lib.async_playwright"
            ) as mock_playwright:
...
class TestSecurityMeasures:
    ...
    def test_sql_like_pattern_escaping(self):
        ...
        assert (
            escape_sql_like_pattern("'; DROP TABLE cookies; --")
            == "'; DROP TABLE cookies; --"
        )
    ...
    def test_secure_temp_file_usage(self):
        ...
        with patch(
            "tldw_chatbook.Web_Scraping.Article_Extractor_Lib.get_temp_manager"
        ) as mock_manager:
            ...
            from tldw_chatbook.Web_Scraping.Article_Extractor_Lib import (
                generate_temp_sitemap_from_links,
            )
...
class TestErrorHandling:
    ...
    def test_scrape_and_no_summarize_invalid_url(self):
        ...
        result = scrape_and_no_summarize_then_ingest(
            "invalid-url", ["test", "keywords"], "Test Title"
        )
