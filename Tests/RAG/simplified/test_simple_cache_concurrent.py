        ...
        def worker(worker_id, operation_count=10):
            ...
            try:
                for i in range(operation_count):
                    ...
                    # Get operation
                    result = cache.get(query=key, search_type="test", top_k=10)
        ...
        async def async_worker(worker_id):
            ...
            for i in range(10):
                ...
                result = await cache.get_async(
                    f"async_query_{worker_id}_{i}", "test", 10
                )
                assert result is not None
...
class TestErrorPaths:
    ...
    def test_memory_pressure_handling(self):
        ...
        cache = SimpleRAGCache(
            max_size=100, ttl_seconds=60, max_memory_mb=0.1
        ...
        def interruptible_worker():
            ...
            try:
                ...
                while not stop_flag.is_set():
                    cache.put(
                        f"interrupt_query_{i}", "test", 10, [{"id": i}], f"Context {i}"
                    )
                    i += 1
                    if i % 10 == 0:
...
class TestRaceConditions:
    ...
    def test_concurrent_expiry_and_access(self):
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = [executor.submit(reader), executor.submit(pruner)]
            for future in as_completed(futures):
                future.result()
    ...
    def test_stats_calculation_race(self):
        ...
        def modifier():
            ...
            for i in range(50):
                try:
                    if i % 3 == 0:
                        cache.put(
                            f"stats_query_{i}", "test", 10, [{"id": i}], f"Context {i}"
                        )
                    elif i % 3 == 1:
        ...
        # Run concurrent operations
        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = [executor.submit(modifier), executor.submit(stats_reader)]
            results = []
            for future in as_completed(futures):
