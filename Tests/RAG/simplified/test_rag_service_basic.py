...
class TestRAGServiceBasics:
    ...
    async def test_index_empty_document(self, test_rag_config):
        ...
        with pytest.raises(ValueError, match="content must be a non-empty string"):
            await service.index_document(
                doc_id="empty_doc", content="", title="Empty Document"
            )
    ...
    async def test_index_large_document(self, test_rag_config):
        ...
        large_content = " ".join(
            ["This is sentence number " + str(i) + "." for i in range(100)]
        )
    ...
    async def test_search_basic(self, test_rag_config):
        ...
        results = await service.search(
            query="Python programming", search_type="semantic", top_k=5
        )
    ...
    async def test_get_stats(self, test_rag_config):
        ...
        # Index a document
        await service.index_document(
            doc_id="stats_test", content="Test content for stats"
        )
    ...
    def test_indexing_result_to_dict(self):
        ...
        result = IndexingResult(
            doc_id="test", chunks_created=5, time_taken=1.23, success=True, error=None
        )
...
class TestRAGServiceSearch:
    ...
    async def test_semantic_search(self, test_rag_config):
        ...
        # Semantic search
        results = await service.search(
            query="AI and neural networks", search_type="semantic", top_k=10
        )
    ...
    async def test_keyword_search(self, test_rag_config):
        ...
        await service.index_document(
            doc_id="kw2", content="The cat sat on the mat.", title="Cat Story"
        )

        # Keyword search for exact term
        results = await service.search(query="fox", search_type="keyword", top_k=10)
    ...
    async def test_hybrid_search(self, test_rag_config):
        ...
        # Hybrid search
        results = await service.search(
            query="Python tutorial", search_type="hybrid", top_k=10
        )
...
class TestRAGServiceCache:
    ...
    async def test_cache_hit(self, test_rag_config):
        ...
        # Index a document
        await service.index_document(
            doc_id="cache_test", content="Content for cache testing"
        )
    ...
    async def test_clear_cache(self, test_rag_config):
        ...
        await service.index_document(doc_id="clear_cache_test", content="Test content")
...
class TestRAGServiceIntegration:
    ...
    async def test_full_workflow(self, memory_rag_config):
        ...
        sem_results = await service.search(
            "data science programming", search_type="semantic", top_k=5
        )
        assert len(sem_results) > 0

        # Keyword search (not implemented yet, returns empty)
        kw_results = await service.search("Python", search_type="keyword", top_k=5)
        assert len(kw_results) == 0  # Keyword search not implemented

        # Filtered search
        ml_results = await service.search(
            "learning", search_type="semantic", filter_metadata={"category": "ml"}
        )
...
class TestRAGServicePerformance:
    ...
    async def test_search_performance(self, memory_rag_config):
        ...
        for i in range(10):
            start = time.time()
            results = await service.search(f"topic {i}", search_type="hybrid", top_k=20)
...
class TestRAGProfiles:
    ...
    def test_profile_features(self, memory_rag_config):
        ...
        enhanced_service = create_rag_service(
            "hybrid_enhanced", config=memory_rag_config
        )
