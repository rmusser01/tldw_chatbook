...
def simple_chunking_service():
    ...
    class SimpleChunkingService:
        def chunk_text(
            self, text: str, chunk_size: int = 100, chunk_overlap: int = 20
        ) -> List[Dict[str, Any]]:
            if not text:
                return []
...
def test_rag_config(chroma_persist_dir, request):
    ...
    return RAGConfig(
        embedding=EmbeddingConfig(
            model="mock", device="cpu", cache_size=100, batch_size=10
        ),
        ...
        chunking=ChunkingConfig(
            chunk_size=100, chunk_overlap=20, chunking_method="words"
        ),
        search=SearchConfig(
            default_top_k=5,
            score_threshold=0.5,
...
def memory_rag_config(request):
    ...
    return RAGConfig(
        ...
        vector_store=VectorStoreConfig(
            type="memory", collection_name=unique_collection, distance_metric="cosine"
        ),
        chunking=ChunkingConfig(
            chunk_size=100, chunk_overlap=20, chunking_method="words"
        ),
        search=SearchConfig(default_top_k=10, enable_cache=False),
    )
...
"""
                ...
                content = (
                    f"Document {i} contains various information about topic {i}. " * 10
                )
...
pytest.mark.requires_rag_deps = pytest.mark.skipif(
    not has_rag_deps, reason="RAG dependencies not available"
)

pytest.mark.requires_chromadb = pytest.mark.skipif(
    not has_rag_deps, reason="ChromaDB not available"
)
