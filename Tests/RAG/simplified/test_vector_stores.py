...
try:
        ...
        def search(
            ...
            for doc_id, embedding in self.embeddings.items():
                ...
                distance = compute_similarity(
                    query_embedding, embedding, self.distance_metric
                )
                results.append(
                    {
                        "id": doc_id,
    ...
    def compute_similarity(
        vec1: List[float], vec2: List[float], metric: str = "cosine"
    ) -> float:
...
class TestChromaVectorStore:
    ...
    def test_persistence_directory(self, temp_dir):
        ...
        # Add documents
        store.add_documents(
            ["doc1"], ["Document 1"], [{"test": True}], [[0.1, 0.2, 0.3]]
        )
    ...
    def test_with_real_chromadb_mock(self, mock_client, temp_dir):
        ...
        result = store.add_documents(
            ["doc1"], ["Test document"], [{"key": "value"}], [[0.1, 0.2, 0.3]]
        )
...
class TestVectorStoreFactory:
    ...
    def test_create_in_memory_store(self):
        ...
        store = create_vector_store(
            "in_memory", collection_name="test", distance_metric="cosine"
        )
...
class TestVectorStoreEdgeCases:
    ...
    def test_update_nonexistent(self):
        ...
        result = store.update(["non_existent"], documents=["New content"])
    ...
    def test_empty_metadata_filter(self):
        ...
        store.add_documents(
            ["doc1", "doc2"], ["Doc 1", "Doc 2"], [{"a": 1}, {"b": 2}], [[1, 0], [0, 1]]
        )
...
class TestVectorStoreIntegration:
    ...
    def test_full_workflow(self):
        ...
        initial_embeddings = [[1, 0, 0, 0], [0.7, 0.7, 0, 0], [0, 0, 1, 0]]
        ...
        prog_results = store.search(
            [1, 0, 0, 0], top_k=10, filter_dict={"category": "programming"}
        )
    ...
    def test_batch_operations(self):
        ...
        embeddings = [
            [i / 100, (i + 1) / 100, (i + 2) / 100] for i in range(batch_size)
        ]
        ...
        results = store.search([0.5, 0.5, 0.5], top_k=20, filter_dict={"batch": 5})
    ...
    def test_multiple_collections(self, temp_dir):
        ...
        store1.add_documents(["doc1"], ["Collection 1 data"], [{"col": 1}], [[1, 0]])

        store2.add_documents(["doc1"], ["Collection 2 data"], [{"col": 2}], [[0, 1]])
...
class TestVectorStorePerformance:
    ...
    def test_large_scale_operations(self, performance_timer):
        ...
        with performance_timer.measure("add_1000_docs") as timer:
            ...
            embeddings = [
                [
                    float(j + i) / (embedding_dim * num_docs)
        ...
        with performance_timer.measure("filtered_search") as timer:
            results = store.search(
                query_embedding, top_k=20, filter_dict={"category": 5}
            )
