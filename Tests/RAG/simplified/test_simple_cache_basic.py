...
class TestSimpleRAGCacheBasic:
    ...
    def test_different_parameters_different_keys(self):
        ...
        cache.put(
            "query", "semantic", 10, ["result2"], "context2", None
        )  # Different top_k
        cache.put("query", "hybrid", 5, ["result3"], "context3", None)  # Different type
        cache.put(
            "other query", "semantic", 5, ["result4"], "context4", None
        )  # Different query
    ...
    def test_with_filters(self):
        ...
        cache.put(
            "query", "semantic", 5, ["python_results"], "python_context", filters1
        )
        cache.put("query", "semantic", 5, ["java_results"], "java_context", filters2)

        # Should be cached separately
        assert cache.get("query", "semantic", 5, filters1) == (
            ["python_results"],
            "python_context",
        )
        assert cache.get("query", "semantic", 5, filters2) == (
            ["java_results"],
            "java_context",
        )
        assert (
            cache.get("query", "semantic", 5, None) is None
        )  # No filters = different key
