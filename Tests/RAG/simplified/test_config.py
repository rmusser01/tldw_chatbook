...
try:
    ...
    def create_config_for_collection(
        collection_name: str, persist_dir: Optional[Path] = None, **kwargs
    ) -> RAGConfig:
        ...
        defaults = {
            "media": {"chunk_size": 500, "chunk_overlap": 100, "top_k": 10},
            "conversations": {"chunk_size": 300, "chunk_overlap": 50, "top_k": 15},
            "notes": {"chunk_size": 400, "chunk_overlap": 80, "top_k": 12},
        }
    ...
    def load_config_from_settings(settings: Dict[str, Any]) -> Dict[str, RAGConfig]:
        ...
        collections = rag_settings.get(
            "collections", ["media", "conversations", "notes"]
        )
        for collection in collections:
            collection_settings = rag_settings.get(collection, {})
            configs[collection] = create_config_for_collection(
                collection, base_persist_dir, **collection_settings
            )
...
class TestRAGConfig:
    ...
    def test_config_creation_minimal(self):
        ...
        config = RAGConfig(collection_name="test", persist_directory="/tmp/test")
...
class TestConfigFactoryFunctions:
    ...
    def test_create_config_for_testing_override(self):
        ...
        config = create_config_for_testing(
            collection_name="custom_test", chunk_size=200, enable_cache=True
        )
    ...
    def test_load_config_from_settings_with_overrides(self):
        ...
        settings = {
            "rag": {
                ...
                "media": {"chunk_size": 1000, "embedding_model": "large-model"},
                "conversations": {"chunk_size": 200, "enable_cache": False},
            }
        }
...
class TestConfigValidation:
    ...
    def test_validate_missing_collection_name(self):
        ...
        config = RAGConfig(collection_name="", persist_directory="/tmp/test")
    ...
    def test_validate_invalid_chunk_size(self):
        ...
        config = RAGConfig(
            collection_name="test", persist_directory="/tmp/test", chunk_size=0
        )
...
class TestConfigEnvironmentVariables:
    ...
    def test_config_from_env_embedding_settings(self):
        ...
        config = create_config_for_collection(
            "test", embedding_provider=provider, embedding_model=model
        )
...
class TestConfigProperties:
    ...
    def test_config_serialization_properties(self, config_dict):
        ...
        config_dict["chunk_overlap"] = min(
            config_dict["chunk_overlap"], config_dict["chunk_size"] - 1
        )
...
class TestConfigEdgeCases:
    ...
    def test_config_with_unicode_paths(self):
        ...
        config = RAGConfig(
            collection_name="test", persist_directory="/tmp/测试/тест/🔍"
        )
    ...
    def test_config_with_very_long_paths(self):
        ...
        config = RAGConfig(collection_name="test", persist_directory=long_path)
    ...
    def test_config_with_special_collection_names(self):
        ...
        for name in special_names:
            config = RAGConfig(collection_name=name, persist_directory="/tmp/test")
