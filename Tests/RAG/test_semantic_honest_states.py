...
class TestResolveSemanticRagService:
    ...
    def test_factory_raising_maps_to_init_failed(self, monkeypatch):
        ...
        _factory(
            monkeypatch,
            lambda profile_name=None: (_ for _ in ()).throw(RuntimeError("boom")),
        )
...
class TestSearchSemanticHonestStates:
    ...
    def test_deps_missing_records_reason_and_does_no_heavy_work(self, monkeypatch):
        ...
        assert (
            state["message"]
            == SEMANTIC_UNAVAILABLE_MESSAGES[SEMANTIC_REASON_DEPS_MISSING]
        )
    ...
    def test_init_failure_records_reason_instead_of_crashing(self, monkeypatch):
        ...
        _factory(
            monkeypatch,
            lambda profile_name=None: (_ for _ in ()).throw(RuntimeError("no model")),
        )

        diagnostics: Dict[str, Any] = {}
        results = asyncio.run(
            pfs.search_semantic(app, "query", {}, diagnostics=diagnostics)
        )
    ...
    def test_search_raise_in_semantic_pipeline_degrades_not_crashes(self):
        ...
        app = SimpleNamespace(
            media_db=None, db_config={}, _rag_service=RaisingRagService()
        )
        diagnostics: Dict[str, Any] = {}
...
class TestRetrieveStepSplatRegression:
    ...
    def test_retrieve_step_forwards_only_supported_kwargs(self):
        ...
        assert (
            context["diagnostics"][SEMANTIC_DIAGNOSTICS_KEY]["status"]
            == SEMANTIC_STATUS_OK
        )
...
class TestHybridFtsOnlyIndication:
    ...
    class _FakeMediaDB:
        @staticmethod
        def search_media_db(
            search_query, search_fields, page, results_per_page, include_trash
        ):
            return ([{"id": 1, "title": "Doc", "content": "keyword content"}], 1)

    def test_parallel_step_records_search_error_when_vector_leg_raises(
        self, monkeypatch
    ):
        ...
        monkeypatch.setitem(
            pbs.RETRIEVAL_FUNCTIONS, "search_semantic", raising_semantic
        )
    ...
    def test_hybrid_search_records_deps_missing_but_keeps_fts_results(
        self, monkeypatch
    ):
        from tldw_chatbook.Event_Handlers.Chat_Events.chat_rag_events import (
...
class TestChatSidebarHonesty:
    ...
    def test_custom_pipeline_fts_only_uses_keyword_only_wording(self, monkeypatch):
        ...
        async def fake_custom(
            app_arg, query, sources, pipeline_id, diagnostics=None, **kwargs
        ):
