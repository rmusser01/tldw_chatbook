...
from tldw_chatbook.Model_Artifacts import ArtifactInUseError, ArtifactRef
...
class _Service:
    ...
    def delete(self, reference: ArtifactRef) -> None:
        ...
        if self.lease.close_count == 0:
            raise ArtifactInUseError("artifact is in use")
...
async def test_legacy_blank_llamafile_handler_schedules_embedded_static_worker(
    ...
    widgets: dict[str, object] = {
        "#llamafile-exec-path": _InputWidget(value="/private/llamafile"),
...
@pytest.mark.asyncio
async def test_handler_prefers_explicit_source_snapshot_without_model_input() -> None:
    widgets: dict[str, object] = {
        "#llamacpp-exec-path": _InputWidget(value="/private/llama-server"),
...
def test_stubborn_managed_process_blocks_delete_until_exact_death(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    lease = _Lease()
    ...
    monkeypatch.setattr(
        events,
        "acquire_managed_gguf",
        lambda actual_service, reference: (
            tmp_path / "managed" / "model.gguf",
            lease,
        ),
    )
    monkeypatch.setattr(events, "subprocess", _Subprocess(process))
