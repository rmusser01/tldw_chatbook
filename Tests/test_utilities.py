...
def mock_app_config():
    ...
    return {
        ...
        "ui_settings": {"theme": "dark", "font_size": 14, "show_tooltips": True},
        "rag_settings": {
            "enabled": False,
            "chunk_size": 1000,
            "overlap": 200,
...
def assert_datetime_recent(dt_str: str, max_age_seconds: int = 60):
    ...
    assert age <= max_age_seconds, (
        f"Datetime {dt_str} is {age}s old, expected <= {max_age_seconds}s"
    )
    ...
    if ignore_keys:
        ...
        def remove_keys(obj):
            if isinstance(obj, dict):
                return {
                    k: remove_keys(v) for k, v in obj.items() if k not in ignore_keys
                }
