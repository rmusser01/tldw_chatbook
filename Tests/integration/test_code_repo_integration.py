...
class TestCodeRepoIntegration:
    ...
    def mock_github_api(self):
        ...
        with patch(
            ...
            mock_instance.get_repository_info = AsyncMock(
                return_value={"name": "test-repo", "full_name": "test-owner/test-repo"}
            )
            mock_instance.get_branches = AsyncMock(return_value=["main", "develop"])
            mock_instance.get_repository_tree = AsyncMock(
                return_value=[
                    {
                        "path": "README.md",
                        "name": "README.md",
            ...
            mock_instance.get_file_content = AsyncMock(
                side_effect=lambda o, r, p, b: f"Content of {p}"
            )
    ...
    async def test_branch_switching_workflow(self, app_pilot, mock_github_api):
        ...
        main_tree = [
            {"path": "main.txt", "name": "main.txt", "type": "blob", "children": []}
        ]
        develop_tree = [
            {
                "path": "develop.txt",
