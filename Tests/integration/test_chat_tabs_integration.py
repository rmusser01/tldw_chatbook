...
async def integration_setup(mock_app, mock_config):
    ...
    return {"app": mock_app, "chat_window": chat_window, "tab_container": tab_container}
...
class TestChatTabsCharacterIntegration:
    ...
    async def test_character_assignment_per_tab(self, integration_setup):
        ...
        tab_container.tab_bar.update_tab_title.assert_any_call(
            tab1_id, "Chat with Alice", "Alice"
        )
        tab_container.tab_bar.update_tab_title.assert_any_call(
            tab2_id, "Chat with Bob", "Bob"
        )
...
class TestChatTabsUIConsistency:
    ...
    async def test_button_state_consistency(self, integration_setup):
        ...
        await tab_container.switch_to_tab_async(tab1_id)
        assert (
            session1.is_send_button is True
        )  # Suspend clears inactive streaming state
    ...
    async def test_widget_id_uniqueness(self, integration_setup):
        ...
        for tab_id in tab_ids:
            ...
            for widget_id in widget_ids:
                assert widget_id not in all_widget_ids, (
