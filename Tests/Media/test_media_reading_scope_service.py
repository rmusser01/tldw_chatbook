...
class FakeLocalMediaService:
    ...
    def list_ingestion_sources(self):
        ...
        return [{"id": 3, "source_type": "local_directory", "sink_type": "media", "policy": "canonical", "enabled": True}]
    ...
    def get_ingestion_source(self, source_id):
        ...
        return {"id": source_id, "source_type": "local_directory", "sink_type": "media", "policy": "canonical", "enabled": True}

    def patch_ingestion_source(self, source_id, **changes):
        self.calls.append(("patch_ingestion_source", source_id, changes))
        return {"id": source_id, "source_type": "local_directory", "sink_type": "media", "policy": "canonical", **changes}
    ...
    def import_reading_items(self, file_path, *, source="auto", merge_tags=True):
        self.calls.append(("import_reading_items", file_path, source, merge_tags))
        return {"job_id": 14, "status": "completed"}
            ...
            "jobs": [
                {
                    "job_id": 14,
                    "status": "completed",
                    "result": {"source": "pocket", "imported": 1, "updated": 0, "skipped": 0, "errors": []},
                }
            ],
            "total": 1,
            "limit": limit,
...
@pytest.mark.asyncio
async def test_scope_service_routes_local_ingestion_source_operations_and_policy():
    policy = FakePolicyEnforcer()
    local = FakeLocalMediaService()
    scope_service = MediaReadingScopeService(
        local_service=local,
...
@pytest.mark.asyncio
async def test_scope_service_denies_local_create_ingestion_source_when_policy_blocks_it():
    ...
    assert exc.value.reason_code == "blocked"
    assert policy_enforcer.calls == ["media.ingestion_sources.create.local"]
...
@pytest.mark.asyncio
async def test_scope_service_routes_local_reading_saved_searches_with_policy():
    policy = FakePolicyEnforcer()
    local = FakeLocalMediaService()
    ...
    listed = await scope.list_reading_saved_searches(mode="local", limit=25, offset=5)
    updated = await scope.update_reading_saved_search(mode="local", search_id=9, name="Updated")
    deleted = await scope.delete_reading_saved_search(mode="local", search_id=9)

    assert created["id"] == "local:reading_saved_search:9"
    assert listed["items"][0]["entity_kind"] == "reading_saved_search"
    assert updated["name"] == "Updated"
    assert deleted == {"ok": True}
    assert policy.calls == [
    ...
    assert local.calls[-4:] == [
        ("create_reading_saved_search", {"name": "Local saved", "query": {"status": ["saved"]}, "sort": "updated_desc"}),
        ("list_reading_saved_searches", 25, 5),
        ("update_reading_saved_search", 9, {"name": "Updated"}),
...
async def test_scope_service_routes_local_reading_import_jobs_with_policy():
    ...
    submitted = await scope.import_reading_items(
        mode="local",
        file_path="/tmp/pocket.json",
        source="pocket",
        merge_tags=False,
    )
    jobs = await scope.list_reading_import_jobs(mode="local", status="completed", limit=10, offset=0)
    job = await scope.get_reading_import_job(mode="local", job_id=14)

    assert submitted["id"] == "local:reading_import_job:14"
    assert jobs["jobs"][0]["id"] == "local:reading_import_job:14"
    assert job["status"] == "completed"
    assert policy.calls == [
    ...
    assert local.calls[-3:] == [
        ("import_reading_items", "/tmp/pocket.json", "pocket", False),
        ("list_reading_import_jobs", "completed", 10, 0),
        ("get_reading_import_job", 14),
    ]
...
@pytest.mark.asyncio
async def test_scope_service_routes_server_reading_digest_schedules_and_outputs_with_policy():
    policy = FakePolicyEnforcer()
    server = FakeServerMediaService()
    scope = MediaReadingScopeService(local_service=None, server_service=server, policy_enforcer=policy)
    ...
    schedules = await scope.list_reading_digest_schedules(mode="server", limit=25, offset=5)
    schedule = await scope.get_reading_digest_schedule(mode="server", schedule_id="sched-1")
    updated = await scope.update_reading_digest_schedule(mode="server", schedule_id="sched-1", enabled=False)
    deleted = await scope.delete_reading_digest_schedule(mode="server", schedule_id="sched-1")
    ...
    assert schedules[0]["id"] == "server:reading_digest_schedule:sched-1"
    assert schedule["entity_kind"] == "reading_digest_schedule"
    assert updated["enabled"] is False
    assert deleted == {"ok": True}
    assert outputs["items"][0]["id"] == "server:reading_digest_output:77"
    assert outputs["items"][0]["schedule_id"] == "sched-1"
    assert policy.calls == [
...
@pytest.mark.asyncio
async def test_scope_service_enforces_policy_for_local_reading_import_jobs():
    policy = FakePolicyEnforcer.deny("blocked")
    scope = MediaReadingScopeService(
        local_service=FakeLocalMediaService(),
        server_service=FakeServerMediaService(),
    ...
    assert policy.calls == ["media.reading_import.list.local"]


@pytest.mark.asyncio
async def test_scope_service_enforces_policy_for_local_saved_searches():
    policy = FakePolicyEnforcer.deny("blocked")
    scope = MediaReadingScopeService(
        local_service=FakeLocalMediaService(),
        server_service=FakeServerMediaService(),
    ...
    assert policy.calls == ["media.reading_saved_searches.list.local"]
