...
st_required_text = (
    ...
    .map(lambda s: s.strip().replace("\x00", ""))
...
def st_media_data(draw):
    ...
    return {
        ...
        "content": content,
        "media_type": draw(
            st.sampled_from(["article", "video", "obsidian_note", "pdf"])
        ),
...
class TestMediaItemProperties:
    ...
    def test_media_item_roundtrip(self, db_instance: MediaDatabase, media_data: dict):
        ...
        linked_keywords = {
            kw.lower().strip() for kw in fetch_keywords_for_media(db_instance, media_id)
        }
        ...
        doc_versions = db_instance.get_all_document_versions(
    ...
    def test_update_increments_version_and_changes_data(
        ...
        assert original["version"] == 1, (
            f"Expected initial version to be 1, got {original['version']}"
        )
        ...
        assert media_id_up == media_id, (
            f"Update returned different ID: {media_id_up} vs original {media_id}"
        )
        ...
        assert updated["version"] == 2, (
            f"Expected version to be incremented to 2, got {updated['version']}"
        )
...
class TestSearchProperties:
    ...
    def test_search_finds_item_by_its_properties(
        ...
        media_id, _, _ = db_instance.add_media_with_keywords(**media_data)
        results, total = db_instance.search_media_db(
            search_query=unique_word, search_fields=["content"]
        )
        assert total == 1
        assert results[0]["id"] == media_id
        ...
        results, total = db_instance.search_media_db(
            search_query=None,
            must_have_keywords=[keyword_to_find],
        ...
        results, total = db_instance.search_media_db(
            search_query=None,
            media_types=[media_data["media_type"]],
            media_ids_filter=[media_id],
        )
        assert total == 1
        assert results[0]["id"] == media_id
...
class TestIdempotencyAndConstraints:
    ...
    def test_add_media_with_conflicting_hash_is_handled(
        self, db_instance: MediaDatabase, media1: dict, media2: dict
    ):
        """
        ...
        id2, uuid2, msg2 = db_instance.add_media_with_keywords(
            **media2, overwrite=False
        )
        ...
        assert "updated" in msg3.lower() or "already" in msg3.lower(), (
            f"Expected update confirmation in message, got: {msg3}"
        )
        ...
        assert final_item["version"] == original_version + 1, (
...
class TestTimeBasedAndSearchQueries:
    ...
    def test_empty_trash_respects_time_threshold(
        ...
        with db_instance.transaction():
            db_instance.execute_query(
                ...
                (
                    ...
                    item_v2["version"]
                    + 1,  # Manually increment version for this setup step
                    "test_setup_client",
        ...
        final_item = db_instance.get_media_by_id(
            media_id, include_trash=True, include_deleted=True
        )
        assert final_item["deleted"] == 1
        assert (
            final_item["version"] == 4
