...
def client_db(memory_db_factory):
    ...
    # Import the proper MediaDatabase class
    from tldw_chatbook.DB.Client_Media_DB_v2 import MediaDatabase

    # Create a MediaDatabase instance directly
    import tempfile
...
class TestClientSyncEngineState:
    ...
    def test_state_load_existing_file(self, client_db, temp_state_file):
        ...
        initial_state = {
            "last_local_log_id_sent": 10,
            "last_server_log_id_processed": 25,
        }
        with open(temp_state_file, "w") as f:
    ...
    def test_state_save(self, sync_engine):
        ...
        with open(sync_engine.state_file, "r") as f:
            state = json.load(f)
            assert state == {
                "last_local_log_id_sent": 15,
                "last_server_log_id_processed": 30,
            }
...
class TestClientSyncEnginePush:
    @patch(
        "tldw_chatbook.DB.Sync_Client.requests.post"
    )  # Mock the post method used in sync_client
    ...
    @patch("tldw_chatbook.DB.Sync_Client.requests.post")
    def test_push_successful(self, mock_post, sync_engine, client_db):
        ...
        assert (
            sent_payload["changes"][0]["change_id"] == 1
        )  # Assuming it's the first log entry
...
class TestClientSyncEnginePullApply:
    ...
    def test_pull_and_apply_create_success(self, mock_get, sync_engine, client_db):
        ...
        mock_response.json.return_value = {
            "changes": [server_change],
            "latest_change_id": 101,
        }
        mock_get.return_value = mock_response
        ...
        mock_get.assert_called_once()
        # Verify DB state
        cursor = client_db.execute_query(
            "SELECT * FROM Keywords WHERE uuid = ?", (kw_uuid,)
        )
        row = cursor.fetchone()
        ...
        if isinstance(row["last_modified"], datetime):
            assert (
                row["last_modified"].strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    def test_pull_and_apply_update_success(self, mock_get, sync_engine, client_db):
        ...
        client_db.add_keyword(
            "initial_name"
        )  # Assume this creates version 1, kw_uuid = '...' - need the actual uuid
        cursor = client_db.execute_query(
            "SELECT uuid FROM Keywords WHERE keyword = ?", ("initial_name",)
        )
    ...
    def test_pull_and_apply_delete_success(self, mock_get, sync_engine, client_db):
        ...
        kw_uuid = client_db.execute_query(
            "SELECT uuid FROM Keywords WHERE keyword = ?", ("to_be_deleted",)
        ).fetchone()["uuid"]
        assert get_entity_version(client_db, "Keywords", kw_uuid) == 1
        ...
        # 4. Assertions
        cursor = client_db.execute_query(
            "SELECT deleted, version, last_modified FROM Keywords WHERE uuid = ?",
            (kw_uuid,),
        )
        row = cursor.fetchone()
    ...
    def test_apply_idempotency(self, mock_get, sync_engine, client_db):
        ...
        server_change = create_mock_log_entry(
            101, "Keywords", kw_uuid, "create", "other", 1, {"keyword": "idem"}, "ts1"
        )
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {
        ...
        assert (
            get_entity_version(client_db, "Keywords", kw_uuid) == 1
        )  # Still version 1
    ...
    def test_apply_old_change(self, mock_get, sync_engine, client_db):
        ...
        kw_uuid = client_db.execute_query(
            "SELECT uuid FROM Keywords WHERE keyword=?", ("v1",)
        ).fetchone()["uuid"]
        # Manually update to V2 locally - Use client_db here:
        client_db.execute_query(
            "UPDATE Keywords SET keyword='v2', version=2, last_modified=? WHERE uuid=?",
        ...
        server_change_v1 = create_mock_log_entry(
            101,
            "Keywords",
            kw_uuid,
            "update",
        ...
        assert get_entity_version(client_db, "Keywords", kw_uuid) == 2
        cursor = client_db.execute_query(
            "SELECT keyword FROM Keywords where uuid=?", (kw_uuid,)
        )
...
class TestClientSyncEngineConflict:
    @patch("tldw_chatbook.DB.Sync_Client.requests.get")
    def test_conflict_detected_and_remote_wins_lww(
        self, mock_get, sync_engine, client_db
    ):
        ...
        kw_uuid = client_db.execute_query(
            "SELECT uuid FROM Keywords WHERE keyword=?", ("synced_v1",)
        ).fetchone()["uuid"]
        ts_v1 = client_db.execute_query(
            "SELECT last_modified FROM Keywords WHERE uuid=?", (kw_uuid,)
        ).fetchone()[0]
        assert get_entity_version(client_db, "Keywords", kw_uuid) == 1
        ...
        client_db.execute_query(
            ...
            (ts_local_v2, sync_engine.client_id, kw_uuid),
            commit=True,
        )
        assert get_entity_version(client_db, "Keywords", kw_uuid) == 2
        ...
        assert (
            row["version"] == 3
        )  # NEW Assertion: Expects version to be incremented by force_apply logic
    ...
    @patch("tldw_chatbook.DB.Sync_Client.requests.get")
    def test_conflict_detected_and_local_wins_lww(
        self, mock_get, sync_engine, client_db
    ):
        ...
        kw_uuid = client_db.execute_query(
            "SELECT uuid FROM Keywords WHERE keyword=?", ("synced_v1_lw",)
        ).fetchone()["uuid"]
        ts_v1 = client_db.execute_query(
            "SELECT last_modified FROM Keywords WHERE uuid=?", (kw_uuid,)
        ).fetchone()[0]
