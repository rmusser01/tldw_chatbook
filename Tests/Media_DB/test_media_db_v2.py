...
class TestDatabaseCRUDAndSync:
    ...
    def test_v1_to_v2_seed_failure_rolls_back_ddl_and_version(self, tmp_path):
        ...
        conn.executescript(
            ...
            CREATE TABLE MediaChunks(id INTEGER PRIMARY KEY);
            CREATE TABLE UnvectorizedMediaChunks(id INTEGER PRIMARY KEY);
            """
        )
        conn.commit()

        migration_db = object.__new__(Database)
        migration_db.db_path_str = str(path)
        migration_db.get_connection = lambda: conn
        template = {
            "name": "duplicate-seed",
        ...
        migration_db._DEFAULT_CHUNKING_TEMPLATES = [template, template]

        try:
            with pytest.raises(DatabaseError, match="Migration v1->v2 failed"):
                migration_db._apply_migration_v1_to_v2(conn)
            ...
            migration_db._DEFAULT_CHUNKING_TEMPLATES = [template]
            migration_db._apply_migration_v1_to_v2(conn)
            assert conn.execute(
                "SELECT version FROM schema_version"
            ).fetchone()[0] == 2
