...
def custom_config():
    ...
    return {
        ...
        "defaults": {"unknown_models_vision": True, "log_unknown_models": False},
    }
...
class TestModelCapabilitiesBasics:
    ...
    def test_initialization_with_custom_config(self, model_capabilities_custom):
        ...
        assert (
            model_capabilities_custom.direct_mappings["test-model-1"]["vision"] is True
        )
    ...
    def test_direct_mapping_lookup(self, model_capabilities_custom):
        ...
        # Known vision model
        assert (
            model_capabilities_custom.is_vision_capable("AnyProvider", "test-model-1")
            is True
        )

        # Known non-vision model
        assert (
            model_capabilities_custom.is_vision_capable("AnyProvider", "test-model-2")
            is False
        )

        # Custom field in capabilities
        caps = model_capabilities_custom.get_model_capabilities(
            "AnyProvider", "custom-vision-model"
        )
...
class TestPatternMatching:
    ...
    def test_pattern_matching_basic(self, model_capabilities_custom):
        ...
        assert (
            model_capabilities_custom.is_vision_capable(
                "TestProvider", "test-vision-123"
            )
            is True
        )
        assert (
            model_capabilities_custom.is_vision_capable("TestProvider", "test-text-abc")
            is False
        )
        ...
        assert (
            model_capabilities_custom.is_vision_capable(
                "CustomProvider", "model-vision"
            )
            is True
        )
        assert (
            model_capabilities_custom.is_vision_capable("CustomProvider", "test-vision")
            is True
        )
        ...
        result = model_capabilities_custom.is_vision_capable(
            "CustomProvider", "vision-model"
        )
    ...
    def test_pattern_matching_case_insensitive(self, model_capabilities_custom):
        ...
        assert (
            model_capabilities_custom.is_vision_capable(
                "TestProvider", "TEST-VISION-ABC"
            )
            is True
        )
        assert (
            model_capabilities_custom.is_vision_capable(
                "TestProvider", "Test-Vision-XYZ"
            )
            is True
        )
    ...
    def test_pattern_priority(self, model_capabilities_custom):
        ...
        model_capabilities_custom.add_model_capability(
            "test-vision-direct", {"vision": False}
        )

        # Even though it matches the vision pattern, direct mapping says no vision
        assert (
            model_capabilities_custom.is_vision_capable(
                "TestProvider", "test-vision-direct"
            )
            is False
        )
...
class TestDefaultModels:
    ...
    def test_openai_models(self, model_capabilities_empty):
        ...
        # Direct mappings
        assert (
            model_capabilities_empty.is_vision_capable("OpenAI", "gpt-4-vision-preview")
            is True
        )
        assert (
            model_capabilities_empty.is_vision_capable("OpenAI", "gpt-4-turbo") is True
        )
        ...
        # Pattern matching
        assert (
            model_capabilities_empty.is_vision_capable("OpenAI", "gpt-4-vision-2024")
            is True
        )
        assert (
            model_capabilities_empty.is_vision_capable("OpenAI", "gpt-40-custom")
            is True
        )

        # Non-vision models
        assert (
            model_capabilities_empty.is_vision_capable("OpenAI", "gpt-3.5-turbo")
            is False
        )
    ...
    def test_anthropic_models(self, model_capabilities_empty):
        ...
        # Non-vision models
        assert (
            model_capabilities_empty.is_vision_capable("Anthropic", "claude-2.1")
            is False
        )
    ...
    def test_google_models(self, model_capabilities_empty):
        ...
        # Direct mappings
        assert (
            model_capabilities_empty.is_vision_capable("Google", "gemini-pro-vision")
            is True
        )
        assert (
            model_capabilities_empty.is_vision_capable("Google", "gemini-1.5-pro")
            is True
        )

        # Pattern matching
        assert (
            model_capabilities_empty.is_vision_capable("Google", "gemini-2.0-flash")
            is True
        )
        assert (
            model_capabilities_empty.is_vision_capable("Google", "gemini-ultra-vision")
            is True
        )
...
class TestUnknownModels:
    ...
    def test_unknown_provider_known_model(self, model_capabilities_custom):
        ...
        assert (
            model_capabilities_custom.is_vision_capable(
                "CompletelyUnknownProvider", "test-model-1"
            )
            is True
        )
...
class TestCachingAndPerformance:
    ...
    def test_capability_caching(self, model_capabilities_custom):
        ...
        # First call
        caps1 = model_capabilities_custom.get_model_capabilities(
            "TestProvider", "test-model"
        )

        # Second call should return cached result
        caps2 = model_capabilities_custom.get_model_capabilities(
            "TestProvider", "test-model"
        )
...
class TestModuleFunctions:
    ...
    def test_is_vision_capable_function(self, mock_load_config):
        ...
        mock_load_config.return_value = {
            "model_capabilities": {"models": {"test-model": {"vision": True}}}
        }
...
class TestEdgeCases:
    ...
    def test_add_model_capability(self, model_capabilities_custom):
        ...
        # Add new model
        model_capabilities_custom.add_model_capability(
            "dynamic-model", {"vision": True, "custom": "value"}
        )

        # Should be detected
        assert (
            model_capabilities_custom.is_vision_capable("AnyProvider", "dynamic-model")
            is True
        )

        # Should have custom field
        caps = model_capabilities_custom.get_model_capabilities(
            "AnyProvider", "dynamic-model"
        )
