...
def test_default_data_base_prefers_xdg_data_home(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    assert config._default_cli_data_base() == xdg_data_home / "tldw_cli"
...
def test_default_data_base_falls_back_to_home(
    ...
    assert (
        config._default_cli_data_base() == Path.home() / ".local" / "share" / "tldw_cli"
    )
...
def test_default_data_directory_is_private_under_permissive_umask(
    ...
    default_base = tmp_path / "application-data"
    monkeypatch.setattr(config, "BASE_DATA_DIR_CLI", default_base)
...
def test_default_data_directory_rejects_intermediate_symlink(
    ...
    monkeypatch.setattr(config, "BASE_DATA_DIR_CLI", alias / "application-data")
    _patch_data_dir_settings(monkeypatch, None)

    with pytest.raises(PrivatePathError):
        config.get_user_data_dir()

    assert not (target / "application-data").exists()
...
def test_default_data_directory_rejects_unsafe_intermediate_namespace(
    ...
    unsafe.mkdir()
    unsafe.chmod(0o777)
    monkeypatch.setattr(config, "BASE_DATA_DIR_CLI", unsafe / "application-data")
    _patch_data_dir_settings(monkeypatch, None)

    with pytest.raises(PrivatePathError):
        config.get_user_data_dir()

    assert not (unsafe / "application-data").exists()
