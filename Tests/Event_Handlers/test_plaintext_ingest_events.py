...
def mock_app():
    ...
    app._widget_mocks = {
        ...
        "#tldw-api-urls-plaintext": Mock(
            spec=TextArea, text="http://example.com/test.txt"
        ),
    ...
    def query_one_side_effect(selector, widget_type=None):
        if (
            widget_type
            and hasattr(widget_type, "__name__")
            and widget_type.__name__ == "IngestWindow"
        ):
        ...
        if (
            widget_type
            and hasattr(widget_type, "__name__")
            and widget_type.__name__.startswith("IngestTldwApi")
        ):
