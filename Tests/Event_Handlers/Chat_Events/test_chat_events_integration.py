...
pytestmark = [
    ...
    pytest.mark.skip(
        reason="Integration tests need refactoring for proper Textual app testing"
    ),
]
...
"""
    ...
    app.app_config = {
        ...
        "api_settings": {"openai": {"streaming": True, "api_key": "test-key-12345"}},
    }
...
class TestChatBasicOperations:
    ...
    async def test_save_ephemeral_chat(self, real_app):
        ...
        async with real_app.run_test() as pilot:
            ...
            chat_log = real_app.query_one("#chat-log", VerticalScroll)
            msg1 = ChatMessage(
                message="Test message 1", role="User", generation_complete=True
            )
            msg2 = ChatMessage(
                message="Test response", role="AI", generation_complete=True
            )
            ...
            conv = ccl.get_conversation_by_id(
                real_app.chachanotes_db, real_app.current_chat_conversation_id
            )
...
class TestChatCharacterIntegration:
    ...
    async def test_load_character(self, real_app):
        ...
        async with real_app.run_test() as pilot:
            ...
            with patch(
                "tldw_chatbook.Event_Handlers.Chat_Events.chat_events.display_available_characters_for_chat",
                return_value=character["id"],
            ):
                # Click load character button
                load_char_button = real_app.query_one(
                    "#chat-load-character-button", Button
                )
            ...
            assert (
                real_app.current_chat_active_character_data["name"] == "Assistant Bot"
            )
    ...
    async def test_clear_active_character(self, real_app):
        ...
        async with real_app.run_test() as pilot:
            ...
            clear_button = real_app.query_one(
                "#chat-clear-active-character-button", Button
            )
...
class TestChatMessageEditing:
    ...
    async def test_edit_message(self, real_app):
        ...
        async with real_app.run_test() as pilot:
            ...
            # Verify in database
            messages = ccl.get_messages_for_conversation(
                real_app.chachanotes_db, conv_id
            )
...
class TestChatWithStreaming:
    ...
    async def test_streaming_response(self, real_app):
        ...
        async with real_app.run_test() as pilot:
            ...
            with patch.object(
                real_app, "chat_wrapper", side_effect=mock_streaming_chat
            ):
                # Send a message
                chat_input = real_app.query_one("#chat-input", TextArea)
                await pilot.click(chat_input)
...
class TestChatErrorHandling:
    ...
    async def test_api_error_handling(self, real_app):
        ...
        async with real_app.run_test() as pilot:
            ...
            # Mock API error
            with patch.object(
                real_app, "chat_wrapper", side_effect=Exception("API Error")
            ):
                # Send a message
                chat_input = real_app.query_one("#chat-input", TextArea)
                await pilot.click(chat_input)
    ...
    async def test_database_error_handling(self, real_app):
        ...
        async with real_app.run_test() as pilot:
            ...
            # Mock database error
            with patch.object(
                ccl, "create_conversation", side_effect=Exception("DB Error")
            ):
                ...
                save_button = real_app.query_one(
