...
def test_clear_and_disable_media_display(mock_app):
    ...
    assert (
        mock_app.query_one("#chat-media-copy-content-button", Button).disabled is True
    )
...
async def test_perform_media_sidebar_search_with_results(mock_app):
    ...
    mock_app.media_db.search_media_db.return_value = (
        mock_media_items,
        len(mock_media_items),
    )
    mock_results_list = mock_app.query_one(
        "#chat-media-search-results-listview", ListView
    )

    with patch(
        "tldw_chatbook.Event_Handlers.Chat_Events.chat_events_sidebar.ListItem",
        side_effect=ListItem,
    ) as mock_list_item_class:
        ...
        mock_app.query_one(
            "#chat-media-content-display", TextArea
...
async def test_perform_media_sidebar_search_no_results(mock_app):
    ...
    with patch(
        "tldw_chatbook.Event_Handlers.Chat_Events.chat_events_sidebar.Label",
...
async def test_handle_chat_media_load_selected_button_pressed(mock_app):
    ...
    mock_app.query_one(
        "#chat-media-title-display", TextArea
    ).load_text.assert_called_once_with("Loaded Title")
    mock_app.query_one(
        "#chat-media-content-display", TextArea
    ).load_text.assert_called_once_with("This is the full content.")
    mock_app.query_one(
        "#chat-media-author-display", TextArea
    ).load_text.assert_called_once_with("Author Name")
    mock_app.query_one(
        "#chat-media-url-display", TextArea
    ).load_text.assert_called_once_with("http://example.com")

    # Verify copy buttons were enabled
    assert mock_app.query_one("#chat-media-copy-title-button", Button).disabled is False
    assert (
        mock_app.query_one("#chat-media-copy-content-button", Button).disabled is False
    )
    assert (
        mock_app.query_one("#chat-media-copy-author-button", Button).disabled is False
    )
...
async def test_handle_chat_media_load_selected_no_selection(mock_app):
    ...
    mock_app.notify.assert_called_once_with(
        "Selected item has no data.", severity="warning"
    )
