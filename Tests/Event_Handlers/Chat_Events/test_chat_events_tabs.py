...
def mock_config():
    ...
    with patch(
        "tldw_chatbook.Event_Handlers.Chat_Events.chat_events_tabs.get_cli_setting"
    ) as mock_get_setting:
...
class TestChatEventsTabsHelpers:
    ...
    def test_get_widget_id_for_session_no_tabs(self, session_data):
        ...
        with patch("tldw_chatbook.config.get_cli_setting", return_value=False):
            result = chat_events_tabs.get_widget_id_for_session(
                "chat-input", session_data
            )
...
class TestChatEventsTabsHandlers:
    ...
    async def test_handle_chat_send_button_pressed_with_tabs_preserves_expanded_metadata(
        ...
        class FakeTabContext:
            def __init__(self, app, session, *, query_one=None, query=None):
                self.query_one = Mock(
                    side_effect=lambda selector, *args: query_one_calls.append(selector)
                )
                self.query = Mock(
                    side_effect=lambda selector, *args: query_calls.append(selector)
                )
        ...
        with patch.object(chat_events_tabs, "TabContext", FakeTabContext):
            with patch.object(
                chat_events_tabs,
                "get_tab_state_manager",
                return_value=fake_state_manager,
            ):
                with patch(
        ...
        assert (
            session_data.current_ai_message_widget == mock_app.current_ai_message_widget
        )
    ...
    async def test_tab_send_sets_current_handoff_payload_for_original_handler(
        ...
        with patch.object(
            chat_events_tabs, "get_tab_state_manager", return_value=fake_state_manager
        ):
            with patch(
                "tldw_chatbook.Event_Handlers.Chat_Events.chat_events.handle_chat_send_button_pressed",
    ...
    @pytest.mark.asyncio
    async def test_handle_chat_send_button_pressed_with_tabs(
        self, mock_app, session_data, mock_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_chat_send_button_pressed_no_session(
        self, mock_app, mock_config
    ):
        """Test send button handler with no active session."""
        # Make get_active_session_data return None
        with patch.object(
            chat_events_tabs, "get_active_session_data", return_value=None
        ):
            button = Mock(spec=Button)
            event = Mock()
            event.button = button
            ...
            mock_app.notify.assert_called_once_with(
                "No active chat session", severity="error"
            )
    ...
    async def test_query_one_monkey_patching(self, mock_app, session_data, mock_config):
        ...
        query_one_calls = []
        mock_app.query_one.side_effect = lambda selector, *args: query_one_calls.append(
            selector
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_stop_chat_generation_pressed_with_tabs(
        self, mock_app, session_data, mock_config
    ):
        ...
        with patch(
            "tldw_chatbook.Event_Handlers.Chat_Events.chat_events.handle_stop_chat_generation_pressed"
        ) as mock_handler:
            mock_handler.return_value = None
    ...
    @pytest.mark.skip(reason="Complex mocking of TabContext and state_manager required")
    @pytest.mark.asyncio
    async def test_handle_respond_for_me_button_pressed_with_tabs(
        self, mock_app, session_data, mock_config
    ):
        """Test tab-aware respond-for-me handler."""
        with patch(
            "tldw_chatbook.Event_Handlers.Chat_Events.chat_events.handle_respond_for_me_button_pressed"
        ) as mock_handler:
            mock_handler.return_value = None
...
class TestChatEventsTabsStateSynchronization:
    ...
    @pytest.mark.asyncio
    async def test_session_state_update_after_send(
        self, mock_app, session_data, mock_config
    ):
        ...
        with patch(
            ...
            assert session_data.current_worker == mock_app.current_chat_worker
            assert (
                session_data.current_ai_message_widget
                == mock_app.current_ai_message_widget
            )
            assert session_data.assistant_kind == "persona"
            assert session_data.assistant_id == "assistant-123"
            assert session_data.persona_memory_mode == "workspace"
    ...
    @pytest.mark.asyncio
    async def test_display_conversation_updates_tab_title(
        self, mock_app, session_data, mock_config
    ):
...
class TestChatEventsTabsErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_handle_exception_in_wrapped_handler(
        self, mock_app, session_data, mock_config
    ):
...
class TestChatEventsTabsIntegration:
    ...
    async def test_concurrent_streaming_in_different_tabs(self, mock_app, mock_config):
        ...
        session1 = ChatSessionData(
            tab_id="tab1", is_streaming=True, current_worker=Mock()
        )
        session2 = ChatSessionData(
            tab_id="tab2", is_streaming=False, current_worker=None
        )
...
class TestChatEventsTabsEdgeCases:
    ...
    @pytest.mark.skip(reason="Recursion error due to complex mocking")
    @pytest.mark.asyncio
    async def test_widget_id_mapping_with_special_selectors(
        self, mock_app, session_data, mock_config
    ):
...
class TestChatEventsTabsToolCalling:
    ...
    @pytest.mark.asyncio
    async def test_tool_call_message_display_in_tab(
        self, mock_app, session_data, mock_config
    ):
        ...
        with patch(
            "tldw_chatbook.Widgets.tool_message_widgets.ToolCallMessage"
        ) as MockToolCallMessage:
            ...
            with patch(
                "tldw_chatbook.Event_Handlers.Chat_Events.chat_events.display_tool_call_message"
            ) as mock_display:
                ...
                MockToolCallMessage.assert_called_with(
                    tool_calls=tool_calls, message_id="msg_123"
                )

    @pytest.mark.skip(reason="Function display_tool_result_message not implemented")
    @pytest.mark.asyncio
    async def test_tool_result_message_display_in_tab(
        self, mock_app, session_data, mock_config
    ):
        ...
        with patch(
            "tldw_chatbook.Widgets.tool_message_widgets.ToolResultMessage"
        ) as MockToolResultMessage:
            ...
            with patch(
                "tldw_chatbook.Event_Handlers.Chat_Events.chat_events.display_tool_result_message"
            ) as mock_display:
...
class TestChatEventsTabsFileAttachments:
    ...
    @pytest.mark.asyncio
    async def test_file_attachment_in_tab_context(
        self, mock_app, session_data, mock_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_attachment_indicator_update_per_tab(
        self, mock_app, session_data, mock_config
    ):
        ...
        with patch(
            "tldw_chatbook.Event_Handlers.Chat_Events.chat_events.update_attachment_indicator"
        ) as mock_update:
...
class TestChatEventsTabsSettings:
    ...
    @pytest.mark.skip(reason="Recursion error due to complex mocking")
    @pytest.mark.asyncio
    async def test_settings_apply_to_active_tab_only(
        self, mock_app, session_data, mock_config
    ):
        ...
        with patch(
            ...
            def handler_with_settings_check(app, event):
                ...
                assert (
                    app.query_one("#chat-system-prompt").text
                    == "New system prompt for tab1"
                )
