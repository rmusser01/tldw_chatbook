...
class TestChatImageProperties:
    ...
    @pytest.mark.asyncio
    async def test_process_image_preserves_aspect_ratio(
        self, dimensions, format_name, mode
    ):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=f".{format_name.lower()}", delete=True
        ) as f:
    ...
    @given(extension=valid_extensions, size_bytes=file_sizes)
        ...
        with tempfile.NamedTemporaryFile(suffix=extension, delete=True) as f:
            ...
            if size_bytes > ChatImageHandler.MAX_IMAGE_SIZE:
                ...
                try:
                    ...
                    assert (
                        "cannot identify image file" in str(e).lower()
                        or "not a valid" in str(e).lower()
                    )

    @given(image_data=st.binary(min_size=10, max_size=1000))
    ...
    @given(dimensions=st.lists(image_dimensions, min_size=1, max_size=5))
    ...
    async def test_consistent_resize_behavior(self, dimensions):
        ...
        for width, height in dimensions:
            ...
            with tempfile.NamedTemporaryFile(suffix=".png", delete=True) as f:
                ...
                try:
                    processed_data, _ = await ChatImageHandler.process_image_file(
                        f.name
                    )
                    processed_img = PILImage.open(BytesIO(processed_data))
                    ...
                    was_resized = (
                        processed_img.width != width or processed_img.height != height
                    )
...
class TestImageDataIntegrity:
    ...
    @given(quality=st.integers(min_value=1, max_value=100))
    @settings(max_examples=10)
