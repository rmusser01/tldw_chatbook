...
async def test_successful_prompt_import_invokes_success_callback():
    ...
    with (
        patch(
            "tldw_chatbook.Event_Handlers.prompt_ingest_events.prompts_db_initialized",
            return_value=True,
        ),
        patch(
            "tldw_chatbook.Event_Handlers.prompt_ingest_events.import_prompts_from_files",
            return_value=results,
        ),
    ):
        await handle_ingest_prompts_import_now_button_pressed(
