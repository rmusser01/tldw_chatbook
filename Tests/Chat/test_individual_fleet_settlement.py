...
def test_individual_event_has_terminal_row_before_slow_sibling_drains(
    ...
    class FailedResultWriteDB(AgentRunsDB):
        def set_terminal_with_step(self, run_id, status, result, step, **kwargs):
            if result == "fast answer":
                raise sqlite3.OperationalError("scripted result write failure")
            return super().set_terminal_with_step(run_id, status, result, step, **kwargs)
    ...
    try:
        ...
        assert event.child.status == row["status"] == (
            "error" if failure == "provider" else "done"
        )
        if failure == "result_write":
            assert any(step["kind"] == "capture_failed" for step in row["steps"])
        if not failure:
...
def test_unavailable_durable_row_suppresses_individual_event_but_preserves_drain(
    ...
    assert sum(len(event.children) for event in drains) == (0 if failure == "create" else 2)
    rows = [
