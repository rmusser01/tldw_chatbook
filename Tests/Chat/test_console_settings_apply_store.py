...
def test_unsaved_apply_stages_without_empty_conversation_then_first_persist_merges() -> None:
    persistence = _SettingsPersistence()
    store = ConsoleChatStore(persistence=persistence)
    session = store.create_session(settings=_settings("old"))
    commit = store.commit_console_settings_live(
    ...
    metadata = persistence.created_conversations[0]["metadata"]
    assert conversation_id == "created-conversation"
