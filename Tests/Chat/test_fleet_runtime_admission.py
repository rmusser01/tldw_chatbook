...
def test_failed_thread_start_releases_capacity_and_spawn_allowance(
    ...
    try:
        ...
        assert {row["task"]: row["status"] for row in rows} == {"failed": "error", "retry": "done"}
        # Upstream causal history retains the failed-start receipt; it must
        # not retain admission capacity or consume the retry allowance.
    finally:
        for thread in service._fleet_threads.values():
            thread.join(5)
        db.close()
