...
pytestmark = [
    ...
    pytest.mark.skip(
        reason="Legacy Chat media sidebar is not mounted in the Console-native ChatScreen."
    ),
]
...
async def test_media_search_initial_state(real_app_media_test: TldwCli):
    ...
    async with app.run_test() as pilot:
        ...
        try:
            ...
            print(
                f"Widget IDs after splash: {[w.id for w in all_widgets[:20] if w.id]}"
            )
        except Exception:
        ...
        try:
            ...
            if tabbed_content:
                print(
                    f"TabbedContent found with {len(tabbed_content.children)} children"
                )
        ...
        try:
            ...
            print(
                f"Available Collapsibles: {[(c.id, c.title) for c in collapsibles if c.id]}"
            )
            # List all widgets with 'media' in their ID
            media_widgets = [
                w for w in app.query("*").results() if w.id and "media" in w.id
            ]
            print(
                f"Media-related widgets: {[(w.id, type(w).__name__) for w in media_widgets]}"
            )
            # Check for chat-related widgets
            chat_widgets = [
                w for w in app.query("*").results() if w.id and "chat" in w.id
            ]
            print(
                f"Chat-related widgets: {[(w.id, type(w).__name__) for w in chat_widgets[:10]]}"
            )  # First 10
            raise
        ...
        results_listview = app.query_one(
...
async def test_media_load_for_review(real_app_media_test: TldwCli):
    ...
    async with app.run_test() as pilot:
        ...
        assert (
            app.query_one("#chat-media-copy-content-button", Button).disabled is False
        )
...
@pytest.mark.parametrize(
    "button_id, field_key, expected_notification",
    [
...
async def test_media_copy_buttons(
    real_app_media_test: TldwCli, button_id, field_key, expected_notification
):
