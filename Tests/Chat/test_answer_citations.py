...
def _bundle(
    *references: EvidenceReference, status: str = "available"
) -> EvidenceBundle:
...
def test_format_evidence_for_cited_answer_reports_insufficient_available_evidence() -> (
    None
):
    ...
    assert (
        "[S1] Incident Review (note-42) - Source authority: local - blocked" in context
    )
    assert "Do not cite this reference as trusted evidence." in context


def test_extract_citation_markers_preserves_first_seen_order_without_duplicates() -> (
    None
):
...
def test_answer_citation_validation_is_insufficient_when_no_available_evidence_exists() -> (
    None
):
    result = build_answer_citation_validation(
...
def test_answer_citation_validation_does_not_duplicate_cited_blocked_reference() -> (
    None
):
    result = build_answer_citation_validation(
...
def test_citation_artifact_summary_preserves_falsy_values_and_caps_reference_scan() -> (
    None
):
...
def test_answer_citation_validation_marks_mixed_validated_and_blocked_unverified() -> (
    None
):
    result = build_answer_citation_validation(
    ...
    assert [citation["status"] for citation in payload["citations"]] == [
        "validated",
        "blocked",
    ]


def test_answer_citation_validation_quote_uses_question_and_exclamation_boundaries() -> (
    None
):
    result = build_answer_citation_validation(
