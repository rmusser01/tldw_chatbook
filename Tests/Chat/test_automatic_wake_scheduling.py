...
def additional_conversation(rig, *, saved=True):
    items = list(rig)
    items[4] = rig[3].create_session(title="Another conversation")
    if saved:
        rig[0].add_conversation({"id": items[4].id, "title": "Another conversation"})
        rig[3].rebind_persisted_conversation(items[4].id, items[4].id)
    return tuple(items)
...
async def test_two_conversations_wake_while_one_primary_slot_remains_manual(rig):
    ...
    manual_rig = additional_conversation(rig, saved=False)
    gate = asyncio.Event()
...
async def test_new_results_do_not_extend_the_fixed_coalescing_window(rig):
    ...
    try:
        ...
        deadline = rig[7].fleet_wake._coalesce_until[session.id]
        queue_result(rig, result_for(rig, chain))
        assert rig[7].fleet_wake._coalesce_until[session.id] == deadline
