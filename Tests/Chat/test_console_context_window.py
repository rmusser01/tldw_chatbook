...
async def test_resolve_for_send_never_waits_on_metadata():
    """task-32904: a slow metadata endpoint must not delay the send itself."""
    from tldw_chatbook.Chat.console_provider_gateway import (
        ConsoleProviderGateway,
        ConsoleProviderSelection,
    )
...
async def test_answered_endpoint_is_not_retried_on_the_failure_ttl():
    """task-32904: only transport failures retry on the short TTL."""
    ...
    answered_target = ContextWindowTarget("vllm", "vllm", "http://a.invalid", "m")
    ...
    failed_target = ContextWindowTarget("vllm", "vllm", "http://b.invalid", "m")
...
async def test_declared_oversized_content_length_is_not_downloaded():
    """task-32904: a Content-Length over the cap aborts before reading the body."""
