...
def get_provider_env_vars(provider_name: str) -> Dict[str, Any]:
    ...
    config = {
        ...
        "api_url": os.getenv(
            f"{provider_name.upper().replace('-', '_')}_TEST_URL"
        ),  # For local/self-hosted
        "api_base": os.getenv(
            f"{provider_name.upper().replace('-', '_')}_TEST_API_BASE"
...
def provider_config(request):
    ...
    is_local_type = provider_name in [
        "ollama",
        "llama_cpp",
        "ooba",
        "koboldcpp",
    ...
    if (
        not env_vars.get("model") and "model" in params_map
    ):  # If model is an expected param for the handler
        ...
        if not (
            is_local_type and provider_name == "koboldcpp"
        ):  # Kobold native doesn't take model in payload
            pytest.skip(
                f"{provider_name.upper()}_TEST_MODEL not set. Skipping integration test for {provider_name}."
            )

    if not is_local_type and not env_vars.get("api_key") and "api_key" in params_map:
        pytest.skip(
            f"{provider_name.upper()}_API_KEY not set. Skipping integration test for {provider_name}."
        )
    ...
    if provider_name in [
        ...
        "custom-openai-api-2",
        "aphrodite",
    ] and not (env_vars.get("api_url") or env_vars.get("api_base")):
        pytest.skip(
            f"{provider_name.upper()}_TEST_URL or _TEST_API_BASE not set. Skipping for {provider_name}."
        )
    ...
    if provider_name == "llama_cpp" and not env_vars.get(
        "api_url"
    ):  # Assuming llama_cpp handler uses api_url from config
    ...
    if is_local_type:
        ...
        if url_to_check:
            try:
                ...
                health_url = (
                    url_to_check.rstrip("/") + "/v1/models"
                    if "openai" in provider_name
                    or provider_name in ["vllm", "local-llm", "ollama", "llama_cpp"]
                    else url_to_check
                )
                if (
                    provider_name == "kobold_cpp"
                ):  # Kobold native API doesn't have /v1/models
                    health_url = url_to_check.replace(
                        "/api/v1/generate", "/api/v1/model"
                    )  # Check model endpoint
                elif (
                    provider_name == "koboldcpp"
                ):  # Fix for koboldcpp (different from kobold_cpp)
                    health_url = (
                        url_to_check.rstrip("/") + "/api/v1/model"
                        if "/api/v1" not in url_to_check
                        else url_to_check.replace("/generate", "/model")
                    )
                ...
                pytest.skip(
                    f"Could not connect to {provider_name} at {url_to_check}. Skipping integration test."
                )
            except requests.exceptions.Timeout:
                pytest.skip(
                    f"Connection to {provider_name} at {url_to_check} timed out. Skipping integration test."
                )
            except Exception as e:
                # Catch any other connection errors
                pytest.skip(
                    f"Connection error for {provider_name} at {url_to_check}: {e}. Skipping integration test."
                )
...
def test_provider_api_basic_call(provider_config: Dict[str, Any]):
    ...
    try:
        ...
        if e.status_code and 500 <= e.status_code <= 599:
            pytest.warning(
                f"Provider {provider} returned a server error {e.status_code}: {e.message}"
            )
        ...
        pytest.fail(
            f"Authentication failed for {provider}. Check API key: {api_key[:5]}..."
        )

    assert response is not None
    if (
        provider_config["provider_name"] == "kobold_cpp"
    ):  # Kobold native format is different
...
def test_provider_api_streaming_call(provider_config: Dict[str, Any]):
    ...
    messages: List[Dict[str, Any]] = [
        {"role": "user", "content": "Stream three words."}
    ]
    params_for_call = {
        "api_endpoint": provider,
        "messages_payload": messages,
    ...
    try:
        ...
        if e.status_code and 500 <= e.status_code <= 599:
            pytest.warning(
                f"Provider {provider} (streaming) returned server error {e.status_code}: {e.message}"
            )
            return
        raise
    except ChatRateLimitError:
    ...
    assert isinstance(response_generator, Generator), (
        "Response should be a generator for streaming"
    )
    ...
    try:
        for i, chunk_str in enumerate(response_generator):
            ...
            ):  # Limit chunks for very verbose local models unless they are known to be openai-like sse
                # Some local model streams might not have a clear [DONE] or might be too verbose
                print(
                    f"Provider {provider} (streaming): Received {i + 1} chunks, stopping test iteration early."
                )
                break
            if i >= 20:  # Absolute safety break
                print(
                    f"Provider {provider} (streaming): Received {i + 1} chunks, safety break."
                )
                break
    ...
    assert len(received_content) > 0, (
        "Should have received at least one chunk for streaming"
    )
    ...
    is_openai_like_stream = provider not in [
        "kobold_cpp"
    ]  # Add others that don't use OpenAI SSE
    if is_openai_like_stream:
        ...
        for chunk in reversed(received_content):
            if chunk.strip():
                ...
                break
        assert "data: [DONE]" in last_meaningful_chunk, (
            f"Stream for {provider} did not end with [DONE] marker. Last chunk: {last_meaningful_chunk}"
        )
...
def test_main_chat_function_integration_one_provider():
    ...
    provider_to_test = os.getenv(
        "PRIMARY_INTEGRATION_TEST_PROVIDER", "ollama"
    )  # Default to ollama
    ...
    if (
        provider_to_test != "ollama" and not api_key
    ):  # Assuming cloud providers need a key
        pytest.skip(
            f"API key for {provider_to_test} not set. Skipping main chat integration."
        )
    if provider_to_test == "ollama" and not api_url:
        pytest.skip(
            f"OLLAMA_TEST_URL for {provider_to_test} not set. Skipping main chat integration."
        )
    ...
    with patch.dict(
        os.environ,
        {"OLLAMA_API_URL": api_url} if provider_to_test == "ollama" and api_url else {},
    ):
        ...
        try:
            ...
            pytest.skip(
                f"Rate limit hit for {provider_to_test} in main chat integration. Skipping."
            )
        except ChatAuthenticationError:
            pytest.fail(
                f"Authentication failed for {provider_to_test} in main chat integration."
            )
        except ChatProviderError as e:
            if e.status_code and 500 <= e.status_code <= 599:
                pytest.warning(
