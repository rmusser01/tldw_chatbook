...
def test_keyless_provider_is_disabled_and_not_persistable():
    state = chat_api_key_field_state(
        _readiness(
            requires_api_key=False, ready=True, provider="Ollama", provider_key="ollama"
        ),
        locked=False,
    )
...
def test_persist_skips_when_not_persistable():
    state = ChatApiKeyFieldState(
        value="", disabled=True, placeholder="", can_persist=False
    )
...
def test_persist_skips_blank_and_placeholder():
    state = ChatApiKeyFieldState(
        value="", disabled=False, placeholder="", can_persist=True
    )
...
def test_persist_skips_unchanged_config_value():
    state = ChatApiKeyFieldState(
        value="test-key-abc123", disabled=False, placeholder="", can_persist=True
    )
...
def test_persist_returns_stripped_new_value():
    state = ChatApiKeyFieldState(
        value="test-old-key", disabled=False, placeholder="", can_persist=True
    )
