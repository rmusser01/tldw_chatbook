...
from .mock_api_responses import OPENAI_CHAT_RESPONSE, ANTHROPIC_CHAT_RESPONSE
...
class TestMockedChatAPIs:
    ...
    def test_openai_chat_mocked(self, mock_post, mock_messages):
        ...
        result = chat_api_call(
            "openai", mock_messages, api_key="test-key", model="gpt-3.5-turbo"
        )
    ...
    def test_api_error_handling(self, mock_post):
        ...
        with pytest.raises(ChatProviderError) as exc_info:
            chat_api_call(
                "openai", [{"role": "user", "content": "test"}], api_key="invalid-key"
            )
        ...
        with pytest.raises(ChatProviderError) as exc_info:
            chat_api_call(
                "openai", [{"role": "user", "content": "test"}], api_key="test-key"
            )
