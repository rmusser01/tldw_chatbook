...
class TestDiarizationService(unittest.TestCase):
    ...
    def test_segment_creation(self):
        ...
        speech_timestamps = [{"start": 0.0, "end": 5.0}, {"start": 6.0, "end": 10.0}]
    ...
    @patch(
        "tldw_chatbook.Local_Ingestion.diarization_service.DiarizationService._load_audio"
    )
    @patch(
        "tldw_chatbook.Local_Ingestion.diarization_service.DiarizationService._detect_speech"
    )
    @patch(
        "tldw_chatbook.Local_Ingestion.diarization_service.DiarizationService._create_segments"
    )
    @patch(
        "tldw_chatbook.Local_Ingestion.diarization_service.DiarizationService._extract_embeddings"
    )
    @patch(
        "tldw_chatbook.Local_Ingestion.diarization_service.DiarizationService._cluster_speakers"
    )
    def test_full_diarization_pipeline(
        self, mock_cluster, mock_extract, mock_create, mock_detect, mock_load
    ):
    ...
    def test_progress_callback(self):
        ...
        def progress_callback(progress, status, data):
            progress_calls.append(
                {"progress": progress, "status": status, "data": data}
            )

        # Mock the pipeline
        with (
            patch.object(self.service, "_load_audio") as mock_load,
    ...
    def test_single_speaker_case(self):
        ...
        similar_embeddings = np.array(
            [base_embedding + 0.01 * np.random.randn(192) for _ in range(10)]
        )
