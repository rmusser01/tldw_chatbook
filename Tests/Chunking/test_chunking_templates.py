        ...
        template = ChunkingTemplate(
            ...
            pipeline=[
                ChunkingStage(
                    stage="preprocess",
                    operations=[
                        ChunkingOperation(type="normalize_whitespace", params={})
                    ],
                ),
                ChunkingStage(
                    stage="chunk", method="sentences", options={"max_size": 5}
                ),
            ],
        )
...
class TestChunkingTemplateManager:
    ...
    def manager_with_temp_dir(self, temp_templates_dir):
        ...
        return ChunkingTemplateManager(
            templates_dir=temp_templates_dir, user_templates_dir=temp_templates_dir
        )
    ...
    def test_save_and_load_template(self, manager_with_temp_dir, temp_templates_dir):
        ...
        template = ChunkingTemplate(
            ...
            base_method="words",
            pipeline=[
                ChunkingStage(stage="chunk", method="words", options={"max_size": 200})
            ],
        )
    ...
    def test_template_inheritance(self, manager_with_temp_dir):
        ...
        parent = ChunkingTemplate(
            ...
            pipeline=[
                ChunkingStage(
                    stage="preprocess",
                    operations=[ChunkingOperation(type="normalize_whitespace")],
                ),
                ChunkingStage(stage="chunk", method="words", options={"max_size": 100}),
            ],
        )
        ...
        preprocess_stages = [
            s for s in loaded_child.pipeline if s.stage == "preprocess"
        ]
    ...
    def test_pipeline_with_operations(self, pipeline, manager):
        ...
        template = ChunkingTemplate(
            ...
            pipeline=[
                ...
                ChunkingStage(
                    stage="chunk", method="sentences", options={"max_size": 2}
                ),
                ChunkingStage(
                    stage="postprocess",
                    operations=[
                        ChunkingOperation(
                            type="filter_empty", params={"min_length": 10}
                        )
                    ],
                ),
            ],
...
class TestImprovedChunkingProcessWithTemplates:
    ...
    def test_improved_process_with_template(self):
        ...
        results = improved_chunking_process(
            text, template="sentences", chunk_options_dict={"max_size": 3}
        )
    ...
    def test_template_with_domain_specific(self):
        ...
        try:
            results = improved_chunking_process(text, template="academic_paper")
...
class TestCustomOperations:
    ...
    def test_register_custom_operation(self):
        ...
        def custom_op(
            text: str, chunks: List[str], options: Dict[str, Any]
        ) -> List[str]:
        ...
        result = manager._operations["add_prefix"](
            "test", ["chunk1", "chunk2"], {"prefix": "[TEST]"}
        )
