...
from tldw_chatbook.Chunking.engine import chunker as chunker_module
from tldw_chatbook.Chunking.engine.base import ChunkMetadata, ChunkResult
from tldw_chatbook.Chunking.engine.constants import FRONTMATTER_SENTINEL_KEY
from tldw_chatbook.Chunking.engine.error_policy import CHUNKER_NONCRITICAL_EXCEPTIONS
from tldw_chatbook.Chunking.engine.exceptions import (
    ChunkingError,
    InvalidChunkingMethodError,
    InvalidInputError,
)
from tldw_chatbook.Chunking.engine.llm_context import _LLM_UNSET, llm_override_scope
from tldw_chatbook.Chunking.engine.option_utils import _coerce_bool_option
from tldw_chatbook.Chunking.engine.process_text import dispatch as process_dispatch
from tldw_chatbook.Chunking.engine.process_text import metadata as process_metadata
from tldw_chatbook.Chunking.engine.process_text import models, preparation
from tldw_chatbook.Chunking.engine.process_text import options as process_options
from tldw_chatbook.Chunking.engine.process_text import pipeline as process_pipeline
from tldw_chatbook.Chunking.engine.process_text.dispatch import dispatch_chunks
from tldw_chatbook.Chunking.engine.process_text.metadata import (
...
from tldw_chatbook.Chunking.engine.process_text.options import (
    METHOD_OPTION_EXCLUDES,
    resolve_process_options,
)
from tldw_chatbook.Chunking.engine.process_text.preparation import (
    extract_header,
    prepare_frontmatter,
)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                assert alias.name != "tldw_chatbook.Chunking.engine.chunker"
                assert not alias.name.endswith(".chunker")
                assert alias.name != "chunker"
                assert alias.asname != "Chunker"
        elif isinstance(node, ast.ImportFrom):
            module_name = node.module or ""
            assert module_name != "tldw_chatbook.Chunking.engine.chunker"
