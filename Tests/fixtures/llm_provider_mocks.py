...
def mock_koboldcpp_server():
    ...
    responses = {
        ...
        "/api/v1/model": MockKoboldCPPResponse(json_data={"result": "test-model-7b"}),
    }
...
def mock_sync_server():
    ...
    endpoints = {
        ...
        "/api/v1/sync/push": MockSyncResponse(
            json_data={"accepted": True, "conflicts": [], "new_version": 2}
        ),
        "/api/v1/sync/status": MockSyncResponse(
            json_data={"server_version": 1, "client_registered": True}
        ),
    }
...
def mock_ollama_api():
    ...
    def mock_request(method, url, **kwargs):
        ...
        if path in responses:
            return MagicMock(status_code=200, json=lambda: responses[path])
...
def mock_all_providers():
    ...
    with (
        ...
        mock_post.return_value = MagicMock(
            status_code=200, json=lambda: {"message": {"content": "Mocked response"}}
        )
        ...
        mock_aio_request.return_value = AsyncMock(
            status=200, json=AsyncMock(return_value={"status": "ok"})
        )

        yield {"post": mock_post, "get": mock_get, "aio_request": mock_aio_request}
