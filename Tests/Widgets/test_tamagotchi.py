...
class TestValidators:
    ...
    def test_personality_validation(self):
        ...
        assert (
            TamagotchiValidator.validate_personality("balanced", PERSONALITIES)
            == "balanced"
        )
        assert (
            TamagotchiValidator.validate_personality("ENERGETIC", PERSONALITIES)
            == "energetic"
        )
...
class TestBaseTamagotchi:
    ...
    def test_initialization_invalid(self, mock_app):
        ...
        with pytest.raises(ValidationError):
            BaseTamagotchi(name="Valid", personality="invalid_personality")
    ...
    def test_rate_limiting(self, mock_app):
        """Test rate limiting functionality."""
        pet = BaseTamagotchi(
            name="Pixel", enable_rate_limiting=True, global_cooldown=1.0
        )
        ...
        # Mock time
        with patch(
            "tldw_chatbook.Widgets.Tamagotchi.base_tamagotchi.time.time"
        ) as mock_time:
            ...
            calls = [
                call
                for call in pet.post_message.call_args_list
    ...
    def test_stat_validation_on_interact(self, mock_app):
        ...
        pet = BaseTamagotchi(name="Pixel", enable_rate_limiting=False)
        pet.post_message = MagicMock()
        pet._play_animation = MagicMock()
...
class TestIntegration:
    ...
    def test_full_lifecycle(self):
        ...
        try:
            ...
            pet2 = Tamagotchi(name="Different", storage=storage)
    ...
    def test_corruption_recovery(self):
        ...
        try:
            ...
            pet = Tamagotchi(name="Recovery", storage=storage)
            pet.id = "corrupt_pet"
