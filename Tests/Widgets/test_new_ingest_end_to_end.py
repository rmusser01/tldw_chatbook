...
async def test_backend_service_integration():
    ...
    # Test job submission
    config = VideoConfig(files=[Path("test.mp4")], title="Test Video Processing")
...
async def test_complete_workflow_simulation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dashboard.update_job_status(
            job_id, ProcessingState.PROCESSING, 0.0, "Starting..."
        )
        ...
        dashboard.update_job_status(
            job_id, ProcessingState.COMPLETED, 1.0, "All files processed"
        )
