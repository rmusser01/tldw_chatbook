        ...
        async with app.run_test(size=(120, 40)) as pilot:
            ...
            try:
                ...
                processor = UnifiedProcessor(
                    app, initial_files=[test_video1, test_video2]
                )
            ...
            mock_service.get_job_status.return_value = Mock(
                job_id="job-test-123", state=ProcessingState.PROCESSING, progress=0.5
            )

            with patch(
                "tldw_chatbook.Widgets.NewIngest.BackendIntegration.get_processing_service",
                ...
                try:
                    ...
                    print(
                        f"DEBUG: Button visible: {button_region.overlaps(screen_region)}"
                    )
                ...
                try:
                    ...
                    print(
                        f"DEBUG: Selected files count: {len(processor.selected_files)}"
                    )
                    print(
                        f"DEBUG: Processing status: {processor.processing_status.state}"
                    )
                except Exception as e:
            ...
            job = dashboard.add_job(
                "job-test-123", "Test Video Processing", [test_video1, test_video2]
            )
            await pilot.pause()

            # Simulate processing progress updates
            dashboard.update_job_status(
                "job-test-123", ProcessingState.PROCESSING, 0.3, "Processing video 1..."
            )
            await pilot.pause(0.1)

            dashboard.update_job_file_progress(
                "job-test-123", str(test_video1), 1.0, "completed"
            )
            await pilot.pause(0.1)

            dashboard.update_job_file_progress(
                "job-test-123", str(test_video2), 0.8, "processing"
            )
            await pilot.pause(0.1)

            # Complete processing
            dashboard.update_job_file_progress(
                "job-test-123", str(test_video2), 1.0, "completed"
            )
            dashboard.update_job_status(
                "job-test-123",
...
async def test_processing_dashboard_controls():
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        dashboard.update_job_status(
            "job-2", ProcessingState.PROCESSING, 0.3, "Processing..."
        )
        dashboard.update_job_status(
            "job-3", ProcessingState.COMPLETED, 1.0, "Completed"
        )
