...
class TestChatSession:
    ...
    async def test_on_mount(self, chat_session):
        ...
        chat_session.set_interval.assert_called_once_with(
            0.5, chat_session._check_streaming_state
        )
    ...
    def test_get_chat_input(self, chat_session):
        ...
        chat_session.query_one.assert_called_with(
            "#chat-input-test-session-123", TextArea
        )
    ...
    def test_get_chat_log(self, chat_session):
        ...
        chat_session.query_one.assert_called_with(
            "#chat-log-test-session-123", VerticalScroll
        )
    ...
    def test_set_draft_text_loads_tab_input_text(self, chat_session):
        ...
        chat_session.query_one.assert_called_once_with(
            "#chat-input-test-session-123", TextArea
        )
...
class TestChatSessionButtonHandlers:
    ...
    async def test_handle_send_stop_button_send_mode(self, chat_session):
        ...
        with patch(
            "tldw_chatbook.Event_Handlers.Chat_Events.chat_events_tabs.handle_chat_send_button_pressed_with_tabs"
        ) as mock_handler:
            ...
            mock_handler.assert_called_once_with(
                chat_session.app_instance, event, chat_session.session_data
            )
    ...
    async def test_handle_send_stop_button_stop_mode(self, chat_session):
        ...
        # Mock the tab-aware handler
        with patch(
            "tldw_chatbook.Event_Handlers.Chat_Events.chat_events_tabs.handle_stop_chat_generation_pressed_with_tabs"
        ) as mock_handler:
            mock_handler.return_value = asyncio.Future()
            mock_handler.return_value.set_result(None)
    ...
    async def test_handle_suggest_button(self, chat_session):
        ...
        with patch(
            "tldw_chatbook.Event_Handlers.Chat_Events.chat_events_tabs.handle_respond_for_me_button_pressed_with_tabs"
        ) as mock_handler:
            mock_handler.return_value = asyncio.Future()
            mock_handler.return_value.set_result(None)
...
class TestChatSessionEnhancedMode:
    ...
    @pytest.mark.skip(
        reason="Requires active Textual app context to inspect composed children"
    )
...
class TestChatSessionStateManagement:
    ...
    def test_session_data_reactivity(self, chat_session):
        ...
        new_data = ChatSessionData(
            tab_id="new-session", title="New Session", is_streaming=True
        )
