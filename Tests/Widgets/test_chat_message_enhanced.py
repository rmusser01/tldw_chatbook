...
class TestChatMessageEnhancedInitialization:
    ...
    def test_ai_message_initialization(self):
        ...
        message = ChatMessageEnhanced(
            message="AI response", role="Assistant", generation_complete=False
        )
...
class TestChatMessageEnhancedComposition:
    ...
    async def test_compose_structure_user_message(
        self, widget_pilot, sample_image_data
    ):
    ...
    async def test_compose_without_image(self, widget_pilot):
        ...
        async with await widget_pilot(
            ChatMessageEnhanced, message="Text only", role="User"
        ) as pilot:
            widget = pilot.app.test_widget
            await pilot.pause()
...
class TestChatMessageEnhancedInteractions:
    ...
    async def test_button_action_events(self, widget_pilot, mock_app_instance):
        ...
        async with await widget_pilot(
            ChatMessageEnhanced, message="Test message", role="User"
        ) as pilot:
            app = pilot.app
            widget = app.test_widget
    ...
    async def test_ai_generation_state_handling(self, widget_pilot, wait_for_condition):
        ...
        async with await widget_pilot(
            ...
            await wait_for_condition(
                lambda: not actions_bar.has_class("-generating"), timeout=2.0
            )
...
class TestChatMessageEnhancedImageHandling:
    ...
    async def test_image_rendering_textual_image(self, widget_pilot, sample_image_data):
        ...
        with patch(
            "tldw_chatbook.Widgets.chat_message_enhanced.TextualImage"
        ) as mock_image_class:
            mock_image = Mock()
    ...
    async def test_save_image_functionality(
        self, widget_pilot, sample_image_data, tmp_path, mock_app_instance
    ):
        """Test saving image to file."""
        with patch(
            "tldw_chatbook.Widgets.chat_message_enhanced.Path.home"
        ) as mock_home:
    ...
    async def test_corrupt_image_handling(self, widget_pilot):
        ...
        async with await widget_pilot(
            ChatMessageEnhanced, message="Test", role="User", image_data=b"not-an-image"
        ) as pilot:
            widget = pilot.app.test_widget
            await pilot.pause()
...
class TestChatMessageEnhancedProperties:
    ...
    def test_message_text_reactive_property(self):
        ...
        message = ChatMessageEnhanced(message="Initial", role="User")
    ...
    def test_pixel_mode_watcher(self):
        ...
        message = ChatMessageEnhanced(
            message="Test", role="User", image_data=b"fake-image"
        )
...
class TestChatMessageEnhancedEdgeCases:
    ...
    async def test_no_image_data_save(self, widget_pilot, mock_app_instance):
        ...
        async with await widget_pilot(
            ChatMessageEnhanced, message="No image", role="User"
        ) as pilot:
            widget = pilot.app.test_widget
    ...
    async def test_empty_message_handling(self, widget_pilot):
        ...
        async with await widget_pilot(
            ChatMessageEnhanced, message="", role="User"
        ) as pilot:
            widget = pilot.app.test_widget
            await pilot.pause()
    ...
    async def test_missing_optional_dependencies(self, widget_pilot, sample_image_data):
        ...
        with patch(
            "tldw_chatbook.Widgets.chat_message_enhanced.TEXTUAL_IMAGE_AVAILABLE", False
        ):
            with patch(
                "tldw_chatbook.Widgets.chat_message_enhanced.Pixels",
