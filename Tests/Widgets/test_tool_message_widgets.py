...
def single_tool_call():
    ...
    return [
        {"function": {"name": "calculate", "arguments": json.dumps({"x": 5, "y": 10})}}
    ]
...
def success_tool_result():
    ...
    return [{"tool_call_id": "call_123", "result": {"answer": 42, "status": "success"}}]


@pytest.fixture
def error_tool_result():
    """Fixture for an error tool result."""
    return [{"tool_call_id": "call_456", "error": "Division by zero"}]
...
class TestToolCallMessage:
    ...
    def test_format_tool_calls_with_invalid_json(self):
        ...
        tool_calls = [
            {"function": {"name": "broken_function", "arguments": "invalid json {{"}}
        ]
        widget = ToolCallMessage(tool_calls=tool_calls)
    ...
    def test_format_tool_calls_with_long_values(self):
        ...
        text_line = [
            line for line in widget.message_text.split("\n") if "text:" in line
        ][0]
...
class TestToolResultMessage:
    ...
    def test_multiple_results_mixed_types(self):
        ...
        tool_results = [
            {"tool_call_id": "call_1", "result": {"status": "ok"}},
            {"tool_call_id": "call_2", "error": "Network timeout"},
            {"tool_call_id": "call_3", "result": [1, 2, 3]},
        ]
        widget = ToolResultMessage(tool_results=tool_results)
...
class TestToolExecutionWidget:
    ...
    def test_init_with_calls_and_results(self, single_tool_call, success_tool_result):
        ...
        widget = ToolExecutionWidget(
            tool_calls=single_tool_call, tool_results=success_tool_result
        )
    ...
    async def test_update_results_creates_widget(self, single_tool_call):
        ...
        async with app.run_test() as pilot:
            ...
            # Update with results
            tool_results = [{"tool_call_id": "async_1", "result": "completed"}]
...
class TestEdgeCases:
    ...
    def test_unicode_handling(self):
        ...
        tool_results = [{"tool_call_id": "unicode_1", "result": "Привет мир 🌏"}]
