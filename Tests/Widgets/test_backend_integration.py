...
async def test_document_processor_simulation():
    ...
    config = DocumentConfig(
        files=[Path("test.docx")], ocr_enabled=True, preserve_formatting=True
    )
...
async def test_video_processing_with_urls():
    ...
    config = VideoConfig(urls=["https://youtube.com/watch?v=test"], files=[])
...
async def test_web_processing():
    ...
    config = WebConfig(
        urls=["https://example.com/article"], extract_links=True, clean_html=True
    )
