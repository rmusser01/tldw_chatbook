...
class TestTreeNode:
    ...
    def test_file_icon_mapping(self, filename, expected_icon):
        ...
        node = TreeNode(
            path=f"test/{filename}", name=filename, is_directory=False, level=0
        )
    ...
    def test_directory_icon_states(self):
        ...
        node = TreeNode(path="src", name="src", is_directory=True, level=0)
    ...
    def test_file_size_formatting(self, size, expected):
        ...
        node = TreeNode(
            path="test.txt", name="test.txt", is_directory=False, level=0, size=size
        )
    ...
    async def test_directory_expand_button(self, widget_pilot):
        ...
        async with widget_pilot(
            TreeNode, path="src", name="src", is_directory=True, level=0
        ) as pilot:
            node = pilot.app.test_widget
    ...
    async def test_file_no_expand_button(self, widget_pilot):
        ...
        async with widget_pilot(
            TreeNode, path="test.py", name="test.py", is_directory=False, level=0
        ) as pilot:
            node = pilot.app.test_widget
...
class TestTreeView:
    ...
    def sample_tree_data(self):
        ...
        return [
            {"path": "README.md", "name": "README.md", "type": "blob", "size": 1234},
            {
                "path": "src",
                "name": "src",
                "type": "tree",
    ...
    def test_tree_view_creation(self):
        ...
        tree = TreeView(on_selection_change=on_selection, on_node_expanded=on_expansion)
    ...
    def test_get_selected_files(self):
        ...
        tree.nodes = {"src": dir_node, "src/file1.py": file1, "src/file2.py": file2}
    ...
    async def test_expand_node(self, widget_pilot):
        ...
        async with widget_pilot(TreeView) as pilot:
            ...
            # Create parent node
            parent = TreeNode(path="src", name="src", is_directory=True, level=0)
    ...
    async def test_event_handling(self, widget_pilot):
        ...
        async with widget_pilot(
            TreeView, on_selection_change=on_selection, on_node_expanded=on_expansion
        ) as pilot:
            tree = pilot.app.test_widget
