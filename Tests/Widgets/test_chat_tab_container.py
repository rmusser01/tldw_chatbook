...
def mock_config():
    ...
    with patch(
        "tldw_chatbook.Widgets.Chat_Widgets.chat_tab_container.validate_text_input"
    ) as mock_validate:
...
class TestChatTabContainer:
    ...
    async def test_on_mount_creates_initial_tab(self, tab_container):
        ...
        with patch.object(
            tab_container, "create_new_tab", new_callable=AsyncMock
        ) as mock_create:
    ...
    async def test_create_new_tab(self, tab_container):
        ...
        with patch("uuid.uuid4") as mock_uuid:
            ...
            tab_container.query_one = Mock(
                side_effect=[mock_container, mock_placeholder]
            )
    ...
    @pytest.mark.asyncio
    async def test_create_new_tab_derives_title_from_session_contract(
        self, tab_container
    ):
        ...
        with patch("uuid.uuid4") as mock_uuid:
            mock_uuid.return_value = Mock()
            mock_uuid.return_value.__str__ = Mock(
                return_value="87654321-1234-5678-1234-567812345678"
            )
    ...
    async def test_create_new_tab_does_not_reuse_same_conversation_across_different_runtime(
        ...
        with patch("uuid.uuid4") as mock_uuid:
            mock_uuid.return_value = Mock()
            mock_uuid.return_value.__str__ = Mock(
                return_value="99999999-1234-5678-1234-567812345678"
            )
    ...
    async def test_create_new_tab_max_limit(self, tab_container):
        ...
        tab_container.app_instance.notify.assert_called_once_with(
            "Maximum number of tabs (10) reached", severity="warning"
        )
    ...
    async def test_close_tab(self, tab_container):
        ...
        session = Mock(spec=ChatSession)
        session.session_data = ChatSessionData(
            tab_id=tab_id, is_ephemeral=True, has_unsaved_changes=False
        )
    ...
    async def test_close_tab_with_unsaved_changes(self, tab_container):
        ...
        session = Mock(spec=ChatSession)
        session.session_data = ChatSessionData(
            tab_id=tab_id, is_ephemeral=False, has_unsaved_changes=True
        )
    ...
    async def test_switch_to_tab(self, tab_container):
        ...
        tab_container.sessions = {"abc12345": session1, "def67890": session2}
        ...
        tab_container.tab_bar.set_active_tab.assert_called_once_with(
            "def67890", emit=False
        )
...
class TestChatTabContainerEvents:
    ...
    async def test_on_tab_closed(self, tab_container):
        ...
        with patch.object(
            tab_container, "close_tab", new_callable=AsyncMock
        ) as mock_close:
...
class TestChatTabContainerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_restore_sessions_from_state_replaces_live_tabs_and_updates_presentation(
        self, tab_container
    ):
        """Restoring sessions replaces existing mounted tabs and refreshes tab-bar presentation."""
        existing_session = Mock(spec=ChatSession)
        existing_session.session_data = ChatSessionData(
            tab_id="a1b2c3d4", title="Live Session"
        )
        ...
        tab_container._force_close_tab = AsyncMock(
            side_effect=lambda tab_id: tab_container.sessions.pop(tab_id, None)
        )
    ...
    @pytest.mark.asyncio
    async def test_restore_sessions_from_state_derives_missing_title_from_contract(
        self, tab_container
    ):
    ...
    @pytest.mark.asyncio
    async def test_restore_sessions_from_state_reuses_duplicate_persisted_conversations(
        self, tab_container
    ):
        ...
        with patch("uuid.uuid4") as mock_uuid:
            first_uuid = Mock()
            first_uuid.__str__ = Mock(
                return_value="11111111-1234-5678-1234-567812345678"
            )
            second_uuid = Mock()
            second_uuid.__str__ = Mock(
                return_value="22222222-1234-5678-1234-567812345678"
            )
...
class TestChatTabContainerErrorHandling:
    ...
    async def test_create_tab_mount_error(self, tab_container):
        ...
        assert tab_id == ""
        tab_container.app_instance.notify.assert_called_once_with(
            "Failed to add tab to container", severity="error"
        )
