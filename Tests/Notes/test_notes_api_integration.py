...
#
# Local Imports
from tldw_Server_API.app.api.v1.endpoints import notes as notes_router_module  # noqa: E402
...
MockConflictError = type(
    "ConflictError", (MockCharactersRAGDBError,), {"entity": None, "entity_id": None}
)
...
def test_app():
    app = FastAPI()
    app.include_router(
        notes_router_module.router, prefix="/api/v1/notes", tags=["Notes"]
    )
    app.dependency_overrides[notes_router_module.get_chacha_db_for_user] = (
        override_get_chacha_db_for_user
    )
    return app
...
def create_timestamped_data(
    base_data: Dict[str, Any], client_id: str
) -> Dict[str, Any]:
    ...
    if (
        "title" in base_data and "content" not in base_data
    ):  # Heuristic: if it looks like a note
...
def test_create_note_db_error(client: TestClient):
    ...
    mock_chacha_db_instance.add_note.side_effect = Actual_CharactersRAGDBError(
        "DB connection failed"
    )
    response = client.post("/api/v1/notes/", json=note_create_payload)
    assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
    assert (
        "A database error occurred while processing your request for note."
...
def test_create_note_input_error(client: TestClient):
    ...
    mock_chacha_db_instance.add_note.side_effect = Actual_InputError(
        "DB says: Title cannot be empty"
    )
...
def test_create_note_conflict_error(client: TestClient):
    ...
    mock_chacha_db_instance.add_note.side_effect = Actual_ConflictError(
        message=f"Note with ID '{note_id_val}' already exists.",
        entity="note",
    ...
    assert (
        f"A conflict occurred with note (ID: {note_id_val})."
        in response.json()["detail"]
    )
...
def test_list_notes(client: TestClient):
    ...
    mock_notes_data = [
        create_timestamped_data(
            {"id": note1_id, "title": "Note 1", "content": "Content for Note 1"},
            expected_db_client_id,
        ),
        create_timestamped_data(
            {"id": note2_id, "title": "Note 2", "content": "Content for Note 2"},
            expected_db_client_id,
        ),
    ]
...
def test_update_note(client: TestClient):
    ...
    mock_chacha_db_instance.update_note.assert_called_once_with(
        ...
        expected_version=expected_version_header,
    )
...
def test_update_note_conflict(client: TestClient):
    ...
    response = client.put(
        f"/api/v1/notes/{note_id_val}",
        json=update_payload,
        headers={"expected-version": str(wrong_version_header)},
    )
    assert response.status_code == status.HTTP_409_CONFLICT
    assert (
...
def test_update_note_no_fields(client: TestClient):
    note_id_val = str(uuid.uuid4())
    response = client.put(
        f"/api/v1/notes/{note_id_val}", json={}, headers={"expected-version": "1"}
    )
    assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_delete_note_conflict(client: TestClient):
    ...
    response = client.delete(
        f"/api/v1/notes/{note_id_val}", headers={"expected-version": "1"}
    )
    assert response.status_code == status.HTTP_409_CONFLICT
    assert (
        "The resource has been modified since you last fetched it"
...
def test_search_notes(client: TestClient):
    ...
    mock_chacha_db_instance.search_notes.assert_called_once_with(
        search_term=query_term, limit=5
    )
...
def test_create_keyword(client: TestClient):
    ...
    mock_chacha_db_instance.add_keyword.assert_called_once_with(
        keyword_text="ProjectAlpha"
    )
...
def test_get_keyword(client: TestClient):
    ...
    data = response.json()
    assert data["id"] == keyword_id_val
    mock_chacha_db_instance.get_keyword_by_id.assert_called_once_with(
...
def test_link_note_to_keyword(client: TestClient):
    ...
    expected_db_client_id = "test_api_client_for_user_db"
    mock_chacha_db_instance.get_note_by_id.return_value = create_timestamped_data(
        {"id": note_id_val, "content": "linking note"},
        client_id=expected_db_client_id,  # Added content
    )
    mock_chacha_db_instance.get_keyword_by_id.return_value = create_timestamped_data(
        {"id": keyword_id_val, "keyword": "linking keyword"},
        client_id=expected_db_client_id,
    )
...
def test_link_note_to_keyword_note_not_found(client: TestClient):
    ...
    mock_chacha_db_instance.get_note_by_id.return_value = None
    mock_chacha_db_instance.get_keyword_by_id.return_value = create_timestamped_data(
        {"id": keyword_id_val, "keyword": "Some Keyword"},
        client_id=expected_db_client_id,
    )
    response = client.post(f"/api/v1/notes/{note_id_val}/keywords/{keyword_id_val}")
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_get_keywords_for_note(client: TestClient):
    ...
    expected_db_client_id = "test_api_client_for_user_db"
    mock_chacha_db_instance.get_note_by_id.return_value = create_timestamped_data(
        {"id": note_id_val, "content": "note with keywords"},
        client_id=expected_db_client_id,  # Added content
    )
    mock_chacha_db_instance.get_keywords_for_note.return_value = [
        create_timestamped_data(
            {"id": k1_id, "keyword": "Tag1"}, expected_db_client_id
        ),
        create_timestamped_data(
            {"id": k2_id, "keyword": "Tag2"}, expected_db_client_id
        ),
    ]
    ...
    mock_chacha_db_instance.get_keywords_for_note.assert_called_once_with(
        note_id=note_id_val
    )
