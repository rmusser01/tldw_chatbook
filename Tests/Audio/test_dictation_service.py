...
class TestLiveDictationService:
    ...
    def mock_recording_service(self):
        ...
        with patch(
            "tldw_chatbook.Audio.dictation_service.AudioRecordingService"
        ) as mock_class:
            mock_instance = Mock()
    ...
    def mock_transcription_service(self):
        ...
        with patch(
            "tldw_chatbook.Audio.dictation_service.TranscriptionService"
        ) as mock_class:
            mock_instance = Mock()
            mock_class.return_value = mock_instance
            yield mock_instance
    ...
    def dictation_service(self, mock_recording_service, mock_transcription_service):
        ...
        service = LiveDictationService(transcription_provider="test", language="en")
        return service
    ...
    def test_start_dictation_recording_failure(
        self, dictation_service, mock_recording_service
    ):
    ...
    def test_streaming_transcriber_initialization(
        self, dictation_service, mock_transcription_service
    ):
        """Test streaming transcriber setup."""
        mock_transcriber = Mock()
        mock_transcription_service.create_streaming_transcriber.return_value = (
            mock_transcriber
        )
        ...
        mock_transcription_service.create_streaming_transcriber.assert_called_once_with(
            provider="test", model=None, language="en"
        )
    ...
    def test_process_audio_buffer_without_streaming(
        self, dictation_service, mock_transcription_service
    ):
    ...
    def test_processing_loop_with_silence_timeout(self, dictation_service):
        ...
        with patch.object(
            dictation_service, "_finalize_current_segment"
        ) as mock_finalize:
...
class TestDictationResult:
    ...
    def test_dictation_result_defaults(self):
        ...
        result = DictationResult(transcript="Test", segments=[], duration=1.0)
