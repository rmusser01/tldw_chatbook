    ...
    def test_recording_to_file_integration(self, simulated_audio_data):
        ...
        with patch("tldw_chatbook.Audio.recording_service.PYAUDIO_AVAILABLE", True):
            with patch(
                ...
                chunk_size = 1024
                chunks = [
                    simulated_audio_data[i : i + chunk_size]
        ...
        with patch("tldw_chatbook.Audio.recording_service.PYAUDIO_AVAILABLE", True):
            with patch("tldw_chatbook.Audio.recording_service.pyaudio.PyAudio"):
                ...
                service = LiveDictationService(
                    transcription_provider="faster-whisper", language="en"
                )
    ...
    def test_voice_activity_detection_integration(self):
        ...
        with patch("tldw_chatbook.Audio.recording_service.PYAUDIO_AVAILABLE", True):
            with patch("tldw_chatbook.Audio.recording_service.VAD_AVAILABLE", True):
                with patch(
                    "tldw_chatbook.Audio.recording_service.webrtcvad.Vad"
                ) as mock_vad_class:
                    # Setup VAD mock
                    mock_vad = Mock()
...
class TestErrorRecovery:
    ...
    def test_recording_preserves_chunks_before_stream_error(self):
        """Test that a stream error stops capture without losing prior chunks."""
        with patch("tldw_chatbook.Audio.recording_service.PYAUDIO_AVAILABLE", True):
            with patch(
                "tldw_chatbook.Audio.recording_service.pyaudio.PyAudio"
            ) as mock_pa_class:
                ...
                assert chunks == [b"\x00\x01" * 512, b"\x00\x01" * 512]
