...
def _stub_settings(monkeypatch, buffer_duration_ms: int) -> None:
    ...
    # content for flush/timing behavior unrelated to voice activity. A real
    # VAD would (correctly) treat it as silence and gate it out; force the
    # "no VAD available" path so `_chunk_has_speech` degrades to its
    # pre-VAD always-true behavior, same as a machine without webrtcvad
    # installed. See Tests/Audio/test_dictation_vad_finalization.py for the
    # dedicated VAD-gating coverage.
