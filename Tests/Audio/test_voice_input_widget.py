...
class TestVoiceInputWidget:
    ...
    def mock_dictation_service(self):
        ...
        with patch(
            "tldw_chatbook.Widgets.voice_input_widget.LiveDictationService"
        ) as mock_class:
            mock_instance = Mock()
            ...
            mock_instance.stop_dictation.return_value = Mock(
                transcript="Test transcript", duration=5.0, word_count=2
            )
        ...
        class TestApp(App):
            def compose(self):
                yield VoiceInputWidget(
                    show_device_selector=True, show_transcript_preview=True
                )
    ...
    async def test_device_selector_populated(self, widget_app, mock_dictation_service):
        ...
        device_options = [
            option
            for option in device_selector._options
    ...
    @pytest.mark.asyncio
    async def test_record_button_starts_recording(
        self, widget_app, mock_dictation_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_record_button_stops_recording(
        self, widget_app, mock_dictation_service
    ):
    ...
    async def test_widget_without_device_selector(self):
        ...
        class TestApp(App):
            def compose(self):
                yield VoiceInputWidget(
                    show_device_selector=False, show_transcript_preview=True
                )
    ...
    async def test_widget_without_transcript_preview(self):
        ...
        class TestApp(App):
            def compose(self):
                yield VoiceInputWidget(
                    show_device_selector=True, show_transcript_preview=False
                )
        ...
        with patch(
            ...
            async with app.run_test() as pilot:
                ...
                mock_service.stop_dictation.return_value = Mock(
                    transcript="Hello world", duration=2.0, word_count=2
                )
