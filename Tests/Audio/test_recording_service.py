...
class TestAudioRecordingService:
    ...
    def test_initialization_no_backend_available(self):
        ...
        with patch("tldw_chatbook.Audio.recording_service.PYAUDIO_AVAILABLE", False):
            with patch(
                "tldw_chatbook.Audio.recording_service.SOUNDDEVICE_AVAILABLE", False
            ):
    ...
    def test_set_device_valid(self, mock_pyaudio):
        ...
        with patch.object(
            service,
            "get_audio_devices",
            return_value=[{"id": 0, "name": "Device 1"}, {"id": 1, "name": "Device 2"}],
        ):
    ...
    def test_set_device_invalid(self, mock_pyaudio):
        ...
        # Mock get_audio_devices to return a device
        with patch.object(
            service, "get_audio_devices", return_value=[{"id": 0, "name": "Device 1"}]
        ):
    ...
    def test_recording_session_context_manager(self, mock_pyaudio):
        ...
        with patch.object(service, "start_recording", return_value=True) as mock_start:
            with patch.object(
                service, "stop_recording", return_value=b"audio"
            ) as mock_stop:
    ...
    @pytest.mark.parametrize(
        ...
        [(8000, 1, 512), (16000, 2, 1024), (44100, 1, 2048), (48000, 2, 4096)],
    )
    def test_various_audio_configurations(
        self, mock_pyaudio, sample_rate, channels, chunk_size
    ):
    ...
    def test_device_enumeration_error(self):
        ...
        with patch("tldw_chatbook.Audio.recording_service.PYAUDIO_AVAILABLE", True):
            with patch(
                ...
                mock_pa_instance.get_device_count.side_effect = Exception(
                    "Audio system error"
                )
