...
class TestConversationManagement:
    ...
    def test_create_conversation_with_initial_messages(
        ...
        conv_id = create_conversation(
            db_instance, character_id=char_id, initial_messages=initial_messages
        )
        assert conv_id is not None
        ...
        assert (
            messages[1]["sender"] == "Talker"
        )  # Sender 'AI' is mapped to character name
    ...
    def test_get_conversation_details_and_messages(self, db_instance: CharactersRAGDB):
        ...
        conv_id = create_conversation(
            db_instance, character_id=char_id, title="Test Details"
        )
...
class TestCharacterLoading:
    def test_restore_character_card_undeletes_with_optimistic_version(
        self, db_instance: CharactersRAGDB
    ):
        ...
        assert (
            db_instance.soft_delete_character_card(char_id, original["version"]) is True
        )
        assert db_instance.get_character_card_by_id(char_id) is None
        assert (
            db_instance.restore_character_card(char_id, original["version"] + 1) is True
        )
    ...
    def test_load_character_and_image(self, db_instance: CharactersRAGDB):
        ...
        char_data, history, img = load_character_and_image(
            db_instance, char_id, "Tester"
        )
    ...
    def test_load_chat_and_character(self, db_instance: CharactersRAGDB):
        ...
        char_data, history, img = load_chat_and_character(
            db_instance, conv_id, "TestUser"
        )

        assert char_data["name"] == "Chatter"
        assert (
            len(history) == 1
        )  # Only the two added messages, first_message not included in conversation history
    ...
    def test_load_character_wrapper(self, db_instance: CharactersRAGDB):
        ...
        # Test with UI string
        char_data_str, _, _ = load_character_wrapper(
            db_instance, f"WrappedChar (ID: {char_id})", "User"
        )
...
class TestCharacterImport:
    ...
    def test_import_chat_history_and_save(self, db_instance: CharactersRAGDB):
        ...
        # 3. Import the log
        conv_id, new_char_id = load_chat_history_from_file_and_save_to_db(
            db_instance, file_obj
        )
...
class TestHighLevelChatFlow:
    def test_start_new_chat_session(self, db_instance: CharactersRAGDB):
        char_id = db_instance.add_character_card(
            {"name": "SessionStart", "first_message": "Greetings!"}
        )

        conv_id, char_data, ui_history, img = start_new_chat_session(
            db_instance, char_id, "TestUser"
        )
    ...
    def test_post_message_to_conversation(self, db_instance: CharactersRAGDB):
        ...
        # Post user message
        user_msg_id = post_message_to_conversation(
            db_instance, conv_id, "Poster", "User msg", is_user_message=True
        )
        assert user_msg_id is not None
        # Post character message
        char_msg_id = post_message_to_conversation(
            db_instance, conv_id, "Poster", "Char reply", is_user_message=False
        )
    ...
    def test_retrieve_conversation_messages_for_ui(self, db_instance: CharactersRAGDB):
        ...
        conv_id = start_new_chat_session(db_instance, char_id, "TestUser")[
            0
        ]  # Get conv_id
        post_message_to_conversation(
            db_instance, conv_id, "UIRetriever", "User message", True
        )
        post_message_to_conversation(
            db_instance, conv_id, "UIRetriever", "Bot reply", False
        )

        ui_history = retrieve_conversation_messages_for_ui(
            db_instance, conv_id, "UIRetriever", "TestUser"
        )
