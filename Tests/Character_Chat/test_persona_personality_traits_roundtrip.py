...
The persona service is DTO-driven (`create_user_profile` validates
`UserProfileCreate` -> `model_dump` -> JSON store; `_persona_profile_view`
...
def test_personality_traits_persists_and_reloads(tmp_path):
    ...
    svc = LocalCharacterPersonaService(None, user_profile_store_path=store_path)

    created = svc.create_user_profile(
        UserProfileCreate(
    ...
    svc2 = LocalCharacterPersonaService(None, user_profile_store_path=store_path)
    got = svc2.get_user_profile("guide")
...
def test_personality_traits_defaults_to_empty_string():
    ...
    created = svc.create_user_profile(UserProfileCreate(name="Blank"))
...
def test_personality_traits_updates_and_reloads(tmp_path):
    ...
    svc = LocalCharacterPersonaService(None, user_profile_store_path=store_path)
    created = svc.create_user_profile(
        UserProfileCreate(id="guide", name="Guide", personality_traits="calm")
    )

    updated = svc.update_user_profile(
        "guide",
        UserProfileUpdate(personality_traits="fierce, loyal"),
...
def test_personality_traits_update_to_empty_string_clears_and_reloads(tmp_path):
    ...
    ``update_user_profile`` builds its patch via
    ``UserProfileUpdate.model_dump(exclude_none=True)`` - which excludes
    ...
    svc = LocalCharacterPersonaService(None, user_profile_store_path=store_path)
    created = svc.create_user_profile(
        UserProfileCreate(id="guide", name="Guide", personality_traits="x")
    )
    assert created["personality_traits"] == "x"

    updated = svc.update_user_profile(
        "guide",
        UserProfileUpdate(personality_traits=""),
        expected_version=created["version"],
    )
    ...
    # in-memory record returned by update_user_profile looks right.
    svc2 = LocalCharacterPersonaService(None, user_profile_store_path=store_path)
    got = svc2.get_user_profile("guide")
