...
class TestCharacterImageUpload:
    ...
    def test_update_character_add_image(self, db_instance, png_image_bytes):
        ...
        # Update with image
        update_data = {"image": png_image_bytes}
        updated = db_instance.update_character_card(
            char_id, update_data, current_version
        )
    ...
    def test_export_import_character_with_image(
        self, db_instance, png_image_bytes, tmp_path
    ):
        ...
        success = export_character_card_to_png(
            db_instance, char_id, str(export_path), str(tmp_path)
        )
        ...
        db_instance.update_character_card(
            char_id, {"name": "Old Character"}, char["version"]
        )
    ...
    def test_character_json_export_excludes_image_bytes(
        self, db_instance, png_image_bytes
    ):
        ...
        json_str_no_img = export_character_card_to_json(
...
class TestCharacterImageDisplay:
    ...
    def test_character_without_image_shows_placeholder(self, db_instance):
        ...
        char_data = {"name": "No Image Display Test", "description": "No image"}
        char_id = db_instance.add_character_card(char_data)
