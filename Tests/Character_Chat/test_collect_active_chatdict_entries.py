...
def test_character_dictionary_fires_in_a_send(db):
    ...
    dict_id = service.create_dictionary(
        {"name": "CharDict", "entries": [{"pattern": "hi", "replacement": "hello"}]}
    )["id"]
    ...
    rec_ext["chat_dictionaries"] = [
        {
            "name": "Shared",
            "enabled": True,
            "entries": [{"key": "s", "content": "CHAR"}],
        },
        {"name": "Extra", "enabled": True, "entries": [{"key": "e", "content": "x"}]},
    ]
    db.update_character_card(
        char_id, {"extensions": rec_ext}, expected_version=rec["version"]
    )
    char_data = db.get_character_card_by_id(char_id)
...
def test_collect_disabled_conversation_dict_does_not_shadow_character(db):
    ...
    service = LocalChatDictionaryService(db)
    conv_id = db.add_conversation({"title": "chat"})
    did = service.create_dictionary(
        {"name": "Dup", "entries": [{"pattern": "a", "replacement": "conv"}]}
    )["id"]
    rec = service.get_dictionary(did)
    service.update_dictionary(
        did, {"is_active": False}, expected_version=rec["version"]
    )  # disable it
    conv = db.get_conversation_by_id(conv_id)
    ...
    r = db.get_character_card_by_id(char_id)
    ext = r["extensions"] if isinstance(r["extensions"], dict) else {}
    ext["chat_dictionaries"] = [
        {"name": "Dup", "enabled": True, "entries": [{"key": "a", "content": "char"}]}
    ]
    db.update_character_card(
        char_id, {"extensions": ext}, expected_version=r["version"]
    )
    entries = collect_active_chatdict_entries(
        db, conv_id, db.get_character_card_by_id(char_id)
    )
    assert [e.content for e in entries] == [
        "char"
    ]  # character applies; disabled conv dict didn't shadow
