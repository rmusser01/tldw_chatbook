...
def test_entry_diagnostic_to_dict_round_trips_fields():
    rec = WorldBookEntryDiagnostic(
        ...
        content_preview="The grim jailer…",
        depth_level=0,
    )
    assert rec.to_dict() == {
        "entry_id": 7,
...
def test_plain_process_messages_byte_identical_disabled_and_selective():
    book = _book(
        ...
        [
            ...
            _entry(
                3, ["Vault"], "sealed door", selective=True, secondary_keys=["gold"]
            ),
        ],
    )
    proc = WorldInfoProcessor(world_books=[book])
    before = copy.deepcopy(
        proc.process_messages("The Warden guards the Vault of gold.", [])
    )
...
def test_classify_entry_match_decomposes_reason():
    book = _book(
        1, "B", [_entry(3, ["Vault"], "x", selective=True, secondary_keys=["gold"])]
    )
    proc = WorldInfoProcessor(world_books=[book])
    entry = next(c for c in proc._candidate_entries if c["_entry_id"] == 3)
    text = "The Vault is sealed."
    primary_hit, pk, sec_req, sec_hit, sk = proc._classify_entry_match(
...
def test_diagnostics_multi_book_priority_offset_agrees_with_plain():
    hi = _book(
        10, "Hi", [_entry(1, ["Warden"], "hi-content")], priority=1
    )  # offset 1000
...
def test_high_priority_entry_survives_budget_over_low_priority():
    ...
    book = _book(
        ...
        [
            _entry(
                1, ["low"], "AAAA " * 200, insertion_order=0, priority=0
            ),  # ~250 tok, first by order
            _entry(
                2, ["high"], "BBBB " * 200, insertion_order=1, priority=90
            ),  # ~250 tok, high priority
        ],
        token_budget=300,
    )  # fits exactly one entry, not two
    proc = WorldInfoProcessor(world_books=[book])
...
def test_low_book_token_budget_is_honored():
    ...
    book = _book(
        ...
        [
            ...
            _entry(
                2, ["b"], "BBBB " * 30, insertion_order=1
            ),  # ~30 tok, over a 40-token budget
        ],
        token_budget=40,
    )
    proc = WorldInfoProcessor(world_books=[book])
...
def test_recursive_entry_reorders_by_insertion_order():
    ...
    assert injected.index("a fearsome dragon") < injected.index(
        "the castle guards a dragon"
    )
