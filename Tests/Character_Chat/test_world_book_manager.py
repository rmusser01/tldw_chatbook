...
class TestWorldBookManager:
    ...
    def test_update_world_book_optimistic_locking(self, wb_manager):
        ...
        success = wb_manager.update_world_book(
            wb_id, description="Update 1", expected_version=1
        )
        assert success is True

        # Update with wrong version should fail
        with pytest.raises(ConflictError):
            wb_manager.update_world_book(
                wb_id, description="Update 2", expected_version=1
            )
    ...
    def test_update_world_book_entry(self, wb_manager):
        ...
        entry_id = wb_manager.create_world_book_entry(
            wb_id, keys=["original"], content="Original content"
        )
    ...
    def test_delete_world_book_entry(self, wb_manager):
        ...
        entry_id = wb_manager.create_world_book_entry(
            wb_id, keys=["delete"], content="To delete"
        )
    ...
    def test_conversation_associations(self, wb_manager):
        ...
        conv_id = db.add_conversation(
            {"character_id": character_id, "title": "Test Conversation"}
        )
    ...
    def test_export_world_book(self, wb_manager):
        ...
        wb_manager.create_world_book_entry(
            wb_id, keys=["sword"], content="A legendary blade", position="after_char"
        )
    ...
    def test_import_with_name_override(self, wb_manager):
        ...
        wb_id = wb_manager.import_world_book(
            import_data, name_override="Existing Book (Copy)"
        )
...
class TestWorldInfoProcessorIntegration:
    ...
    def test_processor_with_world_books(self, wb_manager):
        ...
        wb1_id = wb_manager.create_world_book(
            name="Book 1", scan_depth=3, token_budget=500
        )
        wb_manager.create_world_book_entry(
            wb1_id,
            keys=["kingdom"],
        ...
        wb2_id = wb_manager.create_world_book(
            name="Book 2", scan_depth=5, token_budget=1000
        )
        wb_manager.create_world_book_entry(
            wb2_id, keys=["hero"], content="The hero arrives", position="after_char"
        )
        ...
        # Test processing
        result = processor.process_messages("The hero enters the kingdom", [])
    ...
    def test_processor_with_character_and_world_books(self, wb_manager):
        ...
        wb_manager.create_world_book_entry(
            wb_id, keys=["world"], content="World background info", position="at_start"
        )
        ...
        processor = WorldInfoProcessor(
            character_data=character_data, world_books=world_books
        )
...
def test_entry_priority_round_trips(wb_manager):
    ...
    eid = wb_manager.create_world_book_entry(
        book_id, ["Warden"], "grim jailer", priority=75
    )
