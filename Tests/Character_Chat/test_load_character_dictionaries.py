...
def test_skips_malformed_blocks_and_entries_without_raising():
        ...
        "extensions": {
            "chat_dictionaries": [
                ...
                {"name": "", "entries": []},  # no name → skipped
                {
                    "name": "Bad",
                    "entries": [{"content": "x"}],
                },  # entry missing 'key' → entry skipped
    ...
    assert (
        load_character_dictionaries(char)[0]["entries"] == []
    )  # 'Bad' dropped its bad entry
...
def test_tolerates_none_and_missing_and_string_extensions():
    ...
    assert (
        load_character_dictionaries({"extensions": '{"chat_dictionaries": []}'}) == []
    )
