...
class FakeWritingService:
    ...
    async def update_manuscript(
        self, manuscript_id, *, expected_version=None, **kwargs
    ):
        self.calls.append(
            ("update_manuscript", manuscript_id, expected_version, kwargs)
        )
        return {
            "id": manuscript_id,
    ...
    async def restore_version(
        self, entity_type, entity_id, version_number, *, expected_version=None
    ):
        self.calls.append(
            (
                "restore_version",
        ...
        return [
            {
                ...
                "is_pov": kwargs.get("is_pov", False),
            }
        ]
    ...
    async def analyze_scene(self, scene_id, **kwargs):
        ...
        return [
            {
                "id": f"{self.source}-analysis-1",
                "scope_type": "scene",
                "scope_id": scene_id,
                "analysis_type": kwargs["analysis_types"][0],
            }
        ]
...
async def test_writing_scope_service_routes_by_backend_and_policy():
    ...
    server_project = await scope.create_project(
        mode="server", title="Novel", author="Ada"
    )
    manuscript = await scope.create_manuscript(
        mode="server", project_id=server_project["id"], title="Book One"
    )
...
async def test_writing_scope_service_routes_update_and_delete_crud_actions():
    ...
    project = await scope.update_project(
        mode="server",
    ...
    deleted_scene = await scope.delete_scene(
        mode="server", scene_id="server-scene-1", expected_version=2
    )
...
async def test_writing_scope_service_routes_manual_version_actions():
    ...
    versions = await scope.list_versions(
        mode="server", entity_type="scene", entity_id="scene-1"
    )
    fetched = await scope.get_version(
        mode="server", entity_type="scene", entity_id="scene-1", version_number=1
    )
...
async def test_writing_scope_service_routes_structure_actions():
    ...
    assert (
        structure["manuscripts"][0]["record_id"]
        == "server:writing_manuscript:server-manuscript-1"
    )
    assert (
        structure["unassigned_chapters"][0]["record_id"]
        == "server:writing_chapter:server-chapter-loose"
    )
    assert (
        structure["unassigned_chapters"][0]["outline_bucket"] == "unassigned_chapters"
    )
...
async def test_writing_scope_service_routes_auxiliary_server_actions():
    ...
    character = await scope.create_character(
        mode="server", project_id="project-1", name="Ada"
    )
    characters = await scope.list_characters(
        mode="server", project_id="project-1", role="protagonist"
    )
    world = await scope.create_world_info(
        mode="server", project_id="project-1", kind="location", name="Capital"
    )
    plot_line = await scope.create_plot_line(
        mode="server", project_id="project-1", title="Main Plot"
    )
    plot_event = await scope.create_plot_event(
        mode="server", plot_line_id=plot_line["id"], title="Inciting Incident"
    )
    plot_hole = await scope.create_plot_hole(
        mode="server", project_id="project-1", title="Continuity Issue"
    )
    scene_characters = await scope.link_scene_character(
        mode="server", scene_id="scene-1", character_id=character["id"], is_pov=True
    )
    scene_world = await scope.link_scene_world_info(
        mode="server", scene_id="scene-1", world_info_id=world["id"]
    )
    citation = await scope.create_citation(
        mode="server", scene_id="scene-1", source_type="manual"
    )
    research = await scope.research_scene(
        mode="server", scene_id="scene-1", query="context"
    )
    scene_analyses = await scope.analyze_scene(
        mode="server", scene_id="scene-1", analysis_types=["pacing"]
    )
    chapter_analyses = await scope.analyze_chapter(
        mode="server", chapter_id="chapter-1", analysis_types=["continuity"]
    )
    plot_hole_analyses = await scope.analyze_project_plot_holes(
        mode="server", project_id="project-1"
    )
    consistency_analyses = await scope.analyze_project_consistency(
        mode="server", project_id="project-1"
    )
    listed_analyses = await scope.list_analyses(
        mode="server", project_id="project-1", scope_type="scene"
    )
    ...
    assert (
        ...
        == "server:writing_scene_character_link:scene-1:server-character-1"
    )
    assert (
        scene_world[0]["record_id"]
        == "server:writing_scene_world_info_link:scene-1:server-world-1"
    )
    assert citation["record_id"] == "server:writing_citation:server-citation-1"
    assert (
        research["results"][0]["record_id"]
        == "server:writing_research_result:server-source-1"
    )
    assert scene_analyses[0]["record_id"] == "server:writing_analysis:server-analysis-1"
    assert (
        chapter_analyses[0]["record_id"] == "server:writing_analysis:server-analysis-2"
    )
    assert (
        plot_hole_analyses[0]["record_id"]
        == "server:writing_analysis:server-analysis-3"
    )
    assert (
        consistency_analyses[0]["record_id"]
        == "server:writing_analysis:server-analysis-4"
    )
    assert (
        listed_analyses["analyses"][0]["record_id"]
        == "server:writing_analysis:server-analysis-5"
    )
    assert server.calls == [
        ...
        (
            "link_scene_character",
            "scene-1",
            {"character_id": "server-character-1", "is_pov": True},
        ),
        ...
        (
            "analyze_scene",
            "scene-1",
            {"analysis_types": ["pacing"], "provider": None, "model": None},
        ),
        (
            "analyze_chapter",
            "chapter-1",
            {"analysis_types": ["continuity"], "provider": None, "model": None},
        ),
        (
            "analyze_project_plot_holes",
            "project-1",
            {"analysis_types": None, "provider": None, "model": None},
        ),
        (
        ...
        (
            "list_analyses",
            "project-1",
            {"scope_type": "scene", "analysis_type": None, "include_stale": False},
        ),
    ]
    assert policy.calls == [
