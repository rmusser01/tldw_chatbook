...
st_character_card_data = st.tuples(
    ...
    st.one_of(
        st.none(), st.lists(st.text(max_size=50)).map(json.dumps)
    ),  # alternate_greetings as json string
    st.one_of(
        st.none(), st.lists(st.text(max_size=20)).map(json.dumps)
    ),  # tags as json string
).map(
    lambda t: {
        "name": t[0],
...
st_json_dict = st.dictionaries(st.text(max_size=20), st.text(max_size=100)).map(
    json.dumps
)
...
class TestCharacterCardProperties:
    ...
    def test_character_card_roundtrip(
        self, db_instance: CharactersRAGDB, card_data: dict
    ):
        """
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(
        initial_card=st_character_card_data(), update_payload=st_character_card_data()
    )
    def test_update_increments_version_and_changes_data(
        self, db_instance: CharactersRAGDB, initial_card: dict, update_payload: dict
    ):
        """
        ...
        try:
            success = db_instance.update_character_card(
                card_id, update_payload, expected_version=original_card["version"]
            )
    ...
    def test_soft_delete_makes_item_unfindable(
        ...
        success = db_instance.soft_delete_character_card(
            card_id, expected_version=original_card["version"]
        )
        assert success is True
        ...
        raw_record = conn.execute(
            "SELECT deleted, version FROM character_cards WHERE id = ?", (card_id,)
        ).fetchone()
    ...
    def test_update_with_stale_version_always_fails(
        self,
        ...
        with pytest.raises(ConflictError):
            db_instance.update_character_card(
                card_id, update_payload, expected_version=stale_version
            )
    ...
    def test_update_does_not_change_immutable_fields(
        self, db_instance: CharactersRAGDB, initial_card: dict, update_payload: dict
    ):
        """
        ...
        malicious_payload["created_at"] = (
            "1999-01-01T00:00:00Z"  # Try to change creation time
        )

        try:
            db_instance.update_character_card(
                card_id, malicious_payload, expected_version=original_card["version"]
            )
        except ConflictError:
...
class TestNoteAndKeywordProperties:
    ...
    def test_add_keyword_is_idempotent_on_undelete(
        self, db_instance: CharactersRAGDB, keyword: str
    ):
        """
        ...
        kw_v2_raw = (
            ...
            .execute("SELECT * FROM keywords WHERE id = ?", (kw_id_v1,))
            .fetchone()
        )
    ...
    def test_linking_and_unlinking_properties(
        self, db_instance: CharactersRAGDB, note_data: dict, keyword_text: str
    ):
        """
...
class TestAdvancedProperties:
    ...
    def test_soft_deleted_item_is_not_in_fts(
        self, db_instance: CharactersRAGDB, note_data: dict
    ):
        """
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(note_data=st_note_data())
    def test_add_creates_sync_log_entry(
        self, db_instance: CharactersRAGDB, note_data: dict
    ):
        """
        ...
        new_log_entries = db_instance.get_sync_log_entries(
        ...
        assert (
            log_entry["payload"]["title"] == note_data["title"].strip()
        )  # The log stores the stripped version
        assert log_entry["payload"]["content"] == note_data["content"]

    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(note_data=st_note_data(), update_content=st.text(max_size=500))
    def test_update_creates_sync_log_entry(
        self, db_instance: CharactersRAGDB, note_data: dict, update_content: str
    ):
        """
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(note_data=st_note_data())
    def test_delete_creates_sync_log_entry(
        self, db_instance: CharactersRAGDB, note_data: dict
    ):
        """
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(note_data=st_note_data(), keyword_text=st_required_text)
    def test_link_action_creates_correct_sync_log(
        self, db_instance: CharactersRAGDB, note_data: dict, keyword_text: str
    ):
        """
        ...
        link_log_entries = db_instance.get_sync_log_entries(
            since_change_id=latest_change_id_before, entity_type="note_keywords"
        )
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(note_data=st_note_data(), keyword_text=st_required_text)
    def test_unlink_action_creates_correct_sync_log(
        self, db_instance: CharactersRAGDB, note_data: dict, keyword_text: str
    ):
        """
        ...
        unlink_log_entries = db_instance.get_sync_log_entries(
            since_change_id=latest_change_id_before, entity_type="note_keywords"
        )
...
def populated_conversation(db_instance: CharactersRAGDB):
    ...
    conv_id = db_instance.add_conversation(
        {"character_id": card["id"], "title": "Cascade Conv"}
    )
    conv = db_instance.get_conversation_by_id(conv_id)

    msg_id = db_instance.add_message(
        {"conversation_id": conv["id"], "sender": "user", "content": "Cascade Msg"}
    )
...
class TestCascadeAndLinkingProperties:
    def test_soft_deleting_conversation_makes_messages_unfindable(
        ...
        db_instance.soft_delete_conversation(
            conv["id"], expected_version=conv["version"]
        )
    ...
    def test_deleting_keyword_cascades_to_link_tables(
        self, db_instance: CharactersRAGDB, populated_conversation, keyword_text: str
    ):
        """
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(note_data=st_note_data(), keyword_text=st_required_text)
    def test_unlinking_is_idempotent(
        self, db_instance: CharactersRAGDB, note_data: dict, keyword_text: str
    ):
        """
...
class CharacterCardLifecycleMachine(RuleBasedStateMachine):
    ...
    def update_card(self, card_id, update_data):
        ...
        try:
            success = self.db.update_character_card(
                card_id, update_data, self.expected_version
            )
            assert success
            self.expected_version += 1
...
class ConversationMachine(RuleBasedStateMachine):
    ...
    def create_conversation(self):
        ...
        self.conv_id = self.db.add_conversation(
            {"character_id": self.card_id, "title": "Test Chat"}
        )
    ...
    def add_message(self, content, sender):
        ...
        msg_id = self.db.add_message(
            {"conversation_id": self.conv_id, "sender": sender, "content": content}
        )
...
class TestDataIntegrity:
    def test_add_conversation_with_nonexistent_character_fails(
    ...
    def test_add_message_to_nonexistent_conversation_fails(
        self, db_instance: CharactersRAGDB
    ):
        """
    ...
    def test_rating_outside_range_fails(self, db_instance: CharactersRAGDB):
        ...
        conv_id = db_instance.add_conversation(
            {"character_id": card_id, "title": "Rating Conv"}
        )
        ...
        with pytest.raises(CharactersRAGDBError, match="Database constraint violation"):
            ...
            with db_instance.transaction():
                db_instance.execute_query(
                    "UPDATE conversations SET rating = 10 WHERE id = ?", (conv_id,)
                )
...
class TestConcurrency:
    ...
    def test_wal_mode_allows_concurrent_reads_during_write_transaction(
        ...
        def writer_thread():
            with db_instance.transaction():
                db_instance.update_character_card(
                    card_id, {"description": "long update"}, 1
                )
        ...
        assert (
            read_result[0]["description"] is None
        )  # It read the state before the writer committed.


class TestComplexQueries:
    def test_get_keywords_for_conversation_filters_deleted_keywords(
        self, db_instance: CharactersRAGDB
    ):
        """
...
class TestDBOperations:
    def test_backup_and_restore_correctness(
        self, db_instance: CharactersRAGDB, tmp_path: Path
    ):
        """
