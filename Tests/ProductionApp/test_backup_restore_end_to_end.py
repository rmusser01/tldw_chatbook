...
from textual.widgets import Input,Button,Static,Checkbox,Select
...
from Tests.Backup_Recovery.thread_diagnostics import observe_threads,observe_recovery_failures,observe_capture_review,snapshot_threads
stop_failures=observe_recovery_failures(fixture/'recovery-failures.log')
stop_capture_review=observe_capture_review(fixture/'capture-review.log')
...
selected_groups=os.environ['TLDW_F9_TEST_SELECTED_GROUPS']=='1'
...
async def main():
 ...
 async with app.run_test(headless=False,size=(120,42)) as pilot:
  ...
  if credentials:
   ...
   # its timer can correctly invalidate review while omissions are acknowledged.
   await app._perform_change_review_retention()
  ...
  assert screen.query_one('#backup-data-mode',Select).value=='all'
  if selected_groups:
   screen.query_one('#backup-data-mode',Select).value='choose'
   await pilot.pause()
   choice=screen.query_one('#backup-data-group-conversations',Checkbox)
   choice.scroll_visible(immediate=True);choice.focus();await pilot.pause()
   await pilot.press('space')
   assert choice.value and screen._options()['data_groups']==('conversations',)
  ...
  if credentials:
   assert any(item.owner=='db.agent_runs' and item.status=='included' for item in screen._preview.items)
  assert ('Complete selected coverage' if selected_groups else 'Complete coverage') in str(screen.query_one('#backup-coverage',Static).render())
  ...
  if selected_groups:
   ...
   assert doc.format_version==2 and doc.group_scope.requested_groups==('conversations',)
   assert 'settings' not in doc.group_scope.effective_groups
   assert all(row.logical_id in doc.group_scope.support_ids or group_for_owner(row.owner_id) in doc.group_scope.effective_groups for row in doc.files)
  else:
   assert doc.format_version==1 and doc.group_scope is None
  ...
  if selected_groups:
   assert plan.effective_groups==doc.group_scope.effective_groups
...
@pytest.mark.parametrize(
    "encrypted,credentials,selected_groups",
    [
        (False, False, False),
        (True, False, False),
        (True, True, False),
...
def test_f9_created_archive_restores_and_opens_through_actual_controls(
    tmp_path, encrypted, credentials, selected_groups, native_package
):
    ...
    environment = dict(
        ...
        TLDW_F9_TEST_SELECTED_GROUPS="1" if selected_groups else "0",
