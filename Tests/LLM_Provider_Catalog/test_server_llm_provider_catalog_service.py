...
def test_server_llm_provider_catalog_service_direct_client_takes_precedence_over_provider():
    client = object()
    provider = ExplodingClientProvider()
    service = ServerLLMProviderCatalogService(client=client, client_provider=provider)

    assert service._require_client() is client
    assert provider.build_calls == 0


def test_server_llm_provider_catalog_service_from_server_context_provider_is_lazy():
    client = object()
    provider = FakeClientProvider(client)
    service = ServerLLMProviderCatalogService.from_server_context_provider(provider)

    assert isinstance(service, ServerLLMProviderCatalogService)
...
def test_server_llm_provider_catalog_service_re_resolves_provider_without_service_local_client_cache():
...
def test_server_llm_provider_catalog_service_from_config_returns_provider_backed_service():
    service = ServerLLMProviderCatalogService.from_config(
