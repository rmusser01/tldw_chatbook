...
def test_authoring_module_import_does_not_import_config():
    code = (
        "import sys; import tldw_chatbook.Internal_Prompts.authoring; "
        "sys.exit(1 if 'tldw_chatbook.config' in sys.modules else 0)"
    )
    result = subprocess.run(
        [sys.executable, "-c", code], capture_output=True, text=True
    ...
    assert result.returncode == 0, (
        "tldw_chatbook.Internal_Prompts.authoring imported tldw_chatbook.config "
        f"at module import time. stderr: {result.stderr}"
    )
