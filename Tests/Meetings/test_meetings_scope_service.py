...
async def test_meetings_scope_service_routes_server_and_normalizes_records():
    ...
    sessions = await scope.list_meeting_sessions(
        mode="server", status_filter="scheduled"
    )
    ...
    events = [
        event
        async for event in scope.stream_meeting_session_events(
            "meeting-1", mode="server"
        )
    ]
    ...
    assert server.calls == [
        ...
        (
            ...
            {"name": "Default", "schema_json": {"sections": ["summary"]}},
        ),
