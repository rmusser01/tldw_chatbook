...
def test_manager_hands_cleanup_tail_to_screen_before_output_is_complete(
    ...
    try:
        ...
        backend.write((command + "\n").encode())
        assert _wait_for(holder_pid_file.exists, timeout=3.0)
        _read_until(backend, b"$", timeout=3.0)
...
def test_unrelated_darwin_enumeration_denial_does_not_poison_death_proof(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    backend = _backend(tmp_path)
    ...
    try:
        ...
        assert denied_scan.complete is True
