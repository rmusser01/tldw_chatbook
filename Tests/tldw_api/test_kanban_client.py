...
async def test_kanban_board_client_routes_crud_archive_restore_and_detail(monkeypatch):
    ...
    mocked = AsyncMock(
        side_effect=[
            {
                "boards": [_board_payload()],
                "pagination": {"total": 1, "limit": 25, "offset": 5, "has_more": False},
            },
    ...
    listed = await client.list_kanban_boards(
        include_archived=True, include_deleted=False, limit=25, offset=5
    )
    fetched = await client.get_kanban_board(
        11, include_lists=True, include_cards=False, include_archived=True
    )
    ...
    updated = await client.update_kanban_board(
        11, KanbanBoardUpdate(name="Launch v2"), expected_version=3
    )
...
async def test_kanban_list_client_routes_crud_archive_restore_and_reorder(monkeypatch):
    ...
    listed = await client.list_kanban_lists(
        11, include_archived=True, include_deleted=True
    )
    created = await client.create_kanban_list(
        11, KanbanListCreate(name="Doing", client_id="local-list-21", position=1)
    )
    reordered = await client.reorder_kanban_lists(
        11, KanbanReorderRequest(ids=[21, 22])
    )
    fetched = await client.get_kanban_list(21)
    updated = await client.update_kanban_list(
        21, KanbanListUpdate(name="Done"), expected_version=2
    )
    ...
    assert mocked.await_args_list[2].args[:2] == (
        "POST",
        "/api/v1/kanban/boards/11/lists/reorder",
    )
...
@pytest.mark.asyncio
async def test_kanban_card_client_routes_crud_move_copy_archive_restore_and_reorder(
    monkeypatch,
):
    client = TLDWAPIClient("http://localhost:8000")
    mocked = AsyncMock(
    ...
    listed = await client.list_kanban_cards(
        21, include_archived=True, include_deleted=False
    )
    ...
    reordered = await client.reorder_kanban_cards(
        21, KanbanReorderRequest(ids=[31, 32])
    )
    ...
    moved = await client.move_kanban_card(
        31, KanbanCardMoveRequest(target_list_id=22, position=0)
    )
    ...
    assert mocked.await_args_list[2].args[:2] == (
        "POST",
        "/api/v1/kanban/lists/21/cards/reorder",
    )
