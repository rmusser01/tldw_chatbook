...
@pytest.mark.asyncio
async def test_jobs_namespace_gateway_routes_status_events_and_attachment_surfaces(
    monkeypatch,
):
    ...
    await client.call_server_jobs_endpoint(
        "GET", "/api/v1/jobs/events", params={"limit": 50}
    )
    await client.call_server_jobs_endpoint(
        "POST",
        "job-1/attachments",
...
@pytest.mark.asyncio
async def test_jobs_namespace_gateway_rejects_cross_namespace_and_unsafe_routes(
    monkeypatch,
):
    client = TLDWAPIClient("http://localhost:8000")
    mocked = AsyncMock(return_value={"ok": True})
