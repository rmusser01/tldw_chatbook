...
async def test_flashcard_routes_wire_and_return_typed_models(monkeypatch):
    ...
    created_deck = await client.create_flashcard_deck(
        FlashcardDeckCreateRequest(
            name="Biology", description="Cell review", scheduler_type="fsrs"
        )
    )
...
async def test_flashcard_update_and_delete_routes_wire_correctly(monkeypatch):
    ...
    assert mocked.await_args_list[0].args[:2] == (
        "PATCH",
        "/api/v1/flashcards/card-server-1",
    )
...
async def test_flashcard_extended_routes_wire_and_return_typed_models(monkeypatch):
    ...
    mocked = AsyncMock(
        side_effect=[
                ...
                "results": [
                    {
                        "uuid": CARD_UUID,
                        "status": "updated",
                        "flashcard": _card_payload(version=2),
                    }
                ]
            },
            ...
            },
            {
                ...
                "user_message": _assistant_message(
                    id=13, role="user", content="Explain this"
                ),
    ...
    analytics = await client.get_flashcard_analytics_summary(
        deck_id=7, include_workspace_items=True
    )
    card = await client.get_flashcard(CARD_UUID)
    reset = await client.reset_flashcard_scheduling(
        CARD_UUID, FlashcardResetSchedulingRequest(expected_version=1)
    )
    set_tags = await client.set_flashcard_tags(
        CARD_UUID, FlashcardTagsUpdate(tags=["biology", "cell"])
    )
    ...
    imported = await client.import_flashcards(
        FlashcardsImportRequest(content="Deck\tFront\tBack\nBio\tQ\tA", has_header=True)
    )
    sessions = await client.list_flashcard_review_sessions(
        deck_id=7, status="completed"
    )
    assistant = await client.get_flashcard_assistant(CARD_UUID)
    response = await client.respond_flashcard_assistant(
        CARD_UUID,
        StudyAssistantRespondRequest(
            action="explain", message="Explain this", expected_thread_version=1
        ),
    )
    generated = await client.generate_flashcards(
        FlashcardGenerateRequest(text="Cells use ATP.", num_cards=1)
    )
    ...
    assert mocked.await_args_list[5].args[:2] == (
        "GET",
        f"/api/v1/flashcards/id/{CARD_UUID}",
    )
    assert mocked.await_args_list[6].args[:2] == (
        "POST",
...
async def test_flashcard_file_routes_wire_multipart_and_binary(monkeypatch):
    ...
    uploaded = await client.upload_flashcard_asset(
        ("cell.png", b"pngdata", "image/png")
    )
    content = await client.get_flashcard_asset_content(CARD_UUID)
    imported_json = await client.import_flashcards_json(
        ("cards.json", b"[]", "application/json"), max_items=10
    )
    imported_apkg = await client.import_flashcards_apkg(
        ("cards.apkg", b"apkg", "application/octet-stream")
    )
    exported = await client.export_flashcards(
        deck_id=7, format="csv", delimiter="\t", include_header=True
    )
    ...
    assert binary_mock.await_args_list[0].args[:2] == (
        "GET",
        f"/api/v1/flashcards/assets/{CARD_UUID}/content",
    )
    ...
    assert request_mock.await_args_list[2].args[:2] == (
        "POST",
        "/api/v1/flashcards/import/apkg",
    )
    assert binary_mock.await_args_list[1].args[:2] == (
        "GET",
...
async def test_study_pack_routes_wire_and_return_typed_models(monkeypatch):
    ...
    mocked = AsyncMock(
        side_effect=[
            ...
            {
                **job_payload,
                "job": {**job_payload["job"], "status": "completed"},
                "study_pack": study_pack_payload,
            },
            study_pack_payload,
    ...
    request = StudyPackCreateJobRequest(
        ...
        source_items=[
            StudyPackSourceSelection(
                source_type="note", source_id="note-1", label="Chapter notes"
            )
        ],
    )
    ...
    assert mocked.await_args_list[1].args[:2] == (
        "GET",
        "/api/v1/flashcards/study-packs/jobs/42",
    )
