...
@pytest.mark.asyncio
async def test_mcp_governance_client_routes_external_server_and_policy_calls(
    monkeypatch,
):
    ...
    servers = await client.list_mcp_external_servers(
    ...
    updated_server = await client.update_mcp_external_server(
        "github", MCPExternalServerUpdate(enabled=False)
    )
    deleted_server = await client.delete_mcp_external_server("github")
    secret = await client.set_mcp_external_server_secret(
        "github", MCPSecretSetRequest(secret="token")
    )
    profiles = await client.list_mcp_permission_profiles(
        owner_scope_type="team", owner_scope_id=7
    )
    ...
    updated_policy = await client.update_mcp_permission_profile(
        11, {"is_active": False}
    )
    deleted_policy = await client.delete_mcp_permission_profile(11)
    approvals = await client.list_mcp_approval_policies(
        owner_scope_type="team", owner_scope_id=7
    )
    created_approval = await client.create_mcp_approval_policy(
        MCPApprovalPolicyCreate(
            name="Ask", owner_scope_type="team", owner_scope_id=7, mode="ask_every_time"
        )
    )
    updated_approval = await client.update_mcp_approval_policy(
        12, MCPApprovalPolicyUpdate(mode="allow_silently")
    )
    ...
    assert mocked.await_args_list[2].args[:2] == (
        "PUT",
        "/api/v1/mcp/hub/external-servers/github",
    )
    ...
    assert mocked.await_args_list[4].args[:2] == (
        "POST",
        "/api/v1/mcp/hub/external-servers/github/secret",
    )
    ...
    assert mocked.await_args_list[7].args[:2] == (
        "PUT",
        "/api/v1/mcp/hub/permission-profiles/11",
    )
    ...
    assert mocked.await_args_list[11].args[:2] == (
        "PUT",
        "/api/v1/mcp/hub/approval-policies/12",
    )
    ...
    assert mocked.await_args_list[14].kwargs["params"] == {
...
async def test_mcp_governance_client_routes_catalog_and_registry_calls(monkeypatch):
    ...
    mocked = AsyncMock(
        side_effect=[
            ...
            {
                "mapping_id": "filesystem-read",
                "normalized_mapping": {"capability_name": "filesystem.read"},
            },
            {"id": 21, "mapping_id": "filesystem-read"},
    ...
    team_catalog = await client.create_mcp_team_tool_catalog(
        7, MCPCatalogCreate(name="Team Tools")
    )
    org_catalog = await client.create_mcp_org_tool_catalog(
        2, MCPCatalogCreate(name="Org Tools")
    )
    entry = await client.add_mcp_team_catalog_entry(
        7, 3, MCPCatalogEntryCreate(tool_name="search")
    )
    ...
    mappings = await client.list_mcp_capability_mappings(
        owner_scope_type="team", owner_scope_id=7
    )
    preview = await client.preview_mcp_capability_mapping(
        MCPCapabilityMappingCreate(
            mapping_id="filesystem-read", capability_name="filesystem.read"
        )
    )
    ...
    updated = await client.update_mcp_capability_mapping(
        21, MCPCapabilityMappingUpdate(is_active=False)
    )
    ...
    assert mocked.await_args_list[3].args[:2] == (
        "GET",
        "/api/v1/teams/7/mcp/tool_catalogs",
    )
    assert mocked.await_args_list[4].args[:2] == (
        "POST",
        "/api/v1/teams/7/mcp/tool_catalogs",
    ...
    assert mocked.await_args_list[5].args[:2] == (
        "POST",
        "/api/v1/orgs/2/mcp/tool_catalogs",
    )
    assert mocked.await_args_list[6].args[:2] == (
        "POST",
        "/api/v1/teams/7/mcp/tool_catalogs/3/entries",
    )
    assert mocked.await_args_list[7].args[:2] == (
        "DELETE",
        "/api/v1/teams/7/mcp/tool_catalogs/3/entries/search",
    )
    assert mocked.await_args_list[8].args[:2] == (
        "DELETE",
        "/api/v1/teams/7/mcp/tool_catalogs/3",
    )
    assert mocked.await_args_list[9].args[:2] == (
        "GET",
    ...
    assert mocked.await_args_list[12].args[:2] == (
        "PUT",
        "/api/v1/mcp/hub/capability-mappings/21",
    )
    assert mocked.await_args_list[13].args[:2] == (
        "DELETE",
        "/api/v1/mcp/hub/capability-mappings/21",
