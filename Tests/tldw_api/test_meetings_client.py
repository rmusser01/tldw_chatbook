...
async def test_meetings_routes_wire_and_stream_events(monkeypatch):
    ...
    mocked = AsyncMock(
        side_effect=[
            ...
            {
                "session_id": "meeting-1",
                "artifacts": [_artifact(), _artifact("action_items")],
            },
            {
                "dispatch_id": 11,
                "session_id": "meeting-1",
    ...
    listed = await client.list_meeting_sessions(
        status_filter="scheduled", limit=10, offset=2
    )
    ...
    templates = await client.list_meeting_templates(
    ...
    finalized = await client.finalize_meeting_session(
        "meeting-1",
        MeetingFinalizeRequest(
            transcript_text="Transcript", include=["summary", "action_items"]
        ),
    )
    slack = await client.share_meeting_session_to_slack(
        "meeting-1",
        MeetingShareRequest(
            webhook_url="https://example.com/slack", artifact_ids=["artifact-summary"]
        ),
    )
    ...
    events = [
        event async for event in client.stream_meeting_session_events("meeting-1")
    ]
    ...
    assert mocked.await_args_list[3].args[:2] == (
        "GET",
        "/api/v1/meetings/sessions/meeting-1",
    )
    assert mocked.await_args_list[4].args[:2] == (
        "POST",
        "/api/v1/meetings/sessions/meeting-1/status",
    )
    ...
    assert mocked.await_args_list[7].args[:2] == (
        "GET",
        "/api/v1/meetings/templates/template-1",
    )
    assert mocked.await_args_list[8].args[:2] == (
        "POST",
        "/api/v1/meetings/sessions/meeting-1/artifacts",
    )
    ...
    assert mocked.await_args_list[10].args[:2] == (
        "POST",
        "/api/v1/meetings/sessions/meeting-1/commit",
    )
    assert mocked.await_args_list[11].args[:2] == (
        "POST",
        "/api/v1/meetings/sessions/meeting-1/share/slack",
    )
    assert mocked.await_args_list[12].args[:2] == (
        "POST",
        "/api/v1/meetings/sessions/meeting-1/share/webhook",
    )
