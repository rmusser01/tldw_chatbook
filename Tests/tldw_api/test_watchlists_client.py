...
async def test_watchlist_source_admin_routes_wire_and_return_typed_models(monkeypatch):
    ...
    checked = await client.check_watchlist_sources_now(
        WatchlistSourceCheckNowRequest(source_ids=[17])
    )
    draft_preview = await client.test_watchlist_source_draft(
        WatchlistSourceTestRequest(
            url="https://example.com/feed.xml", source_type="rss"
        ),
        limit=5,
    )
    stored_preview = await client.test_watchlist_source(17, limit=5)
    exported = await client.export_watchlist_sources(
        tag=["ai"], group=[2], source_type="rss"
    )
    imported = await client.import_watchlist_sources(
        b"<opml />", filename="feeds.opml", tags=["ai"], group_id=2
    )
    restored = await client.restore_watchlist_source(17)

    assert mocked.await_args_list[0].args[:2] == (
        "GET",
        "/api/v1/watchlists/sources/17/seen",
    )
    ...
    assert mocked.await_args_list[4].args[:2] == (
        "POST",
        "/api/v1/watchlists/sources/17/test",
    )
    ...
    assert mocked.await_args_list[5].kwargs["data"] == {
        "active": True,
        "tags": ["ai"],
        "group_id": 2,
    }
    assert mocked.await_args_list[6].args[:2] == (
        "POST",
        "/api/v1/watchlists/sources/17/restore",
    )
...
async def test_watchlist_bulk_telemetry_and_composer_routes_wire(monkeypatch):
    ...
    mocked = AsyncMock(
        side_effect=[
            ...
            {
                "counters": {"started": 1},
                "rates": {"setup_completion_rate": 1.0},
                "timings": {},
                "since": "2026-04-21",
                "until": "2026-04-22",
            },
            {"accepted": True},
            {
                "items": [{"variant": "experimental", "events": 3, "sessions": 1}],
                "since": "2026-04-21",
                "until": "2026-04-22",
            },
            {
                "onboarding": {"counters": {}, "rates": {}, "timings": {}},
                "uc2_backend": {},
                "ia_experiment": {},
                "baseline": {},
                "thresholds": [],
            },
            {
                ...
                "content": "Intro.",
                "warnings": [],
                "diagnostics": {"generation_mode": "manual_preview_stub"},
            },
            {
                "mode": "suggest_only",
                "issues": [],
    ...
    section = await client.compose_watchlist_template_section(
        WatchlistTemplateComposerSectionRequest(
            run_id=101, block_id="intro", prompt="Draft intro"
        )
    )
...
@pytest.mark.asyncio
async def test_watchlist_group_tag_and_job_routes_wire_and_return_typed_models(
    monkeypatch,
):
    client = TLDWAPIClient("http://localhost:8000")
    mocked = AsyncMock(
        side_effect=[
            ...
            {
                "items": [{"id": 2, "name": "Research", "description": "Feeds"}],
                "total": 1,
            },
            ...
            {
                "filters": [
                    {"type": "keyword", "action": "include", "value": {"query": "ai"}}
                ]
            },
            {
                "filters": [
            ...
            },
            {
                ...
                "job_id": 7,
                "restore_window_seconds": 10,
                "restore_expires_at": "2026-04-21T12:00:10Z",
            },
            {
                "id": 7,
                "name": "Daily",
    ...
    created_group = await client.create_watchlist_group(
        WatchlistGroupCreateRequest(
            name="ML", description="ML feeds", parent_group_id=2
        )
    )
    updated_group = await client.update_watchlist_group(
        3, WatchlistGroupUpdateRequest(name="ML Updated")
    )
    deleted_group = await client.delete_watchlist_group(3)
    created_job = await client.create_watchlist_job(
        WatchlistJobCreateRequest(
            name="Daily", scope={"sources": [17]}, schedule_expr="0 8 * * *"
        )
    )
    preview = await client.preview_watchlist_job(
        7, limit=10, per_source=3, include_content=True
    )
    ...
    updated_job = await client.update_watchlist_job(
        7, WatchlistJobUpdateRequest(name="Daily Updated", active=False)
    )
    replaced_filters = await client.replace_watchlist_job_filters(
        7,
        {
            "filters": [
                {"type": "keyword", "action": "include", "value": {"query": "ai"}}
    ...
    appended_filters = await client.append_watchlist_job_filters(
        7,
        {
            "filters": [
                {"type": "keyword", "action": "include", "value": {"query": "ai"}}
    ...
    assert mocked.await_args_list[6].args[:2] == (
        "POST",
        "/api/v1/watchlists/jobs/7/preview",
    )
    ...
    assert mocked.await_args_list[10].args[:2] == (
        "PATCH",
        "/api/v1/watchlists/jobs/7/filters",
    )
    assert mocked.await_args_list[11].args[:2] == (
        "POST",
        "/api/v1/watchlists/jobs/7/filters:add",
    )
    assert mocked.await_args_list[12].args[:2] == (
        "DELETE",
        "/api/v1/watchlists/jobs/7",
    )
    assert mocked.await_args_list[13].args[:2] == (
        "POST",
        "/api/v1/watchlists/jobs/7/restore",
    )
...
async def test_watchlist_run_routes_wire_and_return_typed_models(monkeypatch):
    ...
    detail = await client.get_watchlist_run_details(
        101, include_tallies=True, filtered_sample_max=3
    )

    assert mocked.await_args_list[0].args[:2] == (
        "POST",
        "/api/v1/watchlists/jobs/7/run",
    )
    assert mocked.await_args_list[1].args[:2] == (
        "GET",
        "/api/v1/watchlists/jobs/7/runs",
    )
    ...
    assert mocked.await_args_list[3].args[:2] == (
        "GET",
        "/api/v1/watchlists/runs/101/details",
    )
...
async def test_watchlist_run_item_output_template_and_cluster_routes_wire(monkeypatch):
    ...
    exported_runs = await client.export_watchlist_runs_csv(
        scope="global", include_tallies=True
    )
    ...
    items = await client.list_watchlist_items(
        run_id=101, reviewed=False, page=2, size=10
    )
    item = await client.get_watchlist_item(501)
    updated_item = await client.update_watchlist_item(
        501, WatchlistScrapedItemUpdateRequest(reviewed=True, status="reviewed")
    )
    output = await client.create_watchlist_output(
        WatchlistOutputCreateRequest(run_id=101, title="Briefing")
    )
    ...
    created_template = await client.create_watchlist_template(
        WatchlistTemplateCreateRequest(
            name="daily", content="{{ title }}", overwrite=True
        )
    )
    validation = await client.validate_watchlist_template(
        WatchlistTemplateValidationRequest(content="{{ title }}")
    )
    preview = await client.preview_watchlist_template(
        WatchlistTemplatePreviewRequest(content="{{ title }}", run_id=101)
    )
    ...
    assert mocked.await_args_list[0].args[:2] == (
        "POST",
        "/api/v1/watchlists/runs/101/cancel",
    )
    assert binary.await_args_list[0].args[:2] == (
        "GET",
        "/api/v1/watchlists/runs/export.csv",
    )
    assert binary.await_args_list[0].kwargs["params"] == {
        "scope": "global",
        "include_tallies": True,
    }
    assert mocked.await_args_list[1].args[:2] == (
        "GET",
        "/api/v1/watchlists/runs/101/audio",
    )
    assert binary.await_args_list[1].args[:2] == (
        "GET",
        "/api/v1/watchlists/runs/101/tallies.csv",
    )
    assert mocked.await_args_list[2].args[:2] == (
        "GET",
    ...
    assert mocked.await_args_list[8].args[:2] == (
        "GET",
        "/api/v1/watchlists/outputs/801",
    )
    assert binary.await_args_list[2].args[:2] == (
        "GET",
        "/api/v1/watchlists/outputs/801/download",
    )
    ...
    assert mocked.await_args_list[13].args[:2] == (
        "GET",
        "/api/v1/watchlists/templates/daily/versions",
    )
    assert mocked.await_args_list[14].args[:2] == (
        "GET",
        "/api/v1/watchlists/templates/daily",
    )
    ...
    assert mocked.await_args_list[18].args[:2] == (
        "DELETE",
        "/api/v1/watchlists/7/clusters/33",
    )
...
@pytest.mark.asyncio
async def test_watchlist_alert_rule_crud_routes_wire_and_return_typed_models(
    monkeypatch,
):
    client = TLDWAPIClient("http://localhost:8000")
    mocked = AsyncMock(
    ...
    assert mocked.await_args_list[2].args[:2] == (
        "PATCH",
        "/api/v1/watchlists/alert-rules/12",
    )
    assert mocked.await_args_list[2].kwargs["json_data"] == {
        "name": "Too many updated",
        "enabled": False,
