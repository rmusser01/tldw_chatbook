...
async def test_web_scraping_client_routes_management_subset(monkeypatch):
    ...
    set_cookies = await client.set_web_scraping_cookies(
    ...
    assert mocked.await_args_list[3].args[:2] == (
        "GET",
        "/api/v1/web-scraping/progress/task-1",
    )
    assert mocked.await_args_list[4].args[:2] == (
        "GET",
        "/api/v1/web-scraping/cookies/example.com",
    )
