...
class TestChatConversationClient:
    ...
    async def test_list_chat_conversations_forwards_filters_and_rankings(
        self, monkeypatch, order_by
    ):
    ...
    async def test_get_chat_conversation_forwards_scope_params(self, monkeypatch):
        ...
        await client.get_chat_conversation(
    ...
    async def test_update_chat_conversation_patches_normalized_payload(
        self, monkeypatch
    ):
        client = TLDWAPIClient("http://localhost:8000")
        mocked = AsyncMock(return_value={"id": "conv-1"})
    ...
    async def test_get_chat_conversation_tree_forwards_pagination_and_scope(
        self, monkeypatch
    ):
        client = TLDWAPIClient("http://localhost:8000")
        mocked = AsyncMock(
            return_value={"conversation": {}, "root_threads": [], "pagination": {}}
        )
        monkeypatch.setattr(client, "_request", mocked)
    ...
    async def test_get_chat_conversation_messages_with_context_forwards_scope_and_context_flag(
        self, monkeypatch
    ):
        client = TLDWAPIClient("http://localhost:8000")
    ...
    async def test_chat_adjunct_routes_wire_commands_knowledge_share_links_and_analytics(
        self, monkeypatch
    ):
        client = TLDWAPIClient("http://localhost:8000")
        mocked = AsyncMock(
            side_effect=[
                ...
                },
                {
                    ...
                    "token": "token",
                    "share_path": "/knowledge/shared/token",
                },
        ...
        links = await client.list_chat_conversation_share_links(
            "conv-1", scope_type="workspace", workspace_id="ws-1"
        )
        revoked = await client.revoke_chat_conversation_share_link(
        ...
        _assert_request_call(
            mocked.await_args_list[0], "GET", "/api/v1/chat/commands", {}
        )
        _assert_request_call(
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                ("conv-1", ConversationUpdateRequest(version=1)),
                {"scope_type": "workspace"},
            ),
    ...
    async def test_list_chat_conversations_rejects_invalid_state_before_request_dispatch(
        self, monkeypatch
    ):
        client = TLDWAPIClient("http://localhost:8000")
        mocked = AsyncMock(return_value={"ok": True})
    ...
    async def test_chat_loop_client_routes_run_events_approval_and_cancel(
        self, monkeypatch
    ):
        client = TLDWAPIClient("http://localhost:8000")
        mocked = AsyncMock(
        ...
        started = await client.start_chat_loop_run(
            ChatLoopStartRequest(messages=[{"role": "user", "content": "Hi"}])
        )
        ...
        rejected = await client.reject_chat_loop_call(
            "run_123",
            ChatLoopApprovalDecisionRequest(
                approval_id="approval-2", decision="reject"
            ),
        )
        ...
        _assert_request_call(
            mocked.await_args_list[4], "POST", "/api/v1/chat/loop/run_123/cancel", {}
        )
