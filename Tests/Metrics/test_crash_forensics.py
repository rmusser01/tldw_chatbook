...
def private_log_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        logging_config, "get_cli_log_file_path", lambda: log_file, raising=False
    )
...
def test_crash_forensics_enables_faulthandler(private_log_dir):
...
def test_dump_covers_all_threads(private_log_dir, monkeypatch):
    ...
    def spy_enable(file=None, all_threads=True):
        seen["all_threads"] = all_threads
...
def test_on_demand_dump_signal_is_registered(private_log_dir, monkeypatch):
    ...
    def spy_register(signum, file=None, all_threads=True, chain=False):
        registered["signum"] = signum
        registered["all_threads"] = all_threads
