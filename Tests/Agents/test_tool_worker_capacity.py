...
def test_definitive_local_dispatch_obeys_shared_tool_capacity(tmp_path):
    from tldw_chatbook.Agents.agent_models import ToolCall
    from tldw_chatbook.Agents.tool_catalog import ToolExecutionPolicy

    class DefinitiveTool(RunIdProbeProvider):
        def execution_policy_for(self, tool_id):
            return ToolExecutionPolicy.DEFINITIVE_AFTER_START

    capacity = RuntimeCapacity(max_tool_workers=1, reserved_manual_tool_workers=0)
    blocker = capacity.begin_execution(origin=WorkOrigin.MANUAL, conversation_id="other")
    occupied = blocker.reserve_tool()
    owner = capacity.begin_execution(origin=WorkOrigin.MANUAL, conversation_id="c")
    registry = ToolCatalogRegistry()
    provider = DefinitiveTool()
    registry.register_provider(provider)
    db = AgentRunsDB(tmp_path / "runs.db", client_id="definitive-capacity")
    try:
        service = AgentService(db, registry, runtime_capacity=capacity)
        with owner.activate():
            invoke = service._make_invoke_tool(
                AgentConfig(model="test", system_prompt="Test", allowed_tools=("whoami",)),
                {"whoami"}, lambda: False, run_id="run",
            )
            refused = invoke(ToolCall("whoami", {}))
            assert not refused.ok
            assert provider.seen == []
            occupied.finish()
            assert invoke(ToolCall("whoami", {})).ok
            assert provider.seen == ["run"]
    finally:
        occupied.finish()
        blocker.finish_root()
        owner.finish_root()
