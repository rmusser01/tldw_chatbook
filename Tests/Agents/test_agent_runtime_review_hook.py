...
CFG = AgentConfig(
    model="m", system_prompt="s", allowed_tools=("calculator", "echo", SPAWN_TOOL_NAME)
)
...
def test_non_proceed_verdict_skips_invoke_and_shapes_native_tool_result():
    ...
    result_steps = {
        s.tool_name: s.result for s in out.steps if s.kind == STEP_TOOL_RESULT
    }
...
def test_non_proceed_verdict_fence_mode_user_role_shaping():
    ...
    def call_model(messages, active_schemas):
        ...
        return ModelTurn(text=fence_text) if len(seen) == 1 else ModelTurn(text="ok")
...
def test_hook_exception_is_logged():
    ...
    turns = [
        _native_turn([ToolCall(name="calculator", args={}, call_id="idA")]),
        ModelTurn(text="done"),
    ]
    ...
    try:
        ...
        assert any(
            "review_tool_calls hook raised" in m.record["message"]
            and "calculator" in m.record["message"]
            for m in records
        ), (
