...
def test_causal_and_plain_steering_share_capacity_and_preserve_event_identity():
    fleet = FleetCoordinator(1, lambda: 0.0)
    handle = fleet.reserve("child", None)
    for index in range(31):
        assert fleet.post_steering_with_cause(
            handle.handle_id, "supervisor", str(index), f"event-{index}"
        )
    assert fleet.post_steering(handle.handle_id, "user", "last")
    assert not fleet.post_steering_with_cause(
        handle.handle_id, "supervisor", "overflow", "event-overflow"
    )
    expected = [("supervisor", str(i), f"event-{i}") for i in range(31)]
    assert fleet.drain_steering_with_causes(handle.handle_id) == [
        *expected, ("user", "last", None)
    ]
    assert fleet.post_steering_with_cause(
        handle.handle_id, "supervisor", "next", "event-next"
    )
