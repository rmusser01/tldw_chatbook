...
_PREVIOUS_UI_TEST_ENV = {name: os.environ.get(name) for name in _SANDBOXED_ENV_NAMES}
...
_BOOTSTRAP_XDG_CONFIG_HOME = _BOOTSTRAP_CONFIG_ROOT / "xdg-config"
_BOOTSTRAP_CONFIG_PATH = _BOOTSTRAP_CONFIG_ROOT / "config" / "config.toml"
for _sandbox_directory in (
    _BOOTSTRAP_HOME,
    _BOOTSTRAP_DATA_HOME,
    _BOOTSTRAP_XDG_CONFIG_HOME,
    _BOOTSTRAP_CONFIG_PATH.parent,
):
    _sandbox_directory.mkdir(parents=True, mode=0o700, exist_ok=True)
os.environ["HOME"] = str(_BOOTSTRAP_HOME)
os.environ["XDG_DATA_HOME"] = str(_BOOTSTRAP_DATA_HOME)
os.environ["XDG_CONFIG_HOME"] = str(_BOOTSTRAP_XDG_CONFIG_HOME)
