...
class TestUIBestPractices:
    ...
    async def test_widget_initialization(self, widget_pilot, mock_app_instance):
        ...
        async with await widget_pilot(
            ChatWindow, app_instance=mock_app_instance
        ) as pilot:
            app = pilot.app
    ...
    @pytest.mark.asyncio
    async def test_widget_state_changes(
        self, widget_pilot, mock_app_instance, assert_widget_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_css_class_manipulation(
        self, widget_pilot, mock_app_instance, assert_widget_state
    ):
    ...
    async def test_async_operations(
        self, widget_pilot, mock_app_instance, wait_for_condition
    ):
        ...
        def compose():
            with Container(id="test-container"):
                for i, msg in enumerate(messages):
                    yield ChatMessage(
                        message=msg["content"], role=msg["role"], id=f"chat-message-{i}"
                    )
    ...
    async def test_dynamic_content_update(self, widget_pilot, mock_app_instance):
        ...
        async with await widget_pilot(
            ...
            test_message = ChatMessage(
                message="Test dynamic message", role="user", id="dynamic-1"
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ),  # This button shows different tooltips based on state
...
@pytest.mark.asyncio
async def test_button_tooltips_parametrized(
    widget_pilot, mock_app_instance, button_id, expected_tooltip
):
