...
@pytest.mark.asyncio
async def test_save_note_posts_notes_changed(notes_fixture, monkeypatch):
    """TASK-18060 Task 5 (review-rail spec §2): a successful save posts
    ``NotesChanged(run_id)`` -- verified here that no prior screen hook
    existed for this path (neither ``_save_note`` nor ``_delete_note``
    posted anything before this task), so the rail's changed-files guard
    has a signal to invalidate its cached summary on.
    """
    db, service, provider, root, run_id = notes_fixture
    ...
    async with _Host(lambda: provider, run_id).run_test(size=(120, 40)) as pilot:
        card = await _settled_card(pilot)
        posted: list = []
        original_post = card.post_message

        def capture(message):
            posted.append(message)
            return original_post(message)

        monkeypatch.setattr(card, "post_message", capture)

        body = await _expand_first_row(pilot, card)
        note_input = await _open_note_input(pilot, body)
        note_input.value = "watch for the guard reset"
        note_input.focus()
        await pilot.press("enter")

        for _ in range(60):
            if any(
                isinstance(m, ConsoleTurnFileCard.NotesChanged) for m in posted
            ):
                break
            await pilot.pause(0.02)
        changed = [
            m for m in posted if isinstance(m, ConsoleTurnFileCard.NotesChanged)
        ]
        assert len(changed) == 1
        assert changed[0].run_id == run_id


@pytest.mark.asyncio
async def test_delete_note_posts_notes_changed(notes_fixture, monkeypatch):
    """TASK-18060 Task 5: a successful (pending-note) delete also posts
    ``NotesChanged(run_id)`` -- the refused-delete path (already-delivered
    note) must NOT post it, since nothing actually changed.
    """
    db, service, provider, root, run_id = notes_fixture
    ...
    async with _Host(lambda: provider, run_id).run_test(size=(120, 40)) as pilot:
        ...
        body = await _expand_first_row(pilot, card)
        note_input = await _open_note_input(pilot, body)
        note_input.value = "delete me too"
        note_input.focus()
        await pilot.press("enter")
