...
# Local Imports
from tldw_chatbook.Widgets.Media_Ingest.base_media_ingest_window import (
    BaseMediaIngestWindow,
)
...
class IngestTestHelper:
    ...
    async def add_test_files(self, file_count: int = 3) -> List[Path]:
        ...
        test_files = [Path(f"/tmp/test_video_{i}.mp4") for i in range(file_count)]
    ...
    async def configure_transcription_options(
        ...
        if provider_select.options and any(
            opt[0] == provider for opt in provider_select.options
        ):
        ...
        if model_select.options and any(
            opt[0] == model for opt in model_select.options
        ):
        ...
        if language_select.options and any(
            opt[0] == language for opt in language_select.options
        ):
            language_select.value = language
            await self.pilot.pause(0.1)

    async def wait_for_processing_state(
        self, expected_state: str, timeout: float = 5.0
    ) -> bool:
    ...
    def assert_form_validity(self, should_be_valid: bool = True) -> None:
        ...
        if should_be_valid:
            assert not process_button.disabled, (
                "Form should be valid (process button enabled)"
            )
        else:
            assert process_button.disabled, (
                "Form should be invalid (process button disabled)"
            )
    ...
    def assert_no_validation_error(self, field_id: str) -> None:
        ...
        if ingest_windows:
            ...
            assert error is None, (
                f"Expected no validation error for field {field_id}, got: {error}"
            )
...
class IngestTestApp:
    ...
    def create_video_app(config: Optional[Dict[str, Any]] = None):
        ...
        from tldw_chatbook.Widgets.Media_Ingest.Ingest_Local_Video_Window import (
            VideoIngestWindowRedesigned,
        )
    ...
    def create_simplified_video_app(config: Optional[Dict[str, Any]] = None):
        ...
        from tldw_chatbook.Widgets.Media_Ingest.IngestLocalVideoWindowSimplified import (
            IngestLocalVideoWindowSimplified,
        )
    ...
    def create_factory_test_app(media_type: str = "video", ui_style: str = "default"):
        ...
        class FactoryTestApp(App):
            ...
            def compose(self):
                with patch(
                    "tldw_chatbook.config.get_ingest_ui_style", return_value=ui_style
                ):
...
async def wait_for_condition(
    pilot: Pilot, condition_func, timeout: float = 1.0
) -> bool:
...
def assert_widget_visible(app: App, widget_selector: str) -> None:
    ...
    try:
        ...
        assert "hidden" not in widget.classes, (
            f"Widget {widget_selector} should be visible"
        )
    except Exception:
...
def assert_input_has_proper_styling(app: App, input_id: str) -> None:
    ...
    has_height_style = (
        hasattr(input_widget.styles, "height")
        and input_widget.styles.height is not None
    )
