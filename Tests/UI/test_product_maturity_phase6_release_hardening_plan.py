...
def test_phase6_release_hardening_plan_tracks_pr_sized_gates() -> None:
    ...
    assert "Status: TASK-13.1 through TASK-13.6 done; TASK-13.7 not started" in qa_readme
    ...
    for task_id, task_path in PHASE_6_CHILD_TASKS.items():
        ...
        elif task_id in {"TASK-13.2", "TASK-13.3", "TASK-13.4", "TASK-13.5", "TASK-13.6"}:
            assert "status: Done" in task
            for ac_number in range(1, 5):
                assert f"- [x] #{ac_number}" in task
            assert "## Implementation Notes" in task
    ...
    assert "in-progress; TASK-13.1 through TASK-13.6 done" in phase_row[2]
