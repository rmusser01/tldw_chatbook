...
@pytest.mark.asyncio
async def test_console_live_work_card_swap_anchors_below_framed_staged_context_tray():
    """Task-398: swapped cards mount as direct siblings after the framed tray.
    ...
    tray is a valid direct-child mount anchor in the inspector rail body.
    This drives the real swap seam both directions and pins the ancestry.
    """
    app = _build_test_app()
    host = ConsoleHarness(app)
    ...
    async with host.run_test(size=(180, 40)) as pilot:
        ...
        # body (no frame wrapper), sharing a parent with the swap targets.
        assert tray.parent is rail_body
        assert tray.has_class("console-frame-quiet")

        # Readiness -> pending-launch swap mounts after the tray anchor.
        screen._stage_console_library_rag_launch(_library_rag_launch("searching"))
        await pilot.pause()
        await _wait_for_selector(screen, pilot, "#console-pending-launch-card")
