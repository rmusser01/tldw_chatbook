...
def _painted_notice(host, console):
    ...
    assert region.width > 0 and region.height > 0, [
        (type(n).__name__, n.id, n.region, str(n.styles.display))
        for n in [notice, *notice.ancestors]
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("interrupted_work", "work may have run", "Review View all runs"),
        ("legacy_lineage", "earlier work", "Review View all runs"),
        ("history_unavailable", "history error", "Retry View all runs"),
    ],
)
async def test_pause_copy_is_painted_with_no_live_children_and_survives_view_clear(
    ...
    async with host.run_test(size=(180, 48)) as pilot:
        ...
        console._set_console_rail_preference(
            right_open=True, section_updates={"agent": True}, notify_on_failure=False
        )
        ...
        marker = console._fleet._console_run_marker_with_unseen(
            session, console._fleet._console_fleet_unseen_ids()
        )
...
async def test_run_history_and_manual_send_leave_old_paused_chain_unchanged(tmp_path):
    ...
    async with host.run_test(size=(180, 48)) as pilot:
        ...
        assert await _settle(pilot, lambda: bool(gateway.payloads)), (
            ...
            composer.draft_text(),
            str(console._console_send_blocked_reason()),
            str(host.focused),
            [(m.role, m.content) for m in store.messages_for_session(session.id)],
        )
...
async def test_legacy_completion_intake_repaints_the_pause_without_recovery(tmp_path):
    ...
    async with host.run_test(size=(180, 48)) as pilot:
        ...
        assert "Results saved" in painted
        assert "Review View all runs" in painted
    ...
    async with host.run_test(size=(35, 16)) as pilot:
        ...
        section.sync_state(
            ConsoleInspectorSectionState(
                rows=rows,
                summary="Paused",
                notice="Paused: usage unclear. Results saved. Review View all runs.",
            )
        )
        await pilot.pause()
...
async def test_unreadable_history_does_not_claim_unverified_results_are_saved(tmp_path):
    ...
    async with host.run_test(size=(180, 48)) as pilot:
        ...
        try:
            ...
            assert "Retry View all runs" in painted
