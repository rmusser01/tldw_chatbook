...
from Tests.UI.test_mcp_inspector import _tool
from Tests.UI.test_mcp_inspector_reachability import (
    ReachabilityApp,
    assert_visible,
    painted,
)
...
async def test_raw_response_keyboard_access_survives_resize(
    ...
    async with app.run_test(size=size) as pilot:
        ...
        await settle(pilot)
        inspector.show_test_preview(preview(tool))
        raw = json.dumps(
        ...
        assert_visible(app, title)
        assert "Raw response" in " ".join(painted(app, title).split())
        ...
        assert "END" in painted(app, scroll)
