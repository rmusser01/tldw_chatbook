...
async def test_study_window_renders_workspace_scope_banner_and_exit_controls():
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        workspace_name = study_window.query_one("#study-scope-workspace-name", Static)
        backend_status = study_window.query_one("#study-scope-backend-status", Static)
...
@pytest.mark.asyncio
async def test_study_window_back_to_workspace_button_routes_via_notes_details_seam():
    StudyScopeContext, StudyScopeType = _load_study_scope_models()
    open_notes_workspace = Mock()
    app_instance = SimpleNamespace(
        pending_study_scope_context=StudyScopeContext(
        ...
        current_runtime_backend="server",
        runtime_backend="server",
        open_notes_workspace=open_notes_workspace,
        open_study_screen=Mock(),
        notify=Mock(),
    )
    screen = StudyScreen(app_instance=app_instance)
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        open_notes_workspace.assert_called_once_with(
            "workspace-9",
            subview=WorkspaceSubview.DETAILS,
        )
