...
@pytest.mark.asyncio
async def test_an_embed_shorter_than_the_editor_pane_does_not_wrap_at_100x30(
    tmp_path: Path,
) -> None:
    """Critique A 57: ``![[attachments/diagram.png]]`` wrapped mid-token.

    Textual wraps at spaces and only splits a token that cannot fit a row by
    itself, so the AC holds exactly for lines no longer than the editor's
    content width -- which this pins at the critique's 100x30 for an embed
    line on its own.
    """
    root = tmp_path / "vault"
    ...
    async with _production_workspace_context(workspace, size=CRITIQUE_COMPACT) as pilot:
        ...
        await pilot.pause()
        content_width = editor.content_region.width
        assert content_width >= len(EMBED), (
            f"the editor pane is {content_width} cells wide at 100x30; the "
            f"{len(EMBED)}-cell embed cannot render on one row"
        )
        embed_line = editor.text.splitlines().index(EMBED)
        assert editor.wrapped_document.get_offsets(embed_line) == []
        painted = _painted_text_in_region(pilot.app, editor.content_region)
        assert any(EMBED in row for row in painted.splitlines()), painted
    await workspace.shutdown()
    replica.close()
