...
async def test_settings_long_detail_and_inspector_panes_are_scrollable_containers():
    ...
    async with host.run_test(size=(180, 50)) as pilot:
        ...
        # task-1665: BOTH panes are now fixed-header-over-scroll-body;
...
async def test_settings_provider_detail_shows_field_guidance_and_readable_draft_state():
    ...
    async with host.run_test(size=(180, 50)) as pilot:
        ...
        # task-1665: the in-place refresh folds dotted keys at separators,
...
# ---- critique round-3 batch: tasks 1623-1625 and 1660-1662 ----


def test_state_banner_leads_with_persistence_badge():
    """Five save models coexist on this screen; the badge names the active
    category's model in the same State-bar position everywhere (task-1660),
...
def test_workspaces_banner_names_reversal_paths():
    ...
    walked back instead of leaving 'no draft' unexplained (task-1660)."""
    app = _build_test_app()
    screen = SettingsScreen(app)
...
async def test_filter_clears_after_opening_a_match():
    ...
    last search's matches for the rest of the session (task-1661)."""
    app = _build_test_app()
    host = DestinationHarness(app, "settings")
    async with host.run_test(size=(190, 55)) as pilot:
        await _settle_settings_mount_storm(pilot)
...
def test_threshold_field_has_focused_guidance():
    ...
    Threshold field answered with the empty fallback (task-1662)."""
    app = _build_test_app()
    screen = SettingsScreen(app)
...
async def test_numeric_labels_carry_units():
    ...
    widened label column carries the unit (task-1662)."""
    app = _build_test_app()
    host = DestinationHarness(app, "settings")
    async with host.run_test(size=(190, 55)) as pilot:
        await _open_settings_category(pilot, "#settings-category-console-behavior")
...
async def test_every_category_renders_the_state_banner():
    """task-1660 AC: the persistence badge appears in the same State-bar
...
async def test_field_search_enter_focuses_the_field():
    ...
    task-1664: opening the category is not enough -- landing focus on the
...
async def test_state_banner_is_pinned_outside_detail_scroll():
    ...
    task-1665: the persistence badge is the save-contract carrier, so it
...
def test_t_hint_uses_each_categorys_real_verb():
    ...
    task-1663: "t test category" named an abstraction while the on-page
...
async def test_reassurance_short_line_off_overview():
    ...
    task-1663: elsewhere a single line keeps the local-first promise
