...
@pytest.mark.asyncio
async def test_media_window_entry_normalizes_restored_server_saved_context_from_runtime_state_type():
    scope_service = Mock()
    scope_service.get_read_it_later_context_capability.side_effect = [
        ReadItLaterContextCapability(
            available=False,
    ...
    scope_service.search_media = AsyncMock(
        return_value={
            "items": [{"id": "server:reading_item:201", "media_type": "article", "title": "Article 201"}],
            "total": 1,
        }
    )
    window, app = _build_media_window(runtime_backend="server", scope_service=scope_service)
    window.active_media_type = None
    ...
    window._sync_saved_view_context_on_entry()
    await asyncio.gather(*tasks)

    assert window.active_media_type is None
    assert window.runtime_state.active_browse_subview == "all"
    assert window.runtime_state.selected_record_id is None
    assert [item["id"] for item in window.runtime_state.browse_items] == ["server:reading_item:201"]
    assert window.list_panel.load_items.call_args_list[:2] == [
        call([], 1, 1),
        call(window.runtime_state.browse_items, 1, 1),
    ]
    assert [call.kwargs for call in scope_service.get_read_it_later_context_capability.call_args_list[:2]] == [
        {"mode": "server", "media_type_slug": "article"},
        {"mode": "server", "media_type_slug": "article"},
    ]
    app.notify.assert_called_once_with(
        "Read-it-later is only available in server mode from All Media.",
...
async def test_media_search_panel_shows_saved_view_disabled_reason():
    ...
    async with app.run_test() as pilot:
        ...
        assert status.content == "Only available in server mode."
