...
class _PolicyRecorder:
    ...
    def __call__(self, *, action_id: str, scope_type: str | None = None):
        ...
        return SimpleNamespace(
            allowed=self.allowed, action_id=action_id, scope_type=scope_type
        )
...
class _StubWidget:
    def __init__(
        self, *, display: bool = True, value: object = False, text: str = ""
    ) -> None:
...
def build_window(
    *,
    ...
    app.watchlist_scope_service.delete_job = AsyncMock(
        return_value={"deleted": True, "job_id": 31}
    )
    ...
    app.watchlist_scope_service.cancel_run = AsyncMock(
        return_value={"cancelled": True, "run_id": 91}
    )
    ...
    app.watchlist_scope_service.delete_alert_rule = AsyncMock(
        return_value={"deleted": True, "rule_id": 12}
    )
    ...
    app.server_notifications_scope_service.delete_reminder = AsyncMock(
    ...
    app.server_notifications_scope_service.mark_notification_read = AsyncMock(
        return_value={"updated": 1}
    )
    app.server_notifications_scope_service.dismiss_notification = AsyncMock(
        return_value={"dismissed": True}
    )
    app.server_notifications_scope_service.snooze_notification = AsyncMock(
        return_value={"task_id": "snooze-1"}
    )
    app.server_notifications_scope_service.cancel_notification_snooze = AsyncMock(
    ...
    app.server_notifications_scope_service.stream_feed_events = Mock(
        side_effect=fake_feed_stream
    )
    ...
    widgets = {
        ...
        "#watchlist-job-payload": _StubWidget(
            display=True, text='{"name":"Daily Briefing"}'
        ),
        "#watchlist-run-detail": _StubWidget(display=True, text=""),
        "#watchlist-alert-rule-payload": _StubWidget(
            display=True, text='{"name":"No items","condition_type":"no_items"}'
        ),
...
def test_subscription_window_consumes_pending_watchlist_run_detail_context(
    tmp_path: Path,
):
    window = build_window(
        tmp_path=tmp_path,
...
@pytest.mark.asyncio
async def test_save_uses_scope_service_with_normalized_payload_and_refreshes(
    tmp_path: Path,
):
    window = build_window(tmp_path=tmp_path, runtime_backend="server")
...
@pytest.mark.asyncio
async def test_delete_after_save_uses_active_edit_context_when_list_selection_is_cleared(
    tmp_path: Path,
):
    window = build_window(tmp_path=tmp_path, runtime_backend="server")
    await window.refresh_backend_view()
...
@pytest.mark.asyncio
async def test_delete_button_routes_through_backend_controller_and_refreshes(
    tmp_path: Path,
):
    window = build_window(tmp_path=tmp_path, runtime_backend="server")
    await window.refresh_backend_view()
...
async def test_notifications_tab_marks_read_through_policy_and_store(tmp_path: Path):
    ...
    assert (
        window.app_instance.require_ui_action_allowed.calls[-1]["action_id"]
...
@pytest.mark.asyncio
async def test_notifications_policy_denial_skips_store_reads_and_mutations(
    tmp_path: Path,
):
    denied_window = build_window(
        tmp_path=tmp_path, runtime_backend="local", policy_allowed=False
    )
    ...
    updated = await denied_window.notifications_controller.mark_read(
        row["id"], is_read=True
    )
...
async def test_server_mode_refresh_loads_jobs_runs_and_alert_rules(tmp_path: Path):
    ...
    assert (
        window._test_widgets["#watchlist-jobs-list"].items[0].data["id"]
        == "server:watchlist_job:31"
    )
    assert (
        window._test_widgets["#watchlist-runs-list"].items[0].data["id"]
        == "server:watchlist_run:91"
    )
    assert (
        window._test_widgets["#watchlist-alert-rules-list"].items[0].data["id"]
...
async def test_local_mode_shows_watchlist_control_plane_guidance(tmp_path: Path):
    ...
    assert window._test_widgets["#watchlist-alert-rules-local-state"].display is False
    assert (
        "local subscriptions scheduler"
        in str(window._test_widgets["#watchlist-jobs-local-state"].updated[-1]).lower()
    )
    window.app_instance.watchlist_scope_service.list_jobs.assert_not_called()
    window.app_instance.watchlist_scope_service.list_runs.assert_awaited_once_with(
        runtime_backend="local"
    )
    window.app_instance.watchlist_scope_service.list_alert_rules.assert_awaited_once_with(
        runtime_backend="local"
    )


@pytest.mark.asyncio
async def test_local_mode_pending_watchlist_run_context_loads_run_detail(
    tmp_path: Path,
):
    window = build_window(
        tmp_path=tmp_path,
...
async def test_server_mode_refresh_loads_remote_reminders_and_feed(tmp_path: Path):
    ...
    assert (
        window._test_widgets["#server-reminders-list"].items[0].data["id"]
        == "server:reminder_task:task-1"
    )
    assert (
        window._test_widgets["#server-feed-list"].items[0].data["id"]
...
async def test_local_mode_shows_remote_reminder_feed_guidance(tmp_path: Path):
    ...
    assert (
        "server reminders"
        in str(
            window._test_widgets["#server-reminders-local-state"].updated[-1]
        ).lower()
    )
...
async def test_server_feed_watch_event_renders_stream_payload(tmp_path: Path):
    ...
    assert (
        window._test_widgets["#server-feed-list"].items[0].data["notification_id"] == 12
    )
