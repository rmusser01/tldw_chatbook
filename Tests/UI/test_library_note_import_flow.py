...
async def test_obsidian_review_defaults_on_shows_skips_and_never_touches_the_vault(
    ...
    with _import_ready_host(tmp_path, monkeypatch) as host:
        async with host.run_test(size=LIBRARY_TEST_SIZE) as pilot:
            ...
            # task-32135 folds the effect into the one-line row and strips
            ...
            # Off means the plain walk: no vault-level skip survives, and the
            # config tree is walked file by file. Those files are still not
            # "Failed" rows -- task-32130 reclassified an app config file as
            # SKIPPED with its own reason whatever the Obsidian toggle says.
            assert "Obsidian configuration" not in review_off
            assert "Obsidian trash" not in review_off
            assert "Obsidian template" not in review_off
            assert "vault/.obsidian/app.json" in review_off
            assert "Not a note file (app configuration)" in review_off
