"""Design-token governance (ADR-042, TASK-2511).
...
# Every sheet that existed when ADR-042 landed (2026-08-05), including the
# three legacy files not in the build manifest (_unified_sidebar, _new_ingest,
...
_LEGACY_SHEETS: frozenset[str] = frozenset(
    ...
    features/_chatbooks_improved.tcss
...
def test_all_referenced_ds_tokens_are_defined() -> None:
    ...
    assert not offenders, (
        ...
        "core/_variables.tcss, per ADR-042):\n"
...
def test_documented_design_vocabulary_is_available() -> None:
    """ADR-042's catalog must exist even before a feature consumes a token."""
    required = {
        "$ds-space-0",
        "$ds-space-1",
        "$ds-space-2",
...
def test_hex_literals_are_ratcheted_outside_token_definitions() -> None:
    for module in _source_modules():
        ...
        assert count <= allowance, (
            ...
            "Colors belong in core/_variables.tcss tokens (ADR-042). "
            "The grandfathered allowance may only decrease."
        )


def test_new_sheets_must_use_spacing_tokens() -> None:
    """Modules added after ADR-042 may not hardcode numeric padding/margin."""
    for module in _source_modules():
        relative = str(module.relative_to(CSS_ROOT))
        ...
        assert not matches, (
            f"{relative} is a post-ADR-042 sheet and must use the "
