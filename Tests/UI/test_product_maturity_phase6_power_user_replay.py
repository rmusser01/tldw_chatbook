...
async def test_phase6_power_user_release_replay_exposes_fast_repeat_paths() -> None:
    ...
    with patch("tldw_chatbook.app.get_cli_setting", side_effect=_test_cli_setting):
        async with app.run_test(size=(180, 50)) as pilot:
            ...
            assert "Study Dashboard" in library_text
...
def test_phase6_power_user_release_replay_evidence_and_tracking_are_current() -> None:
    ...
    task = _text(TASK_13_3)
    metadata = _metadata(evidence)

    _assert_no_local_path_prefixes(evidence)
    assert metadata["task"] == "TASK-13.3"
    assert metadata["parent_task"] == "TASK-13"
    assert metadata["persona"] == "power-user"
    assert metadata["decision"] == "power_user_release_replay_recorded"
    assert set(metadata["verified_workflows"]) == REQUIRED_WORKFLOWS
    ...
    assert "Tests/UI/test_product_maturity_phase6_power_user_replay.py" in evidence

    workflow_rows = _workflow_matrix_rows(evidence)
    assert set(workflow_rows) == REQUIRED_WORKFLOWS
    for workflow, row in workflow_rows.items():
        assert len(row) >= 5, f"{workflow} row is missing required columns"
        assert row[1] in {"verified", "accepted-risk"}
        assert row[3] in {"P0", "P1", "P2", "P3", "none"}

    assert EVIDENCE.name in readme
    assert "Phase 6.3 Power-user workflow release replay" in readme
    assert "Status: TASK-13.1 through TASK-13.3 done; TASK-13.4 through TASK-13.7 not started" in readme

    phase6_row = _markdown_table_row(tracker, "Phase 6: Release Hardening And Documentation")
    assert "in-progress; TASK-13.1 through TASK-13.3 done" in phase6_row[2]
    assert "TASK-13.3" in phase6_row[3]
    assert EVIDENCE.name in phase6_row[4]
    assert "power-user workflow replay verified" in phase6_row[5].lower()

    qa_row = _markdown_table_row(tracker, "Phase 6.3")
    assert EVIDENCE.as_posix() in qa_row[1]
    assert "verified; TASK-13.3 done" == qa_row[2]

    assert "TASK-13.3" in parent_task
