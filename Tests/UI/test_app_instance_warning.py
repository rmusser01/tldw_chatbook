"""Wiring tests for the multi-instance boot warning (RAG-53 / task-7).

``TldwCli`` is enormous to construct for real (it boots the whole app), so
``_maybe_warn_second_instance`` is exercised unbound against a lightweight
stub object -- the same pattern used elsewhere in this suite for pinning a
single method's behaviour without paying for a full app instance (see e.g.
``inspect.getsource`` pins in ``Tests/UI/test_console_rail_sections.py`` and
``Tests/UI/test_chatbook_wizard_open_folder.py``).

A second test pins that ``_push_initial_screen`` actually calls
``_maybe_warn_second_instance`` in its source, so the wiring can't silently
regress (a future refactor moving/removing the call would otherwise pass
every other test in this file while quietly disarming the warning at boot).
"""
