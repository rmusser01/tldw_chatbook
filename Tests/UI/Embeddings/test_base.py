...
class EmbeddingsTestBase:
    ...
    def mock_embedding_factory(self):
        ...
        with patch(
            "tldw_chatbook.Embeddings.Embeddings_Lib.EmbeddingFactory"
        ) as mock_factory:
            instance = MagicMock()
    ...
    async def wait_for_condition(
        self, pilot: Pilot, condition_func, timeout: float = 2.0
    ):
    ...
    async def get_widget_by_id(
        self, pilot: Pilot, widget_id: str, widget_type: Type[Widget] = Widget
    ) -> Widget:
    ...
    def assert_notification(
        self, app_mock: MagicMock, message: str, severity: str = None
    ):
        ...
        assert any(message in msg for msg in messages), (
            f"Notification '{message}' not found in {messages}"
        )
