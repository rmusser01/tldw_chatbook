...
async def safe_click(
    pilot: Pilot, selector_or_widget: Union[str, Widget], force_visible: bool = True
) -> bool:
    """
    ...
    try:
        ...
        if force_visible:
            ...
            try:
                ...
                for container in app.query(
                    ".evals-scroll-container, VerticalScroll, ScrollableContainer"
                ):
                    try:
...
async def prepare_window_for_testing(
    pilot: Pilot, collapse_sections: bool = True
) -> None:
    """
...
async def assert_click_succeeded(
    pilot: Pilot, selector_or_widget: Union[str, Widget], message: str = None
) -> None:
    """
    ...
    if not result:
        if message:
            ...
            widget_str = (
                selector_or_widget
                if isinstance(selector_or_widget, str)
                else str(selector_or_widget)
            )
            assert False, (
                f"Failed to click {widget_str} - widget may be outside viewport"
            )
...
async def set_select_by_index(
    pilot: Pilot, select_widget: Select, index: int = 0
) -> bool:
    """
