...
TASK = Path(
    "backlog/tasks/task-8.4 - Product-Maturity-Phase-1.4-Keyboard-And-Focus-Sweep.md"
)
...
@pytest.mark.asyncio
async def test_command_palette_keyboard_fallback_exposes_top_level_product_model() -> (
    None
):
    ctrl_p_bindings = [
        binding for binding in TldwCli.BINDINGS if "ctrl+p" in str(binding.key).lower()
    ]
    ...
    for destination_id in TOP_LEVEL_DESTINATION_IDS:
        ...
        matching_hits = [
            ...
            if destination.accessible_label in text
            or destination.accessible_label in help_text
        ]

        assert matching_hits, (
            f"missing command-palette hit for {destination.destination_id}"
        )
