...
class TestChatModulesIntegration:
    ...
    def mock_app(self):
        ...
        app = Mock()
        app.app_config = {"chat_defaults": {"enable_tabs": False}}
    ...
    async def test_attachment_button_delegation(self, chat_window):
        ...
        chat_window.attachment_handler.handle_attach_image_button.assert_called_once_with(
            event
        )
    ...
    async def test_notes_button_delegation(self, chat_window):
        ...
        chat_window.sidebar_handler.handle_notes_expand_button.assert_called_once_with(
            event
        )
...
class TestChatMessageSystem:
    ...
    async def test_file_selected_message_handler(self, chat_window):
        ...
        chat_window.attachment_handler.process_file_attachment.assert_called_once_with(
            "/test/file.txt"
        )
    ...
    async def test_sidebar_toggled_message_handler(self, chat_window):
        ...
        chat_window.sidebar_handler.toggle_sidebar_visibility.assert_called_once_with(
            "left-sidebar"
        )
