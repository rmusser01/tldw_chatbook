...
def valid_filename_strategy():
    ...
    return st.text(alphabet=valid_chars, min_size=1, max_size=50).filter(
...
def markdown_table_strategy():
    ...
    def table_builder(draw):
        ...
        # Generate headers
        headers = [
            draw(st.text(min_size=1, max_size=10, alphabet=string.ascii_letters))
            for _ in range(cols)
        ]
...
class TestFileExtractionProperties:
    ...
    @given(
        language=code_language_strategy(), content=st.text(min_size=1, max_size=1000)
    )
    ...
    @given(content_size=st.integers(min_value=0, max_value=15 * 1024 * 1024))
    ...
    @given(text=st.text(alphabet=string.printable, min_size=0, max_size=1000))
    ...
    @given(
        terraform_resource_type=st.sampled_from(
