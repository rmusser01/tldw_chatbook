        ...
        for path, node in self.tree_view.nodes.items():
            if node.selected:
                assert path in self.tree_view.selection, (
                    f"Node {path} marked selected but not in selection"
                )
            else:
                assert path not in self.tree_view.selection, (
                    f"Node {path} not marked selected but in selection"
                )
    ...
    def selected_files_are_files(self):
        ...
        for file_path in selected_files:
            assert file_path in self.file_paths, (
                f"{file_path} returned by get_selected_files is not a file"
            )
            assert file_path not in self.directory_paths, (
                f"{file_path} is a directory but in selected files"
            )
...
class TestTreeNodeProperties:
    ...
    def test_file_node_properties(self, path, name, level, size):
        ...
        node = TreeNode(
            path=path, name=name, is_directory=False, level=level, size=size
        )
    ...
    def test_directory_node_properties(self, path, name, level):
        ...
        node = TreeNode(path=path, name=name, is_directory=True, level=level)
    ...
    def test_size_formatting(self, size):
        ...
        node = TreeNode(
            path="test", name="test", is_directory=False, level=0, size=size
        )
    ...
    @given(level=st.integers(min_value=0, max_value=20))
    def test_indentation_level(self, level):
        """Test that indentation is proportional to level."""
        node = TreeNode(path="test", name="test", is_directory=True, level=level)
...
class TestTreeViewProperties:
    ...
    @given(tree_data=st.lists(tree_node_strategy(), min_size=0, max_size=20))
...
TestTreeViewStateMachine.settings = settings(max_examples=50, stateful_step_count=20)
...
class TestSelectionProperties:
    ...
    def test_directory_cascade_selection(self, parent_path, child_count):
        ...
        parent_node = TreeNode(
            path=parent_path, name=parent_path, is_directory=True, level=0
        )
