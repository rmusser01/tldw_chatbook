        ...
        for file_path, file_data in files.items():
            ...
            if percent < 50 and summary.get("num_statements", 0) > 10:
                low_coverage_files.append(
                    (file_path, percent, summary.get("missing_lines", 0))
                )

        for file_path, percent, missing in sorted(
...
def main():
    ...
    for fmt in formats:
        if fmt not in valid_formats:
            print(
                f"Error: Invalid format '{fmt}'. Valid formats: {', '.join(valid_formats)}"
            )
            sys.exit(1)
    ...
    reporter = CoverageReporter(
        formats=formats, threshold=args.threshold, module=args.module, excludes=excludes
    )
