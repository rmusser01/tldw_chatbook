...
RUN_REAL_EMBEDDINGS = os.environ.get(
    "TLDW_RUN_REAL_EMBEDDINGS", ""
).strip().lower() in {
    "1",
    "true",
    "yes",
...
requires_sentence_transformers = pytest.mark.skipif(
    ...
    or not RUN_REAL_EMBEDDINGS,
    reason="sentence-transformers not installed or TLDW_RUN_REAL_EMBEDDINGS is not enabled",
)
requires_chromadb = pytest.mark.skipif(
    not DEPENDENCIES_AVAILABLE.get("chromadb", False), reason="chromadb not installed"
)
requires_torch = pytest.mark.skipif(
    not DEPENDENCIES_AVAILABLE.get("torch", False), reason="torch not installed"
)
...
class TestRealEmbeddingsWorkflow:
    ...
    def test_full_rag_workflow_with_real_model(self, persist_dir, sample_texts):
        ...
        config.vector_store.type = (
            "chromadb" if DEPENDENCIES_AVAILABLE.get("chromadb", False) else "memory"
        )
        ...
        search_results = rag_service.search_sync(query=query, top_k=3)
    ...
    def test_real_concurrent_operations(self, persist_dir, sample_texts):
        ...
        def process_documents(thread_id):
            try:
                ...
                thread_texts = [
                    f"{text} (Thread {thread_id})" for text in sample_texts[:2]
                ]
                ...
                if embeddings is None:
                    raise RuntimeError(
                        f"Thread {thread_id} failed to create embeddings"
                    )

                # Add to collection
                collection_name = f"thread_{thread_id}_collection"
                doc_ids = [
                    f"thread_{thread_id}_doc_{i}" for i in range(len(thread_texts))
                ]
                ...
                search_results = vector_store.search(query_embeddings[0], top_k=1)
...
class TestRealMemoryUsage:
    ...
    def test_real_model_memory_usage(self, real_embedding_service):
        ...
        for batch_size in batch_sizes:
            texts = [
                f"Test document number {i} with some content."
            ...
            print(
                f"After {batch_size} texts: {current_memory:.1f} MB (delta: {current_memory - initial_memory:.1f} MB)"
            )
...
class TestRealChromaDBIntegration:
    ...
    def test_chromadb_metadata_filtering(self, persist_dir):
        ...
        try:
            ...
            raise
        vector_store = ChromaVectorStore(
            persist_directory=persist_dir, collection_name="metadata_test"
        )
        ...
        vector_store.add(doc_ids, embeddings, docs, metadatas)

        # Search with metadata filter
        query_embedding = embeddings_service.create_embeddings(
            ["programming languages"]
        )

        # Note: Actual metadata filtering depends on ChromaDB capabilities
        results = vector_store.search(query_embedding[0], top_k=10)
...
class TestRealErrorHandling:
    ...
    def test_invalid_model_name(self):
        ...
        try:
            ...
            pytest.fail(
                "Expected an error when using invalid model, but none was raised"
            )
        except Exception as e:
    ...
    def test_real_rag_search_quality(self, real_rag_service):
        ...
        for i, doc in enumerate(documents):
            result = service.index_document_sync(
                doc_id=f"doc_{i}", content=doc, title=f"Document {i}"
            )
            results.append(result)
        ...
        for query, expected_keywords in test_queries:
            results = service.search_sync(query=query, top_k=2)
            ...
            assert found, (
                f"Expected keywords {expected_keywords} not found for query '{query}'"
            )
