        ...
        with patch(
            ...
            def thread_safe_embed(texts, as_list=True):
                ...
                with lock:
                    call_count += 1
                    return np.array(
                        [[float(call_count) + i * 0.1] * 384 for i in range(len(texts))]
                    )
...
class TestRAGServiceIntegration:
    ...
    def test_rag_service_workflow(self, temp_dir):
        ...
        with patch.object(
            embeddings_wrapper, "_ensure_embeddings_imported", return_value=True
        ):
            # Patch EmbeddingConfigSchema
            with patch(
                "tldw_chatbook.RAG_Search.simplified.embeddings_wrapper.EmbeddingConfigSchema",
                mock_config_schema,
            ):
                ...
                with patch(
                    ...
                    mock_factory_impl, mock_instance = create_mock_embedding_factory(
                        dimension=384, return_numpy=True
                    )
                    ...
                    rag_service = create_rag_service(
                        profile_name="hybrid_basic", config=config
                    )
                    ...
                    search_results = rag_service.search_sync(
                        query="programming languages like Python", top_k=2
                    )
...
class TestMemoryTracking:
    ...
    def test_memory_usage_tracking(self):
        ...
        with patch(
            ...
            with patch(
                "tldw_chatbook.RAG_Search.simplified.embeddings_wrapper.psutil.Process"
            ) as mock_process:
                # Mock memory info
                ...
                mock_memory.memory_info.return_value = MagicMock(
                    rss=600 * 1024 * 1024
                )  # 600MB
...
class TestDifferentModelTypes:
    ...
    def test_huggingface_model_integration(self):
        ...
        with patch(
            ...
            service = EmbeddingsServiceWrapper(
                model_name="bert-base-uncased", device="cpu"
            )
...
class TestErrorRecoveryIntegration:
    ...
    def test_vector_store_failure_recovery(self):
        ...
        success1 = store.add_documents(
            "test_collection", ["doc1"], [[0.1, 0.2]], [{"id": 1}], ["id1"]
        )
        assert success1
        ...
        with pytest.raises(RuntimeError):
            store.add_documents(
                "test_collection", ["doc2"], [[0.3, 0.4]], [{"id": 2}], ["id2"]
            )

        # Third add succeeds
        success3 = store.add_documents(
            "test_collection", ["doc3"], [[0.5, 0.6]], [{"id": 3}], ["id3"]
        )
        assert success3
