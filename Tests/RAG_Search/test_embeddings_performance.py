...
class TestEmbeddingPerformance:
        ...
        with patch(
            ...
            def simulate_cache(texts, as_list=True):
                ...
                for text in texts:
                    ...
                    result.append(
                        [hash(text) % 100 / 100.0 + i * 0.01 for i in range(384)]
                    )
    ...
    def test_real_model_performance(self):
        ...
        for batch_size in batch_sizes:
            ...
            results[batch_size] = {"avg_time": avg_time, "throughput": throughput}
...
class TestVectorStorePerformance:
    ...
    def test_document_insertion_performance(self):
        ...
        embeddings = [
            [i / 1000.0 + j * 0.001 for j in range(384)] for i in range(num_docs)
        ]
        ...
        success = store.add_documents(
            "perf_collection", texts, embeddings, metadatas, ids
        )
    ...
    def test_search_performance_scaling(self):
        ...
        for size in collection_sizes:
            ...
            embeddings = [
                [i / float(size) + j * 0.001 for j in range(384)] for i in range(size)
            ]
            ...
            # Measure search time
            query_embedding = [
                [0.5 + j * 0.001 for j in range(384)]
            ]  # Middle-range embedding

            times = []
            for _ in range(5):  # Multiple runs for average
                start_time = time.time()
                store.search(collection_name, query_embedding, top_k=10)
                elapsed = time.time() - start_time
                times.append(elapsed)
    ...
    def test_chromadb_vs_memory_performance(self, temp_dir):
        ...
        texts = [f"Document {i}" for i in range(num_docs)]
        embeddings = [
            [i / float(num_docs) + j * 0.001 for j in range(384)]
        ...
        start_time = time.time()
        memory_store.add_documents("test_collection", texts, embeddings, metadatas, ids)
        ...
        start_time = time.time()
        chroma_store.add_documents("test_collection", texts, embeddings, metadatas, ids)
...
class TestConcurrencyPerformance:
    ...
    def test_concurrent_embedding_throughput(self):
        ...
        with patch(
            ...
            print(
                f"  Sequential: {sequential_time:.3f}s ({sequential_throughput:.0f} texts/sec)"
            )
            print(
                f"  Concurrent: {concurrent_time:.3f}s ({concurrent_throughput:.0f} texts/sec)"
            )
    ...
    def test_concurrent_request_handling(self):
        ...
        with patch(
            ...
            def process_texts(thread_id):
                texts = [
                    f"Thread {thread_id} text {i}" for i in range(texts_per_thread)
                ]
                embeddings = service.create_embeddings(texts)
            ...
            print(f"  Total time: {elapsed:.3f}s")
            print(
                f"  All results correct: {all(shape == (texts_per_thread, 384) for shape in results)}"
            )
