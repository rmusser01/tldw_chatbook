...
class TestErrorHandler:
    ...
    def test_handle_error_with_standard_exception(self, handler):
        ...
        assert result.category in [
            ...
            ErrorCategory.DATASET_LOADING,
        ]
        assert result.severity in [ErrorSeverity.ERROR, ErrorSeverity.CRITICAL]
        assert (
            "File not found" in result.message
            or "test.txt" in result.message
            or "FileNotFoundError" in result.message
        )
    ...
    async def test_retry_with_backoff_all_failures(self, handler):
        ...
        with pytest.raises(EvaluationError) as exc_info:
            await handler.retry_with_backoff(mock_func, max_retries=2, base_delay=0.01)

        # The error handler wraps exceptions, check for the wrapped message
        assert "Unexpected error" in str(exc_info.value) or "Always fails" in str(
            exc_info.value
        )
    ...
    async def test_retry_with_non_retryable_error(self, handler):
        ...
        with pytest.raises(EvaluationError) as exc_info:
            await handler.retry_with_backoff(mock_func, max_retries=3, base_delay=0.01)
...
class TestErrorHandlingDecorator:
    ...
    async def test_decorator_with_specific_error_types(self):
        ...
        assert (
            "Invalid value provided" in exc_info.value.context.message
            or "Test error" in exc_info.value.context.message
        )
    ...
    async def test_decorator_unhandled_error(self):
        ...
        assert "Wrong type" in str(exc_info.value) or "Unexpected error" in str(
            exc_info.value
        )
