...
class TestEvalProgress:
    ...
    def test_eval_progress_creation(self):
        ...
        progress = EvalProgress(
            current=25, total=100, current_task="Processing sample 25"
        )
    ...
    def test_progress_percentage(self):
        ...
        progress = EvalProgress(current=50, total=200)
    ...
    def test_estimated_time_remaining(self):
        ...
        progress = EvalProgress(current=10, total=100)
...
class TestBasicEvaluation:
    ...
    async def test_run_single_sample(self, mock_llm_interface, sample_task_config):
        ...
        sample = EvalSample(
            id="sample_1", input_text="What is 2+2?", expected_output="4"
        )
    ...
    async def test_run_multiple_samples(self, mock_llm_interface, sample_task_config):
        ...
        with patch("tldw_chatbook.Chat.Chat_Functions.chat_api_call") as mock_llm_class:
            ...
            samples = [
                ...
                EvalSample(
                    id="sample_2", input_text="What is 3+3?", expected_output="6"
                ),
                EvalSample(
                    id="sample_3", input_text="What is 5+5?", expected_output="10"
                ),
            ]
    ...
    async def test_run_with_progress_callback(
        self, mock_llm_interface, sample_task_config
    ):
        ...
        with patch("tldw_chatbook.Chat.Chat_Functions.chat_api_call") as mock_llm_class:
            ...
            with patch(
                ...
                results = await runner.run_evaluation(
                    max_samples=5, progress_callback=progress_callback
                )
...
class TestDifferentTaskTypes:
    ...
    async def test_text_generation_task(self, mock_llm_interface):
        ...
        assert (
            ...
            == "A robot named R2 worked in a factory and dreamed of adventure."
        )
...
class TestMetricsCalculation:
    ...
    def test_contains_answer_metric(self):
        ...
        score = MetricsCalculator.calculate_contains_match(
...
class TestErrorHandling:
    ...
    async def test_api_timeout_handling(self, mock_failing_llm, sample_task_config):
        ...
        assert result.actual_output is None
        assert (
            "timeout" in str(result.error_info).lower()
            or "error" in str(result.error_info).lower()
        )
    ...
    async def test_partial_failure_handling(self, sample_task_config):
        ...
        eval_samples = [
            EvalSample(
                id="success_1", input_text="Normal question", expected_output="Success"
            ),
            EvalSample(
                id="failure_1", input_text="This should fail", expected_output="Success"
            ),
            EvalSample(
                id="success_2",
                input_text="Another normal question",
...
class TestConcurrencyAndPerformance:
    ...
    async def test_concurrent_execution(self, sample_task_config):
        ...
        runner = create_test_runner(
            mock_llm_interface=mock_llm, max_concurrent_requests=3
        )
        ...
        eval_samples = [
            EvalSample(
                id=f"sample_{i}", input_text=f"Question {i}", expected_output="Response"
            )
            for i in range(6)
        ]
...
class TestAdvancedFeatures:
    ...
    async def test_few_shot_prompting(self, mock_llm_interface):
        ...
        EvalSample(id="fewshot_sample", input_text="What is 2+2?", expected_output="4")
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Custom prompt templates not yet implemented in eval runner"
    )
    async def test_custom_prompt_templates(
        self, mock_llm_interface, sample_task_config
    ):
        """Test custom prompt template functionality."""
        sample_task_config.metadata["prompt_template"] = (
            "Question: {question}\nPlease answer: "
        )
