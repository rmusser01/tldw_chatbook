...
class TestTaskOperations:
    ...
    def test_update_task(self, in_memory_db):
        ...
        success = in_memory_db.update_task(
            task_id, description="Updated description", config_data={"version": 2}
        )
...
class TestDatasetOperations:
    ...
    def test_delete_dataset_soft_deletes_dataset(self, in_memory_db):
        ...
        assert dataset_id not in [
            dataset["id"] for dataset in in_memory_db.list_datasets()
        ]
...
class TestMetricsOperations:
    ...
    def test_get_run_metrics(self, in_memory_db):
        ...
        # Store metrics
        in_memory_db.store_run_metrics(
            run_id=run_id, metrics={"accuracy": (0.92, "accuracy")}
        )
...
class TestSearchOperations:
    ...
    def test_search_tasks_escapes_like_metacharacters(self, in_memory_db):
        """A literal '_'/'%' must match itself, not act as a LIKE wildcard.

        The short/punctuation branch is chosen *because* the query contains a
        metacharacter, so an unescaped '_' or '%' would otherwise return every
        task instead of the ones that literally contain that character.
        """
        in_memory_db.create_task(
            name="load_test",
...
class TestThreadSafety:
    ...
    def test_concurrent_read_write(self, temp_db):
        ...
        assert success_rate >= 0.8, (
            f"Expected at least 80% read success rate, got {success_rate:.2%}"
        )
