...
class TestFullEvaluationPipeline:
    ...
    def setup_test_environment(self, tmp_path):
        ...
        config_data = {
            ...
            "budget": {"default_limit": 1.0, "warning_threshold": 0.8},
        }
    ...
    async def test_complete_evaluation_flow(self, setup_test_environment):
        ...
        with patch(
            ...
            task_id = await orchestrator.create_task_from_file(
                str(task_file), format_type="custom"
            )
    ...
    async def test_error_handling_integration(self, setup_test_environment):
        ...
        with pytest.raises(
            (EvaluationError, FileSystemError, FileNotFoundError)
        ) as exc_info:
            await orchestrator.create_task_from_file(
                "/nonexistent/file.json", format_type="custom"
            )
...
class TestConfigurationIntegration:
    ...
    def test_config_loader_with_validator(self, tmp_path):
        ...
        config_data = {
            ...
            "metrics": {"custom_task": ["custom_metric"]},
            "required_fields": {"task": ["name", "task_type", "custom_field"]},
        }
        ...
        with patch(
            "tldw_chatbook.Evals.configuration_validator.get_eval_config"
        ) as mock_get:
    ...
    def test_metrics_with_runner(self):
        ...
        class TestRunner(BaseEvalRunner):
            ...
            def calculate_metrics(self, expected, actual):
                return {
                    "exact_match": self.calculator.calculate_exact_match(
                        expected, actual
                    )
                }
