...
class TestMultilingualEvaluationRunner:
    ...
    async def test_translation_task(self, runner):
        ...
        assert result.metadata["language_analysis"]["language_detected"] in [
            "french",
            "unknown",
        ]
...
class TestCreativeEvaluationRunner:
    ...
    async def test_story_completion(self, runner):
        ...
        sample = EvalSample(
            id="1", input_text="Once upon a time, in a distant kingdom..."
        )
...
class TestRobustnessEvaluationRunner:
    ...
    async def test_adversarial_qa(self, runner):
        ...
        sample = EvalSample(
            id="1", input_text="What is the capital of France?", expected_output="Paris"
        )
    ...
    def test_perturbation_generation(self, runner):
        ...
        assert (
            perturbations_punct["punctuation"] != text_with_punct
        )  # Should remove the period
...
class TestSpecializedRunnersIntegration:
    ...
    async def test_runner_registry(self):
        ...
        # Check aliases
        assert (
            SPECIALIZED_RUNNER_REGISTRY["multilingual"] == MultilingualEvaluationRunner
        )
    ...
    async def test_runner_metrics_logging(self):
        ...
        with (
            patch(
                "tldw_chatbook.Evals.specialized_runners.log_counter"
            ) as mock_counter,
            patch(
                "tldw_chatbook.Evals.specialized_runners.log_histogram"
            ) as mock_histogram,
        ):
            ...
            with patch("tldw_chatbook.Chat.Chat_Functions.chat_api_call"):
                ...
                sample = EvalSample(
                    id="1", input_text="Hello", expected_output="Bonjour"
                )
