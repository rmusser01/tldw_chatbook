...
class TestRealDatabaseIntegration:
    ...
    def test_task_creation_and_retrieval(self, temp_db):
        ...
        task_id = db.create_task(
            ...
            config_data={"dataset_name": "test.json", "metric": "accuracy"},
        )
...
class TestConcurrencyManager:
    ...
    async def test_register_and_unregister_runs(self):
        ...
        success = await manager.register_run(
            run_id="run-1", task_id="task-1", model_id="model-1"
        )
...
class TestConfigurationValidator:
    ...
    def test_validate_task_config_missing_fields(self):
        ...
        assert any(
            "required field" in error.lower() or "missing" in error.lower()
            for error in errors
        )
    ...
    def test_validate_task_config_invalid_type(self):
        ...
        assert len(errors) > 0
        assert any(
            "task_type" in error.lower() or "invalid" in error.lower()
            for error in errors
        )
    ...
    def test_validate_model_config_success(self):
        ...
        validator = ConfigurationValidator()
        config = {"provider": "openai", "model_id": "gpt-4", "api_key": "test-key"}
    ...
    def test_validate_model_config_missing_api_key(self):
        ...
        assert len(errors) > 0
        assert any(
            "api_key" in error.lower() or "key" in error.lower() for error in errors
        )
...
class TestUnifiedErrorHandler:
    ...
    def test_error_counting(self):
        ...
        assert (
            summary["categories"].get("validation", 0) >= 2
        )  # ValueErrors map to validation
    ...
    async def test_non_retryable_error(self):
        ...
        with pytest.raises(FileNotFoundError):
            await handler.retry_with_backoff(
                failing_operation, max_retries=3, base_delay=0.01
            )
    ...
    async def test_complete_evaluation_flow(self, temp_workspace):
        ...
        with patch(
            ...
            run_id = await orchestrator.run_evaluation(
                task_id=task_id, model_id=model_id, run_name="Test Run", max_samples=1
            )
