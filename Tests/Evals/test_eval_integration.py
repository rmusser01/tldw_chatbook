...
class TestEndToEndEvaluation:
    ...
    async def test_complete_evaluation_pipeline(self, temp_db_path, tmp_path):
        ...
        for i, result in enumerate(results):
            ...
            )  # Should match since we're mocking correct answers

        # Update run status
        orchestrator.db.update_run(
            run_id, {"status": "completed", "completed_samples": 3}
        )
    ...
    async def test_eleuther_task_integration(self, temp_db_path, tmp_path):
        ...
        eleuther_task = {
            ...
            "metric_list": ["acc"],
            "generation_kwargs": {"temperature": 0.0, "max_tokens": 1},
        ...
        for s in samples:
            ...
            formatted_prompt = DatasetLoader._apply_template(
                eleuther_task["doc_to_text"], s
            )
...
"""
        ...
        runner = QuestionAnswerRunner(
            task_config=task_config_obj, model_config=model_config
        )
        ...
        eval_samples = DatasetLoader.load_dataset_samples(
            task_config_obj, max_samples=4
        )
...
class TestMultiProviderIntegration:
    ...
    async def test_multiple_provider_evaluation(self, temp_db_path):
        ...
        for provider_config in providers:
            model_id = orchestrator.db.create_model(
                **provider_config, config={"api_key": "test-key"}
            )
        ...
        eval_samples = [
            EvalSample(
                id=s["id"], input_text=s["question"], expected_output=s["answer"]
            )
    ...
    async def test_provider_fallback_mechanism(self, temp_db_path):
        ...
        orchestrator.create_model_config(
            name="Primary Model", provider="openai", model_id="gpt-3.5-turbo"
        )

        # Fallback provider (succeeds)
        orchestrator.create_model_config(
            name="Fallback Model", provider="anthropic", model_id="claude-3-sonnet"
        )
    ...
    async def test_code_evaluation_integration(self, temp_db_path):
        ...
        task_id = orchestrator.db.create_task(
            ...
            config_data={
                ...
                "generation_kwargs": {"language": "python", "timeout": 10},
                "metadata": {
                    "category": "coding",
...
class TestConcurrentEvaluations:
    ...
    async def test_concurrent_task_creation(self, temp_db_path, tmp_path):
        ...
        tasks = [
            create_and_run_task(orchestrators[i], task_files[i], i) for i in range(3)
        ]
...
class TestErrorRecoveryIntegration:
    ...
    async def test_partial_failure_recovery(self, temp_db_path):
        ...
        samples_data = [
            ...
            {
                "id": "failure_2",
                "question": "FAIL_TRIGGER",
                "answer": "Should also fail",
            },
            {
                "id": "success_3",
                "question": "Final normal question",
                "answer": "Final answer",
            },
        ]
        ...
        successful_results = [
            r
            for r in results
            if r["actual_output"] is not None and "error" not in r["metrics"]
        ]
        failed_results = [
            r for r in results if r["actual_output"] is None or "error" in r["metrics"]
        ]
    ...
    async def test_database_recovery_integration(self, temp_db_path):
        ...
        eval_samples = [
            EvalSample(id="sample_1", input_text="Test", expected_output="Test")
        ]
        ...
        with patch(
            ...
            with patch.object(
                orchestrator.db, "store_result", side_effect=mock_store_result
            ):
                # Mock at the runner level
                with patch(
                    "tldw_chatbook.Evals.specialized_runners.QuestionAnswerRunner._call_llm"
        ...
        with patch(
            ...
            with patch(
                ...
                mock_llm.side_effect = [
                    f"Answer {i}" for i in range(large_sample_count)
                ]

                run_id = await orchestrator.run_evaluation(
                    task_id=task_id, model_id=model_id, max_samples=large_sample_count
                )
    ...
    async def test_memory_efficiency_integration(self, temp_db_path):
        ...
        with patch(
            ...
            with patch(
                ...
                # Generate appropriate responses
                mock_llm.side_effect = [
                    f"Memory test answer {i}" for i in range(sample_count)
                ]

                run_id = await orchestrator.run_evaluation(
                    task_id=task_id, model_id=model_id, max_samples=sample_count
                )
