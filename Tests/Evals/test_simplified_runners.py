...
class TestMultilingualEvaluationRunner:
    ...
    def test_translation_metrics_calculation(self):
        ...
        if hasattr(runner, "_calculate_translation_metrics"):
            ...
            metrics = runner._calculate_translation_metrics(
                source="Hello", translation="Bonjour", expected=None
            )
    ...
    async def test_evaluate_sample_error_handling(self):
        ...
        sample = EvalSample(id="test-1", input_text="Hello", expected_output="Bonjour")
...
class TestCodeEvaluationRunner:
    ...
    def test_code_prompt_creation(self):
        ...
        if hasattr(runner, "_create_code_prompt"):
            prompt = runner._create_code_prompt("Write a function to add two numbers")
...
```"""
    ...
    async def test_evaluate_sample_full_flow(self):
        ...
        sample = EvalSample(
            ...
            metadata={"test_cases": [{"input": "(1, 2)", "expected": "3"}]},
        )
...
class TestSafetyEvaluationRunner:
    ...
    def test_safety_analysis(self):
        ...
        if hasattr(runner, "_analyze_safety"):
            ...
            sample = EvalSample(
                id="test", input_text="test prompt", expected_output="safe"
            )

            # Test safe text
            safe_scores = runner._analyze_safety(
                "This is a helpful response about cooking.", sample
            )
            assert "contains_harmful_content" in safe_scores
            assert safe_scores["contains_harmful_content"] is False

            # Test text with refusal
            refusal_scores = runner._analyze_safety(
                "I will not help with illegal activities.", sample
            )
            assert (
                "appropriate_refusal" in refusal_scores or "refused" in refusal_scores
            )
        else:
    ...
    async def test_evaluate_sample_safe_response(self):
        ...
        assert (
            "safety_score" in result.metrics
            or "overall_safety" in result.metrics
            or len(result.metrics) > 0
        )
    ...
    async def test_evaluate_sample_with_refusal(self):
        ...
        assert (
            "refused" in result.metrics
            or "appropriate_refusal" in result.metrics
            or "safety_score" in result.metrics
        )
...
class TestEvalResult:
    ...
    def test_result_with_error(self):
        ...
        assert "error" in result.error_info or "Connection timeout" in str(
            result.error_info
        )
