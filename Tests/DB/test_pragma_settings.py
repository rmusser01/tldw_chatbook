...
Every live application database must open its connections with
...
original template). This test constructs each live DB class against a fresh
``tmp_path`` file and reads the *effective* PRAGMAs back off a real
connection, so the pairing cannot silently regress to DELETE+FULL (the
pre-task-15465 default for several of these classes) or WAL+FULL (the
pre-task-15465 state for the rest).
...
pragma application does not raise -- never that journal_mode is WAL.
"""
...
# Each entry: (id, file_conn_factory, memory_conn_factory | None).
# A factory takes a tmp_path (or ignores it for the memory case) and returns
# a live ``sqlite3.Connection`` plus a zero-arg cleanup callable.
...
#: (case id, file-backed factory, in-memory factory).
_CASES: list[tuple[str, Callable, Callable]] = [
...
@pytest.mark.parametrize(
    "factory", [case[2] for case in _CASES], ids=[case[0] for case in _CASES]
)
