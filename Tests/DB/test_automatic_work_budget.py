...
def test_automatic_work_indexes_serve_real_queries_without_statistics(db):
    from Tests.DB.test_automatic_wake_attempts import claim, survivor
    from tldw_chatbook.DB.automatic_work import AutomaticWorkRefused
    ...
    with db.connection() as conn:
        ...
        try:
            db.automatic_work.snapshot(chain_id)
            with pytest.raises(AutomaticWorkRefused, match="conversation_wake_active"):
                claim(db, chain_id, [run_id], attempt="duplicate-conversation")
            assert db.automatic_work.abort_wake("attempt-31", owner_id="owner")
        finally:
        ...
        for fragment, index in (
            ...
            (
                ...
                "idx_automatic_wake_conversation_active",
            ),
            (
