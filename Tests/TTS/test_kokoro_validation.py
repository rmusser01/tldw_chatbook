...
class TestKokoroValidation:
    ...
    @pytest.mark.asyncio
    async def test_format_conversion_streaming(
        self, backend_onnx, mock_kokoro_instance
    ):
        ...
        async def mock_convert(
            audio_array, format, source_format="pcm", sample_rate=24000
        ):
    ...
    async def test_phoneme_generation(self, backend_onnx):
        ...
        mock_instance = Mock()
        mock_instance.generate_from_phonemes = Mock(
...
async def test_integration_with_real_kokoro():
    ...
    try:
        ...
        audio_bytes, timestamps = await backend.generate_with_timestamps(
            "Test word timestamps", voice="af_bella"
        )
