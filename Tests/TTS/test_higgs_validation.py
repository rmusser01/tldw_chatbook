...
pytestmark = pytest.mark.skipif(
    not TORCH_AVAILABLE, reason="Higgs Audio backend tests require torch (higgs_tts extra)"
)
...
class TestHiggsValidation:
    ...
    def mock_serve_engine(self):
        ...
        def mock_generate(
            ...
            samples = (
                np.random.randn(24000 * 2).astype(np.float32) * 0.1
            )  # 2 seconds at 24kHz
    ...
    async def test_voice_profile_creation(self, backend):
        ...
        mock_audio_data = np.random.randn(44100 * 2).astype(
            np.float32
    ...
    async def test_multi_speaker_parsing(self, backend):
        ...
        assert simple_sections == expected_simple, (
            f"Expected {expected_simple}, got {simple_sections}"
        )
        ...
        assert sections == expected_sections, (
            f"Expected {expected_sections}, got {sections}"
        )
        ...
        assert single_sections == expected_single, (
            f"Expected {expected_single}, got {single_sections}"
        )
        ...
        assert empty_sections == expected_empty, (
            f"Expected {expected_empty}, got {empty_sections}"
        )
        ...
        assert mixed_sections == expected_mixed, (
            f"Expected {expected_mixed}, got {mixed_sections}"
        )
        ...
        assert consecutive_sections == expected_consecutive, (
            f"Expected {expected_consecutive}, got {consecutive_sections}"
        )
        ...
        assert special_sections == expected_special, (
            f"Expected {expected_special}, got {special_sections}"
        )
        ...
        expected_whitespace = [("Alice", "Hello!"), ("Bob", "Hi there!")]
        assert whitespace_sections == expected_whitespace, (
            f"Expected {expected_whitespace}, got {whitespace_sections}"
        )
    ...
    async def test_multi_speaker_generation(self, backend, mock_serve_engine):
        ...
        request = OpenAISpeechRequest(
            input=dialog, voice="storyteller_male", response_format="pcm"
        )
    ...
    async def test_error_handling(self, backend):
        ...
        request = OpenAISpeechRequest(
            input="This should fail", voice="default", response_format="mp3"
        )
        ...
        if not error_occurred:
            ...
            assert (
                "ERROR" in error_text or "Failed" in error_text or "boson" in error_text
            )
    ...
    async def test_performance_metrics(self, backend, mock_serve_engine):
        ...
        # Generate some audio
        request = OpenAISpeechRequest(
            input="Test performance metrics", voice="default", response_format="pcm"
        )
...
class TestHiggsVoiceManager:
    ...
    def test_profile_deletion(self, manager, sample_audio_file):
        ...
        # Create profile
        manager.create_profile(
            profile_name="delete_test", reference_audio_path=sample_audio_file
        )
    ...
    def test_profile_export_import(self, manager, sample_audio_file):
        ...
        with tempfile.TemporaryDirectory() as export_dir:
            ...
            success, message = manager.import_profile(
                str(package_dir), profile_name="imported_test"
            )
            assert success is True
    ...
    def test_backup_restore(self, manager, sample_audio_file):
        ...
        manager.create_profile(
            "backup_test_1", sample_audio_file, description="Original"
        )
