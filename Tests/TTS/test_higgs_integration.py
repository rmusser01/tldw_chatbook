...
pytestmark = [
    ...
    pytest.mark.skipif(
        not HIGGS_AVAILABLE, reason="Higgs Audio (boson-multimodal) not installed"
    ),
]
    ...
    async def test_model_loading(self, backend):
        ...
        try:
            ...
            if "boson_multimodal" in str(e):
                pytest.skip(
                    "Higgs Audio model not installed. Install with: pip install boson-multimodal"
                )
            raise
...
Can I join the conversation too?"""
    ...
    async def test_streaming_with_callback(self, backend):
        ...
        request = OpenAISpeechRequest(
            input=text, voice="calm_female", response_format="pcm"
        )
        ...
        logger.info(
            f"Received {len(chunks_received)} chunks, {total_bytes} total bytes"
        )
    ...
    async def test_long_text_generation(self, backend, test_audio_dir):
        ...
        request = OpenAISpeechRequest(
            input=long_text, voice="narrator_male", response_format="wav"
        )
        ...
        logger.info(
            f"Generated {len(audio_data)} bytes, memory increase: {mem_increase:.1f}MB"
        )
    ...
    async def test_concurrent_generation(self, backend, test_audio_dir):
        ...
        async def generate_task(index, text):
            request = OpenAISpeechRequest(
                input=text, voice="professional_female", response_format="wav"
            )
    ...
    async def test_error_handling(self, backend, test_audio_dir):
        ...
        request = OpenAISpeechRequest(
            input="", voice="professional_female", response_format="wav"
        )
        ...
        try:
            await backend.create_voice_profile(
                profile_name="bad_clone", reference_audio_path="/nonexistent/audio.wav"
            )
            # Should fail
...
class TestHiggsVoiceManagerIntegration:
    ...
    async def test_voice_profile_persistence(
        self, voice_manager, backend, sample_audio_file
    ):
    ...
    async def test_profile_management(
        self, voice_manager, backend, sample_audio_file, test_audio_dir
    ):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            for i, (name, freq) in enumerate([("alice", 300), ("bob", 200)]):
                t = np.linspace(0, duration, int(sample_rate * duration))
                audio_data = (0.5 * np.sin(2 * np.pi * freq * t) * 32767).astype(
                    np.int16
                )
...
The more perspectives, the better!"""
            ...
            # Generate the dialog
            request = OpenAISpeechRequest(
                input=dialog_text, voice="multi", response_format="wav"
            )
            ...
            logger.info(
                f"Full workflow completed: {len(audio_data)} bytes in {chunk_count} chunks"
            )
