...
def _configure_full_app_ingest_startup(
    app: TldwCli,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    app.app_config["_first_run"] = False
    app._initial_tab_value = "ingest"
...
async def _wait_for_mounted_media_screen(app: TldwCli, pilot) -> MediaIngestScreen:
    for _ in range(100):
    ...
    raise AssertionError("full app did not mount its configured MediaIngestScreen")
...
async def test_full_app_coordinator_store_failure_retains_every_precommit_surface(
    ...
    _configure_full_app_ingest_startup(app, monkeypatch)
...
async def test_full_app_coordinator_without_candidate_updates_mounted_media_screen(
    ...
    async def record_refresh(window: MediaIngestWindowRebuilt) -> None:
        refreshes.append(window.runtime_state.runtime_backend)

    monkeypatch.setattr(
        MediaIngestWindowRebuilt,
        "refresh_backend_view",
        record_refresh,
    )
    _configure_full_app_ingest_startup(app, monkeypatch)
    ...
    async with app.run_test() as pilot:
        ...
        assert (
            mounted_screen.media_ingest_window.runtime_state is app.media_runtime_state
        )
