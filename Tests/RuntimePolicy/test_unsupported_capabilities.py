...
from tldw_chatbook.Companion_Interop.companion_scope_service import (
...
from tldw_chatbook.Personalization_Interop.personalization_scope_service import (
...
from tldw_chatbook.Voice_Assistant_Interop.voice_assistant_scope_service import (
    VoiceAssistantScopeService,
)
from tldw_chatbook.Web_Clipper_Interop.web_clipper_scope_service import (
    WebClipperScopeService,
)
from tldw_chatbook.Web_Scraping_Interop.web_scraping_scope_service import (
...
@pytest.mark.parametrize(
    ...
    [
        (
            ChatConversationScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            CharacterPersonaScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            CompanionScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            PersonalizationScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
        ...
        (
            MediaReadingScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            PromptChatbookScopeService(
                local_prompt_service=None, server_prompt_service=None
            ),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            StudyScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            QuizScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            OutputsScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
        ...
        (
            WritingScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            ResearchScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            ResearchSearchScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            ChatGrammarsScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            FeedbackScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            ClaimsScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            MeetingsScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            PromptStudioScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            KanbanScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            TranslationScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            NotificationsScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            ServerRuntimeScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            LLMProviderCatalogScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            AuthAccountScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            AudioServicesScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            CollectionsFeedsScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            ConnectorsScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            SkillsScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            ToolsScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            MCPGovernanceScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            Text2SQLScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            SyncScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            UserGovernanceScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            VoiceAssistantScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            SharingScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            WebClipperScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            WebScrapingScopeService(server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            EvaluationScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
        (
            RAGAdminScopeService(local_service=None, server_service=None),
            [{"mode": "local"}, {"mode": "server"}],
        ),
    ],
)
def test_scope_service_unsupported_reports_match_the_shared_contract(
    service, call_kwargs
):
