...
def test_throttle_emits_on_first_call_and_phase_change():
    ...
    assert (
        t.should_emit("media", 2, 10, now=100.1) is False
    )  # within interval, same phase
...
def test_throttle_total_zero_does_not_bypass_time_throttle():
    ...
    assert (
        t.should_emit("media", 1, 0, now=0.1) is False
...
def test_format_progress_line():
    ...
    assert (
        format_export_progress_line("packaging", 210, 540)
