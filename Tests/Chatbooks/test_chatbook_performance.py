...
from Tests.Chatbooks.factories import CharacterFactory, ConversationFactory, NoteFactory
...
class PerformanceMetrics:
    ...
    def __init__(self):
        self.metrics = {"times": [], "memory": [], "sizes": []}

    def add_time(self, operation: str, duration: float):
        """Add time measurement."""
        self.metrics["times"].append({"operation": operation, "duration": duration})

    def add_memory(self, operation: str, memory_mb: float):
        """Add memory measurement."""
        self.metrics["memory"].append({"operation": operation, "memory_mb": memory_mb})
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_memory": max(
                (m["memory_mb"] for m in self.metrics["memory"]), default=0
            ),
...
class TestChatbookPerformance:
    ...
    def populate_large_dataset(
        self, db_setup: Dict[str, Any], size: str = "medium"
    ) -> Dict[str, List[int]]:
        ...
        for i in range(config["notes"]):
            ...
            note_id = chacha_db.add_note(title=note["title"], content=note["content"])
            if note_id:
    ...
    def test_import_performance_with_conflicts(self, performance_db_setup, tmp_path):
        ...
        success, _ = importer.import_chatbook(
            chatbook_path=export_path, conflict_resolution=ConflictResolution.SKIP
        )
        assert success is True
        ...
        for strategy in strategies:
            start_time = time.perf_counter()
            success, _ = importer.import_chatbook(
                chatbook_path=export_path, conflict_resolution=strategy
            )
    ...
    def test_concurrent_operations(self, performance_db_setup, tmp_path):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            start_time = time.perf_counter()
            futures = [
                executor.submit(create_chatbook, i) for i in range(num_concurrent)
            ]
        ...
        print(
            f"Concurrent exports: total={total_time:.3f}s, "
    ...
    def test_memory_usage_large_export(self, performance_db_setup, tmp_path):
        ...
        print(
            f"Memory usage: baseline={baseline_memory:.2f}MB, "
    ...
    def test_zip_compression_efficiency(self, performance_db_setup, tmp_path):
        ...
        for i in range(10):
            ...
            note_id = chacha_db.add_note(
                title=f"Random Note {i}", content=random_content
            )
            if note_id:
        ...
        print(
            f"Compression efficiency: text={text_size / 1024:.2f}KB, "
    ...
    def test_incremental_import_performance(self, performance_db_setup, tmp_path):
        ...
        for i, chatbook_file in enumerate(chatbook_files):
            start_time = time.perf_counter()
            success, _ = importer.import_chatbook(
                chatbook_path=chatbook_file, conflict_resolution=ConflictResolution.SKIP
            )
            import_time = time.perf_counter() - start_time
    ...
    def test_stress_test_extreme_scale(self, performance_db_setup, tmp_path):
        ...
        file_size = output_path.stat().st_size
        print(
            f"Stress test: {len(note_ids)} notes, "
