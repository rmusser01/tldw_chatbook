...
class MediaFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        media_type = media_type or random.choice(
...
class PromptFactory:
    ...
    @staticmethod
    def create(name: Optional[str] = None, **kwargs) -> Dict[str, Any]:
...
class ChatbookDataFactory:
    ...
    def create_simple_chatbook() -> Dict[str, Any]:
        ...
        conversation = ConversationFactory.create(
            name="Simple Conversation", character_id=1, message_count=3
        )
        note = NoteFactory.create(title="Simple Note")

        content_items = [
            ContentItemFactory.create(
                ContentType.CHARACTER, "char1", character["name"]
            ),
            ContentItemFactory.create(
                ContentType.CONVERSATION, "conv1", conversation["name"]
            ),
            ContentItemFactory.create(ContentType.NOTE, "note1", note["title"]),
        ]

        manifest = ManifestFactory.create(
            name="Simple Test Chatbook", content_items=content_items
        )
    ...
    def create_complex_chatbook() -> Dict[str, Any]:
        ...
        for i, char in enumerate(characters):
            content_items.append(
                ContentItemFactory.create(
                    ContentType.CHARACTER, f"char{i + 1}", char["name"]
                )
            )
        ...
        for i, conv in enumerate(conversations):
            content_items.append(
                ContentItemFactory.create(
                    ContentType.CONVERSATION, f"conv{i + 1}", conv["name"]
                )
            )
        ...
        for i, note in enumerate(notes):
            content_items.append(
                ContentItemFactory.create(
                    ContentType.NOTE, f"note{i + 1}", note["title"]
                )
            )
        ...
        for i, media in enumerate(media_items):
            content_items.append(
                ContentItemFactory.create(
                    ContentType.MEDIA, f"media{i + 1}", media["title"]
                )
            )
        ...
        for i, prompt in enumerate(prompts):
            content_items.append(
                ContentItemFactory.create(
                    ContentType.PROMPT, f"prompt{i + 1}", prompt["name"]
                )
            )
