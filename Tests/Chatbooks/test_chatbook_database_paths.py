...
async def test_live_chatbooks_refresh_hardens_export_directory(tmp_path: Path) -> None:
    export_dir = tmp_path / "Documents" / "Chatbooks"
    export_dir.mkdir(parents=True, mode=0o755)
    target = SimpleNamespace(
        _export_path=export_dir,
        chatbooks=None,
    ...
    previous = os.umask(0)
    try:
        await ChatbooksWindowImproved._refresh_chatbooks(target)
    finally:
        os.umask(previous)

    assert target.chatbooks == []
