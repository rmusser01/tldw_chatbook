...
class TestChatbookIntegration:
    ...
    def setup_test_databases(self, tmp_path):
        ...
        if conv2_id:
            chacha_db.add_message(
                {"conversation_id": conv2_id, "sender": "user", "content": "Hello"}
            )
            chacha_db.add_message(
                {
        ...
        note1_id = chacha_db.add_note(
            title="ML Notes", content="Notes about machine learning concepts"
        )

        note2_id = chacha_db.add_note(
            title="Python Tips", content="Useful Python programming tips"
        )
    ...
    def test_full_export_import_cycle(self, setup_test_databases, tmp_path):
        ...
        content_selections = {
            ...
            ContentType.CHARACTER: [str(test_data["char_id"])]
            if test_data["char_id"]
            else [],
        }
        ...
        import_chacha_db = CharactersRAGDB(
            import_db_paths["ChaChaNotes"], "test_client"
        )
        ...
        conv1_results = import_chacha_db.search_conversations_by_title(
            "Export Test Conv 1"
        )
        assert len(conv1_results) > 0
        assert conv1_results[0]["title"] == "Export Test Conv 1"

        conv2_results = import_chacha_db.search_conversations_by_title(
            "Export Test Conv 2"
        )
    ...
    def test_selective_export_import(self, setup_test_databases, tmp_path):
        ...
        content_selections = {
            ContentType.CONVERSATION: [str(test_data["conv_ids"][0])]
            if test_data["conv_ids"]
            else [],
            ContentType.NOTE: [str(test_data["note_ids"][0])]
    ...
    def test_conflict_resolution_scenarios(self, tmp_path):
        ...
        existing_note_id = chacha_db.add_note(
            title="Existing Note", content="Old content"
        )
...
New content from import""".format(
    ...
    def test_large_chatbook_performance(self, tmp_path):
        ...
        for i in range(50):
            note_id = chacha_db.add_note(
                title=f"Note {i + 1}", content=f"Content for note {i + 1}" * 10
            )
            if note_id:
        ...
        content_selections = {
            ...
            ContentType.CHARACTER: [
                str(perf_char_id)
            ],  # Explicitly include the character
        }
        ...
        start_time = time.time()
        success, message = importer.import_chatbook(
            chatbook_path=export_path, import_status=status
        )
        ...
        assert (
            status.successful_items == 151
        )  # 100 conversations + 50 notes + 1 character (auto-included)
        ...
        with zipfile.ZipFile(export_path, "r") as zf:
            ...
            char_rel = next(
                (
                    ...
                    if r["relationship_type"] == "uses_character"
                ),
                None,
            )
