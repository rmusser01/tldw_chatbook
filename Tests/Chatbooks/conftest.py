...
def memory_db_paths():
    ...
    return {"ChaChaNotes": ":memory:", "Prompts": ":memory:", "Media": ":memory:"}
...
def populated_chachanotes_db(mock_db_paths):
    ...
    conv1_id = db.add_conversation(
        {"conversation_name": "Test Conversation 1", "character_id": char1_id}
    )

    conv2_id = db.add_conversation(
        {"conversation_name": "Test Conversation 2", "character_id": char2_id}
    )
    ...
    note2_id = db.add_note(
        title="Test Note 2", content="Another test note with **markdown** formatting."
    )
...
def populated_prompts_db(mock_db_paths):
    ...
    return {"db": db, "prompt_ids": [prompt1_id, prompt2_id]}
...
def populated_media_db(mock_db_paths):
    ...
    return {"db": db, "media_ids": [media1_id, media2_id]}
...
"""
        ...
        zf.writestr(
            "content/conversations/conversation_conv1.json",
...
This is a test note for the chatbook."""
        ...
        zf.writestr(
            "content/characters/character_char1.json", json.dumps(char_data, indent=2)
        )
