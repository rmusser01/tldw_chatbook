...
def test_build_notes_upsert_update_uses_mirror_base():
    mirror = NotesMirror(":memory:")
    mirror.record(
        "ds_1", "n1", object_revision=1, object_hash="sha256:prev", server_cursor=5
    )
    env = _builder(mirror).build_notes_note_upsert(
        note_id="n1", title="T2", content="B2"
    )
...
def test_build_notes_tombstone_requires_mirror_base():
    mirror = NotesMirror(":memory:")
    mirror.record(
        "ds_1", "n1", object_revision=2, object_hash="sha256:p", server_cursor=9
    )
...
def test_apply_notes_upsert_creates_local_note_and_updates_mirror():
    ...
    applier = SyncEnvelopeApplier(
        local_store=store, notes_mirror=mirror, dataset_id="ds_1"
    )
...
def test_apply_drops_stale_older_revision_envelope():
    ...
    mirror = NotesMirror(":memory:")
    mirror.record(
        "ds_1", "n1", object_revision=5, object_hash="sha256:newer", server_cursor=50
    )
    applier = SyncEnvelopeApplier(
        local_store=store, notes_mirror=mirror, dataset_id="ds_1"
    )
