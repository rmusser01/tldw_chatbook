...
def test_article_url_routes_to_extractor_and_sets_real_url():
    ...
    with patch(
        "tldw_chatbook.Local_Ingestion.web_article_ingestion.extract_article_for_ingest",
        return_value=fake,
    ) as ex:
...
def test_video_url_routes_to_audio_video_branch():
    ...
    with patch(
        ...
        VP.return_value.process_videos.return_value = {
            "results": [{**fake_result, "status": "Success", "metadata": {}}]
        }
...
def test_article_permanent_error_propagates_unwrapped():
    ...
    assert (
        classify_parse_failure(ei.value) is True
    )  # stays permanent through the wrapper
