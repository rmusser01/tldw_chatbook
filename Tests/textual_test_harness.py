...
class IsolatedWidgetTestApp(App[None]):
    ...
    def _create_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "api_endpoints": {"openai": {"api_key": "test-key"}},
            "chat_defaults": {"temperature": 0.7, "max_tokens": 1000},
        }
...
async def wait_for_widget_by_id(
    pilot: Pilot, widget_id: str, widget_type: Type[W] = Widget, timeout: float = 5.0
) -> W:
    """
    ...
    while True:
        try:
            ...
            if asyncio.get_event_loop().time() - start_time > timeout:
                raise TimeoutError(
                    f"Widget with ID '{widget_id}' not found within {timeout}s"
                )
...
async def verify_notification(
    pilot: Pilot, expected_text: str, severity: Optional[str] = None
) -> bool:
    """
