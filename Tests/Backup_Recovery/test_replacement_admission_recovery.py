...
def test_registered_file_namespace_recovers_native_rename_gap(
    ...
    with replacement_case(tmp_path, monkeypatch, prepared=False, tree=tree) as case:
        ...
        if sys.platform == "win32":
            ...
            alias.unlink()
            # Exercise the duplicate-name recovery boundary without admitting
            # a reparse point that this platform deliberately refuses.
            authority.register("aaa.selected.config.alias", (selector,))
        else:
            authority.register("aaa.selected.config.alias", (alias,))
