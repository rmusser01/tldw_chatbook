...
def test_application_sink_sentinel_matrix(scenario: str, tmp_path: Path) -> None:
    ...
    try:
        if scenario in {"success", "parsing_error", "timeout", "cache"}:
            _emit_owned_loguru_payload(
                ...
                f"tool arguments and result={PRIVATE_SENTINEL}",
            )
            status = {
            ...
            log_persistent_metadata(
                logging.getLogger("tldw_chatbook.Agents.builtin_tool_gate"),
                logging.INFO,
                "tool_execution",
