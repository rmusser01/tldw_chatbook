...
class TestFasterWhisperUnit:
    ...
    def mock_whisper_model(self):
        ...
        with patch(
            ...
            info = MockTranscriptionInfo(
                language="en", language_probability=0.99, duration=7.5
            )
    ...
    def test_model_loading_and_caching(
        ...
        result = transcription_service._transcribe_with_faster_whisper(
            audio_path=sample_audio_file, model="base", language="en", vad_filter=True
        )
    ...
    def test_different_model_configurations(
        self, transcription_service, mock_whisper_model
    ):
    ...
    def test_language_detection(
        ...
        result = transcription_service._transcribe_with_faster_whisper(
            audio_path=sample_audio_file, model="base", language="auto", vad_filter=True
        )
    ...
    def test_vad_filter_options(
        ...
        transcription_service._transcribe_with_faster_whisper(
            audio_path=sample_audio_file, model="base", language="en", vad_filter=False
        )
    ...
    def test_progress_callback(
        ...
        assert (
            mock_callback.call_count >= 4
        )  # Start, language detection, segments, complete
        ...
        lang_calls = [
            ...
            if "Language detected" in call[0][1]
        ]
    ...
    def test_beam_size_configuration(
        self, transcription_service, mock_whisper_model, sample_audio_file
    ):
    ...
    def test_empty_segments_handling(
        ...
        empty_info = MockTranscriptionInfo(
            language="en", language_probability=0.99, duration=0.0
        )
...
class TestFasterWhisperIntegration:
    ...
    @pytest.mark.integration
    @pytest.mark.slow
    def test_real_transcription_auto_language(
        self, real_transcription_service, test_audio_file
    ):
        """Test automatic language detection."""
        result = real_transcription_service.transcribe(
    ...
    def test_real_transcription_different_models(
        self, real_transcription_service, test_audio_file, model
    ):
    ...
    def test_real_transcription_compute_types(
        self, real_transcription_service, test_audio_file, compute_type
    ):
        ...
        try:
            ...
            if "compute type" in str(e).lower():
                pytest.skip(
                    f"Compute type {compute_type} not supported on this hardware"
                )
            else:
                raise
    ...
    def test_real_transcription_invalid_file(self, real_transcription_service):
        ...
        with pytest.raises(TranscriptionError):
            real_transcription_service.transcribe(
                audio_path="non_existent_file.wav", provider="faster-whisper"
            )
    ...
    def test_real_transcription_empty_file(self, real_transcription_service):
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_file:
            ...
            try:
                result = real_transcription_service.transcribe(
                    audio_path=tmp_file.name, provider="faster-whisper", model="tiny"
                )
    ...
    @pytest.mark.integration
    @pytest.mark.slow
    def test_real_transcription_with_vad(
        self, real_transcription_service, test_audio_file
    ):
    ...
    def test_real_transcription_progress_tracking(
        ...
        lang_updates = [
            ...
            if u.get("metadata") and "language" in u.get("metadata", {})
        ]
    ...
    def test_model_loading_time(self, real_transcription_service):
        ...
        for model in models:
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
                ...
                try:
                    real_transcription_service.transcribe(
                        audio_path=tmp.name, provider="faster-whisper", model=model
                    )
