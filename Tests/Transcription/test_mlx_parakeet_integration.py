...
pytestmark = pytest.mark.skipif(
    sys.platform != "darwin", reason="MLX Parakeet tests only run on macOS"
)
...
class TestMLXParakeetIntegration:
    ...
    def mock_model_download(self):
        ...
        def create_mock_model(*args, **kwargs):
            ...
            mock_model._model_name = (
                args[0] if args else "mlx-community/parakeet-tdt-0.6b-v2"
            )
            return mock_model

        with patch(
            "tldw_chatbook.Local_Ingestion.transcription_service.parakeet_from_pretrained",
            side_effect=create_mock_model,
        ) as mock:
            yield mock
    ...
    def transcription_service(self):
        ...
        with patch(
            "tldw_chatbook.Local_Ingestion.transcription_service.PARAKEET_MLX_AVAILABLE",
            True,
        ):
            yield TranscriptionService()

    def test_basic_transcription(
        self, transcription_service, create_wav_file, mock_model_download
    ):
        ...
        result = transcription_service.transcribe(
            audio_path=audio_file, provider="parakeet-mlx"
        )
    ...
    def test_error_handling_corrupted_audio(
        self, transcription_service, mock_model_download
    ):
        ...
        try:
            ...
            try:
                result = transcription_service.transcribe(
                    audio_path=corrupted_file, provider="parakeet-mlx"
                )
    ...
    def test_model_caching(
        ...
        result1 = transcription_service.transcribe(
            audio_path=audio_file1, provider="parakeet-mlx"
        )

        # Second transcription should use cached model
        result2 = transcription_service.transcribe(
            audio_path=audio_file2, provider="parakeet-mlx"
        )
    ...
    def test_concurrent_transcriptions(
        ...
        def transcribe_file(audio_path):
            return transcription_service.transcribe(
                audio_path=audio_path, provider="parakeet-mlx"
            )

        # Run transcriptions concurrently
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(transcribe_file, audio_file)
    ...
    def test_precision_options(
        self, transcription_service, create_wav_file, mock_model_download, precision
    ):
        """Test different precision options."""
        audio_file = create_wav_file(duration=0.5)

        result = transcription_service.transcribe(
            audio_path=audio_file, provider="parakeet-mlx", precision=precision
        )
...
class TestMLXParakeetRealIntegration:
    ...
    def test_real_transcription(self, real_audio_file):
        ...
        result = service.transcribe(audio_path=real_audio_file, provider="parakeet-mlx")
