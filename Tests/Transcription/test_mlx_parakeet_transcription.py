...
class TestMLXParakeetUnit:
    ...
    def transcription_service(self, mock_parakeet_available):
        ...
        with (
            ...
            patch(
                "tldw_chatbook.Local_Ingestion.transcription_service.time.time",
                return_value=1000.0,
            ),
        ):
    ...
    def test_parakeet_initialization(self, transcription_service):
        ...
        assert (
            transcription_service._parakeet_mlx_config["model"]
    ...
    def test_model_loading(
        self, transcription_service, mock_parakeet_model, sample_audio_file
    ):
        ...
        # Mock audio loading
        with (
            patch("tldw_chatbook.Local_Ingestion.transcription_service.sf") as mock_sf,
            patch(
                "tldw_chatbook.Local_Ingestion.transcription_service.time.time",
                return_value=1000.0,
            ),
            ...
            result = transcription_service._transcribe_with_parakeet_mlx(
                audio_path=sample_audio_file, model="mlx-community/parakeet-tdt-0.6b-v2"
            )
    ...
    def test_model_reload_on_config_change(
        ...
        with (
            ...
            transcription_service._transcribe_with_parakeet_mlx(
                audio_path=sample_audio_file, model="mlx-community/parakeet-tdt-1.1b"
            )
    ...
    def test_attention_type_configuration(
        self, transcription_service, mock_parakeet_model, sample_audio_file
    ):
    ...
    def test_chunking_for_long_audio(self, transcription_service, mock_parakeet_model):
        ...
        with (
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav") as tmp:
                result = transcription_service._transcribe_with_parakeet_mlx(
                    audio_path=tmp.name, model="mlx-community/parakeet-tdt-0.6b-v2"
                )
    ...
    def test_audio_loading_error(self, transcription_service, mock_parakeet_model):
        ...
        with patch("tldw_chatbook.Local_Ingestion.transcription_service.sf") as mock_sf:
            ...
            with pytest.raises(TranscriptionError) as exc_info:
                transcription_service._transcribe_with_parakeet_mlx(
                    audio_path="invalid.wav", model="mlx-community/parakeet-tdt-0.6b-v2"
                )
    ...
    def test_sample_rate_conversion(self, transcription_service, mock_parakeet_model):
        ...
        with (
            ...
            mock_sf.read.return_value = (audio_data, sample_rate)
            mock_sf.info.return_value = MockAudioInfo(
                duration=duration, samplerate=sample_rate
            )
...
class TestMLXParakeetIntegration:
    ...
    @pytest.mark.integration
    @pytest.mark.slow
    def test_real_transcription_with_different_precisions(
        self, real_transcription_service, test_audio_file
    ):
...
class TestMLXParakeetPerformance:
    ...
    def test_transcription_speed(self, real_transcription_service, test_audio_file):
        ...
        for config in configurations:
            ...
            result = real_transcription_service.transcribe(
                audio_path=test_audio_file, provider="parakeet-mlx", **config
            )
        ...
        assert all(r["elapsed_time"] < 30 for r in results), (
