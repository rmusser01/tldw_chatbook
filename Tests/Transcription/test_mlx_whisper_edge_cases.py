...
class TestMLXWhisperEdgeCases:
    ...
    def mock_service(self):
        ...
        with (
            ...
            patch(
                ...
                side_effect=getsize_side_effect,
            ),
        ):
    ...
    def large_audio_file(self):
        ...
        audio_data = np.random.randint(
            -32768, 32767, size=(samples, channels), dtype=np.int16
        )
            ...
            def transcribe_with_model(model_name):
                try:
                    ...
                    result = mock_service._transcribe_with_lightning_whisper_mlx(
                        audio_path="dummy.wav", model=model_name
                    )
    ...
    def test_special_characters_in_transcription(self, mock_service):
        ...
        with patch(
            ...
            )  # Newline preserved within text
            assert (
                result["segments"][1]["text"] == "[Special] <Characters>"
            )  # Tab is stripped by strip()
    ...
    def test_progress_callback_exception(self, mock_service):
        ...
        with patch(
            ...
            result = mock_service._transcribe_with_lightning_whisper_mlx(
                audio_path="dummy.wav", progress_callback=bad_callback
            )
    ...
    def test_quantization_values(self, mock_service, quant_value):
        ...
        with patch(
            ...
            if quant_value == "invalid":
                ...
                mock_mlx.side_effect = ValueError(
                    f"Invalid quantization: {quant_value}"
                )

                with pytest.raises(TranscriptionError):
                    mock_service._transcribe_with_lightning_whisper_mlx(
                        audio_path="dummy.wav", quant=quant_value
                    )
            else:
                mock_instance = MagicMock()
                mock_instance.transcribe.return_value = {
                ...
                result = mock_service._transcribe_with_lightning_whisper_mlx(
                    audio_path="dummy.wav", model="base", quant=quant_value
                )
                ...
                mock_mlx.assert_called_with(
                    model="base", batch_size=12, quant=quant_value
                )
...
class TestMLXWhisperRobustness:
    ...
    def test_network_timeout_during_model_download(self, mock_service):
        ...
        with patch(
            ...
            with pytest.raises(TranscriptionError) as exc_info:
                mock_service._transcribe_with_lightning_whisper_mlx(
                    audio_path="dummy.wav", model="large-v3"
                )
