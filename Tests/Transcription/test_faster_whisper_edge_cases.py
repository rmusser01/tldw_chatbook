...
class MockInfo:
    ...
    def __init__(
        self, language="en", language_probability=0.99, duration=10.0, **kwargs
    ):
...
class TestFasterWhisperEdgeCases:
    ...
    def mock_service(self):
        ...
        with (
            patch(
                "tldw_chatbook.Local_Ingestion.transcription_service.FASTER_WHISPER_AVAILABLE",
                True,
            ),
            patch(
            ...
            patch(
                "tldw_chatbook.Local_Ingestion.transcription_service.os.path.exists",
                side_effect=mocked_exists,
            ),
            patch(
                "tldw_chatbook.Local_Ingestion.transcription_service.os.path.getsize",
                side_effect=mocked_getsize,
            ),
        ):
    ...
    def test_concurrent_model_loading(self, mock_service):
        ...
        with patch(
            "tldw_chatbook.Local_Ingestion.transcription_service.WhisperModel"
        ) as mock_model_class:
        ...
        with patch(
            ...
            for model, device, compute_type in configs:
                ...
                mock_service._transcribe_with_faster_whisper(
                    audio_path="dummy.wav", model=model, language="en", vad_filter=True
                )
    ...
    def test_segment_generator_exceptions(self, mock_service):
        ...
        with patch(
            ...
            with pytest.raises(TranscriptionError) as exc_info:
                mock_service._transcribe_with_faster_whisper(
                    audio_path="dummy.wav", model="base", language="en", vad_filter=True
                )
    ...
    def test_progress_callback_exceptions(self, mock_service):
        ...
        with patch(
            ...
            mock_model = MagicMock()
            mock_model.transcribe.return_value = (
                iter([MockSegment(0, 1, "First"), MockSegment(1, 2, "Second")]),
    ...
    def test_vad_filter_edge_cases(self, mock_service):
        ...
        with patch(
            ...
            result = mock_service._transcribe_with_faster_whisper(
                audio_path="silence.wav", model="base", language="en", vad_filter=True
            )
    ...
    def test_compute_type_compatibility(self, mock_service):
        ...
        with patch(
            ...
            mock_model_class.side_effect = RuntimeError(
    ...
    def test_concurrent_transcriptions_same_model(self, mock_service):
        ...
        with patch(
            ...
            def concurrent_transcribe(*args, **kwargs):
                ...
                return iter(
                    [MockSegment(0, 1, f"Thread {threading.current_thread().name}")]
                ), MockInfo()
            ...
            for i in range(5):
                ...
                thread = threading.Thread(
                    target=transcribe, args=(i,), name=f"Thread-{i}"
                )
...
class TestFasterWhisperMemoryManagement:
    ...
    def test_generator_cleanup_on_exception(self, mock_service):
        ...
        with patch(
            ...
            mock_model.transcribe.return_value = (segment_generator(), MockInfo())
            mock_model_class.return_value = mock_model
