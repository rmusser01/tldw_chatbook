...
class TestMLXParakeetEdgeCases:
        ...
        for channels in channel_configs:
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
                ...
                for ch in range(channels):
                    ...
                    freq = 440 * (ch + 1)
                    audio_data[:, ch] = (np.sin(2 * np.pi * freq * t) * 16000).astype(
                        np.int16
                    )
    ...
    def test_memory_pressure_handling(self, service, mock_model_minimal):
        ...
        try:
            ...
            with pytest.raises(TranscriptionError) as exc_info:
                service.transcribe(audio_path=audio_file, provider="parakeet-mlx")
    ...
    def test_model_switching(self, service, mock_model_minimal):
        ...
        try:
            ...
            for model in models:
                result = service.transcribe(
                    audio_path=audio_file, provider="parakeet-mlx", model=model
                )
                results.append(result)
    ...
    def test_garbage_collection_during_transcription(self, service, mock_model_minimal):
        ...
        try:
            ...
            result = service.transcribe(audio_path=audio_file, provider="parakeet-mlx")
