...
pytestmark = pytest.mark.skipif(
    sys.platform != "darwin", reason="MLX Whisper tests only run on macOS"
)
...
class TestMLXWhisperUnit:
    ...
    def mock_lightning_whisper_available(self):
        ...
        with patch(
            "tldw_chatbook.Local_Ingestion.transcription_service.LIGHTNING_WHISPER_AVAILABLE",
            True,
        ):
            yield
    ...
    def mock_lightning_whisper_model(self):
        ...
        with patch(
            "tldw_chatbook.Local_Ingestion.transcription_service.LightningWhisperMLX"
        ) as mock:
    ...
    def test_model_loading(
        self, transcription_service, mock_lightning_whisper_model, sample_audio_file
    ):
        ...
        result = transcription_service._transcribe_with_lightning_whisper_mlx(
            audio_path=sample_audio_file, model="base"
        )

        # Check model was instantiated with correct parameters
        mock_class.assert_called_once_with(model="base", batch_size=12, quant=None)
    ...
    def test_model_reload_on_config_change(
        self, transcription_service, mock_lightning_whisper_model, sample_audio_file
    ):
        ...
        transcription_service._transcribe_with_lightning_whisper_mlx(
            audio_path=sample_audio_file, model="large-v3"
        )

        # Model should be reloaded
        mock_class.assert_called_once_with(model="large-v3", batch_size=12, quant=None)
    ...
    def test_quantization_options(
        ...
        transcription_service._transcribe_with_lightning_whisper_mlx(
            audio_path=sample_audio_file, model="base", quant="4bit"
        )

        mock_class.assert_called_with(model="base", batch_size=12, quant="4bit")

        # Reset and test with 8-bit quantization
        mock_class.reset_mock()
        transcription_service._transcribe_with_lightning_whisper_mlx(
            audio_path=sample_audio_file, model="base", quant="8bit"
        )

        mock_class.assert_called_with(model="base", batch_size=12, quant="8bit")
    ...
    def test_batch_size_configuration(
        ...
        transcription_service._transcribe_with_lightning_whisper_mlx(
            audio_path=sample_audio_file, model="base", batch_size=24
        )

        mock_class.assert_called_with(model="base", batch_size=24, quant=None)
    ...
    def test_progress_callback(
        ...
        transcription_service._transcribe_with_lightning_whisper_mlx(
            audio_path=sample_audio_file, model="base", progress_callback=mock_callback
        )
    ...
    def test_error_handling_transcription(
        self, transcription_service, mock_lightning_whisper_model, sample_audio_file
    ):
    ...
    def test_model_name_mapping(
        ...
        transcription_service._transcribe_with_lightning_whisper_mlx(
            audio_path=sample_audio_file, model="large"
        )

        mock_class.assert_called_with(model="large-v3", batch_size=12, quant=None)
    ...
    def test_result_metadata(
        ...
        result = transcription_service._transcribe_with_lightning_whisper_mlx(
            audio_path=sample_audio_file, model="base", batch_size=24, quant="4bit"
        )
...
class TestMLXWhisperIntegration:
    ...
    @pytest.mark.slow
    def test_real_transcription_basic(
    ...
    @pytest.mark.integration
    @pytest.mark.slow
    def test_real_transcription_with_quantization(
        self, real_transcription_service, test_audio_file
    ):
    ...
    @pytest.mark.integration
    @pytest.mark.slow
    def test_real_transcription_different_models(
        self, real_transcription_service, test_audio_file
    ):
    ...
    def test_real_transcription_invalid_file(self, real_transcription_service):
        ...
        with pytest.raises(TranscriptionError):
            real_transcription_service.transcribe(
                audio_path="non_existent_file.wav", provider="lightning-whisper-mlx"
            )
    ...
    def test_real_transcription_batch_sizes(
        self, real_transcription_service, test_audio_file, batch_size
    ):
    ...
    def test_real_transcription_progress_tracking(
        ...
        def progress_callback(percentage, message, metadata):
            progress_updates.append(
                {"percentage": percentage, "message": message, "metadata": metadata}
            )
        ...
        assert any(
            "complete" in update["message"].lower() for update in progress_updates
        )
...
class TestMLXWhisperPerformance:
    ...
    def test_transcription_speed(self, real_transcription_service, test_audio_file):
        ...
        for config in configurations:
            ...
            result = real_transcription_service.transcribe(
                audio_path=test_audio_file, provider="lightning-whisper-mlx", **config
            )
        ...
        assert all(r["elapsed_time"] < 60 for r in results), (
