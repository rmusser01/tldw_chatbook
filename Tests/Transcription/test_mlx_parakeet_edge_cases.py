    ...
    def test_concurrent_model_loading(self, mock_service):
        ...
        with (
            ...
            with patch(
                ...
                threads = [
                    threading.Thread(target=load_and_transcribe) for _ in range(3)
                ]
                for t in threads:
                    t.start()
                for t in threads:
            ...
            for sample_rate, description in test_cases:
                with patch(
                    ...
                    mock_sf.info.return_value = MockAudioInfo(
                        duration=1.0, samplerate=sample_rate
                    )
    ...
    def test_extreme_audio_lengths(self, mock_service):
        ...
        with patch(
            ...
            with patch(
                ...
                result = mock_service._transcribe_with_parakeet_mlx(
                    audio_path="short.wav", model="mlx-community/parakeet-tdt-0.6b-v2"
                )
            ...
            with patch(
                ...
                mock_model.transcribe.side_effect = [
                    f"Chunk {i}" for i in range(chunk_count)
                ]

                result = mock_service._transcribe_with_parakeet_mlx(
                    audio_path="long.wav", model="mlx-community/parakeet-tdt-0.6b-v2"
                )
    ...
    def test_invalid_precision_values(self, mock_service):
        ...
        for precision, should_fail in test_cases:
            with patch(
                ...
                if should_fail:
                    mock_pretrained.side_effect = ValueError(
                        f"Invalid dtype: {str(precision)}"
                    )
                else:
                    mock_model = MagicMock()
                    result_obj = MagicMock()
        ...
        with patch(
            ...
            for channels, description in channel_configs:
                with patch(
                    ...
                    mock_sf.info.return_value = MockAudioInfo(
                        duration=1.0, channels=channels
                    )
            ...
            for duration, chunk_size, overlap, description in test_cases:
                with patch(
                    ...
                    if chunk_size >= duration:
                        ...
                        expected_chunks = 1 + (duration - chunk_size) // (
                            chunk_size - effective_overlap
                        )
                    ...
                    mock_model.transcribe.side_effect = [
                        f"Chunk {i}"
                        for i in range(expected_chunks + 5)  # Extra for safety
                    ]
    ...
    def test_nan_inf_in_audio_data(self, mock_service):
        ...
        with patch(
            ...
            test_cases = [
                ...
                (np.array([-np.inf] * 16000, dtype=np.float32), "-Inf values"),
                (
                    np.array([np.nan, 0.5, -0.5, np.inf] * 4000, dtype=np.float32),
                    "Mixed values",
                ),
            ]
        ...
        with patch(
            ...
            with patch(
                ...
                def transcribe_with_model(model_name):
                    result = mock_service._transcribe_with_parakeet_mlx(
                        audio_path="test.wav", model=model_name
                    )
                    results.append(result)
    ...
    def test_model_download_interruption(self, mock_service):
        ...
        with patch(
            ...
            with patch(
                ...
                result = mock_service._transcribe_with_parakeet_mlx(
                    audio_path="test.wav", model="mlx-community/parakeet-tdt-0.6b-v2"
                )
