...
conversation (or a character) and every message you send is rewritten on its way
out — "Meridian" becomes "Meridian, the sunken clock-tower city"; a shorthand
becomes a full instruction. It keeps terminology consistent without retyping.
...
"{N} entries · on" (or "· off") underneath. Highlight a row and press **Space**
to switch that dictionary on or off without opening it.
...
rule is dimmed and its pattern cell ends in "off". Click a row to load it into
the form below.
...
| Add / Update / Delete / Move up / Move down | Add a rule, save the highlighted one, remove it, reorder it |
...
- **Literal vs regex.** A literal pattern matches whole words only, so "cat"
  ...
  expression, or write the `/pattern/flags` form yourself with the flags `i`
  (ignore case), `m` (multi-line), `s` (dot matches newlines). Case-sensitivity
  applies to literal patterns only — for a regex use `/i`.
- **Probability** is a percentage roll per message: 100 always fires, 25 fires
  ...
  within one message, so "1" leaves later mentions alone.
...
"Priority must be a whole number.", "Select an entry row first."
...
never fire.` These **warn, they never block**. Click one to jump the table to
...
**global_lore_first**) setting the order rules are considered in when the budget
is tight; **Token budget** (default 1000), capping how much replacement text one
message may pull in; an **Enabled** switch for the whole dictionary; **Save
settings**, which answers "Saved." or "A name is required."; and **Export JSON**
/ **Export Markdown** under the note "Exports read the last saved state."
...
is live in ("Not attached to any conversation yet." until it is), with
**Attach to conversation…** and **Detach**.
...
marked, and the status line reads "No differences - no entry changed the
sample." or "Changed spans highlighted below." (until you pick a dictionary it
prompts "Select a dictionary to preview substitutions."). A summary then counts
"{fired} fired · {skipped} skipped · {used}/{budget} tokens" — plus "· over
budget" when the replacements did not fit — over one line per rule that fired
...
- "replacement failed — see logs" / "no replacement — text changed by an earlier entry"
...
announced as "Name in use - imported as '…'." **Export JSON** is the full
backup. **Export Markdown** is lossy and says so first: "Markdown keeps only
pattern and replacement text. These fields are DROPPED: regex/type, probability,
group, max replacements, timed effects, enabled, case-sensitivity, priority. Use
JSON for a full backup." with **Export anyway** / **Cancel**. Markdown *import*
drops the same fields.
...
- **To a character** — the **Dictionaries (copied into this character)** panel
  on the character card (see [Characters & personas](characters-and-personas.md)).
  That copy is a **snapshot**: editing the source dictionary afterwards does
  *not* change what the character carries — re-attach to refresh it.
...
character" rows, the " (shadowed)" / " (disabled)" markers (Console shows the
marker, not the rule above), and Console's **Attach dictionary…** /
**Detach dictionary…** — is in
...
1. **Create a dictionary and add one rule.** Press **New** (or **Ctrl+N**); you
   ...
   settings**. On **Entries**, type the text to find into **Pattern**, the
...
5. **Attach it to a conversation.** Open **Attachments**, press **Attach to
   conversation…**, search, select the row, press **Attach**.
6. **Back it up.** On **Settings** press **Save settings**, then **Export
   JSON**; the status line reports where the file went.
...
**Ctrl+S does not save a dictionary.** Use **Save settings**; entry edits save
the moment you press **Add**, **Update**, **Delete** or a move button. Elsewhere
on this screen Ctrl+Enter is the attach-to-Console shortcut, a no-op for a
dictionary.
...
- [Chat dictionaries — concepts & file format](../../Features/ChatDictionaries-Documented.md)
  ...
  Markdown file format. Two caveats: its UI walkthrough describes a retired tab,
  not this workbench; and the `[group]key: value|33` syntax it shows is **not**
  parsed by the current importer (Markdown import keeps pattern + replacement).
...
- [Roleplay & Chat Dictionaries](../roleplay-chat-dictionaries.md) — the
  screen's other three modes; [Guide index](../index.md) — global keys.
...
- **A preview can fire rules a real send drops.** The preview uses the
  ...
  fixed 500-token budget and `sorted_evenly`. If a rule fires in the preview but
  not for real, check the preview's token count against 500.
- **Timed effects have no editor fields.** Cooldown, delay and sticky survive
  JSON export/import and **Duplicate**, but nothing here sets them — the preview
  can still report "skipped: cooldown or delay" for an imported rule.
- **The entry list reloads after every change.** Rules are tracked by position,
  ...
  were on may shift. Preview results below 100% probability are not repeatable
  either — by design; the skip reason says so.
