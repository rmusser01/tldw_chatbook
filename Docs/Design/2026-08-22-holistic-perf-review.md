...
| 21110 splash / initial-screen overlap | Interleaved arms, n=10 boots, one boot per process, medians. Initial screen's import paid **on the event loop 0.307 s → 0.000 s**; splash close → screen usable **1.410 s → 1.083 s** (−23%); whole-process time-to-interactive **4.576 s → 4.282 s**. First boot after an upgrade (all `.pyc` discarded, n=4): import on the loop **0.944 s → 0.000 s**, close → usable **2.103 s → 1.128 s** (−46%). The 0.307 s did not relocate — the two post-push phases moved +0.015 s net across the boundary between them. Cost, measured not assumed: the splash animates at 20 Hz on that loop, and the GIL-holding import thread pushes the worst inter-frame gap **51.0 → 86.8 ms** (p95 50.9 → 52.9 ms, 2 gaps >100 ms per 10 boots, 0 dropped frames) at the chosen 0.2 s start delay; a 0 s start costs 111.5 ms worst / 69.8 ms p95 / 2 dropped frames, a 0.5 s start is no smoother and no longer fits a cold import inside the splash. |
