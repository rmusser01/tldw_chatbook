...
Fix tracking: backlog tasks **275–285 plus 257–262** (each finding names its task).
*Renumbering note (2026-07-17): the original batch 246–256 collided with the RAG-audit batch (both minted concurrently); the first re-target (263–273) was itself occupied by newer tasks before merging. Final mapping: 246→275, 247→276, 248→277, 249→278 (completed on dev while still numbered 249 — older commit messages cite that ID), 250→279, 251→280, 252→281, 253→282, 254→283, 255→284, 256→285. Tasks 257–262 were never renumbered.*
...
### A1. DB layer stringifies every query's params for a disabled debug log — task-275
...
### A2. Console `save_state()` runs twice per tab-switch-away — task-276
...
### A3. 100 ms guaranteed event-loop freeze every 2 s on the Embeddings screen — task-277
...
### A4. Conversation search uses a correlated `LIKE` scan; the FTS index already exists — task-278
...
### A5. Chatbook import commits ~1,500 transactions for a 50×30 import — task-279
...
### B1. The Console 0.2 s sync tick — task-280 (umbrella)
...
### B2. Library: 124 whole-screen recomposes; targeted `sync_state()` has zero callers — task-281
...
### B3. Home dashboard: 3 sync DB queries on the UI thread per visit/click — task-282
...
### B4. Debounced searches run sync sqlite/FTS on the event loop — task-283
...
### B5. Library RAG panel rebuilds ~100+ widgets per keystroke — task-284
...
### C1. `tldw_api` package: 1,313 eager Pydantic models, ~469 ms (31 % of import) — task-285
