...
| Personas | `ccp`, `characters`, `prompts`, `conversations_characters_prompts` | `PersonasScreen` (destination-native workbench) | Personas | Personas owns behavior and identity management; legacy routes resolve to the `personas` destination; the destination-native workbench replaced both the legacy CCP screen and the thin snapshot shell, and the legacy screen/widgets were removed; staged persona context uses Chat handoff payloads |
