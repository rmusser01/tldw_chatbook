...
- [ ] **Step 1:** Broad sweep: `$PY -m pytest Tests/Chat Tests/Library Tests/UI/test_console_command_composer.py Tests/UI/test_console_prompt_picker.py Tests/UI/test_console_system_prompt.py Tests/UI/test_console_native_chat_flow.py Tests/UI/test_console_session_settings.py Tests/UI/test_console_persistent_rails.py Tests/UI/test_library_shell.py Tests/UI/test_non_obscuring_focus_contract.py -q` → green (known pre-existing failure exempt: workspace-marker test, task-195).
- [ ] **Step 2:** Live verification against llama.cpp @127.0.0.1:9099 on a fresh profile: seed prompts; capture `/prompt` picker + insertion (short + collapsed-paste), unknown-command hint + Enter-again send, `/system` apply + `System:` rail preview, system modal, and ONE REAL SEND whose provider request contains the applied system message (assert via the llama.cpp server log or a captured request through a local proxy — the send succeeding with visibly system-prompt-shaped behavior plus the DB message row is acceptable evidence if request capture is impractical; state which was used). Captures → `Docs/superpowers/qa/console-prompts-2026-07/` + README.
- [ ] **Step 3:** Update spec/backlog: mark the plan checkboxes, commit evidence. **STOP — present captures for user approval; PR only after approval.**
