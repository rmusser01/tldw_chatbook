...
- [x] Full targeted sweep: the three tasks' test files + `Tests/TTS/test_profile_service.py` + `Tests/UI/test_stts_playground_catalog.py`; repo-wide `--collect-only`; ruff check + format on all touched files. 734 passed / 14 failed (pre-existing dead-widget drift, unchanged); 31539 collected, no import errors; ruff check clean; format --check flags 2 files with pre-existing (merge-base-reproduced) drift only.
- [x] OLD-PHRASE sweep (the free-floating-string lesson, run as its own numbered step): grep for "Unverified — this provider has no catalog check", "· Unverified ·" (legacy paths), "Profile availability is unverified", "Profile preview unverified" — every remaining hit must be either audio_cpp-scoped-and-true or inside the documented-dead `TTSPlaygroundWidget`. Zero unaccounted-for hits.
- [x] Live check (tmux, scratch config): Lab ▸ Speech ▸ Voice Profiles — with at least one legacy profile present (create via Playground save if the store is empty), observe the library row showing "No catalog check" and the detail line; adopt a legacy profile in the Playground and observe the adoption banner. If creating a profile live is impractical, drive the surfaces via the Textual pilot harness and say plainly what was and was not observed live. Driven fully live (see task-4-report.md LIVE VERIFICATION section); zero "unverified" occurrences observed anywhere in the flow.
- [x] Backlog: scan ALL worktrees for max task id, leapfrog with headroom; file the slice-2 task Done with Implementation Notes (including the enum-vs-presentation interpretation and its rationale). Update the spec's Status line for slice 2 in `Docs/superpowers/specs/2026-08-04-voice-profiles-expansion-design.md`. Filed TASK-2950 (Done) + TASK-2951/TASK-2952 (To Do, carry-forward findings); spec Status line updated.
- [x] Do NOT open a PR and do NOT merge — the controller owns that. (Honored — no PR opened.)
