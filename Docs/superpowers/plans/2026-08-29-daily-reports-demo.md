...
5. **Artifacts v1 action set (spec §2 promised more)**: v1 rows are labels + per-row Play plus one generic "Open Watchlists" button that navigates to the screen root — not deep-linked to the owning watchlist's artifacts pane. Reading, keep, and export stay in the Watchlists artifacts pane, one hop away. The spec's kept badge is deferred: `kept_briefings` lives in the ChaChaNotes DB (`Subscriptions/briefing_keep.py`), not SubscriptionsDB, so the spec's single-join premise does not hold — the badge needs a cross-DB lookup, not a join extension. Row-action deepening is follow-up work (recorded in ADR-079 Decision 6 and follow-up task TASK-21514).
