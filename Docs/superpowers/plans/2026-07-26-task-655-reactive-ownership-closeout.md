...
**Backlog:** [TASK-655](../../../backlog/tasks/task-655%20-%20Close-TldwCli-reactive-ownership-with-installed-distribution-sentinels.md)
...
**Depends on:** TASK-647, TASK-648, TASK-649, TASK-650, TASK-651, TASK-652, TASK-653, TASK-654

**ADR required:** yes

**ADR path:** `backlog/decisions/025-immutable-installed-distribution-assets.md`; `backlog/decisions/026-application-session-state-ownership.md`

**Reason:** ADR-026 defines the final application ownership boundary and ADR-025 requires the installed artifact—not the source checkout—to be the final release gate.
...
- [ ] Run focused source and production-app sentinels, then commit the
  TASK-655 source/test candidate before any release build:
...
git commit -m "test(state): enforce installed reactive ownership (task-655)"
```
...
- [ ] Re-read TASK-647 through TASK-655 with `backlog task <id> --plain`.
  ...
  earlier invariant.
- [ ] Check TASK-655 acceptance criteria only after the fresh source,
...
- [ ] Update the approved specification status to `Implemented` and add the
  final task/plan links only after TASK-647–654 are Done and TASK-655 has all
  implementation, verification, ADR, and documentation evidence complete.
- [ ] Add TASK-655 Implementation Notes with the exact source,
