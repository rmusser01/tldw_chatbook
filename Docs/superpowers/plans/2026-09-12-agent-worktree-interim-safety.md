...
Files: Agents/agent_worktree.py (small closed refusal/capability seam if needed); Agents/agent_service.py (request-plan disclosure, admission, merge/discard wrappers and cleanup); related focused Agents worktree/runtime tests; Console preview parity tests if the shared planner does not cover both routes; Docs/User_Guide/console/agent-runs-and-tools.md. Avoid adding a general backend registry, configuration switch or new schema.
