...
### Task 2.5: The sync storm (added by coordinator ruling after Task 2's review — the felt-freeze second half)
Task 2 eliminated the structural cause (0 whole-screen recomposes; mounts 177→81/26) but the felt freeze held (~98 ms block), and Task 2's review corrected the record's attribution: three canvas rebuilds per switch — one on the OUTGOING canvas (~21 mounts while still displayed), one while the destination is STILL HIDDEN (24 of 62 mounts on a later notes switch) — plus restyle cost that is substantially rebuild-independent (459 `Stylesheet.apply` calls on a 26-mount arm vs 405 on the 81-mount arm). This task OPENS with an attribution measurement (apply-calls by trigger: mount-proportional vs full-tree restyle — the reviewer-prescribed instrument), then scopes explicitly over: (1) the outgoing-canvas rebuild, (2) the pre-display hidden rebuild, (3) whatever the attribution shows of the restyle split. Known-useless moves (measured in Task 2, do not repeat): equality short-circuit inside `sync_state`; removing any of the first three coalesced sync calls. Acceptance: the felt-freeze evidence the plan's Goal names (block-time before/after under §9's same-location pairing) plus structural pins where measurable (e.g. rebuilds-per-switch, no repaint of a hidden or outgoing canvas); the strict-provenance rule applies to any pin derived here.
