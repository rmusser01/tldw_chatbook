...
- [ ] **Step 2: Write RED staged-identity tests.** Inject conversation/policy write failures and assert `persisted_conversation_id`, title, scope holder, message IDs, attachments, and policy holder remain byte-for-byte pre-call. A retry creates one conversation and publishes ID/title only after commit. Validate workspace first, keep `conversations.workspace_id` authoritative, and project WorkspaceDB membership idempotently after commit.
- [ ] **Step 3: Write RED promotion tests.** Promotion refuses unresolved preparation/checkpoint analogue before any write; success persists policy/full lineage/contributions atomically inside ChaChaNotes; each injected Chat write failure restores ephemeral identity, policy, messages, scope, contributions, and retryability. Workspace projection failure instead preserves committed identity and reconciles without duplicate UUID or membership.
...
- [ ] **Step 6: Integrate holder/coordinator ownership.** Register/unregister holders on restore/create/close/rollback/state replacement, hydrate restored policy asynchronously through the coordinator before execution, centralize the current-default provider in Store/runtime, publish committed saves to same-process siblings, and share one coordinator per app/store.
...
- [ ] **Step 2: Add destination disclosure combinations.** Test Automatic+Blocked as well as both Allowed combinations; verify Unknown never projects an on-device runtime category. User-facing destination copy is deferred to Task 20 and must not be invented here.
- [ ] **Step 3: Run the delivery battery.** Run `python -m pytest Tests/Chat/test_console_turn_library_authority.py Tests/Chat/test_console_library_destination.py Tests/Chat/test_console_chat_store.py Tests/Chat/test_console_library_runtime_policy.py Tests/Agents/test_library_name_reservation.py Tests/Agents/test_library_tool_provider.py Tests/Agents/test_tool_catalog_owner_cache.py Tests/Chat/test_console_agent_bridge.py -q`, then scoped Ruff and `git diff --check`. The store file is load-bearing evidence for exact attempt/message settlement, completion-before-observer ordering, replacement ordering, and session isolation; the owner-cache file is load-bearing evidence for overlapping registries, callable isolation, and peer cleanup.
