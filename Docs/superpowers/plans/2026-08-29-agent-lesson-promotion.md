...
- Before code changes, read `backlog/docs/lessons-testing-evidence.md`, verify TASK-24309 is Done, set TASK-24613 to `In Progress`, and add its Implementation Plan section linking this document and ADR-104.
- Read the approved spec, ADR-104, ADR-102, ADR-009, ADR-032, ADR-069, and current project-instruction/file-tool tests.
...
ADR path: `backlog/decisions/104-human-reviewed-agent-lesson-promotion.md`

Reason: promotion crosses untrusted Notes, file authority, project-instruction activation, approval, and managed-skill trust. ADR-104 fixes those boundaries; this task implements it without creating a new contract.
...
- [ ] **Step 5: Self-review and close.** Inspect every eligibility/role/capability combination, approval clearing/consumption, target/path races, trust transitions, ACs, and ADR-104. Add Implementation Notes, set TASK-24613 Done, and repeat provisional task/ADR collision checks before merge.
