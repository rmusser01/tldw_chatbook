...
- [ ] Read the source and root's isolated baseline logs `approval-baseline-1.log` through `approval-baseline-7.log` under `.superpowers/sdd/2026-09-12-agent-orchestration-remaining/`. All seven failed individually on the unmodified baseline; no need to repeat unchanged baseline runs. Logs 1/2 identify preregistration-before-publication and obsolete wall-clock accounting; logs 3–7 identify hidden cards/setup-modal focus/zero geometry.
- [ ] In `test_bridges_do_not_share_a_head`, wait for each exact published head before reading payloads. Use the existing `_wait_until` and identity helpers; keep both independent-head and clearing assertions. The required shape is:
