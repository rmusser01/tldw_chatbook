...
## 2026-09-08 rebase reconciliation

The checked steps and counts above record the original checkpoint. Dev now initializes MediaState before the moved block and constructs Ingest, Prompts, and MediaController after it. Preserve the complete early MediaState assignment, replace only the same six constructor assignments with the existing assembly call, and retain that later sequence before reader preferences. All six assignment ASTs match after only self/screen normalization; do not enlarge the factory or its 338-line cap. Retarget the construction-location test to the actual sequence and compare canonical class identities at their current module home. ADR required: no; this preserves dev's existing ownership and initialization order.
