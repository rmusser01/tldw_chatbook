...
- [ ] Close exact module-fixture-owned app databases and profile-lock handles after mounted harness teardown; a named FD census demonstrated retained regular files in new usage tests. Preserve borrowed resources and production lifecycle; compare complete targeted teardown before/after, never raise the sentinel threshold.
- [ ] Reconcile the inherited `_sync_stub` no-selection return with `sync_selected_fork_eligibility`'s actual two-item contract before judging transcript repaint failures; keep all behavioral assertions and change no production ChatScreen code.
...
- [ ] Set the existing `wrap_secondary` option while a nonzero child live-usage segment is present so realistic task text cannot ellipsize the count; preserve the existing terminal unread wrap condition. Confirm wide/narrow painted visibility. No new component field, stylesheet, or timer is needed.
