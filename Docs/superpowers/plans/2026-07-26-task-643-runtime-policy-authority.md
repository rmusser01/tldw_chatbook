...
**Reason:** ADR-033 already defines the runtime authority, mutation, projection, thread-affinity, and persistence boundary implemented by this task.
...
- Modify `tldw_chatbook/runtime_policy/source_state.py`: make runtime-policy JSON use the ADR-029 verified private reader and random-name atomic private writer.
