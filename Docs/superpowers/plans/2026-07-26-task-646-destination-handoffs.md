...
**Reason:** ADR-033 already defines the remaining cross-screen ownership, exact-target, current-session recovery, settlement, and final architecture boundary.
...
- Run, but do not weaken, `Tests/Packaging/test_installed_distribution.py`: integrated installed-wheel gate from TASK-903/ADR-025.
...
- add concise task-specific Implementation Notes to each task with approach,
  tradeoffs, modified files, ADR-033, and the exact focused plus shared gate
  evidence;
