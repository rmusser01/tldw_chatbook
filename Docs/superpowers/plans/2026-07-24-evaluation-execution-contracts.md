# TASK-496 Evaluation Execution Contracts Implementation Plan

> Execute this plan with the repository's `executing-plans`,
> `test-driven-development`, `systematic-debugging`, and
> `verification-before-completion` skills. Do not begin TASK-497 changes in
> this task.
...
**Reason:** ADR-024 already decides the provider, concurrency, callback,
...
## Task 6: Reconcile UI callbacks and shutdown

**Files**

- Modify: `tldw_chatbook/Event_Handlers/eval_events.py`
- Create: `Tests/Event_Handlers/test_eval_events.py`
- Modify: `tldw_chatbook/Evals/eval_orchestrator.py`
- Modify: `Tests/Evals/test_eval_orchestrator.py`
- Modify: `tldw_chatbook/Evals/README.md`
- Modify: `tldw_chatbook/Evals/DEVELOPER_GUIDE.md`

1. Add event-handler tests that exercise the real callback signature. Assert
   progress/cost updates run, the durable run ID binds the progress adapter and
   cost tracker, and no placeholder UUID is generated.
2. Assert same-loop callbacks do not call `app.call_from_thread()`. Use direct
   widget updates or the application's same-loop scheduling seam.
3. Update cancellation handling to await
   `orchestrator.cancel_evaluation(run_id)` and surface success only after
   cleanup.
4. Add `aclose()` tests: signal every active run before draining, drain all,
   ...
   `quick_eval`.
5. Update public Evals documentation for the callback signatures, run-start
   callback, async cancellation, and shutdown behavior.

## Task 7: Verify and reconcile TASK-496
...
1. Run focused warning-strict tests:
   ...
   python -m pytest \
     ...
     Tests/Event_Handlers/test_eval_events.py \
...
3. Run the affected UI/event integration tests selected by the changed call
   sites.
4. Run Ruff on changed Python files, `python -m compileall` on changed source,
   and `git diff --check`.
5. Review the final diff against every TASK-496 acceptance criterion. Add
   implementation notes with exact test counts and any verified baseline-only
   failures, check all criteria, and only then mark TASK-496 Done.
