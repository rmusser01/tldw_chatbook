...
- Approved Task8 integration extensions: `Chunking/lab_state.py` (pure record-field edits, lineage-fenced saved-record association, unsaved captured record fields) and `Chunking/lab_coordinator.py` (read-only existing recovery warning); focused state and recovery-flow tests.
- Approved integration cancellation repair: `UI/Chunking_Lab_Modules/results_region.py` submits lazy async callables to exclusive workers, with a focused regression in `Tests/UI/test_chunking_lab_results.py`; no rendering or result-contract change.
