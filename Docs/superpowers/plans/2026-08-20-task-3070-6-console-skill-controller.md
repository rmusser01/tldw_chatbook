...
- [x] Commit only the reviewed task/plan records, fetch `origin/dev`, rebase this isolated branch, and prove the worktree contains no implementation change before baseline execution.
- [x] Record exact results for the existing skill command, install/script decision, command-popup, visit-refresh, root-state, resolver, substitution, modal-dismissal, CSS-sync, and Wave 6 inventory tests before any production edit:
  ...
  ```
- [x] Record `chat_screen.py` line/direct-method counts and the current diagnostic non-write result so inherited failures remain distinguishable from TASK-3070.6 changes.
- [x] Stop before implementation if the rebased source no longer matches the approved nine-M/three-D/four-X inventory.

### Task 1: Lock ownership and deletion requirements with RED tests

- [x] Add `Tests/UI/test_console_skill_controller.py` with plain async fakes/call recorders and no Textual mount.
- [x] Assert default candidate state, fresh context success/failure, trusted candidate filtering/order, blocked exact/prefix behavior, `/skills` list/hint copy, and pending install/script field isolation.
- [x] Extend the Wave 6 architecture inventory so all nine M methods exist only on `ConsoleSkillController`; all three D methods remain on `ChatScreen` with their framework binding and at-most-five-line spans; the candidate descriptor targets `_skill`; the controller has no DOM/sibling reach-through.
- [x] Add deletion assertions for the four X methods, `KIND_FALLBACK` screen dispatch, picker module/import/test/CSS selectors, and stale fallback/picker runtime references across the new `skill.py`, `chat_screen.py`, resolver, and style-picker files; scan all production Python sources for removed `ChatScreen` skill-policy ownership tokens.
- [x] Add a non-vacuity mutation fixture proving the deletion oracle catches a synthetic fallback/picker reference.
- [x] Run the new controller and architecture nodes and confirm RED is caused by absent ownership and still-present dead surface.

### Task 2: Move the live skill policy into the controller

- [x] Create `ConsoleSkillController` with `app_instance` plus named callbacks for transcript append, command-popup sync, task-resume-state read/write, and current chat-controller lookup.
- [x] Initialize `_console_skill_candidates` in the controller.
- [x] Move the nine M methods without changing user-visible copy, filtering, ordering, exception containment, or field-replacement semantics; rewrite their internal documentation so it no longer names the deleted fallback/picker chain.
- [x] Add controller bodies for the three D entry points: registered `/skills`, install decision, and script decision.
- [x] Construct `screen._skill` in `build_console_controllers`; add a focused wiring test for the concrete controller type and late-bound callbacks, update only `wiring.py`'s full-graph count/order documentation, preserve `test_console_controller_wiring.py`'s original six-controller `_EXPECTED_SLOTS`/order/shared-accessor contract unchanged, and expose no screen-module class re-export.
- [x] Add `_ControllerState("_skill", "_console_skill_candidates")` and remove the screen's eager default assignment.
- [x] Repoint mount/resume refreshes, unknown-command blocked checks, popup candidate reads, and chat-controller pending callbacks directly to `_skill`.
- [x] Reduce each D screen entry point to plain-value extraction/event stop plus one controller call.
- [x] Run the isolated controller, architecture, wiring, command, popup, visit, pending-card, and root-state nodes to GREEN.

### Task 3: Delete the unreachable fallback/picker surface

- [x] Remove `KIND_FALLBACK` from the screen import and command-send/dispatch branches.
- [x] Delete `_console_skill_search`, `_console_command_run_skill`, `_run_resolved_console_skill`, and `_open_console_skill_picker`.
- [x] Delete `_CONSOLE_SKILL_SEARCH_LIMIT` and imports/constants used only by that picker/fallback chain; prove remaining imports still have live callers and that neither the moved controller nor the screen retains picker/fallback documentation.
- [x] Delete the picker module and its dedicated test file; remove its modal-dismissal contract/import and update the exact contract cardinality.
- [x] Remove the picker CSS source block and regenerate the modular bundle using `python tldw_chatbook/css/build_css.py`; verify with `python tldw_chatbook/css/check_bundle_sync.py`.
- [x] Delete obsolete dead-path tests (including the direct resolved-picker send test) while preserving all live `/skills`, `$name`, blocked/refusal, command-popup, and modal contracts.
- [x] Rewrite resolver/style-picker source/test documentation and moved-owner precedent references so no live code names the deleted picker, dead slash fallback, or removed `ChatScreen` skill-policy methods.
- [x] Run the deletion architecture nodes, CSS sync/build integrity nodes, modal-dismissal contract, resolver tests, style-picker tests, and live skill tests to GREEN.

### Task 4: Prove behavior and mutation sensitivity

- [x] Run the focused behavior matrix recorded in the baseline: skill commands, install/script confirmation, command popup, visit refresh, root-state pending bridge, resolver, substitution, modal dismissal, plus the new controller/architecture tests.
- [x] Mutate fresh-context retrieval to reuse cached candidates; confirm the freshness/refusal test fails, then restore.
- [x] Remove the trust-block filter or stable sort; confirm the isolated projection test fails, then restore.
- [x] Replace the whole resume state or update the wrong pending field; confirm the field-isolation test fails, then restore.
- [x] Remove each of the three screen delegations in turn; confirm the delegate/behavior test fails, then restore.
- [x] Reintroduce one dead picker/fallback token in a synthetic fixture; confirm the deletion oracle fails, then restore.
- [x] Measure `chat_screen.py` line/method change without changing the ratchet ceiling; stop if the approved inventory no longer produces the expected reduction.

### Task 5: Static checks, review, and closeout

- [x] Run Ruff lint and format checks on changed Python files only; do not bulk-format unrelated inherited drift.
- [x] Run `py_compile` for `skill.py`, `wiring.py`, and `chat_screen.py` under one validated temporary pycache root, then remove only that root.
- [x] Run CSS bundle sync, `git diff --check`, the metadata-only diagnostic architecture node, and the persistent-diagnostic checker; update the inventory only for a proven owner move with unchanged sink topology.
