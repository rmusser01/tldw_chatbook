...
7. **Errata (controller ruling, Task 5 fix round 1):** the brief's `run_simulation_layer` returned error cells from the `except` branch without appending them to `cells` (and `asyncio.gather` discards return values), silently dropping chat-exception evidence and shrinking the activation denominator. Ruling: error cells are counted as failures, never dropped — they MUST be appended like parsed cells (`activated=None`, `error=str(exc)`, `raw=""`) so `failure_rate` counts them, Task 10 artifact saving carries the evidence, and the activation denominator includes them. Implemented in `simulation.py` (`one()` builds the cell in both try/except paths, one shared append + progress fire); pinned by `test_simulation_layer_records_error_cells`. Do not reintroduce early returns from the except branch.
