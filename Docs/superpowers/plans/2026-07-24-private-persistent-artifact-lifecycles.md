...
- [ ] Add tests proving basename-only application log selection rejects
  absolute paths, separators, dot entries, and traversal without creating an
  outside directory.
- [ ] Add POSIX tests proving new and eligible existing application log files
  and rotated generations become `0600` beneath a `0700` application data
  directory.
- [ ] Add log-handler tests proving a symlinked active or rotated generation
  disables only the file sink.
- [ ] Extend MCP tests for private creation, existing-file hardening, rotation,
  no-follow count/read/append, corrupt-line tolerance, and unsafe target
  containment.
- [ ] Add cache tests proving pickle is never invoked, legacy pickle remains
...
- [ ] Add target/parent replacement and simulated unverified-platform tests
  around the shared private append/replace seam.
- [ ] Run the new tests and record the expected failures against the current
...
- [ ] Add a private append opener that verifies or exclusively creates a
  current-user regular file, applies `0600`, pins identity, and never follows
  the leaf.
- [ ] Add atomic private byte/text replacement using a `0600` sibling created
...
- [ ] Preserve the existing Windows `unverified_platform` contract without
  claiming POSIX mode or ACL verification.
- [ ] Run the focused utility tests until green.
...
- [ ] Make `get_cli_log_file_path()` accept only a non-empty basename and
  return a child of the secured user data directory without creating
  user-controlled parents.
- [ ] Add a `RotatingFileHandler` subclass whose active stream uses the shared
  no-follow private append opener.
- [ ] Before installation and after rollover, verify or harden every eligible
  active/rotated generation; reject links and non-regular entries.
- [ ] On an unsafe file target, omit only the file handler and emit a bounded
  metadata-only warning while Rich/UI/terminal handlers remain installed.
- [ ] Run the focused logging tests until green.
...
- [ ] Secure the application-owned parent before every operation.
- [ ] Count and read generations through the pinned private binary opener.
- [ ] Append through the private append seam.
- [ ] Rotate by safely reading the active generation and atomically replacing
  the rotated and active generations; never follow or overwrite an unsafe
  target.
- [ ] Keep torn/corrupt final-line tolerance and the two-generation bound.
- [ ] Run the focused MCP tests until green.
...
- [ ] Remove the pickle import and define one versioned JSON cache envelope
  with explicit entry count, byte-size, key, expiry, finite-number, depth, and
  JSON-value validation.
- [ ] Load through the private binary opener; harden but ignore legacy,
  corrupt, oversized, or invalid files without deserializing executable data.
- [ ] Persist only JSON-compatible entries through atomic private replacement;
  leave unsupported results in the bounded in-memory cache.
- [ ] Preserve LRU order, TTL expiry, cache-hit return values, and clear
  behavior.
- [ ] Run the focused cache tests until green.
...
- [ ] Run `pytest Tests/Utils/test_private_persistent_artifacts.py Tests/MCP/test_execution_log.py Tests/Tools/test_tool_result_cache_persistence.py Tests/test_logging_private_files.py -q`.
- [ ] Run relevant broader MCP, config/private-path, and tool tests.
- [ ] Run Ruff on changed Python files, Python compilation, and
  `git diff --check`.
- [ ] Run a real sentinel/mode probe against all three persistent artifacts.
- [ ] Self-review the complete diff for unsafe path races, payload diagnostics,
  and scope creep.
- [ ] Check all TASK-490 acceptance criteria, add concise implementation notes
