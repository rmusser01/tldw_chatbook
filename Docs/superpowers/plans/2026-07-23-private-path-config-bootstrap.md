...
Reason: ADR-029 already defines the private local-data boundary. This task directly implements that accepted decision and does not make a new architecture choice.
...
- Modify `backlog/tasks/task-943 - Establish-private-path-boundary-and-harden-config-bootstrap.md`: record this plan, then add implementation notes and completion evidence after verification.
...
### Task 6: Verify TASK-943 and close its Backlog record

**Files:**

- Modify: `backlog/tasks/task-943 - Establish-private-path-boundary-and-harden-config-bootstrap.md`
...
2. add concise implementation notes listing the private-path module, config bootstrap integration, exact ignore guard, test evidence, and ADR-029;
3. set TASK-943 to `Done` only after all Definition of Done checks pass.

- [ ] **Step 6: Commit task completion metadata**

```bash
git add \
  "backlog/tasks/task-943 - Establish-private-path-boundary-and-harden-config-bootstrap.md"
git commit -m "docs(backlog): complete private path bootstrap task"
```

## Plan Self-Review

- Spec coverage: every TASK-943 acceptance criterion maps to Tasks 1–5; Task 6 verifies and records the evidence.
...
- Scope boundary: TASK-943 changes config selection/bootstrap only. TASK-491 remains responsible for the full persistence owner, raw editor, encryption operations, recovery, exports, backups, cache generations, and live runtime snapshots.
