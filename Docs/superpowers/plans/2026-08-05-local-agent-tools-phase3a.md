...
- [x] **Step 1:** Add to the composed system prompt one line: `Additional tools (file, web, and more) are available but not shown; use find_tools to search the catalog and load_tools to load their schemas before calling them.` — shown only when the run's catalog exceeds the direct-disclosure threshold (implemented as a conditional `offer_find_load` kwarg; the call site knows the registry size). NOTE: the plan originally said "file, web, git, and more" — "git" was dropped in review until the git tools land in 3b-ii. Test: composed prompt contains the hint when offered.
