...
- [x] Write RED real Git+SQLite tests before implementation for denial/no callback, original-base apply after parent HEAD advances, new/untracked/binary changes, explicit merge commit, same-file conflict preserving parent and source, changed preview refusal, wrong conversation/binding/identity, held/uncertain refusal and two competing claims.
- [x] Add RED logical discard tests for clean and dirty child, ignored entries, external symlink target survival, retained detached baseline, exact-ref mismatch, nested-repository refusal, and unsupported primitive refusal before mutation. No tests touch user repositories.
- [x] Implement the shared operation function and narrow fixed lifecycle changes. Test interruption before consent and mutation-completion persistence failure: reopening must not allow automatic replay. Test known-no-effect claim release separately from uncertain completion.
- [x] Wire real same-turn closures and disclosure with targeted actual run_turn tests; include waiting for actual owner drain, not only fleet terminal status. Existing tests that expected the superseded blanket refusal should now test missing authority/card explicitly.
- [x] Run focused new engine tests and directly affected helper/service/schema nodes. Record output, compare added Ruff identities and edited-hunk formatting against the source base, and run diff-check. Keep inherited warning qualification precise.
- [x] Report behavior/evidence and public interfaces for Console integration. Leave source unstaged for independent review; the visible card, preview call-site parity and retained recovery list are the next slice, so neither task is Done yet.

## Completion

Implemented in720bc15f92; merge-receipt correctionfc9c01473e. Independent task review identified the false already-incorporated merge receipt; the correction passed scoped rereview. Root verified the reported targeted evidence and scoped static outputs. The confirmed implementation uses the new recovery/private Git modules; legacy lifecycle helpers remain unchanged and are not called by confirmed production actions. See [the durable review record](../reviews/2026-09-12-agent-worktree-restoration.md) for exact evidence, process qualifications and rulings. Console integration remains a separate unfinished plan.
