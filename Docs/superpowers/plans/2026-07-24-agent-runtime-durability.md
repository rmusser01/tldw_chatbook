...
5. **Tool timeout:** `RunBudget.max_tool_call_seconds` (0 = unlimited); enforced via a module-level `_call_with_timeout` using a **per-call daemon thread + join(timeout)** (NOT a ThreadPoolExecutor `with`, NOT a shared pool); wraps ONLY the builtin/custom `registry.invoke_by_name` (skill calls route around it); MUST be added to `clamp_child_budget`.
   ...
   **Also corrected:** Task 4 Step 3c's `_call_with_timeout` code block below shows `except Exception` in `_runner` and a bare `return box["result"]` at the end. The shipped helper instead catches `except BaseException` (a `SystemExit`/`asyncio.CancelledError` raised by a tool must still surface as a failed `ToolResult`, not propagate out of the daemon thread's target and leave neither `box` key set) and uses `box.get("result")` with an explicit "tool call produced no result" fallback ToolResult (not a bare dict index, which would `KeyError` if the box were ever empty). See the spec's CORRECTION block for the full reasoning.
