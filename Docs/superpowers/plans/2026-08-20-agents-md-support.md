...
| `tldw_chatbook/LLM_Calls/LLM_API_Calls.py` | Consume the ephemeral-origin marker during final OpenAI/Anthropic/Gemini native conversion and emit provider-valid result/context grouping. |
...
- [ ] **Step 1: Seed failing migration tests from a real v32 database.** Add named cases `test_v32_to_v33_adds_nullable_local_column`, `test_v32_to_v33_recovers_column_present_version_still_32`, `test_v32_to_v33_rejects_wrong_start_version`, and `test_fresh_schema_contains_console_project_context_column`. Assert v33, nullable `console_project_context_json`, rollback/version guards, v32-to-v33 data preservation, and fresh database availability. Use only `tmp_path`/in-memory DBs.
...
- [ ] **Step 4: Run `python -m pytest Tests/DB/test_chachanotes_console_project_context_migration.py Tests/Chat/test_console_chat_store_project_instructions.py Tests/Chatbooks/test_chatbook_importer.py -q`.** Expected: named failures for the missing fresh-schema column, v32→v33 migration/accessors, state lifecycle, and importer preservation.
- [ ] **Step 5: Implement the fresh schema and rollback-safe additive migration.** Add nullable `console_project_context_json TEXT` to the canonical `CREATE TABLE conversations`; set `_CURRENT_SCHEMA_VERSION = 33`; add `_MIGRATE_V32_TO_V33_SQL`, `_migrate_from_v32_to_v33`, and migration-map entry `32`. The method requires start version 32, checks `PRAGMA table_info(conversations)`, executes the `ALTER TABLE` only when the column is absent, then performs the guarded `32 -> 33` version update in the surrounding rollback-safe transaction. A database left with the column present and version 32 completes by updating only the version; any other partial/error state raises `SchemaError` and leaves version 32.
...
- [ ] **Step 7: Extend the persistence protocol/service and session lifecycle.** Add one `project_instruction_state` field to `ConsoleChatSession` whose dataclass default factory is `ProjectInstructionControlState.legacy_disabled`. Only `ConsoleChatStore.create_session()` explicitly supplies `new_session()`; every direct construction and every restore therefore fails closed unless new-session creation opted in. `restore_persisted_session()` decodes the database value and defaults legacy-disabled. Update in-memory state before attempting its local-only write; on failure retain it and post only the content-free warning callback. Temporary sessions keep state in memory; `promote_ephemeral_session()` writes it only after the durable conversation exists, and a forced write failure rolls back the promotion's durable artifacts while preserving the still-temporary session and its in-memory state.
- [ ] **Step 8: Re-run the exact Step-4 command.** Expected: pass, including partial-migration recovery, write-failure behavior, rollback, and no sync/version churn.
...
- Modify: `tldw_chatbook/LLM_Calls/LLM_API_Calls.py` (OpenAI request builder plus Anthropic `1137-1405` and Gemini `3140-3378` native conversion)
...
- [ ] **Step 5: Run `python -m pytest Tests/Agents/test_agent_service.py Tests/Chat/test_console_agent_project_instructions.py Tests/Chat/test_console_local_review_hook.py Tests/Chat/test_console_agent_bridge.py Tests/Chat/test_console_provider_gateway.py Tests/Chat/test_anthropic_native_tools.py Tests/Chat/test_google_native_tools.py Tests/Chat/test_console_rewind_summarize.py -q`.** Expected: failures for the absent candidate/snapshot, exact request/payload budget, consent, native marker, rider, and selected-root behavior.
...
- [ ] **Step 9: Preserve the marker to the actual serialization boundary.** Attach `EPHEMERAL_ORIGIN_KEY` internally and keep it through `ConsoleProviderGateway._chat_api_kwargs()`. Consume it in `LLM_API_Calls.py` while constructing final OpenAI/Anthropic/Gemini payloads, and in the gateway only for direct llama.cpp payloads. Anthropic coalesces all `tool_result` blocks and the following project-context text block into one user turn; Gemini emits all function responses before a separate user text part/turn; OpenAI-compatible emits every tool row before a separate user row. Fenced/local rendering closes the complete results fence before a separately labeled context section. There is no current durable exchange-capture subsystem to modify; the origin tag defines the omission boundary any such subsystem must honor later.
...
- [ ] **Step 12: Re-run the exact Step-5 pytest command, then run `python -m ruff check tldw_chatbook/Chat/console_project_instructions.py tldw_chatbook/Agents/project_instruction_resolver.py tldw_chatbook/Agents/agent_models.py tldw_chatbook/Agents/agent_service.py tldw_chatbook/Agents/local_tool_provider.py tldw_chatbook/Chat/console_agent_bridge.py tldw_chatbook/Chat/console_provider_gateway.py tldw_chatbook/LLM_Calls/LLM_API_Calls.py Tests/Chat/test_console_agent_project_instructions.py Tests/Chat/test_console_local_review_hook.py Tests/Chat/test_console_agent_bridge.py Tests/Chat/test_console_provider_gateway.py Tests/Chat/test_anthropic_native_tools.py Tests/Chat/test_google_native_tools.py Tests/Chat/test_console_rewind_summarize.py`.** Expected: pass.
...
- [ ] **Step 2: Run `python -m ruff check $(git diff --name-only 5047b6962...HEAD -- '*.py')`, `python -m ruff format --check $(git diff --name-only 5047b6962...HEAD -- '*.py')`, and `git diff --check 5047b6962...HEAD`.** Expected: clean. Do not format unrelated code; if Ruff reports pre-existing formatting in a touched file, record the baseline and format only feature-owned hunks/files after checking the diff.
...
- [ ] **Step 1: Write ledger and child-initial-context tests.** Root snapshot is active at dispatch; nested sources pin once; parent/subagents share source outcomes and byte budget; each chain has its own delivery revision/cursor. `initial_context_for_chain(chain_id, payload_state)` returns every currently active root/nested row plus a delivery receipt; the cursor advances only when `mark_payload_sent(receipt)` runs after those rows are included in that chain's first outgoing payload. Cover a child spawned after nested activation and a child whose first request races a later activation.
...
- [ ] **Step 7: Thread exact chain context through `AgentService`.** Add optional `project_instruction_context` and stable `chain_id` inputs to `_run_one`; primary and inline spawned children share the context but get distinct IDs. Before each chain's first call, `_build_model_request` asks `initial_context_for_chain` for the currently active root/nested rows; it includes them in that exact outgoing request and calls `mark_payload_sent(receipt)` only after request construction. Immediately before later preparation, `AgentRuntime` refreshes the same chain's `InstructionChainPayloadState`; the unchanged one-argument `prepare_tool_calls(calls)` closure reads that state. Extend `review_state_scope` tests so a child cannot clobber the parent's state.
- [ ] **Step 8: Re-run the exact Step-5 command and `python -m ruff check tldw_chatbook/Agents/agent_runtime.py tldw_chatbook/Agents/agent_service.py Tests/Agents/test_agent_runtime_review_hook.py Tests/Agents/test_agent_runtime_preparation.py Tests/Agents/test_agent_service.py Tests/Agents/test_agent_service_review_state_scope.py`.** Expected: pass.
...
- [ ] **Step 2: Write exact outgoing-payload grammar tests for four transport families.** Mock the final HTTP send in `LLM_API_Calls.py`, not only the gateway input. OpenAI-compatible: every tool response row precedes a separate user context row. Gemini: every `functionResponse` precedes a separate user text part/turn. Anthropic: one user turn contains all `tool_result` blocks first and one distinct context text block last. Fenced/local: the complete results fence closes before a separately labeled context section. Assert the internal origin key is absent only in the final outgoing payload and context text never appears in a tool result.
...
- [ ] **Step 7: Add transport serialization at the true final boundary.** Canonical runtime order stays transport-independent. The gateway preserves tags when routing through generic adapters; `LLM_API_Calls.py` consumes them while building OpenAI/Anthropic/Gemini request bodies, and the direct llama.cpp builder consumes them itself. Native coalescing follows Step 2 exactly. Fenced/local rendering uses the runtime's canonical result-stub helper and closes the full results section before its context section. No earlier helper strips the marker.
...
- [ ] **Step 9: Re-run the exact Step-5 command, then run `python -m ruff check tldw_chatbook/Chat/console_agent_bridge.py tldw_chatbook/Chat/console_provider_gateway.py tldw_chatbook/LLM_Calls/LLM_API_Calls.py tldw_chatbook/Chat/console_chat_controller.py tldw_chatbook/Chat/console_chat_models.py Tests/Chat/test_console_agent_bridge.py Tests/Chat/test_console_provider_gateway.py Tests/Chat/test_anthropic_native_tools.py Tests/Chat/test_google_native_tools.py Tests/Chat/test_console_project_instruction_provider_grammar.py Tests/Chat/test_console_project_instruction_persistence_boundary.py Tests/Chat/test_console_agent_bridge_local.py`.** Expected: pass.
...
- [ ] **Step 3: Run `python -m ruff check $(git diff --name-only 5047b6962...HEAD -- '*.py')`, `python -m ruff format --check $(git diff --name-only 5047b6962...HEAD -- '*.py')`, and `git diff --check 5047b6962...HEAD`. Set `CHATBOOK_AGENTS_QA_DIR=/tmp/chatbook-agents-md-delivery2`, direct the sentinel tests' pytest capture/SQLite dumps/run logs there, then run `rg -n 'CHATBOOK_AGENTS_SENTINEL_7d1e9c' /tmp/chatbook-agents-md-delivery2`.** Expected: only the allowlisted fake provider-request spy contains the body.
...
- [ ] **Step 3: Run the broader and static gates exactly:** `python -m pytest -q`; `python -m ruff check $(git diff --name-only 5047b6962...HEAD -- '*.py')`; `python -m ruff format --check $(git diff --name-only 5047b6962...HEAD -- '*.py')`; `python -m mypy tldw_chatbook/Agents/project_instruction_resolver.py tldw_chatbook/Agents/project_instruction_runtime.py tldw_chatbook/Chat/console_project_instructions.py`; `python -m bandit -q -r tldw_chatbook/Agents/project_instruction_resolver.py tldw_chatbook/Agents/project_instruction_runtime.py tldw_chatbook/Chat/console_project_instructions.py tldw_chatbook/Chat/console_agent_bridge.py`; `python -m pip check`; `python -c 'import pathlib,tomllib; d=tomllib.loads(pathlib.Path("pyproject.toml").read_text()); assert d["project"]["license"] == "AGPL-3.0-or-later"'`; and `git diff --check 5047b6962...HEAD`. Record exact counts and any established unrelated baseline separately; do not call a baseline failure green.
