...
4. Resolve `frame["asset_id"]` via the assets index (`source_asset_id` match, `asset_bytes_status == "present"`) → `asset_path` (unknown/missing → skip).
