...
Applied books take effect from the send path's `active_char_data['extensions']` — so, exactly like the merged character-dictionary attach, a mid-session attach reflects when that character is the active character on send (a live refresh of an in-flight chat's cached card is out of scope here, same as P1f).
...
- The snapshot is `export_world_book(id)` output — `{name, description, scan_depth, token_budget, recursive_scanning, entries:[…]}`, each entry carrying every matcher field incl. `keys`/`content`/`secondary_keys`/`position`/`insertion_order`/`case_sensitive`/`selective`/`priority`/**`regex`**/`enabled`. **Verified: this shape is already `_process_world_books`-compatible** — that consumer reads `book["name"]`, `book["entries"]`, `scan_depth`/`token_budget`/`recursive_scanning`, and defaults a missing `id`→`None` and `priority`→0 exactly as wanted for a character-attached book (no conversation-junction priority). So the resolver returns the snapshot **book dicts** essentially as-is (after dedup/exclude); it does NOT flatten entries — `_process_world_books`/`_process_entry` iterate entries, honor per-entry `enabled`, and apply the P2d-regex load-time backstop.
- **The ONE seam gap:** `export_world_book` OMITS book-level `enabled` (`_process_world_books` would then default it to `True`). So the attach service stores the snapshot as export output **augmented with the source book's `enabled`**, and the resolver + `_process_world_books` both honor it. (Book-level `id` is intentionally absent → the diagnostics `book_id` is `None`, like a native character book.)
...
A list of snapshot blocks — each is `export_world_book` output **augmented with the source book's `enabled`** (export omits book-level `enabled`; see Ground truths). Idempotent by name (attach is a no-op if a block with that name is already present). Written only by the attach service via `update_character_card({"extensions": ext}, expected_version=...)`.

### 2. Service (`WorldBookManager`, net-new methods)
- `attach_world_book_to_character(self, world_book_id: int, character_id: int) -> Dict[str, Any]` — snapshot the book via `export_world_book`, set `block["enabled"] = source_book["enabled"]` (export omits it), read the character record, append the block iff no same-named block exists, write back under `expected_version`. Returns `{"character_id": int, "name": str, "attached": bool}` (`attached=False` when it was already present). Mirrors `LocalChatDictionaryService.attach_to_character`.
...
- **Placement is load-bearing:** the union must run **before** the existing `if has_character_book or world_books:` processor-init guard. Otherwise a character with attached books but *no* conversation books and *no* native `character_book` would leave `world_books` empty → the processor is never built → the attached books silently never apply. Placing the `+=` before that guard makes `world_books` non-empty so the processor initializes. The resolver runs regardless of whether a conversation is active (it reads only `char_data`).
...
- **Send-path integration:** with a character-attached book and no conversation book, `WorldInfoProcessor` (built the way `chat_events` builds it) fires the attached book's entries; with a same-named enabled conversation book, only one fires (conversation's). **Attached-only character** (no conversation books AND no native `character_book`) still initializes the processor and fires the attached book (guards the before-the-guard placement). A **book-level-disabled** attached snapshot does not fire (guards the `enabled`-augmentation), while its per-entry `enabled=false` entries are also dropped.
...
- [ ] `chat_events.py` unions character-attached books into the send path with **conversation-wins** name dedup, **before** the processor-init guard so an attached-only character (no conversation books, no native book) still applies its books; the native `character_book` and the no-attachment case are byte-identical to today.
- [ ] The stored snapshot carries book-level `enabled` (augmenting `export_world_book`, which omits it), and a disabled attached book does not fire.
