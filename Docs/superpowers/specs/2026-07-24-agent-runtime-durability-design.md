...
- **Documented residual:** a timeout preempts the loop (the run is not wedged, cancellation/budget can proceed) but cannot kill the underlying thread — a truly-hung tool's daemon thread leaks until process exit (same limitation as MCP's future path).

> **CORRECTION (post-implementation review, 2026-07-25).** Two claims above were wrong and are superseded by the shipped code:
> 1. **MCP tools are NOT exempt.** `MCPToolProvider` is registered into the *same* per-run `ToolCatalogRegistry` as builtins (`console_agent_bridge._compose_run_registry_and_allowed`), so MCP calls flow through `registry.invoke_by_name` inside the wrapped closure like any other tool. The parenthetical in **Gap (verified)** above ("skill calls route around `invoke_tool`") remains true; the implied MCP exemption does not.
> 2. **The inner timeout does NOT always fire first.** MCP's own worst case is ~186s (approval wait 120s + 1s poll slack, then execution 60s + 5s result slack), which exceeds the originally-specified 120s default — the approval wait alone (121s) does. A premature outer timeout is not free: the abandoned daemon thread keeps polling, so an "Ask"-gated tool the user approves late still executes for real after the agent was told it failed, and an agent retry can execute it twice on one approval.
>
> **Shipped resolution:** the default is `300.0`, deliberately above MCP's worst case, so the wrapper never preempts MCP's own bounds under default config. Lowering it below ~186s re-opens the double-execution window for side-effecting MCP tools. Related ruling: `max_tool_call_seconds` is deliberately **not** clamped to the parent's remaining wall-clock in `clamp_child_budget` (unlike `max_wall_seconds`) — clamping it down would make premature timeouts more frequent, which is precisely the trigger for that hazard; the parent's `max_wall_seconds` already bounds the run.
>
> **Also corrected (final whole-branch review, 2026-07-25):** the "Tests" paragraph below prescribes releasing a `threading.Event` after the timeout assertion so the daemon thread exits cleanly. The shipped tests use a plain `time.sleep(2.0)` in the blocking fake instead — simpler, and the abandoned daemon thread still exits on its own once the sleep elapses (it's a daemon, so it never blocks interpreter/process exit either way). The covering suite (`Tests/Agents/ Tests/DB/test_agent_runs_db.py`) confirms this produces no leaked-thread warnings.
