...
full digests and the grammar/adapter/layout/semantic-limit contract. It satisfies
the existing 64-byte safe-identifier constraint. Never reassign an ID to changed
manifest bytes. A semantic change allocates a new profile ID.
...
execution policy and compiler security checks must also permit it. Apply refusal
on every execution admission and invalidate active previews/bridge reservations
when an installed policy change is observed. No online revocation lookup or new
remote policy service is introduced; installed application updates carry policy.
...
identity and compiler/security-policy identity; revocation invalidates executable
cache entries. Archives contain source and profile IDs, not executable catalog
bytes or an installer. Import never fetches a missing profile and must not treat
...
5. **Honest status/recovery:** source acceptance without a browser, parse failure
   ...
   View previous, profile revocation during a live load, scripts-disabled open,
   source copy/download and confirmed unsent repair drafts.
...
values are proposed admission ceilings above, to be reviewed with this written
spec. User approval of this document precedes an implementation plan.
