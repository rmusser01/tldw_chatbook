...
**Wall-clock timeout + kill** — do **not** use `communicate()` (it reads to EOF into memory — the OOM bug). Instead a bounded reader (see Output caps) drives the run; on the wall-clock deadline (default 60 s, ≤ the confirm/budget envelope) or on exceeding an output cap, kill the whole process group with `os.killpg(os.getpgid(pid), SIGKILL)` (valid because `start_new_session=True` gave the child its own session/group — the repo's one existing `killpg` at `audio_player.py:404` lacks this and targets a shared group), reap with a short bounded `wait`, and return `timed_out=True` (or `output_capped=True`) with the partial output already read.

**Output caps (bounded, streaming — the OOM fix)** — read stdout and stderr concurrently (a reader thread per stream, or non-blocking select) accumulating into a per-stream buffer capped at a byte limit (default 64 KiB each). Once a stream hits its cap, stop reading it and set `truncated_stdout`/`truncated_stderr`; if both are capped (or the process is still producing past the wall clock), kill the group. Memory use is bounded to `~2 × cap` regardless of how much the child tries to emit — the child cannot OOM the app. Decode the capped bytes with `errors="replace"` for the `ToolResult`.
