...
| Tool scope | **Any trusted + allowed skill** by `skill_name` — not name-bound to the turn's `$`-mentions. Gated purely by policy + per-run trust re-verify + per-run confirm / per-skill grant. **Callable by any agent in the run tree** (primary, skill-forks, and spawned subagents) — caller identity is not a gate, because the confirm card + per-skill grant already governs each run. No new agent-kind machinery. |
...
- **AgentService.** New ctor kwarg `run_skill_script_tool`. **Availability: wired unconditionally** — the schema is pinned in `_run_one` (`agent_service.py:361-372`) and the LoopDeps callable set (`:609-641`) whenever `run_skill_script_tool is not None`, with **no `agent_kind` gate**. Because `_run_one` recurses on the same shared `AgentService` instance, this intentionally reaches every depth — primary, skill-forks, and spawned subagents alike. (Verified: a skill fork and a `spawn_subagent` child are indistinguishable at this site — both `agent_kind == AGENT_KIND_SUBAGENT` — so "forks but not subagents" is not expressible without new machinery; the user chose all-agents, making the shared-instance reach the desired behavior rather than a leak.) This differs from `install_skill`, which *is* `agent_kind == PRIMARY`-gated. The real gate for every call is policy + trust + confirm/grant, applied per-run in the closure and service, not at the schema-pin site.
...
The runner is deliberately conservative and **honest about its limits**. It borrows the *limit values* from `Evals/specialized_runners.py` (PR #851) but deliberately **diverges from its mechanism** in three ways that spec-review (two code-verification scouts) proved necessary: the Evals runner (a) uses `subprocess.run(capture_output=True)`, which buffers *all* output into memory with no byte cap (OOM under a hostile script), (b) sets no process group so grandchildren orphan on timeout, and (c) applies limits by prepending source lines to a *generated Python file* (works only because it owns the file top — useless for arbitrary interpreters/binaries). This runner fixes all three.

**Process launch**
- `subprocess.Popen(trampoline_argv, cwd=<fresh scratch>, env=<scrubbed>, stdin=DEVNULL, stdout=PIPE, stderr=PIPE, shell=False, start_new_session=True)` — never `shell=True`; argv always a list; **bytes pipes** (no `text=True`) so binary output can't raise `UnicodeDecodeError` — we decode with `errors="replace"` only at the truncation boundary.
- **No `preexec_fn`.** The Agents runtime runs synchronously on a worker thread and bridges tool calls via `asyncio.run` (`agent_service.py:6,680`; `tool_catalog.py:168`) — a multi-threaded fork/exec context where an arbitrary-Python `preexec_fn` can deadlock (a documented CPython caveat; the repo has zero `preexec_fn`/`setsid` precedent). Instead: `start_new_session=True` performs the session/process-group setup via CPython's safe internal C fork helper (not arbitrary Python), and resource limits are applied by a **Python trampoline** — the child argv is `[sys.executable, "-c", <prelude>, <mechanism-target-argv…>]` where `<prelude>` calls `resource.setrlimit(...)` for each limit then `os.execv`s the real interpreter/binary (§4.2's target argv). Because the prelude runs in a fresh, single-threaded Python process *after* exec, it is thread-safe, and `execv` replaces it so no extra process lingers. Uniform for both mechanisms: a text-script's `[interpreter, path, *args]` and a direct-exec's `[path, *args]` are both wrapped as the trampoline's target.
- **Scratch cwd**: a fresh `tempfile.mkdtemp(dir=<root>)` per run (root = OS temp dir, or a `[skills] script_scratch_root` if configured — reachable via the correct `get_cli_setting("skills", "script_scratch_root", default)` 3-arg form, **not** the section-dict form that hits the known unreachable-section bug at `config.py:3965`; `Utils/secure_temp_files.py` is the existing `dir=`-aware precedent). **Never the skill directory** — a script cannot mutate its own trusted bundle (which would silently invalidate its fingerprints) and cannot read sibling skills. Deleted in a `finally` (best-effort; `ignore_errors=True`).
- **Env scrub**: start empty, set only `PATH="/usr/bin:/bin"`, `HOME=<scratch>`, `TMPDIR=<scratch>`, `LANG`/`LC_ALL` passthrough for text encoding, and nothing else. **No API keys, no user env** — the app's provider keys never reach a skill script. (A short allowlist of innocuous vars may be added if a real skill needs it; default deny.) Interpreter lookup for the text-script mechanism uses `shutil.which(name, path="/usr/bin:/bin")` — resolved against the *scrubbed* PATH, never `os.environ` (no repo precedent passes `path=`; this is new and deliberate). An interpreter that doesn't resolve there makes that mechanism unavailable with a clear error, rather than falling back to the user environment.

**Resource limits** — applied by the trampoline prelude (fresh single-threaded child, pre-`execv`) so they bound *any* interpreter/binary:
...
- `RLIMIT_AS` (address space / memory) — **best-effort**, wrapped in `try/except (ValueError, OSError)`: on Darwin `setrlimit(RLIMIT_AS, ...)` raises `ValueError` and is skipped (documented; `specialized_runners.py:40-47`). A one-time `sandbox_warning` records that memory is uncapped on this platform.
- **`RLIMIT_NPROC` deliberately NOT set.** It is enforced **per real-UID across the whole session, not per process tree** — an absolute cap (Evals uses `(10,10)`) makes the child's first `fork()`/thread-spawn fail with `EAGAIN` on any desktop where the user already exceeds the cap (silently swallowed on macOS, breaking legitimate scripts on Linux). Fork-bombs are instead bounded by process-group kill + wall-clock + CPU limits. Documented as a residual (a runaway forker is capped in time, not in fan-out).

**Wall-clock timeout + kill** — do **not** use `communicate()` (it reads to EOF into memory — the OOM bug). Instead bounded reader threads (see Output caps) drive the run; on the wall-clock deadline (default 60 s, ≤ the confirm/budget envelope) kill the whole process group with `os.killpg(os.getpgid(pid), SIGKILL)` (valid because `start_new_session=True` gave the child its own session/group — the repo's one existing `killpg` at `audio_player.py:404` lacks this and targets a shared group), reap with a short bounded `wait`, and return `timed_out=True` with the partial output already retained.

**Output caps (bounded, streaming — the OOM fix)** — read stdout and stderr concurrently (one reader thread per stream), **retaining** at most a byte cap each (default 64 KiB) while **continuing to drain and discard** anything beyond it. Draining past the cap is load-bearing: if a reader simply stopped, the child would block forever writing into a pipe nobody empties, deadlocking until the wall clock. Draining instead means memory stays bounded at `~2 × cap` (the child cannot OOM the app) *and* a chatty-but-finite script still exits normally with its real exit code, while a script that never stops is bounded by the wall clock. `truncated_stdout`/`truncated_stderr`/`output_capped` record that output was clipped. Decode the retained bytes with `errors="replace"` for the `ToolResult`.
...
- **Runner unit tests** (`Skills_Interop/skill_script_runner.py`) — real temp scripts: happy stdout; non-zero exit; wall-clock timeout → **process-group** kill (spawn a child that itself forks a `sleep` grandchild, assert `timed_out` and that the grandchild's pgid is gone — proving `start_new_session` + `killpg` reaps the tree, not just the direct child); **bounded-output OOM safety** (a child in a tight `while True: write(64KiB)` loop → assert the run returns with `truncated`/`output_capped` set and resident buffers stay ~2×cap, i.e. the reader stopped and killed rather than buffering to EOF); env scrub (script prints `os.environ` → assert no API key / minimal PATH); scratch cwd is a temp dir, not the skill dir; RLIMIT_CPU/NOFILE/FSIZE applied (a script exceeding each hits the limit) and `RLIMIT_AS` best-effort with a `sandbox_warning` on Darwin; **no `RLIMIT_NPROC`** (a script that forks a modest number of children is not spuriously killed by a per-UID cap). Interpreter-not-on-scrubbed-PATH → clear error. Trampoline applies limits to a **direct-exec binary** target too (not just Python).
...
- **Silent runs under an "always allow" grant, from any agent** — once a skill is granted "always allow this skill", its scripts run without a card, and (per the all-agents caller scope) *any* agent in the run tree — including a subagent processing untrusted/injected content — can trigger those runs with runtime-chosen args. The grant is a deliberate per-skill decision about *that skill's reviewed scripts*, and the same silent-run exposure exists for the primary agent, so caller identity does not widen it; but a user who wants a run to always be re-confirmed should simply not grant "always allow" for that skill. Args remain runtime input in all cases — the sandbox (scrubbed env, scratch cwd) is the compensating control for hostile args, not review.
