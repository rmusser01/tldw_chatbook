...
- Give each task a stable session-local ID and positive integer version.
...
- a monotonically increasing integer ID counter (IDs are rendered as decimal
  strings and are never reused during the session); and
...
is bounded. IDs and versions are generated by the store, never accepted on
create, and Python's unbounded integer prevents counter wraparound. A task ID
is a non-empty canonical decimal string. A version is an exact built-in integer
greater than or equal to 1; booleans are invalid even though Python otherwise
treats `bool` as an `int` subclass. `content` and `activeForm` must also encode
as strict UTF-8; lone surrogates are rejected before any state change.
...
validates exact types, unique positive canonical IDs, task/version/text bounds,
the live-task cap, the one-`in_progress` invariant, and `next_id` greater than
every issued ID. A missing snapshot is the legacy shape and restores an empty
store. An invalid snapshot also restores an empty store and emits one fixed
payload-free warning rather than crashing navigation or partially importing
state. A valid navigation round trip preserves deleted-ID high-water state, so
IDs are not reused after returning to Console.
...
schema-mediated model invocation obey the same contract.
...
- `id` must be a canonical decimal string. `expected_version` must be an exact
  built-in integer at least 1; a boolean is rejected.
...
Input: `{ "id": "1" }`. `id` follows the same canonical-decimal rule as
`todo_update`, and unknown properties are rejected.
...
previous page. `cursor` must be a canonical decimal task ID when present;
...
A cursor is any canonical positive decimal string (`"1"`, not `"0"`, `"01"`,
`"+1"`, or an integer). It is an exclusive numeric lower bound and does not
need to identify a currently live task. A previously returned cursor therefore
remains valid after that task is deleted. A never-issued or future cursor is
also accepted and returns an empty terminal page until tasks with greater IDs
exist. Pagination is live rather than snapshot-isolated: tasks created after an
earlier page may appear once on a later page, updated tasks keep their position,
and deleted tasks disappear. Records that remain live throughout traversal are
...
- concurrent creates receive distinct IDs and both remain present when
  capacity permits; at 49 live tasks, exactly one of two racing creates may
  succeed and the other receives the fixed capacity error;
...
- failed validation, not-found, or version conflict performs no mutation and
  emits no transcript callback.
...
Errors are short, deterministic, and non-reflective where caller content could
contain private data. Required cases include invalid input, task limit reached,
task not found, version conflict, and the one-`in_progress` invariant. IDs and
integer versions may appear in errors; task content and `activeForm` do not.
