...
The design follows ADR-026 unchanged. It does not amend that accepted ADR and
...
- `LLMManagementWindow` owns an `active_view` reactive and handles
  `.llm-nav-button` locally.
...
`LLMManagementWindow` has one button handler: navigation. Start, stop, browse,
download, and related buttons exist in its production composition, while
their handler maps are reachable only from the unused app dispatcher.
...
Navigation itself remains owned by `LLMManagementWindow.active_view`; the root
`llm_active_view` and `llm_nav_events` path are deleted.
...
- Preserve fresh-screen navigation, current snapshot precedence, and
  memory-only handoff behavior from ADR-026.
...
`PendingHandoffStore` channel. It preserves ADR-026 semantics:
...
| LLM server process handles and server-worker registry | Explicitly retained for this tranche | Service/process lifecycle is excluded by ADR-026; destination event registration may depend on them explicitly without moving their ownership. |
| Duplicate `_wire_writing_services()` and `_wire_chat_conversation_services()` calls | Explicitly excluded under ADR-026 | Verified construction/lifecycle defect, but not a destination reactive or worker-context handoff. It requires its own service-lifecycle design. |
...
- LLM navigation and one safe/fault-injected action route through the real
  `LLMManagementWindow` exactly once;
