...
- **Overall deadline**: `deep_search_timeout_s` (default 300 — amended during execution: shipped default is **240**, so it undercuts the agent runtime's 300s `max_tool_call_seconds` tool-call ceiling and a deadline-hit run can still return its partial synthesis before the runtime kills the call outright) — checked between phases; on expiry mid-relevance, sets `cancel_event` and synthesizes from partial analysis. Phase-1 sync searches are bounded by the new per-request timeouts; the residual "an in-flight search may overrun the deadline by up to one request timeout" is documented, not hidden.
