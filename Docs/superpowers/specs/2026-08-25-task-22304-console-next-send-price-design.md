...
**ADR required:** no  
**ADR path:** N/A  
**Reason:** The feature derives ephemeral presentation state from existing session settings, pricing catalog data, conversation state, staged context, and composer content. It introduces no storage, schema, ownership, service contract, security boundary, dependency, or long-lived application-structure decision.
