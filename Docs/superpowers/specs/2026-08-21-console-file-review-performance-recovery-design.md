...
- ADR-077 is accepted before this slice changes the existing default.
...
- Create ADR-077 for Change Review opt-in state, shadow-content ownership and
...
- The atomic-confinement task writes its own ADR for the cross-platform
  execution lease. ADR-077 references but does not duplicate that runtime and
  security boundary.
