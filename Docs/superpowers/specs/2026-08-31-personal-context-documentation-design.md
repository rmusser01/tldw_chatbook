...
- Explain how a standalone Chatbook profile and a server profile become one logical profile only after successful reviewed first linking, identity adoption, and publication of the snapshot resulting from the user-approved content-free reconciliation plan.
...
- Reviewed first-link reconciliation and publication are available. Before approval, the UI shows content-free identifiers, versions, counts, outcomes, and choices rather than profile values. A standalone Chatbook profile and an existing server profile remain separate until the user approves that content-free plan and link completion succeeds. Successful completion publishes the resulting eligible snapshot and establishes matching canonical identities and bytes on both peers.
...
- Fixed mode generates questions locally and makes no model call; its encrypted draft and transcript objects are peer-local. Adaptive mode uses the configured default Console provider without tools. Each request sends the interview audience, allowed topics, attempt number, eligible agent-visible records from the exact selected scope, and—after the first answer—all prior answered turns including raw answer text. The interview screen shows the actual provider and model only after the first provider response completes, before answer entry becomes available. In either mode, approved answer text can be materialized into ordinary canonical record payloads governed by the selected record visibility and syncability. Guides must state this materialization and the adaptive disclosure ordering, explain that adaptive answers leave the app, and avoid implying that provider/model disclosure precedes the first egress.
...
- First-link bootstrap exchanges authentication/capability, device-registration and public-key, display, schema/quota, and purge-generation metadata before the user approves reconciliation. It also downloads the server's current Sync-eligible canonical snapshot, including record and proposal content, and holds that snapshot transiently in memory to build the reconciliation plan. The durable pre-approval link state and visible plan remain content-free. No local profile record or proposal content is uploaded before approval. Guides must disclose the metadata exchange and remote-content download without implying that approval gates all network activity or that the UI displays profile values.
- Chatbook currently exposes **Remove local profile**. It destroys canonical `PersonalContextRepository` content, including that repository's encrypted objects and heads, canonical `encrypted_outbox`, quarantine rows, runtime policy, local mappings, Undo data, and local record-link metadata. It neither deletes the server copy nor unregisters the device. It also does not clean separate `SyncStateRepository` state: `personal_context_link_state`, `sync_profile_state`, `sync_v2_local_outbox` staged encrypted envelopes, remote heads/cursors, and conflict reviews/receipts may remain. Dataset staging keys may remain too. Documentation must not describe this as a complete device-local purge. The encrypted recovery export includes canonical local heads, including device-only records, but no shipped UI or production caller imports/restores that export. Canonical profile-key deletion occurs after repository rows are removed and can fail; in that state Settings must direct the user to **Finish secure removal**. Chatbook does not expose a **Delete everywhere** action.
...
2. Short common workflows for:
   ...
   - exporting and removing the canonical local repository copy, including residual separate Sync state/staging-key limits, the absence of a recovery-import control, and the **Finish secure removal** recovery state;
...
- First-link bootstrap sequencing, metadata exchanged before approval, transient download of the server's eligible content snapshot, and the content-free durable/review surfaces.
- Server URL validation, runtime TLS trust policy, and the different **Test Connection** verification path.
- Local repository removal boundaries, residual separate Sync state and dataset staging keys, post-row canonical key cleanup, recovery-export contents, and the absent production restore caller.
...
This is an object-boundary distinction, not a promise that approved answer content cannot leave Chatbook. The encrypted draft and transcript remain peer-local objects. After user review, saved answer text is materialized as the value or outcome in an ordinary canonical profile record. Its selected `agent_visible`/`user_only` and `syncable`/`device_only` controls then apply normally: an eligible record in the snapshot resulting from the user-approved content-free reconciliation plan is published, an eligible later mutation creates the currently undrained encrypted outbox entry, and a device-only record remains local.
...
1. A user creates or edits context directly, or chooses an optional fixed or adaptive interview. Fixed mode generates questions locally without a provider call, and its draft/transcript objects remain peer-local. Adaptive mode calls the configured default Console provider without tools and sends the bounded request data described in the current implementation boundary; after the first answer, that includes all prior raw answer text.
2. The adaptive screen reveals the actual provider/model after the first question call finishes and before answer entry is enabled. This is a current disclosure-timing limitation, not proof of pre-egress consent. Interview output always goes through user review. When the user saves approved interview changes, answer content becomes ordinary canonical record payloads and follows each record's chosen visibility and syncability; inferred or newly learned agent context becomes a proposal.
...
6. Before approval, bootstrap exchanges connection, capability, device/key, display, schema/quota, and purge-generation metadata and downloads the server's current Sync-eligible canonical snapshot, including record and proposal content, into transient memory. Durable pre-approval state and the review plan are content-free. The UI presents identifiers, versions, counts, outcomes, and choices—not profile values. Approval of that plan then permits upload of local eligible content, publication of the resulting snapshot, and successful first-link convergence.
...
| First-link semantic collision | Distinct local and server record identities describe the same scope/kind/key during reviewed linking. | Use the presented content-free IDs, versions, outcomes, and local/server choices to select which canonical lineage remains active. The UI does not display profile values; do not tell users to compare content there. |
...
| Local removal incomplete or residual state | Canonical `PersonalContextRepository` rows were removed, but canonical profile-key cleanup may have failed; separate Sync state and dataset staging keys may also remain. | Use **Finish secure removal** for canonical profile-key cleanup. Do not imply that this clears `SyncStateRepository`, dataset staging keys, the server copy, or device registration, or that the recovery export can currently be restored in the app. |
...
- Confirm adaptive-interview guidance names every transmitted field, raw prior-answer egress, no-tools execution, default Console provider selection, and the actual provider/model display only after the first request completes. Confirm fixed question generation and draft/transcript storage are peer-local, while approved answer content may become an ordinary canonical record governed by record visibility and syncability.
...
- Confirm first-link guidance discloses both pre-approval bootstrap metadata and transient download of the server's Sync-eligible record/proposal content, while stating that durable state and the review UI are content-free and local record/proposal content is not uploaded until approval.
- Confirm removal guidance distinguishes deleted canonical `PersonalContextRepository` content from residual separate `SyncStateRepository` rows and dataset staging keys, says the server copy and registration remain, states that recovery import is not shipped, and limits **Finish secure removal** to retrying canonical profile-key cleanup.
...
- The guides agree that successful reviewed first linking establishes equal canonical identities and bytes for the eligible snapshot resulting from the user-approved content-free reconciliation plan, while later mutations do not currently converge.
