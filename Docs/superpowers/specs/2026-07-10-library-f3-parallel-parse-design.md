...
`parse_local_file_for_ingest(path, options) -> dict`, extracted from the pre-DB half of `ingest_local_file` (everything before the `add_media_with_keywords` call). The pool's entry point lives in a NEW dedicated light module `Local_Ingestion/ingest_parse_worker.py` whose module scope imports nothing heavy — every parsing import (including `local_file_ingestion` itself) is deferred into the function body.

**Import-chain slimming (prerequisite, in scope).** Importing `local_file_ingestion` measured at ~6.8s / ~5,900 modules — root-caused to two eager edges, both fixable: (1) `OCR_Backends` imports docling (→ torch/transformers/torchvision, ~4.1s / 5,953 modules alone) at REGISTRATION time, and `PDF_Processing_Lib:41` / `Image_Processing_Lib:53` import `ocr_manager` at module scope — backends must register lazily (docling imports on the first actual OCR call) and the `ocr_manager` imports move into the functions that use OCR; (2) `Summarization_General_Lib` (~0.8s / 2,404 modules: nltk/scipy/sklearn/pandas) is imported at module scope by the Document/PDF processors for `analyze` — defer into the analyze call sites (paid only when `perform_analysis` is on). Acceptance: `import tldw_chatbook.Local_Ingestion.local_file_ingestion` completes in ≤1.5s with no `torch`/`docling`/`nltk` in `sys.modules`, guarded by a subprocess-based regression test. Worker first-parse warmup drops to ~1s + the per-type parser's own import; the main app's startup sheds the same dead weight (it imports this chain eagerly at boot). When `perform_analysis` is enabled, analysis (LLM summarization) runs inside the per-type processors — i.e., IN the worker process, which therefore makes network calls using API keys loaded from the same config file; this is existing behavior relocated, not new capability. Returns a picklable payload (content, title, media type, metadata, keywords, analysis inputs). The worker wrapper catches ALL exceptions and returns a structured result `{ok: bool, payload | error: str, permanent: bool}` — never raises across the process boundary (no exception-pickling surprises), and permanent-vs-retryable classification (F1b M4: unsupported type, missing file) happens inside the worker where the real exception type is available. `ingest_local_file` becomes parse + persist composed, so `batch_ingest_files`, `quick_ingest`, and the server path are unchanged.
