...
| Providers & Models | Yes | `chat_defaults.provider`, `chat_defaults.model`, `api_settings.<provider>.endpoint`, `api_settings.<provider>.api_key_env_var` | Settings persisted defaults; Console runtime selection | Provider identity, model, endpoint, and credential source are shared with Console. Sampling and transport defaults route to Console Defaults. Console owns active chat/run state. |
