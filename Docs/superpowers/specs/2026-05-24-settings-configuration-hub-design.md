...
| Providers & Models | Yes | `chat_defaults.provider`, `chat_defaults.model`, `api_settings.<provider>.endpoint`, credential source | Settings persisted defaults; Console runtime selection | Provider identity, model, endpoint, and credential source are shared with Console. Sampling and transport defaults belong to Console Behavior. |
...
| Console Behavior | Yes | `[console]` defaults and future chat sampling defaults | Console | Settings owns global Console defaults; Console owns active chat/run state. |
