...
Direct MCP invocation annotates explicit user deny and permission-Off, not ask-with-no-callback despite shared refusal copy. Local invocation annotates only `LocalToolInvocationReason.APPROVAL_REFUSED` when authoritative, and `PERMISSION_OFF`. Existing `final_gate/approval_consumed/reason_code` facts identify approved execution and can preserve approved provenance when execution subsequently fails. The builtin gate must expose a detailed decision from its existing single check while retaining its string/None `check()` API; its provider consumes this fact without a second policy read. Virtual CLI review deliberately returns proceed even for denied stamps, so the invocation result must carry its actual Off/explicit-deny fact. Raw-shell provider Off checks also bypass review and must carry the fact. Preserve unanswered metadata through existing run/call-keyed stamp storage, pop/clear and nested scopes, and inspect direct callback maps before defaulted choices lose that distinction. A small pure Agents/approval_provenance.py helper holds immutable raw decision/fact stamps and selected-key extraction without importing Chat. Detailed stamp accessors retain current raw-string APIs. Unanswered metadata follows the selected map key, including call-ID-to-name fallback; name-wide deny aggregation remains conservative when contributing rows include unanswered denials, without changing the selected approval scope. Only owners that already accept allow_matching may label it approved. Malformed callbacks are not denials. Opaque later raw-runtime admission refusals supply no authoritative reason and remain unannotated. Other tool providers stay conservative when no authoritative fact exists. No parsing of canonical refusal strings.
