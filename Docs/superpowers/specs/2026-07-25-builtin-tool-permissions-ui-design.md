...
### 3. Appending the rows without touching the MCP path

**As shipped, this is not a merge.** `MCPWorkbench` gains a built-in section (`_builtin_permission_matrix_rows`) built from the above, but it is a **sibling** to `_build_permission_rows()`, not an input to it: `_sync_permissions_mode()` calls `_build_permission_rows()` first to get the MCP-only `rows`, then calls `_builtin_permission_matrix_rows()` separately and **appends** its `PermRow`s (`rows = rows + self._builtin_permission_matrix_rows(...)`). The built-in section is never folded into the `effective` map, never added to the server ordering `_build_permission_rows()` walks, and its tools are never added to `_last_hub_tools` (the MCP catalog list). `effective_tool_states()` and `resolve_effective_state` are **not** modified — MCP resolution stays byte-identical.

An earlier draft of this task considered merging the built-in section into the `effective` map *before* `_build_permission_rows()` runs, on the theory that it would reduce plumbing by letting one code path render both namespaces. That draft was not built, deliberately: merging built-in rows into the same map and catalog list that `effective_tool_states()`/`resolve_effective_state` and `_last_hub_tools`-keyed lookups (audit lookups, the Test panel, tool-id resolution elsewhere in this file) all read from would put `agent:builtin` entries somewhere a later refactor could plausibly wire into `effective_tool_states()` "for consistency," or into an MCP-catalog-keyed lookup that assumes every entry came from a live MCP server. Appending post-hoc, from a method that hard-codes `BUILTIN_TOOL_SERVER_KEY` and takes no `server_key` parameter to branch on, makes that structurally impossible rather than merely avoided by convention: there is no map or list a built-in row is ever a member of that MCP code also reads. This is the same shape of guarantee Constraint 1 asks for at the resolver level (`resolve_builtin_state` vs. `resolve_effective_state` never sharing a code path) — here it is enforced one layer up, at row construction, so the CheetahClaws failure mode (a namespace falling through logic written for a different one) cannot recur even if a future edit touches `_build_permission_rows()` without touching this method.
