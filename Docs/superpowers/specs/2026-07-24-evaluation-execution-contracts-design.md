...
Reconciled: 2026-07-27 against the current-dev eval-console retirement
ADR:
[ADR-031](../../../backlog/decisions/031-bounded-evaluation-and-tool-worker-execution.md)
Backlog:
[TASK-902](../../../backlog/tasks/task-902%20-%20Enforce-evaluation-execution-and-run-state-contracts.md)
...
The A/B wrapper retains the same three-argument shape. Tests cover both
synchronous and asynchronous callbacks and assert that no coroutine is left
unawaited. A later current-dev tranche retired the unreachable gen-2
`Event_Handlers/eval_events.py` cluster, so there is no live UI adapter to
modify or test here and this work must not recreate one. Any future full-app
consumer must accept `(completed, total, result)` and perform same-loop updates;
thread-safe marshalling remains appropriate only for actual thread workers.
...
Future consumers can await the public method and use the ID delivered by
`run_started_callback`; no separately generated placeholder UUID is needed.
...
- A/B callback-adapter signature and awaited callback updates;
...
`eval_orchestrator.py`, `ab_testing.py`, `Evals_DB.py`, and the existing Evals
tests. The former `eval_events.py` adapter was re-reviewed only to reconcile
its later deletion; it is not a live implementation target. The second pass
resolved:
