...
**Detection.** A response is a PDF iff the content-type main type is `application/pdf` **or** the body's first chunk starts with `%PDF-` (no valid HTML/text starts with those bytes; servers mislabel PDFs as `application/octet-stream`, `text/html`, or nothing constantly, so the sniff wins over the declared type).
...
**Cache + rate limiting** are inherited: the cached value is the extracted text, keyed like every other fetch. **Cache-key fix (v1 quirk, corrected here because PDF text now flows through it):** the cache is currently keyed by URL alone, so a fetch at `max_bytes=100` poisons a later full-cap call with 100 bytes of text. Key becomes `(url, effective_max_bytes)`.
...
1. Queue starts at `(url, depth 0)`; visited set holds normalized URLs (fragment stripped, host lowercased, query kept, non-http(s) schemes dropped).
2. Per dequeued URL: egress guard (`_validate_hop`) → per-domain rate limit → GET via `_fetch_once` semantics with per-hop redirect validation exactly like `web_fetch`. Both the requested and the final URL are marked visited.
3. HTML responses: extract `<title>` (regex, entities unescaped), full text via `_extract_text` (trafilatura with tag-strip fallback), excerpt = first 200 chars; collect `<a href>` links via `HTMLParser`, resolve with `urljoin`, keep same-host only, enqueue at depth+1 if depth allows.
...
- **MCP server** (`MCP/server.py`): `web_crawl` registered following the v1 pattern (nested async tool, docstring-first-line description, `{"error": ...}` envelope). Over stdio the operator is the user, so no interactive approval server-side — v1's recorded ADR posture; budgets + guard are the enforcement. `web_fetch`'s PDF support rides along with no MCP change.
- **No new config.** Budgets and ceilings are module constants like v1's `FETCH_*`; a config surface can come later if anyone asks.
...
- **Crawl:** a fixture site as a MockTransport routing table — link graph with depth >5, off-host links, `www.`-variant links, private-address links (guard refusal counted as blocked), a redirect-to-off-host page (listed, not expanded), a redirect-into-private-space page (blocked), non-HTML nodes (listed with marker, budget consumed), a hanging route (deadline stop, monkeypatched clock), page/depth budget stops, visited dedup across fragments, cache warm-write asserted (`web_fetch` after crawl hits cache without a new request), sitemap `urlset` + one-level `sitemapindex`, sitemap URL off-host filtered, footer text exact per stop reason, total/block byte caps.
- **Registration:** schema/handler wiring for `web_crawl` in the provider spec list; MCP registration smoke, matching how v1's registration is covered.
