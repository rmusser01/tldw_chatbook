...
- PDF → read ceiling becomes `PDF_MAX_BYTES = 20 * 1024 * 1024` for this response, regardless of the caller's `max_bytes` (when pymupdf is installed; when absent the ordinary `max_bytes` cap applies and the fetch is refused with `[missing-dep]` after download). One request; no re-fetch, no second rate-limit hit.
...
Internal constants (not parameters — YAGNI): `CRAWL_DEADLINE_SECONDS = 120.0` wall-clock bound, `CRAWL_PAGE_TIMEOUT_SECONDS = 10.0` per-page HTTP timeout (a single hung page must not eat the deadline; `web_fetch`'s own 30 s stays as-is), `CRAWL_EXCERPT_MAX_CHARS = 200`, per-page read cap = `FETCH_MAX_BYTES` (1 MiB), `CRAWL_RESULT_MAX_BYTES = 24 * 1024` total / `CRAWL_BLOCK_MAX_BYTES = 1024` per page block (byte caps in the `web_search` style, same truncate helper, omission marker when the total cap cuts the list), `CRAWL_MAX_LINKS_PER_PAGE = 500` — the frontier bound: a page stops contributing links to the `visited`/queue set once it has enqueued this many, so link spam on one page can't grow crawl memory unboundedly (`max_pages` alone only bounds fetch attempts, not the frontier itself).
...
**Sitemap mode.** `sitemap_url` is egress-guarded and fetched (cap `SITEMAP_MAX_BYTES = 5 MiB`); `<loc>` entries parsed from a `urlset`, or — for a `sitemapindex` — one level of child sitemaps is fetched (each guarded, within the deadline) until `max_pages` URLs are collected. Host rules, precisely: `sitemap_url` itself may live on any public host (sitemaps sometimes sit on a CDN); **child sitemaps must share `sitemap_url`'s host** (that's where indexes point); **page URLs must share `url`'s host** — `url` defines the crawl scope in both modes. Sitemap fetches themselves are discovery overhead and do **not** consume `max_pages`; the deadline bounds a pathological index (e.g. thousands of empty child sitemaps), and `SITEMAP_MAX_CHILDREN = 20` additionally caps the number of child sitemaps actually fetched from one index — off-host-only children never trip `max_pages`'s early exit, so a same-host index with dozens of children could otherwise pull tens of megabytes per child before the deadline caught it. Discovered URLs are then processed exactly like BFS step 2–4 **except no link expansion** — the sitemap *is* the discovery. Every URL still passes the same-host filter and the egress guard individually: a sitemap listing private/internal addresses must not become an SSRF path. **A child sitemap that fails is skipped, not fatal** — a fetch/redirect error, a deadline expiry mid-fetch, an oversized body, or a parse refusal (including a defusedxml hardening refusal) each skip that one child and count it; the total is surfaced in the footer as `N child sitemaps skipped` (see Output below) rather than silently reporting zero signal when every child in an index fails.
...
When one or more child sitemaps were skipped (sitemap mode only), the count parenthetical grows a third clause: `Crawled 0 pages (0 failed, 0 blocked; 2 child sitemaps skipped). Stopped: sitemap exhausted.`

An attempt can also spend its slot without producing a row: either a redirect landing on an already-listed final URL or a plain fetch of a URL an earlier redirect already listed — which occurs first depends on discovery order. Both are deduped against the same listed-URL set (BFS and sitemap mode share the fetch loop) and counted; when the count is nonzero the parenthetical grows a `N duplicate redirects skipped` clause, e.g. `Crawled 2 pages (0 failed, 0 blocked; 1 duplicate redirects skipped). Stopped: no more links within depth.` — otherwise these row-less attempts were invisible to the model even though they consumed part of the `max_pages` budget.

The status footer is always present and states the stop reason (`page budget reached` / `no more links within depth` / `deadline reached` / `sitemap exhausted` / `sitemap child budget reached` — the latter when the `SITEMAP_MAX_CHILDREN` cap left child sitemaps unfetched, so the footer doesn't claim the index was fully consulted when it wasn't). In sitemap mode specifically, the seed can also be cut short by the ordinary page budget — e.g. a plain `urlset` with more same-host URLs than `max_pages` (the common case at the default `max_pages=20`) — in which case the stop reason is `page budget reached`, not `sitemap exhausted`: `sitemap exhausted` is reserved for when every page URL and, for an index, every child sitemap within `SITEMAP_MAX_CHILDREN`, was actually considered. Priority when more than one condition holds: `deadline reached` > `sitemap child budget reached` > `page budget reached` > `sitemap exhausted`.
