...
- `parse_serper_results`: standardized items from `organic[]` — `title`, `url` ← `link`, `content` ← `snippet`; `metadata.snippet` ← `snippet`, `metadata.date_published` ← `date`, `metadata.relevance_score` ← `position` (inverse rank is Serper's only relevance signal; stored as-is with the field meaning "position" documented in the parser). `answerBox`/`knowledgeGraph` blocks are deliberately ignored (non-goal; organic web results only, like every sibling parser).
...
- Body: `{"query": {"searchType": "SEARCH_TYPE_COM", "queryText": query}, "folderId": <yandex_search_folder_id>, "responseFormat": "FORMAT_XML", "groupSpec": {"groupsOnPage": result_count}}` — `searchType` is required by the proto; `SEARCH_TYPE_COM` (international) is the default; both key AND folder id missing → the same clear `ValueError` path.
...
- Per engine: request shape pinned (exact URL, auth header name+value source, full JSON body incl. serper's lowercased `gl`, exa's `contents.highlights`, yandex's required `searchType`/`folderId`); parser output pinned against a realistic captured-shape fixture (yandex's fixture is a real-structure base64-encoded XML doc with `<hlword>` tags and a passage-less doc); missing-key `ValueError` copy; HTTP-error propagation into `perform_websearch`'s envelope; `process_web_search_results` end-to-end for each engine.
