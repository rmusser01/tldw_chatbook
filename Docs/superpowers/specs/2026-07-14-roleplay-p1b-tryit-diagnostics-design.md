...
- **Per-entry token cost** = `len(entry.content.split())`, inlined per-record (avoids a debug-log call per entry); the diagnostics **total** (`tokens_used`) uses one batched `calculate_token_usage(...)` call over budget survivors instead of summing per-candidate calls (log-noise avoidance).
