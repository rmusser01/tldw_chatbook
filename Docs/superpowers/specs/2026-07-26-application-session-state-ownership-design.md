...
Date: 2026-07-26
Status: Implemented and verified; TASK-643 through TASK-646 complete
ADR:
...
Chat context has an additional settlement boundary: TASK-577 already retired
the legacy tab container, so the production consumer transfers into the native
Console. The partially integrated consumer claims the revisioned slot but
settles success by assigning a removed raw app field, leaving the claim in
flight and blocking later replacements from being consumed.
...
- Make runtime publication persist-before-publish and resistant to stale async
  results.
- Keep runtime-policy persistence inside the ADR-029 private-file boundary and
...
- Make native Console Chat handoffs acknowledge only after screen-local
  ownership transfer and release the exact claim on failure or cancellation.
...
ADR-029 private-path rules apply before parsing:
...
Chat context is consumed by the registered native Console screen. A valid
character payload creates a character-bound Console session; other payloads
transfer into Console staged live work. The exact claim is acknowledged only
after either transfer succeeds. Failure or cancellation before transfer
releases the exact claim, allowing the current value to retry without
disturbing a newer replacement. There is no legacy tab creation or rollback
contract.
...
ADR-029 applies to all new diagnostics.
...
- Make runtime-policy persistence follow the effective config path and ADR-029
...
- Native Console Chat success acknowledges the exact claim after ownership
  transfer; failure and cancellation release it, and a newer replacement
  survives either settlement.
...
pytest -q Tests/ProductionApp Tests/RuntimePolicy Tests/State/test_pending_handoff_store.py Tests/test_application_state_ownership.py
```
...
superseded by ADR-033 and this specification. Their alternate refactored app
