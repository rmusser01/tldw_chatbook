"""Native keyboard mode and source-scope changes in Search/RAG.
...
def main():
    ...
    async def journey(pilot):
        ...
        try:
            ...
            query = "auditmodescope"
            title = "Mode and source scope review"
            media_id, _, _ = app.media_db.add_media_with_keywords(
                title=title,
                media_type="document",
                content=query + " An expired credential caused the incident.",
                keywords=[query],
            )
            ...
            async def toggle(selector):
                old = screen.query_one(selector, Button)
                assert screen.focused is old and painted(old)
                await pilot.press("enter")
                await wait_for(
                    lambda: (
                        screen.query_one(selector, Button) is not old
                        and screen.focused is screen.query_one(selector, Button)
                        and painted(screen.focused)
                    ),
                    "toggle focus restored",
                )
                await settle()
            ...
            for theme in ("textual-dark", "textual-light"):
                ...
                for size in ((170, 48), (80, 24)):
                    ...
                    stem = f"{theme}-{size[0]}"
                    start_searches, start_answers = len(completed), len(calls)
                    await submit(query)
                    rows = screen._rag_search_state.results
                    assert len(rows) == 1 and rows[0].source_id == str(media_id)
                    scope_selector = "#library-rag-scope-toggle-media"
                    screen.query_one(scope_selector, Button).focus()
                    await wait_for(lambda: painted(screen.focused), "scope focused")
                    await toggle(scope_selector)
                    assert (
                    ...
                    assert str(
                        screen.query_one(scope_selector, Button).label
                    ).startswith("☐")
                    assert screen._rag_search_state.results is rows
                    app.save_screenshot(stem + "-scope-off.svg", path=str(evidence))
                    # No focus setup here: the same Enter must reverse the toggle.
                    await toggle(scope_selector)
                    assert (
                        screen._library_rag_panel_state().scope.selected_source_types
                        == ("media",)
                    )
                    assert len(screen._library_rag_panel_state().results) == 1
                    assert not screen.query_one(
                        "#library-rag-run-query", Button
                    ).disabled
                    mode_selector = "#library-rag-mode-toggle"
                    screen.query_one(mode_selector, Button).focus()
                    await wait_for(lambda: painted(screen.focused), "mode focused")
                    for expected in ("rag", "search", "rag"):
                        await toggle(mode_selector)
                        assert screen._rag_search_state.mode == expected
                        assert not screen._rag_search_state.results
                        ...
                        assert screen._rag_search_state.history == (query,)
                    assert len(completed) == start_searches + 1
                    assert len(calls) == start_answers
                    app.save_screenshot(stem + "-rag-mode.svg", path=str(evidence))
                    await submit(query)
                    await wait_for(
                        lambda: (
                    ...
                    await settle()
                    assert len(completed) == start_searches + 2
                    assert len(calls) == start_answers + 1
                    assert completed[-1] == {
                        "query": query,
                        "scope": ("media",),
                        "mode": "rag",
                    ...
                    assert painted(screen.focused)
                    app.save_screenshot(stem + "-answered.svg", path=str(evidence))
                    screen.query_one(mode_selector, Button).focus()
                    await wait_for(lambda: painted(screen.focused), "mode focused")
                    await toggle(mode_selector)
                    assert screen._rag_search_state.mode == "search"
                    ...
                    result["cells"].append(
                        {
                            ...
                            "size": size,
                            "scope": "media",
                            "scope_off_blocks_run_and_hides_rows": True,
                            "scope_on_restores_rows_without_search": True,
                            "mode_resets_results_and_answer": True,
                            "repeated_enter_retains_visible_toggle_focus": True,
                            "query_history_retained": True,
                            "real_keyword_searches": 2,
                            "controlled_answers": 1,
                        }
                    )
                    record()
            ...
            assert before == after
            assert len(completed) == 8 and len(calls) == 4
            result.update(
                real_keyword_searches=8,
