"""TASK-32788 native compact source, wrapped Tool/State and permission authority.
...
def main():
    ...
    async def journey(pilot):
        ...
        try:
            ...
            def cell_paint(table, index):
                cell = table._get_cell_region(Coordinate(table.cursor_row, index))
            ...
            for theme in ("textual-dark", "textual-light"):
                for size in ((80, 24), (170, 48)):
                    ...
                    ) in compact(paint(purpose.content_region))
                    await focus("#mcp-rail-source")
                    source_label = app.screen.query_one(
                        "#mcp-rail-source SelectCurrent #label"
                    )
                    assert source_label.content_region.height == 1
                    assert "Local" in paint(source_label.content_region)
                    await capture(stem + "-source")
                    await focus("#mcp-perm-table")
                    target = max(
                        canvas._all_rows, key=lambda row: len(_tool_column_text(row))
                    )
                    ...
                    await pilot.wait_for_scheduled_animations()
                    await pilot.pause()
                    assert compact(_tool_column_text(target)) in compact(
                        cell_paint(table, 0)
                    )
                    assert target.state_label in cell_paint(table, 1)
                    await capture(stem + "-wrapped-state")
                    ...
                    # Tool and State share the initial viewport; tags can still scroll.
                    ...
                    result["cells"].append(
                        {
                            ...
                            "source_local_one_line": True,
                            "complete_tool_and_state_together": True,
