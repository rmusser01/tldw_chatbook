...
| Custom button/list rows | background, marker text, classes, text-style | named Console/Library rows, Console session tabs, legacy chat tabs, Settings category selections, and Search/RAG nav selections use readable focus/selected cues | each custom row needs explicit selector audit | source contract or mounted test |
