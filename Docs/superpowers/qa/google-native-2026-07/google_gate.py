"""task-266 live gate: Google/Gemini native tool-calls vs the REAL API.

Real ConsoleAgentBridge -> real ConsoleProviderGateway -> real chat_api_call
-> chat_with_google -> generativelanguage.googleapis.com (streaming ON —
exercises the whole-fragment functionCall emission live). The native-set
FLIP has not landed: this harness overrides NATIVE_TOOLS_PROVIDERS
in-process so the flip commit only happens after this evidence (AC #2).

The API key is read from the git-excluded repo-root google-api-key.txt
(gemini-api-key.txt as fallback) and never printed.
...
native_tools.NATIVE_TOOLS_PROVIDERS = frozenset(
    native_tools.NATIVE_TOOLS_PROVIDERS | {"google"})
...
MODEL = os.environ.get("GATE_GOOGLE_MODEL", "gemini-2.0-flash")
_REPO = Path("/Users/macbook-dev/Documents/GitHub/tldw_chatbook")
for _name in ("google-api-key.txt", "gemini-api-key.txt"):
    _p = _REPO / _name
    if _p.is_file():
        KEY = _p.read_text().strip()
        break
else:
    raise SystemExit("no google/gemini key file found at repo root")
...
def resolution() -> "ConsoleProviderResolution":
    """Build the ready Google resolution the gate cases share.

    Returns:
        A streaming-on resolution routed at execution_key "google".
    """
    return ConsoleProviderResolution(
        provider="google", base_url="", model=MODEL, ready=True,
        readiness_key="google", execution_key="google",
...
def main() -> int:
    ...
    """
    dbdir = os.environ.get("GATE_DBDIR") or tempfile.mkdtemp(prefix="google-gate-")
    ...
    second_turn_ok = len(gw.calls) >= 2  # functionResponse turn accepted (no 400)
    print("A native + no fence:", bool(native), "| round-trip done:", roundtrip,
          "| functionResponse turn accepted:", second_turn_ok)
