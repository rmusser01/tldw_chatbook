"""Native raw-response disclosure and success/validation/recovery through real stdio.
        ...
        try:
            ...
            for theme in ("textual-dark", "textual-light"):
                for width, height in ((80, 24), (170, 48)):
                    ...
                    title = await focus(
                        "#mcp-inspector-test-result-raw CollapsibleTitle"
                    )
                    region = title.region
                    title_paint = " ".join(
                    ...
                    assert "Raw response" in " ".join(title_paint.split()), title_paint
                    assert disclosure.collapsed
                    await capture(stem + "-title")
                    await pilot.press("enter")
                    await settle()
                    assert not disclosure.collapsed
                    raw_scroll = await focus("#mcp-inspector-test-result-raw-scroll")
                    await pilot.press("end")
                    await settle()
                    assert raw_scroll.scroll_y == raw_scroll.max_scroll_y > 0
                    raw_body = inspector.query_one(
                        "#mcp-inspector-test-result-raw-body", Static
                    )
                    assert "[bold] literal [/]" in str(raw_body.renderable)
                    assert not raw_body._render_markup
                    region = raw_scroll.region
                    tail = " ".join(
                        app.screen._compositor.render_strips()[y]
                        .crop(region.x, region.right)
                        .text.strip()
                    ...
                    assert "END" in tail, tail
                    await capture(stem + "-raw-end")
                    field = await focus("#mcp-schema-field-0")
                    field.value = ""
                    await focus("#mcp-inspector-test-run")
                    await pilot.press("enter")
                    ...
                    await wait_for(
                        lambda before=before: (
                            len(service.execution_log.read_recent(50)) == before + 2
                            and inspector._test_preview is not None
                        ),
                        "corrected execution",
                    )
                    ...
                    assert "CURRENT response" in str(raw_body.renderable)
                    assert "PREVIOUS" not in str(raw_body.renderable)
                    assert (
                        client.sessions["execution-review"].process is process
                        and process.returncode is None
                    )
                    await focus("#mcp-inspector-test-result-raw-scroll")
                    await capture(stem + "-corrected")
                    ...
                    result["cells"].append(
                        {
                            ...
                            "tail_paint": tail,
                            "validation_cleared_previous_output": True,
                            "invalid_attempt_not_executed": True,
                            "corrected_output_current": True,
            ...
            assert sum(row["method"] == "tools/call" for row in requests) == 8
