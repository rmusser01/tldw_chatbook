"""Qualify Library conversation source staging in a guarded native private profile.
...
def main():
    ...
    def seed():
        ...
        for label in (
            ...
            cid = str(uuid5(NAMESPACE_URL, "task-32706/" + label))
    ...
    async def journey(pilot):
        ...
        def check_context(fixture):
            ...
            expected = "Conversation staged: " + fixture["title"]
            assert payload["snippet"] == expected
            refs = payload["evidence_bundle"]["references"]
            assert len(refs) == 1
            assert (
                refs[0]["source_id"] == fixture["id"] and refs[0]["snippet"] == expected
            )
            assert all(
                m["content"] not in refs[0]["snippet"] for m in fixture["messages"]
            )
        ...
        try:
            ...
            for fixture, (theme, (width, height)) in zip(
                ...
                assert [s.id for s in store.sessions()] == session_ids
                assert store.session_draft(keeper) == draft
                # TASK-2502: the separate live-work entry point must also
                # consume on warm return, with no resident and with a prior
                # launch. These are synthetic boundary launches, not a RAG
                # retrieval or source-authority qualification.
                assert console._pending_console_launch_context is None
                live_work_revisions = []
                for ordinal in (1, 2):
                    if ordinal == 2:
                        await return_reader(fixture)
                    title = f"Warm source {name} {ordinal}"
                    channel = HandoffChannel.CONSOLE_LIVE_WORK
                    revision = app.pending_handoffs._slot_for(channel).revision + 1
                    app.open_console_for_live_work(
                        source="Source handoff audit",
                        title=title,
                        payload={"source_id": fixture["id"], "snippet": title},
                        status="staged",
                    )
                    await wait_for(
                        lambda channel=channel, revision=revision, title=title: (
                            app.screen is console
                            and app.pending_handoffs.exact_revision_status(
                                channel, revision
                            )
                            == "settled"
                            and console._pending_console_launch_context is not None
                            and console._pending_console_launch_context.title == title
                        ),
                        "warm live-work launch settled",
                    )
                    await wait_for(
                        lambda title=title: (
                            title
                            in painted(
                                console.query_one("#console-staged-evidence-row-0")
                            )
                        ),
                        "warm live-work title painted",
                    )
                    assert store.active_session_id == keeper
                    ...
                    assert console._console_composer_or_none().draft_text() == draft
                    assert not store.messages_for_session(keeper)
                    live_work_revisions.append(revision)
                await capture(name + "-warm")
                await press("#console-unstage-evidence")
                await wait_for(
                    lambda: console._pending_console_launch_context is None,
                    "warm launch unstaged",
                )
                result["steps"].append(
                    {
                        "theme": theme,
                        ...
                        "warm_live_work_revisions": live_work_revisions,
                        "warm_first_and_replacement_launch_painted": True,
                        "evidence_snippet": first_payload["snippet"],
                        "content_excerpt_absent_known_task": "TASK-2376",
                    }
                )
                record()
