...
def main():
    ...
    original_foreign = json.loads((evidence / "foreign-membership.json").read_text())
    assert [
        m for m in memberships if m["item_type"] == "conversation"
    ] == original_foreign
    media_before = json.loads((evidence / "media-before.json").read_text())
    assert media_before == json.loads((evidence / "media-after.json").read_text())
    media_memberships = [m for m in memberships if m["item_type"] == "media"]
    assert len(media_memberships) == 4
    assert {m["item_id"] for m in media_memberships} == {
        str(m["id"]) for m in media_before
    }
    assert all(m["workspace_id"] == result["workspace_id"] for m in media_memberships)
    with sqlite3.connect(f"file:{root / 'data/db/media.db'}?mode=ro", uri=True) as db:
        db.row_factory = sqlite3.Row
        assert db.execute("SELECT count(*) FROM Media").fetchone()[0] == 4
        for expected in media_before:
            actual = dict(
                db.execute(
                    "SELECT * FROM Media WHERE id=?", (expected["id"],)
                ).fetchone()
            )
            # get_media_by_id returns the stored Media row, not a rendered projection.
    ...
    receipt = {
        ...
        "only_fixture_media_and_original_foreign_memberships_remain": True,
        "all_media_fields_unchanged": True,
