"""TASK-32792 native Permissions table reflow and selected-row continuity.
...
def main():
    ...
    async def journey(pilot):
        ...
        def readable(table, tool):
            ...
            assert compact(_tool_column_text(tool)) in compact(cell_paint(table, 0))
        ...
        try:
            ...
            result["fixture_setup"] = (
                ...
                "No tool execution, permission writes or provider requests."
            )
            for theme in ("textual-dark", "textual-light"):
                for size in ((80, 24), (170, 48)):
                    ...
                    await pilot.press(*(["down"] * table.get_row_index(_row_key(tool))))
                    await settle()
                    target_key, _ = table.coordinate_to_cell_key((table.cursor_row, 0))
                    assert target_key.value == _row_key(tool)
                    readable(table, tool)
                    ...
                    await wait_for(
                        lambda tool=tool: (
                            workbench.query_one(MCPInspector)._current_permission_tool
                            is not None
                            and workbench.query_one(
                                MCPInspector
                            )._current_permission_tool.name
                            == tool.tool_name
                            and workbench.query_one(
                                MCPInspector
                            )._current_permission_profile_context
                            == canvas._profile_context
                        ),
                        "Exact row inspected",
                    )
                    field = await focus("#mcp-perm-filter-text")
                    assert isinstance(field, Input)
                    await pilot.press("home", "shift+end", "backspace", *tool.tool_name)
                    await wait_for(
                        lambda field=field, tool=tool: field.value == tool.tool_name,
                        "Tool name filter",
                    )
                    await focus("#mcp-perm-table")
                    assert (
                    ...
                    await focus("#mcp-perm-filter-text")
                    ...
                    result["cells"].append(
                        {
                            ...
                            "short_full_matrix_reflow": True,
                            "selected_row_visible_through_resize": True,
                            "exact_row_inspected": _row_key(tool),
                            "text_filter_and_current_context_usable": True,
