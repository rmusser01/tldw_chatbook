"""TASK-32789 native MCP Tools switch, filters and catalog access.
    ...
    async def journey(pilot):
        ...
        try:
            ...
            result["fixture_setup"] = (
                ...
                "use focus and keyboard. Toggle off/on restores the original setting. "
            ...
            for theme in ("textual-dark", "textual-light"):
                for size in ((80, 24), (170, 48)):
                    ...
                    for enabled in (False, True):
                        ...
                        assert compact(str(button.label)) in compact(
                            paint(button.content_region)
                        )
                        await pilot.press("enter")
                        await wait_for(
                            lambda enabled=enabled, button=button: (
                                get_cli_setting("console", "local_tools_enabled")
                                is enabled
                                and canvas._local_tools_enabled is enabled
                                and not workbench._reloading
                                and (": on" if enabled else ": off")
                                in str(button.label)
                            ),
                            "Local tools persisted " + str(enabled),
                        )
                        button = await focus("#mcp-tools-local-enabled")
                        assert compact(str(button.label)) in compact(
                            paint(button.content_region)
                    ...
                    await focus("#mcp-tools-table")
                    assert target_key in table.rows
                    cursor_visible(table)
                    ...
                    await capture(stem + "-server-filter")
                    ...
                    result["cells"].append(
                        {
                            "theme": theme,
                            "size": size,
                            "full_switch_labels": True,
                            "off_on_setting_persisted_and_restored": True,
                            "first_last_rows_visible": True,
