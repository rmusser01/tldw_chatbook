"""TASK-32765: complete Notes next-step paint in the real terminal app.
        ...
        try:
            ...
            for theme in ("textual-dark", "textual-light"):
                for size in ((170, 48), (80, 24)):
                    ...
                    await wait_for(
                        lambda: (
                            bool(app.screen.query("#library-notes-filter"))
                            and app.screen.focused is not None
                        ),
                        "Notes controls and initial focus",
                    )
                    await pilot.press("f6")
                    await wait_for(
                        lambda: (
                            app.screen.focused is not None
                            and app.screen.focused.is_attached
                            and app.screen.focused
                            in app.screen._compositor.visible_widgets
                        ),
                        "Visible Notes focus after F6",
                    )
                    ...
                    authority = canvas.query_one("#library-notes-authority")
                    region = authority.content_region.intersection(app.screen.region)
                    paint = " ".join(
                    ...
                    assert (
                        paint
                        == "Library notes · Ready · Next: Create a note or add from files."
                    )
                    result.setdefault("authority", []).append(
                        {
                            "theme": theme,
                            "size": size,
                            "region": list(region),
