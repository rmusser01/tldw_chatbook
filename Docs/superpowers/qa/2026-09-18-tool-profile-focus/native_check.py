"""TASK-32781 native Tool Profile import, removal and keyboard focus.
    ...
    async def journey(pilot):
        ...
        try:
            ...
            result["fixture_setup"] = (
                "One private source profile and real service-exported archive. All import, revise, cancel and removal actions use the real UI and service. No provider requests."
            )
            ...
            for theme in ("textual-dark", "textual-light"):
                for size in ((80, 24), (170, 48)):
                    ...
                    await capture(stem + "-options")
                    await inspect()
                    await focus("#tool-pack-import-unbound")
                    await capture(stem + "-review")
                    await press("#tool-pack-import-revise")
                    await wait_for(
                        lambda settings=settings, profile_id=profile_id: isinstance(
                            app.screen, ToolPackImportOptionsModal
                        ),
                        "Revised options",
                    )
                    assert (
                    ...
                    await wait_for(
                        lambda settings=settings, profile_id=profile_id: (
                            ...
                            and settings._tool_profiles_result == "Import cancelled"
                        ),
                        "Cancelled import",
                    )
                    ...
                    await expect_focus("#tool-profiles-import")
                    row = settings._tool_profiles_listing.by_id(profile_id)
                    assert row.origin == "imported" and row.binding_state == "unbound"
                    assert row.reference_counts == (0, 0) and row.removal_eligible
                    snapshot = store.read_snapshot_strict().payload["profiles"][
                        profile_id
                    ]
                    assert snapshot["profile_kind"] == "tool_pack_imported"
                    await capture(stem + "-imported")

                    remove = action_selector(settings, profile_id, "remove")
                    await press(remove)
                    await wait_for(
                        lambda settings=settings, profile_id=profile_id: isinstance(
                            app.screen, ConfirmationDialog
                        ),
                        "Removal confirmation",
                    )
                    await focus("#confirm-button")
                    await capture(stem + "-remove-review")
                    imported_bytes = store.path.read_bytes()
                    await pilot.press("escape")
                    await wait_for(
                        lambda settings=settings, profile_id=profile_id: (
                            app.screen is settings
                            and settings._tool_profiles_result == "Remove cancelled"
                        ),
                        "Cancelled removal",
                    )
                    old = await expect_focus(remove)
                    assert store.path.read_bytes() == imported_bytes

                    extra = f"a-extra-{len(result['cells'])}"
                    await asyncio.to_thread(store.ensure_profile, extra)
                    settings._request_tool_profiles_listing()
                    await listing_ready(settings, extra)
                    remove = action_selector(settings, profile_id, "remove")
                    await expect_focus(remove)
                    ...
                    result["cells"].append(
                        {
                            ...
                            "reordered_focus_preserved": True,
