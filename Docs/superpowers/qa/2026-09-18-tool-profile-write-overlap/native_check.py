"""TASK-32786 native Tool Profile write overlap and truthful completion.
    ...
    async def journey(pilot):
        ...
        try:
            ...
            result["fixture_setup"] = (
                "One private source profile and real service-exported archive. Import and removal use the real UI/service. A fixture thread holds the existing lifecycle mutation lock to delay real removal while another same-operation action is attempted. No service replacements or provider requests."
            )
                ...
                for size in ((80, 24), (170, 48)):
                    ...
                    await wait_for(
                        lambda settings=settings: (
                            "remove" in settings._tool_profile_writes
                        ),
                        "Removal admitted",
                    )
                    first = next(
                    ...
                    await press(remove)
                    assert app.screen is settings and not first.is_cancelled
                    assert (
                    ...
                    result["cells"].append(
                        {
                            ...
                            "repeat_action_kept_first_worker": True,
