"""TASK-32782 native Tool Profile removal refusal, recovery and keyboard focus.
    ...
    async def journey(pilot):
        ...
        try:
            ...
            result["fixture_setup"] = (
                "One private source profile and real service-exported archive. Import and removal use the real UI/service. An exact-profile runtime lease supplies a real in-use refusal before release and a user-confirmed retry. No provider requests."
            )
            ...
            for theme in ("textual-dark", "textual-light"):
                for size in ((80, 24), (170, 48)):
                    ...
                    # Acquire a real runtime lease after the row's action was reviewed.
                    # The actual removal authority must refuse; no service replacement.
                    with service.lifecycle.lease(profile_id):
                        await press("#confirm-button")
                        await wait_for(
                            lambda settings=settings: (
                                "active run" in settings._tool_profiles_result
                            ),
                            "In-use refusal",
                        )
                        await wait_for(
                            lambda settings=settings, profile_id=profile_id: (
                                not settings.query_one(ToolProfilesPanel)
                                .row(profile_id)
                                .removal_eligible
                            ),
                            "Refreshed in-use facts",
                        )
                        await expect_focus(
                            action_selector(settings, profile_id, "export")
                        )
                        receipt = next(
                            w
                            for w in settings.query(Static)
                            if w.display
                            and str(w.renderable) == settings._tool_profiles_result
                        )
                        region, clip = app.screen._compositor.visible_widgets[receipt]
                        assert region.intersection(clip) == region
                        assert store.path.read_bytes() == imported_bytes
                        await capture(stem + "-refused")
                    await category("Theme", "theme")
                    await category("Tool Profiles", "tool-profiles")
                    await wait_for(
                        lambda settings=settings, profile_id=profile_id: (
                            settings.query_one(ToolProfilesPanel)
                            .row(profile_id)
                            .removal_eligible
                        ),
                        "Released lease permits review",
                    )
                    await press(action_selector(settings, profile_id, "remove"))
                    await wait_for(
                        lambda: isinstance(app.screen, ConfirmationDialog),
                        "Retry review",
                    )
                    await focus("#confirm-button")
                    await capture(stem + "-retry-review")
                    await press("#confirm-button")
                    await wait_for(
                        lambda settings=settings, profile_id=profile_id: (
                            app.screen is settings
                    ...
                    region, clip = app.screen._compositor.visible_widgets[receipt]
                    assert region.intersection(clip) == region
                    tombstone = store.read_snapshot_strict().payload["profiles"][
                        profile_id
                    ]
                    ...
                    result["cells"].append(
                        {
                            ...
                            "refusal_policy_bytes_unchanged": True,
                            "refreshed_in_use_facts": True,
                            "visible_refusal_and_continuation": True,
                            "explicit_retry_after_lease_release": True,
