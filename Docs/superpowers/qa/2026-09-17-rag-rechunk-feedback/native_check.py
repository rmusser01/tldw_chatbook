"""Native Re-chunk feedback over real private SQLite media and chunk rows.
        ...
        try:
            ...
            for theme in ("textual-dark", "textual-light"):
                ...
                for size in ((170, 48), (80, 24)):
                    await wait_for(
                        lambda: not screen.query("Toast"), "prior notifications expired"
                    )
                    await tmux(
                        "resize-window",
                        "-t",
                    ...
                    try:
                        ...
                        for selector in (
                            ...
                            "#library-rag-scope-toggle-media",
                        ):
                            old_button = screen.query_one(selector, Button)
                            await press(selector)
                            await wait_for(
                                lambda selector=selector, old_button=old_button: (
                                    screen.query_one(selector, Button) is not old_button
                                    and screen.focused
                                    is screen.query_one(selector, Button)
                                ),
                                "toggle rebuilt",
                            )
                            await settle()
                            ...
                            assert screen.query_one(receipt, Static).display
                    ...
                    await wait_for(
                        lambda: not screen.query("Toast"), "completion toast expired"
                    )
                    ...
                    result["cells"].append(
                        {
                            ...
                            "legacy_after": 1,
                            "summary": line,
                            "running_feedback_survives_mode_and_scope": True,
                            "receipt_survives_mode_change": True,
