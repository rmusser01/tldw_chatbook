"""PR2707 closeout: native Settings and MCP visual replay on integrated source."""
...
def main():
    ...
    result = {
        ...
        "cells": [],
        "fixture_scope": "Real TldwCli and private config/service. Production MCP navigation, keyboard Servers/built-in selection, direct focus of first gate followed by real Tab traversal, and real deep-research gate save/reversal. Adds real Settings category navigation and focused Console budget and Tool Profiles import captures without changing those settings. No tool execution, remote server, permission mutation, recovery dispatch, or broader gate-save concurrency qualification.",
    }
        ...
        try:
            ...
            for theme in ("textual-dark", "textual-light"):
                for width, height in ((80, 24), (170, 48)):
                    ...
                    await wait_for(
                        lambda: app.screen.active_category == "console-behavior",
                        "Console category",
                    )
                    budget = app.screen.query_one(
                        "#settings-console-agent-max-steps", Input
                    )
                    budget.focus()
                    await settle()
                    visible(app.screen, budget)
                    await capture(stem + "-console-settings")
                    await pilot.press("escape", "/", *"Tool Profiles", "enter")
                    await wait_for(
                        lambda: app.screen.active_category == "tool-profiles",
                        "Tool Profiles category",
                    )
                    await wait_for(
                        lambda: bool(app.screen.query("#tool-profiles-import")),
                        "Import control",
                    )
                    app.screen.query_one("#tool-profiles-import", Button).focus()
                    await settle()
                    assert_label(app.screen.query_one("#tool-profiles-import", Button))
                    await capture(stem + "-tool-profiles")
                    await app.handle_screen_navigation(NavigateToScreen("mcp"))
                    await wait_for(
                        lambda: getattr(app.screen, "screen_name", None) == "mcp",
                        "MCP return",
