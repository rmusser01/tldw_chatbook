...
Authored now: local media, reading progress, reading highlights, and direct local ingest executions are authored locally unless the user is in server mode; server reading progress, reading highlights, web-content ingest, ingestion sources, and ingest jobs are authored on the server. Viewed now: media libraries, reading state, and selected-record highlights are shown from one source at a time. Local: Chatbook must keep local media CRUD, reading progress, highlights, and local ingest execution. Remote: Chatbook must support server reading, item, highlight, web-content ingest, source, and ingest-job flows when connected. Offline: local media and reading continue; server-backed jobs and server libraries are unavailable. Mixed view: deferred.
