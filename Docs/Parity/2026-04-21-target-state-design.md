...
Authored now: local websearch launches are authored locally in local mode, while the server-side research-provider contract is authored on the server in server mode. Viewed now: local websearch providers and server provider-backed research actions are shown from the selected source through one source-scoped catalog rather than a mixed provider list. Local: Chatbook supports local websearch through the existing WebSearch_APIs runner; local paper-search parity and provider-configuration CRUD remain deferred. Remote: Chatbook must discover, configure, trigger, and observe server provider surfaces when connected as that contract stabilizes. Offline: local websearch can continue, but server provider surfaces are unavailable. Mixed view: deferred. Confidence note: the server provider contract remains lower-confidence than the core dual-backend and strong local-first domains.
