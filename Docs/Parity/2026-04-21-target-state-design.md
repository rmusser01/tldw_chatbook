...
Authored now: local media, reading progress, reading highlights, and direct local ingest executions are authored locally unless the user is in server mode; server reading progress, reading highlights, reading export, reading summarize, reading TTS, web-content ingest, ingestion sources, and ingest jobs are authored on the server. Viewed now: media libraries, reading state, selected-record highlights, and server-generated reading action results are shown from one source at a time. Local: Chatbook must keep local media CRUD, reading progress, highlights, and local ingest execution. Remote: Chatbook must support server reading, item, highlight, export, summarize, TTS, web-content ingest, source, and ingest-job flows when connected. Offline: local media and reading continue; server-backed jobs, action generation, and server libraries are unavailable. Mixed view: deferred.
...
Authored now: research sessions and runs are authored locally in local mode, and server research sessions and runs are authored only in server mode. Viewed now: session lists, run status, and bundles are viewed from one source at a time. Local: Chatbook now has the first standalone local research-session slice with persisted run records, status control, artifacts, and bundle retrieval; a real local autonomous research execution engine remains future work. Remote: Chatbook now has typed server research-run create/list/detail/control, bundle/artifact retrieval, and checkpoint approval seams, with live event streaming and richer checkpoint/artifact UI still pending. Offline: local research records and artifacts remain available; server research runs are unavailable. Mixed view: deferred.
