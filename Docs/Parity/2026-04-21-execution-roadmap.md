...
- Current implementation note: source-aware backend foundations are now landed for chat conversation metadata/history, character/persona chat-session administration, media/reading/ingestion, watchlists, study/evaluations, RAG/admin, research sessions/runs, and prompts/chatbooks. Chat now has local/server conversation list/detail/update/tree routing, server messages-with-context/citations wrappers, local create/delete, explicit server unsupported boundaries for missing first-class conversation create/delete endpoints, and explicit local unsupported boundaries for RAG-context adjuncts. Character/persona now has server wrappers and scope routing for character/persona chat-session CRUD/restore, per-chat settings, chat export, author-note info, and lorebook diagnostics in addition to persona profiles, greetings, and presets. Media ingestion now includes local queued source/job persistence, and direct server adapters for these active parity rows now participate in runtime-policy hard stops. Remaining tranche work is primarily UI adoption, broader endpoint coverage, and honest unsupported-boundary reduction rather than first-pass service construction.
