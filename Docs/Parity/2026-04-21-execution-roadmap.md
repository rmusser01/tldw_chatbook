...
- Study Core helper update: source-aware deck update/card detail now work locally and remotely where backed, and server flashcard tags, analytics, review-session listing, assistant, scheduling reset, generation, and template CRUD helpers are policy-gated through explicit action IDs with local/offline unsupported reporting.
- Flashcards service update: Chatbook now has typed wrappers for the broader server flashcards REST surface, and the Study service seam now adopts source-aware server deck update, card detail, scheduling reset, tags, analytics, review-session listing, assistant, generation, and template CRUD helpers. Assets, bulk mutation adoption, structured imports, JSON/APKG import, export, UX, and sync/mirror semantics remain follow-on work.
