...
4. `Media / Reading / Ingestion Sources`
   The approved `Read-it-later` follow-up is landed for the current server contract, and Chatbook now has typed server reading URL-save, typed server unified items list/detail/bulk mutation, typed server backing-media list/search/keyword/trash/metadata/identifier seams, typed server backing-media item lifecycle/keyword/file/reprocess seams, typed server process-only video/audio/PDF/ebook/document/code/email controls plus transcription-model discovery, typed server document-version list/detail/create/delete/rollback/metadata/advanced-upsert seams, typed server ingest-job submit/status/list/cancel/event-stream seams, typed server web-content ingest helpers with first-class server ingest controls, local/server reading-highlight CRUD seams with first-class viewer authoring controls, server reading saved-search CRUD, server reading-item note-link CRUD, server reading import submit/list/detail, server reading archive creation, server reading export bytes, server reading summarize responses, server reading TTS bytes, server Document Workspace outline/figure/navigation/reference retrieval, ingestion-source item reattach, a server job tab with last-batch refresh/cancel, selected-job cancel, selected-batch live watch, recent visible server-job live watch, known-batch lookup by ID, and a remote-only Web Clipper tab for save/status/enrichment. Remaining work is true historical batch discovery, which still needs a server-side batch-discovery or recent-batches contract, server clip browse/history if desired, digest scheduling when workflows are back in scope, local save-by-URL, local unified aggregation, local Document Workspace/navigation backing, plus any future server contract change for per-media-type saved views.
   Current web ingestion coverage includes both the modern `/api/v1/media/ingest-web-content` helper and the legacy `/api/v1/media/process-web-scraping` process route.
   Current MediaWiki dump coverage includes both `/api/v1/media/mediawiki/process-dump` ephemeral page streaming and `/api/v1/media/mediawiki/ingest-dump` persisted ingest event streaming.
   Current direct persistent ingest coverage includes `/api/v1/media/add` for immediate server media item creation from URLs and uploads.
   Current reading URL-save coverage includes `/api/v1/reading/save` for server-side URL saves into the reading list.
   Current unified-items coverage includes `/api/v1/items`, `/api/v1/items/{item_id}`, and `/api/v1/items/bulk` for server-side cross-origin item aggregation and bulk mutation.
5. `Prompts / Chatbooks`
