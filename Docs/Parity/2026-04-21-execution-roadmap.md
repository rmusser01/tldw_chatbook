...
`Chat` now has a backend-only source-aware conversation scope seam. Chatbook wires local conversation metadata/history and server conversation list/detail/update/tree operations behind `chat.list/detail/update.{local,server}` policy actions, preserving local workspace scope filtering and avoiding any chat UI rewrite. Remaining Chat work is remote launch/message execution, delete parity, attachment/export identity, and mounted UX for explicit local/server conversation browsing.
