...
1. `Media / Reading / Ingestion Sources` plus `Collections: Reading List / Read-it-later`
   This is the cleanest next crosswalk because Chatbook already has landed local saved-state persistence, server URL save, server saved-search CRUD, server note links, server reading bulk update/archive/summary wrappers, server save/remove compatibility, the aggregate `All Media` saved view, server ingestion-source create for `archive_snapshot` and `git_repository`, server ingestion-source item reattach, and local queued ingestion-source/job persistence; local saved-search/note-link storage, local archive/summary parity, local ingestion-source item reattach, server TTS streaming wrappers, per-media-type server saved views, actual local execution, and sync/mirror semantics remain deferred.
