...
- Keep `Audio / Speech Services` source-separated: use the landed REST client wrappers for connected-server TTS/STT discovery, generation, jobs, history, transcription, tokenizer encode/decode, and custom voice management while leaving websocket speech/chat, audiobook, and admin job surfaces as separate contract slices.
