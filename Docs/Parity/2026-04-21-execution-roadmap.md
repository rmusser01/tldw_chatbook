...
`Research Sessions / Runs` now has a source-separated surface plus live server event observation. Chatbook has typed research-runs client methods, local run/artifact persistence, local/server scope routing, typed server SSE event streaming, a selected-run `Watch Events` action in the Research screen, and app navigation wiring. Remaining Research work is no longer "no surface"; it is richer checkpoint/artifact/bundle/event-log UX and actual local autonomous research execution.
...
- `Research sessions parity`: Local/server run CRUD, control, and selected-run live server event observation are landed. Follow-on work should add checkpoint/artifact/bundle/event-log UI depth and a local autonomous research execution engine.
