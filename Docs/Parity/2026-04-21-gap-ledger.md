...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Full CRUD
- Current state: Local chat is already strong and mode-aware, and Chatbook now has source-aware chat conversation seams over local metadata/history/delete plus the existing server conversation list/detail/update/tree client contract. Chatbook also has typed server chat-loop start/event/approve/reject/cancel client methods, server conversation share-link methods, public share-token resolution, server RAG-context persistence/readback, messages-with-context, and conversation citation retrieval behind backend services and runtime-policy scope seams. The app wires these backend services without changing the mounted chat UI.
- Gap: Mounted remote chat launch/message handoff, remote delete pending a server conversation-delete contract, attachments/export identity, and richer mounted source selection remain follow-on work. The current backend seams close the prior absence of central source-separated conversation metadata routing, remote chat-loop control, server share-link identity, and server RAG/citation adjunct coverage, but the chat UI still needs a later UX pass for explicit local/server conversation browsing and execution.
...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Full CRUD
- Current state: CCP is mature locally, and Chatbook now has typed server character/persona catalog CRUD, persona exemplar CRUD/review/import, preset/greeting helpers, server character chat session CRUD/settings, server character chat message CRUD/search, and server character memory CRUD/archive/extract seams behind a source-aware `CharacterPersonaScopeService` with runtime-policy action IDs. App wiring now injects the policy enforcer into that scope service.
- Gap: Mounted local/server CCP UX, shared local/server identifier normalization, local service adapters for the newly centralized session/message/memory paths, and richer character import/export/world-book parity remain follow-on work. The prior absence of server session/message/memory client and policy seams is closed.
- Evidence: Server: `../tldw_server/tldw_Server_API/app/api/v1/endpoints/characters_endpoint.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/persona.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/character_chat_sessions.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/character_messages.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/character_memory.py`; Chatbook: `tldw_chatbook/tldw_api/character_persona_schemas.py`, `tldw_chatbook/tldw_api/client.py`, `tldw_chatbook/Character_Chat/server_character_persona_service.py`, `tldw_chatbook/Character_Chat/character_persona_scope_service.py`, `tldw_chatbook/UI/Screens/ccp_screen.py`, `tldw_chatbook/Widgets/CCP_Widgets/`; Verification: `Tests/tldw_api/test_character_persona_client.py`, `Tests/Character_Chat/test_character_persona_scope_service.py`, `Tests/RuntimePolicy/test_runtime_policy_core.py`, and CCP handler/screen tests cover the current server contract and existing mounted paths.
- Recommended tranche: Tranche 1
- Notes: Priority 73. This is now a backend-contract parity foundation; continue this row through UX/source switching, local adapter normalization, world-book/import-export parity, and shared identity design rather than re-opening raw route coverage.
...
- Client obligation: Full CRUD
- Current state: Chatbook now has a first dedicated server outputs surface. Typed output-template and output-artifact client methods are landed, a server-only outputs service plus policy-aware scope seam are wired at the app level, and `Tools & Settings` now exposes an `Outputs` panel for template list/create/get/update/delete/preview plus artifact list/get/create/update/delete in explicit server mode.
- Gap: Local outputs parity remains intentionally unimplemented, and the first slice still stops short of richer file-download/export UX, explicit render-job history separate from artifact browse, and deeper integration with workspace artifacts or later sync/mirror semantics.
...
- Notes: Priority 39. The first server-control-plane slice is now landed; keep follow-on work focused on richer server export/download ergonomics and optional later local parity rather than inventing mixed-source behavior.

### Evaluations: Evaluation CRUD, RAG evals, embedding A/B tests, and run history
- Requirement class: Local parity required + Remote parity required
- Client obligation: Full CRUD
- Current state: Chatbook already has dual-backend evaluation services and screen scope support for core evaluation, dataset, and run CRUD. It now also has typed server-only synthetic evaluation draft seams for draft generation, queue listing, review actions, and promotion into datasets; embedding A/B test create/run/summary/results/significance seams; non-launch benchmark discovery plus recipe manifest/readiness/dataset-validation seams; RAG pipeline preset save/list/get/delete plus ephemeral cleanup seams; and evaluation webhook register/list/unregister/test seams through the shared API client, server evaluations service, and central scope guard.
- Gap: Contract normalization and full mutation parity still need finishing work, especially recipe-run launch/report routes, benchmark run launch, mounted UX, and any local equivalent needed for synthetic draft or embedding A/B workflows.
- Evidence: Server: `../tldw_server/tldw_Server_API/app/api/v1/endpoints/evaluations/evaluations_unified.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/evaluations/evaluations_rag_pipeline.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/evaluations/evaluations_synthetic.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/evaluations/evaluations_embeddings_abtest.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/evaluations/evaluations_benchmarks.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/evaluations/evaluations_recipes.py`; Chatbook: `tldw_chatbook/tldw_api/evaluations_schemas.py`, `tldw_chatbook/tldw_api/client.py`, `tldw_chatbook/Evaluations_Interop/local_evaluations_service.py`, `tldw_chatbook/Evaluations_Interop/server_evaluations_service.py`, `tldw_chatbook/Evaluations_Interop/evaluation_scope_service.py`, `tldw_chatbook/UI/Screens/evals_screen.py`; Verification: `Tests/tldw_api/test_evaluations_client.py` and `Tests/Evaluations_Interop/test_evaluation_scope_service.py`.
- Recommended tranche: Tranche 1
