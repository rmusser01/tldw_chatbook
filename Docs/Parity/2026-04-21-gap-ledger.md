...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Full CRUD
- Current state: Chatbook already has dual-backend evaluation services and screen scope support for core evaluation, dataset, and run CRUD. It now also has typed server-only synthetic evaluation draft seams for draft generation, queue listing, review actions, and promotion into datasets; embedding A/B test create/run/summary/results/significance seams; non-launch benchmark discovery plus recipe manifest/readiness/dataset-validation seams; and RAG pipeline preset save/list/get/delete plus ephemeral cleanup seams through the shared API client, server evaluations service, and central scope guard.
- Gap: Contract normalization and full mutation parity still need finishing work, especially recipe-run launch/report routes, benchmark run launch, evaluation webhooks, mounted UX, and any local equivalent needed for synthetic draft or embedding A/B workflows.
