...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Full CRUD
- Current state: Study seams are already strong across local and server modes, including quizzes and study-guide generation. Chatbook now also has typed server flashcard deck update, flashcard detail, scheduling reset, tag get/set, analytics-summary, and flashcard-template CRUD route coverage through the shared API client, `ServerStudyService`, `StudyScopeService`, and the runtime-policy registry.
- Gap: Remaining flashcard route-family gaps include richer server assets, bulk update/import/export, study-assistant helpers, tag suggestions, mounted affordance depth, and local equivalents for the newly exposed server-only management helpers where required. Workspace-aware remote alignment still needs cleanup.
