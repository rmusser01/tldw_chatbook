...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Full CRUD
- Current state: Study seams are already strong across local and server modes, including quizzes and study-guide generation. Chatbook now also has typed server flashcard deck update, card detail, bulk create/update, scheduling reset, tag get/set, tag suggestions, structured import preview, TSV import, byte export, analytics-summary, and flashcard-template CRUD route coverage through the shared API client, `ServerStudyService`, `StudyScopeService`, and the runtime-policy registry.
- Gap: Remaining flashcard route-family gaps include richer server assets, multipart JSON/APKG import variants, APKG asset-aware export handling, study-assistant helpers, mounted affordance depth, and local equivalents for the newly exposed server-only management helpers where required. Workspace-aware remote alignment still needs cleanup.
