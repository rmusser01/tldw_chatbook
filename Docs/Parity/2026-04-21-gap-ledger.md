...
- Gap: Local saved-search/note-link storage, local archive/summary/export/TTS parity, per-media-type server saved views, chunk-level TTS playback adoption, and any sync/mirror semantics are still deferred for this user-priority standalone collection surface.
