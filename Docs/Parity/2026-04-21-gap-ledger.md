...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Full CRUD
- Current state: Chatbook already has dual-backend evaluation services and screen scope support for core evaluation, dataset, and run CRUD. It now also has typed server-only synthetic evaluation draft seams for draft generation, queue listing, review actions, and promotion into datasets; embedding A/B test create/run/summary/results/significance seams; and non-launch benchmark discovery plus recipe manifest/readiness/dataset-validation seams through the shared API client, server evaluations service, and central scope guard.
- Gap: Contract normalization and full mutation parity still need finishing work, especially recipe-run launch/report routes, benchmark run launch, RAG-pipeline helper routes, webhooks, mounted UX, and any local equivalent needed for synthetic draft or embedding A/B workflows.
