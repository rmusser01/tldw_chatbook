...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Discover / Configure / Trigger / Observe
- Current state: Local and server admin seams are already present for chunking templates and embedding collections. Chatbook now also has typed server media-embedding status, generate, batch-generate, search, delete, job detail, and job list route coverage through the shared client, server RAG admin service, and source-aware scope service. Local template and collection operations remain Chatbook-owned, while server-only media embedding job controls require explicit server mode.
- Gap: Remaining work is mounted operator UX, local equivalents for media-embedding job orchestration if required, source-labeling polish, and route coverage for advanced template validate/match/learn helpers.
