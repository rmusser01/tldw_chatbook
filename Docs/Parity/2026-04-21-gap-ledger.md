...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Discover / Configure / Trigger / Observe
- Current state: Local and server admin seams are already present for chunking templates and embedding collections. Chatbook now also has typed server media-embedding status, generate, batch-generate, search, delete, job detail, and job list route coverage plus server chunking-template validate, match, and learn helper coverage through the shared client, server RAG admin service, and source-aware scope service. Local template and collection operations remain Chatbook-owned, while server-only media embedding job controls and server-only template helper controls require explicit server mode.
- Gap: Remaining work is mounted operator UX, local equivalents for media-embedding job orchestration if required, source-labeling polish, and any remaining media reprocess admin controls not already covered elsewhere.
