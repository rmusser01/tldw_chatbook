...
- Requirement class: Local parity required + Remote parity required
- Client obligation: Full CRUD
- Current state: Local chat is already strong and mode-aware, and Chatbook now has source-aware chat conversation seams over local metadata/history/delete plus the existing server conversation list/detail/update/tree client contract. Chatbook also has typed server chat-loop start/event/approve/reject/cancel client methods and a server-only runtime-policy scope seam for remote chat execution control. The app wires these backend services without changing the mounted chat UI.
- Gap: Mounted remote chat launch/message handoff, remote delete pending a server conversation-delete contract, attachments/export identity, and richer mounted source selection remain follow-on work. The current backend seams close the prior absence of central source-separated conversation metadata routing and remote chat-loop control, but the chat UI still needs a later UX pass for explicit local/server conversation browsing and execution.
- Evidence: Server: `../tldw_server/tldw_Server_API/app/api/v1/endpoints/chat.py`, `../tldw_server/tldw_Server_API/app/api/v1/endpoints/chat_loop.py`; Chatbook: `tldw_chatbook/tldw_api/client.py`, `tldw_chatbook/tldw_api/chat_loop_schemas.py`, `tldw_chatbook/Chat/chat_conversation_service.py`, `tldw_chatbook/Chat/server_chat_conversation_service.py`, `tldw_chatbook/Chat/chat_conversation_scope_service.py`, `tldw_chatbook/Chat/server_chat_loop_service.py`, `tldw_chatbook/Chat/chat_loop_scope_service.py`, `tldw_chatbook/UI/Chat_Window_Enhanced_Refactored.py`, `tldw_chatbook/UI/Screens/chat_screen_state.py`; Verification: `Tests/tldw_api/test_chat_conversation_client.py`, `Tests/tldw_api/test_chat_loop_client.py`, `Tests/Chat/test_chat_conversation_service.py`, `Tests/Chat/test_chat_conversation_scope_service.py`, `Tests/Chat/test_server_chat_loop_service.py`, and `Tests/UI/test_screen_navigation.py`.
