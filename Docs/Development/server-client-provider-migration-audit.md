...
| --- | ---: | --- |
| `tldw_chatbook/Auth_Account_Interop/server_auth_account_service.py` | 52 | Migrated app wiring; compatibility factory still calls the legacy config builder. Semantic match: `client=build_runtime_api_client_from_config(app_config),`. |
| `tldw_chatbook/Server_Runtime_Interop/server_runtime_service.py` | 34 | Migrated app wiring; compatibility factory still calls the legacy config builder. Semantic match: `client=build_runtime_api_client_from_config(app_config),`. |
| `tldw_chatbook/Chat/server_chat_conversation_service.py` | 41 | Migrated app wiring; compatibility factory still calls the legacy config builder. Semantic match: `client=build_runtime_api_client_from_config(app_config),`. |
| `tldw_chatbook/Chat/server_chat_loop_service.py` | 25 | Partially migrated service seam; no app wiring call site in this tranche, and the compatibility factory still calls the legacy config builder. Semantic match: `return cls(client=build_runtime_api_client_from_config(app_config))`. |
| `tldw_chatbook/Character_Chat/server_character_persona_service.py` | 53 | Migrated app wiring; compatibility factory still calls the legacy config builder. Semantic match: `client=build_runtime_api_client_from_config(app_config),`. |
| `tldw_chatbook/Character_Chat/server_chat_dictionary_service.py` | 34 | Migrated app wiring; compatibility factory still calls the legacy config builder. Semantic match: `build_runtime_api_client(app_config=app_config),`. |
