...
- Original production commit: `5655c4820733d24754f872c21cc6196d83bf1504`; the refresh and review follow-up below retain their separate source identities. No production files changed for this qualification.
...
The original harness passed Ruff and formatter checks, compiled, and added no whitespace errors. The [original source manifest](artifacts/buddy-v1-32108/source-manifest.json) binds the selected production files and then-current harness to that historical evidence; it is not the hash of the revised harness below.
...
### PR2536 review follow-up

All six [Qodo findings](https://github.com/rmusser01/tldw_chatbook/pull/2536#issuecomment-5596537996) were checked against the harness and production code. They identify qualification defects or omissions, with no production change required.

| Finding | Disposition and evidence |
| --- | --- |
| Capture output escapes safe roots | Fixed. The shared path validator confines ordinary captures to the disposable fixture and optional exports to `tempfile.gettempdir()/chatbook-buddy-v1-qualification-32108`. Every SVG/JSON destination and hash read is also validated. Absolute, traversal, directory-symlink and leaf-symlink escapes are rejected; relative and absolute paths within the allowed root remain supported. Hash receipts enumerate only captures generated by this run. |
| Failed upgrade setup retains a database handle | Fixed with `try/finally`. A real-SQLite regression activates the legacy graph, raises, then proves the retained handle is closed. The predecessor also verifies schema69 and absence of the independent Buddy tables before current application startup. |
| Schema checks bypass transactions | Fixed. Both predecessor and current-version assertions use the shared transaction context and parameterized queries. The failure-cleanup regression deliberately probes an already-closed raw handle; a new transaction would reopen a connection instead of testing release. |
| Harness arguments lack documentation | Fixed. The parameterized journey and added regression tests document injected fixtures and scenario inputs with Google-style `Args` sections. |
| Workspace clearing was not exercised | Confirmed: a strengthened fresh journey reproduced `#unchanged` instead of `#none`. The revised journey creates a real Persona default, waits for follow-target events, selects None, and verifies persisted clearing plus `assistant_defaults_explicit_none` changing from false to true. |
| Dynamic mode accepted partial animation | Fixed. The mounted check waits up to eight seconds for every ordered frame and the return to zero. Six acceptance guards reject partial, skipped, stopped and reordered sequences while accepting complete loops. Static mode stays at frame zero for one second. |

The revised capture file passed **16 targeted tests**: fourteen harness guards and two mounted profile journeys. The [follow-up verification manifest](artifacts/buddy-v1-32108/qodo-followup/verification.json) records the exact command, result, unchanged production hashes, dirty harness hash, review identifiers, and raw reproduction log identity; the [retained test log](artifacts/buddy-v1-32108/qodo-followup/tests.txt) contains the result and existing dependency/deprecation warnings. This is a fresh gate, not an addition of repeated journeys to the historical 177-test count. Ruff, formatter, compile and whitespace checks cover the revised harness.

Independent follow-up review found that validating only the directory still allowed a pre-existing SVG/JSON leaf symlink to redirect a write, and the old glob could hash unrelated matching links. Three regressions now reject both write targets, exclude an unrelated matching dangling link, and reject a generated leaf replaced before hashing. A disposable negative-control plugin restored the old operations and failed all three tests; its raw log and hash are included in the manifest. Validation here addresses pre-existing path escapes; this test helper is not a filesystem race-security boundary against concurrent hostile replacement.

Reproduce with `TLDW_BUDDY_QUALIFICATION_OUTPUT=pr2536-qodo-leaf-followup-20260909` and the manifest's pytest command. The value now resolves under the fixed disposable export root; the former arbitrary absolute destination examples above describe historical runs. Without the variable, exports remain under each test's `tmp_path`.

Both [fresh](artifacts/buddy-v1-32108/qodo-followup/fresh-evidence.json) and [upgraded](artifacts/buddy-v1-32108/qodo-followup/upgrade-evidence.json) receipts record `0→1→2→3→0`, an initially assigned workspace Persona, its subsequent absence, and the durable explicit-None flag. The Persona-count assertion covers artwork selection before the test deliberately creates a workspace Persona. Original receipts, screenshots, hashes and earlier logs remain unchanged. Their `workspace_explicit_none` field cannot establish clearing an existing assignment, and the old animation assertion did not enforce the full recorded loop.

The separate follow-up bundle includes [fresh workspace management](artifacts/buddy-v1-32108/qodo-followup/fresh-workspace-management.svg), [upgraded workspace management](artifacts/buddy-v1-32108/qodo-followup/upgrade-workspace-management.svg), [dynamic Home](artifacts/buddy-v1-32108/qodo-followup/fresh-dynamic-home.svg), and [settled upgraded inbox](artifacts/buddy-v1-32108/qodo-followup/upgrade-workspace-inbox.svg). The Persona control is scrolled into view before capture. Derived PNG inspection uses the same CairoSVG/Menlo rendering method described below. This remains headless rendered evidence, with no physical terminal, pointer, provider or audio claim.
...
| Upgrade | Follow-up explicitly checks schema69 and absence of Buddy tables, closes the legacy fixture, then boots the current application and checks schema70 plus graph equality | Synthetic predecessor-schema fixture using current migration code, not an archived old executable or a real user's historical installation. Legacy fixture graph preservation is checked; actual bytes/copy/notices/restart behavior is additionally covered by library tests. |
| Static/Dynamic | Original samples recorded a full loop but enforced only multiple frames. Follow-up requires the complete0→1→2→3→0 loop within eight seconds and Static frame0 for one second | Visible idle-state loop works in both profiles. No assertion that every listening/thinking/speaking/error artwork sequence was exercised. |
...
| Workspace and explicit None | Original captures only preserved an already-empty default. Follow-up clears a populated default through the settled manager and checks the durable explicit-None flag, then opens the inbox with Console active session unchanged | Actual Persona-default clearing and empty-workspace projection. No existing chat is moved simply by switching the Buddy's follow target. |
