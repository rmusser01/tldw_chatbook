...
[Chatterbox qualification](chatterbox/README.md) adds 56 verified records, a corrected wheel containing TASK-32494, seven full-content passes with Whisper medium, and the unchanged Whisper-small 6/7 discrepancy. Its final process/GPU/audio observations are empty and the user configuration/default sink are unchanged. These receipts qualify base `8ab21ecaf3` plus the documented fix; later rebased source requires new runtime evidence. Human listening confirmation and final review remain pending. Backlog completion must preserve these limits. ADR required: no; existing ADR-023, speech-settings ADR-039 and Speech Lab ADR-040 apply.
