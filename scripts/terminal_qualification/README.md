...
These Windows paths have host-independent behavioral/source coverage and ran
in the retained native Windows x64 CPython 3.11 qualification row. An
unavailable optional shell is recorded as unavailable; another shell is never
substituted. The remaining Windows architecture/interpreter rows have not run,
and the retained dependency decision remains fail closed because two mandatory
native terminal rows failed.
