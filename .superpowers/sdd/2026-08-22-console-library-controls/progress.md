...
- Task 14 fix round 2/5: one preparation now stages exact conversation/USER/assistant owners across real persistence Retry; a mandatory canonical SHA-256 plan fingerprint authenticates all immutable DB/effect inputs and rejects unsupported contributions; concurrent in-flight refusal cannot steal commit ownership; and queued claims bind exact session/entry/preparation authority across return, reclaim, detached settlement, and replay. Initial RED was 19 failed/1 passed plus one independently RED concurrency ratchet; final prior Task14 was 46 passed, round 2 was 21 passed, exact Task13 was 628 passed, companions were 100 passed, and the exact four-node DB/state command was 126 passed, each with the inherited Requests warning. Four restored mutations plus scoped Ruff/format/privacy/diff checks passed. TASK-19900.3 remains In Progress with all 22 unchecked; Task15 still owns restart and terminal checkpoint reconciliation.
- Task 14 fix round 3/5: a frozen tokenized reservation now claims the exact preparation/session/staged message owner under the preparation RLock before arbitrary fingerprint callbacks; foreign callers cannot canonicalize, mutate preparation/DB state, or clear the owner, while the exact owner alone installs the full digest or clears/pauses on failure. Baseline Task14 was 67; RED was 2 failed/1 control passed; final Task14 was 70, exact Task13 628, companions 100, and DB/state 126 passed with only the inherited Requests warning. The reservation-after-digest mutant failed both caller-order ratchets and was restored. TASK-19900.3 remains In Progress with all 22 unchecked; Task15 terminal/restart recovery remains out of scope.
- Task 14 fix round 4/5: failed reservation release and COMMITTING to PAUSED(PERSISTENCE) publication are now one exact-owner preparation-RLock critical section; foreign/closed owners do not mutate state, and Retry must explicitly CAS back to COMMITTING before reserving. Real-v45 canonicalizer and actual SQLite COMMIT races began RED at 2 failed/1 control passed; final Task14 was 73, exact Task13 628, companions 100, and DB/state 126 passed with only the inherited Requests warning. The pause-outside-lock mutant deterministically produced checkpoint_count=1 beside PAUSED and was restored; scoped Ruff/format/privacy/diff checks passed. TASK-19900.3 remains In Progress with all 22 unchecked; Task15 terminal/restart recovery remains out of scope.
- Task 14: complete (commits c4d0019..6c12e93, independent review clean after four fix rounds; 73 Task14, 628 Task13/controller, 100 queue/lifetime, and 126 DB/migration/state tests passed; no full suite or push).
- Task 15: deterministic local dispatch reconciliation, explicit exact-owner Retry/Discard, atomic terminal settlement, restart-paused queue ownership, and store-owned ephemeral recovery implemented RED-first; final focused 53, Task14 73, Task13 628, queue/UI 95, and repository/state 126 passed with only the inherited Requests warning. Eleven required mutation cases plus settlement fault boundaries were killed/restored; scoped Ruff/format/privacy/source/diff checks passed. Ruling: Task15 replaces only Task14's transitional post-terminal checkpoint seam (cost if wrong: restore historical tests or strand false recovery); TASK-19900.3 remains In Progress with all 22 unchecked and Task16 boundaries intact.
- Task 15 fix round 1/5: explicit runtime-active vs recovery-needed truth now blocks fresh submission and mounted Send without mislabeling healthy delivery; the repository rejects every second checkpoint in the existing transaction; settlement faults restore the exact blocked owner and queue fence; Automatic evidence is re-queryable; authority accepts only new-session `None` to durable revision 1; unresolved ephemeral ownership survives ordinary replacement/close; and one owner-pinned mounted recovery region leaves the queue shelf count/pause-only. RED was 10 failures plus 6 reviewer-added failures/9 controls; final gates were Task15 68, Task14 73, Task13 628, queue/UI 98, repository/state 126, and runtime 10 passed with inherited warnings and one independently stale fixture excluded. Required plus four review-added mutants were killed/restored; Ruff, focused format, Impeccable, privacy/source, and diff checks passed. TASK-19900.3 remains In Progress with all 22 unchecked and Task16 boundaries intact.
- Task 15 fix round 2/5: every accepted postcommit interruption now publishes/restores the exact actionable owner and queued fence without completing the failed effect; production Retry authenticates and resumes the bounded live fingerprint continuation before CAS/provider, while replacement controllers fail closed. Post-CAS success/failure/cancel/local exceptions restore runtime-inactive recovery only under the complete legal checkpoint transition relation. Official RED was 14 failures/2 controls, review expansion added 3 early/CAS failures and 1 partial-relation failure; final gates were Task15 88, Task14 73, Task13 628, queue/UI 98, and repository/state 126 passed with inherited warnings only. Five mutant families were killed/restored; Ruff, focused format, privacy/source, and diff checks passed. TASK-19900.3 remains In Progress with all 22 unchecked and Task16 boundaries intact.
- Task 15 fix round 3/5: app disposal now retires live postcommit continuation/cache/preparation bodies even with no tasks; the existing evidence lease binds and then clears its exact release capability; and live Discard authenticates/resumes required effects only through preparation publication before atomic settlement, retaining recovery on prerequisite failure and refusing alien controllers. RED was 8 failures plus 1 lifecycle-expanded failure and 1 self-review release-fault failure; final gates were Task15 98, Task14 73, Task13 628, queue/UI 98, and repository/state 126 passed with inherited warnings only. Three mutants were killed/restored; Ruff/focused format/privacy/source/diff checks passed. One stale runtime fixture still raises for missing `notes_sync_runtime_owner` before Console disposal. TASK-19900.3 remains In Progress with all 22 unchecked and Task16 boundaries intact.
- Task 15 fix round 4/5: exception-safe close retirement now prevents an evidence-release callback fault from skipping exact cache/preparation/session cleanup; permanent runtime disposal clears volatile recovery/action baselines/queue-hydration fences while preserving SQLite restart truth. RED was 2 failures/1 control plus 1 hydration mutant failure; final gates were Task15 101, Task14 73, Task13 628, queue/UI 98, repository/state 126, and runtime 14 passed with inherited warnings only. Three mutants were killed/restored; Ruff/focused format/privacy/source/diff checks passed. TASK-19900.3 remains In Progress with all 22 unchecked and Task16 boundaries intact.
