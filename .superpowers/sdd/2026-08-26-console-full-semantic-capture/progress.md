...
Task 4: fix round 3/5 implemented in `b53bf54631`; evidence/backlog/progress commit `f1b8bd4816`. RED reproduced the real Textual Off path returning no mutation after misleading acknowledgement cancellation. GREEN: new + retained masked-Global-Full tests `2 passed`, dialog `14 passed`, dialog/export re-review `20 passed`, exact matrix `870 passed, 2 skipped`, 80x24 `108 passed`, Settings/config/layout `379 passed`, real sentinel `1 passed`, Task-4 delta `1 passed`, and Ruff/py_compile/CSS/docs/diff checks clean.

Task 4: fix round 3/5 scoped re-review dispatched over `e6f0bfbe86..f1b8bd4816`.

Task 4: complete (implementation `d685a90009..b53bf54631`, evidence through `f1b8bd4816`; final scoped re-review clean after 3/5 fix rounds, final exact matrix `870 passed, 2 skipped`). `TASK-22507.4` acceptance criteria checked and status set Done.

Final whole-branch review over `de580f20..d0c6ef08` found 3 Critical + 4 Important issues: fail-open conversation-policy hydration under Global Full; streamed base64 split across sub-threshold chunks; delayed Full-to-Safe runtime publication; stale runtime projection after canonical config reload; incomplete/misleading purge availability and confirmation UI; capture-policy lookup aborting an accepted send; and raw exception values at the lowest exchange DB write boundary. Two Minor UI truthfulness issues cover Inspector title/armed copy and misleading Global Inherit.

Final integration: Ruling: reopen `TASK-22507.1`, `TASK-22507.2`, and `TASK-22507.4` (leaving the independently clean purge engine `TASK-22507.3` Done) because the final review invalidates specific child acceptance criteria across privacy persistence, runtime policy, and shared UI surfaces; if wrong, this adds backlog state churn but preserves truthful task evidence instead of leaving violated ACs marked complete.

Final integration: Ruling: no new ADR is required because every correction enforces ADR-092's existing Safe-first, capture-never-breaks-send, Safe-before-disk, content-free logging, and truthful purge/UI contracts; if wrong, the risk is under-documenting a new boundary, but the fixes do not introduce a new storage, ownership, security, or cross-module policy decision.

Final integration: Ruling: when a persisted conversation policy is unavailable or corrupt under Global Full, publish explicit session Safe, retain save-pending, and keep hydration retryable; do not disable Safe capture entirely. ADR-092 defines Safe as the fail-closed diagnostic mode, while disabled capture is reserved for admission bookkeeping failure that cannot safely resolve policy. If wrong, Safe diagnostic metadata may still be retained during a repository outage, but semantic bodies cannot escalate to Full.

Final whole-branch correction: implementation `df67e53bb1`; controller-owned verification complete. RED evidence: first executable focus `17 passed, 2 failed`; new policy-revision fence `1 failed`; first generation-aware exact matrix `882 passed, 4 failed, 2 skipped`. GREEN: changed focus `422 passed, 2 skipped`; cancellation/race `8 passed`; exact privacy/UI matrix `886 passed, 2 skipped`; DB area `1831 passed, 1 skipped`; Task 2 gate `570 passed, 2 skipped`; 80x24 `114 passed`; Settings/config/layout `381 passed`; production sentinel plus Task-4 base delta `2 passed`; Ruff/py_compile/CSS five-way sync/docs inspection/diff-check clean. `chat_screen.py` remains `20,093/633` against the Task-4 base `20,099/633`; the stale absolute ratchet was not run or changed. See `final-review-fix-report.md`. Reopened `TASK-22507.1`, `.2`, and `.4` remain In Progress with affected ACs unchecked for independent re-review.

Final-review fix round 2/5: remaining Important finding reproduced because the sanitized DB error used explicit raw chaining. RED `1 failed`: `__cause__` retained the canary-bearing `OperationalError`. Implementation `873972639f` constructs the content-free outer error while handling only the exception class, leaves the active exception scope, then raises with explicit suppression; rollback and stable content-free logging are unchanged. GREEN: seam focus `1 passed`; message-exchange file `12 passed`; corrected-area cumulative `423 passed, 2 sandbox loopback skips`; DB area `1831 passed, 1 Windows-only skip`; Ruff/py_compile/diff-check clean. Reopened tasks and affected ACs remain open for independent re-review.

Final independent re-review: all original Critical/Important findings and both Minor truthfulness findings are addressed; the final DB exception-chain probe confirms clean outer/log/cause/context/recursive graph/rendered traceback with rollback preserved. Reviewer verdict: `Ready to merge? Yes`; no Critical or Important findings remain.

Final controller verification at `c5d18abc50`: fresh exact 16-file privacy/UI matrix `886 passed, 2 sandbox loopback skips`; fresh lowest DB-boundary file `12 passed`; Ruff across all changed Python files, production `py_compile`, all-five CSS bundle sync, `git diff --check`, and clean-worktree/HEAD checks passed. Incumbent pytest temporary cleanup and dependency warnings did not affect exit status.

Plan complete: `TASK-22507.1`, `.2`, `.3`, `.4`, and parent `TASK-22507` have all acceptance criteria checked, final evidence/ADR/lesson notes recorded, and status Done.

Post-rebase Qodo closeout: rebased onto current `dev` while preserving upstream v50 ownership and moving capture persistence to v51/ADR-092. Addressed all 11 actionable review findings with eight RED/GREEN regressions; retained the concrete purge-revision type and documented the separately nullable Inspector freshness seam. Final focused gate `90 passed`; exact privacy/UI matrix `890 passed, 2 sandbox-only skips`; Ruff, py_compile, CSS sync, and diff checks clean.
